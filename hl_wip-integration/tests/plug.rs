...
fn engine_add_composes_all_three_drivers_and_injects_them() {
    ...
    assert_eq!(engine.names(), vec!["cuda", "vulkan", "gl"], "names present and ordered");
    ...
    assert_eq!(reqs.len(), 3, "one request per attached driver, in registry order");
    let (cuda, vk, gl) = (&reqs[0], &reqs[1], &reqs[2]);

    // ---- CUDA (reqs[0]) --------------------------------------------------------------------------
    // device_request injects the THREE guest shim sonames at their guest paths...
    assert!(mounts_to(cuda, &format!("{LIBDIR}/libcuda.so.1")), "cuda: libcuda.so.1 bound");
    assert!(mounts_to(cuda, &format!("{LIBDIR}/libcudart.so.1")), "cuda: libcudart.so.1 bound");
    assert!(mounts_to(cuda, &format!("{LIBDIR}/libnvidia-ml.so.1")), "cuda: libnvidia-ml.so.1 bound");
    ...
    assert!(has_env(cuda, &format!("HL_GPU_EXEC={GUEST_SOCK}")), "cuda: $HL_GPU_EXEC set");
    assert!(
        has_env(cuda, &format!("LD_LIBRARY_PATH={LIBDIR}:/usr/lib:/lib")),
        "cuda: guest libdir prepended to the existing LD_LIBRARY_PATH",
    );
    assert!(has_env(cuda, "HL_CUDA_NAME=hl Integration GPU"), "cuda: HL_CUDA_NAME advertised");
    assert!(has_env(cuda, "HL_CUDA_CC=8.6"), "cuda: HL_CUDA_CC advertised");
    assert!(has_env(cuda, "HL_CUDA_VRAM_BYTES=17179869184"), "cuda: HL_CUDA_VRAM_BYTES advertised");
    assert!(!cuda.render_node, "cuda: injects libs, not a synthetic render node");

    // ---- VULKAN (reqs[1]) ------------------------------------------------------------------------
    // The ICD is bound at both its versioned + unversioned soname, plus its icd.json manifest...
    assert!(mounts_to(vk, &format!("{LIBDIR}/libvk_hl.so.1")), "vulkan: libvk_hl.so.1 bound");
    assert!(mounts_to(vk, &format!("{LIBDIR}/libvk_hl.so")), "vulkan: unversioned libvk_hl.so bound");
    assert!(mounts_to(vk, &format!("{LIBDIR}/hl_vulkan_icd.json")), "vulkan: icd.json dropped");
    ...
    assert!(has_env(vk, &format!("HL_GPU_EXEC={GUEST_SOCK}")), "vulkan: $HL_GPU_EXEC set");
    assert!(
        has_env(vk, &format!("LD_LIBRARY_PATH={LIBDIR}:/usr/lib:/lib")),
        "vulkan: guest libdir prepended to the existing LD_LIBRARY_PATH",
    );
    assert!(!vk.render_node, "vulkan: injects an ICD, not a synthetic render node");

    // ---- GL (reqs[2]) ----------------------------------------------------------------------------
    // Both GLES/EGL sonames are bound (GLESv2 is the DT_NEEDED->libEGL forwarding stub)...
    assert!(mounts_to(gl, &format!("{LIBDIR}/libEGL.so.1")), "gl: libEGL.so.1 bound");
    assert!(mounts_to(gl, &format!("{LIBDIR}/libGLESv2.so.2")), "gl: libGLESv2.so.2 bound");
    ...
    assert!(has_env(gl, &format!("HL_GPU_EXEC={GUEST_SOCK}")), "gl: $HL_GPU_EXEC set");
    assert!(
        has_env(gl, &format!("LD_LIBRARY_PATH={LIBDIR}:/usr/lib:/lib")),
        "gl: guest libdir prepended to the existing LD_LIBRARY_PATH",
    );
    assert!(has_env(gl, "HL_GL_SURFACE_W=1920"), "gl: HL_GL_SURFACE_W advertised");
    assert!(has_env(gl, "HL_GL_SURFACE_H=1080"), "gl: HL_GL_SURFACE_H advertised");
    assert!(!gl.render_node, "gl: injects shim libs, not a synthetic render node");

    // All three converge on the SAME guest exec socket path — the one host GPU-exec seam they all speak.
    for req in [cuda, vk, gl] {
        assert!(binds_exec_socket(req), "every driver binds the shared $HL_GPU_EXEC socket");
        assert!(has_env(req, &format!("HL_GPU_EXEC={GUEST_SOCK}")), "every driver names the same socket");
    }
}
