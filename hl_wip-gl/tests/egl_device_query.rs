...
fn device_query_extension_end_to_end() {
    ...
    if !egl_path.exists() {
        ...
        eprintln!("staged libEGL missing at {} — skipping (guest std not installed)", dir.display());
        return;
    }
    ...
    let egl_get_error: extern "C" fn() -> i32 = unsafe { std::mem::transmute(sym(egl, "eglGetError")) };
    ...
    assert!(client_ext.contains("EGL_EXT_device_query"), "client ext advertises device_query: {client_ext:?}");
    assert!(client_ext.contains("EGL_EXT_device_base"), "client ext advertises device_base: {client_ext:?}");
    ...
    for name in [
        ...
        assert!(!proc(egl_get_proc_address, name).is_null(), "eglGetProcAddress({name}) must resolve");
    }
    ...
    let dpy = get_platform_display_ext(EGL_PLATFORM_SURFACELESS_MESA, core::ptr::null_mut(), core::ptr::null());
    assert!(!dpy.is_null(), "eglGetPlatformDisplayEXT(SURFACELESS) returns a display");
    let (mut major, mut minor) = (0i32, 0i32);
    assert_eq!(egl_initialize(dpy, &mut major, &mut minor), EGL_TRUE, "display initializes");
    let display_ext = cstr(egl_query_string(dpy, EGL_EXTENSIONS));
    assert!(display_ext.contains("EGL_EXT_device_query"), "display ext advertises device_query: {display_ext:?}");
    assert!(display_ext.contains("EGL_EXT_device_base"), "display ext advertises device_base: {display_ext:?}");
    ...
    let mut count = -1i32;
    assert_eq!(egl_query_devices(0, core::ptr::null_mut(), &mut count), EGL_TRUE);
    assert!(count >= 1, "at least one device is reported (count={count})");
    let mut buf: [*mut c_void; 4] = [core::ptr::null_mut(); 4];
    let mut got = -1i32;
    assert_eq!(egl_query_devices(buf.len() as i32, buf.as_mut_ptr(), &mut got), EGL_TRUE);
    ...
    let mut nope = -1i32;
    assert_eq!(egl_query_devices(0, buf.as_mut_ptr(), &mut nope), EGL_FALSE, "max_devices 0 + array is rejected");
    ...
    assert_eq!(egl_query_display_attrib(dpy, EGL_DEVICE_EXT, &mut dev_attr), EGL_TRUE, "EGL_DEVICE_EXT is answered");
    assert_eq!(dev_attr as usize, device as usize, "the display's device == the enumerated device");
    ...
    assert_eq!(egl_get_error(), EGL_SUCCESS, "error register is clear before the bad query");
    ...
    assert_eq!(egl_get_error(), EGL_BAD_ATTRIBUTE, "and raises EGL_BAD_ATTRIBUTE");
    ...
    assert!(egl_query_device_string(bogus, EGL_EXTENSIONS).is_null(), "foreign device string is null");
    assert_eq!(egl_get_error(), EGL_BAD_DEVICE_EXT, "foreign device raises EGL_BAD_DEVICE_EXT");
    ...
    let mut v: isize = 0;
    assert_eq!(egl_query_device_attrib(bogus, EGL_DEVICE_EXT, &mut v), EGL_FALSE, "foreign device attrib fails");
    assert_eq!(egl_get_error(), EGL_BAD_DEVICE_EXT, "foreign device attrib raises EGL_BAD_DEVICE_EXT");
}
