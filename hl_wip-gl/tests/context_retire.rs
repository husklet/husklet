...
fn retire_all_queues_destroy_for_the_whole_working_set() {
    ...
    c.surf = GlSurface { have: true, width: 16, height: 16 };
    ...
    assert!(!c.has_pending_destroys(), "a healthy frame queues no persistent destroys");

    // Context teardown: retire the whole working set.
    c.retire_all();

    let pending = c.pending_destroys();
    assert!(!pending.is_empty(), "retire_all must queue destroys for the resident working set");
    ...
    assert!(pending.iter().all(is_destroy), "retire_all queues only Destroy* commands");
    assert!(pending.iter().any(|c| matches!(c, Cmd::DestroyBuffer(_))), "vbo retired");
    assert_eq!(
        pending.iter().filter(|c| matches!(c, Cmd::DestroyShader(_))).count(),
        2,
        "both program shader modules retired"
    );
    assert!(pending.iter().any(|c| matches!(c, Cmd::DestroyPipeline(_))), "pipeline retired");
    assert!(pending.iter().any(|c| matches!(c, Cmd::DestroyTexture(_))), "default target texture retired");
    assert!(pending.iter().any(|c| matches!(c, Cmd::DestroySurface(_))), "default surface retired");

    // A second retire is a clean no-op: the caches are already empty.
    let n = c.pending_destroys().len();
    c.retire_all();
    assert_eq!(c.pending_destroys().len(), n, "a second retire_all adds nothing (caches already empty)");
}
...
fn context_teardown_refunds_the_whole_residency_ledger() {
    ...
    assert!(resident > 0 && objects > 0, "a presented frame charges residency ({resident} B, {objects} obj)");
    ...
    c.retire_all();
    assert!(!swap::swap_buffers(&mut c, &mut sink).unwrap(), "delete-only frame presents nothing");
