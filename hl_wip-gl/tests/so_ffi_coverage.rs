...
fn sym(handle: *mut c_void, name: &str) -> *mut c_void {
    ...
    assert!(!p.is_null(), "symbol {name} not resolvable in the dlopened object");
    p
}
...
fn cstr(p: *const u8) -> String {
    assert!(!p.is_null(), "GL string getter returned null (an app dereferences it unconditionally)");
...
fn gl_identity_and_scalar_state_queries_marshal() {
    ...
    let gl_get_stringi = f!(sh.gles, "glGetStringi", extern "C" fn(u32, u32) -> *const u8);
    ...
    assert_eq!(cstr(gl_get_string(GL_SHADING_LANGUAGE_VERSION)), "OpenGL ES GLSL ES 3.00");
    assert_eq!(cstr(gl_get_string(GL_EXTENSIONS)), "", "no non-core extensions are advertised");
    ...
    assert_eq!(getint(GL_NUM_EXTENSIONS), 0, "matches the empty GL_EXTENSIONS list");
    ...
    assert!(gl_get_stringi(GL_EXTENSIONS, 0).is_null(), "no extension #0 in an empty inventory");
    assert_eq!(gl_get_error(), GL_INVALID_VALUE, "OOB glGetStringi index -> GL_INVALID_VALUE");
    assert!(gl_get_stringi(0xBEEF, 0).is_null(), "a non-GL_EXTENSIONS name is null");
    assert_eq!(gl_get_error(), GL_INVALID_ENUM, "bad glGetStringi name -> GL_INVALID_ENUM");
    ...
    assert_eq!(gl_get_error(), GL_NO_ERROR, "null-safe getters raise no error");
}
...
fn gl_indexed_queries_marshal_after_binding() {
    ...
    let gl_get_integeri_v = f!(sh.gles, "glGetIntegeri_v", extern "C" fn(u32, u32, *mut i32));
    let gl_get_integer64i_v = f!(sh.gles, "glGetInteger64i_v", extern "C" fn(u32, u32, *mut i64));
    ...
    assert_eq!(idx as u32, ubo, "indexed UBO binding at slot 2 is our buffer");
    ...
    assert_eq!(bidx, 1, "a non-zero indexed binding reads back as GLboolean 1");
...
fn gl_object_existence_predicates_marshal() {
    ...
    let gl_delete_framebuffers = f!(sh.gles, "glDeleteFramebuffers", extern "C" fn(i32, *const u32));
    ...
    let gl_delete_renderbuffers = f!(sh.gles, "glDeleteRenderbuffers", extern "C" fn(i32, *const u32));
    ...
    let gl_delete_vertex_arrays = f!(sh.gles, "glDeleteVertexArrays", extern "C" fn(i32, *const u32));
    ...
    let gl_fence_sync = f!(sh.gles, "glFenceSync", extern "C" fn(u32, u32) -> *mut c_void);
    let gl_client_wait_sync = f!(sh.gles, "glClientWaitSync", extern "C" fn(*mut c_void, u32, u64) -> u32);
    let gl_get_error = f!(sh.gles, "glGetError", extern "C" fn() -> u32);
    ...
    assert_eq!(gl_is_buffer(0xDEAD), GL_FALSE as u32, "a bogus name is not a buffer");
    ...
    assert_eq!(gl_is_query(q), GL_FALSE as u8, "a merely-reserved query name is not yet a query object");
    gl_begin_query(GL_ANY_SAMPLES_PASSED, q);
    gl_end_query(GL_ANY_SAMPLES_PASSED);
    assert_eq!(gl_is_query(q), GL_TRUE as u8, "after glBeginQuery the name is a live query object");
    ...
    assert_eq!(gl_is_sampler(sm), GL_FALSE as u8, "a merely-reserved sampler name is not yet an object");
    gl_sampler_parameteri(sm, GL_TEXTURE_MIN_FILTER_, GL_NEAREST);
    assert_eq!(gl_is_sampler(sm), GL_TRUE as u8, "after glSamplerParameteri the name is a live sampler");
    ...
    if !syn.is_null() {
        assert_eq!(gl_is_sync(syn), GL_TRUE as u8, "a live fence sync reads back as a sync");
    }
    assert_eq!(gl_is_sync(core::ptr::null_mut()), GL_FALSE as u8, "null sync is not a sync");
    assert_eq!(gl_is_sync(0xDEAD_0000usize as *mut c_void), GL_FALSE as u8, "bogus sync is not a sync");
    ...
    assert_eq!(gl_get_error(), GL_INVALID_VALUE, "and raises GL_INVALID_VALUE");
}
...
fn gl_program_reflection_marshals_real_values() {
    ...
    let gl_create_shader = f!(sh.gles, "glCreateShader", extern "C" fn(u32) -> u32);
    let gl_shader_source =
        f!(sh.gles, "glShaderSource", extern "C" fn(u32, i32, *const *const c_char, *const i32));
    ...
    let gl_get_uniform_location =
        f!(sh.gles, "glGetUniformLocation", extern "C" fn(u32, *const c_char) -> i32);
    let gl_get_attrib_location =
        f!(sh.gles, "glGetAttribLocation", extern "C" fn(u32, *const c_char) -> i32);
    ...
    let gl_get_attached_shaders =
        f!(sh.gles, "glGetAttachedShaders", extern "C" fn(u32, i32, *mut i32, *mut u32));
    let gl_get_program_info_log =
        f!(sh.gles, "glGetProgramInfoLog", extern "C" fn(u32, i32, *mut i32, *mut c_char));
    let gl_get_shader_info_log =
        f!(sh.gles, "glGetShaderInfoLog", extern "C" fn(u32, i32, *mut i32, *mut c_char));
    let gl_uniform4f = f!(sh.gles, "glUniform4f", extern "C" fn(i32, f32, f32, f32, f32));
    ...
    let gl_get_uniformuiv = f!(sh.gles, "glGetUniformuiv", extern "C" fn(u32, i32, *mut u32));
    ...
    let compile = |kind: u32, src: &str| -> u32 {
        ...
        assert_eq!(sl, src.len() as i32 + 1, "GL_SHADER_SOURCE_LENGTH counts the NUL");
        s
    };
    ...
    assert_eq!(loc(gl_get_attrib_location, "nope"), -1, "unknown attribute -> -1");
    ...
    assert_eq!(loc(gl_get_uniform_location, "uTex"), 0, "sampler location space is separate");
    ...
    let active = |func: extern "C" fn(u32, u32, i32, *mut i32, *mut i32, *mut u32, *mut c_char),
     ...
     -> (String, u32, i32, i32) {
        ...
        let mut size: i32 = -1;
        let mut ty: u32 = 0;
        func(prog, index, namebuf.len() as i32, &mut len, &mut size, &mut ty, namebuf.as_mut_ptr());
        let name = unsafe { std::ffi::CStr::from_ptr(namebuf.as_ptr()) }.to_string_lossy().into_owned();
        (name, ty, size, len)
    };
    let a0 = active(gl_get_active_attrib, 0);
    assert_eq!((a0.0.as_str(), a0.1, a0.2), ("aPos", GL_FLOAT_VEC2, 1));
    assert_eq!(a0.3, 4, "GL_ACTIVE_ATTRIB length excludes the NUL (\"aPos\" = 4)");
    ...
    let mut tlen: i32 = -1;
    gl_get_active_attrib(prog, 1, tiny.len() as i32, &mut tlen, &mut 0, &mut 0, tiny.as_mut_ptr());
    let tname = unsafe { std::ffi::CStr::from_ptr(tiny.as_ptr()) }.to_string_lossy().into_owned();
    assert_eq!(tname, "aC", "name truncated to bufSize-1 with a NUL terminator");
    assert_eq!(tlen, 2, "reported length excludes the NUL and matches the truncation");
    ...
    assert_eq!(f32::from_bits(iv as u32), 2.5, "glGetUniformiv copies the same bytes");
    let mut uiv: u32 = 0;
    ...
    assert!(got.contains(&vs) && got.contains(&fs), "attached names are vs+fs, got {got:?}");
    ...
    let vs2 = compile(GL_VERTEX_SHADER, "attribute vec2 aPos;\nvoid main(){ gl_Position = vec4(aPos,0.0,1.0); }\n");
    ...
    assert_eq!(unit, 3, "glGetUniformiv on a sampler reports its bound texture unit");
}
...
fn gl_object_state_queries_marshal_real_state() {
    ...
    let gl_get_buffer_parameteriv =
        f!(sh.gles, "glGetBufferParameteriv", extern "C" fn(u32, u32, *mut i32));
    let gl_get_buffer_parameteri64v =
        f!(sh.gles, "glGetBufferParameteri64v", extern "C" fn(u32, u32, *mut i64));
    let gl_gen_textures = f!(sh.gles, "glGenTextures", extern "C" fn(i32, *mut u32));
    ...
    let gl_get_tex_parameteriv = f!(sh.gles, "glGetTexParameteriv", extern "C" fn(u32, u32, *mut i32));
    let gl_get_tex_parameterfv = f!(sh.gles, "glGetTexParameterfv", extern "C" fn(u32, u32, *mut f32));
    let gl_tex_image2d = f!(
        sh.gles,
        "glTexImage2D",
        extern "C" fn(u32, i32, i32, i32, i32, i32, u32, u32, *const c_void)
    );
    let gl_get_tex_level_parameteriv =
        f!(sh.gles, "glGetTexLevelParameteriv", extern "C" fn(u32, i32, u32, *mut i32));
    let gl_gen_renderbuffers = f!(sh.gles, "glGenRenderbuffers", extern "C" fn(i32, *mut u32));
    let gl_bind_renderbuffer = f!(sh.gles, "glBindRenderbuffer", extern "C" fn(u32, u32));
    let gl_renderbuffer_storage =
        f!(sh.gles, "glRenderbufferStorage", extern "C" fn(u32, u32, i32, i32));
    let gl_get_renderbuffer_parameteriv =
        f!(sh.gles, "glGetRenderbufferParameteriv", extern "C" fn(u32, u32, *mut i32));
    let gl_gen_framebuffers = f!(sh.gles, "glGenFramebuffers", extern "C" fn(i32, *mut u32));
    let gl_bind_framebuffer = f!(sh.gles, "glBindFramebuffer", extern "C" fn(u32, u32));
    let gl_framebuffer_texture2d =
        f!(sh.gles, "glFramebufferTexture2D", extern "C" fn(u32, u32, u32, u32, i32));
    ...
    let bytes = [7u8; 48];
    gl_buffer_data(GL_ARRAY_BUFFER, bytes.len() as isize, bytes.as_ptr() as *const c_void, GL_STATIC_DRAW);
    let mut sz: i32 = -1;
    ...
    assert_eq!(usage as u32, GL_STATIC_DRAW, "GL_BUFFER_USAGE echoes the stored hint");
    ...
    assert_eq!(minf, GL_NEAREST, "GL_TEXTURE_MIN_FILTER reads back what was set");
    let mut magf: f32 = -1.0;
    gl_get_tex_parameterfv(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, &mut magf);
    assert_eq!(magf as i32, GL_LINEAR, "glGetTexParameterfv widens the enum to f32 exactly");

    // glGetTexLevelParameteriv: level-0 extent + internal format of a uploaded texture.
    gl_tex_image2d(GL_TEXTURE_2D, 0, GL_RGBA as i32, 64, 32, 0, GL_RGBA, GL_UNSIGNED_BYTE, core::ptr::null());
    let mut w: i32 = -1;
    let mut h: i32 = -1;
    ...
    assert_eq!(ifmt as u32, GL_RGBA8, "the model materializes every texture as RGBA8");
    ...
    assert_eq!(atype, GL_TEXTURE, "the color attachment's object type is GL_TEXTURE");
    let mut aname: i32 = -1;
    gl_get_fb_attachment_parameteriv(
        GL_FRAMEBUFFER,
        GL_COLOR_ATTACHMENT0,
...
fn gl_compressed_and_copy_textures_allocate_extent() {
    ...
    assert_eq!((cw, ch), (16, 16), "compressed upload allocated the 16x16 extent");
    ...
    assert_eq!(gl_get_error(), GL_NO_ERROR, "a valid glCopyTexImage2D raises no error");
    let mut pw: i32 = -1;
    let mut ph: i32 = -1;
    ...
    assert_eq!((pw, ph), (24, 24), "glCopyTexImage2D allocated the 24x24 destination extent");

    // A bad target/border is GL_INVALID_VALUE (the validated marshalling path).
    gl_copy_tex_image2d(GL_TEXTURE_2D, 0, GL_RGBA, 0, 0, 8, 8, 1 /* bad border */);
    assert_eq!(gl_get_error(), GL_INVALID_VALUE, "a non-zero border is rejected");
}
...
fn egl_context_surface_sync_roundtrips_marshal() {
    ...
    let egl_initialize = f!(sh.egl, "eglInitialize", extern "C" fn(*mut c_void, *mut i32, *mut i32) -> u32);
    ...
    let egl_get_current_context = f!(sh.egl, "eglGetCurrentContext", extern "C" fn() -> *mut c_void);
    let egl_get_current_display = f!(sh.egl, "eglGetCurrentDisplay", extern "C" fn() -> *mut c_void);
    let egl_get_current_surface = f!(sh.egl, "eglGetCurrentSurface", extern "C" fn(i32) -> *mut c_void);
    ...
    let egl_create_sync =
        f!(sh.egl, "eglCreateSync", extern "C" fn(*mut c_void, u32, *const isize) -> *mut c_void);
    let egl_client_wait_sync = f!(
        sh.egl,
        "eglClientWaitSync",
        extern "C" fn(*mut c_void, *mut c_void, i32, u64) -> i32
    );
    let egl_destroy_sync = f!(sh.egl, "eglDestroySync", extern "C" fn(*mut c_void, *mut c_void) -> u32);
    let egl_create_image = f!(
        sh.egl,
        "eglCreateImage",
        extern "C" fn(*mut c_void, *mut c_void, u32, *mut c_void, *const isize) -> *mut c_void
    );
    let egl_destroy_image =
        f!(sh.egl, "eglDestroyImage", extern "C" fn(*mut c_void, *mut c_void) -> u32);
    let egl_wait_client = f!(sh.egl, "eglWaitClient", extern "C" fn() -> u32);
    let egl_swap_interval = f!(sh.egl, "eglSwapInterval", extern "C" fn(*mut c_void, i32) -> u32);
    ...
    let dpy = get_platform_display(EGL_PLATFORM_SURFACELESS_MESA, core::ptr::null_mut(), core::ptr::null());
    assert!(!dpy.is_null());
    assert_eq!(egl_initialize(dpy, &mut 0, &mut 0), EGL_TRUE);

    // eglCreateContext hands back a non-null opaque EGLContext token.
    let ctx = egl_create_context(dpy, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null());
    assert!(!ctx.is_null(), "eglCreateContext returns a non-null context token");
    let surf = egl_create_pbuffer(dpy, core::ptr::null_mut(), core::ptr::null());
    assert!(!surf.is_null(), "eglCreatePbufferSurface returns a surface token");
    ...
    assert_eq!(egl_get_current_context(), ctx, "eglGetCurrentContext reports the bound context");
    assert_eq!(egl_get_current_display(), dpy, "eglGetCurrentDisplay reports the bound display");
    const EGL_DRAW: i32 = 0x3059;
    const EGL_READ: i32 = 0x305A;
    assert_eq!(egl_get_current_surface(EGL_DRAW), surf, "the draw surface round-trips");
    assert_eq!(egl_get_current_surface(EGL_READ), surf, "the read surface round-trips");
    ...
    assert_eq!(qctx(0xBEEF), 0, "an unknown attribute reads 0 and still succeeds");
    assert_eq!(egl_query_context(dpy, ctx, EGL_CONTEXT_CLIENT_TYPE, core::ptr::null_mut()), EGL_FALSE);
    ...
    assert_eq!((w, h), (640, 480), "eglQuerySurface reports the live pbuffer extent");
    assert_eq!(egl_query_surface(dpy, surf, EGL_WIDTH, core::ptr::null_mut()), EGL_FALSE);
    ...
    assert!(!sync.is_null(), "eglCreateSync returns a non-null EGLSync token");
    assert_eq!(
    ...
    let img = egl_create_image(dpy, ctx, 0x30B9 /* EGL_GL_TEXTURE_2D */, core::ptr::null_mut(), core::ptr::null());
    assert!(!img.is_null(), "eglCreateImage returns a non-null image token");
    ...
    assert_eq!(egl_make_current(dpy, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut()), EGL_TRUE);
    assert!(egl_get_current_context().is_null(), "the binding is released");
}
