...
fn program_shaders_and_pipeline_created_once_across_n_frames_then_retired_on_delete() {
    ...
    assert_eq!(total_shaders, 2, "{N} frames create the program's 2 shader modules once, not 2*N");
    assert_eq!(total_pipes, 1, "{N} frames create the program's pipeline once, not N");
    ...
    let (vs_ir, fs_ir) = {
        ...
        (ids.next().expect("vertex module id"), ids.next().expect("fragment module id"))
    };
    ...
    assert!(c.has_pending_destroys(), "glDeleteProgram queues the program's persistent destroys");

    // A swap with nothing to draw returns false but still flushes the queued destroys.
    assert!(!swap::swap_buffers(&mut c, &mut sink).unwrap());
    let destroy_batch = sink.batches.last().unwrap();
    assert!(destroy_batch.contains(&Cmd::DestroyShader(vs_ir)), "deleted program's vertex module is destroyed: {destroy_batch:?}");
    assert!(destroy_batch.contains(&Cmd::DestroyShader(fs_ir)), "deleted program's fragment module is destroyed: {destroy_batch:?}");
    assert!(destroy_batch.contains(&Cmd::DestroyPipeline(pipe_ir)), "deleted program's pipeline is destroyed: {destroy_batch:?}");
    assert!(!c.has_pending_destroys(), "destroys cleared after a successful submit");
}
