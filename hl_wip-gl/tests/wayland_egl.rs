...
fn open(path: &PathBuf) -> *mut c_void {
    ...
    if h.is_null() {
        ...
        let msg = if e.is_null() { String::new() } else { unsafe { std::ffi::CStr::from_ptr(e) }.to_string_lossy().into_owned() };
...
fn sym(handle: *mut c_void, name: &str) -> *mut c_void {
    ...
    assert!(!p.is_null(), "symbol {name} not found in the dlopened object");
    p
}
...
fn wayland_egl_abi_and_window_sizing_end_to_end() {
    ...
    if !egl_path.exists() || !wl_path.exists() {
        ...
        eprintln!("staged shims missing at {} — skipping (guest std not installed)", dir.display());
        return;
    }
    ...
    let egl_query_string: extern "C" fn(*mut c_void, i32) -> *const c_char = unsafe { std::mem::transmute(sym(egl, "eglQueryString")) };
    let egl_get_platform_display: extern "C" fn(u32, *mut c_void, *const isize) -> *mut c_void = unsafe { std::mem::transmute(sym(egl, "eglGetPlatformDisplay")) };
    let egl_initialize: extern "C" fn(*mut c_void, *mut i32, *mut i32) -> u32 = unsafe { std::mem::transmute(sym(egl, "eglInitialize")) };
    let egl_create_window_surface: extern "C" fn(*mut c_void, *mut c_void, *mut c_void, *const i32) -> *mut c_void = unsafe { std::mem::transmute(sym(egl, "eglCreateWindowSurface")) };
    let egl_create_platform_window_surface: extern "C" fn(*mut c_void, *mut c_void, *mut c_void, *const isize) -> *mut c_void = unsafe { std::mem::transmute(sym(egl, "eglCreatePlatformWindowSurface")) };
    let egl_query_surface: extern "C" fn(*mut c_void, *mut c_void, i32, *mut i32) -> u32 = unsafe { std::mem::transmute(sym(egl, "eglQuerySurface")) };

    let wl_egl_window_create: extern "C" fn(*mut c_void, i32, i32) -> *mut c_void = unsafe { std::mem::transmute(sym(wl, "wl_egl_window_create")) };
    let wl_egl_window_get_attached_size: extern "C" fn(*mut c_void, *mut i32, *mut i32) = unsafe { std::mem::transmute(sym(wl, "wl_egl_window_get_attached_size")) };
    let wl_egl_window_resize: extern "C" fn(*mut c_void, i32, i32, i32, i32) = unsafe { std::mem::transmute(sym(wl, "wl_egl_window_resize")) };
    let wl_egl_window_destroy: extern "C" fn(*mut c_void) = unsafe { std::mem::transmute(sym(wl, "wl_egl_window_destroy")) };
    ...
    assert!(!ext.is_null());
    let ext = unsafe { std::ffi::CStr::from_ptr(ext) }.to_string_lossy().into_owned();
    assert!(ext.contains("EGL_KHR_platform_wayland"), "client extensions advertise KHR platform_wayland: {ext:?}");
    assert!(ext.contains("EGL_EXT_platform_wayland"), "client extensions advertise EXT platform_wayland: {ext:?}");
    ...
    assert_eq!((aw, ah), (640, 480), "attached size defaults to the created size");
    ...
    let dpy = egl_get_platform_display(EGL_PLATFORM_WAYLAND_KHR, fake_wl_display, core::ptr::null());
    assert!(!dpy.is_null(), "eglGetPlatformDisplay(EGL_PLATFORM_WAYLAND_KHR) opens a display");
    assert_eq!(egl_initialize(dpy, core::ptr::null_mut(), core::ptr::null_mut()), EGL_TRUE);

    let config = 1usize as *mut c_void;
    let surface = egl_create_window_surface(dpy, config, win, core::ptr::null());
    assert!(!surface.is_null(), "eglCreateWindowSurface(wl_egl_window) returns a surface");
    let mut w = 0i32;
    let mut h = 0i32;
    assert_eq!(egl_query_surface(dpy, surface, EGL_WIDTH, &mut w), EGL_TRUE);
    assert_eq!(egl_query_surface(dpy, surface, EGL_HEIGHT, &mut h), EGL_TRUE);
    assert_eq!((w, h), (640, 480), "the window surface is sized from the wl_egl_window");
    ...
    assert_eq!(egl_query_surface(dpy, surface2, EGL_WIDTH, &mut w), EGL_TRUE);
    assert_eq!(egl_query_surface(dpy, surface2, EGL_HEIGHT, &mut h), EGL_TRUE);
    assert_eq!((w, h), (800, 600), "eglCreatePlatformWindowSurface re-reads the resized wl_egl_window");
