...
fn multi_fbo_frame_lowers_a_pass_per_framebuffer_and_presents_the_window() {
    ...
    assert_eq!(submits, 2, "a render pass per framebuffer (offscreen atlas, then default window)");
    ...
    assert_eq!((ad.width, ad.height), (16, 16), "offscreen target sized to its attachment");
    assert!(ad.usage & texture_usage::SAMPLED != 0, "offscreen target is SAMPLED by the composite pass");
    ...
    assert_eq!((dd.width, dd.height), (64, 64), "default target is the window size");
    match batch.last().unwrap() {
        Cmd::Present { texture, .. } => {
            assert_eq!(*texture, def_rt, "presents the WINDOW target, not the offscreen atlas");
    ...
    assert!(samples_atlas, "the composite pass samples the offscreen render-target texture (cross-pass)");
}
...
fn vertex_buffer_upload(batch: &[Cmd]) -> (u32, Vec<u8>) {
    ...
    match &batch[pos + 1] {
        Cmd::WriteBuffer { id: wid, offset: 0, data } if *wid == id => (id, data.clone()),
...
fn client_side_vertex_array_lowers_a_transient_vertex_buffer_and_binds_slot_0() {
    ...
    assert_eq!(data, expect, "the transient vertex buffer holds the captured client array");

    // The pass binds that transient buffer to slot 0 and draws 3 vertices.
    let ops = submit_ops(batch);
    assert!(
        ops.iter().any(|o| matches!(o, Enc::SetVertexBuffer { slot: 0, buffer, offset: 0 } if *buffer == vb_id)),
        "the client-array draw binds its transient buffer to vertex slot 0"
    );
    assert!(ops.iter().any(|o| matches!(o, Enc::Draw { vertex_count: 3, .. })));
...
fn client_side_index_array_lowers_a_transient_index_buffer() {
    ...
    record::draw_elements(&mut c, GL_TRIANGLES, 6, GL_UNSIGNED_BYTE, idx.as_ptr() as usize);
    ...
    assert_eq!(vdata.len(), 8 * 4, "4 verts * vec2 f32 captured for the index range");
