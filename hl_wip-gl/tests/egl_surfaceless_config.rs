...
fn surfaceless_display_and_config_enumeration_end_to_end() {
    ...
    let egl_choose_config: extern "C" fn(*mut c_void, *const i32, *mut *mut c_void, i32, *mut i32) -> u32 =
        unsafe { std::mem::transmute(sym(egl, "eglChooseConfig")) };
    ...
    assert!(ext.contains("EGL_EXT_platform_base"), "client extensions advertise EGL_EXT_platform_base: {ext:?}");
    ...
    for name in [
        ...
        assert!(!p.is_null(), "eglGetProcAddress({name}) must resolve (advertised by EGL_EXT_platform_base)");
    }
    ...
    assert!(egl_get_proc_address(bogus.as_ptr()).is_null(), "unknown proc resolves to null");
    ...
    assert_eq!(egl_initialize(dpy, &mut major, &mut minor), EGL_TRUE, "surfaceless display initializes");
    assert_eq!((major, minor), (1, 5), "advertises EGL 1.5");
    assert_eq!(cstr(egl_query_string(dpy, EGL_VENDOR)), "hl-gl", "our vendor over the initialized display");

    // 4) eglGetConfigs contract — null buffer reports the count, then a bounded copy fills the array.
    let mut count = -1i32;
    assert_eq!(egl_get_configs(dpy, core::ptr::null_mut(), 0, &mut count), EGL_TRUE);
    assert!(count >= 1, "at least one config is advertised (count={count})");
    let mut buf: [*mut c_void; 8] = [core::ptr::null_mut(); 8];
    let mut got = -1i32;
    assert_eq!(egl_get_configs(dpy, buf.as_mut_ptr(), buf.len() as i32, &mut got), EGL_TRUE);
    assert_eq!(got, count, "the bounded copy returns every advertised config");
    ...
    let mut zero = -1i32;
    assert_eq!(egl_get_configs(dpy, buf.as_mut_ptr(), 0, &mut zero), EGL_TRUE);
    ...
    assert!(attr(EGL_CONFIG_ID).1 > 0, "config id is a valid (nonzero) id");
    assert_eq!(attr(EGL_RENDERABLE_TYPE), (EGL_TRUE, EGL_OPENGL_ES2_BIT | EGL_OPENGL_ES3_BIT));

    // 7) An UNKNOWN attribute is EGL_BAD_ATTRIBUTE + EGL_FALSE (never a fabricated value / a deref).
    assert_eq!(egl_get_error(), EGL_SUCCESS, "no pending error before the bad query");
    let mut v = -12345i32;
    assert_eq!(
