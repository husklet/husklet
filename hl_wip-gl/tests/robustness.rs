...
fn renderbuffer_storage_rejects_oversized_extent_without_panicking() {
    ...
    assert_eq!(intro::renderbuffer_parameter(&c, GL_RENDERBUFFER, GL_RENDERBUFFER_WIDTH), 0);

    // A within-limits storage still works.
    record::renderbuffer_storage(&mut c, GL_RENDERBUFFER, GL_RGBA8, 256, 128);
    assert_eq!(c.take_gl_error(), GL_NO_ERROR);
    assert_eq!(intro::renderbuffer_parameter(&c, GL_RENDERBUFFER, GL_RENDERBUFFER_WIDTH), 256);
    assert_eq!(intro::renderbuffer_parameter(&c, GL_RENDERBUFFER, GL_RENDERBUFFER_HEIGHT), 128);
}
...
fn copy_tex_sub_image_rejects_huge_offset_without_panicking() {
    ...
    record::copy_tex_sub_image_2d(&mut c, GL_TEXTURE_2D, 0, 2_000_000_000, 0, 0, 0, 2_000_000_000, 1);
    assert_eq!(c.take_gl_error(), GL_INVALID_VALUE);
}
...
fn attrib_location_namespace_matches_the_emitted_shader() {
    ...
    assert_eq!(query::attrib_location(&c, p, "aLegacy"), -1, "phantom attribute must not resolve");
}
...
fn indexed_buffer_binding_validates_target_and_index() {
    ...
    assert_eq!(c.take_gl_error(), GL_NO_ERROR);
    assert_eq!(record::indexed_buffer_binding(&c, GL_UNIFORM_BUFFER, 2).map(|x| x.buffer), Some(b));
}
...
fn framebuffer_texture_2d_error_matrix() {
    ...
    record::framebuffer_texture_2d(&mut c, GL_ARRAY_BUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, tex, 0);
    assert_eq!(c.take_gl_error(), GL_INVALID_ENUM);
    ...
    record::framebuffer_texture_2d(&mut c, GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, tex, 3);
    assert_eq!(c.take_gl_error(), GL_INVALID_VALUE);
...
fn pixel_store_rejects_bad_alignment_and_leaves_state_unchanged() {
    ...
    assert_eq!(c.take_gl_error(), GL_INVALID_VALUE);
    assert_eq!(c.pixel_store.unpack_alignment, 4, "the default is unchanged after a rejected set");
...
fn deleting_the_bound_program_and_texture_clears_the_binding() {
    ...
    assert_eq!(c.tex_unit[0], 0, "deleting a bound texture clears the unit binding");
}
...
fn fence_and_sync_error_edges() {
    ...
    assert_eq!(sync::client_wait_sync(&mut c, &mut sink, 777, 0, 0), GL_WAIT_FAILED);
    assert_eq!(c.take_gl_error(), GL_INVALID_VALUE);
