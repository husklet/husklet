...
//!
//! The executor compiles a forwarded shader one of two ways (see `hl_wip-gpu-wgpu::wgsl::glsl_to_wgsl`):
...
//!     route, exercised in `hl_wip-gpu-wgpu`, owns the compile). Mis-routing here IS a driver bug.
//!
...
// ===================================================================================================
// host route — reproduce `hl_wip-gpu-wgpu::wgsl::glsl_to_wgsl`'s DESKTOP path (naga glsl-in → fixups →
...
mod host {
    ...
    pub fn compile_desktop(src: &str, stage: naga::ShaderStage, entry: &str) -> Result<String, String> {
...
fn link_and_forward(vs: &str, fs: &str) -> (String, String) {
    ...
    assert!(record::link_program(&mut c, prog), "glLinkProgram must succeed");
    ...
    let vd = GlslDescriptor::from_words(vw).expect("glsl vs words").expect("decode vs");
    let fd = GlslDescriptor::from_words(fw).expect("glsl fs words").expect("decode fs");
    (vd.source, fd.source)
}
...
fn compile_translated(vs_out: &str, fs_out: &str) -> Result<(), String> {
    for (stage_name, src) in [("vertex", vs_out), ("fragment", fs_out)] {
        for banned in ["#version 300 es", "\nattribute ", "\nvarying ", "gl_FragColor", "texture2D("] {
            if src.contains(banned) {
                return Err(format!("{stage_name}: ES dialect token {banned:?} leaked:\n{src}"));
            }
        }
    }
...
mod render {
    ...
    fn es3_passthrough_renders_exact_pixels() {
        ...
        assert_eq!(texel(&px, 0, 0), [0, 0, 255, 255], "a corner is the blue clear (RGBA)");
    }
    ...
    fn es2_helper_shader_renders_exact_pixels() {
        ...
        assert_eq!(texel(&px, W / 2, H / 2), [0, 255, 0, 255], "center is the green triangle (RGBA)");
        assert_eq!(texel(&px, 0, 0), [0, 0, 255, 255], "a corner is the blue clear (RGBA)");
    }
}
...
fn corpus_compiles_and_routes() {
    let cases = corpus();
    let n = cases.len();
    let gaps = cases.iter().filter(|c| matches!(c.path, Path::KnownDownstreamGap(_))).count();
