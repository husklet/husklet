...
fn uniform_block_index_is_stable_and_binding_round_trips() {
    ...
    let data_size = intro::active_uniform_blockiv(&mut c, prog, i0, GL_UNIFORM_BLOCK_DATA_SIZE).unwrap();
    assert!(data_size >= 16, "a vec4 block is at least 16 bytes, got {data_size}");

    // Default binding is 0; glUniformBlockBinding sets it, and it reads back.
    assert_eq!(intro::active_uniform_blockiv(&mut c, prog, i0, GL_UNIFORM_BLOCK_BINDING), Some(0));
    intro::uniform_block_binding(&mut c, prog, i0, 3);
    assert_eq!(c.take_gl_error(), GL_NO_ERROR);
    assert_eq!(intro::active_uniform_blockiv(&mut c, prog, i0, GL_UNIFORM_BLOCK_BINDING), Some(3));

    // The block name round-trips.
    assert_eq!(intro::active_uniform_block_name(&mut c, prog, i0).as_deref(), Some("Uniforms"));
    ...
    assert_eq!(intro::uniform_block_index(&mut c, 4242, "Uniforms"), GL_INVALID_INDEX);
}
...
fn uniform_indices_and_active_uniformsiv_reflect_the_tables() {
    ...
    assert_eq!(intro::active_uniformsiv(&c, prog, 0, GL_UNIFORM_TYPE), Some(GL_FLOAT_VEC4 as i32));
    assert_eq!(intro::active_uniformsiv(&c, prog, 0, GL_UNIFORM_SIZE), Some(1));
    assert_eq!(intro::active_uniformsiv(&c, prog, 0, GL_UNIFORM_BLOCK_INDEX), Some(0));
    assert_eq!(intro::active_uniformsiv(&c, prog, 0, GL_UNIFORM_OFFSET), Some(0));
    assert_eq!(intro::active_uniformsiv(&c, prog, 0, GL_UNIFORM_NAME_LENGTH), Some("uTint".len() as i32 + 1));

    // The sampler uniform is not backed by a buffer block (offset / block index -1).
    assert_eq!(intro::active_uniformsiv(&c, prog, 1, GL_UNIFORM_TYPE), Some(GL_SAMPLER_2D as i32));
    assert_eq!(intro::active_uniformsiv(&c, prog, 1, GL_UNIFORM_BLOCK_INDEX), Some(-1));
    assert_eq!(intro::active_uniformsiv(&c, prog, 1, GL_UNIFORM_OFFSET), Some(-1));
...
fn program_uniform_writes_into_the_named_program_block() {
    ...
    let got_f: Vec<f32> = got.chunks_exact(4).map(|b| f32::from_le_bytes([b[0], b[1], b[2], b[3]])).collect();
...
fn program_resource_introspection_reflects_uniforms_inputs_outputs() {
    ...
    assert_eq!(intro::program_interfaceiv(&c, prog, GL_UNIFORM, GL_ACTIVE_RESOURCES), Some(2));
    assert_eq!(intro::program_resource_index(&c, prog, GL_UNIFORM, "uTint"), 0);
    assert_eq!(intro::program_resource_index(&c, prog, GL_UNIFORM, "uTex"), 1);
    assert_eq!(intro::program_resource_index(&c, prog, GL_UNIFORM, "nope"), GL_INVALID_INDEX);
    assert_eq!(intro::program_resource_name(&c, prog, GL_UNIFORM, 0).as_deref(), Some("uTint"));
    assert_eq!(intro::program_resourceiv(&c, prog, GL_UNIFORM, 0, GL_TYPE), Some(GL_FLOAT_VEC4 as i32));
    assert_eq!(intro::program_resourceiv(&c, prog, GL_UNIFORM, 0, GL_NAME_LENGTH), Some("uTint".len() as i32 + 1));

    // GL_PROGRAM_INPUT: two vertex attributes in declaration order (aPos, aColor).
    assert_eq!(intro::program_interfaceiv(&c, prog, GL_PROGRAM_INPUT, GL_ACTIVE_RESOURCES), Some(2));
    assert_eq!(intro::program_resource_location(&c, prog, GL_PROGRAM_INPUT, "aPos"), 0);
    assert_eq!(intro::program_resource_location(&c, prog, GL_PROGRAM_INPUT, "aColor"), 1);
    assert_eq!(intro::program_resourceiv(&c, prog, GL_PROGRAM_INPUT, 1, GL_TYPE), Some(GL_FLOAT_VEC3 as i32));

    // GL_PROGRAM_OUTPUT: the one named fragment output resolves to location 0.
    assert_eq!(intro::program_interfaceiv(&c, prog, GL_PROGRAM_OUTPUT, GL_ACTIVE_RESOURCES), Some(1));
...
fn renderbuffer_and_tex_level_parameters_report_real_extents() {
    ...
    assert_eq!(intro::renderbuffer_parameter(&c, GL_RENDERBUFFER, GL_RENDERBUFFER_WIDTH), 64);
    assert_eq!(intro::renderbuffer_parameter(&c, GL_RENDERBUFFER, GL_RENDERBUFFER_HEIGHT), 48);
    assert_eq!(intro::renderbuffer_parameter(&c, GL_RENDERBUFFER, GL_RENDERBUFFER_INTERNAL_FORMAT), GL_RGBA8 as i32);
    ...
    assert_eq!(intro::tex_level_parameter(&c, GL_TEXTURE_2D, 0, GL_TEXTURE_WIDTH), 32);
    assert_eq!(intro::tex_level_parameter(&c, GL_TEXTURE_2D, 0, GL_TEXTURE_HEIGHT), 16);
}
