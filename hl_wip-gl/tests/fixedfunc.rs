...
fn color_mask_all_false_lowers_to_zero_write_mask() {
    ...
    record::draw_arrays(&mut c, GL_TRIANGLES, 0, 3);
    assert!(swap::swap_buffers(&mut c, &mut sink).unwrap());
    assert_eq!(pipeline_desc(&sink.batches[0]).color_targets[0].write_mask, 0x0);
}
...
fn bound_sampler_object_overrides_texture_filter_and_wrap() {
    ...
    es3::sampler_parameter(&mut c, samp, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE as i32, 0.0);
    es3::sampler_parameter(&mut c, samp, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE as i32, 0.0);
    es3::bind_sampler(&mut c, 0, samp);
    record::draw_arrays(&mut c, GL_TRIANGLES, 0, 3);
    assert!(swap::swap_buffers(&mut c, &mut sink).unwrap());
    let sd = first_sampler_desc(&sink.batches[0]);
    assert_eq!(sd.min_filter, Filter::Linear, "bound sampler object's LINEAR min-filter overrides texture NEAREST");
    assert_eq!(sd.mag_filter, Filter::Linear, "object LINEAR mag-filter overrides texture NEAREST");
    assert_eq!(sd.address_u, AddressMode::ClampToEdge, "object CLAMP_TO_EDGE overrides texture REPEAT");
...
fn no_sampler_object_leaves_texture_params_authoritative() {
    ...
    assert!(swap::swap_buffers(&mut c, &mut sink).unwrap());
    let sd = first_sampler_desc(&sink.batches[0]);
    assert_eq!(sd.min_filter, Filter::Nearest, "texture NEAREST wins when no sampler object is bound");
    assert_eq!(sd.address_u, AddressMode::Repeat, "texture REPEAT wins when no sampler object is bound");
}
