...
fn default_block_array_uniform_keeps_its_dimension() {
    ...
    assert!(f.contains("vec4 uKernel[8];"), "array dimension preserved in block:\n{f}");
    // std140 sizing: 8 * vec4 (16 B) = 128 B for the member.
    let (unis, total) = glsl::uni_layout(vs, fs);
    let k = unis.iter().find(|u| u.name == "uKernel").expect("uKernel reflected");
...
fn skia_bare_in_out_get_sequential_and_name_matched_locations() {
    ...
    assert!(v.contains("layout(location = 0) in highp vec4 fillBounds;"), "{v}");
    assert!(v.contains("layout(location = 1) in highp vec4 locations;"), "{v}");
    // Varyings: vertex out gets 0,1 in decl order; the `flat` qualifier is preserved.
    assert!(v.contains("layout(location = 0) out highp vec2 vatlasCoord_S0;"), "{v}");
    assert!(v.contains("layout(location = 1) flat out mediump vec4 vcolor_S0;"), "{v}");
    // Fragment in varyings NAME-MATCH the vertex out locations (0 and 1), flat preserved.
    assert!(f.contains("layout(location = 0) in highp vec2 vatlasCoord_S0;"), "{f}");
    assert!(f.contains("layout(location = 1) flat in mediump vec4 vcolor_S0;"), "{f}");
    // Fragment output: own namespace, location 0.
    assert!(f.contains("layout(location = 0) out mediump vec4 sk_FragColor;"), "{f}");
}
...
fn explicit_locations_are_preserved_and_reserved() {
    ...
    let (v, f) = glsl::inject_io_locations(vs, fs);
    assert!(v.contains("layout(location = 2) in vec4 aExplicit;"), "explicit attr preserved: {v}");
    assert!(v.contains("layout(location = 0) in vec4 aBare;"), "bare attr avoids reserved 2: {v}");
    assert!(v.contains("layout(location = 5) out vec2 vExplicit;"), "explicit varying preserved: {v}");
    assert!(v.contains("layout(location = 0) out vec2 vBare;"), "bare varying avoids reserved 5: {v}");
...
fn prepare_verbatim_program_wraps_uniforms_and_injects_locations_together() {
    ...
    assert!(v.contains("layout(std140, binding = 0) uniform HlUniforms"), "{v}");
    assert!(!v.contains("uniform highp vec4 sk_RTAdjust;"), "bare uniform removed: {v}");
    assert!(v.contains("layout(location = 0) in highp vec4 pos;"), "{v}");
    assert!(v.contains("layout(location = 0) out highp vec2 vUV;"), "{v}");
