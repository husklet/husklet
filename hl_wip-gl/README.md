...
the `eglSwapBuffers` frame boundary**. Structurally IDENTICAL to `hl_wip-cuda`: the only dependency is the
neutral protocol crate `hl_wip-gpu` (package `hl_wip_gpu`, lib `hl_gpu`); every path funnels through the
...
## Layout (mirrors hl_wip-cuda: model / service / adapter / result)
