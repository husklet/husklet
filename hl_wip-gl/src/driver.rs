...
impl Gl {
    ...
    fn staged(&self, soname: &str) -> PathBuf {
        self.spec.stage_root.join("gl").join(self.spec.arch.dir()).join(soname)
    }
}
...
impl Driver for Gl {
    fn device_request(&self, guest_env: &[String]) -> DeviceRequest {
        ...
        // 1. Bind the staged guest shim .so's at their guest sonames in the guest multiarch libdir, so a
        //    guest app that DT_NEEDEDs libEGL.so.1 / libGLESv2.so.2 resolves them (GLESv2 is the thin
        //    DT_NEEDED->libEGL forwarding stub, so every `gl*` symbol lands in the primary libEGL object).
        let mut mounts = vec![
            DeviceMount::ro(
...
mod tests {
    ...
    fn device_request_binds_the_two_sonames_and_socket() {
        ...
        let want = [
            ("/opt/hlroot/gl/aarch64/libEGL.so.1", format!("{libdir}/libEGL.so.1"), true),
            ("/opt/hlroot/gl/aarch64/libGLESv2.so.2", format!("{libdir}/libGLESv2.so.2"), true),
        ];
