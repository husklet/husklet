...
impl Samplers {
    pub fn new() -> Self {
        Self { reserved: HashSet::new(), objects: HashMap::new(), binding: HashMap::new(), next_name: 1 }
    }
...
impl TransformFeedbacks {
    ...
    pub fn varying(&self, program: u32, index: u32) -> Option<&str> {
        self.varyings.get(&program).and_then(|(v, _)| v.get(index as usize)).map(|s| s.as_str())
    }
}
...
impl ProgramPipelines {
    pub fn new() -> Self {
        Self { reserved: HashSet::new(), objects: HashMap::new(), bound: 0, next_name: 1 }
    }
