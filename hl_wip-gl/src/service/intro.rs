...
fn seed_blocks(ctx: &mut GlContext, program: u32) {
    let has_uniforms = ctx.programs.program(program).map(|p| p.has_uniforms()).unwrap_or(false);
    if !has_uniforms {
        return;
    }
    ...
    if blocks.is_empty() {
        ...
        blocks.push(UniformBlock { name: "Uniforms".to_string(), binding: 0 });
    }
}
...
pub fn uniform_block_index(ctx: &mut GlContext, program: u32, name: &str) -> u32 {
    ...
    blocks.push(UniformBlock { name: name.to_string(), binding: 0 });
    idx
}
...
pub fn active_uniform_block_name(ctx: &mut GlContext, program: u32, block_index: u32) -> Option<String> {
    seed_blocks(ctx, program);
    ctx.uniform_blocks
        .get(&program)
...
pub fn active_uniform_blockiv(ctx: &mut GlContext, program: u32, block_index: u32, pname: u32) -> Option<i32> {
    seed_blocks(ctx, program);
    let (binding, name_len) = {
        let b = ctx.uniform_blocks.get(&program).and_then(|b| b.get(block_index as usize))?;
...
pub fn interface_resources(ctx: &GlContext, program: u32, interface: u32) -> Vec<Resource> {
    ...
    match interface {
        GL_UNIFORM => {
            ...
            data.into_iter()
                ...
                .map(|(i, d)| Resource {
                    ...
                    location: if i < n_data { i as i32 } else { (i - n_data) as i32 },
                })
                .collect()
        }
        GL_PROGRAM_INPUT => glsl::collect_vertex_attrs(&p.vs_src)
            .into_iter()
            .enumerate()
            .map(|(i, d)| Resource { name: d.name.clone(), gl_type: gl_type_enum(&d.ty), location: i as i32 })
            .collect(),
...
pub fn program_interfaceiv(ctx: &GlContext, program: u32, interface: u32, pname: u32) -> Option<i32> {
    if interface == GL_UNIFORM_BLOCK {
        // Uniform blocks are reflected through the block table (block 0 = the implicit block).
        let n = if ctx.programs.program(program).map(|p| p.has_uniforms()).unwrap_or(false) { 1 } else { 0 };
        return Some(match pname {
            GL_ACTIVE_RESOURCES => n,
            GL_MAX_NAME_LENGTH => if n > 0 { "Uniforms".len() as i32 + 1 } else { 0 },
            _ => 0,
        });
    }
    ...
    Some(match pname {
        ...
        GL_MAX_NAME_LENGTH => res.iter().map(|r| r.name.len() as i32 + 1).max().unwrap_or(0),
        _ => 0,
    })
}
...
pub fn program_resource_name(ctx: &GlContext, program: u32, interface: u32, index: u32) -> Option<String> {
    interface_resources(ctx, program, interface).into_iter().nth(index as usize).map(|r| r.name)
}
...
pub fn program_resourceiv(ctx: &GlContext, program: u32, interface: u32, index: u32, prop: u32) -> Option<i32> {
    let res = interface_resources(ctx, program, interface);
...
pub fn frag_data_location(ctx: &GlContext, program: u32, name: &str) -> i32 {
    let Some(p) = ctx.programs.program(program) else { return -1 };
    if !p.linked {
        return -1;
    }
...
pub fn get_shader_source(ctx: &GlContext, shader: u32) -> String {
    ctx.programs.shader(shader).and_then(|s| s.src.clone()).unwrap_or_default()
}
...
pub fn framebuffer_attachment_parameter(ctx: &GlContext, target: u32, attachment: u32, pname: u32) -> i32 {
