...
mod tests {
    ...
    fn our_config_reports_truthful_color_depth_stencil() {
        ...
        assert_eq!(config_attrib(true, EGL_COLOR_BUFFER_TYPE), Ok(EGL_RGB_BUFFER));
        ...
        assert_eq!(config_attrib(true, EGL_SURFACE_TYPE), Ok(EGL_WINDOW_BIT | EGL_PBUFFER_BIT));
    }

    #[test]
    fn unknown_attribute_is_bad_attribute_not_a_fake_zero() {
        // A garbage enum must be reported, never silently answered with 0.
        assert_eq!(config_attrib(true, 0x1234_5678), Err(ConfigError::BadAttribute));
        assert_eq!(config_attrib(true, 0), Err(ConfigError::BadAttribute));
    }

    #[test]
    fn foreign_config_handle_is_bad_config() {
        assert_eq!(config_attrib(false, EGL_RED_SIZE), Err(ConfigError::BadConfig));
    }
