...
pub fn map_buffer_range(ctx: &mut GlContext, target: u32, offset: isize, length: isize, _access: u32) -> Option<(u32, usize)> {
...
pub fn flush_mapped_range(ctx: &mut GlContext, sink: &mut dyn CommandSink, target: u32, offset: isize, length: isize) -> Result<()> {
    if offset < 0 || length < 0 {
        ctx.set_gl_error(GL_INVALID_VALUE);
        return Ok(());
    }
    ...
    let bytes = ctx.buffers.range_bytes(name, abs, length as usize).unwrap_or_default();
...
fn flush_bytes(ctx: &mut GlContext, sink: &mut dyn CommandSink, name: u32, data_offset: usize, bytes: Vec<u8>) -> Result<()> {
    if bytes.is_empty() {
        return Ok(());
    }
    let total = ctx.buffers.get(name).map(|b| b.data.len()).unwrap_or(data_offset + bytes.len()) as u64;
    let ir = ctx.alloc_buffer_ir();
    let cmds = vec![
        Cmd::CreateBuffer(ir, BufferDesc { size: total, usage: buffer_usage::STORAGE | buffer_usage::COPY_DST, label: String::new() }),
        Cmd::WriteBuffer { id: ir, offset: data_offset as u64, data: bytes },
    ];
    sink.submit(&cmds)
}
