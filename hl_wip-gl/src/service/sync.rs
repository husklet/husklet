...
pub fn fence_sync(ctx: &mut GlContext, sink: &mut dyn CommandSink, condition: u32, flags: u32) -> Option<usize> {
    ...
    cmds.push(Cmd::Submit(CommandBuffer { encoder: Vec::new(), signal: Some((fence, value)) }));
...
pub fn client_wait_sync(ctx: &mut GlContext, sink: &mut dyn CommandSink, sync: usize, flags: u32, timeout: u64) -> u32 {
...
pub fn wait_sync(ctx: &mut GlContext, sink: &mut dyn CommandSink, sync: usize, flags: u32, timeout: u64) {
    ...
    let _ = sink.submit(&[Cmd::WaitFence { id: ctx.fence_ir, value }]);
}
...
pub fn get_synciv(ctx: &mut GlContext, sync: usize, pname: u32) -> Option<i32> {
    ...
    match pname {
        GL_SYNC_STATUS => Some(if ctx.fence_signaled_through >= value { GL_SIGNALED as i32 } else { GL_UNSIGNALED as i32 }),
