    ...
    while let Some(rel) = find_from(b, kb, p) {
        ...
        if q < b.len() && b[q] == b'{' {
            ...
            while out.len() < 32 {
                ...
                let marr = read_array_subscript(b, &mut q);
                ...
                if !mty.is_empty() && !mnm.is_empty() {
                    out.push(Decl { ty: mty, name: mnm, arr: marr });
                }
            }
            if q < b.len() && b[q] == b'}' {
                q += 1;
        ...
        let arr = read_array_subscript(b, &mut q);
...
pub fn uni_layout(vs: &str, fs: &str) -> (Vec<Uni>, i32) {
    ...
    for d in unis.iter().take(16) {
        let (esz, eal) = msl_type_layout(&d.ty).unwrap_or((4, 4));
...
pub fn prepare_verbatim_program(vs: &str, fs: &str, combined: &[Decl]) -> (String, String) {
    let vs_u = prepare_verbatim_stage(vs, combined);
    let fs_u = prepare_verbatim_stage(fs, combined);
    inject_io_locations(&vs_u, &fs_u)
}
...
fn is_precision(w: &str) -> bool {
    matches!(w, "highp" | "mediump" | "lowp")
}
    ...
    while i < n {
        ...
        if is_word(c) {
            ...
            if brace == 0 && paren == 0 && (word == "in" || word == "out") {
                if let Some(name) = parse_io_decl_forward(b, i) {
                    let (stmt_start, merge_rparen, explicit_loc) = preceding_io_qualifiers(b, start);
                    out.push(IoDecl { is_in: word == "in", name, stmt_start, merge_rparen, explicit_loc });
                }
            }
            continue;
        }
...
fn preceding_io_qualifiers(b: &[u8], kw_start: usize) -> (usize, Option<usize>, Option<u32>) {
    ...
    loop {
        ...
        if ch == b')' {
            ...
            let mut pp = lparen;
            while pp > 0 && is_space(b[pp - 1]) {
                pp -= 1;
            }
            if pp >= 6 && &b[pp - 6..pp] == b"layout" && !(pp > 6 && is_word(b[pp - 7])) {
                let grp = &b[lparen..=rparen];
                if grp.windows(8).any(|w| w == b"location") {
                    explicit = explicit.or_else(|| Some(parse_location_value(grp).unwrap_or(u32::MAX)));
                }
...
fn parse_location_value(grp: &[u8]) -> Option<u32> {
    ...
    let mut p = pos + 8;
    while p < grp.len() && (is_space(grp[p]) || grp[p] == b'=') {
        p += 1;
    }
    let s = p;
...
pub fn inject_io_locations(vs: &str, fs: &str) -> (String, String) {
    use std::collections::{BTreeMap, BTreeSet};
    let vsd = scan_io_decls(vs);
    let fsd = scan_io_decls(fs);

    // Lowest free location not yet used in `used`, then reserve it.
    fn take(used: &mut BTreeSet<u32>) -> u32 {
        let mut c = 0u32;
    ...
    let mut attr_used: BTreeSet<u32> =
        vsd.iter().filter(|d| d.is_in).filter_map(|d| d.explicit_loc.filter(|&l| l != u32::MAX)).collect();
    let mut fs_out_used: BTreeSet<u32> =
        fsd.iter().filter(|d| !d.is_in).filter_map(|d| d.explicit_loc.filter(|&l| l != u32::MAX)).collect();
    ...
    for d in vsd.iter().filter(|d| !d.is_in && d.explicit_loc.is_none()) {
        varying_map.entry(d.name.clone()).or_insert_with(|| take(&mut varying_used));
    }
    ...
    for d in &vsd {
        ...
        let loc = if d.is_in { take(&mut attr_used) } else { varying_map[&d.name] };
    ...
    for d in &fsd {
        ...
        let loc = if d.is_in { varying_map[&d.name] } else { take(&mut fs_out_used) };
        fs_edits.push(edit_for(d, loc));
    }
    (apply_edits(vs, &mut vs_edits), apply_edits(fs, &mut fs_edits))
}
...
fn apply_edits(src: &str, edits: &mut [(usize, String)]) -> String {
    if edits.is_empty() {
        return src.to_string();
    }
    edits.sort_by_key(|(p, _)| *p);
    let mut out = String::with_capacity(src.len() + edits.len() * 24);
    let mut last = 0usize;
    for (pos, text) in edits.iter() {
        out.push_str(&src[last..*pos]);
        out.push_str(text);
        last = *pos;
    }
...
/// The sampler types the host executor's ES route (`hl_wip-gpu-wgpu::glsl_es::split_sampler_ty`) splits +
...
fn host_sampler_ty(t: &str) -> Option<bool> {
    match t {
...
pub fn verbatim_sampler_bindings(samp_names: &[String], fs: &str) -> Vec<u32> {
    ...
    while w + 2 < words.len() {
        if words[w].1 == "uniform" {
            if let Some(is_external) = host_sampler_ty(words[w + 1].1) {
