...
mod tests {
    ...
    fn missing_shm_global_is_a_soft_error() {
        ...
        assert!(err.is_unavailable(), "a missing global must be a soft (fall-back) failure");
    }
    ...
    fn short_plane_is_hard_bad_size() {
        ...
        assert!(!err.is_unavailable(), "a live present failure is hard (EGL_CONTEXT_LOST), not a fall-back");
    }
    ...
    fn error_softness_classification() {
        ...
        for e in [WlAppError::BadSize, WlAppError::ShmAlloc, WlAppError::Marshal, WlAppError::Flush] {
