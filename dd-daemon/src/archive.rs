...
pub(crate) async fn archive_put(State(a): State<App>, Path(id): Path<String>, Query(q): Query<ArchiveQ>, body: axum::body::Bytes) -> Response {
    let (cid, upper, rootfs, binds) = { let g = a.inner.lock().await;
        let Some(full) = resolve_cid(&g, &id) else { return no_such(&id); };
        let Some(c) = g.containers.get(&full) else { return no_such(&id); };
        (full.clone(), c.upper.clone(), c.rootfs.clone(), c.binds.iter().cloned().chain(mounts_as_binds(&c.mounts)).collect::<Vec<_>>()) };
    ...
    let out = {
        use std::io::Write;
        use std::process::Stdio;
        match std::process::Command::new("tar").arg("xf").arg("-").arg("-C").arg(&host)
            .stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped()).spawn() {
            Ok(mut child) => {
                if let Some(mut si) = child.stdin.take() {
                    let body = body.clone(); // axum Bytes: cheap Arc clone, moved into the writer thread
                    std::thread::spawn(move || { let _ = si.write_all(&body); }); // drop closes stdin -> tar EOF
                }
    ...
    // #374 docker-cp coherence: the daemon just mutated a (possibly live) container's filesystem from
