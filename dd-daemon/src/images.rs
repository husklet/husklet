...
pub(crate) async fn images_create(State(a): State<App>, Query(q): Query<ImageCreateQ>, headers: axum::http::HeaderMap, body: axum::body::Bytes) -> Response {
    ...
    {
        // The tag is already present locally, but its stored config may be stale (issue #295) — e.g. an
...
/// Best-effort refresh of an already-cached image's stored config on a re-pull (issue #295). Runs the
...
async fn refresh_local_config(a: &App, name: &str, tag: &str, want: String, creds: Credentials, archs: Vec<&'static str>) {
    let (dir, nm, tg) = (a.images_dir.clone(), name.to_string(), tag.to_string());
    let fresh = tokio::task::spawn_blocking(move || refresh_image_config(&dir, &nm, &tg, creds, &archs))
        .await.ok().flatten();
    let Some(fresh) = fresh else { return };
    let mut g = a.inner.lock().await;
    if let Some(slot) = g.images.iter_mut()
...
pub(crate) fn pull_image(images_dir: &str, from_image: &str, tag: &str, creds: Credentials, archs: &[&str], progress: &mut dyn FnMut(PullEvent)) -> Result<Image, String> {
    ...
    Ok(image_from_config(images_dir, &iref, &pulled.config, &rootfs))
}


/// Re-fetch an already-cached image's run config from its registry and rebuild its [`Image`] WITHOUT
/// re-downloading the layers (issue #295). `docker pull <img>:<tag>` on a tag that is already present
...
pub(crate) fn refresh_image_config(images_dir: &str, from_image: &str, tag: &str, creds: Credentials, archs: &[&str]) -> Option<Image> {
    let iref = image_ref(from_image, tag);
    let rootfs = std::path::PathBuf::from(format!("{images_dir}/{}/rootfs", safe_name(&iref)));
    if !rootfs.is_dir() { return None; }
    ...
    if !config.get("config").map(|c| c.is_object()).unwrap_or(false) { return None; }
...
pub(crate) fn image_from_config(images_dir: &str, iref: &ImageRef, config: &Value, rootfs: &std::path::Path) -> Image {
    ...
    let workdir = config["config"]["WorkingDir"].as_str().unwrap_or("").to_string();
...
mod refresh_tests {
    ...
    // #295: a re-pull of an already-present tag must refresh the stored config so `docker run` uses the
    ...
    fn image_from_config_uses_real_entrypoint_not_bin_sh() {
        let dir = std::env::temp_dir().join(format!("dd-refresh-{}-{}", std::process::id(), now_nanos()));
        ...
        assert!(!image.cmd.iter().any(|c| c == "/bin/sh"), "refresh must not fall back to /bin/sh");
    ...
    fn image_from_config_entrypoint_only_keeps_empty_cmd() {
        let dir = std::env::temp_dir().join(format!("dd-refresh2-{}-{}", std::process::id(), now_nanos()));
        ...
        assert!(image.cmd.is_empty(), "entrypoint-only image must keep Cmd empty");
        std::fs::remove_dir_all(&dir).ok();
    }
}
