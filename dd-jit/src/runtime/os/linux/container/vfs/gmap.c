...
static struct { uint64_t addr, len, glen; } g_gmap[GMAP_N];
static int g_ngmap;
...
static void gmap_set_glen(uint64_t addr, uint64_t glen) {
    for (int i = 0; i < g_ngmap; i++)
        if (g_gmap[i].addr == addr) { g_gmap[i].glen = glen; return; }
}
...
#define MLK_N 1024
static struct { uint64_t lo, hi; } g_mlk[MLK_N];
static int g_nmlk;
...
static void mlk_del(uint64_t addr, uint64_t len) {
    ...
    for (int i = 0; i < g_nmlk;) {
        ...
        if (!keep_head && !keep_tail) { g_mlk[i] = g_mlk[--g_nmlk]; continue; } // fully covered -> drop
        if (keep_head) g_mlk[i].hi = rlo;
        else g_mlk[i].lo = rhi;
        if (keep_head && keep_tail && g_nmlk < MLK_N) { g_mlk[g_nmlk].lo = rhi; g_mlk[g_nmlk].hi = hi; g_nmlk++; }
        i++;
    }
}
...
static void mlk_reset(void) { g_nmlk = 0; g_mlock_all = 0; } // execve replaces the address space
...
static uint64_t mlk_total_locked(void) {
    uint64_t sum = 0;
    if (g_mlock_all) {
        for (int i = 0; i < g_ngmap; i++) sum += g_gmap[i].glen;
        return sum;
    }
    for (int i = 0; i < g_nmlk; i++) sum += g_mlk[i].hi - g_mlk[i].lo;
    return sum;
}
