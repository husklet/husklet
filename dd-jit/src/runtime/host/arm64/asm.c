...
static void e_mov_sp_from(int rn) {
    emit32(0x9100001Fu | (rn << 5));
}

// mov xrd, sp
static void e_mov_from_sp(int rd) {
    emit32(0x910003E0u | rd);
}
...
// and xd,xn,#0x3FF
static void e_and1023(int d, int n) {
    emit32(0x92400000u | (9 << 10) | (n << 5) | d);
}

// mov xd,xm
static void e_movr(int d, int m) {
    emit32(0xAA0003E0u | (m << 16) | d);
}
