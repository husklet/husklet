...
use wayland_client::protocol::{
    ...
    wl_pointer::{self, Axis, AxisSource, WlPointer}, wl_registry::WlRegistry, wl_seat::WlSeat,
...
impl App {
    fn frame_groups(&self) -> Vec<Vec<PtrEv>> {
        ...
        let mut cur = Vec::new();
        for e in &self.ev {
            if *e == PtrEv::Frame { groups.push(std::mem::take(&mut cur)); } else { cur.push(*e); }
        }
        groups
    }
}
...
fn pointer_axis_discrete() {
    ...
    let mut app = App { surface: surface.clone(), buffer: buffer.clone(), drawn: false, frame_done: false, ev: Vec::new(), entered: false };
    ...
    h.input_tx.send(InputCommand::PointerMotion { x: 90.0, y: 70.0 }).expect("enter");
    ...
    h.input_tx.send(InputCommand::PointerAxisDiscrete { horizontal: HVAL, vertical: VVAL, h120: H120, v120: V120 })
        .expect("discrete axis");
    pump_while(&mut queue, &mut app, 5, |a| a.ev[before..].iter().any(|e| *e == PtrEv::Frame));

    let groups = app.frame_groups();
    let wheel = groups.iter().find(|g| g.iter().any(|e| matches!(e, PtrEv::AxisV(_))))
        .expect("a frame group carried the vertical wheel axis");

    // Smooth values with exact sign.
    assert!(wheel.contains(&PtrEv::AxisV(VVAL.to_bits())), "vertical smooth value +15 delivered, got {wheel:?}");
    assert!(wheel.contains(&PtrEv::AxisH(HVAL.to_bits())), "horizontal smooth value -10 delivered, got {wheel:?}");
    // Wheel source tag.
    assert!(wheel.contains(&PtrEv::Source(u32::from(AxisSource::Wheel))), "axis_source(wheel) delivered, got {wheel:?}");
    // Discrete notch: value120 (v8+) OR the legacy axis_discrete (v5-7). Accept either, exact + signed.
    let v_notch = wheel.contains(&PtrEv::V120(V120)) || wheel.contains(&PtrEv::Discrete(u32::from(Axis::VerticalScroll), 1));
    let h_notch = wheel.contains(&PtrEv::H120(H120)) || wheel.contains(&PtrEv::Discrete(u32::from(Axis::HorizontalScroll), -1));
    assert!(v_notch, "vertical discrete notch (+1 / value120 +120) delivered, got {wheel:?}");
    assert!(h_notch, "horizontal discrete notch (-1 / value120 -120) delivered, got {wheel:?}");
    // The whole wheel event is ONE frame (source + both axes + both notches grouped together).
    assert!(wheel.iter().any(|e| matches!(e, PtrEv::AxisV(_))) && wheel.iter().any(|e| matches!(e, PtrEv::AxisH(_))),
    ...
    fn event(app: &mut Self, _: &WlPointer, e: <WlPointer as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
            ...
            wl_pointer::Event::AxisSource { axis_source } => {
                if let WEnum::Value(s) = axis_source { app.ev.push(PtrEv::Source(u32::from(s))); }
            }
            ...
            wl_pointer::Event::AxisDiscrete { axis, discrete } => {
                if let WEnum::Value(a) = axis { app.ev.push(PtrEv::Discrete(u32::from(a), discrete)); }
            }
