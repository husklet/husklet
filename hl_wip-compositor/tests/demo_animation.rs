...
fn animation_fluency() {
    ...
    for i in 0..N {
        ...
        frames.push(make_buffer(&shm, &qh, &h.runtime_dir, &format!("f{i}"), W, H, &buf));
    }
    ...
    let mut app = App { tl_surface: tl_surface.clone(), frames, drawn: 0, tl_configured: false };
    ...
    for i in 0..N {
        ...
        let frame = h
            ...
            .cloned()
            .unwrap_or_else(|| panic!("animation frame {i} (rect at x={}) never composited", rect_x(i)));

        // The rect is at frame i's position and NOT at any other frame's position — a real move, not a
        // stuck or smeared frame.
        assert_eq!(frame.pixel(cx, cy).unwrap(), RECT, "frame {i}: rect at its position");
        for j in 0..N {
            if j != i {
                let (ox, oy) = rect_center(j);
                assert_eq!(frame.pixel(ox, oy).unwrap(), BASE, "frame {i}: no rect at frame {j}'s position");
            }
        }
    ...
    for i in 1..N {
        assert!(serials[i] > serials[i - 1], "frame {i} presented after frame {}", i - 1);
    }
    // Every frame callback was released (the client drew all N frames — pacing did not stall).
    assert_eq!(app.drawn, N, "all {N} frames drawn via frame-callback pacing");
...
impl Dispatch<WlCallback, ()> for App {
    fn event(app: &mut Self, _: &WlCallback, e: <WlCallback as Proxy>::Event, _: &(), _: &Connection, qh: &QueueHandle<Self>) {
