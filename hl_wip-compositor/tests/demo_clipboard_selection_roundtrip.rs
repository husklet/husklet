...
fn clipboard_selection_roundtrip() {
    ...
    let (_cb, mut qb, mut b, ddm_b, seat_b, qhb) = spawn(&h.runtime_dir, "B", [0x40, 0x40, 0xC0, 0xFF]);
    ...
    while b.selection_offer.is_none() {
        assert!(Instant::now() < deadline, "B never received a selection offer; mimes seen: {:?}", b.offered_mimes);
        let _ = qa.roundtrip(&mut a);
        let _ = qb.roundtrip(&mut b);
        std::thread::sleep(Duration::from_millis(5));
    }
    assert!(b.offered_mimes.contains(&MIME.to_string()),
        "B's data_offer advertised A's mime, got {:?}", b.offered_mimes);
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            panic!("clipboard bytes never arrived; got {} bytes so far: {:?}", got.len(), got);
        }
        std::thread::sleep(Duration::from_millis(5));
    }

    assert_eq!(got, PAYLOAD, "B pasted EXACTLY the bytes A copied");
    assert!(a.source_send_fired, "A's wl_data_source.send fired (the compositor routed B's receive to A)");
