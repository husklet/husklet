...
impl App {
    fn enters(&self) -> Vec<(i32, i32)> {
        self.events.iter().filter_map(|e| if let DndEv::Enter(x, y) = e { Some((*x, *y)) } else { None }).collect()
    }
    fn motions(&self) -> Vec<(i32, i32)> {
        self.events.iter().filter_map(|e| if let DndEv::Motion(x, y) = e { Some((*x, *y)) } else { None }).collect()
    }
}

fn spawn(dir: &std::path::Path, tag: &str, w: i32, h: i32)
    -> (Connection, EventQueue<App>, App, WlDataDeviceManager, WlSeat, QueueHandle<App>)
{
    ...
    let ddm: WlDataDeviceManager = globals.bind(&qh, 3..=3, ()).expect("wl_data_device_manager v3");

    let buffer = make_buffer(&shm, &qh, dir, tag, w, h, &solid(w, h, [0x40, 0x50, 0x60, 0xFF]));
    ...
    let mut app = App { surface: Some(surface.clone()), buffer: Some(buffer), ..App::default() };
...
fn drag_and_drop() {
    ...
    h.input_tx.send(InputCommand::PointerMotion { x: START.0 as f64, y: START.1 as f64 }).expect("enter A");
    h.input_tx.send(InputCommand::PointerButton { button: BTN_LEFT, pressed: true }).expect("press");
    ...
    while !h.observations.lock().unwrap().dnd_active {
        assert!(Instant::now() < deadline, "DnD grab never became active (start_drag not honoured)");
        let _ = qa.roundtrip(&mut a);
        std::thread::sleep(Duration::from_millis(5));
    }

    // ---- drag over the target: enter + motion at exact surface-local coords ----
    h.input_tx.send(InputCommand::PointerMotion { x: T1.0 as f64, y: T1.1 as f64 }).expect("drag→T1");
    assert!(
        pump_while(&mut qb, &mut b, 5, |b| b.enters().contains(&T1)),
        "target never got drag `enter` at {T1:?}; events={:?}", b.events,
    );
    assert!(b.offered_mimes.contains(&MIME.to_string()), "offer advertised the source mime, got {:?}", b.offered_mimes);
    assert_eq!(b.source_actions, Some(DndAction::Copy), "offer advertised the source's Copy action");

    h.input_tx.send(InputCommand::PointerMotion { x: T2.0 as f64, y: T2.1 as f64 }).expect("drag→T2");
    assert!(
        pump_while(&mut qb, &mut b, 5, |b| b.motions().contains(&T2)),
        "target never got drag `motion` at {T2:?}; events={:?}", b.events,
    );

    // ---- cancel: move OFF the target → `leave` ----
    h.input_tx.send(InputCommand::PointerMotion { x: OUT.0 as f64, y: OUT.1 as f64 }).expect("drag→OUT");
    assert!(
        pump_while(&mut qb, &mut b, 5, |b| b.events.contains(&DndEv::Leave)),
        "target never got drag `leave` on moving off; events={:?}", b.events,
    );

    // ---- move back on → `enter` again (a fresh offer) ----
    h.input_tx.send(InputCommand::PointerMotion { x: T3.0 as f64, y: T3.1 as f64 }).expect("drag→T3");
    assert!(
        pump_while(&mut qb, &mut b, 5, |b| b.enters().contains(&T3)),
        "target never re-entered at {T3:?}; events={:?}", b.events,
    );
    ...
    h.input_tx.send(InputCommand::PointerButton { button: BTN_LEFT, pressed: false }).expect("release");
    assert!(
        pump_while(&mut qb, &mut b, 5, |b| b.events.contains(&DndEv::Drop)),
        "target never got `drop`; events={:?}", b.events,
    );

    // ---- the target reads the dragged bytes over a real pipe ----
    let offer = b.current_offer.clone().expect("target holds a data offer at drop");
    ...
    loop {
        ...
        assert!(Instant::now() < deadline, "dragged bytes never arrived; got {:?}", b.received);
        std::thread::sleep(Duration::from_millis(5));
    }
    assert_eq!(b.received, PAYLOAD, "target received EXACTLY the source's dragged bytes");
    ...
    assert!(obs.dnd_drop_validated, "the drop was negotiated (accepted mime + Copy action)");
    ...
    let idx_enter1 = b.events.iter().position(|e| *e == DndEv::Enter(T1.0, T1.1)).unwrap();
    let idx_motion = b.events.iter().position(|e| *e == DndEv::Motion(T2.0, T2.1)).unwrap();
    let idx_leave = b.events.iter().position(|e| *e == DndEv::Leave).unwrap();
    let idx_enter2 = b.events.iter().position(|e| *e == DndEv::Enter(T3.0, T3.1)).unwrap();
    let idx_drop = b.events.iter().position(|e| *e == DndEv::Drop).unwrap();
    assert!(
        idx_enter1 < idx_motion && idx_motion < idx_leave && idx_leave < idx_enter2 && idx_enter2 < idx_drop,
        "DnD events in order [enter,motion,leave,enter,drop]; got {:?}", b.events,
    );
...
impl Dispatch<WlPointer, ()> for App {
    fn event(app: &mut Self, _: &WlPointer, e: <WlPointer as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        ...
        if let wl_pointer::Event::Button { serial, button, state, .. } = e {
            if button == BTN_LEFT && matches!(state, WEnum::Value(ButtonState::Pressed)) {
                if let (Some(dd), Some(source), Some(surface)) = (&app.dd, &app.source, &app.surface) {
...
impl Dispatch<WlDataSource, ()> for App {
    fn event(app: &mut Self, _: &WlDataSource, e: <WlDataSource as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
...
impl Dispatch<WlDataDevice, ()> for App {
    fn event(app: &mut Self, _: &WlDataDevice, e: <WlDataDevice as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
            wl_data_device::Event::Enter { serial, x, y, id, .. } => {
                ...
                app.events.push(DndEv::Enter(x.round() as i32, y.round() as i32));
            }
            wl_data_device::Event::Motion { x, y, .. } => {
                app.events.push(DndEv::Motion(x.round() as i32, y.round() as i32));
            }
...
impl Dispatch<WlDataOffer, ()> for App {
    fn event(app: &mut Self, _: &WlDataOffer, e: <WlDataOffer as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, WlSeat, XdgToplevel, WlDataDeviceManager);
