...
fn damage_partial_update() {
    ...
    assert_eq!(frame1.pixel(RX + 1, RY + 1).unwrap(), BASE, "frame1 is BASE inside the (not-yet-damaged) sub-rect");
    ...
    surface.commit();
    let frame2 = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.pixel_is(RX + RW / 2, RY + RH / 2, NEW))
    ...
    for y in 0..H {
        for x in 0..W {
            ...
            if in_sub(x, y) {
                ...
                assert_eq!(p2, p1, "pixel ({x},{y}) OUTSIDE sub-rect is byte-identical to frame1");
                assert_eq!(p2, BASE, "pixel ({x},{y}) outside sub-rect is still BASE");
                identical_outside += 1;
            }
        }
    }
    assert_eq!(changed_inside, (RW * RH) as u64, "exactly the sub-rect changed");
    assert_eq!(identical_outside, (W * H - RW * RH) as u64, "everything else is unchanged");
    ...
    surface.commit();
    let frame3 = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.serial > frame2.serial && f.pixel_is(1, 1, BASE))
        .expect("zero-damage re-present never captured");
    assert_eq!(frame3.rgba, frame2.rgba, "zero-damage re-present is byte-identical to the damaged frame");
