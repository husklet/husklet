...
use wayland_client::protocol::{
    ...
    wl_surface::WlSurface, wl_touch::{self, WlTouch},
};
...
fn touch_multitouch() {
    ...
    let mut app = App { surface: surface.clone(), buffer: buffer.clone(), drawn: false, frame_done: false, events: Vec::new() };
    ...
    h.input_tx.send(InputCommand::TouchDown { id: 0, x: 30.0, y: 40.0 }).unwrap();
    h.input_tx.send(InputCommand::TouchDown { id: 1, x: 90.0, y: 20.0 }).unwrap();
    h.input_tx.send(InputCommand::TouchFrame).unwrap();
    // move point 0, framed
    h.input_tx.send(InputCommand::TouchMotion { id: 0, x: 35.0, y: 45.0 }).unwrap();
    h.input_tx.send(InputCommand::TouchFrame).unwrap();
    // lift both, framed
    ...
    h.input_tx.send(InputCommand::TouchDown { id: 2, x: 50.0, y: 50.0 }).unwrap();
    ...
    while app.events.len() < want {
        let _ = queue.roundtrip(&mut app);
        assert!(Instant::now() < deadline, "touch events incomplete: {:?}", app.events);
        std::thread::sleep(Duration::from_millis(5));
    }

    let expected = vec![
        Ev::Down { id: 0, x: 30, y: 40 },
        Ev::Down { id: 1, x: 90, y: 20 },
        Ev::Frame,
        Ev::Motion { id: 0, x: 35, y: 45 },
        Ev::Frame,
        Ev::Up { id: 0 },
        Ev::Up { id: 1 },
        Ev::Frame,
        Ev::Down { id: 2, x: 50, y: 50 },
        Ev::Cancel,
    ];
    assert_eq!(app.events, expected, "exact multi-touch stream with per-point ids, coords, and frame grouping");
...
impl Dispatch<WlTouch, ()> for App {
    fn event(app: &mut Self, _: &WlTouch, e: <WlTouch as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
            wl_touch::Event::Down { id, x, y, .. } => {
                app.events.push(Ev::Down { id, x: x.floor() as i32, y: y.floor() as i32 });
            }
            wl_touch::Event::Motion { id, x, y, .. } => {
                app.events.push(Ev::Motion { id, x: x.floor() as i32, y: y.floor() as i32 });
            }
