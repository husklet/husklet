...
enum State { Idle, Pressed, Shifted }

fn state_color(s: State) -> [u8; 4] {
    match s { State::Idle => IDLE, State::Pressed => PRESSED, State::Shifted => SHIFTED }
}
...
impl App {
    fn recompute(&mut self, qh: &QueueHandle<App>) {
        ...
        let next = if self.shift_held { State::Shifted } else if self.a_held { State::Pressed } else { State::Idle };
        if next != self.state {
            self.state = next;
...
fn indicator_buffer(state: State) -> Vec<u8> {
    ...
    fill_rect(&mut buf, W, H, IX - IND / 2, IY - IND / 2, IND, IND, state_color(state));
    buf
}
...
fn key_drives_render() {
    ...
    let buffers = vec![
        (State::Idle, make_buffer(&shm, &qh, &h.runtime_dir, "idle", W, H, &indicator_buffer(State::Idle))),
        (State::Pressed, make_buffer(&shm, &qh, &h.runtime_dir, "pressed", W, H, &indicator_buffer(State::Pressed))),
        (State::Shifted, make_buffer(&shm, &qh, &h.runtime_dir, "shifted", W, H, &indicator_buffer(State::Shifted))),
    ];
    ...
    let mut app = App {
        surface: surface.clone(), buffers, drawn: false, frame_done: false,
        a_held: false, shift_held: false, state: State::Idle,
    };
    ...
    let idle = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(IX, IY, IDLE))
    ...
    let pressed = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.serial > s0 && f.pixel_is(IX, IY, PRESSED))
        .expect("PRESSED indicator never composited after KEY_A down");
    assert_eq!(pressed.pixel(IX, IY).unwrap(), PRESSED, "indicator PRESSED while KEY_A held");

    // ---- KEY_A up → IDLE composited ----
    let s1 = pressed.serial;
    h.input_tx.send(InputCommand::Key { keycode: KEY_A, pressed: false }).expect("A up");
    let idle2 = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.serial > s1 && f.pixel_is(IX, IY, IDLE))
        .expect("IDLE indicator never re-composited after KEY_A up");
    assert_eq!(idle2.pixel(IX, IY).unwrap(), IDLE, "indicator back to IDLE after KEY_A release");
    ...
    let shifted = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.serial > s2 && f.pixel_is(IX, IY, SHIFTED))
        .expect("SHIFTED indicator never composited after LEFTSHIFT down");
    assert_eq!(shifted.pixel(IX, IY).unwrap(), SHIFTED, "indicator SHIFTED while shift held");
    ...
    let idle3 = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.serial > s3 && f.pixel_is(IX, IY, IDLE))
        .expect("IDLE indicator never re-composited after LEFTSHIFT up");
    assert_eq!(idle3.pixel(IX, IY).unwrap(), IDLE, "indicator back to IDLE after shift release");
...
impl Dispatch<XdgSurface, ()> for App {
    fn event(app: &mut Self, xdg: &XdgSurface, e: <XdgSurface as Proxy>::Event, _: &(), _: &Connection, qh: &QueueHandle<Self>) {
        if let xdg_surface::Event::Configure { serial } = e {
            ...
            if !app.drawn {
                let buf = &app.buffers.iter().find(|(s, _)| *s == State::Idle).unwrap().1;
    ...
    fn event(app: &mut Self, _: &WlKeyboard, e: <WlKeyboard as Proxy>::Event, _: &(), _: &Connection, qh: &QueueHandle<Self>) {
        match e {
            wl_keyboard::Event::Key { key, state, .. } => {
                ...
                if key == KEY_A { app.a_held = pressed; }
                app.recompute(qh);
            }
