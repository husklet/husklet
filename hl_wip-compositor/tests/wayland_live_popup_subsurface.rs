...
use hl_compositor::adapter::smithay::{self, input_channel, CapturedFrame, InputCommand, PngPresenter};

use wayland_client::globals::{registry_queue_init, GlobalListContents};
use wayland_client::protocol::{
    wl_buffer::WlBuffer, wl_callback::WlCallback, wl_compositor::WlCompositor,
    wl_registry::WlRegistry, wl_seat::WlSeat, wl_shm::{self, WlShm}, wl_shm_pool::WlShmPool,
...
fn popup_and_subsurface_composite_at_placed_positions_and_grab_dismisses() {
    ...
    let subcompositor: WlSubcompositor =
        globals.bind(&qh, 1..=1, ()).expect("wl_subcompositor global");
    ...
    while !(app.tl_drawn && app.tl_released && app.tl_frame_done) {
        assert!(
            ...
            app.tl_drawn, app.tl_released, app.tl_frame_done,
        );
        queue.blocking_dispatch(&mut app).expect("client dispatch (map toplevel)");
    }
    assert!(
        wait_for(&captures, |f| f.width == TL_W && f.pixel_is(TL_W / 2, TL_H / 2, TL)),
    ...
    while popup_frame.is_none() {
        assert!(Instant::now() < deadline, "popup never composited at its placed position");
    ...
    assert_eq!((popup_frame.x, popup_frame.y), EXPECT_POP, "popup composited at the resolved placement");
    assert_eq!(popup_frame.pixel(POP_W / 2, POP_H / 2).unwrap(), POP, "popup is its own color");
    ...
    while sub_frame.is_none() {
        assert!(Instant::now() < deadline, "subsurface never composited at parent+offset");
        queue.blocking_dispatch(&mut app).ok();
        sub_frame = captures
            .lock()
            .unwrap()
    ...
    assert_eq!((sub_frame.x, sub_frame.y), SUB_POS, "subsurface composited at parent + set_position");
    assert_eq!(sub_frame.pixel(SUB_W / 2, SUB_H / 2).unwrap(), SUB, "subsurface is its own color");
    assert_ne!(SUB, TL, "subsurface color is distinct from the toplevel color");
    ...
    input_tx.send(InputCommand::PointerMotion { x: 5.0, y: 5.0 }).expect("send motion outside popup");
    input_tx.send(InputCommand::PointerButton { button: 0x110, pressed: true }).expect("send press");

    let deadline = Instant::now() + Duration::from_secs(5);
    while !app.pop_done {
        assert!(Instant::now() < deadline, "popup grab was never dismissed (no popup_done)");
        queue.roundtrip(&mut app).expect("client dispatch (dismiss)");
        std::thread::sleep(Duration::from_millis(20));
    }
    assert!(app.pop_done, "client received xdg_popup.popup_done on the outside press");
