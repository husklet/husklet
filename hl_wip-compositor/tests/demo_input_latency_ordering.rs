...
fn input_latency_ordering() {
    ...
    h.input_tx.send(InputCommand::PointerMotion { x: PX as f64, y: PY as f64 }).expect("enter");
    h.input_tx.send(InputCommand::PointerMotion { x: (PX + 20) as f64, y: PY as f64 }).expect("motion");
    h.input_tx.send(InputCommand::Key { keycode: KEY_A, pressed: true }).expect("key down");
    h.input_tx.send(InputCommand::Key { keycode: KEY_A, pressed: false }).expect("key up");
    ...
    loop {
        ...
        assert!(Instant::now() < deadline, "burst incomplete: events={:?} marker={}", app.events, marker_frame.is_some());
        std::thread::sleep(Duration::from_millis(5));
    }

    // ---- (a) EXACT injected order ----
    let got_kinds: Vec<EvKind> = app.events.iter().map(|(k, _)| *k).collect();
    assert_eq!(got_kinds, expected, "client received the burst in the exact injected order");
    ...
    assert_eq!(serials.len(), 6, "six events carry a serial (kbd enter, ptr enter, 2 keys, 2 buttons)");
    for w in serials.windows(2) {
        assert!(w[0] < w[1], "serials strictly increase: {} < {}", w[0], w[1]);
    }

    // ---- (c) the input-driven render landed at the very next present cycle ----
    let mf = marker_frame.unwrap();
    assert_eq!(mf.serial, baseline + 1, "marker rendered at the immediately following present cycle");
    assert_eq!(mf.pixel(PX, PY).unwrap(), MARKER, "marker present at the injected pointer position");
    assert_eq!(mf.pixel(1, 1).unwrap(), BASE, "background elsewhere stays BASE");
    ...
    assert_eq!(after_baseline, 1, "exactly one present after baseline (bounded, no redundant frames)");
    ...
    fn event(app: &mut Self, _: &WlPointer, e: <WlPointer as Proxy>::Event, _: &(), _: &Connection, qh: &QueueHandle<Self>) {
        match e {
            ...
            wl_pointer::Event::Button { serial, button, state, .. } => {
                ...
                app.events.push((EvKind::Button(button, pressed), Some(serial)));
            }
            _ => {}
        }
    }
