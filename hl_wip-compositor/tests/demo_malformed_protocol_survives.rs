...
fn malformed_protocol_survives() {
    ...
    let buffer = make_buffer(&shm, &qh, &h.runtime_dir, "recover", W, H, &solid(W, H, RECOVER));
    ...
    let mut app = Recover {
        surface: surface.clone(), buffer: buffer.clone(),
        configured: false, empty_committed: false, drew: false, frame_done: false,
    };
    ...
    while !(app.drew && app.frame_done) {
        assert!(Instant::now() < deadline, "recover client never mapped through the empty-commit path");
        queue.blocking_dispatch(&mut app).expect("recover dispatch");
    }
    let recover_frame = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(1, 1, RECOVER))
        .expect("recover client never composited after an empty commit");
    assert_eq!(recover_frame.pixel(W / 2, H / 2).unwrap(), RECOVER, "recovered client is solid magenta");
    assert!(app.empty_committed, "the no-buffer commit path was actually exercised");
    ...
    let recover_again = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.pixel_is(1, 1, RECOVER))
        .expect("recover client no longer serves after the fatal-error abuses");
    assert_eq!(recover_again.pixel(2, 2).unwrap(), RECOVER, "recover client unaffected by neighbors' abuse");
...
fn hostile_double_destroy() {
    ...
    let (globals, mut queue) = registry_queue_init::<Plain>(&conn).expect("double-destroy registry");
    let qh = queue.handle();
    let compositor: WlCompositor = globals.bind(&qh, 1..=6, ()).expect("wl_compositor");
    let surface = compositor.create_surface(&qh, ());
