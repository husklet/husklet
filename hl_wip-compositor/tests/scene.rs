...
use hl_compositor::scene::model::{
    ...
    Scene, SubsurfaceState, SurfaceRole, Visibility,
};
...
struct FakePresenter {
    ...
    visibility: Mutex<Vec<(SurfaceId, Visibility)>>,
}
impl FakePresenter {
...
impl Presenter for FakePresenter {
    fn present(
        ...
        let outcome = self.scripted.lock().unwrap().pop().unwrap_or_else(|| {
            ...
            PresentOutcome::Delivered { serial: s, timing: Some(timing) }
        });
        PresentationFeedback { outcome }
    }
    fn set_visibility(&mut self, surface: SurfaceId, visibility: Visibility) {
        self.visibility.lock().unwrap().push((surface, visibility));
    }
}
...
fn surface_commit_attaches_buffer_and_marks_damage() {
    ...
    assert!(scene.is_dirty(id), "attaching a buffer marks the surface dirty");
    ...
    assert!(!changed, "a frame-callback-only commit reports no content change");
    assert!(!scene.is_dirty(id), "a frame-callback-only commit does not dirty the surface");
    ...
    let changed = commit_surface(
        &mut scene,
        id,
        Commit { buffer: BufferChange::Keep, ..Commit::default() }.with_damage(Rect::new(10, 20, 30, 40)),
    );
    assert!(changed, "damage against a live buffer is a content change");
    assert!(scene.is_dirty(id));
    assert_eq!(scene.get(id).unwrap().damage.bounding_box(), Some(Rect::new(10, 20, 30, 40)));

    // Detach clears the buffer.
    let changed = commit_surface(&mut scene, id, Commit { buffer: BufferChange::Removed, ..Commit::default() });
...
fn compose_frame_walks_subsurfaces_and_popups_in_z_order() {
    ...
    scene.set_role(sub_bottom, SurfaceRole::Subsurface(SubsurfaceState { parent: top, x: 10, y: 20, sync: false }));
    commit_surface(&mut scene, sub_bottom, Commit::attach(shm(100, 100)));
    let sub_top = scene.create_surface();
    scene.set_role(sub_top, SurfaceRole::Subsurface(SubsurfaceState { parent: top, x: 30, y: 40, sync: false }));
    ...
    assert_eq!((placement.parent, placement.x, placement.y), (top, 200, 150));
    ...
    assert!(frame.damage().contains(&Rect::new(0, 0, 1000, 700)), "root full-surface damage");
    assert!(frame.damage().contains(&Rect::new(200, 150, 180, 240)), "popup damage in root space");
}
...
fn compositor_presents_the_tree_layers_in_order_through_the_presenter() {
    ...
    c.scene.set_role(sub, SurfaceRole::Subsurface(SubsurfaceState { parent: top, x: 30, y: 40, sync: false }));
    commit_surface(&mut c.scene, sub, Commit::attach(shm(100, 100)));
    let popup = c.scene.create_surface();
    c.scene.set_role(popup, popup_role(top, Rect::new(200, 150, 180, 240)));
    ...
    assert_eq!(c.presenter().present_order(), vec![top, sub, popup], "presenter recorded the z-order");
...
fn nested_popup_offsets_accumulate_up_the_chain() {
    ...
    let submenu_item = frame.items.iter().find(|i| i.image.surface == submenu).unwrap();
...
fn popup_placement_anchors_and_applies_gravity() {
    ...
    let p2 = Positioner { offset: (5, -7), ..p };
...
fn popup_placement_flips_then_slides_to_stay_on_screen() {
    ...
    let p = Positioner {
        ...
        constraint_adjustment: ConstraintAdjustment { flip_y: true, ..ConstraintAdjustment::NONE },
        offset: (0, 0),
    };
    ...
    assert_eq!(geo.right(), 500, "slide_x pushes the popup flush to the right edge");
...
fn popup_placement_resizes_when_it_cannot_fit() {
    ...
    let geo = place_popup(&p, 500, 400);
    assert!(geo.right() <= 500 && geo.x >= 0, "resized popup fits the target");
...
fn vsync_throttle_coalesces_commits_within_one_refresh_interval() {
    ...
    assert!(schedule::should_present(0, None, refresh), "first frame always due");
    assert!(!schedule::should_present(refresh / 2, Some(0), refresh), "half an interval later: not due");
    assert!(schedule::should_present(refresh, Some(0), refresh), "a full interval later: due");
    assert!(schedule::should_present(1_000, Some(0), 0), "unknown refresh: always due");
}
...
fn compositor_paces_presents_across_scripted_clock_times() {
    ...
    c.apply_commit(top, Commit { buffer: BufferChange::Keep, ..Commit::default() }.with_damage(Rect::new(0, 0, 10, 10)));
    let out = c.present_root(top);
    assert!(out.throttled, "a commit within one refresh interval is throttled");
    assert_eq!(c.presenter().count(), 1, "no new present while throttled");
    assert!(c.scene.is_dirty(top), "the throttled frame stays dirty for the next tick");

    // A full interval later the retained frame presents.
    c.clock().set(refresh);
    let out = c.present_root(top);
    assert_eq!(out.pacing, FramePacing::Presented);
    assert_eq!(c.presenter().count(), 2, "the coalesced frame presents once the interval elapses");
}
...
fn clean_tree_skips_present_but_still_fires_frame_callbacks() {
    ...
    assert_eq!(out.callbacks_fired, 1, "the frame-callback-only commit's callback fires");
}
...
fn failed_present_retains_callbacks_until_a_later_delivery() {
    ...
    let out = c.commit(top, Commit::attach(shm(800, 600)).into_frame_callback()).frame.unwrap();
    assert_eq!(out.pacing, FramePacing::RetryableFailure);
    assert_eq!(out.callbacks_fired, 0, "a failed present fires no callbacks");
    assert_eq!(c.retained_callbacks(top), 1, "the callback is retained for retry");
    assert!(c.scene.is_dirty(top), "a failed present keeps the tree dirty");

    // The next present delivers: the retained callback fires now.
    c.clock().set(1_000_000_000);
    let out = c.present_root(top);
    assert_eq!(out.pacing, FramePacing::Presented);
    assert_eq!(out.callbacks_fired, 1, "the retained callback fires on the next delivery");
...
fn invisible_root_does_not_present() {
    ...
    let out = c.present_root(top);
    assert_eq!(out.pacing, FramePacing::RetryableFailure);
    assert_eq!(c.presenter().count(), 0, "a minimized root never reaches the presenter");
}
...
fn pacing_policy_matches_the_ported_state_machine() {
    ...
    assert_eq!(service::from_outcome(PresentOutcome::Offscreen), FramePacing::RetryableFailure);
    assert_eq!(
        service::from_outcome(PresentOutcome::Delivered { serial: 9, timing: None }),
...
fn fully_occluded_surface_does_not_force_a_present() {
    ...
    scene.set_role(bg, SurfaceRole::Subsurface(SubsurfaceState { parent: top, x: 0, y: 0, sync: false }));
    commit_surface(&mut scene, bg, Commit::attach(shm(100, 100)));
    let fg = scene.create_surface();
    scene.set_role(fg, SurfaceRole::Subsurface(SubsurfaceState { parent: top, x: 0, y: 0, sync: false }));
    commit_surface(
        &mut scene,
        fg,
        Commit {
            buffer: BufferChange::New(BufferState { format: Format::Xrgb8888, ..shm(200, 200) }),
    ...
    c.present_root(top);
    assert!(!c.scene.any_dirty(), "the initial present clears the whole tree");
    ...
    assert!(!is_tree_dirty(&c.scene, top), "a surface fully behind an opaque cover is not visible");

    // Damaging the root (whose full 1000×700 rect the 200×200 cover cannot occlude) IS visible.
    c.apply_commit(
        top,
        Commit { buffer: BufferChange::Keep, ..Commit::default() }.with_damage(Rect::new(500, 500, 20, 20)),
    );
    assert!(is_tree_dirty(&c.scene, top), "an un-occluded surface forces a present");
}
...
fn pointer_hit_test_returns_topmost_input_surface() {
    ...
    scene.set_role(child, SurfaceRole::Subsurface(SubsurfaceState { parent: top, x: 100, y: 100, sync: false }));
