...
fn injected_pointer_and_keyboard_reach_the_focused_client() {
    ...
    assert!(caps.contains(Capability::Keyboard), "seat advertises keyboard, got {caps:?}");
    ...
    assert!(pointer.is_alive() && keyboard.is_alive(), "pointer + keyboard objects alive");
    ...
    input_tx.send(InputCommand::PointerMotion { x: PX, y: PY }).expect("send enter motion");
    input_tx.send(InputCommand::PointerMotion { x: PX2, y: PY2 }).expect("send tracking motion");
    input_tx.send(InputCommand::PointerButton { button: BTN_LEFT, pressed: true }).expect("send button");
    input_tx.send(InputCommand::PointerAxis { horizontal: 0.0, vertical: SCROLL }).expect("send axis");
    input_tx.send(InputCommand::Key { keycode: KEY_A, pressed: true }).expect("send key");
    ...
    while !app.input_complete() {
        assert!(
            ...
            app.pointer_enter, app.pointer_motion, app.pointer_button, app.keyboard_enter, app.keyboard_key,
        );
    ...
    assert!(enter_matched, "wl_pointer.enter named a different surface than the client's toplevel");
    assert_eq!((enter_x, enter_y), (PX, PY), "wl_pointer.enter surface-local coordinate");
    assert_eq!(app.pointer_motion.unwrap(), (PX2, PY2), "wl_pointer.motion surface-local coordinate");

    // Pointer button: BTN_LEFT, pressed.
    assert_eq!(app.pointer_button.unwrap(), (BTN_LEFT, true), "wl_pointer.button (button, pressed)");

    // Pointer axis: the injected downward scroll arrived as a positive vertical value.
    assert_eq!(app.pointer_axis_vert.unwrap(), SCROLL, "wl_pointer.axis vertical scroll value");

    // Keyboard entered our surface, then delivered the injected key (evdev KEY_A, pressed).
    assert_eq!(app.keyboard_enter.unwrap(), true, "wl_keyboard.enter named the client's surface");
    assert_eq!(app.keyboard_key.unwrap(), (KEY_A, true), "wl_keyboard.key (keycode, pressed)");
...
ignore_dispatch!(WlCompositor, WlSurface, WlShm, WlShmPool, XdgToplevel, WlOutput);
