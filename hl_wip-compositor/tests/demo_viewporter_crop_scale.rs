...
fn viewporter_crop_scale() {
    ...
    let viewporter: WpViewporter = globals.bind(&qh, 1..=1, ()).expect("wp_viewporter (adapter must advertise it)");

    let src_buffer = make_buffer(&shm, &qh, &h.runtime_dir, "vp-src", BUF, BUF, &source_buffer());
    ...
    let mut app = App { surface: surface.clone(), src_buffer: src_buffer.clone(), viewport: viewport.clone(), configured: false };
    ...
    let frame = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == DST.0 && f.height == DST.1 && f.pixel_is(0, 0, RED))
    ...
    assert_eq!(frame.pixel(0, 0).unwrap(), RED, "top-left corner == crop TL (red)");
    assert_eq!(frame.pixel(DST.0 - 1, 0).unwrap(), GREEN, "top-right corner == crop TR (green)");
    assert_eq!(frame.pixel(0, DST.1 - 1).unwrap(), BLUE, "bottom-left corner == crop BL (blue)");
    assert_eq!(frame.pixel(DST.0 - 1, DST.1 - 1).unwrap(), YELLOW, "bottom-right corner == crop BR (yellow)");
    // Center of the crop scales to the center of the destination.
    assert_eq!(frame.pixel(DST.0 / 2, DST.1 / 2).unwrap(), WHITE, "center == crop center (white)");
    // Interior background between markers is the crop's gray fill.
    assert_eq!(frame.pixel(30, 30).unwrap(), GRAY, "interior background is the crop gray");

    // The out-of-crop magenta must have been cropped away entirely.
    for y in 0..frame.height {
        for x in 0..frame.width {
            assert_ne!(frame.pixel(x, y).unwrap(), MAGENTA, "out-of-crop magenta leaked at ({x},{y})");
        }
    }
    ...
    fn event(app: &mut Self, xdg: &XdgSurface, e: <XdgSurface as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        if let xdg_surface::Event::Configure { serial } = e {
            ...
            if !app.configured {
                ...
                app.viewport.set_source(CROP.0 as f64, CROP.1 as f64, CROP.2 as f64, CROP.3 as f64);
