...
impl Client {
    fn redraw(&mut self, focused: bool, qh: &QueueHandle<Client>) {
        self.surface.attach(Some(if focused { &self.focused } else { &self.base }), 0, 0);
...
fn spawn(dir: &std::path::Path, tag: &str, color: [u8; 4]) -> (Connection, EventQueue<Client>, Client) {
    ...
    let base = make_buffer(&shm, &qh, dir, &format!("{tag}-base"), W, H, &solid(W, H, color));
    ...
    let mut app = Client {
        surface: surface.clone(), base, focused, base_color: color,
        drawn: false, frame_done: false, keys: Vec::new(),
    };
...
fn latest(caps: &std::sync::Arc<std::sync::Mutex<Vec<CapturedFrame>>>, base: [u8; 4], center: [u8; 4], after: u64) -> Option<CapturedFrame> {
    caps.lock().unwrap().iter().rev().find(|f| f.serial > after && f.width == W && f.pixel_is(1, 1, base) && f.pixel_is(CX, CY, center)).cloned()
}
...
fn await_focus_frame(
    ...
    qs: [&mut EventQueue<Client>; 3], apps: [&mut Client; 3],
    base: [u8; 4], center: [u8; 4], after: u64, what: &str,
) -> CapturedFrame {
    let [qa, qb, qc] = qs;
    let [a, b, c] = apps;
    let deadline = Instant::now() + Duration::from_secs(5);
...
fn focus_indicator_rerender() {
    ...
    await_focus_frame(&h.captures, [&mut qa, &mut qb, &mut qc], [&mut a, &mut b, &mut c], RED, RED, 0, "A unfocused");
    await_focus_frame(&h.captures, [&mut qa, &mut qb, &mut qc], [&mut a, &mut b, &mut c], GREEN, GREEN, 0, "B unfocused");
    await_focus_frame(&h.captures, [&mut qa, &mut qb, &mut qc], [&mut a, &mut b, &mut c], BLUE, BLUE, 0, "C unfocused");

    // ---- focus A → A's indicator goes WHITE ----
    h.input_tx.send(InputCommand::FocusToplevelIndex(0)).expect("focus A");
    let fa = await_focus_frame(&h.captures, [&mut qa, &mut qb, &mut qc], [&mut a, &mut b, &mut c], RED, WHITE, 0, "A focused indicator");
    assert_eq!(fa.pixel(CX, CY).unwrap(), WHITE, "A drew its focus indicator on gaining focus");
    ...
    let a_defocus = await_focus_frame(&h.captures, [&mut qa, &mut qb, &mut qc], [&mut a, &mut b, &mut c], RED, RED, fa.serial, "A defocused indicator");
    let fb = await_focus_frame(&h.captures, [&mut qa, &mut qb, &mut qc], [&mut a, &mut b, &mut c], GREEN, WHITE, 0, "B focused indicator");
    assert_eq!(a_defocus.pixel(CX, CY).unwrap(), RED, "A cleared its indicator when focus left it");
    assert_eq!(fb.pixel(CX, CY).unwrap(), WHITE, "B drew its focus indicator on gaining focus");
    assert!(a_defocus.serial > fa.serial, "A's defocus repaint is a later present than its focus repaint");

    // ---- key while B focused: only B ----
    h.input_tx.send(InputCommand::Key { keycode: KEY_A, pressed: true }).expect("key");
    let deadline = Instant::now() + Duration::from_secs(5);
    ...
    assert!(a.keys.is_empty(), "unfocused A received no key, got {:?}", a.keys);
    assert!(c.keys.is_empty(), "unfocused C received no key, got {:?}", c.keys);

    // ---- focus C → B clears, C goes WHITE ----
    h.input_tx.send(InputCommand::FocusToplevelIndex(2)).expect("focus C");
    let b_defocus = await_focus_frame(&h.captures, [&mut qa, &mut qb, &mut qc], [&mut a, &mut b, &mut c], GREEN, GREEN, fb.serial, "B defocused indicator");
    let fc = await_focus_frame(&h.captures, [&mut qa, &mut qb, &mut qc], [&mut a, &mut b, &mut c], BLUE, WHITE, 0, "C focused indicator");
    assert_eq!(b_defocus.pixel(CX, CY).unwrap(), GREEN, "B cleared its indicator when focus left it");
    assert_eq!(fc.pixel(CX, CY).unwrap(), WHITE, "C drew its focus indicator on gaining focus");
    save_composited("focus_indicator-final", 3 * W + 40, H, [0x10, 0x10, 0x10, 0xFF], &[(&a_defocus, 0, 0), (&b_defocus, W + 20, 0), (&fc, 2 * W + 40, 0)]);
...
impl Dispatch<WlKeyboard, ()> for Client {
    fn event(app: &mut Self, _: &WlKeyboard, e: <WlKeyboard as Proxy>::Event, _: &(), _: &Connection, qh: &QueueHandle<Self>) {
        match e {
            wl_keyboard::Event::Enter { surface, .. } if surface.id() == app.surface.id() => app.redraw(true, qh),
            wl_keyboard::Event::Leave { surface, .. } if surface.id() == app.surface.id() => app.redraw(false, qh),
