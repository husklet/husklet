...
fn keyboard_shortcuts_inhibit_grants_and_tracks() {
    ...
    let ksi_mgr: ZwpKeyboardShortcutsInhibitManagerV1 =
        globals.bind(&qh, 1..=1, ()).expect("zwp_keyboard_shortcuts_inhibit_manager_v1 advertised");

    let buffer = make_buffer(&shm, &qh, &h.runtime_dir, "ksi", W, H, &solid(W, H, [0x40, 0x40, 0x40, 0xFF]));
    ...
    let mut app =
        App { surface: surface.clone(), buffer, drawn: false, frame_done: false, inhibit_active: None };
    ...
    let inhibitor: ZwpKeyboardShortcutsInhibitorV1 = ksi_mgr.inhibit_shortcuts(&surface, &seat, &qh, ());
    let granted = poll(&mut queue, &mut app, 5, |app| {
        app.inhibit_active == Some(true)
            && h.observations.lock().unwrap().shortcuts_inhibited.contains(&pid)
    });
    assert!(
        granted,
    ...
    assert!(released, "inhibitor destroyed → surface {pid} no longer tracked as key-grabbed");
...
impl Dispatch<ZwpKeyboardShortcutsInhibitorV1, ()> for App {
    fn event(app: &mut Self, _: &ZwpKeyboardShortcutsInhibitorV1, e: <ZwpKeyboardShortcutsInhibitorV1 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
            zwp_keyboard_shortcuts_inhibitor_v1::Event::Active => app.inhibit_active = Some(true),
            zwp_keyboard_shortcuts_inhibitor_v1::Event::Inactive => app.inhibit_active = Some(false),
            _ => {}
        }
    }
}
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, XdgToplevel, WlSeat, ZwpKeyboardShortcutsInhibitManagerV1);
