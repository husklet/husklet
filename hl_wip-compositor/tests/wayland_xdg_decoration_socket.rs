...
fn real_client_negotiates_xdg_shell_configure_and_decoration_then_composites() {
    ...
    let deco_mgr: ZxdgDecorationManagerV1 =
        globals.bind(&qh, 1..=1, ()).expect("zxdg_decoration_manager_v1 global");
    ...
    assert!(tw > 0 && th > 0, "xdg_toplevel.configure must carry a non-zero size, got {tw}x{th}");
    assert!(
...
ignore_dispatch!(WlCompositor, WlSurface, WlShm, WlShmPool, ZxdgDecorationManagerV1);
