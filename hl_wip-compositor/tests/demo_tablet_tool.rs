...
fn tablet_tool() {
    ...
    let tablet_mgr: ZwpTabletManagerV2 = globals.bind(&qh, 1..=1, ()).expect("zwp_tablet_manager_v2");
    ...
    let mut app = App {
        ...
        tool_added: false, events: Vec::new(),
    };
    ...
    while !app.tool_added {
        ...
        queue.blocking_dispatch(&mut app).expect("dispatch tool_added");
    }

    // ---- drive a full stylus interaction ----
    h.input_tx.send(InputCommand::TabletToolProximityIn { x: 60.0, y: 40.0 }).unwrap();
    h.input_tx.send(InputCommand::TabletToolMotion { x: 70.0, y: 55.0, pressure: 0.6 }).unwrap();
    h.input_tx.send(InputCommand::TabletToolTipDown).unwrap();
    h.input_tx.send(InputCommand::TabletToolMotion { x: 80.0, y: 65.0, pressure: 0.8 }).unwrap();
    h.input_tx.send(InputCommand::TabletToolTipUp).unwrap();
    h.input_tx.send(InputCommand::TabletToolProximityOut).unwrap();
    ...
    while app.events.len() < want {
        ...
        assert!(Instant::now() < deadline, "tablet events incomplete: {:?}", app.events);
        std::thread::sleep(Duration::from_millis(5));
    }
    assert_eq!(app.events, expected, "exact stylus interaction: proximity, motion, pressure, tip, out");
...
impl Dispatch<ZwpTabletSeatV2, ()> for App {
    fn event(_: &mut Self, _: &ZwpTabletSeatV2, _: <ZwpTabletSeatV2 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {}
...
impl Dispatch<ZwpTabletV2, ()> for App {
    fn event(_: &mut Self, _: &ZwpTabletV2, _: <ZwpTabletV2 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {}
}
impl Dispatch<ZwpTabletToolV2, ()> for App {
    fn event(app: &mut Self, _: &ZwpTabletToolV2, e: <ZwpTabletToolV2 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        use zwp_tablet_tool_v2::Event;
        match e {
            Event::Done => app.tool_added = true,
            Event::ProximityIn { surface, .. } => {
                app.events.push(Ev::ProximityIn { on_surface: surface == app.surface });
            }
            Event::ProximityOut => app.events.push(Ev::ProximityOut),
            Event::Down { .. } => app.events.push(Ev::Down),
            Event::Up => app.events.push(Ev::Up),
            Event::Motion { x, y } => app.events.push(Ev::Motion { x: x.floor() as i32, y: y.floor() as i32 }),
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, WlSeat, XdgToplevel, ZwpTabletManagerV2);
