...
//!
//!  1. **Advertisement is truthful.** The adapter advertises `zwp_linux_dmabuf_v1` (a v4/v5 feedback
//!     global). A v3 binder receives the `modifier` events; this demo binds at v3 and asserts the exact
//!     format table it must expose so a probing toolkit gets a correct answer: `ARGB8888` + `XRGB8888`,
//!     both with the `LINEAR` modifier. A second bind at v4 exercises Chrome's feedback path and asserts
//!     the feedback carries a `main_device` (the synthetic headless render node) and a `format_table`.
//!
...
// The synthetic headless render node the adapter's v4 feedback names as `main_device`: makedev(226, 128).
const HEADLESS_RENDER_NODE: u64 = (226u64 << 8) | 128;
...
struct App {
    ...
    /// The `main_device` dev_t decoded from the v4 feedback (little-endian).
    fb_main_device: Option<u64>,
    /// The v4 feedback `format_table` size (bytes) — non-zero proves a real table was provided.
    fb_table_size: Option<u32>,
    /// Whether the v4 feedback delivered its terminating `done`.
    fb_done: bool,
}
...
fn dmabuf_present() {
    ...
    let dmabuf_v3: ZwpLinuxDmabufV1 =
        globals.bind(&qh, 3..=3, ()).expect("zwp_linux_dmabuf_v1 (v3) — adapter must advertise it");
    // Bind a second instance at v4 and request default feedback — Chrome's ozone/GPU path.
    let dmabuf_v4: ZwpLinuxDmabufV1 =
        globals.bind(&qh, 4..=4, ()).expect("zwp_linux_dmabuf_v1 (v4) feedback");
    let feedback: ZwpLinuxDmabufFeedbackV1 = dmabuf_v4.get_default_feedback(&qh, ());
    ...
    let buffer: WlBuffer =
        params.create_immed(W, H, DRM_FORMAT_ARGB8888, Flags::empty(), &qh, ());
    ...
    assert!(
        app.formats_v3.contains(&(DRM_FORMAT_ARGB8888, DRM_FORMAT_MOD_LINEAR)),
        "adapter must advertise ARGB8888 LINEAR; got {:?}",
        app.formats_v3
    );
    assert!(
        app.formats_v3.contains(&(DRM_FORMAT_XRGB8888, DRM_FORMAT_MOD_LINEAR)),
        "adapter must advertise XRGB8888 LINEAR; got {:?}",
        app.formats_v3
    );

    // ---- (1b) v4 feedback (Chrome's path) is live and coherent ----
    assert!(app.fb_done, "v4 dmabuf feedback never delivered `done`");
    assert_eq!(
        app.fb_main_device,
        Some(HEADLESS_RENDER_NODE),
        "v4 feedback must name the synthetic headless render node as main_device"
    );
    assert!(
        app.fb_table_size.unwrap_or(0) >= 16,
        "v4 feedback format_table must be a real (>= one 16-byte pair) table; got {:?}",
        app.fb_table_size
    );
    ...
    let _ = (&feedback, &params, &buffer, &toplevel, &xdg, &surface, &dmabuf_v3, &dmabuf_v4, &backing);
...
impl Dispatch<ZwpLinuxDmabufV1, ()> for App {
    fn event(app: &mut Self, _: &ZwpLinuxDmabufV1, e: <ZwpLinuxDmabufV1 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
...
impl Dispatch<ZwpLinuxDmabufFeedbackV1, ()> for App {
    fn event(app: &mut Self, _: &ZwpLinuxDmabufFeedbackV1, e: <ZwpLinuxDmabufFeedbackV1 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
            zwp_linux_dmabuf_feedback_v1::Event::MainDevice { device } => {
                if device.len() == 8 {
                    let mut b = [0u8; 8];
                    b.copy_from_slice(&device);
                    app.fb_main_device = Some(u64::from_le_bytes(b));
                }
            }
            zwp_linux_dmabuf_feedback_v1::Event::FormatTable { fd: _, size } => {
                app.fb_table_size = Some(size);
            }
...
ignore!(WlCompositor, WlSurface, WlBuffer, XdgToplevel, ZwpLinuxBufferParamsV1);
