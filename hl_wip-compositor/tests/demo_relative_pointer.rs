...
fn relative_pointer() {
    ...
    let rel_mgr: ZwpRelativePointerManagerV1 =
        globals.bind(&qh, 1..=1, ()).expect("zwp_relative_pointer_manager_v1 advertised");
    ...
    pump_while(&mut queue, &mut app, 5, |a| a.entered && !a.deltas.is_empty());
    assert!(app.entered, "pointer entered the surface");

    // Inject the move sequence.
    for &(x, y) in MOVES {
        h.input_tx.send(InputCommand::PointerMotion { x, y }).expect("motion");
    }
    ...
    assert_eq!(app.deltas.len(), expected.len(), "one relative_motion per move, got {:?}", app.deltas);
    for (i, ((dx, dy, dxu, dyu), (ex, ey))) in app.deltas.iter().zip(expected.iter()).enumerate() {
        assert_eq!((*dx, *dy), (*ex, *ey), "relative delta #{i} exact (with sign): got ({dx},{dy}) want ({ex},{ey})");
        // The headless adapter applies no acceleration: unaccelerated == accelerated.
        assert_eq!((*dxu, *dyu), (*ex, *ey), "unaccelerated delta #{i} equals accelerated");
    }
...
impl Dispatch<ZwpRelativePointerV1, ()> for App {
    fn event(app: &mut Self, _: &ZwpRelativePointerV1, e: <ZwpRelativePointerV1 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        if let zwp_relative_pointer_v1::Event::RelativeMotion { dx, dy, dx_unaccel, dy_unaccel, .. } = e {
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, WlSeat, XdgToplevel, ZwpRelativePointerManagerV1);
