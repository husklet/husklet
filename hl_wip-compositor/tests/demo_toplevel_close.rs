...
fn toplevel_close() {
    ...
    let buffer = make_buffer(&shm, &qh, &h.runtime_dir, "close", W, H, &solid(W, H, COLOR));
    ...
    let mut app = App { surface: surface.clone(), buffer, drawn: false, frame_done: false, close_received: 0 };
    ...
    assert!(
        pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(1, 1, COLOR)).is_some(),
        "toplevel first frame never composited",
    );

    // ---- the compositor asks the toplevel to close ----
    h.input_tx.send(InputCommand::CloseTopmostToplevel).expect("send close");

    let deadline = Instant::now() + Duration::from_secs(5);
    while app.close_received == 0 {
        assert!(Instant::now() < deadline, "client never received xdg_toplevel.close");
        let _ = queue.roundtrip(&mut app);
        std::thread::sleep(Duration::from_millis(5));
    }
    assert_eq!(app.close_received, 1, "the client received EXACTLY one xdg_toplevel.close");
    ...
    let mut neighbor = Neighbor::map(&h.runtime_dir, "after-close", 100, 80, [0x20, 0xE0, 0x20, 0xFF]);
