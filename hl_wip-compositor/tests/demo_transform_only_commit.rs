...
fn corners(f: &CapturedFrame) -> ([u8; 4], [u8; 4], [u8; 4], [u8; 4]) {
    ...
    (f.pixel(2, 2).unwrap(), f.pixel(w - 3, 2).unwrap(), f.pixel(2, h - 3).unwrap(), f.pixel(w - 3, h - 3).unwrap())
}
...
fn transform_only_commit() {
    ...
    let buffer = make_buffer(&shm, &qh, &h.runtime_dir, "toc", BUF_W, BUF_H, &source_buffer());
    ...
    assert_eq!((upright.width, upright.height), (BUF_W, BUF_H), "phase 1 is upright dimensions");
    let (tl, tr, bl, br) = corners(&upright);
    assert_eq!((tl, tr, bl, br), (RED, GREEN, BLUE, YELLOW), "phase 1 corners upright");
    ...
    assert_eq!((rotated.width, rotated.height), (BUF_H, BUF_W), "phase 2 swaps dimensions (90°)");
    assert_eq!((rotated.logical_width, rotated.logical_height), (BUF_H, BUF_W), "phase 2 swaps logical size");
