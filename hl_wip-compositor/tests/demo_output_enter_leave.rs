...
use wayland_client::protocol::{
    ...
    wl_registry::WlRegistry, wl_shm::WlShm, wl_shm_pool::WlShmPool, wl_surface::{self, WlSurface},
};
...
fn output_enter_leave() {
    ...
    let mapped = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(W / 2, H / 2, COL))
    ...
    while app.enters.is_empty() {
        assert!(Instant::now() < deadline, "no wl_surface.enter after mapping");
        let _ = queue.roundtrip(&mut app);
        std::thread::sleep(Duration::from_millis(5));
    }
    assert_eq!(app.enters, vec![output_id.clone()], "map delivers exactly one enter naming the advertised wl_output");
    ...
    while app.leaves.is_empty() {
        assert!(Instant::now() < deadline, "no wl_surface.leave after unmapping");
        let _ = queue.roundtrip(&mut app);
        std::thread::sleep(Duration::from_millis(5));
    }
    assert_eq!(app.leaves, vec![output_id.clone()], "unmap delivers exactly one leave for the same wl_output");
    ...
    while app.enters.len() < 2 {
        assert!(Instant::now() < deadline, "no second wl_surface.enter after re-mapping");
        let _ = queue.roundtrip(&mut app);
        std::thread::sleep(Duration::from_millis(5));
    }
    assert_eq!(app.enters, vec![output_id.clone(), output_id.clone()], "re-map delivers a second enter for the same output");
    assert_eq!(app.leaves.len(), 1, "exactly one leave over the whole cycle");
...
impl Dispatch<WlSurface, ()> for App {
    fn event(app: &mut Self, s: &WlSurface, e: <WlSurface as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        let _ = s;
        match e {
...
ignore!(WlCompositor, WlShm, WlShmPool, WlBuffer, WlOutput, XdgToplevel);
