...
fn damage_region_precision() {
    ...
    assert!(bw >= 80, "union bbox width {bw} spans both adjacent rects (>= 80)");
    ...
    assert_eq!(changed, union_area, "changed-pixel count equals the exact union area (overlap once, no seam)");
    ...
    app.surface.commit();
    let frame3 = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.serial > frame2.serial)
    ...
    assert_eq!((frame3.x, frame3.y), (OVER_INB.0, OVER_INB.1), "overhang damage origin clamped to surface");
    ...
    for y in 0..H {
        for x in 0..W {
            ...
            if in_over_inbounds(x, y) {
                assert_eq!(p3, NEW2, "pixel ({x},{y}) in the in-bounds overhang is NEW2");
                assert_ne!(p3, p2, "pixel ({x},{y}) in the in-bounds overhang changed");
                over_changed += 1;
            } else {
                assert_eq!(p3, p2, "pixel ({x},{y}) outside the in-bounds overhang is byte-identical to frame2");
            }
        }
    }
    assert_eq!(
    ...
    app.surface.commit();
    let frame4 = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.serial > frame3.serial && f.pixel_is(1, 1, BASE))
        .expect("zero-damage re-present never captured");
    assert_eq!(frame4.rgba, frame3.rgba, "zero-damage re-present is byte-identical to the damaged frame");
