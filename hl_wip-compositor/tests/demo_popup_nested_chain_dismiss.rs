...
enum Role { Toplevel, Popup1, Popup2 }
...
fn is_p1(f: &CapturedFrame) -> bool { f.width == P1_W && f.height == P1_H && f.pixel_is(P1_W / 2, P1_H / 2, P1) }
fn is_p2(f: &CapturedFrame) -> bool { f.width == P2_W && f.height == P2_H && f.pixel_is(P2_W / 2, P2_H / 2, P2) }
fn is_tl(f: &CapturedFrame) -> bool { f.width == TL_W && f.height == TL_H && f.pixel_is(TL_W / 2, TL_H / 2, TL) }
...
fn popup_nested_chain_dismiss() {
    ...
    let p1_buffer = make_buffer(&shm, &qh, &h.runtime_dir, "p1", P1_W, P1_H, &solid(P1_W, P1_H, P1));
    let p2_buffer = make_buffer(&shm, &qh, &h.runtime_dir, "p2", P2_W, P2_H, &solid(P2_W, P2_H, P2));
    ...
    let mut app = App {
        tl_surface: tl_surface.clone(), tl_buffer, tl_drawn: false, tl_frame_done: false,
        p1_surface: p1_surface.clone(), p1_buffer, p1_xdg: None, p1_popup: None, p1_drawn: false,
        p2_surface: p2_surface.clone(), p2_buffer, p2_xdg: None, p2_popup: None, p2_drawn: false,
    ...
    assert!(pump_until(&mut queue, &mut app, &h.captures, 5, is_tl).is_some(), "toplevel never composited");
    ...
    p1_surface.commit();
    assert!(pump_until(&mut queue, &mut app, &h.captures, 5, is_p1).is_some(), "menu popup never composited");
    ...
    p2_surface.commit();
    let submenu = pump_until(&mut queue, &mut app, &h.captures, 5, is_p2).expect("submenu popup never composited");
    let boundary = submenu.serial;
    save_composited("popup_nested_chain-open", TL_W, TL_H, TL, &[(&submenu, 100, 100)]);

    // ---- press OUTSIDE the whole chain → both dismissed, innermost first ----
    h.input_tx.send(InputCommand::PointerMotion { x: 5.0, y: 5.0 }).expect("motion outside");
    ...
    assert_eq!(app.dismiss_order, vec![2, 1], "popup_done arrived innermost-first (submenu 2 before menu 1)");
    let _ = queue.roundtrip(&mut app);

    // ---- both popups gone from the composited output ----
    let after = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.serial > boundary && is_tl(f))
        .expect("toplevel never re-presented after the chain dismissed");
    let caps = h.captures.lock().unwrap();
    assert!(!caps.iter().any(|f| f.serial > boundary && is_p1(f)), "menu popup was still composited after dismissal");
    assert!(!caps.iter().any(|f| f.serial > boundary && is_p2(f)), "submenu popup was still composited after dismissal");
    drop(caps);
    save_composited("popup_nested_chain-closed", TL_W, TL_H, TL, &[(&after, 0, 0)]);
...
impl Dispatch<XdgPopup, u8> for App {
    fn event(app: &mut Self, _: &XdgPopup, e: <XdgPopup as Proxy>::Event, id: &u8, _: &Connection, _: &QueueHandle<Self>) {
        if let xdg_popup::Event::PopupDone = e {
            app.dismiss_order.push(*id);
            // Tear down this popup in protocol order.
            if *id == 2 {
                if let Some(p) = app.p2_popup.take() { p.destroy(); }
                if let Some(x) = app.p2_xdg.take() { x.destroy(); }
                app.p2_surface.destroy();
            } else {
                if let Some(p) = app.p1_popup.take() { p.destroy(); }
                if let Some(x) = app.p1_xdg.take() { x.destroy(); }
...
impl Dispatch<WlCallback, ()> for App {
    fn event(app: &mut Self, _: &WlCallback, e: <WlCallback as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        if let wayland_client::protocol::wl_callback::Event::Done { .. } = e { app.tl_frame_done = true; }
    }
}
macro_rules! ignore {
    ($($t:ty),*) => {$(
