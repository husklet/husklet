...
struct Flooder { surface: WlSurface, buffer: WlBuffer, drawn: bool, done: u32 }
...
fn spawn_flooder(dir: &std::path::Path) -> (Connection, EventQueue<Flooder>, Flooder) {
    ...
    surface.commit();
    let mut app = Flooder { surface: surface.clone(), buffer, drawn: false, done: 0 };
...
fn spawn_victim(dir: &std::path::Path, qh_out: &mut Option<QueueHandle<Victim>>) -> (Connection, EventQueue<Victim>, Victim, WlSeat) {
    ...
    let mut app = Victim {
        ...
        drawn: false, frame_done: false, marker_drawn: false,
    };
    let deadline = Instant::now() + Duration::from_secs(5);
...
fn flood_fairness_latency() {
    ...
    while victim_marker.is_none() {
        ...
        let _ = qb.roundtrip(&mut b);
        if let Some(f) = h.captures.lock().unwrap().iter().rev().find(|f| f.pixel_is(PX, PY, B_MARK)).cloned() {
            t1 = Some(Instant::now());
            victim_marker = Some(f);
        }
        assert!(Instant::now() < deadline, "victim's input never presented under flood (PRIORITY INVERSION)");
    }
    ...
    assert_eq!(mf.pixel(PX, PY).unwrap(), B_MARK, "victim's marker at the injected pointer position");
    assert_eq!(mf.pixel(1, 1).unwrap(), B_BASE, "victim frame is its own content (not the flooder's)");

    // A really was flooding (many commits happened during B's single-input cycle).
    assert!(flood_iters >= 1, "flooder committed during the victim's cycle (iters={flood_iters})");
...
impl Dispatch<WlCallback, ()> for Flooder {
    fn event(app: &mut Self, _: &WlCallback, e: <WlCallback as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        if let wayland_client::protocol::wl_callback::Event::Done { .. } = e { app.done += 1; }
    }
}
...
ignore_a!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, XdgToplevel);
...
ignore_b!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, WlSeat, XdgToplevel);
