...
fn frame_pacing_skip() {
    ...
    let mut app = App { surface: surface.clone(), base_buffer: base_buffer.clone(), drawn: false, dones: 0, mapped_done: false };
    ...
    let f0 = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(1, 1, C0))
    ...
    app.surface.commit();
    let f1 = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.pixel_is(1, 1, C1))
        .expect("damaged commit (C1) never presented");
    assert_eq!(f1.serial, 2, "first damaged commit is present #2");
    assert_eq!(h.captures.lock().unwrap().len(), 2, "two presents after the first damaged commit");
    ...
    app.surface.commit();
    let f2 = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.pixel_is(1, 1, C2))
    ...
    assert_eq!(caps.len(), 3, "exactly 3 presents (map + 2 damaged); the 2 no-damage commits skipped");
    assert_eq!(caps.iter().map(|f| f.serial).collect::<Vec<_>>(), vec![1, 2, 3], "dense monotonic serials");
