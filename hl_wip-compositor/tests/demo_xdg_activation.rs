...
fn activation_focuses_target_toplevel() {
    ...
    let activation: XdgActivationV1 =
        globals.bind(&qh, 1..=1, ()).expect("xdg_activation_v1 advertised");
    ...
    for (i, color) in [[0xC0, 0x40, 0x40, 0xFF], [0x40, 0x40, 0xC0, 0xFF]].into_iter().enumerate() {
        let buffer = make_buffer(&shm, &qh, &h.runtime_dir, &format!("win{i}"), W, H, &solid(W, H, color));
    ...
    assert!(got_a, "activation of A focused A; enter_log={:?}", app.enter_log);
    assert_eq!(app.enter_log.last(), Some(&pid_a), "the enter targeted EXACTLY surface A");
    assert!(!app.enter_log.contains(&pid_b), "B was never focused by A's activation");
    ...
    assert!(got_b, "activation of B focused B; enter_log={:?} leave_log={:?}", app.enter_log, app.leave_log);
    assert!(app.leave_log.contains(&pid_a), "A received wl_keyboard.leave when B was activated");
    assert_eq!(app.enter_log.last(), Some(&pid_b), "the last enter targeted EXACTLY surface B");

    for t in toplevels { std::mem::forget(t); }
    for x in xdgs { std::mem::forget(x); }
    std::mem::forget(_kbd);
    h.shutdown();
}
...
fn mint_token(
    ...
    while app.last_token.is_none() {
        assert!(Instant::now() < deadline, "compositor never answered xdg_activation_token_v1.done");
...
fn poll(queue: &mut EventQueue<App>, app: &mut App, secs: u64, pred: impl Fn(&App) -> bool) -> bool {
...
impl Dispatch<XdgSurface, usize> for App {
    fn event(app: &mut Self, xdg: &XdgSurface, e: <XdgSurface as Proxy>::Event, idx: &usize, _: &Connection, qh: &QueueHandle<Self>) {
...
impl Dispatch<WlCallback, usize> for App {
    fn event(app: &mut Self, _: &WlCallback, e: <WlCallback as Proxy>::Event, idx: &usize, _: &Connection, _: &QueueHandle<Self>) {
        if let wayland_client::protocol::wl_callback::Event::Done { .. } = e { app.frame_done[*idx] = true; }
    }
}
impl Dispatch<XdgActivationTokenV1, ()> for App {
    fn event(app: &mut Self, _: &XdgActivationTokenV1, e: <XdgActivationTokenV1 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, WlSeat, XdgToplevel, XdgActivationV1);
