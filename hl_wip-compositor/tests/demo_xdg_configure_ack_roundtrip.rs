...
fn present_with(caps: &std::sync::Arc<std::sync::Mutex<Vec<CapturedFrame>>>, col: [u8; 4]) -> Option<CapturedFrame> {
    caps.lock().unwrap().iter().rev().find(|f| f.width == W && f.height == H && f.pixel_is(W / 2, H / 2, col)).cloned()
}
...
fn xdg_configure_ack_roundtrip() {
    ...
    let map_buffer = make_buffer(&shm, &qh, &h.runtime_dir, "cfg-map", W, H, &solid(W, H, MAP_COL));
    let max_buffer = make_buffer(&shm, &qh, &h.runtime_dir, "cfg-max", W, H, &solid(W, H, MAX_COL));
    ...
    while app.configure_count < 1 {
        assert!(Instant::now() < deadline, "no initial xdg_surface.configure arrived");
        queue.blocking_dispatch(&mut app).expect("dispatch initial configure");
    }
    let serial1 = app.pending_serial.expect("initial configure carried a serial");
    // EXACT initial configure: floating size + states == [Activated].
    assert_eq!(app.tl_size, Some(FLOATING), "initial configure size is the floating size");
    assert_eq!(app.tl_states, vec![STATE_ACTIVATED], "a freshly-mapped focused toplevel is exactly Activated");

    // Content must NOT be on screen before the ack: we have attached no buffer and not acked.
    assert!(present_with(&h.captures, MAP_COL).is_none(), "no content before ack_configure");
    ...
    let mapped = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(W / 2, H / 2, MAP_COL))
        .expect("mapped content never presented after ack_configure");
    assert_eq!(mapped.pixel(W / 2, H / 2).unwrap(), MAP_COL, "mapped content color");
    ...
    while app.configure_count <= before {
        assert!(Instant::now() < deadline, "set_maximized produced no new configure");
        queue.blocking_dispatch(&mut app).expect("dispatch maximize configure");
    }
    let serial2 = app.pending_serial.expect("maximize configure carried a serial");
    assert!(serial2 > serial1, "maximize configure serial ({serial2}) is newer than the initial ({serial1})");
    assert_eq!(app.tl_size, Some(OUTPUT_LOGICAL), "maximize configure size is the output logical size");
    let mut states = app.tl_states.clone();
    states.sort_unstable();
    assert_eq!(states, vec![STATE_MAXIMIZED, STATE_ACTIVATED], "maximized configure states are exactly Maximized + Activated");
    ...
    let maxed = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(W / 2, H / 2, MAX_COL))
        .expect("maximized content never presented after ack_configure(serial2)");
    assert_eq!(maxed.pixel(W / 2, H / 2).unwrap(), MAX_COL, "maximized content color");
    assert!(maxed.serial > mapped.serial, "maximized frame ({}) presents after the mapped frame ({})", maxed.serial, mapped.serial);
...
impl Dispatch<XdgSurface, ()> for App {
    fn event(app: &mut Self, _: &XdgSurface, e: <XdgSurface as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
...
impl Dispatch<XdgToplevel, ()> for App {
    fn event(app: &mut Self, _: &XdgToplevel, e: <XdgToplevel as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        if let xdg_toplevel::Event::Configure { width, height, states } = e {
