...
fn content_type_hint_read_at_commit() {
    ...
    let ct_mgr: WpContentTypeManagerV1 =
        globals.bind(&qh, 1..=1, ()).expect("wp_content_type_manager_v1 advertised");

    let buffer = make_buffer(&shm, &qh, &h.runtime_dir, "media", W, H, &solid(W, H, [0x80, 0x40, 0xC0, 0xFF]));
    ...
    assert!(saw_video, "compositor read the `video` content-type hint at commit (got {:?})",
    ...
    assert!(saw_game, "compositor re-read the `game` content-type hint after re-commit (got {:?})",
        h.observations.lock().unwrap().content_type.get(&pid));
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, XdgToplevel, WpContentTypeManagerV1, WpContentTypeV1);
