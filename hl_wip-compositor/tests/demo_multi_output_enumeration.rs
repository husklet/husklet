...
fn multi_output_enumeration() {
    ...
    let xdg_output_mgr: ZxdgOutputManagerV1 =
        globals.bind(&qh, 1..=3, ()).expect("zxdg_output_manager_v1 (adapter must advertise it)");
    ...
    let mut app = App { outputs: vec![OutputInfo::default(); count] };

    // Pump until every output has reported all five field groups.
    let deadline = Instant::now() + Duration::from_secs(5);
    while !app.outputs.iter().all(OutputInfo::complete) {
        assert!(Instant::now() < deadline, "output enumeration incomplete: {:?}", summarize(&app));
        queue.blocking_dispatch(&mut app).expect("dispatch output enumeration");
    }

    // Identify the monitors by their distinct mode WIDTH (order of global enumeration is not guaranteed).
    let a = app.outputs.iter().find(|o| o.mode.unwrap().0 == 1920).expect("output A (1920 wide) advertised");
    let b = app.outputs.iter().find(|o| o.mode.unwrap().0 == 2560).expect("output B (2560 wide) advertised");
    ...
    assert_eq!((amake.as_str(), amodel.as_str()), ("hl", "hl-virtual"), "A make/model");
    assert_eq!(a.mode.unwrap(), (1920, 1080, 60_000), "A mode is 1920×1080@60000 mHz");
    assert_eq!(a.scale.unwrap(), 1, "A scale is 1");
    assert_eq!(a.logical_position.unwrap(), (0, 0), "A xdg logical position is the origin");
    assert_eq!(a.logical_size.unwrap(), (1920, 1080), "A xdg logical size == mode ÷ scale 1");

    // ---- Output B: 2560×1440@1920,0, scale 2 ----
    let (bx, by, bt, ref bmake, ref bmodel) = b.geometry.clone().unwrap();
    assert_eq!((bx, by), (1920, 0), "B geometry position is to the right of A (x=1920)");
    assert_eq!(bt, Transform::Normal, "B is not rotated");
    assert_eq!((bmake.as_str(), bmodel.as_str()), ("hl", "hl-virtual"), "B make/model");
    assert_eq!(b.mode.unwrap(), (2560, 1440, 60_000), "B mode is 2560×1440@60000 mHz (physical, unscaled)");
    assert_eq!(b.scale.unwrap(), 2, "B scale is 2");
    assert_eq!(b.logical_position.unwrap(), (1920, 0), "B xdg logical position is (1920, 0)");
    assert_eq!(b.logical_size.unwrap(), (1280, 720), "B xdg logical size == mode ÷ scale 2 == 1280×720");
    ...
    fn event(app: &mut Self, _: &WlOutput, e: <WlOutput as Proxy>::Event, idx: &usize, _: &Connection, _: &QueueHandle<Self>) {
        ...
        match e {
            wl_output::Event::Geometry { x, y, make, model, transform, .. } => {
                info.geometry = Some((x, y, transform.into_result().unwrap_or(Transform::Normal), make, model));
            }
...
impl Dispatch<ZxdgOutputV1, usize> for App {
    fn event(app: &mut Self, _: &ZxdgOutputV1, e: <ZxdgOutputV1 as Proxy>::Event, idx: &usize, _: &Connection, _: &QueueHandle<Self>) {
        let info = &mut app.outputs[*idx];
        match e {
            zxdg_output_v1::Event::LogicalPosition { x, y } => info.logical_position = Some((x, y)),
            zxdg_output_v1::Event::LogicalSize { width, height } => info.logical_size = Some((width, height)),
            _ => {}
        }
    }
}
