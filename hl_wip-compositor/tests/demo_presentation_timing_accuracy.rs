...
fn presentation_timing_accuracy() {
    ...
    let buf_a = make_buffer(&shm, &qh, &h.runtime_dir, "pa", W, H, &solid(W, H, [0x30, 0x70, 0xA0, 0xFF]));
    let buf_b = make_buffer(&shm, &qh, &h.runtime_dir, "pb", W, H, &solid(W, H, [0xA0, 0x40, 0x30, 0xFF]));
    ...
    assert_eq!(app.presented.len(), FRAMES, "exactly {FRAMES} frames presented");
    ...
    for (i, p) in app.presented.iter().enumerate() {
        ...
        assert!(p.time_ns > 0, "frame {i}: presentation timestamp is nonzero");
    }
    ...
    for i in 1..app.presented.len() {
        ...
        assert!(cur >= prev, "frame {i}: timestamp monotonic, {prev} then {cur}");
    }

    // Sequence is a true frame counter: starts at 1, +1 exactly per frame, contiguous, no gaps/dups.
    assert_eq!(app.presented[0].seq, 1, "first presented frame is sequence 1");
