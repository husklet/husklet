...
fn is_pop_at(f: &CapturedFrame, at: (i32, i32)) -> bool {
    f.width == POP_W && f.height == POP_H && (f.x, f.y) == at && f.pixel_is(POP_W / 2, POP_H / 2, POP)
}
...
fn popup_reposition() {
    ...
    let pop_buffer = make_buffer(&shm, &qh, &h.runtime_dir, "pop", POP_W, POP_H, &solid(POP_W, POP_H, POP));
    ...
    let mut app = App {
        ...
        pop_surface: pop_surface.clone(), pop_buffer, pop_configure: None, repositioned_token: None,
    };
    ...
    let first = pump_until(&mut queue, &mut app, &h.captures, 5, |f| is_pop_at(f, EXPECT1))
        .expect("popup never composited at EXPECT1");
    assert_eq!((first.x, first.y), EXPECT1, "popup opened at the P1-resolved offset");
    assert_eq!(app.pop_configure.map(|(x, y, ..)| (x, y)), Some(EXPECT1), "initial xdg_popup.configure carried EXPECT1");
    save_composited("popup_reposition-before", TL_W, TL_H, TL, &[(&first, EXPECT1.0, EXPECT1.1)]);
    ...
    let moved = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.serial > boundary && is_pop_at(f, EXPECT2))
        .expect("popup never re-composited at EXPECT2 after reposition");
    assert_eq!(app.repositioned_token, Some(TOKEN), "client received xdg_popup.repositioned with the exact token");
    assert_eq!(app.pop_configure.map(|(x, y, w, hh)| (x, y, w, hh)), Some((EXPECT2.0, EXPECT2.1, POP_W, POP_H)),
        "reposition configure carried the NEW offset + size");
    assert_eq!((moved.x, moved.y), EXPECT2, "popup composited at the P2-resolved offset");

    // The popup is no longer composited at EXPECT1 after the move.
    let still_old = h.captures.lock().unwrap().iter().any(|f| f.serial > boundary && is_pop_at(f, EXPECT1));
    assert!(!still_old, "popup was still composited at the OLD offset after reposition");
    save_composited("popup_reposition-after", TL_W, TL_H, TL, &[(&moved, EXPECT2.0, EXPECT2.1)]);
...
impl Dispatch<XdgPopup, ()> for App {
    fn event(app: &mut Self, _: &XdgPopup, e: <XdgPopup as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
            xdg_popup::Event::Configure { x, y, width, height } => app.pop_configure = Some((x, y, width, height)),
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, XdgToplevel, XdgPositioner);
