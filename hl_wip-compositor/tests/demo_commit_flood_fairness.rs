...
fn spawn(dir: &std::path::Path, tag: &str, color: [u8; 4]) -> (Connection, EventQueue<Client>, Client) {
    ...
    surface.commit();
    let mut app = Client { surface: surface.clone(), buffer, drawn: false, done: 0 };
    ...
    while !(app.drawn && app.done >= 1) {
        assert!(Instant::now() < deadline, "client {tag} never mapped/presented its first frame");
...
fn commit_flood_fairness() {
    ...
    let normal_first = wait_for(&h.captures, 5, |f| f.pixel_is(1, 1, NORMAL)).expect("normal first frame");

    // Baseline: how many NORMAL-colored frames existed before the flood; count new ones after.
    let normal_before = h.captures.lock().unwrap().iter().filter(|f| f.pixel_is(1, 1, NORMAL)).count();
    ...
    let normal_after = h.captures.lock().unwrap().iter().filter(|f| f.pixel_is(1, 1, NORMAL)).count();
    ...
    let latest_normal = wait_for(&h.captures, 2, |f| f.pixel_is(1, 1, NORMAL)).expect("a NORMAL frame is present");
    assert_eq!(latest_normal.pixel(W / 2, H / 2).unwrap(), NORMAL, "normal client's frame is solid blue");

    save_composited("commit_flood-fairness", 2 * W + 20, H, [0x10, 0x10, 0x10, 0xFF],
