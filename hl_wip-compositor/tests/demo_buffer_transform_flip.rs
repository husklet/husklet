...
fn buffer_transform_flip() {
    ...
    assert_eq!((ff.width, ff.height), (BUF_W, BUF_H), "flip keeps presented dimensions");
    let (tl, tr, bl, br) = corners(&ff);
    assert_eq!(tl, GREEN, "flip: surface top-left = buffer top-right (green) — mirrored L/R");
    assert_eq!(tr, RED, "flip: surface top-right = buffer top-left (red)");
    assert_eq!(bl, YELLOW, "flip: surface bottom-left = buffer bottom-right (yellow)");
    assert_eq!(br, BLUE, "flip: surface bottom-right = buffer bottom-left (blue)");
    ...
    assert_eq!((ff90.width, ff90.height), (BUF_H, BUF_W), "flip_90 swaps presented dimensions");
    assert_eq!((ff90.logical_width, ff90.logical_height), (BUF_H, BUF_W), "flip_90 swaps logical size");
    let (tl, tr, bl, br) = corners(&ff90);
    assert_eq!(tl, YELLOW, "flip_90: surface top-left = buffer bottom-right (yellow)");
    assert_eq!(tr, GREEN, "flip_90: surface top-right = buffer top-right (green)");
    assert_eq!(bl, BLUE, "flip_90: surface bottom-left = buffer bottom-left (blue)");
    assert_eq!(br, RED, "flip_90: surface bottom-right = buffer top-left (red)");
