...
fn presentation_time() {
    ...
    assert_eq!(app.clock_id, Some(1), "wp_presentation advertised CLOCK_MONOTONIC (clk id 1)");
    ...
    for frame in 0..2 {
        ...
        while app.presented.len() < want {
            assert!(Instant::now() < deadline, "frame {frame}: presented feedback never arrived, got {:?}", app.presented);
            let _ = queue.roundtrip(&mut app);
    ...
    assert!(p0.time_ns > 0, "frame 0 presentation timestamp is nonzero, got {p0:?}");
    assert!(p1.time_ns >= p0.time_ns, "presentation timestamp is monotonic: {} then {}", p0.time_ns, p1.time_ns);
    // Strictly increasing sequence.
    assert!(p0.seq >= 1, "frame 0 sequence is a real frame counter (>=1), got {}", p0.seq);
    assert!(p1.seq > p0.seq, "presentation sequence strictly increases: {} then {}", p0.seq, p1.seq);
    // Nonzero refresh (~16.67 ms for 60 Hz) + vsync flag.
    assert!(p0.refresh_ns > 0, "refresh interval is nonzero, got {}", p0.refresh_ns);
    assert_eq!(p0.refresh_ns, 16_666_666, "refresh matches the 60 Hz output interval, got {}", p0.refresh_ns);
    ...
    let deadline = Instant::now() + Duration::from_secs(5);
    while !app.discarded {
        assert!(Instant::now() < deadline, "feedback on a never-shown, destroyed surface was never discarded");
        let _ = queue.roundtrip(&mut app);
        std::thread::sleep(Duration::from_millis(5));
    }
    assert!(app.discarded, "a never-shown surface's feedback was discarded (not falsely presented)");
    ...
    assert_eq!(app.sync_outputs, app.presented.len() as u32,
        "one sync_output per presented frame, got {} for {} frames", app.sync_outputs, app.presented.len());
...
impl Dispatch<WpPresentation, ()> for App {
    fn event(app: &mut Self, _: &WpPresentation, e: <WpPresentation as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
