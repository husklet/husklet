...
fn next_present_due_is_none_until_a_frame_ships_then_last_plus_refresh() {
    ...
    assert_eq!(c.next_present_due_ns(top), None, "no due time before the first present");
...
fn throttled_frame_ships_when_redriven_at_its_due_boundary() {
    ...
    c.apply_commit(
        top,
        Commit { buffer: BufferChange::Keep, ..Commit::default() }.with_damage(Rect::new(0, 0, 20, 20)),
    );
    let throttled = c.present_root(top);
    assert!(throttled.throttled, "commit within one interval is throttled");
    assert_eq!(c.presenter().count(), 1, "nothing shipped while throttled");

    // The adapter would arm a repaint at exactly this boundary.
    let due = c.next_present_due_ns(top).expect("a shipped frame has a due time");
    assert_eq!(due, refresh, "boundary is one interval after the t=0 present");

    // Re-drive at the boundary with NO new commit: the retained frame now ships and clears dirty.
    c.clock().set(due);
    let shipped = c.present_root(top);
    assert_eq!(shipped.pacing, FramePacing::Presented, "the retained frame ships at its boundary");
    assert_eq!(c.presenter().count(), 2, "the coalesced frame presents exactly once, one interval later");
    assert!(!c.scene.is_dirty(top), "shipping the retained frame clears its dirty state");
}
...
fn a_burst_of_throttled_commits_coalesces_to_one_shipped_frame() {
    ...
    for i in 1..=5 {
        ...
        assert!(c.present_root(top).throttled, "each mid-interval commit is throttled");
    }
    assert_eq!(c.presenter().count(), 1, "the whole burst shipped nothing yet");

    // One re-drive at the boundary ships exactly one coalesced frame for the whole burst.
    c.clock().set(refresh);
    assert_eq!(c.present_root(top).pacing, FramePacing::Presented);
    assert_eq!(c.presenter().count(), 2, "the five coalesced commits ship as a single present");
}
