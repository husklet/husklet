...
fn surface_storm() {
    ...
    run_storm(&compositor, &wm_base, &qh, &mut queue, STORM);
    neighbor.pump();
    let after = pump_until(&mut neighbor.queue, &mut neighbor.app, &h.captures, 5, |f| f.width == NW && f.pixel_is(1, 1, NEIGHBOR))
        .expect("neighbor stopped compositing after a second storm (leak/corruption?)");
    assert_eq!(after.pixel(NW / 2, NH / 2).unwrap(), NEIGHBOR, "neighbor still solid amber after both storms");
...
fn run_storm(compositor: &WlCompositor, wm_base: &XdgWmBase, qh: &QueueHandle<Storm>, queue: &mut EventQueue<Storm>, n: usize) {
    for i in 0..n {
...
impl Dispatch<XdgSurface, ()> for Storm {
    fn event(_: &mut Self, xdg: &XdgSurface, e: <XdgSurface as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        if let xdg_surface::Event::Configure { serial } = e { xdg.ack_configure(serial); }
    }
}
macro_rules! ignore {
    ($($t:ty),*) => {$(
