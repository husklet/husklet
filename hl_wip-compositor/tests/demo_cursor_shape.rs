...
fn cursor_shape_named_reaches_seat() {
    ...
    let shape_mgr: WpCursorShapeManagerV1 =
        globals.bind(&qh, 1..=2, ()).expect("wp_cursor_shape_manager_v1 advertised");
    ...
    let mut app =
        App { surface: surface.clone(), buffer, drawn: false, frame_done: false, enter_serial: None };
    ...
    assert!(
        pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(1, 1, BG)).is_some(),
    ...
    assert!(got_serial, "client never received a wl_pointer.enter serial for its surface");
...
impl Dispatch<WlPointer, ()> for App {
    fn event(app: &mut Self, _: &WlPointer, e: <WlPointer as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        if let wl_pointer::Event::Enter { serial, surface, .. } = e {
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, WlSeat, XdgToplevel, WpCursorShapeManagerV1, WpCursorShapeDeviceV1);
