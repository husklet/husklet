...
fn popup_dismiss_on_outside_click() {
    ...
    save_composited("popup_dismiss-frame0-open", TL_W, TL_H, TL, &[(&popup_frame, EXPECT_POP.0, EXPECT_POP.1)]);
    ...
    assert!(pump_while(&mut queue, &mut app, 5, |a| a.pop_destroyed), "client never destroyed the popup");
    ...
    assert!(!popup_after, "a popup layer was still composited after dismissal — popup did not disappear");

    save_composited("popup_dismiss-frame1-closed", TL_W, TL_H, TL, &[(&after, 0, 0)]);
