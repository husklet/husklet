...
fn buffer_offset_present() {
    ...
    let mut app = App { surface: surface.clone(), buffer: buf_a.clone(), drawn: false, frame_done: false };
    ...
    let frame1 = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(1, 1, COLOR))
        .expect("baseline (zero-offset) frame never composited");
    for (x, y) in [(W / 2, H / 2), (0, 0), (W - 1, 0), (0, H - 1), (W - 1, H - 1)] {
        assert_eq!(frame1.pixel(x, y).unwrap(), COLOR, "baseline pixel ({x},{y}) is the solid color");
    }
    ...
    for (x, y) in [(W / 2, H / 2), (0, 0), (W - 1, 0), (0, H - 1), (W - 1, H - 1)] {
        assert_eq!(frame2.pixel(x, y).unwrap(), COLOR, "offset frame pixel ({x},{y}) still the solid color (not shifted)");
    }
    assert_eq!(frame2.rgba, frame1.rgba, "toplevel buffer offset is a graceful no-op: content byte-identical");
    ...
    assert_eq!((frame2.x, frame2.y), (0, 0), "offset did not shift the reported composite origin");
