...
fn run_transform(h: &Harness, tag: &str, transform: Transform) -> CapturedFrame {
    ...
    let buffer = make_buffer(&shm, &qh, &h.runtime_dir, tag, BUF_W, BUF_H, &source_buffer());
    ...
    let mut app = App { surface: surface.clone(), buffer: buffer.clone(), transform, configured: false };
...
fn buffer_transform_rotate() {
    ...
    assert_eq!((f90.width, f90.height), (BUF_H, BUF_W), "90 swaps presented dimensions");
    assert_eq!((f90.logical_width, f90.logical_height), (BUF_H, BUF_W), "90 swaps logical size");
    let (tl, tr, bl, br) = corners(&f90);
    assert_eq!(tl, GREEN, "90: surface top-left = buffer top-right (green)");
    assert_eq!(tr, YELLOW, "90: surface top-right = buffer bottom-right (yellow)");
    assert_eq!(bl, RED, "90: surface bottom-left = buffer top-left (red)");
    assert_eq!(br, BLUE, "90: surface bottom-right = buffer bottom-left (blue)");
    ...
    assert_eq!((f180.width, f180.height), (BUF_W, BUF_H), "180 keeps presented dimensions");
    let (tl, tr, bl, br) = corners(&f180);
    assert_eq!(tl, YELLOW, "180: surface top-left = buffer bottom-right (yellow)");
    assert_eq!(tr, BLUE, "180: surface top-right = buffer bottom-left (blue)");
    assert_eq!(bl, GREEN, "180: surface bottom-left = buffer top-right (green)");
    ...
    assert_eq!((f270.width, f270.height), (BUF_H, BUF_W), "270 swaps presented dimensions");
    let (tl, tr, bl, br) = corners(&f270);
    assert_eq!(tl, BLUE, "270: surface top-left = buffer bottom-left (blue)");
    assert_eq!(tr, RED, "270: surface top-right = buffer top-left (red)");
    assert_eq!(bl, YELLOW, "270: surface bottom-left = buffer bottom-right (yellow)");
    assert_eq!(br, GREEN, "270: surface bottom-right = buffer top-right (green)");
