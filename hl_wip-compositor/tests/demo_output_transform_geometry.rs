...
fn output_transform_geometry() {
    ...
    let output: WlOutput = globals.bind(&qh, 2..=4, ()).expect("wl_output (>=v2 for transform)");
    ...
    let mut app = App { geometry_transform: None, mode_size: None, logical_size: None };
    ...
    assert_eq!(app.geometry_transform, Some(Transform::_90), "wl_output.geometry advertises the 90° transform");
    // ...while the mode still reports the PHYSICAL panel size, unswapped.
    assert_eq!(app.mode_size, Some((1920, 1080)), "wl_output.mode reports the physical panel size (unswapped)");
    // xdg-output's logical size is SWAPPED for the 90° rotation (scale 1).
    assert_eq!(app.logical_size, Some((1080, 1920)), "xdg-output logical size is swapped for the 90° rotation");
...
impl Dispatch<WlOutput, ()> for App {
    fn event(app: &mut Self, _: &WlOutput, e: <WlOutput as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
...
impl Dispatch<ZxdgOutputV1, ()> for App {
    fn event(app: &mut Self, _: &ZxdgOutputV1, e: <ZxdgOutputV1 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
