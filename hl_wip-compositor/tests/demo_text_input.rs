...
fn text_input() {
    ...
    let ti_manager: ZwpTextInputManagerV3 = globals.bind(&qh, 1..=1, ()).expect("zwp_text_input_manager_v3");
    let im_manager: ZwpInputMethodManagerV2 =
        globals.bind(&qh, 1..=1, ()).expect("zwp_input_method_manager_v2");
    ...
    let mut app = App {
        surface: surface.clone(), buffer, drawn: false, frame_done: false, entered: false,
        commit_count: 0, committed: String::new(), preedit: String::new(), preedit_cursor: (0, 0),
        pending_commit: None, pending_preedit: None, pending_delete: None,
    };
    ...
    h.input_tx.send(InputCommand::FocusTopmostKeyboard).expect("focus");
    let deadline = Instant::now() + Duration::from_secs(5);
    while !app.entered {
        assert!(Instant::now() < deadline, "text-input never received `enter`");
        let _ = queue.roundtrip(&mut app);
        std::thread::sleep(Duration::from_millis(5));
    }
    assert_eq!(app.commit_count, 1, "client enabled + committed exactly once on enter");
    ...
    h.input_tx
        .send(InputCommand::ImePreeditString { text: "hel".into(), cursor_begin: 0, cursor_end: 3 })
        .expect("preedit");
    assert!(
        pump_while(&mut queue, &mut app, 5, |a| a.preedit == "hel"),
        "preedit never became \"hel\" (got {:?})", app.preedit,
    );
    assert_eq!(app.committed, "", "preedit is transient — committed text is still empty");
    assert_eq!(app.preedit_cursor, (0, 3), "preedit cursor byte range delivered exactly");

    // ---- commit "hello": replaces the preedit, committed text becomes "hello" ----
    h.input_tx.send(InputCommand::ImeCommitString("hello".into())).expect("commit hello");
    assert!(
        pump_while(&mut queue, &mut app, 5, |a| a.committed == "hello"),
        "committed text never became \"hello\" (got {:?})", app.committed,
    );
    assert_eq!(app.preedit, "", "commit cleared the preedit overlay");

    // ---- delete_surrounding(2,0) then commit "p!": "hello" -> "hel" -> "help!" ----
    h.input_tx
        .send(InputCommand::ImeDeleteSurrounding { before_length: 2, after_length: 0 })
        .expect("delete surrounding");
    assert!(
        pump_while(&mut queue, &mut app, 5, |a| a.committed == "hel"),
        "delete_surrounding(2,0) never trimmed \"hello\" to \"hel\" (got {:?})", app.committed,
    );
    h.input_tx.send(InputCommand::ImeCommitString("p!".into())).expect("commit p!");
    assert!(
        pump_while(&mut queue, &mut app, 5, |a| a.committed == "help!"),
        "committed text never became \"help!\" (got {:?})", app.committed,
    );
...
impl Dispatch<ZwpTextInputV3, ()> for App {
    fn event(app: &mut Self, ti: &ZwpTextInputV3, e: <ZwpTextInputV3 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
...
ignore!(
    WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, WlSeat, WlKeyboard, XdgToplevel,
    ZwpTextInputManagerV3, ZwpInputMethodManagerV2, ZwpInputMethodV2
);
