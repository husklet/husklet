...
fn fractional_scale_buffer_scale() {
    ...
    let frac_mgr: WpFractionalScaleManagerV1 =
        globals.bind(&qh, 1..=1, ()).expect("wp_fractional_scale_manager_v1 (adapter must advertise it)");

    let buffer = make_buffer(&shm, &qh, &h.runtime_dir, "fs", BUF, BUF, &solid(BUF, BUF, COL));
    ...
    let mut app = App { surface: surface.clone(), buffer: buffer.clone(), configured: false, preferred: None };

    // Await both the configure (drives the map) and the preferred_scale event.
    let deadline = Instant::now() + Duration::from_secs(5);
    while !(app.configured && app.preferred.is_some()) {
        assert!(Instant::now() < deadline, "no configure / preferred_scale (configured={}, preferred={:?})", app.configured, app.preferred);
        queue.blocking_dispatch(&mut app).expect("dispatch configure + preferred_scale");
    }

    // EXACT preferred fractional scale on the wire.
    assert_eq!(app.preferred, Some(EXPECT_PREFERRED), "preferred_scale == round(output_scale * 120)");

    // The presented frame: raw buffer 240x240, but logical (on-screen) size 120x120 from buffer scale 2.
    let frame = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == BUF && f.height == BUF && f.pixel_is(BUF / 2, BUF / 2, COL))
        .expect("scaled buffer never presented");
    assert_eq!((frame.width, frame.height), (BUF, BUF), "backing buffer stays 240x240 device pixels");
    assert_eq!(
        (frame.logical_width, frame.logical_height),
        (LOGICAL, LOGICAL),
        "presented LOGICAL size is buffer/scale == 120x120 (scale {SCALE})",
    );
    assert_eq!(frame.pixel(BUF / 2, BUF / 2).unwrap(), COL, "presented content color");
