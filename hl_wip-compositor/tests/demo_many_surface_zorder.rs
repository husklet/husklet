...
fn newest_serial(caps: &Arc<Mutex<Vec<CapturedFrame>>>, i: usize) -> Option<u64> {
    caps.lock().unwrap().iter().filter(|f| is_sub(f, i)).map(|f| f.serial).max()
}
...
fn read_stacking(
    ...
    let ok = pump_while(queue, app, 5, |_| {
        (0..K).all(|i| newest_serial(caps, i).map(|s| s > baseline).unwrap_or(false))
    });
    assert!(ok, "not all {K} subsurfaces re-presented after the repaint");
    let mut serials: Vec<(usize, u64)> = (0..K).map(|i| (i, newest_serial(caps, i).unwrap())).collect();
    let smin = serials.iter().map(|(_, s)| *s).min().unwrap();
    let smax = serials.iter().map(|(_, s)| *s).max().unwrap();
    assert_eq!(smax - smin, (K as u64) - 1, "the {K} subsurfaces present in ONE contiguous compose cycle");
...
fn many_surface_zorder() {
    ...
    let sub_buffers: Vec<WlBuffer> = (0..K)
        .map(|i| make_buffer(&shm, &qh, &h.runtime_dir, &format!("s{i}"), SUB_W, SUB_H, &solid(SUB_W, SUB_H, COLORS[i])))
        .collect();
    ...
    assert_eq!(order0_idx, vec![0, 1, 2, 3, 4, 5], "default stacking is creation order, bottom -> top");
    ...
    assert_eq!(order1_idx, vec![1, 2, 3, 4, 5, 0], "after place_above, surface 0 is on top");
...
fn confront_overlap(caps: &Arc<Mutex<Vec<CapturedFrame>>>, subs: &[WlSurface], name: &str, top_idx: usize) {
    // Order the six layers by their newest serial (== bottom -> top stacking this cycle).
    let mut layers: Vec<(usize, CapturedFrame)> = Vec::new();
    for i in 0..K {
        let f = caps.lock().unwrap().iter().filter(|f| is_sub(f, i)).max_by_key(|f| f.serial).cloned();
        if let Some(f) = f {
            layers.push((i, f));
        }
    }
    ...
    let ordered: Vec<(&CapturedFrame, i32, i32)> =
        layers.iter().map(|(i, f)| (f, sub_pos(*i).0, sub_pos(*i).1)).collect();
    ...
    assert_eq!(layers.last().map(|(i, _)| *i), Some(top_idx), "topmost layer is surface {top_idx}");
    for (px, py) in OVERLAP_POINTS {
        ...
        assert_eq!(seen, top_color, "overlap point ({px},{py}) shows the topmost surface {top_idx}");
    }
}
