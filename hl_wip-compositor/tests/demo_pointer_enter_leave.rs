...
use wayland_client::protocol::{
    ...
    wl_pointer::{self, WlPointer}, wl_registry::WlRegistry, wl_seat::WlSeat, wl_shm::WlShm,
...
impl App {
    fn draw(&mut self, marker: bool, qh: &QueueHandle<App>) {
        self.surface.attach(Some(if marker { &self.marker } else { &self.base }), 0, 0);
    ...
    fn enters(&self) -> usize {
        self.ev.iter().filter(|e| matches!(e, PtrEv::Enter(..))).count()
    }
...
fn pointer_enter_leave_rerender() {
    ...
    let mut app = App {
        surface: surface.clone(), base: base.clone(), marker, drawn: false, frame_done: false,
    ...
    h.input_tx.send(InputCommand::PointerMotion { x: IN.0 as f64, y: IN.1 as f64 }).expect("motion in");
    ...
    assert_eq!(entered.pixel(IN.0, IN.1).unwrap(), MARKER, "marker present where the pointer entered");
    assert_eq!(app.enters(), 1, "exactly one wl_pointer.enter delivered");
    assert!(app.ev.contains(&PtrEv::Enter(IN.0, IN.1)), "enter carried the exact surface-local coord, got {:?}", app.ev);
    ...
    h.input_tx.send(InputCommand::PointerMotion { x: OUT.0 as f64, y: OUT.1 as f64 }).expect("motion out");
    ...
    assert_eq!(app.leaves(), 1, "exactly one wl_pointer.leave delivered on exiting the surface");
    assert_eq!(left.pixel(IN.0, IN.1).unwrap(), BASE, "marker cleared where the pointer left");
    ...
    h.input_tx.send(InputCommand::PointerMotion { x: IN.0 as f64, y: IN.1 as f64 }).expect("motion back in");
    ...
    assert_eq!(app.enters(), 2, "a second wl_pointer.enter delivered on re-entry");
    assert_eq!(reentered.pixel(IN.0, IN.1).unwrap(), MARKER, "marker returned on re-entry");

    // Exact focus-event sequence: enter, leave, enter (the pointer crossed the boundary out then back).
    let focus_seq: Vec<PtrEv> = app.ev.iter().copied().filter(|e| !matches!(e, PtrEv::Motion(..))).collect();
    assert_eq!(
        focus_seq,
        vec![PtrEv::Enter(IN.0, IN.1), PtrEv::Leave, PtrEv::Enter(IN.0, IN.1)],
...
impl Dispatch<WlPointer, ()> for App {
    fn event(app: &mut Self, _: &WlPointer, e: <WlPointer as Proxy>::Event, _: &(), _: &Connection, qh: &QueueHandle<Self>) {
        match e {
            wl_pointer::Event::Enter { surface, surface_x, surface_y, .. } if surface.id() == app.surface.id() => {
                app.ev.push(PtrEv::Enter(surface_x.round() as i32, surface_y.round() as i32));
            ...
            wl_pointer::Event::Motion { surface_x, surface_y, .. } => {
                app.ev.push(PtrEv::Motion(surface_x.round() as i32, surface_y.round() as i32));
            }
            _ => {}
        }
    }
