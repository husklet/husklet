    ...
    -> (Connection, EventQueue<App>, App, WlDataDeviceManager, WlSeat, QueueHandle<App>)
{
    ...
    let ddm: WlDataDeviceManager = globals.bind(&qh, 1..=3, ()).expect("wl_data_device_manager");
...
fn clipboard_paste_rerender() {
    ...
    let (_ca, mut qa, mut a, ddm_a, seat_a, qha) = spawn(&h.runtime_dir, "A", [0xC0, 0x40, 0x40, 0xFF]);
    ...
    while b.selection_offer.is_none() {
        assert!(Instant::now() < deadline, "B never received a selection offer; mimes: {:?}", b.offered_mimes);
        let _ = qa.roundtrip(&mut a);
        let _ = qb.roundtrip(&mut b);
        std::thread::sleep(Duration::from_millis(5));
    }
    assert!(b.offered_mimes.contains(&MIME.to_string()), "B's offer advertised A's mime, got {:?}", b.offered_mimes);
    ...
    loop {
        ...
        assert!(Instant::now() < deadline, "clipboard bytes never arrived; got {:?}", got);
        std::thread::sleep(Duration::from_millis(5));
    }
    ...
    assert_ne!(pasted_color, B_BASE, "the pasted color must differ from B's base for the re-render to be visible");
...
impl Dispatch<WlDataDevice, ()> for App {
    fn event(app: &mut Self, _: &WlDataDevice, e: <WlDataDevice as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        if let wl_data_device::Event::Selection { id } = e { app.selection_offer = id; }
    }
...
impl Dispatch<WlDataOffer, ()> for App {
    fn event(app: &mut Self, _: &WlDataOffer, e: <WlDataOffer as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        if let wl_data_offer::Event::Offer { mime_type } = e { app.offered_mimes.push(mime_type); }
    }
}
macro_rules! ignore {
    ($($t:ty),*) => {$(
