...
use wayland_client::protocol::{
    ...
    wl_keyboard::{self, KeymapFormat, WlKeyboard}, wl_registry::WlRegistry, wl_seat::WlSeat,
...
fn keyboard_repeat_info() {
    ...
    let mut app = App { surface: surface.clone(), buffer: buffer.clone(), drawn: false, repeat_info: None, keymap_ok: false };
    ...
    let ok = pump_while(&mut queue, &mut app, 5, |a| a.repeat_info.is_some() && a.keymap_ok);
    assert!(ok, "wl_keyboard.repeat_info / keymap never arrived");
    assert_eq!(app.repeat_info, Some((EXPECT_RATE, EXPECT_DELAY)), "repeat_info carries the seat's exact (rate, delay)");
...
impl Dispatch<WlCallback, ()> for App {
    fn event(_: &mut Self, _: &WlCallback, _: <WlCallback as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {}
}
