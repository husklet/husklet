...
use hl_compositor::scene::model::{
    BufferState, BufferTransform, DamageRegion, Format, Output, OutputId, Rect, Scene, SubsurfaceState,
    SurfaceRole, Viewport,
};
use hl_compositor::scene::service::{commit_surface, compose_frame, is_tree_dirty, BufferChange, Commit};
...
// =================================================================================================
// 1. Rect primitive: right/bottom/translate/union/contains/intersects under overflowing coordinates
// =================================================================================================
...
// =================================================================================================
// 3. compose_frame: hostile subsurface offsets (offset accumulation) + damage translation
// =================================================================================================
...
fn logical_size_survives_hostile_viewport_and_transform() {
    ...
    let cases = [
        (BufferState { buffer_scale: i32::MAX, ..shm(i32::MAX, i32::MAX) }, Viewport::default()),
        (shm(i32::MAX, i32::MAX), Viewport { dst: Some((i32::MAX, i32::MAX)), src: None }),
        (shm(4, 4), Viewport { dst: Some((-5, -5)), src: None }),
        (shm(4, 4), Viewport { dst: None, src: Some((0.0, 0.0, 1e30, 1e30)) }),
        (shm(4, 4), Viewport { dst: None, src: Some((0.0, 0.0, -1.0, -1.0)) }),
...
fn compose_present_crop_survives_huge_viewport_dst() {
    ...
    commit_surface(
        ...
        top,
        Commit {
            viewport: Some(Viewport { dst: Some((i32::MAX, i32::MAX)), src: Some((0.0, 0.0, 1e18, 1e18)) }),
...
mod present_path {
    ...
    fn valid_buf(w: i32, h: i32) -> StoredBuffer {
        StoredBuffer { width: w, height: h, rgba: vec![0x77u8; (w * h * 4) as usize] }
    }

    fn drive(p: &mut PngPresenter, buf: StoredBuffer, image: &PresentableImage) -> PresentOutcome {
        p.deposit(image.surface, buf);
        p.present(OutputId(1), image, &[], PresentTiming { present_ns: 0, refresh_ns: 0, vsync: false })
            .outcome
    }
    ...
    fn assert_valid_frame_after(p: &mut PngPresenter) {
        ...
        assert!(matches!(outcome, PresentOutcome::Delivered { .. }), "valid frame delivers after abuse");
        let caps = p.captures();
        let last = caps.lock().unwrap().last().cloned().unwrap();
        assert_eq!((last.width, last.height), (4, 4));
        assert_eq!(last.pixel(0, 0).unwrap()[0], 0x11, "valid pixels reach the capture");
    }
    ...
    fn resample_survives_huge_viewport_logical_size() {
        ...
        let image = img(SurfaceId(1), 2_000_000_000, 2_000_000_000, BufferTransform::Normal, Some((0.0, 0.0, 8.0, 8.0)));
        let outcome = drive(&mut p, valid_buf(8, 8), &image);
        assert!(matches!(outcome, PresentOutcome::Offscreen), "absurd logical size is skipped, not allocated");
    ...
    fn resample_survives_dst_near_i32_overflow() {
        let mut p = PngPresenter::new();
        let image = img(SurfaceId(2), i32::MAX, 1, BufferTransform::Normal, Some((0.0, 0.0, 4.0, 4.0)));
    ...
    fn resample_survives_zero_dimension_source_buffer() {
        ...
        for (bw, bh) in [(0, 4), (4, 0), (0, 0)] {
            let buf = StoredBuffer { width: bw, height: bh, rgba: Vec::new() };
            let image = img(SurfaceId(3), 16, 16, BufferTransform::Normal, Some((0.0, 0.0, 4.0, 4.0)));
            let outcome = drive(&mut p, buf, &image);
            assert!(matches!(outcome, PresentOutcome::Offscreen), "degenerate source buffer skipped");
        }
        assert_valid_frame_after(&mut p);
    }
    ...
    fn resample_survives_negative_source_dimensions() {
        let mut p = PngPresenter::new();
        let buf = StoredBuffer { width: -8, height: -8, rgba: Vec::new() };
        let image = img(SurfaceId(5), 8, 8, BufferTransform::Normal, Some((0.0, 0.0, 4.0, 4.0)));
        assert!(matches!(drive(&mut p, buf, &image), PresentOutcome::Offscreen));
        assert_valid_frame_after(&mut p);
    }
    ...
    fn resample_survives_huge_source_crop_extent() {
        ...
        let mut p = PngPresenter::new();
        let image = img(SurfaceId(6), 16, 16, BufferTransform::Normal, Some((0.0, 0.0, 1e18, 1e18)));
        let outcome = drive(&mut p, valid_buf(8, 8), &image);
        assert!(matches!(outcome, PresentOutcome::Delivered { .. }), "clamped crop still composites");
        let caps = p.captures();
    ...
    fn transform_buffer_survives_huge_dimensions() {
        ...
        let mut p = PngPresenter::new();
        let buf = StoredBuffer { width: 40_000, height: 40_000, rgba: vec![0u8; 64] };
    ...
    fn transform_buffer_survives_inconsistent_rgba_length() {
        ...
        let mut p = PngPresenter::new();
        for t in [BufferTransform::_90, BufferTransform::_180, BufferTransform::_270, BufferTransform::Flipped180] {
            let buf = StoredBuffer { width: 100, height: 100, rgba: vec![0u8; 16] };
            let image = img(SurfaceId(8), 100, 100, t, None);
            assert!(matches!(drive(&mut p, buf, &image), PresentOutcome::Offscreen), "short rgba skipped ({t:?})");
        }
        assert_valid_frame_after(&mut p);
    }
    ...
    fn transform_plus_viewport_survives_hostile_swap() {
        ...
        let mut p = PngPresenter::new();
        let image = img(SurfaceId(9), i32::MAX, i32::MAX, BufferTransform::_270, Some((0.0, 0.0, 8.0, 8.0)));
        assert!(matches!(drive(&mut p, valid_buf(8, 8), &image), PresentOutcome::Offscreen));
        assert_valid_frame_after(&mut p);
    }
    ...
    fn negative_logical_size_frame_is_skipped_then_valid_composes() {
        ...
        let buf = StoredBuffer { width: -100, height: -100, rgba: Vec::new() };
        assert!(matches!(drive(&mut p, buf, &image), PresentOutcome::Offscreen));
        assert_valid_frame_after(&mut p);
    }
}
