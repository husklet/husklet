...
use wayland_client::protocol::{
    ...
    wl_pointer::{self, WlPointer}, wl_region::WlRegion, wl_registry::WlRegistry, wl_seat::WlSeat,
...
fn pointer_constraints() {
    ...
    let rel_mgr: ZwpRelativePointerManagerV1 = globals.bind(&qh, 1..=1, ()).expect("relative_pointer_manager");
    // The newly-wired global under test.
    let constraints: ZwpPointerConstraintsV1 =
        globals.bind(&qh, 1..=1, ()).expect("zwp_pointer_constraints_v1 advertised");
    ...
    h.input_tx.send(InputCommand::PointerMotion { x: ENTER.0, y: ENTER.1 }).expect("enter motion");
    ...
    assert!(app.locked, "the compositor engaged the lock (zwp_locked_pointer_v1.locked received)");

    // ---- inject moves while locked ----
    for &(x, y) in LOCKED_MOVES {
        h.input_tx.send(InputCommand::PointerMotion { x, y }).expect("locked motion");
    }
    pump_while(&mut queue, &mut app, 5, |a| a.rel.len() >= rel_at_lock + LOCKED_MOVES.len());
    ...
    let expected: Vec<(f64, f64)> = positions.windows(2).map(|w| (w[1].0 - w[0].0, w[1].1 - w[0].1)).collect();
    let got_locked = &app.rel[rel_at_lock..];
    assert_eq!(got_locked.len(), expected.len(), "one relative delta per locked move, got {got_locked:?}");
    for (i, (g, e)) in got_locked.iter().zip(expected.iter()).enumerate() {
        assert_eq!(g, e, "relative delta #{i} while locked exact: got {g:?} want {e:?}");
    }
...
impl Dispatch<WlPointer, ()> for App {
    fn event(app: &mut Self, _: &WlPointer, e: <WlPointer as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
            ...
            wl_pointer::Event::Motion { surface_x, surface_y, .. } => app.motions.push((surface_x, surface_y)),
            _ => {}
        }
    }
}
impl Dispatch<ZwpLockedPointerV1, ()> for App {
    fn event(app: &mut Self, _: &ZwpLockedPointerV1, e: <ZwpLockedPointerV1 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
...
impl Dispatch<ZwpRelativePointerV1, ()> for App {
    fn event(app: &mut Self, _: &ZwpRelativePointerV1, e: <ZwpRelativePointerV1 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, WlSeat, WlRegion, XdgToplevel,
    ZwpRelativePointerManagerV1, ZwpPointerConstraintsV1);
