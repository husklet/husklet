...
fn session_lock() {
    ...
    let lock_mgr: ExtSessionLockManagerV1 = globals.bind(&qh, 1..=1, ()).expect("ext_session_lock_manager_v1");
    ...
    let mut app = App {
        shm: shm.clone(), dir: h.runtime_dir.clone(),
        tl_surface: tl_surface.clone(), tl_buf1: tl_buf1.clone(), tl_buf2: tl_buf2.clone(),
        tl_drawn: false, tl_frame_done: false, locked: false,
        lock_surface: None, lock_configured: false, lock_drawn: false, lock_size: (0, 0),
    };
    ...
    let _ = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.pixel_is(1, 1, NORMAL1))
    ...
    while !app.locked {
        ...
        assert!(Instant::now() < deadline, "compositor never confirmed the lock");
        std::thread::sleep(Duration::from_millis(5));
    }
    // Server-side lock state recorded.
    assert!(h.observations.lock().unwrap().session_locked, "compositor recorded session_locked=true");
    ...
    let lock_frame = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.pixel_is(1, 1, LOCK))
        .expect("lock surface never presented");
    assert_eq!(lock_frame.pixel(W / 2, H / 2).unwrap(), LOCK, "lock surface is solid LOCK color");
    ...
    let normal2_shown = h.captures.lock().unwrap().iter().any(|f| f.pixel_is(1, 1, NORMAL2));
    assert!(!normal2_shown, "normal surface must stay HIDDEN while the session is locked (NORMAL2 leaked)");
    ...
    while h.observations.lock().unwrap().session_locked {
        let _ = queue.roundtrip(&mut app);
        assert!(Instant::now() < deadline, "compositor never cleared session_locked");
        std::thread::sleep(Duration::from_millis(5));
    }
    assert!(!h.observations.lock().unwrap().session_locked, "compositor recorded session_locked=false");
    ...
    let _cb: WlCallback = app.tl_surface.frame(&qh, ());
    app.tl_surface.commit();
    let restored = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.pixel_is(1, 1, NORMAL2))
        .expect("normal surface never restored after unlock");
    assert_eq!(restored.pixel(W / 2, H / 2).unwrap(), NORMAL2, "restored normal surface presents its content");
...
impl Dispatch<ExtSessionLockV1, ()> for App {
    fn event(app: &mut Self, _: &ExtSessionLockV1, e: <ExtSessionLockV1 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match e {
...
impl Dispatch<ExtSessionLockSurfaceV1, ()> for App {
    fn event(app: &mut Self, surf: &ExtSessionLockSurfaceV1, e: <ExtSessionLockSurfaceV1 as Proxy>::Event, _: &(), _: &Connection, qh: &QueueHandle<Self>) {
        if let ext_session_lock_surface_v1::Event::Configure { serial, width, height } = e {
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, WlOutput, XdgToplevel, ExtSessionLockManagerV1);
