...
fn idle_inhibit_tracked_and_removed() {
    ...
    let idle_mgr: ZwpIdleInhibitManagerV1 =
        globals.bind(&qh, 1..=1, ()).expect("zwp_idle_inhibit_manager_v1 advertised");

    let buffer = make_buffer(&shm, &qh, &h.runtime_dir, "player", W, H, &solid(W, H, [0x30, 0xC0, 0x50, 0xFF]));
    ...
    assert!(tracked, "compositor registered an idle inhibitor for the surface after create_inhibitor");
    ...
    assert!(removed, "compositor unregistered the inhibitor after zwp_idle_inhibitor_v1.destroy");
    assert!(
...
ignore!(WlCompositor, WlSurface, WlShm, WlShmPool, WlBuffer, XdgToplevel, ZwpIdleInhibitManagerV1, ZwpIdleInhibitorV1);
