...
use wayland_client::protocol::{
    ...
    wl_pointer::{self, ButtonState, WlPointer}, wl_registry::WlRegistry, wl_seat::WlSeat,
...
impl App {
    ...
    fn motions(&self) -> Vec<(i32, i32)> {
        self.ev.iter().filter_map(|e| match e { PtrEv::Motion(x, y, _) => Some((*x, *y)), _ => None }).collect()
    }
    fn buttons(&self) -> Vec<(u32, bool)> {
        self.ev.iter().filter_map(|e| match e { PtrEv::Button(b, p, _) => Some((*b, *p)), _ => None }).collect()
    }
    /// The serials of every event that carries one, in arrival order (enter + both buttons).
    fn serials(&self) -> Vec<u32> {
        self.ev.iter().filter_map(|e| match e { PtrEv::Enter(s) | PtrEv::Button(_, _, s) => Some(*s), _ => None }).collect()
    }
}
...
fn pointer_drag_stream() {
    ...
    for (i, &(px, py)) in std::iter::once(&START).chain(TRACK.iter()).enumerate() {
        ...
        markers.push(((px, py), make_buffer(&shm, &qh, &h.runtime_dir, &format!("m{i}"), W, H, &buf)));
    }
    ...
    let mut app = App {
        surface: surface.clone(), base: base.clone(), markers, drawn: false, frame_done: false,
        ev: Vec::new(), redraws: 0,
    };
    ...
    assert!(
        pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(1, 1, BASE)).is_some(),
    ...
    h.input_tx.send(InputCommand::PointerMotion { x: START.0 as f64, y: START.1 as f64 }).expect("enter");
    let start_frame = pump_until(&mut queue, &mut app, &h.captures, 5, |f| f.width == W && f.pixel_is(START.0, START.1, MARKER))
        .expect("marker never composited at drag start");
    h.input_tx.send(InputCommand::PointerButton { button: BTN_LEFT, pressed: true }).expect("btn down");
    pump_while(&mut queue, &mut app, 5, |a| a.buttons().contains(&(BTN_LEFT, true)));
    assert!(app.buttons().contains(&(BTN_LEFT, true)), "BTN_LEFT press delivered");
    ...
    for (i, &(px, py)) in TRACK.iter().enumerate() {
        h.input_tx.send(InputCommand::PointerMotion { x: px as f64, y: py as f64 }).expect("drag motion");
        ...
        assert_eq!(f.pixel(px, py).unwrap(), MARKER, "marker at drag point {:?}", (px, py));
        // The previous cell cleared (the marker MOVED across the drag, it did not smear).
        let (ppx, ppy) = if i == 0 { START } else { TRACK[i - 1] };
        assert_eq!(f.pixel(ppx, ppy).unwrap(), BASE, "previous drag cell {:?} cleared", (ppx, ppy));
        assert!(f.serial > prev_serial, "drag frame {i} presents after the previous");
        prev_serial = f.serial;
        save_frame(&format!("pointer_drag_stream-{i}"), &f);
    }

    // ---- release: BTN_LEFT up ----
    h.input_tx.send(InputCommand::PointerButton { button: BTN_LEFT, pressed: false }).expect("btn up");
    pump_while(&mut queue, &mut app, 5, |a| a.buttons().contains(&(BTN_LEFT, false)));
    ...
    for &pt in TRACK.iter() {
        assert!(motions.contains(&pt), "motion {pt:?} was delivered, got {motions:?}");
    }
    // The TRACK subsequence appears in the delivered motion order (no reordering).
    let idxs: Vec<usize> = TRACK.iter().map(|pt| motions.iter().position(|m| m == pt).unwrap()).collect();
    assert!(idxs.windows(2).all(|w| w[0] < w[1]), "motions arrived in injected order, got {motions:?}");
    // Buttons: exactly a press then a release of BTN_LEFT.
    assert_eq!(app.buttons(), vec![(BTN_LEFT, true), (BTN_LEFT, false)], "press then release");
    // Serials of serial-carrying events (enter, btn-down, btn-up) are strictly monotonic.
    let serials = app.serials();
    assert_eq!(serials.len(), 3, "enter + 2 buttons carry serials, got {:?}", app.ev);
    assert!(serials.windows(2).all(|w| w[1] > w[0]), "serials strictly increase: {serials:?}");
    // Motion timestamps are non-decreasing (one monotonic input clock).
    let times: Vec<u32> = app.ev.iter().filter_map(|e| match e { PtrEv::Motion(_, _, t) => Some(*t), _ => None }).collect();
    assert!(times.windows(2).all(|w| w[1] >= w[0]), "motion timestamps non-decreasing: {times:?}");
    assert_eq!(app.redraws as usize, 1 + TRACK.len(), "client redrew once per delivered drag position");
...
impl Dispatch<WlPointer, ()> for App {
    fn event(app: &mut Self, _: &WlPointer, e: <WlPointer as Proxy>::Event, _: &(), _: &Connection, qh: &QueueHandle<Self>) {
        match e {
            ...
            wl_pointer::Event::Motion { surface_x, surface_y, time } => {
                app.ev.push(PtrEv::Motion(surface_x.round() as i32, surface_y.round() as i32, time));
                app.draw_marker_at(surface_x.round() as i32, surface_y.round() as i32, qh);
            }
            wl_pointer::Event::Button { serial, button, state, .. } => {
                app.ev.push(PtrEv::Button(button, matches!(state, WEnum::Value(ButtonState::Pressed)), serial));
            }
            _ => {}
        }
    }
