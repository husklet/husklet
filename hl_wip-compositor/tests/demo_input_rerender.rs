...
fn input_drives_rerender() {
    ...
    for (i, &(px, py)) in TRACK.iter().enumerate() {
        h.input_tx.send(InputCommand::PointerMotion { x: px as f64, y: py as f64 }).expect("inject motion");
        ...
        .unwrap_or_else(|| panic!("marker never re-rendered at injected position {:?}", (px, py)));
        ...
        assert_eq!(frame.pixel(px, py).unwrap(), MARKER, "marker present at injected {:?}", (px, py));
        assert_eq!(frame.pixel(1, 1).unwrap(), BASE, "background stays BASE away from the marker");
        if i > 0 {
            let (ppx, ppy) = TRACK[i - 1];
            assert_eq!(frame.pixel(ppx, ppy).unwrap(), BASE, "previous marker cell {:?} cleared to BASE", (ppx, ppy));
        }
    ...
    assert_eq!(app.redraws as usize, TRACK.len(), "client redrew once per injected pointer move");
