...
pub fn place_popup(positioner: &Positioner, target_w: i32, target_h: i32) -> Rect {
    place_popup_in(positioner, Rect::new(0, 0, target_w.max(1), target_h.max(1)))
}
...
pub fn popup_placement(scene: &Scene, surface: SurfaceId) -> Option<PopupPlacement> {
    ...
    Some(PopupPlacement { parent, x: geometry.x, y: geometry.y })
}
...
fn unconstrained(p: &Positioner) -> Rect {
    ...
    Rect::new(ax + gx + p.offset.0, ay + gy + p.offset.1, p.size.0, p.size.1)
}
