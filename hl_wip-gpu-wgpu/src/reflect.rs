...
pub enum BindingKind {
    ...
    Texture { dim: TexDim, sample: TexSample, multi: bool },
...
fn binding_kind(module: &naga::Module, gv: &naga::GlobalVariable) -> Option<BindingKind> {
    match gv.space {
        ...
        naga::AddressSpace::Storage { access } => {
            Some(BindingKind::StorageBuffer { read_only: !access.contains(naga::StorageAccess::STORE) })
        }
        naga::AddressSpace::Handle => match &module.types[gv.ty].inner {
            ...
            naga::TypeInner::Sampler { comparison } => Some(BindingKind::Sampler { comparison: *comparison }),
            _ => None,
        },
        _ => None,
    }
        ...
        .map(|(i, ep)| {
            ...
            let bindings = module
                ...
                .filter_map(|(h, gv)| {
                    ...
                    Some(Binding { group: rb.group, binding: rb.binding, kind })
                })
                .collect();
