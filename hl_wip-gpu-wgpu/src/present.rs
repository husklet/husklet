...
//! surfaces only the [`Presented`] pairing the runtime records.

use hl_gpu::protocol::model::descriptor::SurfaceDesc;
use hl_gpu::protocol::model::id::{SurfaceId, TextureId};
use hl_gpu::runtime::model::resources::SessionResources;
use hl_gpu::{GpuError, Presented, Result};

use crate::texture;

pub fn present(res: &SessionResources, surface_id: u32, texture_id: u32) -> Result<Presented> {
    let sdesc = res
        .surfaces
        .get(surface_id)?
        .downcast_ref::<SurfaceDesc>()
        .ok_or(GpuError::Invalid("wgpu: surface native type mismatch"))?;
    let t = texture::native(res, texture_id)?;
