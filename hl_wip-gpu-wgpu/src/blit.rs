...
//! `blit_texture` (`hl_wip-gpu/src/cpu/service/copy.rs`): for a dest texel `(dx, dy)` the sample point is
...
impl WgpuExecutor {
    ...
    pub(crate) fn blit_texture(
        ...
        let _sp = hl_log::hl_span!(tag::WGPU, "blit");
        ...
        if !ok(src_origin.x, src_origin.y, src_extent.width, src_extent.height, sw, sh)
            || !ok(dst_origin.x, dst_origin.y, dst_extent.width, dst_extent.height, dw, dh)
        {
        ...
        let pipeline =
            cache.pipelines.get(&dst_wfmt).expect("pipeline built by ensure_pipeline above");

        let mut enc =
            device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("hl-blit") });
        {
            ...
            pass.set_scissor_rect(dst_origin.x, dst_origin.y, dst_extent.width, dst_extent.height);
            pass.draw(0..3, 0..1);
        }
