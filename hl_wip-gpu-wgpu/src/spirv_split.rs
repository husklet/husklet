...
//! The driver (`hl_wip-vulkan/src/service/record.rs`) applies the identical offset when it lowers a
...
pub fn split_combined_image_samplers(words: &[u32]) -> Result<Vec<u32>> {
    ...
    let combined_var_ids: Vec<(u32, u32, u32)> = var_candidates
        .iter()
        .filter_map(|&(var, ptr)| {
            ptr_to_sampled.get(&ptr).map(|&sampled| (var, sampled, sampled_to_image[&sampled]))
        })
        .collect();
    ...
    for &(op, s, len) in &insts {
        ...
        if !decorations_injected && is_type_op(op) {
            for &(var, _, _) in &combined_var_ids {
                let c = info[&var];
                emit(&mut out, OP_DECORATE, &[c.image_var, DEC_DESCRIPTOR_SET, c.set]);
                emit(&mut out, OP_DECORATE, &[c.image_var, DEC_BINDING, c.binding]);
                emit(&mut out, OP_DECORATE, &[c.sampler_var, DEC_DESCRIPTOR_SET, c.set]);
                emit(&mut out, OP_DECORATE, &[c.sampler_var, DEC_BINDING, c.binding + SAMPLER_BINDING_OFFSET]);
            }
        ...
        match op {
            ...
            OP_DECORATE | OP_MEMBER_DECORATE | OP_NAME if len >= 2 && combined_ids.contains(&words[s + 1]) => {}
            ...
            OP_VARIABLE if len >= 3 && combined_ids.contains(&words[s + 2]) => {
                ...
                if !ptr_sampler_emitted {
                    emit(&mut out, OP_TYPE_POINTER, &[ptr_sampler, SC_UNIFORM_CONSTANT, sampler_type]);
                    ptr_sampler_emitted = true;
                }
                if image_ptr_emit.contains(&c.image_type) && image_ptr_emitted.insert(c.image_type) {
                    emit(&mut out, OP_TYPE_POINTER, &[c.ptr_image, SC_UNIFORM_CONSTANT, c.image_type]);
                }
                emit(&mut out, OP_VARIABLE, &[c.ptr_image, c.image_var, SC_UNIFORM_CONSTANT]);
                emit(&mut out, OP_VARIABLE, &[ptr_sampler, c.sampler_var, SC_UNIFORM_CONSTANT]);
            }
            ...
            OP_LOAD if len >= 4 && combined_ids.contains(&words[s + 3]) => {
                ...
                emit(&mut out, OP_SAMPLED_IMAGE, &[result_type, result_id, img_tmp, smp_tmp]);
            }
...
mod tests {
    ...
    fn raw_combined_sampler_is_rejected_but_split_is_accepted() {
        ...
        assert!(!parses(&raw), "the raw combined-sampler SPIR-V must be rejected by naga (the gap)");
        // PASS-AFTER: the split rewrites it into the separate model naga accepts.
        let split = split_combined_image_samplers(&raw).unwrap();
        assert!(parses(&split), "the split SPIR-V must parse (separate image + sampler)");
    }
    ...
    fn split_produces_texture_and_sampler_at_coordinated_bindings() {
        ...
        let wgsl = crate::wgsl::spirv_to_wgsl(&combined_frag(0)).expect("spirv_to_wgsl through the split");
        assert!(wgsl.contains("texture_2d"), "expected a separate texture_2d: {wgsl}");
        assert!(wgsl.contains("sampler"), "expected a separate sampler: {wgsl}");
        assert!(wgsl.contains("@binding(16)"), "sampler must reflect at binding 0 + offset 16: {wgsl}");
        ...
        assert!(wgsl3.contains("@binding(19)"), "binding 3 + offset 16 = 19: {wgsl3}");
    }
    ...
    fn shader_without_combined_sampler_is_unchanged() {
        ...
        .validate(&module)
        .unwrap();
        let spirv = naga::back::spv::write_vec(&module, &info, &naga::back::spv::Options::default(), None).unwrap();
        let out = split_combined_image_samplers(&spirv).unwrap();
        assert_eq!(out, spirv, "a shader with no combined sampler must be returned byte-for-byte");
    }
}
