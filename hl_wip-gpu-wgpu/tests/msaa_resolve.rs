...
fn render(exec: &mut WgpuExecutor, sample_count: u32) -> Vec<u8> {
    ...
    let read_id = if sample_count > 1 {
        ...
        let mut cmds = vec![
            ...
            Cmd::CreateTexture(2, tex(W, H, 1, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC)),
        ];
        cmds.extend(shaders);
        ...
        let mut cmds =
            vec![Cmd::CreateTexture(1, tex(W, H, 1, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC))];
        cmds.extend(shaders);
        cmds.push(Cmd::Submit(CommandBuffer {
            encoder: vec![
                Enc::BeginRenderPass {
...
fn msaa_resolve_antialiases_the_edge() {
    ...
    assert_eq!(at(&msaa, interior.0, interior.1), FG, "MSAA interior must be exact fg (white)");
    assert_eq!(at(&msaa, exterior.0, exterior.1), BG, "MSAA exterior must be exact bg (black)");
    assert!(
        msaa_edge >= W / 2,
    ...
    assert_eq!(at(&noaa, interior.0, interior.1), FG, "1× interior must be exact fg (white)");
    assert_eq!(at(&noaa, exterior.0, exterior.1), BG, "1× exterior must be exact bg (black)");
    assert_eq!(
        noaa_edge, 0,
    ...
    let bad = hl_gpu::runtime::submit(
        ...
        0,
        &[
            Cmd::CreateTexture(1, tex(W, H, 1, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC)),
            Cmd::CreateTexture(2, tex(W, H, 1, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC)),
    ...
    assert!(bad.is_err(), "resolving a non-multisampled source must be a clean typed error, not a silent pass");
