...
use hl_gpu::protocol::model::capability::{
    command_bits, format_bits, shader_payload, ALL_COMMANDS, COLOR_FORMATS, DEPTH_FORMATS, PresentKind,
};
...
fn advertised_wire_version_and_command_set_are_the_full_ir() {
    ...
    let caps = exec.capabilities();
    assert_eq!(caps.wire_version, WIRE_VERSION, "advertised wire version must match the protocol");
    ...
    for etag in 0u8..64 {
        if caps.command_bits & (1u64 << etag) != 0 {
            assert!(ALL_COMMANDS.contains(&etag), "advertised etag {etag} is not a real IR command");
        }
    }
    assert_eq!(caps.max_bind_groups, 4, "advertised max_bind_groups pins the per-pass bind-group array size");
    assert_eq!(caps.present_kinds, vec![PresentKind::Shm], "the wgpu backend presents only via shm");
}
...
fn advertised_shader_payloads_match_what_is_accepted() {
    ...
    assert!(!caps.supports_shader_payload(shader_payload::MSL), "MSL must not be advertised (no wire path accepts it)");
    assert!(!caps.supports_shader_payload(shader_payload::WGSL), "WGSL must not be advertised (no wire path accepts it)");
}
...
fn every_advertised_texture_format_is_really_creatable() {
    ...
    assert_eq!(caps.texture_formats, expect, "advertised texture formats must be exactly the backed union");
    ...
    for fmt in all {
        ...
        let is_depth = matches!(fmt, TextureFormat::Depth32Float | TextureFormat::Depth24PlusStencil8);
        let usage = if is_depth {
            texture_usage::RENDER_TARGET
        } else {
            texture_usage::SAMPLED | texture_usage::COPY_SRC | texture_usage::COPY_DST | texture_usage::RENDER_TARGET
        };
        let mut s = session(&exec);
        let desc = TextureDesc {
            width: 2, height: 2, depth: 1, mip_levels: 1, sample_count: 1,
            dim: TextureDim::D2, format: fmt, usage, label: String::new(),
        };
...
fn timeline_fences_advertised_false_but_emulated_wait_services_a_signal() {
    ...
    assert!(!exec.capabilities().supports_timeline_fences, "the wgpu backend has no real external timeline fence");
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            Cmd::CreateFence(1),
            Cmd::Submit(CommandBuffer { encoder: vec![], signal: Some((1, 5)) }),
    ...
    assert!(r.is_err(), "a wait for a never-signalled fence value must error, not silently pass");
}
