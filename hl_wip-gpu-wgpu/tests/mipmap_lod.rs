...
fn explicit_lod_selects_the_matching_mip_level() {
    ...
    assert!(near(base, M0), "lod=0 must sample base mip {M0:?}, got {base:?}");
    assert!(near(mip1, M1), "lod=1 must sample mip 1 {M1:?}, got {mip1:?}");
    assert!(!near(base, mip1), "the two mip levels must differ for the demo to prove LOD selection");
