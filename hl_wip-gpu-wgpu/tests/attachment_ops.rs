...
fn preamble() -> Vec<Cmd> {
    vec![
        ...
        Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, &fs_const([1.0, 0.0, 0.0, 1.0])) },
        Cmd::CreateShader { id: 3, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, &fs_const([0.0, 1.0, 0.0, 1.0])) },
        pipe(1, 2), // red
        pipe(2, 3), // green
    ]
}
...
fn store_true_keeps_drawn_pixels_and_store_false_does_not_panic() {
    ...
    assert!(near(px(&out, W / 2, H / 2), RED), "store=true must keep the drawn red pixel, got {:?}", px(&out, W / 2, H / 2));
    ...
    cmds2.push(Cmd::Submit(CommandBuffer {
        encoder: vec![
            Enc::BeginRenderPass {
                color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0, 0.0, 0.0, 1.0], store: false }],
                depth: None,
            },
            Enc::SetPipeline(1),
            Enc::Draw { vertex_count: 3, instance_count: 1, first_vertex: 0, first_instance: 0 },
    ...
    for second_load in [LoadOp::Load, LoadOp::DontCare] {
        ...
        cmds.push(Cmd::Submit(CommandBuffer {
            encoder: vec![
                ...
                Enc::BeginRenderPass {
                    color: vec![ColorAttachment { texture: 1, load: second_load, clear: [0.0; 4], store: true }],
                    depth: None,
                },
                Enc::SetScissor { x: 0, y: 0, w: W / 2, h: H }, // restrict the green draw to the left half
                Enc::SetPipeline(2),
                Enc::Draw { vertex_count: 3, instance_count: 1, first_vertex: 0, first_instance: 0 },
                Enc::EndRenderPass,
            ],
        ...
        let out = exec.read_texture(&s.resources, 1).unwrap();
        assert!(near(px(&out, 0, H / 2), GREEN), "{second_load:?}: left half must be the freshly drawn green, got {:?}", px(&out, 0, H / 2));
        assert!(
