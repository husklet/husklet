...
fn negative_and_oversized_viewport_clips_and_does_not_nack() {
    ...
    assert_eq!(filled, W as usize * 24, "the clamped viewport must fill exactly the 32×24 visible band");
}
...
fn wholly_out_of_bounds_viewport_draws_nothing_without_nack() {
    ...
    for py in 0..H {
        for pxi in 0..W {
            let got = px(&img, W, pxi, py);
            assert!(near(got, CLEAR), "px ({pxi},{py}): {got:?} must be CLEAR — nothing should rasterize");
        }
    }
}
