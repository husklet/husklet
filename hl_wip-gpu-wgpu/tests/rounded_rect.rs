    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    Enc::BeginRenderPass {
                        color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: colf(BG), store: true }],
                        depth: None,
                    },
                    Enc::SetPipeline(1),
...
fn rounded_rect_sdf_rounds_the_corners_at_the_exact_radius() {
    ...
    for &(x, y) in &[(6u32, 6u32), (41, 6), (6, 41), (41, 41)] {
        ...
        assert!(sdf(x, y) > 1.5, "corner ({x},{y}) must be clearly OUTSIDE the rounded shape in the reference");
        assert!(
    ...
    for &(x, y) in &[(24u32, 8u32), (24, 39), (8, 24), (39, 24)] {
        assert!(sdf(x, y) < -1.5, "edge midpoint ({x},{y}) must be clearly INSIDE in the reference");
        assert!(
