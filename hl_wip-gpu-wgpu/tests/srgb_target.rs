...
fn srgb_target_gamma_encodes_on_write_and_decodes_on_sample() {
    ...
    for (i, p) in unorm.chunks_exact(4).enumerate() {
        assert!(
            near_tol([p[0], p[1], p[2], p[3]], [NAIVE_LINEAR_HALF, NAIVE_LINEAR_HALF, NAIVE_LINEAR_HALF, 255], 2),
    ...
    let from_srgb = sample_stored(&mut exec, TextureFormat::Rgba8Srgb, [SRGB_OF_LINEAR_HALF, SRGB_OF_LINEAR_HALF, SRGB_OF_LINEAR_HALF, 255]);
    // The SAME 188 in a plain Unorm texture is NOT decoded → samples back as 188.
    let from_unorm = sample_stored(&mut exec, TextureFormat::Rgba8Unorm, [SRGB_OF_LINEAR_HALF, SRGB_OF_LINEAR_HALF, SRGB_OF_LINEAR_HALF, 255]);
