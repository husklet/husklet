...
fn run(exec: &mut WgpuExecutor, src: [f32; 4], dst: [f32; 4], blend: BlendState) -> [u8; 4] {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, FS) },
...
fn expected(src: [f32; 4], dst: [f32; 4], b: &BlendState) -> [u8; 4] {
    ...
    for ch in 0..3 {
        ...
        out[ch] = u8c(apply_op(b.op_color, src[ch] * sf, dst[ch] * df, src[ch], dst[ch]));
    }
    let sf = factor(b.src_alpha, 3, src, dst);
    let df = factor(b.dst_alpha, 3, src, dst);
    out[3] = u8c(apply_op(b.op_alpha, src[3] * sf, dst[3] * df, src[3], dst[3]));
    out
}
...
fn every_blend_factor_and_op_composites_exactly() {
    ...
    let factors = [
        ...
        ONE_MINUS_DST_COLOR, DST_ALPHA, ONE_MINUS_DST_ALPHA, SRC_ALPHA_SATURATE,
    ];
    for &f in &factors {
    ...
    for &op in &[OP_ADD, OP_SUBTRACT, OP_REVERSE_SUBTRACT, OP_MIN, OP_MAX] {
        ...
        let got = run(&mut exec, src, dst, b.clone());
        let want = expected(src, dst, &b);
        assert!(near(got, want), "blend op code {op}: expected {want:?}, got {got:?}");
    }
    ...
    let add = run(
        ...
        src,
        dst,
        BlendState { src_color: ONE, dst_color: ONE, op_color: OP_ADD, src_alpha: ONE, dst_alpha: ZERO, op_alpha: OP_ADD },
    );
    let min = run(
        &mut exec,
        src,
        dst,
        BlendState { src_color: ONE, dst_color: ONE, op_color: OP_MIN, src_alpha: ONE, dst_alpha: ZERO, op_alpha: OP_ADD },
    );
    assert!(!near(add, min), "ADD {add:?} and MIN {min:?} must differ — proves op_color is honored");
}
