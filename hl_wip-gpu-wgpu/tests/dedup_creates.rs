...
use hl_gpu::protocol::model::enums::{
    texture_usage, LoadOp, TextureDim, TextureFormat, Topology,
};
...
fn identical_shaders_share_one_backing() {
    ...
    let cmds: Vec<Cmd> = (1..=N)
        .map(|id| Cmd::CreateShader { id, kind: ShaderPayloadKind::Glsl, spirv: words.clone() })
        .collect();
...
fn alias_survives_destroy_of_other_alias() {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: fs.clone() },
            Cmd::CreateShader { id: 3, kind: ShaderPayloadKind::Glsl, spirv: fs.clone() },
        ],
    )
    .expect("VS + two fragment aliases create");

    // 1 VS + 1 shared FS backing (ids 2 and 3 alias the same FS module).
    assert_eq!(exec.shader_backing_count(), 2, "the two FS aliases must share one backing");
...
fn deduped_pipeline_renders_exact_pixels() {
    ...
    hl_gpu::runtime::submit(
        ...
        0,
        &[
            Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: vs.clone() },
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: vs },
            Cmd::CreateShader { id: 3, kind: ShaderPayloadKind::Glsl, spirv: fs.clone() },
            Cmd::CreateShader { id: 4, kind: ShaderPayloadKind::Glsl, spirv: fs },
