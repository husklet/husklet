...
fn pipeline(module: u32, depth: DepthState, topology: Topology) -> RenderPipelineDesc {
    RenderPipelineDesc {
        vertex: ShaderRef { module, entry: "vs_main".into() },
        fragment: Some(ShaderRef { module, entry: "fs_main".into() }),
        vertex_buffers: vec![],
...
fn run(exec: &mut WgpuExecutor, stencil_enabled: bool) -> Vec<u8> {
    ...
    exec.read_texture(&s.resources, 1).expect("read color target")
}
...
fn stencil_test_gates_the_draw_to_the_marked_rect() {
    ...
    assert_eq!(count_color(&enabled, BLUE), (N * N) as usize - 64, "the rest must be the blue clear");
