...
fn glyph_atlas_bilinear_coverage_matches_the_subpixel_reference() {
    ...
    for uv in positions {
        ...
        write_png(&format!("text_coverage_{:.0}_{:.0}", uv[0] * 100.0, uv[1] * 100.0), 1, 1, &got);
        assert!(
