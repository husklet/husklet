...
fn blit_scaled(
    ...
    exec.read_texture(&s.resources, 2).expect("read the blit destination")
}
...
fn scaling_blit_upscales_exact_pixels() {
    ...
    for y in 0..3u32 {
        assert_eq!(px(&lin, 3, 0, y), al, "LINEAR: left column (u=1/6, clamp) must be the left texel");
        assert_eq!(
            px(&lin, 3, 1, y),
            mid,
            "LINEAR: middle column sits at exactly u=0.5 ⇒ the interpolated midpoint (al+bl)/2 = {mid:?}"
        );
        assert_eq!(px(&lin, 3, 2, y), bl, "LINEAR: right column (u=5/6, clamp) must be the right texel");
    }
    ...
    for y in 0..64u32 {
        for x in 0..64u32 {
            ...
            assert_eq!(px(&near_big, 64, x, y), want, "NEAREST 2x2->64x64 block mismatch at ({x},{y})");
        }
    }
    ...
    for y in 0..64u32 {
        for x in 1..64u32 {
            ...
            assert!(r >= l, "LINEAR gradient must be non-decreasing L→R at row {y} col {x}: {l} then {r}");
        }
    }
}
