...
const USAGE: u32 =
    texture_usage::SAMPLED | texture_usage::RENDER_TARGET | texture_usage::COPY_SRC | texture_usage::COPY_DST;
...
fn copy_full(
        ...
        &[
            ...
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    ...
                    Enc::ClearRect { texture: 2, x: 0, y: 0, w, h, color: dst_clear },
    ...
    exec.read_texture(&s.resources, 2).expect("read the copy destination")
}
...
fn rgba_to_bgra_copy_converts_channels_exact() {
    let Some(mut exec) = exec_or_skip() else { return };
    ...
    let out = copy_full(&mut exec, 2, 2, TextureFormat::Rgba8Unorm, &src, TextureFormat::Bgra8Unorm, [0.0; 4]);

    let swap = |c: [u8; 4]| [c[2], c[1], c[0], c[3]];
    assert_eq!(px(&out, 2, 0, 0), swap(A), "texel (0,0) must be channel-swapped exactly");
    assert_eq!(px(&out, 2, 1, 0), swap(B), "texel (1,0) must be channel-swapped exactly");
    assert_eq!(px(&out, 2, 0, 1), swap(C), "texel (0,1) must be channel-swapped exactly");
    assert_eq!(px(&out, 2, 1, 1), swap(D), "texel (1,1) must be channel-swapped exactly");
}
...
fn cross_format_subregion_copy_converts_and_leaves_rest_untouched() {
    ...
    let swap = |c: [u8; 4]| [c[2], c[1], c[0], c[3]];
    assert_eq!(px(&out, 2, 0, 0), swap(D), "the copied 1x1 region is D, channel-swapped");
    assert_eq!(px(&out, 2, 1, 0), red_bgra, "texel (1,0) must be untouched (pre-clear)");
    assert_eq!(px(&out, 2, 0, 1), red_bgra, "texel (0,1) must be untouched (pre-clear)");
    assert_eq!(px(&out, 2, 1, 1), red_bgra, "texel (1,1) must be untouched (pre-clear)");
}
...
fn same_format_copy_is_exact_raw() {
    ...
    let src: Vec<u8> = [A, B, C, D].concat();
    let out = copy_full(&mut exec, 2, 2, TextureFormat::Rgba8Unorm, &src, TextureFormat::Rgba8Unorm, [0.0; 4]);
    assert_eq!(out, src, "same-format copy must be byte-exact (raw fast path)");
}
...
fn srgb_variant_copy_stays_on_the_raw_fast_path() {
    ...
    let src: Vec<u8> = [A, B, C, D].concat();
    let out = copy_full(&mut exec, 2, 2, TextureFormat::Rgba8Unorm, &src, TextureFormat::Rgba8Srgb, [0.0; 4]);
    assert_eq!(out, src, "sRGB-variant copy is copy-compatible → raw byte copy, bytes preserved");
}
