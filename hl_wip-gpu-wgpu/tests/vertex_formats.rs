...
use hl_gpu::protocol::model::descriptor::{
    ColorTargetState, RenderPipelineDesc, ShaderRef, VertexAttr, VertexLayout, TextureDesc,
};
...
fn class_of(kind: u32, normalized: bool) -> Class {
    match kind {
        ...
        5 => Class::Uint,               // u32
        6 => Class::Sint,               // i32
        1 | 3 => if normalized { Class::Float } else { Class::Uint }, // u8 / u16
        2 | 4 => if normalized { Class::Float } else { Class::Sint }, // i8 / i16
        _ => Class::Float,
    }
}
...
fn vs_for(class: Class, comps: u32) -> String {
    let (decl, scalar) = match class {
        Class::Float => (["float", "vec2", "vec3", "vec4"][(comps - 1) as usize], if comps == 1 { "a" } else { "a.x" }),
        Class::Uint => (["uint", "uvec2", "uvec3", "uvec4"][(comps - 1) as usize], if comps == 1 { "a" } else { "a.x" }),
        Class::Sint => (["int", "ivec2", "ivec3", "ivec4"][(comps - 1) as usize], if comps == 1 { "a" } else { "a.x" }),
    };
    format!(
...
fn build(exec: &mut WgpuExecutor, packed: u32, class: Class, comps: u32) -> hl_gpu::Result<Session> {
    ...
    let cmds = vec![
        ...
        Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::VERTEX, &vs_for(class, comps)) },
        ...
        Cmd::CreateRenderPipeline(
            ...
            RenderPipelineDesc {
                ...
                vertex_buffers: vec![VertexLayout {
                    stride: 32,
                    step_mode: 0,
                    attrs: vec![VertexAttr { location: 0, format: packed, offset: 0 }],
                }],
...
fn every_supported_vertex_format_builds_a_pipeline() {
    ...
    assert_eq!(count, 30, "all 30 supported vertex formats must be exercised");
}
...
fn webgpu_invalid_vertex_formats_are_honest_errors() {
    ...
    let invalid = [
        (1u32, 1u32), (1, 3), (2, 1), (2, 3), (3, 1), (3, 3), (4, 1), (4, 3), (7, 1), (7, 3),
    ];
    for (kind, comps) in invalid {
        let packed = vfmt(comps, kind, false);
        let r = build(&mut exec, packed, Class::Float, comps);
        assert!(r.is_err(), "vertex format kind={kind} comps={comps} is WebGPU-invalid and must error, got Ok");
    }
}
