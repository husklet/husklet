...
fn ct(fmt: TextureFormat) -> ColorTargetState {
    ColorTargetState { format: fmt, blend: None, write_mask: 0xF }
}
...
fn draw_const(exec: &mut WgpuExecutor, fmt: TextureFormat) -> Vec<u8> {
    ...
    hl_gpu::runtime::submit(
        ...
        0,
        &[
            Cmd::CreateTexture(1, tex(W, H, fmt, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC)),
            Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::VERTEX, "vmain", VS) },
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, "fmain", FS_CONST) },
    ...
    .unwrap_or_else(|e| panic!("format {fmt:?}: the constant-color draw must run cleanly, got {e:?}"));
    exec.read_texture(&s.resources, 1).unwrap_or_else(|e| panic!("format {fmt:?}: readback failed: {e:?}"))
}
...
fn sample_stored(exec: &mut WgpuExecutor, src_fmt: TextureFormat, stored: &[u8]) -> [u8; 4] {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateTexture(2, tex(1, 1, src_fmt, texture_usage::SAMPLED | texture_usage::COPY_DST)),
            Cmd::CreateBuffer(1, BufferDesc { size: stored.len() as u64, usage: buffer_usage::COPY_SRC, label: String::new() }),
            Cmd::WriteBuffer { id: 1, offset: 0, data: stored.to_vec() },
            Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::VERTEX, "vmain", VS) },
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, "fmain", FS_SAMPLE) },
            ...
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    Enc::CopyBufferToTexture { src: 1, src_offset: 0, bytes_per_row: stored.len() as u32, dst: 2, mip: 0, width: 1, height: 1 },
                    Enc::BeginRenderPass {
                        color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0, 0.0, 0.0, 1.0], store: true }],
                        depth: None,
                    },
...
fn every_color_format_roundtrips_exact_stored_bytes() {
    ...
    for &fmt in COLOR_FORMATS {
        ...
        let bpt = fmt.bytes_per_texel().expect("color format has a texel footprint");
        assert_eq!(raw.len(), 2 * 2 * bpt, "format {fmt:?}: readback is width*height*bpt");
        let t0 = &raw[..bpt]; // first texel — every texel is identical (constant draw)

        match fmt {
            TextureFormat::Rgba8Unorm => {
                assert!(near_tol([t0[0], t0[1], t0[2], t0[3]], [ru, gu, bu, au], 2),
                    "Rgba8Unorm: RGBA order, got {t0:?}, want [{ru},{gu},{bu},{au}]");
            }
            TextureFormat::Bgra8Unorm => {
                // Stored bytes are B,G,R,A — the swizzle vs RGBA is the whole point.
                assert!(near_tol([t0[0], t0[1], t0[2], t0[3]], [bu, gu, ru, au], 2),
            ...
            TextureFormat::Rgba8Srgb => {
                ...
                assert!(near_tol([t0[0], t0[1], t0[2], t0[3]], [rs, gs, bs, au], 2),
            ...
            TextureFormat::R8Unorm => {
                assert!((t0[0] as i16 - ru as i16).abs() <= 2,
                    "R8Unorm: single byte = unorm8(R) = {ru}, got {}", t0[0]);
            }
            TextureFormat::Rg8Unorm => {
                assert!((t0[0] as i16 - ru as i16).abs() <= 2 && (t0[1] as i16 - gu as i16).abs() <= 2,
                    "Rg8Unorm: bytes = [unorm8(R),unorm8(G)] = [{ru},{gu}], got {:?}", &t0[..2]);
            }
            TextureFormat::Rgba16Float => {
                let got = [half_to_f32(le_u16(&t0[0..2])), half_to_f32(le_u16(&t0[2..4])),
                           half_to_f32(le_u16(&t0[4..6])), half_to_f32(le_u16(&t0[6..8]))];
                for k in 0..4 {
            ...
            TextureFormat::Rgba32Float => {
                let got = [le_f32_at(&t0[0..4]), le_f32_at(&t0[4..8]), le_f32_at(&t0[8..12]), le_f32_at(&t0[12..16])];
                assert_eq!(got, C, "Rgba32Float: f32 texels must be C exactly, got {got:?}");
            }
        ...
        if matches!(fmt, TextureFormat::Rgba8Unorm | TextureFormat::Bgra8Unorm
        ...
        eprintln!("format {fmt:?}: {bpt} B/texel, stored texel {:?} — exact round-trip OK", t0);
    }
}
...
fn bgra_swizzles_and_srgb_decodes_on_sample() {
    ...
    assert!(near_tol(raw, [225, 188, 137, 255], 2),
        "Bgra8Unorm passthrough (no decode) must sample R,G,B=[225,188,137], got {raw:?}");
    assert!(raw[0] as i16 - got[0] as i16 >= 20,
        "the sRGB texel decodes ({}) but the identical Unorm texel does not ({}) — decode is real", got[0], raw[0]);

    eprintln!("sample: Bgra8Unorm swizzles B,G,R→R,G,B; Bgra8Srgb additionally decodes sRGB→linear");
}
...
fn depth_run(exec: &mut WgpuExecutor, ds_fmt: TextureFormat, cmp: u32) -> [u8; 4] {
    ...
    let pipe = |module_vs: u32, module_fs: u32| RenderPipelineDesc {
        vertex: ShaderRef { module: module_vs, entry: "vmain".into() },
        fragment: Some(ShaderRef { module: module_fs, entry: "fmain".into() }),
        vertex_buffers: vec![],
        color_targets: vec![ct(TextureFormat::Rgba8Unorm)],
        depth: Some(ds.clone()),
        topology: Topology::TriangleList,
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: depth_vs(0.5) },
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: depth_fs(GREEN) },
            Cmd::CreateShader { id: 3, kind: ShaderPayloadKind::Glsl, spirv: depth_vs(0.2) },
            Cmd::CreateShader { id: 4, kind: ShaderPayloadKind::Glsl, spirv: depth_fs(BLUE) },
            Cmd::CreateShader { id: 5, kind: ShaderPayloadKind::Glsl, spirv: depth_vs(0.8) },
            Cmd::CreateShader { id: 6, kind: ShaderPayloadKind::Glsl, spirv: depth_fs(RED) },
            Cmd::CreateRenderPipeline(1, pipe(1, 2)),
            Cmd::CreateRenderPipeline(2, pipe(3, 4)),
            Cmd::CreateRenderPipeline(3, pipe(5, 6)),
            Cmd::Submit(CommandBuffer { encoder: enc, signal: None }),
        ],
    )
...
fn depth_formats_nearest_occludes_regardless_of_draw_order() {
    ...
    for &ds_fmt in &[TextureFormat::Depth32Float, TextureFormat::Depth24PlusStencil8] {
...
fn executor_advertises_exactly_the_formats_this_suite_proves() {
    ...
    for &fmt in COLOR_FORMATS {
        assert!(fmt.bytes_per_texel().is_some(), "{fmt:?}: advertised color format must have a texel footprint");
    }
}
