...
fn draw(topo: Topology, verts: &[[f32; 2]]) -> Vec<u8> {
    let mut exec = WgpuExecutor::new(DeviceConfig::default()).expect("adapter already probed reachable");
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateBuffer(
                1,
                BufferDesc { size: (data.len() * 4) as u64, usage: buffer_usage::VERTEX, label: String::new() },
            ),
            Cmd::WriteBuffer { id: 1, offset: 0, data: le_f32(&data) },
            ...
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    ...
                    Enc::Draw { vertex_count: n, instance_count: 1, first_vertex: 0, first_instance: 0 },
                    Enc::EndRenderPass,
                ],
                signal: None,
            }),
...
fn line_pixels(a: (i32, i32), b: (i32, i32)) -> Vec<(i32, i32)> {
    ...
    (0..=steps).map(|i| (a.0 + i * sx, a.1 + i * sy)).filter(|&p| p != excluded).collect()
}
...
fn ascii(mask_or_px: impl Fn(u32) -> bool) -> String {
    ...
    for py in 0..TARGET {
        for px in 0..TARGET {
            s.push(if mask_or_px(py * TARGET + px) { '#' } else { '.' });
        }
        s.push('\n');
    }
    s
...
fn assert_exact_and_write(name: &str, px: &[u8], mask: &Mask) {
    ...
    for i in 0..(TARGET * TARGET) as usize {
        ...
        if !approx(p, want) {
            bad += 1;
            if first.is_none() {
                first = Some(((i as u32) % TARGET, (i as u32) / TARGET, [p[0], p[1], p[2], p[3]], want));
            }
        }
    }
    if bad != 0 {
...
fn pointlist_lights_exact_pixels() {
    ...
    let pts = [(2, 2), (29, 2), (2, 29), (29, 29), (16, 16), (10, 20), (23, 8), (16, 17)];
...
fn trianglestrip_quad_equals_trianglelist() {
    ...
    let list = draw(Topology::TriangleList, &[c(tl), c(tr), c(bl), c(tr), c(bl), c(br)]);
