...
fn sample_slice(exec: &mut WgpuExecutor, w: f32) -> [u8; 4] {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateTexture(2, tex3d(1, 1, 3, texture_usage::SAMPLED | texture_usage::COPY_DST)),
            Cmd::CreateBuffer(1, BufferDesc { size: 16, usage: buffer_usage::UNIFORM, label: String::new() }),
            Cmd::WriteBuffer { id: 1, offset: 0, data: le_f32(&[0.5, 0.5, w, 0.0]) },
            Cmd::CreateBuffer(2, BufferDesc { size: 12, usage: buffer_usage::COPY_SRC, label: String::new() }),
            Cmd::WriteBuffer { id: 2, offset: 0, data: volume },
...
fn sampling_a_depth_slice_returns_that_slices_texel() {
    ...
    for (slice, &want) in SLICES.iter().enumerate() {
        ...
        assert!(near(got, want), "slice {slice} (w={w}): must sample {want:?}, got {got:?}");
    }
