...
fn bind_group(set_id: u32, uni: u32, texture: u32) -> Cmd {
    Cmd::CreateBindGroup(
        ...
        BindGroupDesc {
            set: 0,
            entries: vec![
                BindEntry { binding: 0, resource: BindResource::Buffer { id: uni, offset: 0, size: 16 } },
                BindEntry { binding: 1, resource: BindResource::Texture { id: texture } },
...
fn render(exec: &mut WgpuExecutor) -> Vec<u8> {
    ...
    for y in 0..H {
        for x in 0..W {
            let v = if feat(x as i32, y as i32) == 1 { 255 } else { 0 };
    ...
    hl_gpu::runtime::submit(
        ...
        0,
        &[
            Cmd::CreateTexture(1, tex(W, H, texture_usage::SAMPLED | texture_usage::COPY_DST)), // T0 source
            Cmd::CreateTexture(2, tex(W, H, texture_usage::RENDER_TARGET | texture_usage::SAMPLED)), // T1
            Cmd::CreateTexture(3, tex(W, H, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC)), // T2
            Cmd::CreateBuffer(1, BufferDesc { size: (W * H * 4) as u64, usage: buffer_usage::COPY_SRC, label: String::new() }),
            Cmd::WriteBuffer { id: 1, offset: 0, data: src },
            Cmd::CreateBuffer(2, BufferDesc { size: 16, usage: buffer_usage::UNIFORM, label: String::new() }),
            Cmd::WriteBuffer { id: 2, offset: 0, data: uni_h },
            Cmd::CreateBuffer(3, BufferDesc { size: 16, usage: buffer_usage::UNIFORM, label: String::new() }),
            Cmd::WriteBuffer { id: 3, offset: 0, data: uni_v },
            ...
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    ...
                    Enc::BeginRenderPass {
                        color: vec![ColorAttachment { texture: 3, load: LoadOp::Clear, clear: [0.0, 0.0, 0.0, 1.0], store: true }],
                        depth: None,
                    },
                    Enc::SetPipeline(1),
...
fn reference() -> Vec<u8> {
    ...
    for y in 0..H as i32 {
        for x in 0..W as i32 {
            let sum = feat(clampx(x - 1), y) as f32 + feat(x, y) as f32 + feat(clampx(x + 1), y) as f32;
...
fn separable_box_blur_matches_the_two_stage_convolution() {
    ...
    for y in 0..H {
        for x in 0..W {
            ...
            let src_v = if feat(x as i32, y as i32) == 1 { 255 } else { 0 };
    ...
    assert!(px(&img, W, 7, 7)[0] >= 250, "the feature core must stay ~white after a radius-1 blur");
    assert_eq!(px(&img, W, 0, 0)[0], 0, "a far-corner background pixel must stay pure black");
