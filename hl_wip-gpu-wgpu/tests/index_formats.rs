...
fn run(exec: &mut WgpuExecutor, fmt: IndexFormat, idx: Vec<u8>) -> Vec<u8> {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateBuffer(1, BufferDesc { size: 32, usage: buffer_usage::VERTEX, label: String::new() }),
            Cmd::WriteBuffer { id: 1, offset: 0, data: le_f32(&QUAD) },
            Cmd::CreateBuffer(2, BufferDesc { size: idx.len() as u64, usage: buffer_usage::INDEX, label: String::new() }),
            Cmd::WriteBuffer { id: 2, offset: 0, data: idx },
            ...
            Cmd::CreateRenderPipeline(
                ...
                RenderPipelineDesc {
                    ...
                    vertex_buffers: vec![VertexLayout {
                        stride: 8,
                        step_mode: 0,
                        attrs: vec![VertexAttr { location: 0, format: VFMT_F32X2, offset: 0 }],
                    }],
            ...
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    ...
                    Enc::SetIndexBuffer { buffer: 2, offset: 0, format: fmt },
...
fn u16_and_u32_indices_produce_identical_pixels() {
    ...
    let u32_bytes: Vec<u8> = INDICES.iter().flat_map(|v| (*v as u32).to_le_bytes()).collect();
    ...
    assert_eq!(a, b, "u16 and u32 index buffers must produce identical pixels");
    ...
    for py in 0..H {
        for pxi in 0..W {
            ...
            assert!(near(got, want), "px ({pxi},{py}): {got:?} != {want:?} (inside={inside})");
            if inside {
                filled += 1;
            }
        }
    }
    assert_eq!(filled, 16 * 16, "the centered quad must cover exactly a 16×16 block");
