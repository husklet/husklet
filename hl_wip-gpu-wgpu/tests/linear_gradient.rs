...
fn linear_gradient_matches_the_analytic_lerp() {
    ...
    assert!(near2(mid_top, mid_bot), "gradient must be purely horizontal: {mid_top:?} vs {mid_bot:?}");
    ...
    eprintln!("technique 2 (linear_gradient): endpoints+midpoint match analytic lerp within ±2 — PASS");
}
