...
fn sample_1d(exec: &mut WgpuExecutor, coord: f32) -> [u8; 4] {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateTexture(2, tex1d(2, texture_usage::SAMPLED | texture_usage::COPY_DST)),
            Cmd::CreateBuffer(1, BufferDesc { size: 16, usage: buffer_usage::UNIFORM, label: String::new() }),
            Cmd::WriteBuffer { id: 1, offset: 0, data: le_f32(&[coord, 0.0, 0.0, 0.0]) },
            Cmd::CreateBuffer(2, BufferDesc { size: 8, usage: buffer_usage::COPY_SRC, label: String::new() }),
            Cmd::WriteBuffer { id: 2, offset: 0, data: row },
...
fn sampling_a_1d_texel_returns_that_texel() {
    ...
    for (texel, &want) in TEXELS.iter().enumerate() {
        ...
        assert!(near(got, want), "texel {texel} (u={coord}): must sample {want:?}, got {got:?}");
    }
