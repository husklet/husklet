...
//! three (it rejects a non-zero mip copy, and never culls or masks — see `hl_wip-gpu/src/cpu`), exactly like
...
use hl_gpu::protocol::model::enums::{buffer_usage, texture_usage, LoadOp, TextureFormat, Topology};
...
fn const_pipeline(cull: u32, front_face: u32, write_mask: u32) -> RenderPipelineDesc {
    RenderPipelineDesc {
        ...
        vertex_buffers: vec![],
        color_targets: vec![ColorTargetState { format: TextureFormat::Rgba8Unorm, blend: None, write_mask }],
        depth: None,
...
fn render(
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::VERTEX, "vmain", VS_FULLSCREEN) },
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, "fmain", &fs_const(color)) },
            Cmd::CreateRenderPipeline(1, const_pipeline(cull, front_face, write_mask)),
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    Enc::BeginRenderPass {
                        color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: clear_f, store: true }],
                        depth: None,
                    },
                    Enc::SetPipeline(1),
...
fn copy_texture_to_buffer_reads_the_named_mip_level() {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateTexture(1, tex2d_mips(2, 2, 2, texture_usage::COPY_DST | texture_usage::COPY_SRC)),
            Cmd::CreateBuffer(1, BufferDesc { size: 16, usage: buffer_usage::COPY_SRC, label: String::new() }),
            Cmd::WriteBuffer { id: 1, offset: 0, data: m0_plane },
            Cmd::CreateBuffer(2, BufferDesc { size: 4, usage: buffer_usage::COPY_SRC, label: String::new() }),
            Cmd::WriteBuffer { id: 2, offset: 0, data: M1.to_vec() },
            // Readback destinations (mip 1 → buf 3, mip 0 first texel → buf 4).
            Cmd::CreateBuffer(3, BufferDesc { size: 4, usage: buffer_usage::COPY_DST, label: String::new() }),
            Cmd::CreateBuffer(4, BufferDesc { size: 4, usage: buffer_usage::COPY_DST, label: String::new() }),
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    // Upload each level to its OWN mip slot.
                    Enc::CopyBufferToTexture { src: 1, src_offset: 0, bytes_per_row: 8, dst: 1, mip: 0, width: 2, height: 2 },
                    Enc::CopyBufferToTexture { src: 2, src_offset: 0, bytes_per_row: 4, dst: 1, mip: 1, width: 1, height: 1 },
                    // Read back the 1×1 MIP-1 level: must be M1, NOT the base level's M0.
                    Enc::CopyTextureToBuffer { src: 1, mip: 1, width: 1, height: 1, dst: 3, dst_offset: 0, bytes_per_row: 4 },
                    // Read back the base level's first texel: M0 (proving mip 0 still works).
                    Enc::CopyTextureToBuffer { src: 1, mip: 0, width: 1, height: 1, dst: 4, dst_offset: 0, bytes_per_row: 4 },
                ],
                signal: None,
            }),
        ],
    ...
    assert_eq!(mip0.as_slice(), M0.as_slice(), "CopyTextureToBuffer {{ mip: 0 }} must still read the base texel");
}
...
fn copy_texture_to_buffer_rejects_out_of_range_mip() {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            Cmd::CreateTexture(1, tex2d_mips(2, 2, 2, texture_usage::COPY_DST | texture_usage::COPY_SRC)),
            Cmd::CreateBuffer(1, BufferDesc { size: 4, usage: buffer_usage::COPY_DST, label: String::new() }),
        ],
    )
    ...
    assert!(r.is_err(), "a CopyTextureToBuffer naming a non-existent mip must error, not fake success");
}
...
fn cull_and_front_face_are_honored() {
    ...
    assert!(drawn(none), "cull=None must draw the triangle: expected ~{DRAW:?}, got {none:?}");
