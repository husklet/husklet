...
fn depth_tex() -> TextureDesc {
    TextureDesc {
        width: W, height: H, depth: 1, mip_levels: 1, sample_count: 1,
        dim: TextureDim::D2, format: TextureFormat::Depth32Float,
...
fn drew_through(exec: &mut WgpuExecutor, frag: f32, stored: f32, depth_compare: u32) -> bool {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::WriteBuffer { id: 1, offset: 0, data: [frag, 0.0, 0.0, 0.0].iter().flat_map(|f: &f32| f.to_le_bytes()).collect() },
            ...
            Cmd::CreateRenderPipeline(
                ...
                RenderPipelineDesc {
                    ...
                    depth: Some(DepthState::depth_only(TextureFormat::Depth32Float, false, depth_compare)),
            ...
            Cmd::CreateBindGroup(
                1,
                BindGroupDesc { set: 0, entries: vec![BindEntry { binding: 0, resource: BindResource::Buffer { id: 1, offset: 0, size: 16 } }] },
            ),
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    Enc::BeginRenderPass {
                        color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0, 0.0, 0.0, 1.0], store: true }],
                        depth: Some(DepthAttachment { texture: 2, load: LoadOp::Clear, clear_depth: stored, clear_stencil: 0 }),
                    },
                    Enc::SetPipeline(1),
...
fn every_depth_compare_gates_exactly_like_the_oracle() {
    ...
    let stored = 0.5_f32;
    let codes = [
        compare::NEVER, compare::LESS, compare::EQUAL, compare::LESS_EQUAL, compare::GREATER,
        compare::NOT_EQUAL, compare::GREATER_EQUAL, compare::ALWAYS,
    ];
