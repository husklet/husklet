...
fn sample_layer(exec: &mut WgpuExecutor, k: u32) -> [u8; 4] {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateTexture(2, tex2d_array(4, texture_usage::SAMPLED | texture_usage::COPY_DST)),
            Cmd::CreateBuffer(1, BufferDesc { size: 16, usage: buffer_usage::UNIFORM, label: String::new() }),
            Cmd::WriteBuffer { id: 1, offset: 0, data: le_f32(&[k as f32, 0.0, 0.0, 0.0]) },
            Cmd::CreateBuffer(2, BufferDesc { size: 16, usage: buffer_usage::COPY_SRC, label: String::new() }),
            Cmd::WriteBuffer { id: 2, offset: 0, data: all },
...
fn sampling_an_array_layer_returns_that_layers_texel() {
    ...
    for (layer, &want) in LAYERS.iter().enumerate() {
        ...
        assert!(near(got, want), "layer {layer}: must sample {want:?}, got {got:?}");
    }
