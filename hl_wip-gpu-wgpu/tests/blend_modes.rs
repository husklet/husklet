...
fn run(exec: &mut WgpuExecutor, src: [f32; 4], dst: [f32; 4], blend: BlendState) -> [u8; 4] {
    ...
    let target = ColorTargetState { format: TextureFormat::Rgba8Unorm, blend: Some(blend), write_mask: 0xF };
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::WriteBuffer { id: 1, offset: 0, data: src.iter().flat_map(|f| f.to_le_bytes()).collect() },
            Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::VERTEX, "main", VS) },
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, "main", FS) },
            ...
            Cmd::CreateBindGroup(
                ...
                BindGroupDesc {
                    set: 0,
                    entries: vec![BindEntry { binding: 0, resource: BindResource::Buffer { id: 1, offset: 0, size: 16 } }],
                },
            ),
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    Enc::BeginRenderPass {
                        color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: dst, store: true }],
                        depth: None,
                    },
                    Enc::SetPipeline(1),
...
fn blend(src_color: u32, dst_color: u32, src_alpha: u32, dst_alpha: u32) -> BlendState {
    BlendState { src_color, dst_color, op_color: OP_ADD, src_alpha, dst_alpha, op_alpha: OP_ADD }
}
...
fn porter_duff_src_over_multiply_and_screen_match_their_identities() {
    ...
    let src_over = run(&mut exec, src, dst, blend(ONE, ONE_MINUS_SRC_ALPHA, ONE, ONE_MINUS_SRC_ALPHA));
    let so_ref = [
    ...
    assert!(near2(src_over, so_ref), "SrcOver: expected src+dst*(1-srcA) = {so_ref:?}, got {src_over:?}");

    // ---- Multiply: out.rgb = src*dst (alpha kept as src.a via ONE/ZERO). ----
    let multiply = run(&mut exec, src, dst, blend(DST_COLOR, ZERO, ONE, ZERO));
    let mul_ref = [u8c(src[0] * d[0]), u8c(src[1] * d[1]), u8c(src[2] * d[2]), u8c(src[3])];
    assert!(near2(multiply, mul_ref), "Multiply: expected src*dst = {mul_ref:?}, got {multiply:?}");

    // ---- Screen: out.rgb = src + dst*(1-src) = src + dst - src*dst (alpha kept as src.a). ----
    let screen = run(&mut exec, src, dst, blend(ONE, ONE_MINUS_SRC_COLOR, ONE, ZERO));
    let scr_ref = [
    ...
    assert!(near2(screen, scr_ref), "Screen: expected src+dst-src*dst = {scr_ref:?}, got {screen:?}");
    ...
    assert!(!near2(multiply, screen), "Multiply {multiply:?} and Screen {screen:?} must differ");
    assert!(!near2(multiply, src_over), "Multiply {multiply:?} and SrcOver {src_over:?} must differ");
    assert!(!near2(screen, src_over), "Screen {screen:?} and SrcOver {src_over:?} must differ");
    ...
    for k in 0..3 {
        assert!(multiply[k] <= (d[k] * 255.0 + 2.0) as u8, "Multiply must darken channel {k}");
        assert!(screen[k] as f32 >= d[k] * 255.0 - 2.0, "Screen must lighten channel {k}");
    }
