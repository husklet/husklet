...
//! draw/dispatch counts (see `hl_wip-gpu/src/runtime/service/validate.rs`). Every such check lands on the
...
fn tex(w: u32, h: u32, fmt: TextureFormat, usage: u32) -> TextureDesc {
    TextureDesc { width: w, height: h, depth: 1, mip_levels: 1, sample_count: 1, dim: TextureDim::D2, format: fmt, usage, label: String::new() }
}
...
fn assert_survives(g: &mut WgpuExecutor, label: &str) {
            ...
            &[
                ...
                Cmd::Submit(CommandBuffer {
                    encoder: vec![Enc::ClearRect {
                        ...
                        w: 2,
                        h: 2,
                        color: [TEXEL[0] as f32 / 255.0, TEXEL[1] as f32 / 255.0, TEXEL[2] as f32 / 255.0, 1.0],
                    }],
                    signal: None,
                }),
            ],
    ...
    match ran {
        Err(_) => panic!("[{label}] the good program PANICKED after the abuse — the executor did not survive"),
        Ok(Err(e)) => panic!("[{label}] the good program failed ({e:?}) after the abuse — device lost/poisoned"),
        Ok(Ok(_)) => {}
    }
    let px = g.read_texture(&s.resources, 1000).expect("read back the good render target");
    for (i, out) in px.chunks_exact(4).enumerate() {
        assert_eq!(out, TEXEL, "[{label}] good pixel {i} wrong — executor state corrupt after the abuse");
    }
}
...
fn hostile(g: &mut WgpuExecutor, label: &str, cmds: &[Cmd], want: impl Fn(&GpuError) -> bool) {
    let mut s = session(g);
    let r = std::panic::catch_unwind(AssertUnwindSafe(|| hl_gpu::runtime::submit(&mut s, g, 0, cmds)));
    match r {
        Err(_) => panic!("[{label}] PANICKED — hostile IR must return a typed GpuError, never panic"),
        Ok(Ok(_)) => panic!("[{label}] returned Ok — hostile IR must be a typed GpuError, not fake success"),
...
fn is_rejected(e: &GpuError) -> bool {
    matches!(e, GpuError::Invalid(_) | GpuError::ResourceLimit(_) | GpuError::OutOfBounds)
}
...
fn white_triangle_pipeline(pid: u32, vs_id: u32, fs_id: u32) -> Vec<Cmd> {
    ...
    vec![
        Cmd::CreateShader { id: vs_id, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::VERTEX, "vmain", vs) },
        Cmd::CreateShader { id: fs_id, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, "fmain", fs) },
        Cmd::CreateRenderPipeline(pid, RenderPipelineDesc {
...
fn wgsl_to_spirv(src: &str) -> Vec<u32> {
    ...
    let info = naga::valid::Validator::new(naga::valid::ValidationFlags::all(), naga::valid::Capabilities::all())
        .validate(&module).expect("seed wgsl validates");
    naga::back::spv::write_vec(&module, &info, &naga::back::spv::Options::default(), None).expect("emit spir-v")
}

/// A valid compute pipeline (id 10) + storage buffer (id 10) + bind group (id 10) — for dispatch abuses.
fn compute_setup() -> Vec<Cmd> {
    vec![
        Cmd::CreateShader { id: 10, kind: ShaderPayloadKind::SpirV, spirv: wgsl_to_spirv(COMPUTE_WGSL) },
        Cmd::CreateComputePipeline(10, ComputePipelineDesc { compute: ShaderRef { module: 10, entry: "main".into() }, label: String::new() }),
        Cmd::CreateBuffer(10, buf(16, buffer_usage::STORAGE | buffer_usage::COPY_SRC | buffer_usage::COPY_DST)),
        Cmd::CreateBindGroup(10, BindGroupDesc { set: 0, entries: vec![BindEntry { binding: 0, resource: BindResource::Buffer { id: 10, offset: 0, size: 16 } }] }),
    ]
}
...
fn dangling_buffer_in_copy_is_unknown_id() {
    ...
    hostile(&mut g, "dangling_buffer_copy", &[
        Cmd::CreateBuffer(1, buf(64, buffer_usage::COPY_SRC | buffer_usage::COPY_DST)),
        // src 999 was never created.
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::CopyBufferToBuffer { src: 999, src_offset: 0, dst: 1, dst_offset: 0, size: 16 }], signal: None }),
    ], is_unknown);
}
...
fn dangling_texture_in_copy_is_unknown_id() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "dangling_texture_copy", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::CopyTextureToTexture {
            src: 777, src_sub: sub(), src_origin: Origin3d::default(),
            dst: 1, dst_sub: sub(), dst_origin: Origin3d::default(),
...
fn dangling_pipeline_in_draw_is_unknown_id() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "dangling_pipeline_draw", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
...
fn dangling_vertex_buffer_is_unknown_id() {
    let Some(mut g) = exec() else { return };
    let mut cmds = vec![Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT))];
    cmds.extend(white_triangle_pipeline(1, 1, 2));
    cmds.push(Cmd::Submit(CommandBuffer { encoder: vec![
        Enc::BeginRenderPass { color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0, 0.0, 0.0, 1.0], store: true }], depth: None },
        Enc::SetPipeline(1),
        Enc::SetVertexBuffer { slot: 0, buffer: 404, offset: 0 }, // never created
        Enc::Draw { vertex_count: 3, instance_count: 1, first_vertex: 0, first_instance: 0 },
        Enc::EndRenderPass,
    ], signal: None }));
...
fn dangling_pipeline_in_dispatch_is_unknown_id() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "dangling_dispatch_pipeline", &[
        Cmd::CreateBuffer(1, buf(16, buffer_usage::STORAGE)),
        Cmd::CreateBindGroup(1, BindGroupDesc { set: 0, entries: vec![BindEntry { binding: 0, resource: BindResource::Buffer { id: 1, offset: 0, size: 16 } }] }),
        Cmd::Submit(CommandBuffer { encoder: vec![
...
fn copy_buffer_to_buffer_overhang_is_oob() {
    ...
    hostile(&mut g, "c2b_overhang", &[
        ...
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::CopyBufferToBuffer { src: 1, src_offset: 0, dst: 2, dst_offset: 0, size: 64 }], signal: None }),
    ], is_oob);
}
...
fn copy_buffer_to_texture_overhang_is_oob() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "c2t_overhang", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::CreateBuffer(1, buf(65536, buffer_usage::COPY_SRC | buffer_usage::COPY_DST)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::CopyBufferToTexture { src: 1, src_offset: 0, bytes_per_row: 0, dst: 1, mip: 0, width: 64, height: 64 }], signal: None }),
    ], is_oob);
}
...
fn copy_buffer_to_texture_bad_mip_is_oob() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "c2t_bad_mip", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::CreateBuffer(1, buf(4096, buffer_usage::COPY_SRC | buffer_usage::COPY_DST)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::CopyBufferToTexture { src: 1, src_offset: 0, bytes_per_row: 0, dst: 1, mip: 9, width: 4, height: 4 }], signal: None }),
    ], is_oob);
}
...
fn copy_texture_to_texture_overhang_is_oob() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "c2t2t_overhang", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::CreateTexture(2, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::CopyTextureToTexture {
            src: 1, src_sub: sub(), src_origin: Origin3d { x: 3, y: 3, z: 0 },
            dst: 2, dst_sub: sub(), dst_origin: Origin3d::default(),
            extent: Extent3d { width: 4, height: 4, depth: 1 }, // 3+4 > 4
        }], signal: None }),
    ], is_oob);
}
...
fn copy_texture_to_buffer_overhang_is_oob() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "t2b_overhang", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::CreateBuffer(1, buf(65536, buffer_usage::COPY_SRC | buffer_usage::COPY_DST)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::CopyTextureToBuffer { src: 1, mip: 0, width: 64, height: 64, dst: 1, dst_offset: 0, bytes_per_row: 0 }], signal: None }),
    ], is_oob);
}
...
fn fill_buffer_overhang_is_oob() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "fill_overhang", &[
        Cmd::CreateBuffer(1, buf(64, buffer_usage::COPY_DST | buffer_usage::COPY_SRC)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::FillBuffer { buffer: 1, offset: 60, size: 32, value: 0xdead_beef }], signal: None }),
    ], is_oob);
}
...
fn fill_buffer_offset_size_overflow_is_oob() {
    ...
    hostile(&mut g, "fill_overflow", &[
        Cmd::CreateBuffer(1, buf(64, buffer_usage::COPY_DST)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::FillBuffer { buffer: 1, offset: u64::MAX - 2, size: 8, value: 0xff }], signal: None }),
    ], is_oob);
}
...
fn blit_into_smaller_target_is_oob() {
    ...
    hostile(&mut g, "blit_overhang", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::CreateTexture(2, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::BlitTexture {
            src: 1, src_sub: sub(), src_origin: Origin3d::default(), src_extent: Extent3d { width: 4, height: 4, depth: 1 },
            dst: 2, dst_sub: sub(), dst_origin: Origin3d::default(), dst_extent: Extent3d { width: 8, height: 8, depth: 1 },
...
fn clear_rect_overhang_clamps_not_errors() {
    ...
    hl_gpu::runtime::submit(&mut s, &mut *g, 0, &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::Submit(CommandBuffer { encoder: vec![
            Enc::ClearRect { texture: 1, x: 0, y: 0, w: 4, h: 4, color: [0.0, 0.0, 0.0, 1.0] },
            Enc::ClearRect { texture: 1, x: 2, y: 2, w: 8, h: 8, color: [1.0, 0.0, 0.0, 1.0] },
        ], signal: None }),
    ...
    for y in 0..4u32 {
        for x in 0..4u32 {
            ...
            let want = if x >= 2 && y >= 2 { [255, 0, 0, 255] } else { [0, 0, 0, 255] };
...
fn zero_width_texture_is_rejected() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "zero_texture", &[Cmd::CreateTexture(1, tex(0, 4, TextureFormat::Rgba8Unorm, RT))], is_rejected);
}

#[test]
fn huge_texture_is_rejected() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "huge_texture", &[Cmd::CreateTexture(1, tex(100_000, 100_000, TextureFormat::Rgba8Unorm, RT))], is_rejected);
}
...
fn oversized_dispatch_is_oob() {
    ...
    cmds.push(Cmd::Submit(CommandBuffer { encoder: vec![
        Enc::BeginComputePass, Enc::SetPipeline(10), Enc::SetBindGroup { index: 0, group: 10 },
        Enc::Dispatch { x: 4_000_000, y: 1, z: 1 }, Enc::EndComputePass,
    ], signal: None }));
...
fn zero_size_blit_is_invalid() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "zero_blit", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::CreateTexture(2, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::BlitTexture {
            src: 1, src_sub: sub(), src_origin: Origin3d::default(), src_extent: Extent3d { width: 0, height: 4, depth: 1 },
            dst: 2, dst_sub: sub(), dst_origin: Origin3d::default(), dst_extent: Extent3d { width: 4, height: 4, depth: 1 },
            filter: Filter::Nearest,
        }], signal: None }),
    ], is_invalid);
}
...
#[test]
fn copy_texture_to_texture_incompatible_formats_is_invalid() {
    let Some(mut g) = exec() else { return };
    // R8 (1 byte/texel) → Rgba8 (4 bytes/texel): incompatible texel sizes.
    hostile(&mut g, "c2t2t_fmt_mismatch", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::R8Unorm, RT)),
        Cmd::CreateTexture(2, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::CopyTextureToTexture {
            src: 1, src_sub: sub(), src_origin: Origin3d::default(),
            dst: 2, dst_sub: sub(), dst_origin: Origin3d::default(),
...
fn resolve_non_multisampled_source_is_invalid() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "resolve_non_msaa", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)), // single-sampled src
        Cmd::CreateTexture(2, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        Cmd::Submit(CommandBuffer { encoder: vec![Enc::ResolveTexture {
            src: 1, src_sub: sub(), src_origin: Origin3d::default(),
            dst: 2, dst_sub: sub(), dst_origin: Origin3d::default(),
            extent: Extent3d { width: 4, height: 4, depth: 1 },
        }], signal: None }),
...
fn resolve_format_mismatch_is_invalid() {
    ...
    let msaa = TextureDesc { sample_count: 4, ..tex(4, 4, TextureFormat::Rgba8Unorm, texture_usage::RENDER_TARGET) };
    hostile(&mut g, "resolve_fmt_mismatch", &[
...
fn depth_attachment_on_color_format_is_invalid() {
    let Some(mut g) = exec() else { return };
    hostile(&mut g, "depth_attach_color", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
...
fn depth_tested_pipeline_in_color_only_pass_is_invalid() {
    ...
    let fs = "#version 460\nlayout(location=0) out vec4 c; void main(){ c = vec4(1.0); }\n";
    hostile(&mut g, "depth_pipe_color_pass", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
...
fn bind_group_index_out_of_range_is_invalid() {
    ...
    cmds.push(Cmd::CreateBindGroup(1, BindGroupDesc { set: 0, entries: vec![BindEntry { binding: 0, resource: BindResource::Buffer { id: 1, offset: 0, size: 16 } }] }));
    cmds.push(Cmd::Submit(CommandBuffer { encoder: vec![
        Enc::BeginRenderPass { color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0, 0.0, 0.0, 1.0], store: true }], depth: None },
        Enc::SetPipeline(1),
...
fn vertex_buffer_offset_beyond_buffer_is_oob() {
    ...
    let mut cmds = vec![Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT))];
    cmds.extend(white_triangle_pipeline(1, 1, 2));
    cmds.push(Cmd::CreateBuffer(1, buf(32, buffer_usage::VERTEX | buffer_usage::COPY_DST)));
    cmds.push(Cmd::Submit(CommandBuffer { encoder: vec![
        Enc::BeginRenderPass { color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0, 0.0, 0.0, 1.0], store: true }], depth: None },
        Enc::SetPipeline(1),
        Enc::SetVertexBuffer { slot: 0, buffer: 1, offset: 4096 }, // past the 32-byte buffer -> would panic slice()
        Enc::Draw { vertex_count: 3, instance_count: 1, first_vertex: 0, first_instance: 0 },
        Enc::EndRenderPass,
    ], signal: None }));
...
fn draw_range_overflow_is_invalid() {
    ...
    cmds.push(Cmd::Submit(CommandBuffer { encoder: vec![
        ...
        Enc::Draw { vertex_count: 100, instance_count: 1, first_vertex: u32::MAX - 10, first_instance: 0 },
        Enc::EndRenderPass,
    ], signal: None }));
...
fn draw_vertex_count_beyond_bound_buffer_is_invalid() {
    ...
    let fs = "#version 460\nlayout(location=0) out vec4 c; void main(){ c = vec4(1.0); }\n";
    hostile(&mut g, "draw_beyond_vbuf", &[
        Cmd::CreateTexture(1, tex(4, 4, TextureFormat::Rgba8Unorm, RT)),
        ...
        Cmd::CreateRenderPipeline(1, RenderPipelineDesc {
            vertex: ShaderRef { module: 1, entry: "vmain".into() },
            fragment: Some(ShaderRef { module: 2, entry: "fmain".into() }),
            vertex_buffers: vec![VertexLayout { stride: 8, step_mode: 0, attrs: vec![VertexAttr { location: 0, format: 2, offset: 0 }] }],
            color_targets: vec![ColorTargetState { format: TextureFormat::Rgba8Unorm, blend: None, write_mask: 0xF }],
            depth: None, topology: Topology::TriangleList, cull: 0, front_face: 0, sample_count: 1, label: String::new(),
        }),
        Cmd::Submit(CommandBuffer { encoder: vec![
            Enc::BeginRenderPass { color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0, 0.0, 0.0, 1.0], store: true }], depth: None },
            Enc::SetPipeline(1),
            Enc::SetVertexBuffer { slot: 0, buffer: 1, offset: 0 },
            Enc::Draw { vertex_count: 100_000, instance_count: 1, first_vertex: 0, first_instance: 0 },
            Enc::EndRenderPass,
        ], signal: None }),
    ], is_invalid);
}
...
fn set_stencil_reference_without_stencil_is_harmless_noop() {
    ...
    let mut s = session(&g);
    let mut cmds = vec![Cmd::CreateTexture(1, tex(2, 2, TextureFormat::Rgba8Unorm, RT))];
    cmds.extend(white_triangle_pipeline(1, 1, 2));
    ...
    cmds[1] = Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::VERTEX, "vmain", vs) };
    cmds.push(Cmd::Submit(CommandBuffer { encoder: vec![
        Enc::BeginRenderPass { color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0, 0.0, 0.0, 1.0], store: true }], depth: None },
        Enc::SetPipeline(1),
    ...
    hl_gpu::runtime::submit(&mut s, &mut *g, 0, &cmds)
        .expect("SetStencilReference on a non-stencil target must be a harmless no-op, not an error/panic");
    let px = g.read_texture(&s.resources, 1).unwrap();
    for (i, out) in px.chunks_exact(4).enumerate() {
        assert_eq!(out, [255, 255, 255, 255], "pixel {i}: the draw must still paint white despite the stray stencil-ref");
    }
    drop(s);
