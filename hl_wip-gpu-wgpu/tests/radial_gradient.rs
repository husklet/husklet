...
fn render(exec: &mut WgpuExecutor) -> Vec<u8> {
    ...
    let f = |c: [u8; 4]| [c[0] as f32 / 255.0, c[1] as f32 / 255.0, c[2] as f32 / 255.0, c[3] as f32 / 255.0];
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::CreateBindGroup(
                ...
                BindGroupDesc {
                    set: 0,
                    entries: vec![BindEntry { binding: 0, resource: BindResource::Buffer { id: 1, offset: 0, size: 48 } }],
                },
            ),
...
fn radial_gradient_matches_the_analytic_ramp() {
    ...
    assert!(near2(px(&img, W, 24, 24), INNER), "center must be the inner color {INNER:?}");
    assert!(near2(px(&img, W, 0, 0), OUTER), "a corner beyond the radius must be the outer color {OUTER:?}");
    ...
    for x in 24..44 {
        ...
        assert!(r <= last + 2, "red channel must be non-increasing along the radius; {r} after {last}");
        last = r;
    }

    eprintln!("technique 3 (radial_gradient): whole-image match to analytic ramp (worst ±{worst}) — PASS");
}
