...
use hl_gpu::{Cmd, GpuError, GpuExecutor};
...
fn reading_back_a_multisampled_texture_is_unsupported_not_a_panic() {
    ...
    hl_gpu::runtime::submit(
        ...
        &mut exec,
        0,
        &[Cmd::CreateTexture(1, tex(64, 64, 4, texture_usage::RENDER_TARGET))],
    )
    ...
    match exec.read_texture(&s.resources, 1) {
        ...
        Ok(_) => panic!("MSAA readback unexpectedly succeeded — a multisampled texture cannot be copied"),
    }
    ...
    hl_gpu::runtime::submit(
        ...
        &[Cmd::CreateTexture(
            2,
            tex(4, 4, 1, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC),
        )],
    )
    .expect("the executor must still accept work after a guarded MSAA readback");
    let px = exec
        .read_texture(&s.resources, 2)
        .expect("a normal single-sample texture must still read back after the guarded MSAA readback");
    assert_eq!(px.len(), 4 * 4 * 4, "single-sample readback returns a tight RGBA plane");
}
