...
use hl_gpu::protocol::model::enums::{buffer_usage, texture_usage, AddressMode, Filter, LoadOp, Topology};
...
fn render_scissor(exec: &mut WgpuExecutor) -> Vec<u8> {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::WriteBuffer { id: 1, offset: 0, data: le_f32(&colf(RED)) },
            Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::VERTEX, "vmain", VS) },
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, "fmain", SOLID_FS) },
            ...
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    Enc::BeginRenderPass {
                        color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: colf(BLUE), store: true }],
                        depth: None,
                    },
                    Enc::SetPipeline(1),
                    Enc::SetBindGroup { index: 0, group: 1 },
                    Enc::SetScissor { x: SX, y: SY, w: SW, h: SH },
...
fn render_alpha_mask(exec: &mut WgpuExecutor) -> Vec<u8> {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::WriteBuffer { id: 1, offset: 0, data: uni },
            Cmd::CreateBuffer(2, BufferDesc { size: 8, usage: buffer_usage::COPY_SRC, label: String::new() }),
            Cmd::WriteBuffer { id: 2, offset: 0, data: mask },
            Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::VERTEX, "vmain", VS) },
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, "fmain", MASK_FS) },
            Cmd::CreateSampler(
                1,
                SamplerDesc {
...
fn scissor_rect_clips_the_draw_to_its_rectangle() {
    ...
    assert_eq!(inside, SW * SH, "exactly the {}-pixel scissor rect must be RED", SW * SH);
    eprintln!("technique 5a (clip/scissor): exact {}-pixel rect clip, rest untouched — PASS", SW * SH);
}
...
fn alpha_mask_discard_clips_to_the_covered_region() {
    ...
    assert!(near1(px(&img, W, W - 1, H / 2), BLUE), "the discarded half must be the untouched BLUE clear");
    assert!(near1(px(&img, W, 0, H / 2), RED), "the kept half must be RED");
