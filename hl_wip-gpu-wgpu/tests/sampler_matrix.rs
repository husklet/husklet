...
fn target_tex() -> TextureDesc {
    TextureDesc {
        width: 1, height: 1, depth: 1, mip_levels: 1, sample_count: 1,
...
fn sampled_tex(w: u32, h: u32, mips: u32) -> TextureDesc {
    TextureDesc {
        width: w, height: h, depth: 1, mip_levels: mips, sample_count: 1,
        dim: TextureDim::D2, format: TextureFormat::Rgba8Unorm,
...
fn pipe(fs: u32) -> Cmd {
    Cmd::CreateRenderPipeline(
        ...
        RenderPipelineDesc {
            ...
            fragment: Some(ShaderRef { module: fs, entry: "main".into() }),
            vertex_buffers: vec![],
...
fn sample_at(exec: &mut WgpuExecutor, mode: AddressMode, uv_x: f32) -> [u8; 4] {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::WriteBuffer { id: 1, offset: 0, data: uv.iter().flat_map(|f| f.to_le_bytes()).collect() },
            ...
            Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, FS_SAMPLE) },
...
fn every_address_mode_wraps_to_the_exact_texel() {
    ...
    let modes = [AddressMode::ClampToEdge, AddressMode::Repeat, AddressMode::MirrorRepeat];
    // uv=1.25 and uv=2.25 together uniquely identify all three modes (Mirror diverges from Clamp at 2.25).
    for &uv in &[1.25_f32, 2.25] {
        for &mode in &modes {
            let want = if nearest_texel(mode, uv, 2) == 0 { A } else { B };
    ...
    assert!(!near(sample_at(&mut exec, AddressMode::Repeat, 2.25), sample_at(&mut exec, AddressMode::ClampToEdge, 2.25), 2));
}
...
fn linear_mipmap_filter_blends_between_levels() {
    ...
    let run_lod = |exec: &mut WgpuExecutor, mip: Filter| -> [u8; 4] {
        ...
        for _ in 0..4 { base.extend_from_slice(&x); } // 2×2
        ...
        hl_gpu::runtime::submit(
            ...
            &[
                ...
                Cmd::CreateBuffer(1, BufferDesc { size: 16, usage: buffer_usage::UNIFORM, label: String::new() }),
                Cmd::WriteBuffer { id: 1, offset: 0, data: uv.iter().flat_map(|f| f.to_le_bytes()).collect() },
                Cmd::CreateBuffer(2, BufferDesc { size: 16, usage: buffer_usage::COPY_SRC, label: String::new() }),
                Cmd::WriteBuffer { id: 2, offset: 0, data: base },
                Cmd::CreateBuffer(3, BufferDesc { size: 4, usage: buffer_usage::COPY_SRC, label: String::new() }),
                Cmd::WriteBuffer { id: 3, offset: 0, data: mip1 },
                Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::VERTEX, VS) },
                Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::Glsl, spirv: glsl(glsl_stage::FRAGMENT, FS_LOD) },
                ...
                Cmd::Submit(CommandBuffer {
                    encoder: vec![
                        Enc::CopyBufferToTexture { src: 2, src_offset: 0, bytes_per_row: 8, dst: 2, mip: 0, width: 2, height: 2 },
                        Enc::CopyBufferToTexture { src: 3, src_offset: 0, bytes_per_row: 4, dst: 2, mip: 1, width: 1, height: 1 },
                        Enc::BeginRenderPass {
                            color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0; 4], store: true }],
                            depth: None,
                        },
    ...
    assert!(!near(linear, x, 6) && !near(linear, y, 6), "linear-mip result {linear:?} must differ from both levels");
}
