...
export function determinePublication(local, published) {
  for (const name of ["client", "react"]) {
    if (published[name] !== null && published[name] !== local[name]) {
      throw new Error(
        `@husklet/${name} already exists with different tarball integrity`,
      );
    }
  }
  if (published.react !== null && published.client === null) {
    throw new Error(
      "@husklet/react exists while its exact @husklet/client dependency is absent",
    );
  }
...
if (
  process.argv[1] &&
  new URL(import.meta.url).pathname === fs.realpathSync(process.argv[1])
) {
  ...
  assert.match(
    version ?? "",
    /^\d+\.\d+\.\d+$/,
    "RELEASE_VERSION must have exact X.Y.Z form",
  );
  assert(output, "GITHUB_OUTPUT is required");
  const local = {
    client: packedIntegrity("client"),
    react: packedIntegrity("react"),
  };
  const published = {
