...
mod tests {
    ...
    fn launch_path_encodes_the_shared_ir_contract() {
        ...
        let ptx = std::ffi::CString::new(hl_gpu::ptx::VECADD_PTX).unwrap();
        ...
        let dispatched = cmds.iter().any(|c| match c {
            Cmd::Submit(cb) => cb
                .encoder
                .iter()
                .any(|e| matches!(e, hl_gpu::ir::Enc::Dispatch { .. })),
            _ => false,
        });
    ...
    /// and primary-context ref-counting — mirrors `hl-gpu/cuda/cuda_shim.c`.
    #[test]
    ...
    /// own `hl_gpu::ir` decoder and confirm the expected `WriteBuffer`s appear.
    #[test]
