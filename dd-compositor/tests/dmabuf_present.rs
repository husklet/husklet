...
//!
//! A minimal in-process Wayland client (built on `dd_display::wire`) connects over a `socketpair`
...
fn dmabuf_global_and_iosurface_commit_presents() {
    ...
    let plane = dd_display::keymap::anon_fd_with(&vec![0u8; (stride as i32 * h) as usize])
