...
//!      `dd_display::metal::iosurface_metadata` (no mock generation);
...
use dd_display::present::{IOSurfaceMetadata, PresentError, PresentOutcome, Presenter, SurfaceBuffer};
...
impl Presenter for MetalDelegatingPresenter {
    ...
    fn iosurface_metadata(&self, id: u32) -> Option<IOSurfaceMetadata> {
        dd_display::metal::iosurface_metadata(id)
    }
}
...
fn stale_engine_generation_is_rejected_and_the_live_one_imports() {
    ...
    assert!(dd_display::metal::start_gpu_bridge(), "mach bridge register");
    ...
    while dd_display::metal::iosurface_generation(id) != GEN {
