...
//! REAL bridge (`dd_display::metal::start_gpu_bridge`) and runs a C helper that sends the SAME
//! `dd_gpu_msg_t` the engine sends — a real IOSurface send-right + id + generation — then asserts
//! `dd_display::metal::iosurface_generation(id)` reports exactly that generation. Together with
...
fn engine_supplied_generation_reaches_the_compositor_over_the_mach_bridge() {
    ...
    loop {
        if dd_display::metal::iosurface_generation(id) == GENERATION {
            break;
        }
        if Instant::now() > deadline {
            panic!(
                "engine-supplied generation {GENERATION:#x} for id={id} never reached the compositor \
                 (got {:#x})",
                dd_display::metal::iosurface_generation(id)
            );
        }
    ...
    let meta = dd_display::metal::iosurface_metadata(id).expect("metadata for a cached IOSurface");
