...
//! `dd-display/src/main.rs` execs THIS binary when `DD_DISPLAY_SMITHAY=1`, *replacing* the process
//! before it ever starts the GPU executor thread. The legacy hand-written compositor
//! (`dd-display/src/server.rs`) starts the dd-gpu IR executor itself — see `serve_loop_metal` and
//! `run_window` in `dd-display`, which call `metal::start_gpu_bridge()` + spawn
//! `metal_backend::run_executor()`. Without the same startup here, `DD_GPU_BACKEND=wgpu` *and* the
...
//! thread owned before compositor selection"). It respects `DD_GPU_BACKEND` exactly like the legacy
//! path — [`dd_display::metal_backend::run_executor`] itself dispatches to the wgpu backend when
//! [`dd_gpu_wgpu::selected`] is true, else the default Metal replay — so this file names the backend
//! only for logging and never re-implements the selection.
//!
//! ## Platform gating
//! The executor entry points live in `dd-display`'s macOS-only `metal`/`metal_backend` modules (the
...
/// (`DD_GPU_BACKEND` via [`dd_gpu_wgpu::selected`]). Used only for startup/health logging.
...
fn start_impl(disp_socket: &str) {
    let Some(sock) = exec_sock(disp_socket) else {
        eprintln!(
            "dd-compositor: WARNING cannot resolve a GPU executor socket (display socket {disp_socket:?} \
    ...
    // ever connects (parity with dd-display's serve_loop_metal / run_window).
    dd_display::metal::start_gpu_bridge();
    // GPU rung 3: the guest streams framed dd-gpu IR here; `run_executor` replays it on the host GPU
    // into the resolved IOSurface. It branches on DD_GPU_BACKEND internally (wgpu vs default Metal),
    // so this call is the single startup for BOTH backends — nothing here duplicates that selection.
    let backend = backend_name();
    eprintln!(
        "dd-compositor: starting dd-gpu executor (backend={backend}) on {sock} \
    ...
    std::thread::spawn(move || dd_display::metal_backend::run_executor(sock));
}

#[cfg(not(target_os = "macos"))]
fn start_impl(_disp_socket: &str) {
    // The dd-gpu host executor renders on the host GPU (Metal / wgpu-on-Metal), which only exists on
    ...
    eprintln!(
        "dd-compositor: no host GPU on this platform; dd-gpu executor not started \
         (DD_GPU_BACKEND={}). Only wl_shm software clients will render.",
        backend_name()
    );
}
...
pub fn warn_if_accel_client_without_executor() {
    ...
    eprintln!(
        "dd-compositor: ERROR an accelerated client attached a GPU (dmabuf/IOSurface) buffer but NO \
         dd-gpu executor is running — its GPU frames cannot be rendered and WILL appear blank/white. \
         (DD_GPU_BACKEND={}; the host GPU executor is unavailable on this build/platform.)",
        backend_name()
    );
}
