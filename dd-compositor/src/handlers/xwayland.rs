...
impl XwmHandler for DdState {
    ...
    fn minimize_request(&mut self, _xwm: XwmId, window: X11Surface) {
        if let Some(surface) = Self::x11_wl_surface(&window) {
            self.set_surface_visibility(&surface, dd_display::present::SurfaceVisibility::Minimized);
        }
    }
    fn unminimize_request(&mut self, _xwm: XwmId, window: X11Surface) {
        if let Some(surface) = Self::x11_wl_surface(&window) {
            self.set_surface_visibility(&surface, dd_display::present::SurfaceVisibility::Visible);
        }
    }
