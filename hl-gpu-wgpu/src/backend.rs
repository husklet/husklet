...
use hl_gpu::backend::{Capabilities, GpuBackend, PresentToken};
use hl_gpu::id::*;
use hl_gpu::ir::*;
    ...
    fn create_shader(&mut self, id: ShaderId, kind: hl_gpu::ir::ShaderPayloadKind, spirv: &[u32]) -> Result<()> {
        ...
        if self.shader_id_hash.get(&id.0) == Some(&key) {
            ...
            if self.failed_shader_cache.contains(&key) {
                return if kind == hl_gpu::ir::ShaderPayloadKind::SpirV {
        ...
        if matches!(kind, hl_gpu::ir::ShaderPayloadKind::LegacyMsl | hl_gpu::ir::ShaderPayloadKind::DemoBuiltin) {
            self.shaders.remove(&id.0);
            return Ok(());
        }
