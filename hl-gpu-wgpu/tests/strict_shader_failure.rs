...
fn vulkan_shader_translation_failure_never_falls_back_to_builtin_rendering() {
    let Ok(mut backend) = hl_gpu_wgpu::WgpuBackend::new() else {
    ...
    let bytes = hl_gpu::ir::encode_stream(&[cmd]);
    ...
    let decoded = hl_gpu::ir::decode_stream(&bytes).expect("wire remains decodable");
