...
//! `naga::front::spv`. Needs a Metal device, so macOS only. Run: `cargo test -p hl-gpu-wgpu --test spirv_compute`.
//!
...
fn spirv_vecadd_runs_on_real_metal() {
    ...
    let cmds = vec![
        ...
        Cmd::CreateShader { id: 20, kind: hl_gpu::ir::ShaderPayloadKind::SpirV, spirv: words },
    ...
    hl_gpu::replay::replay_stream(&mut be, &ir).expect("replay spirv compute");
