...
mod imp {
    ...
    // ---- IOSurface FFI (mirrors hl-display/src/metal.rs; declared here so the proof is self-contained) ----
    ...
    fn render_flat_quad(be: &mut WgpuBackend) -> hl_gpu::Result<()> {
        ...
        let cmds = vec![
            ...
            Cmd::CreateShader { id: 20, kind: hl_gpu::ir::ShaderPayloadKind::LegacyMsl, spirv: pack_msl("#include <metal_stdlib>\n[[stage_in]] flat") },
        ...
        hl_gpu::replay::replay_stream(be, &ir).map(|_| ())
    }
}
