...
fn wgpu_resolve_averages_samples_into_a_subregion_matching_the_software_oracle() {
    ...
    dd_gpu::replay::replay(&mut be, &setup).unwrap();
    ...
    dd_gpu::replay::replay(&mut be, &[Cmd::Submit(CommandBuffer { encoder: vec![resolve.clone()], signal: None })]).unwrap();
    ...
    dd_gpu::replay::replay(&mut sw, &[Cmd::Submit(CommandBuffer { encoder: vec![resolve], signal: None })]).unwrap();
