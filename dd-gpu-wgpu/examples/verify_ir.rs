...
//! [`dd_gpu_wgpu::WgpuBackend`] into a wgpu-owned render target, read the pixels back, and assert.
//!
...
//!
//! Run on the macOS host: `cargo run --release -p dd-gpu-wgpu --example verify_ir` (exit 0 = PASS).
...
fn main() {
    ...
    // ---- case 1: flat centered red quad over gray clear -------------------------------------------
    {
        ...
        dd_gpu::replay::replay_stream(&mut be, &ir).expect("replay flat");
    ...
    // ---- case 2: textured full-frame quad sampling an 8x8 atlas -----------------------------------
    {
        ...
        let ir = encode_stream(&cmds);
        let mut be = make_backend();
        dd_gpu::replay::replay_stream(&mut be, &ir).expect("replay tex");
        let data = be.read_target(1).expect("readback");
