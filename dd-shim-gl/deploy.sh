...
#
# Usage:  DD_SHIM_IMPL=rust dd-shim-gl/deploy.sh [ARCH]      # ARCH defaults to the host arch
...
# The cdylib bakes SONAME libEGL.so.1 (see dd-shim-gl/build.rs). Its output dir honors CARGO_TARGET_DIR.
echo "building dd-shim-gl (Rust GLES driver) for $ARCH ..."
( cd "$ROOT" && cargo build -p dd-shim-gl --release )
