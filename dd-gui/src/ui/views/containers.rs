...
pub(crate) fn container_info(
    ...
    if c.paused() {
        actions.push(text_btn("Resume", "suggested-action", sender, {
            let id = id.clone();
            move || Msg::UnpauseContainer(id.clone())
        }));
        actions.push(text_btn("Stop", "", sender, {
            let id = id.clone();
            move || Msg::StopContainer(id.clone())
        }));
    } else if c.running() {
        actions.push(text_btn("Stop", "suggested-action", sender, {
            let id = id.clone();
            move || Msg::StopContainer(id.clone())
        }));
        actions.push(text_btn("Restart", "", sender, {
            let id = id.clone();
            move || Msg::RestartContainer(id.clone())
        }));
        actions.push(text_btn("Pause", "", sender, {
            let id = id.clone();
            move || Msg::PauseContainer(id.clone())
        }));
    } else {
