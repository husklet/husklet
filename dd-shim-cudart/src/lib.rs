...
//! lowers the CUDA model into the shared `dd-gpu` IR through [`dd_gpu::cuda::CudaContext`] and EXECUTEs
//! it in-process on the embedded [`SoftwareBackend`](dd_gpu::software::SoftwareBackend) — the SAME IR +
...
//! numerically-correct results — identical to `dd-gpu/cuda/cudart_shim.c` (the parity oracle).
//!
//! ## Coverage
//! The exported entry-point *surface* is code-generated from a committed manifest (`build.rs` +
//! `registry/`), extracted from dd's clean-room `dd-gpu/cuda/cudart_shim.c` runtime surface plus the
...
mod tests {
    ...
    /// `dd-gpu/cuda/fatbin.h` / `test_cudart.c` produce, so the runtime launch path walks a real fatbin.
    ...
    /// `cudaMemcpy` H2D), then decode the accumulated IR bytes with the HOST's own `dd_gpu::ir` decoder.
    ...
    fn vecadd_executes_end_to_end_through_cudart() {
        ...
        let fatbin = build_fatbin(dd_gpu::ptx::VECADD_PTX);
