...
//! `dd-gpu/cuda/cudart_shim.c`. The stateful compute calls lower the CUDA model into the shared `dd-gpu`
//! IR through [`dd_gpu::cuda::CudaContext`] and EXECUTE it on the embedded
//! [`SoftwareBackend`](dd_gpu::software::SoftwareBackend) (CPU PTX interpreter), so a runtime vecadd
...
/// `dd-gpu/cuda/cudart_min.h`). A `#[repr(C)]` with the exact field order/types is required for ABI
