...
//! `dd-gpu/cuda/cudart_min.h`. The map mirrors `dd-gpu/cuda/cudart_shim.c`'s `map_err` exactly so the
...
/// matching the driver shim's `cuDriverGetVersion` (`dd-gpu/cuda/cuda_shim.c` `g_driver_version`).
...
/// `dd-gpu/cuda/cudart_shim.c` `map_err`.
