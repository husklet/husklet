...
//! CUDA→dd-gpu-IR translation — is delegated to [`dd_gpu::cuda::CudaContext`] (memory alloc/copy → IR,
//! PTX module load, kernel launch → compute pipeline + dispatch), and the accumulated IR is EXECUTED
//! in-process on an embedded [`dd_gpu::software::SoftwareBackend`] (the CPU PTX interpreter — the same
...
pub struct CudartState {
    ...
    /// compute `Dispatch` (the PTX interpreter in `dd_gpu::ptx`) — and holds the resulting
