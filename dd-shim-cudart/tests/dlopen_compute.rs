...
//!
//! Skips if the cdylib is not built (`cargo build -p dd-shim-cudart` produces it) or no C toolchain.
...
fn deployed_libcudart_so_runs_vecadd_end_to_end() {
    let Some(lib) = cdylib() else {
        eprintln!("[dlopen_compute] libcudart cdylib not built (run `cargo build -p dd-shim-cudart`); skipping");
        return;
    };
