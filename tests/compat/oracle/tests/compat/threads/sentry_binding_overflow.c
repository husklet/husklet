...
static void *inspect(void *opaque) {
    ...
    while (!atomic_load(&state->go)) sched_yield();
    if (fcntl(state->descriptor, F_GETFD) >= 0) atomic_fetch_add(&state->passed, 1);
    while (atomic_load(&state->go) == 1) sched_yield();
    return NULL;
}
...
int main(void) {
    ...
    while (atomic_load(&state.ready) != THREADS) sched_yield();
    atomic_store(&state.go, 1);
    while (atomic_load(&state.passed) != THREADS) sched_yield();
