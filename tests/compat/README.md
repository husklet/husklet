# Persistent guest compatibility corpus
...
```text
python3 src/tests/compat/priority.py
python3 src/tests/compat/priority.py --check
python3 -m unittest src/tests/compat/priority_test.py
```
...
```text
cmake -S src/tests/compat -B build/compat -DHL_CORPUS_MODE=PREBUILT
cmake --build build/compat --target compat-smoke
```
