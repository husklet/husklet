...
int main(void) {
    ...
    if (signal_fd < 0 || epoll_fd < 0 ||
        epoll_ctl(epoll_fd, EPOLL_CTL_ADD, signal_fd, &control) != 0) return 21;
    ...
    printf("signalfd_epoll empty=%d ready=%d event=%d data=%d bytes=%ld signo=%u code=%d drained=%d\n",
        empty, ready, event_ok, data_ok,
        (long)bytes, info.ssi_signo, info.ssi_code, drained);
    return 0;
}
