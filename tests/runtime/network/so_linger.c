...
int main(void) {
    ...
    struct linger got = {0, 0};
    socklen_t l = sizeof got;
    getsockopt(s, SOL_SOCKET, SO_LINGER, &got, &l);
    int ka = 1;
    setsockopt(s, SOL_SOCKET, SO_KEEPALIVE, &ka, sizeof ka);
    int kag = 0;
    l = sizeof kag;
    getsockopt(s, SOL_SOCKET, SO_KEEPALIVE, &kag, &l);
    int type = 0;
    l = sizeof type;
    getsockopt(s, SOL_SOCKET, SO_TYPE, &type, &l);
    close(s);
    printf("so_linger on=%d t=%d keepalive=%d type_stream=%d\n", got.l_onoff != 0, got.l_linger, kag != 0,
           type == SOCK_STREAM);
    return 0;
}
