...
int main(void) {
    ...
    int snd = 65536;
    int r1 = setsockopt(s, SOL_SOCKET, SO_SNDBUF, &snd, sizeof snd);
    int got = 0;
    socklen_t l = sizeof got;
    int r2 = getsockopt(s, SOL_SOCKET, SO_SNDBUF, &got, &l);
    int rcv = 65536;
    int r3 = setsockopt(s, SOL_SOCKET, SO_RCVBUF, &rcv, sizeof rcv);
    int gotr = 0;
    l = sizeof gotr;
    int r4 = getsockopt(s, SOL_SOCKET, SO_RCVBUF, &gotr, &l);
    close(s);
