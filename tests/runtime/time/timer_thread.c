...
static void periodic_cb(union sigval sv) {
    (void)sv;
    __sync_fetch_and_add(&periodic_count, 1);
}

static void oneshot_cb(union sigval sv) {
    (void)sv;
    __sync_fetch_and_add(&oneshot_count, 1);
}
...
int main(void) {
    ...
    for (int i = 0; i < 200 && periodic_count < 3; i++)
        usleep(5000);
    ...
    for (int i = 0; i < 100 && oneshot_count < 1; i++)
        usleep(5000);
