...
int main(void) {
    ...
    errno = 0;
    int g_which = getitimer(99, &iv) == -1 && errno == EINVAL;
    // getitimer EFAULT.
    errno = 0;
    int g_fault = getitimer(ITIMER_REAL, (struct itimerval *)0x1) == -1 && errno == EFAULT;
    ...
    printf("err s_neg=%d s_big=%d s_which=%d g_which=%d g_fault=%d tf_neg=%d tf_big=%d tf_flags=%d tf_short=%d "
           "tf_unarmed=%d pt_neg=%d pt_big=%d ns_neg=%d\n",
           s_neg, s_big, s_which, g_which, g_fault, tf_neg, tf_big, tf_flags, tf_short, tf_unarmed, pt_neg, pt_big,
           ns_neg);
    return 0;
}
