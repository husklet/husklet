...
static int rd(int fd, uint64_t *o) {
    return read(fd, o, 8) == 8;
}
...
int main(void) {
    ...
    timerfd_settime(fd, 0, &its, NULL);
    memset(&its, 0, sizeof its);
    its.it_value.tv_nsec = 25 * 1000 * 1000;
    timerfd_settime(fd, 0, &its, NULL);
    n = 0;
    int rearm = rd(fd, &n) && n == 1;
    close(fd);
    ...
    int gtafter = cur.it_interval.tv_sec == 5 && (cur.it_value.tv_sec > 0 || cur.it_value.tv_nsec > 0) &&
    ...
    its.it_interval.tv_nsec = 40 * 1000 * 1000;
    timerfd_settime(fd, 0, &its, NULL);
    pid_t pid = fork();
    if (pid == 0) {
        uint64_t c = 0;
        _exit((read(fd, &c, 8) == 8 && c >= 1) ? 0 : 1);
    }
    uint64_t pc = 0;
    int prd = read(fd, &pc, 8) == 8 && pc >= 1;
    int st = 0;
    waitpid(pid, &st, 0);
    int fork_ok = prd && WIFEXITED(st) && WEXITSTATUS(st) == 0;
    close(fd);

    printf("tfd eagain=%d firstpos=%d consumed=%d oldval=%d cloexec=%d nocloexec=%d pollin=%d pollto=%d rearm=%d rt=%d "
           "boot=%d gtafter=%d fork=%d\n",
