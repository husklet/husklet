...
static volatile long ov = -2;
static volatile int sival = 0, scode = 0;

static void h(int s, siginfo_t *si, void *u) {
    (void)s;
    (void)u;
    if (got == 0) {
        ov = timer_getoverrun(g_tid);
        sival = si->si_value.sival_int;
        scode = si->si_code;
    }
    got++;
}
...
int main(void) {
    ...
    sev.sigev_value.sival_int = 0x1234;
    ...
    usleep(10 * 1000);
    memset(&its, 0, sizeof its);
    timer_settime(g_tid, 0, &its, NULL); // disarm
    ...
    timer_create(CLOCK_MONOTONIC, &sev, &a);
    sev.sigev_value.sival_int = 2;
    timer_create(CLOCK_MONOTONIC, &sev, &b);
    memset(&its, 0, sizeof its);
    its.it_value.tv_nsec = 15 * 1000 * 1000;
    timer_settime(a, 0, &its, NULL);
    memset(&its, 0, sizeof its);
    its.it_value.tv_nsec = 30 * 1000 * 1000;
    timer_settime(b, 0, &its, NULL);
    for (int i = 0; i < 12 && got < 2; i++) {
        struct timespec w = {0, 10 * 1000 * 1000}, r;
        while (nanosleep(&w, &r) == -1 && errno == EINTR)
            w = r;
    }
    int two_fire = got >= 2;
    timer_delete(a);
    ...
    printf("ptimer fired=%d overrun_positive=%d overrun_bounded=%d payload=%d post_zero=%d two_fire=%d settime_old=%d "
           "gettime_ok=%d badclk=%d\n",
