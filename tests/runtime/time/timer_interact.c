...
int main(void) {
    ...
    sa.sa_handler = h; // no SA_RESTART
    sigaction(SIGALRM, &sa, NULL);
    hits = 0;
    memset(&iv, 0, sizeof iv);
    iv.it_value.tv_usec = 20 * 1000;
    setitimer(ITIMER_REAL, &iv, NULL);
    struct timespec t = {2, 0}, rem = {0, 0};
    int nr = nanosleep(&t, &rem);
    memset(&iv, 0, sizeof iv);
    setitimer(ITIMER_REAL, &iv, NULL);
    int eintr_rem = nr == -1 && errno == EINTR && hits >= 1 && rem.tv_sec < 2 && (rem.tv_sec > 0 || rem.tv_nsec > 0);
    ...
    sigaction(SIGALRM, &sa, NULL);
    memset(&iv, 0, sizeof iv);
    iv.it_interval.tv_usec = 10 * 1000;
    setitimer(ITIMER_REAL, &iv, NULL);
    usleep(40 * 1000);
    ...
    struct itimerspec gc;
    memset(&gc, 0, sizeof gc);
    timerfd_gettime(fd, &gc);
    int tfd_val0 = gc.it_value.tv_sec == 0 && gc.it_value.tv_nsec == 0;
    close(fd);

    printf("interact alarm_itimer=%d itimer_alarm=%d alarm_override=%d alarm_clears=%d eintr_rem=%d val0_disarm=%d "
           "val0_zero=%d tfd_val0=%d\n",
