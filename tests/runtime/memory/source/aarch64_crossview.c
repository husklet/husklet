...
static int fault_exit(int status) {
    return (WIFSIGNALED(status) && WTERMSIG(status) == SIGSEGV) || (WIFEXITED(status) && WEXITSTATUS(status) == 139);
}
...
int main(void) {
    ...
    if (first_fd < 0 || second_fd < 0 || ftruncate(first_fd, (off_t)(page * 2)) != 0 ||
        ftruncate(second_fd, (off_t)(page * 2)) != 0)
        return 2;
    unsigned char *range = mmap(NULL, page * 2, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (range == MAP_FAILED) return 3;
    if (mmap(range, page, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, first_fd, (off_t)page) != range ||
        mmap(range + page, page, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, second_fd, (off_t)page) !=
            range + page)
        return 4;
    ...
    uint64_t value;
    __asm__ volatile("ldr %x[value], [%x[address]]" : [value] "=r"(value) : [address] "r"(range + page - 4) : "memory");
    ...
    __asm__ volatile("ldp %x[first], %x[second], [%x[address]]"
                     ...
                     : "memory");
    int pair_load = pair_first == UINT64_C(0x8786858483828180) && pair_second == UINT64_C(0x8f8e8d8c8b8a8988);
    ...
    __asm__ volatile("stp %x[first], %x[second], [%x[address]]"
                     :
                     : [first] "r"(pair_new_first), [second] "r"(pair_new_second), [address] "r"(range + page - 8)
                     : "memory");
    int pair_store = 1;
    for (int i = 0; i < 16; ++i)
    ...
    printf("aarch64-cross-view load=%d store=%d vector-load=%d vector-store=%d pair-load=%d pair-store=%d "
           "structure-load=%d structure-store=%d first-fault=%d second-fault=%d no-partial-store=%d\n",
           ok, store_ok, vector_load, vector_store, pair_load, pair_store, structure_load, structure_store, first_fault,
           second_fault, no_partial_store);
    return ok && store_ok && vector_load && vector_store && pair_load && pair_store && structure_load &&
