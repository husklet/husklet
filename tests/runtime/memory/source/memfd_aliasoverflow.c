...
int main(void) {
     ...
     */
    unsigned char *snapshot = mmap(NULL, page, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (snapshot == MAP_FAILED || mmap(snapshot, page, PROT_READ, MAP_SHARED | MAP_FIXED, fd, (off_t)page) != snapshot)
        return 3;

    for (int i = 0; i < ALIAS_COUNT; ++i) {
        aliases[i] = mmap(NULL, page, PROT_READ | PROT_WRITE, MAP_SHARED, fd, (off_t)page);
    ...
    unsigned char *reused =
        mmap(snapshot, page, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
