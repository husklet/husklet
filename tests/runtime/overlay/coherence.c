...
int main(void) {
    ...
    int source = open("/etc/alpine-release", O_RDONLY);
    int copy = open("/etc/alpine-release.copy", O_CREAT | O_EXCL | O_WRONLY, 0600);
    char copy_bytes[128];
    ssize_t copied;
    ...
    struct stat before;
    char expected[4];
    if (fstat(fd, &before) != 0 || before.st_size < 4 || pread(fd, expected, sizeof expected, 0) != sizeof expected)
        return 11;
    memcpy(expected, "MM", 2);
    ...
    if (rename("/etc/alpine-release", "/etc/alpine-release.moved") != 0) return 15;
    fd = open("/etc/alpine-release.moved", O_RDONLY);
    char result[5] = {0};
    if (fd < 0 || read(fd, result, 4) != 4 || close(fd) != 0) return 16;
    if (memcmp(result, expected, sizeof expected) != 0) return 17;
