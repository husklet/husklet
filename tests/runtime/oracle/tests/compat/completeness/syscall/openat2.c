...
#include <fcntl.h>
#include <sys/stat.h>
struct hlc_open_how { unsigned long long flags, mode, resolve; };
int main(void){
  const char *p="/tmp/hlc_oa2"; unlink(p);
  int fd=open(p,O_CREAT|O_WRONLY|O_TRUNC,0644); write(fd,"Z",1); close(fd);
  struct hlc_open_how how; memset(&how,0,sizeof how); how.flags=O_RDONLY;
