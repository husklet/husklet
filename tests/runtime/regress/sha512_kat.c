...
static void sha512(const uint8_t *msg, size_t len, uint8_t out[64]) {
    ...
    for (size_t off = 0; off < padded; off += 128) {
        ...
        u64 w[80];
        for (int i = 0; i < 16; i++)
            w[i] = ((u64)blk[i * 8] << 56) | ((u64)blk[i * 8 + 1] << 48) | ((u64)blk[i * 8 + 2] << 40) |
                   ((u64)blk[i * 8 + 3] << 32) | ((u64)blk[i * 8 + 4] << 24) | ((u64)blk[i * 8 + 5] << 16) |
                   ((u64)blk[i * 8 + 6] << 8) | ((u64)blk[i * 8 + 7]);
        for (int i = 16; i < 80; i++) {
