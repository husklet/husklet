...
static int rd_byte(struct rdr *r, uint8_t *out) {
    if (r->pos >= r->len) return -1;
    *out = r->buf[r->pos++];
    return 0;
}

static uint64_t rd_tell(struct rdr *r) {
    return (uint64_t)r->pos;
} // position AS off_t/u64
...
static int check_file_path(void) {
    ...
    for (int t = 1; t <= 5; t++) {
        off_t want = offs[t - 1];
        uint8_t got[3];
        if (pread(fd, got, 3, want) != 3 || got[0] != (uint8_t)t) {
            ok = 0;
            printf("FILE seek mismatch t=%d off=%lld tag=%d\n", t, (long long)want, got[0]);
        }
        // also lseek-based re-read
        if (lseek(fd, want, SEEK_SET) != want) {
            ok = 0;
            printf("lseek returned wrong pos\n");
        }
        if (read(fd, got, 3) != 3 || got[0] != (uint8_t)t) {
            ok = 0;
            printf("FILE read-after-seek mismatch t=%d\n", t);
        }
    }
    close(fd);
    unlink(path);
...
int main(void) {
    ...
    for (int matchpos = 1; matchpos <= 4; matchpos++) {
        ...
        for (int rec = 0; rec < 5; rec++) {
            ...
            buf[p++] = len;
            for (int i = 0; i < len; i++)
                buf[p++] = (uint8_t)(rec * 31 + i);
        }
        struct rdr r = {buf, p, 0};
        int64_t got = find_offset(&r, 0x42);
        if (got != (int64_t)want_off) {
            fails++;
            printf("BUF matchpos=%d: recorded off=%lld want=%llu\n", matchpos, (long long)got,
                   (unsigned long long)want_off);
        }
        // re-read at the recorded offset: tag must be the target
        if (got >= 0 && (size_t)got < p && buf[got] != 0x42) {
            fails++;
            printf("BUF reread matchpos=%d tag=0x%x != 0x42\n", matchpos, buf[got]);
        }
    }
