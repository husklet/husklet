...
static int test_one(const char *path, off_t off) {
    ...
    if (r != off) {
        printf("FAIL %s: lseek returned %lld want %lld\n", path, (long long)r, (long long)off);
        close(fd);
        return 1;
    }
    ...
    int fails = 0;
    if ((size_t)got != want) {
        printf("FAIL %s off=%lld: read-after-seek got %zd, expected %zu (remaining from offset)\n", path,
               (long long)off, got, want);
        fails++;
    }
    if (got > 0 && pgot > 0 && (got != pgot || memcmp(rbuf, pbuf, got) != 0)) {
        printf("FAIL %s off=%lld: read-after-seek data != pread@offset (seek ignored: reads from 0?) rbuf[0]=%02x "
               "pbuf[0]=%02x\n",
...
int main(int argc, char **argv) {
    ...
    if (argc > 1) {
        ...
        close(fd);
        for (off_t o = 0; o <= 2900; o += 317)
            fails += test_one(path, o);
        unlink(path);
    }
