...
int main(void) {
    ...
    for (int round = 0; round < repeats; ++round) {
        ...
        __asm__ volatile("b      1f\n"
                         ...
                         : "=r"(sw_neg), "=r"(sw_pos), "=r"(w32), "=r"(x64), "=r"(sbits), "=r"(dbits), "=r"(q_lo),
                           "=r"(q_hi)
                         :
                         : "v0", "v1", "v2", "memory");
    }
    ...
    printf("ldrsw-lit sw_neg=%lld sw_pos=%lld w32=%08x x64=%016llx s=%08x d=%016llx q=%016llx:%016llx acc=%016llx\n",
           (long long)sw_neg, (long long)sw_pos, w32, (unsigned long long)x64, sbits, (unsigned long long)dbits,
           (unsigned long long)q_lo, (unsigned long long)q_hi, (unsigned long long)acc);
    return 0;
}
