...
// multi-process application worker<->service-peer command-buffer wakeup: the service peer creates a shm/memfd, sends
// the handle over IPC (SCM_RIGHTS), and the peer mmaps it and FUTEX_WAITs on a word inside; the other side stores +
// FUTEX_WAKEs through its OWN independent mapping. The existing futex-shared-key gate only covers a FORK-INHERITED
// memfd (same fd number, same VA lineage); this covers an SCM_RIGHTS-delivered memfd that lands at a different fd
// number in an unrelated process, mmap'd at an independent VA. A lost wake surfaces as woke=0 within the timed wait
// (never a hang).
#define _GNU_SOURCE
#include <errno.h>
#include <limits.h>
...
int main(void) {
    int zc[2];
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, zc) != 0) {
        perror("zc");
        return 1;
    }
    ...
    if (zyg == 0) {
        ...
        if (r == 0) {
            ...
            char b = 'R';
            (void)!write(rp[1], &b, 1); // tell coordinator we are about to park
