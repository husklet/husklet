...
static int child_main(void) {
    ...
    if (epoll_ctl(ep, EPOLL_CTL_ADD, sock, &se) != 0) {
        printf("child epoll_ctl sock: %s\n", strerror(errno));
        return 21;
    }
    if (epoll_ctl(ep, EPOLL_CTL_ADD, efd, &ee) != 0) {
        printf("child epoll_ctl efd: %s\n", strerror(errno));
        return 22;
    }
    // tell the parent we are parked and about to wait
    char rdy = 'R';
    if (write(sock, &rdy, 1) != 1) {
        printf("child ready-write: %s\n", strerror(errno));
        return 23;
    }
    ...
    for (int rounds = 0; rounds < 2 && !(got_sock && got_efd); rounds++) {
        ...
        if (n <= 0) {
            printf("child epoll_wait timeout n=%d round=%d got_sock=%d got_efd=%d\n", n, rounds, got_sock, got_efd);
            return 24;
        }
        for (int i = 0; i < n; i++) {
...
int main(int argc, char **argv) {
    ...
    if (write(sv[0], m, strlen(m)) < 0) {
        perror("parent write sock");
        return 3;
    }
    uint64_t v = 7;
    if (write(efd, &v, 8) != 8) {
        perror("parent write efd");
        return 4;
    }
