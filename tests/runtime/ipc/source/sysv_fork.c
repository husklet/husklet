...
int main(void) {
    ...
    if (shmid < 0 || semid < 0 || msgid < 0 || shared == (void *)-1 || semctl(semid, 0, SETVAL, argument) != 0)
        return 20;
    *shared = 7;
    ...
    int status = 0;
    int child_ok = waitpid(child, &status, 0) == child && WIFEXITED(status) && WEXITSTATUS(status) == 0;
    int shared_ok = *shared == 19;
    int undo_ok = semctl(semid, 0, GETVAL) == 1;
    struct message message = {0};
    int message_ok = msgrcv(msgid, &message, sizeof(message.value), 1, 0) == 1 && message.value == 42;
    int detached = shmdt(shared) == 0;
    errno = 0;
    struct shmid_ds metadata;
    int removed = detached && shmctl(shmid, IPC_STAT, &metadata) == -1 && (errno == EINVAL || errno == EIDRM);
    int cleaned = semctl(semid, 0, IPC_RMID) == 0 && msgctl(msgid, IPC_RMID, NULL) == 0;

    printf("sysv_fork shared=%d undo=%d message=%d removed=%d child=%d cleaned=%d\n", shared_ok, undo_ok, message_ok,
           removed, child_ok, cleaned);
