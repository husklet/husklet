...
static void emit(const char *t, unsigned __int128 v, uint64_t *h) {
    *h = *h * 1000003ULL + (uint64_t)(v >> 64);
    *h = *h * 1000003ULL + (uint64_t)v;
    (void)t;
}
...
int main(void) {
    ...
    for (uint64_t d = 1; d < 200000; d += 997) {
        ...
        __int128 sd = (__int128)(int64_t)(d - 100000);
        if (sd != 0) {
            emit("s", (unsigned __int128)(sn / sd), &h);
            emit("m", (unsigned __int128)(sn % sd), &h);
        }
    }
    // 128/128 case
