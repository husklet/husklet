...
TGT int main(void) {
    ...
    double g[4], h[4];
    for (int i = 0; i < 8; i++) {
        u[i] = (uint32_t)(i * 2654435761u + 7);
        w[i] = (uint32_t)(i * 40503u + 13);
    }
    for (int i = 0; i < 4; i++) {
        g[i] = (double)(i * 3 - 5) + 0.5;
        h[i] = (double)(i * 7 - 2) - 0.25;
    }
    uint32_t ob[8];
    double od[4];
    int r;
    ...
    // shuffles / unpacks
    _mm_storeu_ps((float *)ob, _mm_shuffle_ps(a, b, 0x4E));
    mixb(ob, 16);
    _mm256_storeu_ps((float *)ob, _mm256_shuffle_ps(A, B, 0x1B));
    mixb(ob, 32);
    _mm_storeu_pd(od, _mm_shuffle_pd(da, db, 0x2));
    mixb(od, 16);
    _mm_storeu_ps((float *)ob, _mm_unpacklo_ps(a, b));
    mixb(ob, 16);
    _mm_storeu_ps((float *)ob, _mm_unpackhi_ps(a, b));
    mixb(ob, 16);
    _mm256_storeu_ps((float *)ob, _mm256_unpacklo_ps(A, B));
    mixb(ob, 32);
    _mm_storeu_pd(od, _mm_unpackhi_pd(da, db));
    mixb(od, 16);
    // permilps/pd imm
    _mm_storeu_ps((float *)ob, _mm_permute_ps(a, 0x93));
    mixb(ob, 16);
    _mm256_storeu_ps((float *)ob, _mm256_permute_ps(A, 0x1B));
    mixb(ob, 32);
    _mm_storeu_pd(od, _mm_permute_pd(da, 0x1));
    mixb(od, 16);
    // blends
    _mm_storeu_ps((float *)ob, _mm_blend_ps(a, b, 0xA));
    mixb(ob, 16);
    _mm256_storeu_ps((float *)ob, _mm256_blend_ps(A, B, 0x5A));
    mixb(ob, 32);
    _mm_storeu_pd(od, _mm_blend_pd(da, db, 0x2));
    mixb(od, 16);
    // movlhps / movhlps
    _mm_storeu_ps((float *)ob, _mm_movelh_ps(a, b));
    mixb(ob, 16);
    _mm_storeu_ps((float *)ob, _mm_movehl_ps(a, b));
    mixb(ob, 16);
    // movmsk
    r = _mm_movemask_ps(a);
    mixb(&r, 4);
    r = _mm256_movemask_ps(A);
    mixb(&r, 4);
    r = _mm_movemask_pd(da);
    mixb(&r, 4);
    r = _mm256_movemask_pd(DA);
    mixb(&r, 4);
    // extract / insert dword
    r = _mm_extract_epi32(ia, 3);
    mixb(&r, 4);
    _mm_storeu_si128((void *)ob, _mm_insert_epi32(ia, 0x12345678, 1));
    mixb(ob, 16);
    r = _mm_extract_epi8(ia, 5);
    mixb(&r, 4);
    _mm_storeu_si128((void *)ob, _mm_insert_epi8(ia, 0x5A, 9));
    mixb(ob, 16);
    // insertps
    _mm_storeu_ps((float *)ob, _mm_insert_ps(a, b, 0x4A));
    mixb(ob, 16);
#else
...
#define ST8(x0, x1, x2, x3, x4, x5, x6, x7)                                                                            \
    do {                                                                                                               \
        ob[0] = (x0);                                                                                                  \
        ob[1] = (x1);                                                                                                  \
        ob[2] = (x2);                                                                                                  \
    ...
    r = ((ud[0] >> 31) & 1) | ((ud[1] >> 31) << 1) | ((ud[2] >> 31) << 2) | ((ud[3] >> 31) << 3);
    mixb(&r, 4);
    ...
    r = (int)((ug[0] >> 63) & 1) | (int)((ug[1] >> 63) & 1) << 1;
    mixb(&r, 4);
