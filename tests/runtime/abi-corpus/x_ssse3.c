...
TGT int main(void) {
    ...
    for (int it = 0; it < 2000; it++) {
        ...
        _mm_storeu_si128((__m128i *)out, _mm_shuffle_epi8(va, vb));
        h = mix(h, out);
        _mm_storeu_si128((__m128i *)out, _mm_hadd_epi32(va, vb));
        h = mix(h, out);
        _mm_storeu_si128((__m128i *)out, _mm_maddubs_epi16(va, vb));
        h = mix(h, out);
        _mm_storeu_si128((__m128i *)out, _mm_sign_epi8(va, vb));
        h = mix(h, out);
        _mm_storeu_si128((__m128i *)out, _mm_abs_epi32(va));
        h = mix(h, out);
        _mm_storeu_si128((__m128i *)out, _mm_alignr_epi8(va, vb, 5));
        h = mix(h, out);
#else
        ...
        // PHADDD: out = [a0+a1, a2+a3, b0+b1, b2+b3] as 32-bit words
        {
            ...
            memcpy(ai, a, 16);
            memcpy(bi, b, 16);
            int32_t r[4] = {ai[0] + ai[1], ai[2] + ai[3], bi[0] + bi[1], bi[2] + bi[3]};
            uint8_t rb[16];
            memcpy(rb, r, 16);
            h = mix(h, rb);
        }
        // PMADDUBSW: word k = sat16( a[2k]*(int8)b[2k] + a[2k+1]*(int8)b[2k+1] ), a unsigned, b signed
        {
            int16_t r[8];
            for (int k = 0; k < 8; k++) {
                int p = (int)(uint8_t)a[2 * k] * (int)(int8_t)b[2 * k] +
                        (int)(uint8_t)a[2 * k + 1] * (int)(int8_t)b[2 * k + 1];
                if (p > 32767) p = 32767;
                if (p < -32768) p = -32768;
                r[k] = (int16_t)p;
            }
        ...
        // PSIGNB: out[j] = b[j]<0 ? -a[j] : (b[j]==0 ? 0 : a[j])  (signed)
        {
            ...
            for (int j = 0; j < 16; j++) {
                int8_t s = (int8_t)b[j];
                int8_t av = (int8_t)a[j];
                r[j] = (uint8_t)(s < 0 ? -av : (s == 0 ? 0 : av));
            }
            h = mix(h, r);
        }
        // PABSD: absolute value of 32-bit lanes of a
        {
            ...
            uint32_t r[4];
            for (int i = 0; i < 4; i++)
                r[i] = (uint32_t)(ai[i] < 0 ? -(int64_t)ai[i] : ai[i]);
            uint8_t rb[16];
            memcpy(rb, r, 16);
            h = mix(h, rb);
        }
