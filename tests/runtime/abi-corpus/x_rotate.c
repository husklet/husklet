...
static uint8_t r8(uint8_t v, unsigned n) {
    n &= 7;
    return n ? (uint8_t)((v << n) | (v >> (8 - n))) : v;
}

static uint16_t r16(uint16_t v, unsigned n) {
    n &= 15;
    return n ? (uint16_t)((v << n) | (v >> (16 - n))) : v;
}

static uint32_t r32(uint32_t v, unsigned n) {
    n &= 31;
    return n ? (v << n) | (v >> (32 - n)) : v;
}

static uint64_t r64(uint64_t v, unsigned n) {
    n &= 63;
