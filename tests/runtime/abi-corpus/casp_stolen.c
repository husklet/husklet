...
static void casp64_rs16(uint64_t m0, uint64_t m1, uint64_t e0, uint64_t e1, uint64_t n0, uint64_t n1, uint64_t *o0,
                        uint64_t *o1, uint64_t *r0, uint64_t *r1) {
...
static void casp32_rt16(uint32_t m0, uint32_t m1, uint32_t e0, uint32_t e1, uint32_t n0, uint32_t n1, uint32_t *r0,
                        uint32_t *r1) {
    uint32_t mem[2] __attribute__((aligned(8))) = {m0, m1};
    __asm__ volatile(".arch armv8.2-a\n"
                     "mov w4,%w[e0]\n mov w5,%w[e1]\n mov w16,%w[n0]\n mov w17,%w[n1]\n" SPLIT
                     "caspa w4, w5, w16, w17, [%[p]]\n" ::[e0] "r"(e0),
                     [e1] "r"(e1), [n0] "r"(n0), [n1] "r"(n1), [p] "r"(mem)
                     : "x9", "x16", "x17", "x4", "x5", "memory");
    *r0 = mem[0];
    *r1 = mem[1];
}
...
static void casp64_rn17(uint64_t m0, uint64_t m1, uint64_t e0, uint64_t e1, uint64_t n0, uint64_t n1, uint64_t *r0,
                        uint64_t *r1) {
    uint64_t mem[2] __attribute__((aligned(16))) = {m0, m1};
    __asm__ volatile(".arch armv8.2-a\n"
                     "mov x4,%[e0]\n mov x5,%[e1]\n mov x6,%[n0]\n mov x7,%[n1]\n mov x17,%[p]\n" SPLIT
