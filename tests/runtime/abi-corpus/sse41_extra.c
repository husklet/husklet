...
static void mpsadbw(uint16_t *r, const uint8_t *a, const uint8_t *b, int imm) {
    ...
    for (int i = 0; i < 8; i++) {
        int s = 0;
        for (int k = 0; k < 4; k++) {
            int d = (int)a[aoff + i + k] - (int)b[boff + k];
            s += d < 0 ? -d : d;
        }
        r[i] = (uint16_t)s;
    }
...
TGT int main(void) {
    ...
    for (int m = 0; m < 6; m++) {
        ...
        __m128i rr;
        switch (mimm[m]) {
        case 0: rr = _mm_mpsadbw_epu8(va, vb, 0); break;
        case 1: rr = _mm_mpsadbw_epu8(va, vb, 1); break;
        case 2: rr = _mm_mpsadbw_epu8(va, vb, 2); break;
        case 4: rr = _mm_mpsadbw_epu8(va, vb, 4); break;
        case 5: rr = _mm_mpsadbw_epu8(va, vb, 5); break;
        default: rr = _mm_mpsadbw_epu8(va, vb, 7);
        }
    ...
    for (int t = 0; t < 8; t++) {
        uint16_t W[8], r[8];
        for (int i = 0; i < 8; i++)
            W[i] = (uint16_t)((i * 2027 + t * 911) % 65521);
#if defined(__x86_64__)
        __m128i vw = _mm_loadu_si128((const __m128i *)W);
        __m128i rr = _mm_minpos_epu16(vw);
        _mm_storeu_si128((__m128i *)r, rr);
#else
        phminpos(r, W);
#endif
    ...
    for (int m = 0; m < 5; m++) {
        ...
        switch (dimm[m]) {
        case 0x31: rr = _mm_dp_pd(va, vb, 0x31); break;
        case 0x33: rr = _mm_dp_pd(va, vb, 0x33); break;
        case 0x11: rr = _mm_dp_pd(va, vb, 0x11); break;
        case 0x21: rr = _mm_dp_pd(va, vb, 0x21); break;
        default: rr = _mm_dp_pd(va, vb, 0x13);
        }
        _mm_storeu_pd(out, rr);
#else
        double s = dppd(DA, DB, dimm[m]);
        out[0] = (dimm[m] & 1) ? s : 0.0;
        out[1] = (dimm[m] & 2) ? s : 0.0;
#endif
        h = mix(h, b64(out[0]));
        h = mix(h, b64(out[1]));
    }
