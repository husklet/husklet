...
TGT int main(void) {
    ...
    for (int it = 0; it < 2000; it++) {
        ...
        uint32_t a[4], b[4];
        for (int i = 0; i < 4; i++) {
            a[i] = (uint32_t)(seed >> (i * 8));
            b[i] = (uint32_t)(seed >> (i * 6 + 9));
        }
        float fa[4], fb[4];
        for (int i = 0; i < 4; i++) {
            fa[i] = (float)(int)((seed >> (i * 8)) & 0xFF) - 128.0f;
            fb[i] = (float)(int)((seed >> (i * 7 + 5)) & 0xFF) - 128.0f;
        }
#if defined(__x86_64__)
        ...
        float o[4];
        _mm_storeu_ps(o, _mm_blendv_ps(vfa, vfb, mask));
        {
            uint32_t u[4];
            memcpy(u, o, 16);
            for (int i = 0; i < 4; i++)
        ...
        _mm_storeu_ps(o, _mm_insert_ps(vfa, vfb, 0x6A));
        {
            uint32_t u[4];
            memcpy(u, o, 16);
            for (int i = 0; i < 4; i++)
        ...
        _mm_storeu_ps(o, _mm_dp_ps(vfa, vfb, 0xF1));
        {
            uint32_t u[4];
            memcpy(u, o, 16);
            for (int i = 0; i < 4; i++)
        ...
        // ptestz: ZF = ((a & b)==0)
        {
            uint32_t r = (a[0] & b[0]) | (a[1] & b[1]) | (a[2] & b[2]) | (a[3] & b[3]);
            h = h * 1000003ULL + (uint32_t)(r == 0);
        }
        // ptestc: CF = ((~a & b)==0)
        {
            uint32_t r = ((~a[0]) & b[0]) | ((~a[1]) & b[1]) | ((~a[2]) & b[2]) | ((~a[3]) & b[3]);
