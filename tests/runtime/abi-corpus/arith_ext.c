...
static int32_t movsxd(int32_t v) {
    int64_t r;
    __asm__ volatile("movslq %1,%0" : "=r"(r) : "r"(v));
    return (int32_t)(r >> 1);
}
#else
static void idiv64(int64_t a, int64_t b, int64_t *q, int64_t *r) {
...
TGT int main(void) {
    ...
    for (int it = 0; it < 5000; it++) {
        ...
        int64_t q, r;
        idiv64(a, b, &q, &r);
        h = h * 1000003ULL + (uint64_t)q;
        h = h * 1000003ULL + (uint64_t)r;
        uint32_t ua = (uint32_t)seed, ub = (uint32_t)(seed >> 9);
        if (ub == 0) ub = 1;
        uint32_t uq, ur;
        div32(ua, ub, &uq, &ur);
        h = h * 1000003ULL + uq;
        ...
        int32_t m;
        int of = __builtin_mul_overflow((int32_t)seed, (int32_t)(seed >> 17), &m);
