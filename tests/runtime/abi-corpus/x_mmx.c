...
TGT int main(void) {
    ...
    for (int it = 0; it < 3000; it++) {
        ...
        uint64_t r;
        r = (uint64_t)_mm_add_si64(ma, mb);
        h = h * 1000003ULL + r; // paddq (single 64-bit)
        {
            __m64 t = _mm_madd_pi16(ma, mb);
            r = (uint64_t)t;
            h = h * 1000003ULL + r;
        } // pmaddwd
        {
            __m64 t = _mm_sad_pu8(ma, mb);
            r = (uint64_t)t;
            h = h * 1000003ULL + r;
        } // psadbw
        {
            __m64 t = _mm_packs_pi16(ma, mb);
            r = (uint64_t)t;
            h = h * 1000003ULL + r;
        } // packsswb
        {
            __m64 t = _mm_unpacklo_pi8(ma, mb);
            r = (uint64_t)t;
            h = h * 1000003ULL + r;
        } // punpcklbw
        {
            __m64 t = _mm_adds_pu8(ma, mb);
            r = (uint64_t)t;
            h = h * 1000003ULL + r;
        } // paddusb
        _mm_empty();
        ...
        // pmaddwd: 4 signed words -> 2 dwords: d0=a0*b0+a1*b1, d1=a2*b2+a3*b3
        {
            ...
            memcpy(aw, ab, 8);
            memcpy(bw, bb, 8);
            int32_t d0 = (int32_t)aw[0] * bw[0] + (int32_t)aw[1] * bw[1],
                    d1 = (int32_t)aw[2] * bw[2] + (int32_t)aw[3] * bw[3];
            uint64_t r = (uint32_t)d0 | ((uint64_t)(uint32_t)d1 << 32);
            h = h * 1000003ULL + r;
        }
        ...
        // packsswb: 4 words a, 4 words b -> 8 signed-sat bytes
        {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (v > 127) v = 127;
                if (v < -128) v = -128;
                r8[i] = (uint8_t)(int8_t)v;
            }
            for (int i = 0; i < 4; i++) {
                int v = bw[i];
                if (v > 127) v = 127;
                if (v < -128) v = -128;
                r8[4 + i] = (uint8_t)(int8_t)v;
            }
            uint64_t r;
            memcpy(&r, r8, 8);
            h = h * 1000003ULL + r;
