...
static int bt_mem(const void *base, long bit) {
    uint64_t f;
    __asm__ volatile("bt %2,%1\n\t pushfq\n\t pop %0" : "=r"(f) : "m"(*(const uint8_t *)base), "r"(bit) : "cc");
    return (int)(f & 1);
}

static int bts_mem(void *base, long bit) {
    uint64_t f;
    __asm__ volatile("bts %2,%1\n\t pushfq\n\t pop %0" : "=r"(f), "+m"(*(uint8_t *)base) : "r"(bit) : "cc");
    return (int)(f & 1);
}

static int btc_mem(void *base, long bit) {
    uint64_t f;
