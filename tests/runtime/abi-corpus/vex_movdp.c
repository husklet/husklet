...
TGT int main(void) {
    ...
    float m2[2];
    double m1;
    _mm_storel_pi((__m64 *)m2, F);
    mixb(m2, 8); // VMOVLPS store (low 64)
    _mm_storeh_pi((__m64 *)m2, F);
    mixb(m2, 8); // VMOVHPS store (high 64)
    _mm_storel_pd(&m1, D);
    mixb(&m1, 8); // VMOVLPD store
    _mm_storeh_pd(&m1, D);
    mixb(&m1, 8); // VMOVHPD store
    _mm_storeu_pd((double *)o, _mm_dp_pd(D, E, 0x31));
    mixb(o, 16); // VDPPD
    _mm_storeu_pd((double *)o, _mm_dp_pd(D, E, 0x23));
    mixb(o, 16);
    __m128 sf = _mm_set_ss((float)scal);
    _mm_store_ss((float *)o, _mm_sqrt_ss(sf));
    mixb(o, 4); // VSQRTSS
    _mm_store_ss((float *)o, _mm_rcp_ss(sf));
    mixb(o, 4); // VRCPSS
    _mm_store_ss((float *)o, _mm_rsqrt_ss(sf));
    mixb(o, 4); // VRSQRTSS
#else
    {
        float m2[2] = {f[0], f[1]};
        mixb(m2, 8);
    } // movlps store: low two floats
    {
        float m2[2] = {f[2], f[3]};
        mixb(m2, 8);
    } // movhps store: high two floats
    {
        double m1 = dd[0];
        mixb(&m1, 8);
    } // movlpd store
    ...
    } // movhpd store
    {
        ...
        double sum = 0;
        for (int i = 0; i < 2; i++)
            if (imm & (0x10 << i)) sum += dd[i] * ee[i];
        double oo[2];
        for (int i = 0; i < 2; i++)
            oo[i] = (imm & (1 << i)) ? sum : 0.0;
        memcpy(o, oo, 16);
        mixb(o, 16);
    }
    {
    ...
    {
        float y = sqrtf((float)scal);
        mixb(&y, 4);
    }
    {
        float y = 1.0f / (float)scal;
        mixb(&y, 4);
    }
    {
        float y = 1.0f / sqrtf((float)scal);
        mixb(&y, 4);
    }
#endif
