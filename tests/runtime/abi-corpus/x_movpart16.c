...
static uint64_t ins16(uint64_t base, uint16_t v) {
    uint64_t r = base;
    __asm__ volatile("mov %1,%w0" : "+r"(r) : "r"(v));
    return r;
}

static uint64_t addw(uint64_t base, uint16_t v) {
    uint64_t r = base;
    __asm__ volatile("add %1,%w0" : "+r"(r) : "r"(v) : "cc");
    return r;
}
#else
static uint64_t ins16(uint64_t base, uint16_t v) {
    return (base & ~0xFFFFULL) | v;
}

static uint64_t addw(uint64_t base, uint16_t v) {
    return (base & ~0xFFFFULL) | (uint16_t)((uint16_t)base + v);
}
#endif
