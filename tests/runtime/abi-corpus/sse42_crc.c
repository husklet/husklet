...
static uint32_t crc32c_u16(uint32_t c, uint16_t v) {
    c = crc32c_byte(c, (uint8_t)v);
    c = crc32c_byte(c, (uint8_t)(v >> 8));
    return c;
}

static uint32_t crc32c_u32(uint32_t c, uint32_t v) {
    for (int i = 0; i < 4; i++)
        c = crc32c_byte(c, (uint8_t)(v >> (i * 8)));
    return c;
}

static uint64_t crc32c_u64(uint64_t c, uint64_t v) {
    for (int i = 0; i < 8; i++)
        c = crc32c_byte((uint32_t)c, (uint8_t)(v >> (i * 8)));
    return c;
}

TGT int main(void) {
    uint64_t h = 0, seed = 0x0123456789ABCDEFULL;
    uint32_t cb = 0xFFFFFFFF, cw = 0x1234, cd = 0xABCDEF01;
    uint64_t cq = 0xDEADBEEFCAFE0000ULL;
    for (int it = 0; it < 5000; it++) {
        ...
        h = h * 1000003ULL + cw;
        h = h * 1000003ULL + cd;
        h = h * 1000003ULL + (uint32_t)cq + (uint32_t)(cq >> 32);
    }
    printf("crc=%016llx cb=%08x cw=%08x cd=%08x cq=%016llx\n", (unsigned long long)h, cb, cw, cd,
