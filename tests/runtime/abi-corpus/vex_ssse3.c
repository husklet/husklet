...
TGT int main(void) {
    ...
    int32_t d1[8], d2[8];
    for (int i = 0; i < 16; i++) {
        w[i] = (int16_t)(i * 777 - 3000);
        w2[i] = (int16_t)(i * -555 + 2000);
    }
    for (int i = 0; i < 32; i++) {
        b1[i] = (int8_t)(i * 7 - 60);
        b2[i] = (int8_t)(i * -5 + 40);
    ...
    for (int i = 0; i < 8; i++) {
        d1[i] = i * 111111 - 400000;
        d2[i] = i * -99999 + 300000;
    }
    int8_t bmin[32], bcontrol[32];
    int16_t wmin[16], wcontrol[16];
    int32_t dmin[8], dcontrol[8];
    memcpy(bmin, b1, sizeof(bmin));
    memcpy(bcontrol, b2, sizeof(bcontrol));
    memcpy(wmin, w, sizeof(wmin));
    memcpy(wcontrol, w2, sizeof(wcontrol));
    memcpy(dmin, d1, sizeof(dmin));
    memcpy(dcontrol, d2, sizeof(dcontrol));
    bmin[0] = INT8_MIN;
    bcontrol[0] = -1;
    wmin[0] = INT16_MIN;
    wcontrol[0] = -1;
...
#define PH16(av, sv, sub, sat)                                                                                         \
    do {                                                                                                               \
        const uint8_t *aa = (const uint8_t *)(av), *bb = (const uint8_t *)(sv);                                        \
        for (int lane = 0; lane < 32; lane += 16) {                                                                    \
            int16_t x[8], y[8], oo[8];                                                                                 \
            memcpy(x, aa + lane, 16);                                                                                  \
            memcpy(y, bb + lane, 16);                                                                                  \
            for (int i = 0; i < 4; i++) {                                                                              \
                int va = (sub) ? x[2 * i] - x[2 * i + 1] : x[2 * i] + x[2 * i + 1];                                    \
                int vb = (sub) ? y[2 * i] - y[2 * i + 1] : y[2 * i] + y[2 * i + 1];                                    \
                oo[i] = (sat) ? sat16(va) : (int16_t)va;                                                               \
                oo[i + 4] = (sat) ? sat16(vb) : (int16_t)vb;                                                           \
            }                                                                                                          \
            memcpy(o + lane, oo, 16);                                                                                  \
        }                                                                                                              \
        mixb(o, 32);                                                                                                   \
    } while (0)
#define PH32(av, sv, sub)                                                                                              \
    do {                                                                                                               \
        const uint8_t *aa = (const uint8_t *)(av), *bb = (const uint8_t *)(sv);                                        \
        for (int lane = 0; lane < 32; lane += 16) {                                                                    \
            int32_t x[4], y[4], oo[4];                                                                                 \
            memcpy(x, aa + lane, 16);                                                                                  \
            memcpy(y, bb + lane, 16);                                                                                  \
            oo[0] = (sub) ? x[0] - x[1] : x[0] + x[1];                                                                 \
            oo[1] = (sub) ? x[2] - x[3] : x[2] + x[3];                                                                 \
            oo[2] = (sub) ? y[0] - y[1] : y[0] + y[1];                                                                 \
            oo[3] = (sub) ? y[2] - y[3] : y[2] + y[3];                                                                 \
            memcpy(o + lane, oo, 16);                                                                                  \
        }                                                                                                              \
        mixb(o, 32);                                                                                                   \
    } while (0)
    ...
    // psign b/w/d: es
#define PSIGN(av, sv, es)                                                                                              \
    do {                                                                                                               \
        const uint8_t *aa = (const uint8_t *)(av), *bb = (const uint8_t *)(sv);                                        \
        for (int i = 0; i < 32; i += (es)) {                                                                           \
            uint64_t x = 0, y = 0;                                                                                     \
            memcpy(&x, aa + i, (es));                                                                                  \
            memcpy(&y, bb + i, (es));                                                                                  \
...
#define PABS(sv, es)                                                                                                   \
    do {                                                                                                               \
        const uint8_t *bb = (const uint8_t *)(sv);                                                                     \
        for (int i = 0; i < 32; i += (es)) {                                                                           \
            uint64_t x = 0;                                                                                            \
            memcpy(&x, bb + i, (es));                                                                                  \
            uint64_t mask = (UINT64_C(1) << ((es) * 8)) - 1, sign = UINT64_C(1) << ((es) * 8 - 1);                     \
            uint64_t oo = (x & sign) ? (UINT64_C(0) - x) & mask : x;                                                   \
            memcpy(o + i, &oo, (es));                                                                                  \
        }                                                                                                              \
        mixb(o, 32);                                                                                                   \
    } while (0)
    ...
    // maddubs: src1 unsigned bytes, src2 signed bytes -> saturated words
    {
        ...
        for (int lane = 0; lane < 32; lane += 16) {
            int16_t oo[8];
            for (int k = 0; k < 8; k++) {
                int p = (int)(uint8_t)aa[lane + 2 * k] * (int)(int8_t)bb[lane + 2 * k] +
                        (int)(uint8_t)aa[lane + 2 * k + 1] * (int)(int8_t)bb[lane + 2 * k + 1];
                oo[k] = sat16(p);
            }
            memcpy(o + lane, oo, 16);
        }
    ...
    // mulhrs
    {
        for (int i = 0; i < 32; i += 2) {
            int16_t x, y;
            memcpy(&x, (uint8_t *)w + i, 2);
            memcpy(&y, (uint8_t *)w2 + i, 2);
            int16_t v = (int16_t)((((x * y) >> 14) + 1) >> 1);
            memcpy(o + i, &v, 2);
        }
        mixb(o, 32);
    }
