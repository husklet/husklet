...
#define OUT(x)                                                                                                         \
    ...
    r = _mm256_testz_si256(D, D2);
    mixb(&r, 4);
    r = _mm256_testc_si256(D, D2);
    mixb(&r, 4);
    r = _mm256_testz_ps(F, F2);
    mixb(&r, 4); // VTESTPS
    r = _mm256_testc_ps(F, F2);
    mixb(&r, 4);
    r = _mm256_testz_pd(G, G2);
    mixb(&r, 4); // VTESTPD
    r = _mm256_testc_pd(G, G2);
    mixb(&r, 4);
    __m128i mn = _mm_minpos_epu16(_mm_loadu_si128((void *)phin));
    _mm_storeu_si128((void *)o, mn);
    mixb(o, 16); // VPHMINPOSUW
#else
    const int W = 32;
    ...
    for (int lane = 0; lane < W; lane += 16) {
        int32_t x[4], y[4];
        int64_t oo[2];
        memcpy(x, (uint8_t *)d1 + lane, 16);
        memcpy(y, (uint8_t *)d2 + lane, 16);
        oo[0] = (int64_t)x[0] * y[0];
        oo[1] = (int64_t)x[2] * y[2];
        memcpy(o + lane, oo, 16);
    }
    mixb(o, 32);
    for (int i = 0; i < W; i += 8) {
        uint64_t x, y;
        memcpy(&x, (uint8_t *)d1 + i, 8);
        memcpy(&y, (uint8_t *)d2 + i, 8);
        uint64_t v = (x == y) ? ~0ull : 0;
        memcpy(o + i, &v, 8);
    }
    mixb(o, 32); // cmpeq_epi64
    for (int i = 0; i < W; i += 8) {
        int64_t x, y;
        memcpy(&x, (uint8_t *)d1 + i, 8);
        memcpy(&y, (uint8_t *)d2 + i, 8);
    ...
    mixb(o, 32); // cmpgt_epi64
    for (int i = 0; i < W; i += 4) {
        int32_t x, y;
        memcpy(&x, (uint8_t *)d1 + i, 4);
        memcpy(&y, (uint8_t *)d2 + i, 4);
        int32_t v = x < y ? x : y;
        memcpy(o + i, &v, 4);
    }
    mixb(o, 32); // min_epi32
    ...
    for (int i = 0; i < W; i += 2) {
        uint16_t x, y;
        memcpy(&x, (uint8_t *)uw + i, 2);
        memcpy(&y, (uint8_t *)uw + i, 2);
        uint16_t v = x < y ? x : y;
        memcpy(o + i, &v, 2);
    }
    mixb(o, 32); // min_epu16 (U==U2)
    for (int i = 0; i < W; i += 2) {
        uint16_t x, y;
        memcpy(&x, (uint8_t *)uw + i, 2);
        memcpy(&y, (uint8_t *)uw + i, 2);
    ...
    mixb(o, 32); // max_epu16
    memcpy(o, d1, 32);
    mixb(o, 32); // movntdqa: streaming load == the source bytes
    ...
    // phminposuw: min unsigned word + index into low two words of a 128-bit result
    {
        ...
        uint16_t oo[8] = {best, (uint16_t)idx, 0, 0, 0, 0, 0, 0};
        memcpy(o, oo, 16);
        mixb(o, 16);
    }
#endif
