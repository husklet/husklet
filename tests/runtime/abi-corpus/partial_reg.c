...
static uint64_t ins8l(uint64_t base, uint8_t v) {
    uint64_t r = base;
    __asm__ volatile("mov %1,%b0" : "+q"(r) : "q"(v));
    return r;
}
...
static uint16_t add16(uint16_t a, uint16_t b, unsigned *fl) {
    ...
    *fl = (unsigned)(f & 0x8D5);
    return r; // CF|PF|AF|ZF|SF|OF
}
#else
static uint64_t ins8l(uint64_t base, uint8_t v) {
    return (base & ~0xFFULL) | v;
}
...
TGT int main(void) {
    ...
    for (int it = 0; it < 6000; it++) {
        ...
        unsigned fl;
        uint16_t r = add16((uint16_t)seed, (uint16_t)(seed >> 16), &fl);
        h = h * 1000003ULL + r + fl;
