...
#define OUT(x)                                                                                                         \
    ...
    sat_addsub(o, a, b, W, 1, 0, 0);
    mixb(o, 32); // adds_epi8
    sat_addsub(o, a, b, W, 2, 0, 0);
    mixb(o, 32); // adds_epi16
    sat_addsub(o, a, b, W, 1, 1, 0);
    mixb(o, 32); // adds_epu8
    sat_addsub(o, a, b, W, 2, 1, 0);
    mixb(o, 32); // adds_epu16
    sat_addsub(o, a, b, W, 1, 0, 1);
    mixb(o, 32); // subs_epi8
    sat_addsub(o, a, b, W, 2, 0, 1);
    mixb(o, 32); // subs_epi16
    sat_addsub(o, a, b, W, 1, 1, 1);
    mixb(o, 32); // subs_epu8
    sat_addsub(o, a, b, W, 2, 1, 1);
    mixb(o, 32); // subs_epu16
    for (int i = 0; i < W; i++)
        o[i] = a[i] < b[i] ? a[i] : b[i]; // min_epu8
    mixb(o, 32);
    for (int i = 0; i < W; i++)
        o[i] = a[i] > b[i] ? a[i] : b[i]; // max_epu8
    mixb(o, 32);
    for (int i = 0; i < W; i += 2) {
        int16_t x, y;
        memcpy(&x, a + i, 2);
    ...
    mixb(o, 32);
    for (int i = 0; i < W; i++) {
        uint16_t v = (uint16_t)(((uint16_t)a[i] + b[i] + 1) >> 1);
        o[i] = (uint8_t)v;
    } // avg_epu8
    mixb(o, 32);
    for (int i = 0; i < W; i += 2) {
        ...
        memcpy(&x, a + i, 2);
        memcpy(&y, b + i, 2);
        uint16_t v = (uint16_t)(((uint32_t)x + y + 1) >> 1);
        memcpy(o + i, &v, 2);
    } // avg_epu16
    mixb(o, 32);
    for (int i = 0; i < W; i += 2) {
    ...
    for (int i = 0; i < W; i += 2) {
        ...
        memcpy(&x, a + i, 2);
        memcpy(&y, b + i, 2);
        uint16_t v = (uint16_t)(((int32_t)x * y) >> 16);
        memcpy(o + i, &v, 2);
    } // mulhi_epi16
    mixb(o, 32);
    for (int i = 0; i < W; i += 2) {
        ...
        memcpy(&x, a + i, 2);
        memcpy(&y, b + i, 2);
        uint16_t v = (uint16_t)(((uint32_t)x * y) >> 16);
        memcpy(o + i, &v, 2);
    } // mulhi_epu16
    mixb(o, 32);
    for (int i = 0; i < W; i += 4) {
        ...
        memcpy(&y0, b + i, 2);
        memcpy(&y1, b + i + 2, 2);
        int32_t v = (int32_t)x0 * y0 + (int32_t)x1 * y1;
        memcpy(o + i, &v, 4);
    } // madd_epi16
    mixb(o, 32);
    memset(o, 0, 32);
