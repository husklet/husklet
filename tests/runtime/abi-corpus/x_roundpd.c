...
TGT int main(void) {
    ...
    for (int it = 0; it < 3000; it++) {
        ...
        double d[2];
        for (int i = 0; i < 2; i++)
            d[i] = (double)((int)((seed >> (i * 13 + 3)) & 0x3FFF)) / 16.0 - 512.0;
#if defined(__x86_64__)
        __m128d vd = _mm_loadu_pd(d);
        double od[2];
        _mm_storeu_pd(od, _mm_round_pd(vd, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC));
        {
            uint64_t u[2];
            memcpy(u, od, 16);
            for (int i = 0; i < 2; i++)
        ...
        _mm_storeu_pd(od, _mm_round_pd(vd, _MM_FROUND_TO_NEG_INF | _MM_FROUND_NO_EXC));
        {
            uint64_t u[2];
            memcpy(u, od, 16);
            for (int i = 0; i < 2; i++)
        ...
        _mm_storeu_pd(od, _mm_round_pd(vd, _MM_FROUND_TO_POS_INF | _MM_FROUND_NO_EXC));
        {
            uint64_t u[2];
            memcpy(u, od, 16);
            for (int i = 0; i < 2; i++)
        ...
        _mm_storeu_pd(od, _mm_round_pd(vd, _MM_FROUND_TO_ZERO | _MM_FROUND_NO_EXC));
        {
            uint64_t u[2];
            memcpy(u, od, 16);
            for (int i = 0; i < 2; i++)
