...
TGT int main(void) {
    ...
    double da[4];
    for (int i = 0; i < 8; i++) {
        fa[i] = (float)(i * 5 - 17) * 0.5f + 0.3f;
        fb[i] = (float)(i * 3 - 4) * 0.25f - 0.7f;
    }
    for (int i = 0; i < 4; i++)
        da[i] = (double)(i * 7 - 11) * 0.5 + 0.4;
    float ob[8];
    double od[4];
#if defined(__x86_64__)
    ...
    _mm256_storeu_ps(ob, _mm256_round_ps(A, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC));
    mixb(ob, 32);
    _mm256_storeu_ps(ob, _mm256_round_ps(A, _MM_FROUND_TO_ZERO | _MM_FROUND_NO_EXC));
    mixb(ob, 32);
    _mm256_storeu_ps(ob, _mm256_round_ps(A, _MM_FROUND_TO_POS_INF | _MM_FROUND_NO_EXC));
    mixb(ob, 32);
    _mm256_storeu_ps(ob, _mm256_round_ps(A, _MM_FROUND_TO_NEG_INF | _MM_FROUND_NO_EXC));
    mixb(ob, 32);
    _mm256_storeu_pd(od, _mm256_round_pd(DA, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC));
    mixb(od, 32);
    _mm256_storeu_pd(od, _mm256_round_pd(DA, _MM_FROUND_TO_ZERO | _MM_FROUND_NO_EXC));
    mixb(od, 32);
    _mm_storeu_ps(ob, _mm_dp_ps(a, b, 0x71));
    mixb(ob, 16);
    _mm_storeu_ps(ob, _mm_dp_ps(a, b, 0xFF));
    mixb(ob, 16);
    _mm256_storeu_ps(ob, _mm256_dp_ps(A, B, 0xF1));
    mixb(ob, 32);
    _mm256_storeu_ps(ob, _mm256_dp_ps(A, B, 0x3B));
    mixb(ob, 32);
#else
    for (int i = 0; i < 8; i++)
        ob[i] = __builtin_roundevenf(fa[i]);
    mixb(ob, 32);
    for (int i = 0; i < 8; i++)
        ob[i] = __builtin_truncf(fa[i]);
    mixb(ob, 32);
    for (int i = 0; i < 8; i++)
        ob[i] = __builtin_ceilf(fa[i]);
    mixb(ob, 32);
    for (int i = 0; i < 8; i++)
        ob[i] = __builtin_floorf(fa[i]);
    mixb(ob, 32);
    for (int i = 0; i < 4; i++)
        od[i] = __builtin_roundeven(da[i]);
    mixb(od, 32);
    for (int i = 0; i < 4; i++)
        od[i] = __builtin_trunc(da[i]);
    mixb(od, 32);
