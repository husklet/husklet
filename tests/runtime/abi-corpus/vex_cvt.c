...
TGT int main(void) {
    ...
    _mm_storeu_si128((void *)o, _mm256_cvtpd_epi32(D));
    mixb(o, 16); // VCVTPD2DQ (round)
    _mm_storeu_si128((void *)o, _mm256_cvttpd_epi32(D));
    mixb(o, 16); // VCVTTPD2DQ (truncate)
    _mm256_storeu_pd((double *)o, _mm256_cvtepi32_pd(I4));
    mixb(o, 32); // VCVTDQ2PD
    _mm256_storeu_ps((float *)o, _mm256_rcp_ps(RF));
    mixb(o, 32); // VRCPPS
    _mm256_storeu_ps((float *)o, _mm256_rsqrt_ps(RF));
    mixb(o, 32); // VRSQRTPS
#else
    {
        int32_t oo[4];
        for (int i = 0; i < 4; i++)
            oo[i] = (int32_t)__builtin_rint(dd[i]);
        memcpy(o, oo, 16);
        mixb(o, 16);
    }
    {
        int32_t oo[4];
        for (int i = 0; i < 4; i++)
            oo[i] = (int32_t)__builtin_trunc(dd[i]);
        memcpy(o, oo, 16);
        mixb(o, 16);
    }
    {
