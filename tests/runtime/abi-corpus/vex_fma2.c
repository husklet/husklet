...
TGT int main(void) {
    ...
    for (int i = 0; i < 8; i++) {
        fa[i] = (float)(i - 3) + 0.5f;
        fb[i] = (float)(i * 2 - 5) + 0.25f;
        fc[i] = (float)(i - 1) - 0.75f;
    }
    for (int i = 0; i < 4; i++) {
        da[i] = (double)(i - 2) + 0.5;
        db[i] = (double)(i * 3 - 4) + 0.25;
        dc[i] = (double)(i - 1) - 0.5;
    }
    int32_t iv[8];
    for (int i = 0; i < 8; i++)
    ...
    __m128 sf = _mm_set_ss(2.7f);
    __m128d sd = _mm_set_sd(-3.4);
    _mm256_storeu_ps(of, _mm256_fmaddsub_ps(FA, FB, FC));
    mixb(of, 32);
    _mm256_storeu_pd(od, _mm256_fmaddsub_pd(DA, DB, DC));
    mixb(od, 32);
    _mm256_storeu_ps(of, _mm256_fmsubadd_ps(FA, FB, FC));
    mixb(of, 32);
    _mm256_storeu_pd(od, _mm256_fmsubadd_pd(DA, DB, DC));
    mixb(od, 32);
    _mm_store_ss(of, _mm_round_ss(sf, sf, _MM_FROUND_TO_POS_INF | _MM_FROUND_NO_EXC));
    mixb(of, 4);
    _mm_store_sd(od, _mm_round_sd(sd, sd, _MM_FROUND_TO_NEG_INF | _MM_FROUND_NO_EXC));
    mixb(od, 8);
    _mm256_storeu_si256((void *)o, _mm256_lddqu_si256((void *)iv));
    mixb(o, 32);
    _mm_storeu_si128((void *)o, _mm_lddqu_si128((void *)iv));
    mixb(o, 16);
#else
    // fmaddsub: even lane subtract, odd lane add; fmsubadd: opposite. 213 form: m1=a, m2=b, add=c.
#define FMAS(av, bv, cv, n, dbl, subadd)                                                                               \
    do {                                                                                                               \
        for (int i = 0; i < (n); i++) {                                                                                \
            int even = (i & 1) == 0;                                                                                   \
            int sub = (subadd) ? !even : even;                                                                         \
            if (dbl) {                                                                                                 \
                double r = __builtin_fma((av)[i], (bv)[i], sub ? -(cv)[i] : (cv)[i]);                                  \
                od[i] = r;                                                                                             \
            } else {                                                                                                   \
                float r = __builtin_fmaf((av)[i], (bv)[i], sub ? -(cv)[i] : (cv)[i]);                                  \
                of[i] = r;                                                                                             \
            }                                                                                                          \
        }                                                                                                              \
        mixb(dbl ? (void *)od : (void *)of, (n) * ((dbl) ? 8 : 4));                                                    \
    } while (0)
    ...
    {
        float y = rnd_f(2.7f, 2);
        mixb(&y, 4);
    } // round_ss toward +inf
    {
        double y = rnd_d(-3.4, 1);
        mixb(&y, 8);
    } // round_sd toward -inf
    memcpy(o, iv, 32);
    mixb(o, 32); // lddqu 256
