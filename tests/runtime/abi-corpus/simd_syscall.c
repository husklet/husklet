...
// Output is arch-independent booleans, so the .oracle() check (JIT stdout == native/qemu stdout) catches
// any corruption on either guest arch. getpid is used (never an engine inline-fast syscall) so every
...
static void fill(unsigned char *b, int n, unsigned seed) {
    for (int i = 0; i < n; i++)
        b[i] = (unsigned char)(seed * 131u + i * 17u + 0x5A);
}
...
static void clean_multi(const unsigned char *pat, unsigned char *out, long n) {
    __asm__ volatile("ldp q0, q1, [%[p], #0]   \n"
                     "ldp q2, q3, [%[p], #32]  \n"
                     "ldp q4, q5, [%[p], #64]  \n"
                     "ldp q6, q7, [%[p], #96]  \n"
                     "mov x9, %[n]             \n"
...
static void clean_multi(const unsigned char *pat, unsigned char *out, long n) {
    __asm__ volatile("movdqu 0(%[p]), %%xmm0   \n"
                     "movdqu 16(%[p]), %%xmm1  \n"
                     "movdqu 32(%[p]), %%xmm2  \n"
                     "movdqu 48(%[p]), %%xmm3  \n"
                     "movdqu 64(%[p]), %%xmm4  \n"
                     ...
                     :
                     : [p] "r"(pat), [o] "r"(out), [n] "r"(n)
                     : "rax", "rcx", "r11", "r12", "xmm0", "xmm1", "xmm2", "xmm3", "xmm4", "xmm5", "xmm6", "xmm7",
                       "memory");
}

static void dirty_block(const unsigned char *pat, unsigned char *out) {
    __asm__ volatile("movdqu 0(%[p]), %%xmm3 \n"
                     "mov $39, %%eax         \n"
