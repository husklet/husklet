...
TGT int main(void) {
    ...
    for (int it = 0; it < 2000; it++) {
        ...
        uint64_t aq[4], shq[4];
        for (int i = 0; i < 8; i++) {
            a[i] = (uint32_t)(seed >> (i * 3));
            sh[i] = (uint32_t)((seed >> (i * 4)) & 0x3F);
        }
        for (int i = 0; i < 4; i++) {
            aq[i] = seed * (i + 1) + 0x9E3779B9ULL;
            shq[i] = (seed >> (i * 7)) & 0x7F;
        }
        uint8_t out[32];
#if defined(__x86_64__)
        ...
        _mm256_storeu_si256((__m256i *)out, _mm256_sllv_epi32(va, vsh));
        h = mix32(h, out);
        _mm256_storeu_si256((__m256i *)out, _mm256_srlv_epi32(va, vsh));
        h = mix32(h, out);
        _mm256_storeu_si256((__m256i *)out, _mm256_srav_epi32(va, vsh));
        h = mix32(h, out);
        _mm256_storeu_si256((__m256i *)out, _mm256_sllv_epi64(vaq, vshq));
        h = mix32(h, out);
        h = h * 1000003ULL + (uint32_t)_mm256_movemask_epi8(va);
        _mm256_storeu_si256((__m256i *)out, _mm256_shuffle_epi32(va, 0x1B));
        h = mix32(h, out);
#else
        ...
        }
        {
            uint32_t r[8];
            for (int i = 0; i < 8; i++) {
                int32_t s = (int32_t)a[i];
                uint32_t c = sh[i];
                r[i] = (uint32_t)((c >= 32) ? (s >> 31) : (s >> c));
            }
            uint8_t rb[32];
            memcpy(rb, r, 32);
            h = mix32(h, rb);
