...
static uint32_t rd(void) {
    uint32_t m;
    __asm__ volatile("stmxcsr %0" : "=m"(m));
    return m;
}

static void wr(uint32_t m) {
...
static int quiet_inv(float a, float b) {
    wr(rd() & ~EXCMASK);
    int r;
    __asm__ volatile("ucomiss %2,%1" : "=@ccae"(r) : "x"(a), "x"(b) : "cc");
    (void)r;
    return (rd() & 1) != 0;
}
...
static int signaling_inv(float a, float b) {
    wr(rd() & ~EXCMASK);
    int r;
    __asm__ volatile("comiss %2,%1" : "=@ccae"(r) : "x"(a), "x"(b) : "cc");
    (void)r;
    return (rd() & 1) != 0;
}
...
static uint32_t rd_fpsr(void) {
    uint64_t v;
    __asm__ volatile("mrs %0, fpsr" : "=r"(v));
    return (uint32_t)v;
}

static void wr_fpsr(uint32_t m) {
    uint64_t v = m;
...
static uint32_t exc_clear_and_read_after(volatile float *out) {
    wr(rd() & ~EXCMASK);
    volatile float minf = u2f(0x00800000u);
    *out = minf * 0.5f;
    return rd() & EXCMASK;
}
#elif defined(__aarch64__)
...
int main(void) {
    ...
    wr(save | FLUSHBITS);
    {
        volatile float t = u2f(0x00000001u);
        volatile float s = t + t;
