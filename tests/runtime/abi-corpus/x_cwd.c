...
static int16_t cwd_dx(int16_t ax) {
    int16_t dx;
    __asm__ volatile("cwd" : "=d"(dx) : "a"(ax) : "cc");
    return dx;
}

static int16_t cbw_ax(int8_t al) {
    int16_t ax;
    __asm__ volatile("cbtw" : "=a"(ax) : "a"(al));
    return ax;
}
#else
static int16_t cwd_dx(int16_t ax) {
    return (int16_t)((ax < 0) ? -1 : 0);
}
