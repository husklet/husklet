...
static unsigned shmask(unsigned cnt) {
    return cnt == 0 ? 0x8D5u : (0xC5u | (cnt == 1 ? 0x800u : 0u));
}

// defined-flag mask for a ROTATE of masked count cnt (rotates only set CF, and OF iff cnt==1)
static unsigned romask(unsigned cnt) {
    return 0x01u | (cnt == 1 ? 0x800u : 0u);
}
...
static unsigned shl32(uint32_t v, uint8_t c, unsigned in_fl, uint32_t *res) {
    ...
    __asm__ volatile("push %3\n\t popfq\n\t shl %%cl,%0\n\t pushfq\n\t pop %1"
                     : "+r"(r), "=r"(f)
                     : "c"(c), "r"((uint64_t)in_fl)
                     : "cc");
    *res = r;
    return (unsigned)f & shmask(c & 31);
}

static unsigned rol32(uint32_t v, uint8_t c, uint32_t *res) {
    uint64_t f;
    uint32_t r = v;
    __asm__ volatile("rol %%cl,%0\n\t pushfq\n\t pop %1" : "+r"(r), "=r"(f) : "c"(c) : "cc");
    *res = r;
    return (unsigned)f & romask(c & 31);
}

static unsigned ror64(uint64_t v, uint8_t c, uint64_t *res) {
    uint64_t f, r = v;
    __asm__ volatile("ror %%cl,%0\n\t pushfq\n\t pop %1" : "+r"(r), "=r"(f) : "c"(c) : "cc");
    *res = r;
    return (unsigned)f & romask(c & 63);
}
#else
static unsigned shl32(uint32_t v, uint8_t c, unsigned in_fl, uint32_t *res) {
    unsigned cnt = c & 31;
    if (cnt == 0) {
        *res = v;
        return in_fl & shmask(0);
    } // count 0: all flags unchanged
    uint32_t r = v << cnt;
    unsigned f = 0;
...
static unsigned rol32(uint32_t v, uint8_t c, uint32_t *res) {
    ...
    uint32_t r = (v << cnt) | (v >> (32 - cnt));
    unsigned f = (r & 1);                                    // CF = new LSB
...
static unsigned ror64(uint64_t v, uint8_t c, uint64_t *res) {
    ...
    uint64_t r = (v >> cnt) | (v << (64 - cnt));
    unsigned f = (unsigned)((r >> 63) & 1);                          // CF = new MSB
...
TGT int main(void) {
    ...
    for (int it = 0; it < 6000; it++) {
        ...
        uint32_t s;
        unsigned f = shl32((uint32_t)seed, (uint8_t)(seed >> 8), inf, &s); // count incl. 0
        h = h * 1000003ULL + s + f;
        uint32_t rl;
        f = rol32((uint32_t)(seed >> 3), (uint8_t)(((seed >> 11) % 31) + 1), &rl);
        h = h * 1000003ULL + rl + f;
        uint64_t rr;
        f = ror64(seed, (uint8_t)(((seed >> 17) % 63) + 1), &rr);
