...
int main(int argc, char **argv) {
    ...
    printf("dup3_same_einval=%d dup3_ok=%d dup3_cloexec=%d dup3_badflag_einval=%d\n", (same < 0 && errno == EINVAL),
           (ok == hi), ((cf & FD_CLOEXEC) != 0), (badf < 0 && errno == EINVAL));
    close(hi);
    close(fd);
    ...
    int all_ce = 1;
    for (int f = lo; f <= c; f++) {
        int g = fcntl(f, F_GETFD);
        if (g < 0 || !(g & FD_CLOEXEC)) all_ce = 0;
    }
    long cr = syscall(__NR_close_range, lo, c, 0);
    int all_closed = 1;
    for (int f = lo; f <= c; f++)
        if (fcntl(f, F_GETFD) >= 0) all_closed = 0;
    ...
    printf("cr_cloexec=%d cr_all_cloexec=%d cr_close=%d cr_all_closed=%d cr_std_alive=%d "
           ...
           (crc == 0), all_ce, (cr == 0), all_closed, std_alive, (lohi < 0 && errno == EINVAL),
           (crbad < 0 && errno == EINVAL));
    ...
    printf("sched_getcpu_valid=%d getcpu_ok=%d getcpu_consistent=%d cpu_in_range=%d\n", (sc >= 0 && sc < np), (gc == 0),
           ((int)gcpu == sc), ((int)gcpu >= 0 && (int)gcpu < np));
    return 0;
}
