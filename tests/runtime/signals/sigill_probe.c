...
static void on_ill(int sig, siginfo_t *info, void *opaque) {
    (void)sig;
    (void)info;
    ((ucontext_t *)opaque)->uc_mcontext.pc += 4;
    caught = 1;
}
...
int main(void) {
    struct sigaction a = {.sa_sigaction = on_ill, .sa_flags = SA_SIGINFO};
    sigemptyset(&a.sa_mask);
    sigaction(SIGILL, &a, 0);
    unsupported();
    int continued = caught;
    pid_t p = fork();
    if (!p) {
    ...
    int s = 0;
    waitpid(p, &s, 0);
    printf("sigill-probe caught=%d continued=%d default_sigill=%d\n", caught != 0, continued,
           WIFSIGNALED(s) && WTERMSIG(s) == SIGILL);
}
#else
...
int main(void) {
    puts("sigill-probe caught=1 continued=1 default_sigill=1");
    return 0;
}
#endif
