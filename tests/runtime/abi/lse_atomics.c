...
int main(void) {
    ...
    {
        uint32_t v = 0xAABBCCDD, e = v;
        int ok = __atomic_compare_exchange_n(&v, &e, 0x01020304u, 0, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE);
        CHK(ok && v == 0x01020304u, "cas32");
    }

    // ---- SWP ----
    {
        uint64_t v = 0x11, old = __atomic_exchange_n(&v, 0x99, __ATOMIC_ACQ_REL);
        CHK(old == 0x11 && v == 0x99, "swp64");
    }
    {
        uint32_t v = 0x7, old = __atomic_exchange_n(&v, 0x5A5A5A5Au, __ATOMIC_ACQUIRE);
        CHK(old == 7 && v == 0x5A5A5A5Au, "swp32");
    }

    // ---- LDADD (fetch_add) ----
    {
        uint64_t v = 100, old = __atomic_fetch_add(&v, 23, __ATOMIC_RELEASE);
        CHK(old == 100 && v == 123, "ldadd64");
    }
    {
        uint32_t v = 0xFFFFFFF0u, old = __atomic_fetch_add(&v, 0x20, __ATOMIC_RELAXED);
        CHK(old == 0xFFFFFFF0u && v == 0x10u, "ldadd32 wrap");
    }

    // ---- LDSET (fetch_or), LDCLR (fetch_and via ~), LDEOR (fetch_xor) ----
    {
        uint64_t v = 0xF0F0, old = __atomic_fetch_or(&v, 0x0F0F, __ATOMIC_ACQ_REL);
        CHK(old == 0xF0F0 && v == 0xFFFF, "ldset");
    }
    {
        uint64_t v = 0xFFFF, old = __atomic_fetch_and(&v, 0xF0F0, __ATOMIC_ACQ_REL);
        CHK(old == 0xFFFF && v == 0xF0F0, "ldclr");
    }
    {
        uint64_t v = 0xFFFF, old = __atomic_fetch_xor(&v, 0x0FF0, __ATOMIC_ACQ_REL);
        CHK(old == 0xFFFF && v == 0xF00F, "ldeor");
    }
    ...
    // ---- realistic: a refcount / spinlock-ish loop (glibc pattern) ----
    {
        ...
        for (int i = 0; i < 1000; i++)
            __atomic_fetch_add(&rc, 1, __ATOMIC_ACQ_REL);
        for (int i = 0; i < 1000; i++)
            __atomic_fetch_sub(&rc, 1, __ATOMIC_ACQ_REL);
    ...
    // CAS-based lock acquire/release cycle
    {
        ...
        for (int i = 0; i < 500; i++) {
            uint32_t e = 0;
            while (!__atomic_compare_exchange_n(&lock, &e, 1, 0, __ATOMIC_ACQUIRE, __ATOMIC_RELAXED))
                e = 0;
