...
int main(void) {
    ...
    const char *files[] = {
        "cpuset.cpus.effective", "cpuset.mems.effective", "pids.peak",        "memory.oom.group", "pids.events",
        "pids.events.local",     "memory.swap.events",    "memory.swap.peak", "cpu.stat.local",
    };
