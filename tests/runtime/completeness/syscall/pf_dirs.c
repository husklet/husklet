...
int main(void) {
    ...
    int topo =
        dir_ok("/sys/devices/system/cpu/cpu0/topology") && dir_has("/sys/devices/system/cpu/cpu0/topology", "core_id");
    ...
    char lb[64];
    int ns =
        dir_ok("/proc/self/ns") && dir_has("/proc/self/ns", "net") && readlink("/proc/self/ns/net", lb, sizeof lb) > 0;
    int devfd = 0;
    {
