...
int main(void) {
    ...
    if (fd >= 0) close(fd);
    int clean = n >= 0 && !strstr(b, "sha3") && !strstr(b, "sm3") && !strstr(b, "sm4");
    pid_t p = fork();
    if (!p) {
        uint64_t v;
    ...
    printf("cpu-discovery hwcap=%lx cpuid=%d sha3=%d sm3=%d sm4=%d ctr=%llx dczid=%llu proc_clean=%d id_sigill=%d\n", h,
           !!(h & (1ul << 11)), !!(h & (1ul << 17)), !!(h & (1ul << 18)), !!(h & (1ul << 19)), (unsigned long long)ctr,
           (unsigned long long)dczid, clean, WIFSIGNALED(s) && WTERMSIG(s) == 4);
    return 0;
}
#else
