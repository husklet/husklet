...
#define MEMSH64(op, init, cnt)                                                                                         \
    do {                                                                                                               \
        uint64_t _m = (uint64_t)(init);                                                                                \
        unsigned long _f = 0;                                                                                          \
        __asm__ volatile("movq %[c], %%rcx\n\t" op " %%cl, %[m]\n\t"                                                   \
                         "pushfq\n\t pop %[f]\n\t"                                                                     \
                         : [m] "+m"(_m), [f] "=r"(_f)                                                                  \
                         : [c] "r"((uint64_t)(cnt))                                                                    \
                         : "rcx", "cc");                                                                               \
        printf("%s.64 in=%016llx cnt=%2llu -> %016llx fl=%03lx\n", op, (unsigned long long)(uint64_t)(init),           \
               (unsigned long long)(uint64_t)(cnt), (unsigned long long)_m, _f & FLMASK);                              \
    } while (0)

/* 32-bit memory-dest shift by CL. */
#define MEMSH32(op, init, cnt)                                                                                         \
    do {                                                                                                               \
        uint32_t _m = (uint32_t)(init);                                                                                \
        unsigned long _f = 0;                                                                                          \
        __asm__ volatile("movq %[c], %%rcx\n\t" op " %%cl, %[m]\n\t"                                                   \
                         "pushfq\n\t pop %[f]\n\t"                                                                     \
                         : [m] "+m"(_m), [f] "=r"(_f)                                                                  \
                         : [c] "r"((uint64_t)(cnt))                                                                    \
                         : "rcx", "cc");                                                                               \
        printf("%s.32 in=%08x cnt=%2llu -> %08x fl=%03lx\n", op, (unsigned)(uint32_t)(init),                           \
               (unsigned long long)(uint64_t)(cnt), (unsigned)_m, _f & FLMASK);                                        \
    } while (0)

/* Full SIB form: shr/shl/sar [base + idx*8 - 8], cl -- the exact jemalloc bitmap_init encoding. */
#define SIBSH64(op, cnt)                                                                                               \
    do {                                                                                                               \
        uint64_t a[4] = {0xfedcba9876543210ULL, 0x0f1e2d3c4b5a6978ULL, 0xffffffffffffffffULL, 0x8000000000000001ULL};  \
        uint64_t idx = 3; /* -> a[idx-1] = a[2] */                                                                     \
        __asm__ volatile("movq %[c], %%rcx\n\t"                                                                        \
                         "leaq %[base], %%rax\n\t"                                                                     \
                         "movq %[i], %%rdx\n\t" op " %%cl, -0x8(%%rax,%%rdx,8)\n\t"                                    \
                         : [base] "+m"(a)                                                                              \
                         : [c] "r"((uint64_t)(cnt)), [i] "r"(idx)                                                      \
                         : "rax", "rcx", "rdx", "cc");                                                                 \
        printf("%s.sib cnt=%2llu -> a2=%016llx\n", op, (unsigned long long)(uint64_t)(cnt), (unsigned long long)a[2]); \
    } while (0)
...
int main(void) {
    ...
    MEMSH64("shrq", v64, 62);
    MEMSH64("shrq", v64, 63);
    MEMSH64("shrq", v64, 64 /* &63 == 0: no change, no flags */);
    MEMSH32("shrl", v32, 1);
    MEMSH32("shrl", v32, 5);
    MEMSH32("shrl", v32, 31);
