...
int main(void) {
    ...
    unsigned long v;
    unsigned char cl;
#define SH1(name, mn, init, rmask, show)                                                                               \
    v = init;                                                                                                          \
    __asm__ volatile(mn "\n\tpushfq\n\tpop %1" : "+q"(v), "=r"(_f)::"cc");                                             \
    PR(name, v &rmask, show)
#define SH1R(name, mn, init, rmask, show)                                                                              \
    v = init;                                                                                                          \
    __asm__ volatile(mn "\n\tpushfq\n\tpop %1" : "+r"(v), "=r"(_f)::"cc");                                             \
    PR(name, v &rmask, show)
#define SHCL(name, mn, init, c, rmask, show)                                                                           \
    v = init;                                                                                                          \
    cl = c;                                                                                                            \
    __asm__ volatile(mn "\n\tpushfq\n\tpop %1" : "+r"(v), "=r"(_f) : "c"(cl) : "cc");                                  \
    PR(name, v &rmask, show)
...
#define IM2(name, ty, mn, ia, ib)                                                                                      \
    {                                                                                                                  \
        ty a = ia, b = ib;                                                                                             \
        __asm__ volatile(mn "\n\tpushfq\n\tpop %1" : "+r"(a), "=r"(_f) : "r"(b) : "cc");                               \
        PR(name, (unsigned long)a, SHOW_MUL);                                                                          \
    }
    ...
    {
        int a = 0x40000000, r;
        __asm__ volatile("imull $4,%2,%0\n\tpushfq\n\tpop %1" : "=r"(r), "=r"(_f) : "r"(a) : "cc");
        PR("imul3_of", (unsigned long)(unsigned)r, SHOW_MUL);
    }
    {
        int a = 7, r;
        __asm__ volatile("imull $3,%2,%0\n\tpushfq\n\tpop %1" : "=r"(r), "=r"(_f) : "r"(a) : "cc");
        PR("imul3_ok", (unsigned long)(unsigned)r, SHOW_MUL);
    }
    // ---- MUL unsigned one-operand: CF/OF only ----
    {
        unsigned long a = 0x100000000UL, d;
        __asm__ volatile("mulq %2\n\tpushfq\n\tpop %0" : "=r"(_f), "+a"(a), "=d"(d) : "r"(2UL) : "cc");
        PR("mulq_big", d, SHOW_MUL);
    }
    {
        unsigned int a = 3, d;
        __asm__ volatile("mull %2\n\tpushfq\n\tpop %0" : "=r"(_f), "+a"(a), "=d"(d) : "r"(5U) : "cc");
        PR("mull_small", a, SHOW_MUL);
    }
    return 0;
}
