...
__attribute__((target("sha,sse4.1,ssse3"))) static void sha256_block(uint32_t state[8], const uint8_t *data) {
    ...
    CDGH = STATE1;
    for (int i = 0; i < 4; i++)
        M[i] = _mm_shuffle_epi8(_mm_loadu_si128((const __m128i *)(data + 16 * i)), MASK);
    for (int i = 0; i < 16; i++) {
        __m128i cur = M[i & 3];
...
// ---------------- SHA-1 via SHA-NI ----------------
__attribute__((target("sha,sse4.1,ssse3"))) static void sha1_blocks(uint32_t state[5], const uint8_t *data,
                                                                    uint64_t length) {
...
int main(void) {
    ...
    static const uint32_t k256_empty[8] = {0xe3b0c442, 0x98fc1c14, 0x9afbf4c8, 0x996fb924,
                                           0x27ae41e4, 0x649b934c, 0xa495991b, 0x7852b855};
    static const uint32_t k256_abc[8] = {0xba7816bf, 0x8f01cfea, 0x414140de, 0x5dae2223,
                                         0xb00361a3, 0x96177a9c, 0xb410ff61, 0xf20015ad};
    static const uint32_t k256_448[8] = {0x248d6a61, 0xd20638b8, 0xe5c02693, 0x0c3e6039,
                                         0xa33ce459, 0x64ff2167, 0xf6ecedd4, 0x19db06c1};
    static const uint32_t k256_mil[8] = {0xcdc76e5c, 0x9914fb92, 0x81a1c7e2, 0x84d73e67,
                                         0xf1809a48, 0xa497200e, 0x046d39cc, 0xc7112cd0};
    sha256((const uint8_t *)"", 0, d8);
    p256("sha256-empty", d8);
    ok &= eq(d8, k256_empty, 8);
    sha256((const uint8_t *)"abc", 3, d8);
    p256("sha256-abc", d8);
    ok &= eq(d8, k256_abc, 8);
    ...
    sha256(buf, 1000000, d8);
    p256("sha256-million-a", d8);
    ok &= eq(d8, k256_mil, 8);
    ...
    sha1((const uint8_t *)"", 0, d5);
    p160("sha1-empty", d5);
    ok &= eq(d5, k1_empty, 5);
    sha1((const uint8_t *)"abc", 3, d5);
    p160("sha1-abc", d5);
    ok &= eq(d5, k1_abc, 5);
    ...
    static const int lens[] = {1,   3,   8,   31,  55,  56,  57,  63,  64,  65,  119, 120,
                               127, 128, 129, 191, 255, 256, 300, 447, 448, 511, 512, 577};
    for (unsigned i = 0; i < sizeof lens / sizeof lens[0]; i++) {
        ...
        char tag[32];
        sha256(buf, (uint64_t)L, d8);
        snprintf(tag, sizeof tag, "r256-%d", L);
        p256(tag, d8);
        sha1(buf, (uint64_t)L, d5);
        snprintf(tag, sizeof tag, "r1-%d", L);
        p160(tag, d5);
    }
