...
__attribute__((target("sse4.1"))) static long go(void) {
    ...
    __m128i m = _mm_mullo_epi32(a, b);
    __m128i mn = _mm_min_epu32(a, b);
    __m128i mx = _mm_max_epi32(a, b);
    __m128 fa = _mm_set_ps(1, 2, 3, 4);
    __m128 dp = _mm_dp_ps(fa, fa, 0xff);
    ...
    long r = ex;
    int t[4];
    _mm_storeu_si128((__m128i *)t, m);
    for (int i = 0; i < 4; i++)
        r += t[i];
    _mm_storeu_si128((__m128i *)t, mn);
    for (int i = 0; i < 4; i++)
        r += t[i];
    ...
    for (int i = 0; i < 4; i++)
        r += t[i];
    short s[8];
    _mm_storeu_si128((__m128i *)s, wb);
    for (int i = 0; i < 8; i++)
        r += s[i];
    r += (long)_mm_cvtss_f32(dp);
    r += (long)_mm_cvtsd_f64(rd);
    return r;
}
