...
int main(void) {
    void *p = mmap((void *)ADDR, 4096, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
    if (p != (void *)ADDR) {
        printf("mmap fail %p\n", p);
        return 1;
    }
    ...
    *m = 0; // A3 REX.W: mov [ADDR],rax
    __asm__ volatile(".byte 0x48,0xA3,"
                     "0x00,0x00,0x00,0x40,0x01,0x00,0x00,0x00"
                     :
                     : "a"(0xCAFEF00DDEADBEEFUL)
                     : "memory");
    sq = *m;
    *m = 0xFFFFFFFFFFFFFFFFUL; // A3: mov [ADDR],eax (writes low 32; upper 32 of MEM untouched)
    __asm__ volatile(".byte 0xA3,"
                     "0x00,0x00,0x00,0x40,0x01,0x00,0x00,0x00"
                     :
                     : "a"(0x12345678UL)
                     : "memory");
    ...
    *m = 0xFFFFFFFFFFFFFFFFUL; // 66 A3: mov [ADDR],ax (writes low 16)
    __asm__ volatile(".byte 0x66,0xA3,"
                     "0x00,0x00,0x00,0x40,0x01,0x00,0x00,0x00"
                     :
                     : "a"(0xBEEFUL)
                     : "memory");
