...
static int run_comm_matrix(const char *self, const char *directory, const char *temporary) {
    char c[64];
    read_comm(c, sizeof c);
    ...
    snprintf(link, sizeof link, "%s/lnk-selfexe", directory);
    symlink(self, link);
    run_stage(0, link, self, "s2c:lnk");
    char script[PATH_MAX + 16];
    snprintf(script, sizeof script, "%s/shb.sh", directory);
...
int main(int argc, char **argv) {
    ...
    if (commmode) return run_comm_matrix(self, tdr, td);
