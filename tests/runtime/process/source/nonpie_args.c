...
int main(void) {
    ...
    errno = 0;
    TOK("getgroups", getgroups(64, g_groups));
    errno = 0;
    TOK("getresuid", getresuid(&g_ru, &g_eu, &g_su));
    errno = 0;
    TOK("getresgid", getresgid(&g_rg, &g_eg, &g_sg));
    errno = 0;
    TOK("capget", syscall(SYS_capget, g_caphdr, g_capdata));

    // ---- rt_signal family (signal.c + rt_tgsigqueueinfo in rare.c) ----
    sigemptyset(&g_set);
    sigaddset(&g_set, SIGUSR1);
    errno = 0;
    TOK("sigprocmask", sigprocmask(SIG_BLOCK, &g_set, &g_old)); // SIGUSR1 now blocked
    errno = 0;
    TOK("sigpending", sigpending(&g_pend));
    g_act.sa_handler = SIG_IGN;
    errno = 0;
    TOK("sigaction", sigaction(SIGUSR1, &g_act, &g_oact));
    errno = 0;
    TOK("sigaltstack", sigaltstack(NULL, &g_oss)); // query current alt stack
    errno = 0;
    TOK("sigtimedwait", sigtimedwait(&g_set, &g_si, &g_zero)); // no pending -> EAGAIN (not EFAULT)
    errno = 0;
    TOK("rt_sigqueueinfo", syscall(SYS_rt_sigqueueinfo, getpid(), SIGUSR1, &g_si));
    errno = 0;
    TOK("rt_tgsigqueueinfo", syscall(SYS_rt_tgsigqueueinfo, getpid(), syscall(SYS_gettid), SIGUSR1, &g_si));

    // ---- sched / rlimit / wait ----
    errno = 0;
    TOK("getrlimit", getrlimit(RLIMIT_NOFILE, &g_rl));
    errno = 0;
    TOK("setrlimit", setrlimit(RLIMIT_NOFILE, &g_rl)); // set-back the value just read
    errno = 0;
    TOK("sched_getaffinity", sched_getaffinity(0, sizeof g_cpu, &g_cpu));
    errno = 0;
    TOK("sched_getattr", syscall(SYS_sched_getattr, 0, g_schedattr, sizeof g_schedattr, 0));
    // futex on a low .bss word + a low .bss timespec: *word==val so it blocks, then times out (ETIMEDOUT).
    errno = 0;
    TOK("futex", syscall(SYS_futex, &g_futexw, FUTEX_WAIT, g_futexw, &g_tmo));
    ...
    // ---- poll / select on low static fd_set / pollfd / timespec ----
    {
        ...
        if (pipe(pfd) == 0) {
            ...
            FD_SET(pfd[0], &g_rfds);
            errno = 0;
            TOK("pselect", pselect(pfd[0] + 1, &g_rfds, NULL, NULL, &g_tmo, NULL));
            g_pfds[0].fd = pfd[0];
            g_pfds[0].events = POLLIN;
            errno = 0;
            TOK("ppoll", ppoll(g_pfds, 1, &g_tmo, NULL));
            close(pfd[0]);
            close(pfd[1]);
        }
    }
    ...
    // ---- wait4: status + rusage written into low statics ----
    {
        ...
        if (k == 0) _exit(0);
        errno = 0;
        TOK("wait4", wait4(k, &g_status, 0, &g_ru4));
    }
    ...
    // ---- SysV IPC: msg / sem / shm pointers are all low statics ----
    {
        ...
        if (q >= 0) {
            errno = 0;
            TOK("msgsnd", msgsnd(q, &g_msg, sizeof g_msg.mtext, 0));
            errno = 0;
            TOK("msgrcv", msgrcv(q, &g_msgr, sizeof g_msgr.mtext, 0, 0));
            msgctl(q, IPC_RMID, NULL);
        }
        int s = semget(IPC_PRIVATE, 1, 0600);
        if (s >= 0) {
            errno = 0;
            TOK("semctl_setall", semctl(s, 0, SETALL, g_semarr));
            g_sops[0].sem_num = 0;
            g_sops[0].sem_op = 1;
            g_sops[0].sem_flg = 0;
            errno = 0;
            TOK("semop", semop(s, g_sops, 1));
            g_sops[0].sem_op = -1;
            errno = 0;
            TOK("semtimedop", syscall(SYS_semtimedop, s, g_sops, 1, &g_tmo));
            semctl(s, 0, IPC_RMID);
        }
        int h = shmget(IPC_PRIVATE, 4096, 0600);
        if (h >= 0) {
            errno = 0;
            TOK("shmctl", shmctl(h, IPC_STAT, &g_shmds));
