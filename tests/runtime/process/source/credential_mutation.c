...
static int ids_are(uid_t r, uid_t e, uid_t s, gid_t gr, gid_t ge, gid_t gs) {
    uid_t ur, ue, us;
    gid_t rr, re, rs;
    return getresuid(&ur, &ue, &us) == 0 && getresgid(&rr, &re, &rs) == 0 && ur == r && ue == e && us == s &&
           rr == gr && re == ge && rs == gs;
}
...
int main(void) {
    ...
    ok &= setgroups(2, wanted) == 0 && getgroups(2, got) == 2 && got[0] == 7 && got[1] == 8;
    ...
    pid_t child = fork();
    if (child == 0) _exit(ids_are(20, 21, 22, 30, 31, 32) ? 0 : 1);
    int status = 0;
