...
path, which terminated the name at a newline and ignored a zero-length write. The
zero-length half had two layers: `CommFile::write_context` returned early, and the
generic `write` syscall still answers 0 before pinning any object, so the object now
declares `accepts_empty_write`. `comm-status` pins both halves. `fork-self`,
