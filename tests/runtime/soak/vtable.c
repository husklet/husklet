...
#define M(n)                                                                                                           \
    static uint64_t m##n(struct obj *o, uint64_t a) {                                                                  \
        return (a + o->s) * (2u * n + 1u) ^ (o->s >> (n & 7));                                                         \
    }
M(0) M(1) M(2) M(3) M(4) M(5) M(6) M(7) static method methods[8] = {m0, m1, m2, m3, m4, m5, m6, m7};

int main(void) {
    struct obj objs[256];
    for (int i = 0; i < 256; i++) {
        objs[i].m = methods[i & 7];
        objs[i].s = 0x1000003ULL * (uint64_t)(i + 1);
    }
