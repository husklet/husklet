...
int main(void) {
    ...
    for (int i = 0; i < 2000; i++) {
        int fd[2];
        if (pipe(fd) != 0) {
            printf("soak forkpipe pipe_fail@%d\n", i);
            return 1;
        }
        pid_t p = fork();
        if (p < 0) {
            printf("soak forkpipe fork_fail@%d\n", i);
            return 1;
        }
        if (p == 0) {
            close(fd[0]);
            unsigned char b[4] = {(unsigned char)i, (unsigned char)(i >> 8), (unsigned char)(i * 7),
        ...
        size_t got = 0;
        ssize_t n;
        while (got < sizeof r && (n = read(fd[0], r + got, sizeof r - got)) > 0)
            got += (size_t)n;
        close(fd[0]);
        int st = 0;
