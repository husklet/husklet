...
__attribute__((noinline)) static uint64_t ta(int n) {
    if (n < 2) return 1;
    return tb(n - 1) + tc(n - 2) + 1;
}

__attribute__((noinline)) static uint64_t tb(int n) {
    if (n < 2) return 1;
    return tc(n - 1) + ta(n - 2) + 2;
}

__attribute__((noinline)) static uint64_t tc(int n) {
    if (n < 2) return 1;
    return ta(n - 1) + tb(n - 2) + 3;
}
