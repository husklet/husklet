...
int main(void) {
    ...
    int out[2];
    if (pipe(out) < 0) return 1; // worker -> us: the signals it saw
    int rdy[2];
    if (pipe(rdy) < 0) return 1; // leader -> worker: you are the foreground group
    int ack[2];
    if (pipe(ack) < 0) return 1; // worker -> leader: my handlers are armed
    ...
    if (c == 0) {
        ...
        if (w == 0) {
            ...
            char a = 'A';
            if (write(ack[1], &a, 1) != 1) _exit(2); // handlers armed: the leader may proceed
            char r;
            if (read(rdy[0], &r, 1) != 1) _exit(3); // wait until we are the foreground group
            for (int i = 0; i < 3000 && !got_hup; i++)
                usleep(1000);
        ...
        close(ack[1]);
        char a;
        if (read(ack[0], &a, 1) != 1) _exit(5);
        setpgid(w, w);
        tcsetpgrp(s, w); // W is foreground
        char go = 'G';
        if (write(rdy[1], &go, 1) != 1) _exit(6);
        usleep(30000);
