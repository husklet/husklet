...
int main(void) {
    ...
    { char dir[64];
      ...
      f[0].fd=in; f[0].events=POLLIN; f[0].revents=0x5a5a;
      rc=poll(f,1,0); ok("bound_idle_t0",rc,f[0].revents);

      f[0].fd=in; f[0].events=0; f[0].revents=0x5a5a;
      rc=poll(f,1,0); ok("bound_events0",rc,f[0].revents);

      f[0].fd=-1; f[0].events=POLLIN; f[0].revents=0x5a5a;
      f[1].fd=in; f[1].events=POLLIN; f[1].revents=0x5a5a;
      rc=poll(f,2,0); printf("%-28s rc=%d r0=%#x r1=%#x\n","bound_with_negative",rc,f[0].revents,f[1].revents);

      /* every entry bound: the host poll has nothing to observe and can only sleep */
      { long long a=ms_now(); f[0].fd=in; f[0].events=POLLIN; f[0].revents=0;
        rc=poll(f,1,60); long long d=ms_now()-a;
        printf("%-28s rc=%d revents=%#x slept=%d\n","bound_idle_timeout60",rc,f[0].revents,d>=55); }

      /* bound entry beside an ordinary host descriptor, idle then ready */
      f[0].fd=in; f[0].events=POLLIN; f[0].revents=0;
      f[1].fd=p[0]; f[1].events=POLLIN; f[1].revents=0;
      ...
      { int c2=dup(p[0]); close(c2);
        f[0].fd=c2; f[0].events=POLLIN; f[0].revents=0;
        f[1].fd=in; f[1].events=POLLIN; f[1].revents=0;
        rc=poll(f,2,0); printf("%-28s rc=%d r0=%#x r1=%#x\n","bound_with_closed",rc,f[0].revents,f[1].revents); }

      /* the bound object becomes ready */
      { char file[96]; snprintf(file,sizeof file,"%s/f",dir);
        int t=open(file,O_CREAT|O_WRONLY,0600); if(t>=0) close(t);
        f[0].fd=in; f[0].events=POLLIN; f[0].revents=0;
        rc=poll(f,1,1000);
        printf("%-28s rc=%d pollin=%d\n","bound_ready",rc,(f[0].revents&POLLIN)?1:0);
        char buf[4096]; (void)!read(in,buf,sizeof buf); unlink(file); }

      /* select over a bound descriptor: the bound_pselect handler */
      { fd_set r; struct timeval tv={0,0};
        FD_ZERO(&r); FD_SET(in,&r); rc=select(in+1,&r,NULL,NULL,&tv);
        printf("%-28s rc=%d set=%d\n","bound_select_idle",rc,FD_ISSET(in,&r)?1:0);
        long long a=ms_now(); FD_ZERO(&r); FD_SET(in,&r); tv.tv_sec=0; tv.tv_usec=60000;
        rc=select(in+1,&r,NULL,NULL,&tv); long long d=ms_now()-a;
        printf("%-28s rc=%d slept=%d\n","bound_select_timeout60",rc,d>=55); }

      /* EINTR must still end a blocking wait on a bound descriptor promptly */
      { got=0; alarm(1);
        f[0].fd=in; f[0].events=POLLIN; f[0].revents=0;
        long long a=ms_now(); rc=poll(f,1,5000); long long d=ms_now()-a;
        printf("%-28s rc=%d eintr=%d handled=%d prompt=%d\n","bound_blocking_eintr",
               rc,rc<0&&errno==EINTR,got,d<3000);
        alarm(0); }
