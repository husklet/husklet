...
int main(void) {
    ...
    printf("ctty A_pre_enxio=%d A_gsid=%d A_tcgetsid=%d A_devtty=%d B_notleader=%d B_devtty_enxio=%d setsid_eperm=%d "
           "setpgid_leader_fails=%d\n",
           (ra & 1) != 0, (ra & 2) != 0, (ra & 4) != 0, (ra & 8) != 0, (rb & 1) != 0, (rb & 2) != 0, setsid_eperm,
           setpgid_leader);
    close(m);
    return 0;
}
