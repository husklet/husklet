...
int main(void) {
    ...
    struct termios r;
    tcgetattr(s, &r);
    int raw_ok = !(r.c_lflag & (ICANON | ECHO | ISIG | IEXTEN)) && !(r.c_oflag & OPOST) &&
                 !(r.c_iflag & (IXON | ICRNL | BRKINT)) && r.c_cc[VMIN] == 3 && r.c_cc[VTIME] == 7;
