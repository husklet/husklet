...
void _start(void) {
    ...
    long target_fd = call6(SYS_OPENAT, AT_FDCWD, (long)target,
        O_CREAT | O_WRONLY, 0644, 0, 0);
    if (target_fd < 0) finish(25);
    if (call6(SYS_CLOSE, target_fd, 0, 0, 0, 0, 0) != 0) finish(26);
    if (call6(SYS_SYMLINKAT, (long)target, AT_FDCWD,
            (long)link, 0, 0, 0) != 0) finish(27);
    if (call6(SYS_NEWFSTATAT, AT_FDCWD, (long)target,
            (long)stat_buffer, 0, 0, 0) != 0) finish(1);
    if (call6(SYS_NEWFSTATAT, AT_FDCWD, (long)link,
            (long)stat_buffer, AT_SYMLINK_NOFOLLOW, 0, 0) != 0) finish(2);
    if (call6(SYS_STATX, AT_FDCWD, (long)target, 0,
            0x7ff, (long)stat_buffer, 0) != 0) finish(3);
    long count = call6(SYS_READLINKAT, AT_FDCWD, (long)link,
        (long)link_buffer, sizeof(link_buffer), 0, 0);
    ...
    if (call6(SYS_NEWFSTATAT, descriptor, (long)empty,
            (long)stat_buffer, AT_EMPTY_PATH, 0, 0) != 0) finish(6);
    long directory = call6(SYS_OPENAT, AT_FDCWD, (long)root,
        O_DIRECTORY, 0, 0, 0);
    if (directory < 0) finish(7);
    if (call6(SYS_MKDIRAT, directory, (long)work, 0755, 0, 0, 0) != 0) finish(8);
    long file = call6(SYS_OPENAT, directory, (long)a,
        O_CREAT | O_WRONLY, 0644, 0, 0);
    if (file < 0) finish(9);
    if (call6(SYS_CLOSE, file, 0, 0, 0, 0, 0) != 0) finish(10);
    if (call6(SYS_RENAMEAT, directory, (long)a,
            directory, (long)b, 0, 0) != 0) finish(11);
    if (call6(SYS_LINKAT, directory, (long)b,
            directory, (long)c, 0, 0) != 0) finish(12);
    if (call6(SYS_UNLINKAT, directory, (long)b, 0, 0, 0, 0) != 0) finish(13);
    if (call6(SYS_UNLINKAT, directory, (long)c, 0, 0, 0, 0) != 0) finish(14);
    if (call6(SYS_SYMLINKAT, (long)relative_target, directory,
            (long)work_link, 0, 0, 0) != 0) finish(15);
    if (call6(SYS_NEWFSTATAT, directory, (long)work_link,
    ...
    if (call6(SYS_NEWFSTATAT, AT_FDCWD, (long)forked,
            (long)stat_buffer, 0, 0, 0) != 0) finish(23);
