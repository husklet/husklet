...
struct report {
    ...
    unsigned closed_denied, chmod_denied, chown_denied, times_denied;
    // Allowances that must survive: the open directory and this uid's own file.
    unsigned open_allowed, own_chmod, own_file_uid;
...
int main(void) {
    ...
    printf(
        "dac-denial closed=%04o:%u denied=%u:%u chmod=%u chown=%u times=%u open=%u own-chmod=%u "
        "distinct=%u\n",
        first.closed_mode,
        first.closed_uid,
        first.closed_denied,
