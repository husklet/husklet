...
int main(void) {
    ...
    int ok = (unsigned long)root.f_type == OVERLAYFS && root.f_blocks > 0 && consistent(&root) &&
             (unsigned long)proc.f_type == PROCFS && proc.f_blocks == 0 && proc.f_files == 0 &&
             (unsigned long)sys.f_type == SYSFS && sys.f_blocks == 0 && (unsigned long)dev.f_type == TMPFS &&
             consistent(&dev) &&
