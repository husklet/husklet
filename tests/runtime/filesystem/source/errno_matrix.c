...
static char *P(char *buf, const char *rel) {
    snprintf(buf, 512, "%s/%s", B, rel);
    return buf;
}

// returns 1 iff op failed with the expected errno.
static int fail_is(int rc, int want) {
    return rc != 0 && errno == want;
}
...
int main(void) {
    ...
    mkdir(P(p1, "ne"), 0755);
    close(open(P(p2, "ne/x"), O_CREAT | O_WRONLY, 0644));
    ...
    int r2_noreplace =
        fail_is(syscall(SYS_renameat2, AT_FDCWD, P(p1, "rn_a"), AT_FDCWD, P(p2, "rn_b"), RENAME_NOREPLACE), EEXIST);
    int r2_bothflags = fail_is(
        syscall(SYS_renameat2, AT_FDCWD, P(p1, "rn_a"), AT_FDCWD, P(p2, "rn_b"), RENAME_NOREPLACE | RENAME_EXCHANGE),
        EINVAL);
    int r2_exch_miss =
        fail_is(syscall(SYS_renameat2, AT_FDCWD, P(p1, "rn_a"), AT_FDCWD, P(p2, "rn_gone"), RENAME_EXCHANGE), ENOENT);
    ...
    char longtgt[5000];
    memset(longtgt, 'a', sizeof longtgt - 1);
    ...
    printf("errno-matrix rmdir=%d rename=%d renameat2=%d mkdir=%d linksym=%d trunc=%d access=%d\n",
           ...
           ren_dir_over_nonempty && ren_file_over_dir && ren_dir_over_file && ren_missing_src && ren_self_noop &&
               ren_subdir_of_self && ren_trailslash,
           r2_noreplace && r2_bothflags && r2_exch_miss, mkdir_existing && mkdir_under_file && mkdir_under_missing,
           symlink_over_exist && symlink_toolong && link_missing_src && link_dir_eperm && link_over_exist,
           trunc_dir_eisdir && trunc_missing && trunc_negative && chmod_missing && chown_missing, access_xok);
    return 0;
}
