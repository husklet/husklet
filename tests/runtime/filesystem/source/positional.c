...
void _start(void) {
    ...
    if (alias < 0) finish(3);
    if (call6(SYS_PREAD64, alias, (long)bytes, 3, 1, 0, 0) != 3 || !same(bytes, "bcd", 3)) finish(4);
    ...
    reads[1].base = second;
    reads[1].length = 2;
    if (call6(SYS_PREADV, alias, (long)reads, 2, 1, 0, 0) != 4 || !same(first, "bX", 2) || !same(second, "Ye", 2))
        finish(22);
    if (call6(SYS_LSEEK, fd, 0, SEEK_CUR, 0, 0, 0) != 6) finish(23);
    static const char one[] = "12", two[] = "34";
    volatile struct vector writes[2];
    writes[0].base = (void *)one;
    writes[0].length = 2;
    writes[1].base = (void *)two;
    writes[1].length = 2;
    ...
    short_read[0].base = tail;
    short_read[0].length = 1;
    short_read[1].base = tail + 1;
    short_read[1].length = 3;
    if (call6(SYS_PREADV, fd, (long)short_read, 2, 4, 0, 0) != 2 || !same(tail, "4f", 2)) finish(35);
    ...
    if (call6(SYS_READ, fd, (long)bytes, 7, 0, 0, 0) != 7 || !same(bytes, "a1234fZ", 7)) finish(17);
    long crossing_address = call6(SYS_MMAP, 0, 8192, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (crossing_address < 0) finish(40);
    for (int i = 0; i < 4096; ++i)
        ((unsigned char *)crossing_address)[i] = 'R';
    ...
    long read_crossing_address = call6(SYS_MMAP, 0, 8192, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ...
    if (call6(SYS_PREADV, fd, (long)&read_crossing, 1, 8192, 0, 0) != 4096 ||
        ((unsigned char *)read_crossing_address)[0] != 'R' || ((unsigned char *)read_crossing_address)[4095] != 'R')
        finish(46);
