...
static void *w(void *arg) {
    ...
    if (pthread_setname_np(pthread_self(), want) == 0 && pthread_getname_np(pthread_self(), got, sizeof got) == 0 &&
        strcmp(want, got) == 0)
...
int main(void) {
    ...
    char mgot[16] = {0};
    int main_ok = pthread_getname_np(pthread_self(), mgot, sizeof mgot) == 0 && strcmp(mgot, "hlmain") == 0;
