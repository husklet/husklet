...
static void emit_trace(uint32_t *code) {
    ...
    static const uint32_t helper[] = {
        0x4a010003u, 0x7100007fu, 0x4a807c02u, 0x4a817c23u, 0x4b807c42u, 0x4b817c63u, 0x1ac30840u, 0x1b038800u,
        0x7a404804u, 0x4b030002u, 0x1a820000u, 0x4a010002u, 0x7100005fu, 0x5a805400u, 0xd65f03c0u,
    };
    for (unsigned i = 0; i < sizeof helper / sizeof helper[0]; i++)
        code[HELPER + i] = helper[i];
...
static int run_trace(uint32_t *code) {
    ...
    __asm__ volatile("blr %1\n\tmov %w0, w0"
                     : "=r"(result)
                     : "r"(code)
                     : "cc", "memory", "x0", "x1", "x2", "x3", "x4", "x19", "x25", "x26", "x27", "x28", "x30");
    return result;
}
...
int main(void) {
    ...
    for (unsigned i = 0; i < 256; i++) {
        ...
        uint32_t *code =
            mmap((void *)candidate, size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
