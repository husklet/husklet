...
int main(int c, char **v) {
    ...
    char *err = 0;
    sqlite3_exec(db,
                 "PRAGMA journal_mode=WAL; CREATE TABLE IF NOT EXISTS t(id INTEGER PRIMARY KEY, v INTEGER, s TEXT);", 0,
                 0, &err);
    ...
    for (int i = 1; i <= 5000; i++) {
        char s[32];
        snprintf(s, sizeof s, "row-%d", i * 7 % 100);
        sqlite3_bind_int(st, 1, i * i % 1000003);
    ...
    sqlite3_stmt *q;
    sqlite3_prepare_v2(db, "SELECT COUNT(*), SUM(v), MIN(s), MAX(s) FROM t", -1, &q, 0);
    sqlite3_step(q);
    printf("SQLITE rows=%d sum=%lld min=%s max=%s\n", sqlite3_column_int(q, 0), (long long)sqlite3_column_int64(q, 1),
           sqlite3_column_text(q, 2), sqlite3_column_text(q, 3));
    sqlite3_finalize(q);
    sqlite3_close(db);
    return 0;
}
