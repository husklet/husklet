...
static void success(void) {
    ...
    if (call6(GUEST_WRITE, 1, (long)result, sizeof(result) - 1, 0, 0, 0) != sizeof(result) - 1)
        finish(23);
    finish(0);
}
...
static void first_handler(int signal, void *information, unsigned char *context) {
    ...
    (void)information;
    if (signal != 10 || call6(GUEST_SIGALTSTACK, 0, (long)&current, 0, 0, 0, 0) != 0 || current.flags != 1)
        finish(20);
...
static void second(void) {
    ...
    if (call6(GUEST_TGKILL, pid, tid, 10, 0, 0, 0) != 0)
        finish(21);
    finish(22);
}
...
void _start(void) {
    ...
    if (call6(GUEST_SIGALTSTACK, (long)&alternate, 0, 0, 0, 0, 0) != 0)
        finish(11);
    install((void (*)(int))first_handler);

    uint64_t selected = 1ULL << 9;
    uint64_t old = 0;
    if (call6(GUEST_PROCMASK, 0, (long)&selected, (long)&old, 8, 0, 0) != 0 || old != 0)
        finish(12);
    long tid = call6(GUEST_GETTID, 0, 0, 0, 0, 0, 0);
    if (call6(GUEST_TKILL, tid, 10, 0, 0, 0, 0) != 0)
        finish(13);
    uint64_t pending = 0;
    if (call6(GUEST_PENDING, (long)&pending, 8, 0, 0, 0, 0) != 0 || (pending & selected) == 0)
        finish(14);
    if (call6(GUEST_PROCMASK, 1, (long)&selected, 0, 8, 0, 0) != 0)
        finish(15);
    finish(16);
}
