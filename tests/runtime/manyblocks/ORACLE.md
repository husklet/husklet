# manyblocks soak oracle

This deterministic prolonged workload was migrated without behavioral changes from `tests/runtime/legacy/oracle/tests/soak/manyblocks.c`. Its stdout contract is the exact retained golden in `golden/stdout.txt`; exit status zero and byte-for-byte stdout equality are both required. The legacy bounded profile allowed 240 seconds per attempt and one repetition, terminating the complete process group on timeout. The optional typed soak policy preserves those bounds in the ordinary `tests/runtime/<case>/test.yaml` envelope rather than creating a separate soak tree.
