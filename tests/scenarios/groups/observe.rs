...
use crate::support::{raw_http, wait_for_path};
use hl_client::Client;
use hl_container::{
    ContainerSpec, ContainerState, Containers, EndpointSpec, Isolation, Mount, NetworkSpec, Process, Publication,
    Sandbox, Stream, Subnet,
};
...
impl<'a> Observe<'a> {
    ...
    async fn inspect_state(&self) -> Result<(), Error> {
        ...
        self.containers.stop("observe-state", Duration::ZERO).await?;
    ...
    async fn inspect_env(&self) -> Result<(), Error> {
        self.containers
            .create(self.spec("observe-env", Process::new("/bin/true").env("MARKERENV", "zz9")))
            .await?;
        let value = self
            .containers
            .inspect("observe-env")
    ...
    async fn inspect_command(&self) -> Result<(), Error> {
        self.containers
            ...
            .await?;
        let process = self.containers.inspect("observe-command").await?.spec.process;
    ...
    async fn ps_running(&self) -> Result<(), Error> {
        ...
        self.containers.stop("observe-ps-running", Duration::ZERO).await?;
        if running {
            Ok(())
        } else {
