...
impl Images {
    ...
    async fn remove(&self, client: &Client) -> Result<(), Error> {
        ...
        if client.images().inspect("scenario/imagescmd:remove").await.is_err() {
            Ok(())
        } else {
    ...
    async fn history(&self, socket: &Path) -> Result<(), Error> {
        ...
        if response.starts_with("HTTP/1.1 200") {
            ...
            Err(format!("history response: {}", response.lines().next().unwrap_or("empty")).into())
        }
    }
