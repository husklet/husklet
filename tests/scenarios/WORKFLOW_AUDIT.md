...
| `docker.rs` | `hl-client`, `hl-daemon`, and `hl-container` public-contract tests | `DOCKER_WORKFLOW_AUDIT.md` closes the redundant sweep. Only successful root-filesystem import and requested-tag discovery remains; `docker_container.rs` and the redundant `docker` registration were deleted. |
