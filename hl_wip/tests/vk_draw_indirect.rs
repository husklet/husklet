//! VK ADVANCED-COMMAND DEMO — vk_draw_indirect: a real C Vulkan program sources every draw parameter
//! (vertexCount / instanceCount / firstVertex / firstInstance) from a VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT
//! buffer and issues ONE `vkCmdDrawIndirect(buf, 0, drawCount=2, stride=16)`. The two `VkDrawIndirectCommand`
//! structs select a RED LEFT quad (firstVertex 0) and a BLUE RIGHT quad (firstVertex 6) out of a shared
//! 12-vertex table — so the frame is PIXEL-IDENTICAL to the equivalent DIRECT `vkCmdDraw(6,1,0,0);
//! vkCmdDraw(6,1,6,0)` of the vk_multidraw demo. This is the regression guard for the shim reading the
//! indirect argument buffer and lowering each struct to the same direct `Enc::Draw` (before the fix the
//! indirect draw was validated-but-dropped → a BLACK frame). Asserts the exact two-quad raster and writes
//! /tmp/hl-demo/vk_draw_indirect.png.
...
#[test]
fn real_vulkan_draw_indirect_sources_params_from_buffer_on_lavapipe() {
    ...
    let out_dir = std::env::temp_dir().join(format!("hl-demo-vkdrawindirect-{}", std::process::id()));
    std::fs::create_dir_all(&out_dir).unwrap();
    let bin = out_dir.join("vk_draw_indirect");
    ...
    let vs_path = out_dir.join("vkdrawindirect.vert.spv");
    let fs_path = out_dir.join("vkdrawindirect.frag.spv");
    write_spv(&vs_path, &vs_spirv);
    write_spv(&fs_path, &fs_spirv);

    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/vk_draw_indirect.c"))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
        .arg(&bin)
    ...
    let exec = WgpuExecutorServer::start("vkdrawindirect");
    ...
    let run = Command::new(&bin)
        ...
        .output()
        .expect("spawn vk_draw_indirect");
    let stdout = String::from_utf8_lossy(&run.stdout);
    let stderr = String::from_utf8_lossy(&run.stderr);
    eprintln!("--- vk_draw_indirect stdout ---\n{stdout}\n--- stderr ---\n{stderr}");

    assert!(run.status.success(), "guest exited non-zero (code {:?})", run.status.code());
    assert!(stdout.contains("VK_DRAW_INDIRECT_OK"), "guest drove the indirect draw");
    ...
    write_png(&demo_png_dir().join("vk_draw_indirect.png"), W, H, px);
    ...
    // Exactly the vk_multidraw direct-draw raster: firstVertex sourced from the buffer selects each quad.
    assert!(is(16, 32, 255, 0, 0), "indirect cmd0 (firstVertex 0) LEFT region must be RED, got {:?}", at(16, 32));
    assert!(is(48, 32, 0, 0, 255), "indirect cmd1 (firstVertex 6) RIGHT region must be BLUE, got {:?}", at(48, 32));
    ...
    // Both buffer-sourced draws produced coverage — neither the indirect params nor the draw was dropped.
    ...
    assert!(red > 300 && blue > 300, "both indirect draws must cover real area (red={red}, blue={blue})");
