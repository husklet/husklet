...
fn real_gles3_ubo_mvp_places_quad_at_exact_rect_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/gl_ubo_transform.c"))
    ...
    assert!(stdout.contains("GL_UBOXFORM_OK"), "guest asserted the quad at the exact rect over the socket");
    ...
    write_png(&demo_png_dir().join("gl_ubo_transform.png"), W, H, &bgra_to_rgba(px));
