...
//!      counter (hl_wip-cuda/src/model/context.rs `next_buffer = 1`; hl_wip-vulkan/src/model/device.rs
//!      `alloc_ir` from 0→1), and the runtime keys resources per-kind in ONE shared table, rejecting a second
//!      live id of the same kind as `DuplicateId` (hl_wip-gpu/src/runtime/model/resources.rs:43). CUDA and
...
fn cuda_vulkan_gl_coexist_on_one_engine_session() {
    ...
    engine
        .add(Cuda::new(CudaSpec::new(hl_cuda::Arch::X86_64, HOST_SOCK).stage_root("/opt/hlroot")))
        .add(Vulkan::new(VulkanSpec::new(hl_vulkan::Arch::X86_64, HOST_SOCK).stage_root("/opt/hlroot")))
        .add(Gl::new(GlSpec::new(hl_gl::Arch::X86_64, HOST_SOCK).stage_root("/opt/hlroot").surface_size(64, 64)));
    assert_eq!(engine.len(), 3, "all three drivers attached to the one engine");
    assert_eq!(engine.names(), vec!["cuda", "vulkan", "gl"], "drivers composed, ordered");
    ...
    assert!(adapter.contains("llvmpipe") || adapter.contains("lavapipe"), "software Vulkan device required, got {:?}", exec.adapter_name());
    exec.set_kernel_compiler(|desc: &KernelDescriptor| hl_cuda::adapter::ptx::compile(&desc.ptx, &desc.entry, desc.block));
    ...
    assert_eq!(cuda_got, want, "CUDA vecadd on the shared lavapipe executor, all {n} elements exact");
    let cuda_buffers_live = sink.resources().buffers.len();
    assert!(cuda_buffers_live >= 3, "CUDA's a/b/c buffers are live in the shared session");
    ...
    gl.surf = GlSurface { have: true, width: GW, height: GH };
    ...
    sink.submit(&f.cmds).expect("GL frame accepted + rasterized on the shared runtime + executor");
    gl.reset_frame();

    // GL default surface is Bgra8Unorm → blue clear reads back as BGRA [255, 0, 0, 255].
    let gl_px = sink.executor().read_texture(sink.resources(), texture).expect("read GL target");
    let gl_center = {
        let o = ((GH / 2 * GW + GW / 2) * 4) as usize;
        [gl_px[o], gl_px[o + 1], gl_px[o + 2], gl_px[o + 3]]
    };
    assert_eq!(gl_center, [255, 0, 0, 255], "GL target cleared to blue on the shared executor (BGRA)");
    assert!(gl_px.chunks_exact(4).all(|t| t == [255, 0, 0, 255]), "every GL pixel is the clear color");
    ...
    // -------------------------------------------------------------------------------------------
    assert_eq!(sink.resources().buffers.len(), cuda_buffers_live, "GL frame added no buffers / freed none of CUDA's");
    assert!(sink.resources().textures.get(texture).is_ok(), "GL render target resident in the shared session");
    let cuda_after_gl = cuda_readback(&mut sink, &cuda, dc, n * 4);
    assert_eq!(cuda_after_gl, want, "CUDA vecadd result SURVIVES the GL draw — GL did not corrupt CUDA's buffers");
    ...
    assert_eq!(sink.resources().buffers.len(), 0, "CUDA buffers freed from the shared session");
    ...
    assert_eq!(vk_bytes, vec![0u8; 256], "Vulkan buffer read back off the shared lavapipe executor (zero-init)");
    assert_eq!(sink.resources().buffers.len(), 1, "the Vulkan buffer is live in the shared session");
