...
fn real_gles2_fixed_function_blend_composites_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/gl_blend_pipeline.c"))
    ...
    assert!(stdout.contains("GL_BLEND_PIPELINE_OK"), "guest asserted the fixed-function composite over the socket");
    ...
    write_png(&demo_png_dir().join("gl_blend_pipeline.png"), W, H, &bgra_to_rgba(px));
    ...
    let composite = px
        ...
        .count();
    assert!(composite > (W * H / 8) as usize, "the composited color must cover the right third, only {composite} matched");
