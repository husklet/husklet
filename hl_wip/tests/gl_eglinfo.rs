...
fn real_eglinfo_reports_our_egl_identity() {
    ...
    assert!(stdout.contains("EGL vendor string: hl-gl"), "eglinfo saw our EGL vendor 'hl-gl'");
    ...
    assert!(stdout.contains("hl-gl-metal"), "eglinfo saw our renderer string");
    // It successfully enumerated at least one EGLConfig from our driver (the config table header).
    assert!(stdout.contains("Configurations:"), "eglinfo printed our EGLConfig table");
}
