//! VK RENDER-CORRECTNESS DEMO #3 — vk_instanced: 4 instances placed + colored by gl_InstanceIndex form an
//! exact grid in ONE vkCmdDraw(6,4,..) on lavapipe. Instance k lands at pixel column 8/24/40/56 (row 32) in
//! RED/GREEN/BLUE/YELLOW over a BLACK clear. Asserts each cell and writes /tmp/hl-demo/vk_instanced.png.
...
#[test]
fn real_vulkan_instanced_grid_places_each_instance_on_lavapipe() {
    ...
    let out_dir = std::env::temp_dir().join(format!("hl-demo-vkinstanced-{}", std::process::id()));
    std::fs::create_dir_all(&out_dir).unwrap();
    let bin = out_dir.join("vk_instanced");
    ...
    let vs_spirv = wgsl_to_spirv(
        ...
        fn vs_main(@builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> VOut {
            ...
            var C = array<vec4<f32>, 4>(
                vec4<f32>(1.0,0.0,0.0,1.0), vec4<f32>(0.0,1.0,0.0,1.0),
                vec4<f32>(0.0,0.0,1.0,1.0), vec4<f32>(1.0,1.0,0.0,1.0));
            let cx = -0.75 + 0.5 * f32(ii);
            var o: VOut;
            o.pos = vec4<f32>(Q[vi].x * 0.10 + cx, Q[vi].y * 0.10, 0.0, 1.0);
            o.color = C[ii];
            return o;
        }
    "#,
    ...
    let vs_path = out_dir.join("vkinstanced.vert.spv");
    let fs_path = out_dir.join("vkinstanced.frag.spv");
    write_spv(&vs_path, &vs_spirv);
    write_spv(&fs_path, &fs_spirv);

    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/vk_instanced.c"))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
        .arg(&bin)
    ...
    let exec = WgpuExecutorServer::start("vkinstanced");
    ...
    let run = Command::new(&bin)
        ...
        .output()
        .expect("spawn vk_instanced");
    let stdout = String::from_utf8_lossy(&run.stdout);
    let stderr = String::from_utf8_lossy(&run.stderr);
    eprintln!("--- vk_instanced stdout ---\n{stdout}\n--- stderr ---\n{stderr}");

    assert!(run.status.success(), "guest exited non-zero (code {:?})", run.status.code());
    assert!(stdout.contains("VK_INSTANCED_OK"), "guest drove the draw");
    ...
    write_png(&demo_png_dir().join("vk_instanced.png"), W, H, px);
    ...
    assert!(is(8, 32, 255, 0, 0), "instance 0 cell must be RED, got {:?}", at(8, 32));
    assert!(is(24, 32, 0, 255, 0), "instance 1 cell must be GREEN, got {:?}", at(24, 32));
    assert!(is(40, 32, 0, 0, 255), "instance 2 cell must be BLUE, got {:?}", at(40, 32));
    assert!(is(56, 32, 255, 255, 0), "instance 3 cell must be YELLOW, got {:?}", at(56, 32));
    // The four cells sit on a BLACK clear; the space between them stays clear.
    let (r, g, b, _a) = at(16, 32);
    assert!(near(r, 0) && near(g, 0) && near(b, 0), "gap between instances 0/1 must be BLACK, got {:?}", at(16, 32));
