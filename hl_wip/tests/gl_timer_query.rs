...
//! to the `EXT_disjoint_timer_query` extension), and `hl_wip-gl` advertises GLES 3.0 core with an EMPTY
//! extension list, so `glBeginQuery(GL_TIME_ELAPSED_EXT, …)` is an invalid target — see
//! `hl_wip-gl/src/service/es3.rs::is_query_target`. This demo documents that gap by asserting the honest
...
fn real_gles3_timer_query_returns_honest_invalid_enum() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/gl_timer_query.c"))
