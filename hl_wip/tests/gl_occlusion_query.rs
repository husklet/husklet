...
//! `0`; now `hl_wip-gl` accumulates each draw's scissor-clipped footprint into the open query (see
...
fn real_gles3_occlusion_query_reflects_scissor_coverage_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/gl_occlusion_query.c"))
    ...
    assert_ne!(q1, 0, "the un-scissored (visible) query must report samples passed");
    assert_eq!(q2, 0, "the zero-area-scissor query must report NO samples passed");
    ...
    write_png(&demo_png_dir().join("gl_occlusion_query.png"), W, H, &bgra_to_rgba(px));
    ...
    assert_eq!(red, 0, "the fully-scissored RED draw must NOT reach the framebuffer, found {red} red texels");
    let green = px.chunks_exact(4).filter(|t| near(t[2], 0) && near(t[1], 255) && near(t[0], 0)).count();
    assert!(green > (W * H / 2) as usize, "the visible green quad must fill the frame, got {green} texels");
