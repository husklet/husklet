...
use hl_gpu::protocol::model::enums::{buffer_usage, texture_usage, LoadOp, TextureDim, TextureFormat};
use hl_gpu::{
...
fn frame_command_buffer(tex: u32, dim: u32, draws: usize) -> CommandBuffer {
    ...
    encoder.push(Enc::BeginRenderPass {
        color: vec![ColorAttachment { texture: tex, load: LoadOp::Clear, clear: [0.02, 0.02, 0.05, 1.0], store: true }],
        depth: None,
    });
    let rect = 16u32;
    ...
    for i in 0..draws {
        ...
        encoder.push(Enc::ClearRect { texture: tex, x, y, w: rect, h: rect, color: [c, 1.0 - c, 0.5, 1.0] });
    }
...
fn representative_stream(dim: u32, draws: usize) -> Vec<Cmd> {
    ...
    vec![
        ...
        Cmd::CreateBuffer(BUF_A, BufferDesc { size: bytes, usage: buffer_usage::COPY_SRC | buffer_usage::COPY_DST, label: String::new() }),
        Cmd::CreateBuffer(BUF_B, BufferDesc { size: bytes, usage: buffer_usage::COPY_SRC | buffer_usage::COPY_DST, label: String::new() }),
        Cmd::WriteBuffer { id: BUF_A, offset: 0, data: vec![0xABu8; 4096] },
        Cmd::Submit(CommandBuffer {
            encoder: vec![Enc::CopyBufferToBuffer { src: BUF_A, src_offset: 0, dst: BUF_B, dst_offset: 0, size: 4096 }],
            signal: None,
        }),
        Cmd::Submit(frame_command_buffer(TEX, dim, draws)),
        Cmd::Present { surface: SURF, texture: TEX },
    ]
}
...
fn executor_frame_time() {
    ...
    sink.submit(&[Cmd::CreateTexture(1, target_desc(FRAME_DIM))]).expect("create target");

    let frame = vec![Cmd::Submit(frame_command_buffer(1, FRAME_DIM, DRAWS_PER_FRAME))];
    ...
    assert!(per_frame_ms <= ceil_ms, "frame time {:.3}ms > {:.3}ms ceiling", per_frame_ms, ceil_ms);
}
...
fn readback_latency() {
    let mut sink = InProcessCommandSink::new(CpuExecutor::new());
    sink.submit(&[Cmd::CreateTexture(1, target_desc(FRAME_DIM))]).expect("create target");
    sink.submit(&[Cmd::Submit(frame_command_buffer(1, FRAME_DIM, DRAWS_PER_FRAME))]).expect("render");

    let frame_bytes = (FRAME_DIM * FRAME_DIM * 4) as usize;
    let mut out = vec![0u8; frame_bytes];

    // Warm up.
    sink.executor().read_texture(sink.resources(), TextureId(1), &mut out).expect("warmup readback");

    let t0 = Instant::now();
    let mut checksum = 0u64;
    for _ in 0..READBACK_ITERS {
        sink.executor().read_texture(sink.resources(), TextureId(1), &mut out).expect("readback");
    ...
    assert!(per_readback_ms <= ceil_ms, "readback {:.4}ms > {:.4}ms ceiling", per_readback_ms, ceil_ms);
}
...
fn large_frame_transport() {
    ...
    sink.submit(&[
        Cmd::CreateBuffer(1, BufferDesc { size: buf_size, usage: buffer_usage::COPY_SRC | buffer_usage::COPY_DST, label: String::new() }),
        Cmd::CreateBuffer(2, BufferDesc { size: buf_size, usage: buffer_usage::COPY_SRC | buffer_usage::COPY_DST, label: String::new() }),
    ])
    ...
    let frame = vec![
        Cmd::WriteBuffer { id: 1, offset: 0, data: payload.clone() },
        Cmd::Submit(CommandBuffer {
            encoder: vec![Enc::CopyBufferToBuffer { src: 1, src_offset: 0, dst: 2, dst_offset: 0, size: buf_size }],
            signal: None,
        }),
    ];
    ...
    let back = sink.read_buffer(BufferId(2), 0, payload_len).expect("read back dst");
    ...
    assert!(mb_per_sec >= floor_mbps, "transport {:.1} MB/s < {:.1} MB/s floor", mb_per_sec, floor_mbps);
}
