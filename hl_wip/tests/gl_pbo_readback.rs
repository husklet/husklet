...
//! buffer's storage at the byte offset (see `hl_wip-gl/shim/egl/src/driver.rs::glReadPixels`), NOT to the
...
fn real_gles3_pbo_readback_maps_exact_pixels_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/gl_pbo_readback.c"))
    ...
    write_png(&demo_png_dir().join("gl_pbo_readback.png"), W, H, &bgra_to_rgba(px));
    ...
    assert_eq!(green, (W * H) as usize, "the whole frame must be GREEN, got {green}/{}", W * H);
