...
//! See `hl_wip-gl/src/model/es3.rs` (TransformFeedbacks) for the gap note. Needs no host executor.
...
fn real_gles3_transform_feedback_reflects_but_does_not_fake_capture() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/gl_transform_feedback.c"))
    ...
    assert!(stdout.contains("GL_TF_VARYING: \"vValue\""), "glGetTransformFeedbackVarying must reflect the name");
