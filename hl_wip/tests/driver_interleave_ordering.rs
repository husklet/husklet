...
fn fresh_sink() -> Option<InProcessCommandSink<WgpuExecutor>> {
    ...
    assert!(name.contains("llvmpipe") || name.contains("lavapipe"), "software Vulkan device required, got {:?}", exec.adapter_name());
...
fn gl_clear_frame(
    ...
    sink.submit(&f.cmds).expect("GL frame rasterized on the shared runtime + executor");
    gl.reset_frame();
    let px = sink.executor().read_texture(sink.resources(), texture).expect("read GL target");
    (texture, px)
}
...
fn vk_buffer_op(sink: &mut InProcessCommandSink<WgpuExecutor>, pattern: &[u8]) -> u32 {
    ...
    let handle = vk_create::create_buffer(&mut dev, sink, vk_buffer_usage::VERTEX_BUFFER, pattern.len() as u64)
    ...
    sink.submit(&[Cmd::WriteBuffer { id: ir_id, offset: 0, data: pattern.to_vec() }])
...
fn gl_and_vulkan_interleave_is_exact_and_order_independent() {
    ...
    let after_vk = sink.executor().read_texture(sink.resources(), tex).expect("re-read GL target");
    assert_eq!(center(&after_vk), GREEN_BGRA, "GL target survives the interleaved VK op");
    assert!(after_vk.chunks_exact(4).all(|t| t == GREEN_BGRA), "every GL pixel is still green after the VK op");
    ...
    assert_eq!(tex2, tex, "the default GL surface re-presents its one persistent target across frames");
    assert_eq!(center(&red_px), RED_BGRA, "GL renders a new red frame after the VK op interleave");
    assert!(red_px.chunks_exact(4).all(|t| t == RED_BGRA), "every GL pixel is now red");

    // The VK buffer reads back its exact pattern — the GL draws bracketing it corrupted nothing of Vulkan's.
    let vk_bytes = CommandSink::read_buffer(&mut sink, BufferId(vk_id), 0, VK_PATTERN.len()).unwrap();
    assert_eq!(vk_bytes, VK_PATTERN, "VK buffer holds its exact pattern through the interleaved GL draws");
    ...
    let vk_bytes2 = CommandSink::read_buffer(&mut sink2, BufferId(vk_id2), 0, VK_PATTERN.len()).unwrap();
    assert_eq!(vk_bytes2, vk_bytes, "VK result is identical regardless of submit order (VK-first vs GL-first)");
    assert_eq!(a2_px, green_px, "GL target is byte-identical regardless of submit order — order-independent");
