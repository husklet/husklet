...
//! surface, presents through the ENTIRE hl_wip stack, and this test asserts the composited output pixels
...
//!
//!   csrc/gl_present_pattern.c  (real GLES2, our staged libEGL/libGLESv2 via LD_LIBRARY_PATH)
//!     -> a STOCK two-int native window `{64,64}` at `eglCreateWindowSurface` -> shim `wl_surface == 0`
...
fn known_pattern_composites_exact_through_the_full_stack() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/gl_present_pattern.c"))
    ...
    let runtime_dir = std::env::temp_dir().join(format!("hl-present-exact-xdg-{}", std::process::id()));
    ...
    for (x, y, want, label) in checks {
        let got = frame.pixel(x, y).unwrap_or_else(|| panic!("pixel ({x},{y}) out of bounds"));
    ...
    assert!(png.exists(), "a real PNG of the composited frame was written at {png:?}");
    eprintln!(
