...
fn run_guest(
    ...
    let exec = WgpuExecutorServer::start(if blend_on { "vkblend-on" } else { "vkblend-off" });
    ...
    assert!(stdout.contains("VK_BLEND_OK"), "guest drove the blended draws");
...
fn real_vulkan_alpha_blend_composites_exactly_and_disabled_overwrites() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/vk_blend.c"))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
        .arg(&bin)
    ...
    assert_eq!(ov_on, composite, "overlap must be the exact composite {composite:?}, got {ov_on:?}");
    for (x, y) in [(8, 8), (56, 8), (8, 56), (56, 56), (32, 6), (6, 32)] {
        let b = at(&blended, x, y);
        assert_eq!(b, bg_opaque, "border ({x},{y}) must be the opaque background {bg_opaque:?}, got {b:?}");
    }

    // --- blend OFF (regression control): overlap is the RAW foreground (overwrite) --------------
    let ov_off = at(&overwritten, 32, 32);
    assert!(eq(ov_off, fg_raw), "blend-off overlap must be the raw foreground {fg_raw:?}, got {ov_off:?}");
    let border_off = at(&overwritten, 8, 8);
    assert!(eq(border_off, bg_opaque), "blend-off border must still be the background, got {border_off:?}");

    // --- the two runs' overlap pixels MUST differ (composite != overwrite) ----------------------
    assert_ne!(ov_on, ov_off, "blend-on (composite) and blend-off (overwrite) overlaps must differ");
    ...
    let composited = blended
        .chunks_exact(4)
        .filter(|t| near(t[0], composite[0]) && near(t[1], composite[1]) && near(t[2], composite[2]) && near(t[3], composite[3]))
        .count();
    assert!(
