...
//! `csrc/vk_ubo.c` builds a descriptor-set layout (one `UNIFORM_BUFFER` binding at VERTEX stage), a
...
fn real_vulkan_ubo_transform_moves_geometry_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/vk_ubo.c"))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
        .arg(&bin)
    ...
    for y in 0..H {
        for x in 0..W {
            if is_green(texel(x, y)) {
                if x < W / 2 { left_green += 1; } else { right_green += 1; }
            }
        }
    }
    eprintln!(
