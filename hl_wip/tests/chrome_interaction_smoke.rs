...
fn chrome_interaction_smoke_real_process() {
    ...
    let chromium_bin = match which_chromium() {
        Some(p) => p,
        None => {
            eprintln!("no chromium binary (set HL_CHROME_BIN) — skipping the Chrome interaction smoke.");
            return;
        }
    };
    ...
    for lib in ["libEGL.so.1", "libGLESv2.so.2", "libwayland-egl.so.1"] {
        if !gl_dir.join(lib).exists() {
            eprintln!("staged {lib} missing at {gl_dir:?} — skipping the Chrome interaction smoke.");
            return;
        }
    }
    ...
    let mut child = cmd.spawn().unwrap_or_else(|e| panic!("failed to spawn {}: {e}", run_bin.display()));
    ...
    while start.elapsed() < APP_DEADLINE {
        ...
        input_tx.send(InputCommand::PointerMotion { x: 400.0, y: 300.0 }).expect("seat accepted motion");
        input_tx.send(InputCommand::FocusTopmostKeyboard).expect("seat accepted focus");
        input_tx.send(InputCommand::PointerButton { button: BTN_LEFT, pressed: true }).expect("seat accepted btn down");
        input_tx.send(InputCommand::PointerButton { button: BTN_LEFT, pressed: false }).expect("seat accepted btn up");
        input_tx.send(InputCommand::Key { keycode: KEY_A, pressed: true }).expect("seat accepted key down");
        input_tx.send(InputCommand::Key { keycode: KEY_A, pressed: false }).expect("seat accepted key up");
        injected += 6;
        // Compositor liveness: it published its socket at startup and the thread is not finished.
        assert!(!compositor.is_finished(), "compositor thread died while injecting input during a Chrome session");
    ...
    assert!(injected > 0, "at least one input burst was injected during the Chrome session");
    eprintln!(
