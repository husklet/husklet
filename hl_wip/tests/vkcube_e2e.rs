...
//! WSI build) end to end through the ENTIRE hl_wip stack and capture its rendered window off our compositor.
//!
...
fn vkcube_composites_through_the_full_stack() {
    ...
    let runtime_dir = std::env::temp_dir().join(format!("hl-wip-vkcube-xdg-{}", std::process::id()));
    ...
    if frames.is_empty() {
        ...
        // allow-list (hl_wip-vulkan `src/model/capability.rs::INSTANCE_EXTENSIONS`) lists only
        ...
        // `VK_KHR_wayland_surface` (spec rev 6) to that allow-list. That edit is in hl_wip-vulkan (outside
        ...
        if hit_wayland_ext_gap {
            eprintln!(
                 ...
                 * Proven cause: with the SAME loader/env, Mesa lavapipe's ICD makes the loader advertise \
                   ...
                   (hl_wip-vulkan/src/model/capability.rs) omits `VK_KHR_wayland_surface`.\n\
...
fn has_bright_pixel(f: &CapturedFrame) -> bool {
    f.rgba.chunks_exact(4).any(|p| p[0] >= 150 || p[1] >= 150 || p[2] >= 150)
}
...
fn which_vkcube_wayland() -> Option<PathBuf> {
    ...
    for root in roots.iter().map(PathBuf::from).chain(std::iter::once(PathBuf::from(&home))) {
        dirs.push(root.join(".dd/workspaces/vulkanws/upper/usr/bin"));
        dirs.push(root.join(".dd/images/arm64/docker.io%2Flibrary%2Fvkbase%3Alatest/rootfs/usr/bin"));
    }
