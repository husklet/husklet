...
fn real_vulkan_timestamp_pair_is_monotonic_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/vk_timestamp.c"))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
        .arg(&bin)
    ...
    assert!(stdout.contains("VK_TIMESTAMP_OK"), "guest self-checked the timestamp pair");
    ...
    assert!(t1 >= t0, "timestamp pair must be monotonic (T0={t0}, T1={t1})");
    assert!(t1 > t0, "the host-serial model strictly increases (T0={t0}, T1={t1})");
    ...
    assert!(is_green(32, 32), "center must be GREEN, got {:?}", at(32, 32));
    assert!(is_green(2, 2), "corner must be GREEN, got {:?}", at(2, 2));
    assert!(is_green(61, 61), "far corner must be GREEN, got {:?}", at(61, 61));
    let green = px.chunks_exact(4).filter(|t| near(t[0], 0) && near(t[1], 255) && near(t[2], 0) && t[3] == 255).count();
    assert!(green > (W * H * 9 / 10) as usize, "the full-screen quad must fill the frame, got {green} green px");
