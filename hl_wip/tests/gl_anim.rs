...
fn real_gles2_uniform_animation_moves_quad_across_frames_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/gl_anim.c"))
    ...
    assert!(exec.submit_count() >= 3, "guest submitted a batch per frame (>=3)");
    ...
    for (k, &col) in cols.iter().enumerate() {
        let raw = std::fs::read(dump_dir.join(format!("frame{k}.bin"))).expect("frame dump present");
        ...
        write_png(&png_dir.join(format!("gl_anim_frame{k}.png")), W, H, &upright);
        ...
        assert!(is_green(col), "frame{k}: quad must be GREEN at column {col}");
    ...
    assert!(green_at(48), "final frame's quad must be GREEN at column 48 on the host target");
