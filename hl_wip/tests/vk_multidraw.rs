...
fn real_vulkan_multidraw_two_draws_write_disjoint_regions_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/vk_multidraw.c"))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
        .arg(&bin)
    ...
    assert!(is(16, 32, 255, 0, 0), "draw #1 LEFT region must be RED, got {:?}", at(16, 32));
    assert!(is(48, 32, 0, 0, 255), "draw #2 RIGHT region must be BLUE, got {:?}", at(48, 32));
    ...
    assert!(red > 300 && blue > 300, "both draws must cover real area (red={red}, blue={blue})");
