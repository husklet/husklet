...
fn real_gles_scaling_blit_upscales_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!("{manifest}/csrc/gl_blit_scale.c"))
    ...
    let src_line = stdout.lines().find(|l| l.contains("GL_BLIT_SCALE_SRC")).expect("source-truth line");
    let src_colors = triples_after_eq(src_line);
    assert_eq!(src_colors.len(), 4, "four source texels reported, got {src_colors:?}");
    let grad_line = stdout.lines().find(|l| l.contains("GL_BLIT_SCALE_GRAD")).expect("gradient line");
    ...
    assert!(bigs.len() >= 2, "expected two 64x64 upscale textures captured, got {}", bigs.len());
    ...
    let nearest_big = *bigs.iter().find(|t| distinct_reds_mid(t) <= 3).expect("a crisp-block (nearest) 64x64");
    let linear_big = *bigs.iter().find(|t| distinct_reds_mid(t) > 5).expect("a gradient (linear) 64x64");

    write_png(&demo_png_dir().join("gl_blit_scale.png"), BIG, BIG, nearest_big); // already RGBA
    write_png(&demo_png_dir().join("gl_blit_scale_linear.png"), BIG, BIG, linear_big);
    ...
    for (bx, by) in [(0u32, 0u32), (1, 0), (0, 1), (1, 1)] {
        ...
        for y in (by * 32)..(by * 32 + 32) {
            for x in (bx * 32)..(bx * 32 + 32) {
                ...
                assert!(!magenta(c), "NEAREST ({x},{y}) still the magenta sentinel — blit dropped this texel");
            }
        }
    ...
    for y in 0..BIG {
        for x in 1..BIG {
            ...
            assert!(r as i32 + 1 >= l as i32, "LINEAR red must be non-decreasing L→R at ({x},{y}): {l} then {r}");
        }
        let left = px(linear_big, 0, y);
        let right = px(linear_big, BIG - 1, y);
        assert!(near(left[0], al[0]) && near(left[1], al[1]) && near(left[2], al[2]), "LINEAR left edge clamps to al {al:?}, got {left:?}");
        assert!(near(right[0], bl[0]) && near(right[1], bl[1]) && near(right[2], bl[2]), "LINEAR right edge clamps to bl {bl:?}, got {right:?}");
        assert!(!magenta(px(linear_big, 32, y)), "LINEAR ({},{y}) still the magenta sentinel", 32);
    }
