/* VK RENDER-CORRECTNESS DEMO #1 — vk_transform: an MVP (scale + translate) carried in a UNIFORM BUFFER
 * places a full-NDC quad at an EXACT off-center rectangle.
 *
 * A real C Vulkan program (real loader + OUR ICD → IR → lavapipe). The vertex shader reads a std140
 * uniform block at @group(0) @binding(0) holding `scale` (vec4) + `offset` (vec4) and applies
 * `pos.xy = P*scale.xy + offset.xy` to a full-screen quad (6 verts via gl_VertexIndex, no vertex buffer).
 * The C program fills the UBO with scale=(0.5,0.5) offset=(0.25,0.0) so the quad lands in NDC
 * x∈[-0.25,0.75], y∈[-0.5,0.5] → pixel cols [24,56) × rows [16,48) on a 64×64 target, filled GREEN over a
 * BLUE clear. This is the EXACT-RECT guard for UBO-driven transforms: a wrong route collapses the geometry
 * (scale=0 → nothing) or misplaces it (identity → full frame), moving a sampled pixel off its color.
 * Same loader/ICD/descriptor plumbing as vk_ubo.c. Prints "VK_TRANSFORM_OK". */
#define _GNU_SOURCE
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-vk-transform", .apiVersion = VK_API_VERSION_1_1};
    ...
    /* ---- UNIFORM buffer: std140 { vec4 scale; vec4 offset; } filled with the MVP transform ----- */
    float ubo_data[8] = {0.5f, 0.5f, 0.0f, 0.0f,   /* scale.xy  */
                         0.25f, 0.0f, 0.0f, 0.0f};  /* offset.xy */
    ...
    /* Persistently mapped: the ICD flushes still-mapped HOST_COHERENT buffers at vkQueueSubmit. */
    ...
    /* ---- descriptor set layout (binding 0 = UNIFORM_BUFFER, VERTEX) + pipeline layout ---------- */
    ...
    /* ---- record: clear(blue) -> bind pipeline -> bind set -> draw(6) -> end -> copy ----------- */
    ...
    VkClearValue clear;
    clear.color.float32[0] = 0.0f; clear.color.float32[1] = 0.0f;  /* BLUE clear */
    clear.color.float32[2] = 1.0f; clear.color.float32[3] = 1.0f;
    ...
    pCmdDraw(cb_h, 6, 1, 0, 0);   /* two triangles = a full-NDC quad, transformed by the UBO */
    pCmdEndRP(cb_h);
