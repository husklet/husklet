/* VK RENDER-CORRECTNESS DEMO #3 — vk_instanced: N instances placed + colored by gl_InstanceIndex form an
 * EXACT grid in ONE draw.
 *
 * A real C Vulkan program (real loader + OUR ICD → IR → lavapipe). A single `vkCmdDraw(6, 4, 0, 0)` issues
 * 6 vertices × 4 INSTANCES; the vertex shader reads @builtin(instance_index) to place instance k's small
 * quad at NDC x = -0.75 + 0.5k (pixel columns 8/24/40/56, row 32) and picks its color from a 4-entry table
 * (RED/GREEN/BLUE/YELLOW). No vertex/instance BUFFER — placement is driven purely by the instance index,
 * which the wgpu executor honors via the `Draw.instance_count` range. Over a BLACK clear. Prints
 * "VK_INSTANCED_OK". The Rust test asserts each cell's EXACT color off the host executor. */
#define _GNU_SOURCE
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-vk-instanced", .apiVersion = VK_API_VERSION_1_1};
    ...
    pCmdDraw(cb_h, 6, 4, 0, 0);   /* 6 verts × 4 INSTANCES — the grid is placed by gl_InstanceIndex */
    pCmdEndRP(cb_h);
