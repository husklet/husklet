...
use hl_gpu::protocol::model::descriptor::{
    BlendState, BufferDesc, ColorAttachment, ColorTargetState, Extent3d, Origin3d, RenderPipelineDesc,
...
fn tex_ms(w: u32, h: u32, samples: u32, fmt: TextureFormat, usage: u32) -> TextureDesc {
    TextureDesc { width: w, height: h, depth: 1, mip_levels: 1, sample_count: samples, dim: TextureDim::D2, format: fmt, usage, label: String::new() }
}
fn readback(exec: &hl_gpu::CpuExecutor, s: &hl_gpu::Session, id: u32, n: usize) -> Vec<u8> {
    let mut px = vec![0u8; n];
    exec.read_texture(&s.resources, TextureId(id), &mut px).unwrap();
    px
}
...
fn srgb_clear_gamma_encodes_color_channels_but_not_alpha() {
    ...
    for fmt in [TextureFormat::Rgba8Srgb, TextureFormat::Bgra8Srgb] {
        let (exec, s) = run(&[
            Cmd::CreateTexture(1, tex(1, 1, fmt, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC)),
        ...
        assert_eq!(readback(&exec, &s, 1, 4), vec![188, 188, 188, 128], "sRGB {fmt:?} must gamma-encode color to 188 and quantize alpha to 128");
    }
    // Contrast: a LINEAR Rgba8Unorm clear of the same 0.5 quantizes every channel to 128 (no gamma).
    let (exec, s) = run(&[
        Cmd::CreateTexture(1, tex(1, 1, TextureFormat::Rgba8Unorm, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC)),
        Cmd::Submit(CommandBuffer {
            encoder: vec![
                Enc::BeginRenderPass { color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.5, 0.5, 0.5, 0.5], store: true }], depth: None },
                Enc::EndRenderPass,
            ],
            signal: None,
        }),
    ]);
    assert_eq!(readback(&exec, &s, 1, 4), vec![128, 128, 128, 128], "linear clear must NOT gamma-encode");
}
...
fn draw_pipeline(id: u32, blend: Option<BlendState>, write_mask: u32, cull: u32, front_face: u32, stride: u32, color_off: u32) -> Cmd {
    let mut attrs = vec![VertexAttr { location: 0, format: 0, offset: 0 }];
    if stride >= 24 {
        attrs.push(VertexAttr { location: 1, format: 0, offset: color_off });
    }
    Cmd::CreateRenderPipeline(
        ...
        RenderPipelineDesc {
            ...
            fragment: None,
            vertex_buffers: vec![VertexLayout { stride, step_mode: 0, attrs }],
            color_targets: vec![ColorTargetState { format: TextureFormat::Rgba8Unorm, blend, write_mask }],
            depth: None,
...
fn gradient_draw_interpolates_vertex_colors_barycentrically() {
    ...
    let (exec, s) = run(&[
        ...
        Cmd::CreateTexture(1, tex(4, 1, TextureFormat::Rgba8Unorm, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC)),
        Cmd::CreateBuffer(1, buf(verts.len() as u64, buffer_usage::VERTEX | buffer_usage::COPY_DST)),
        Cmd::WriteBuffer { id: 1, offset: 0, data: verts },
        Cmd::Submit(CommandBuffer {
            encoder: vec![
    ...
    assert_eq!(&px[0..4], &[175, 16, 64, 255], "pixel0 barycentric gradient");
    assert_eq!(&px[12..16], &[80, 112, 64, 255], "pixel3 barycentric gradient");
}
...
fn blend_source_over_composites_against_the_destination() {
    ...
    let verts: Vec<u8> = [(-1.0f32, -1.0f32), (3.0, -1.0), (-1.0, 3.0)]
        ...
        .collect();
    let blend = Some(BlendState { src_color: 1, dst_color: 0, op_color: 0, src_alpha: 1, dst_alpha: 0, op_alpha: 0 });
    let (exec, s) = run(&[
        Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::PtxKernel, spirv: kernel_words() },
        ...
        Cmd::Submit(CommandBuffer {
            encoder: vec![
                Enc::BeginRenderPass { color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0, 0.0, 1.0, 1.0], store: true }], depth: None },
                Enc::SetPipeline(1),
                Enc::SetVertexBuffer { slot: 0, buffer: 1, offset: 0 },
                Enc::Draw { vertex_count: 3, instance_count: 1, first_vertex: 0, first_instance: 0 },
                Enc::EndRenderPass,
    ...
    assert_eq!(readback(&exec, &s, 1, 4), vec![128, 0, 128, 255], "red@0.5 over blue = purple");
}
...
fn write_mask_restricts_the_draw_to_enabled_channels() {
    ...
    let scene = |mask: u32| {
        vec![
            ...
            Cmd::CreateTexture(1, tex(1, 1, TextureFormat::Rgba8Unorm, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC)),
            Cmd::CreateBuffer(1, buf(verts.len() as u64, buffer_usage::VERTEX | buffer_usage::COPY_DST)),
            Cmd::WriteBuffer { id: 1, offset: 0, data: verts.clone() },
            Cmd::Submit(CommandBuffer {
                encoder: vec![
                    Enc::BeginRenderPass { color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: [0.0, 0.0, 0.0, 1.0], store: true }], depth: None },
                    Enc::SetPipeline(1),
    ...
    assert_eq!(readback(&exec, &s, 1, 4), vec![255, 0, 0, 255], "R-only mask keeps G/B from the black clear");
    // Control: 0xF writes all channels -> full white.
    let (exec, s) = run(&scene(0xF));
    assert_eq!(readback(&exec, &s, 1, 4), vec![255, 255, 255, 255], "0xF mask writes every channel");
    // G+B mask (0x6): only G,B change -> [0,255,255,255].
    let (exec, s) = run(&scene(0x6));
    assert_eq!(readback(&exec, &s, 1, 4), vec![0, 255, 255, 255], "GB mask keeps R from black + A from clear");
}
...
fn cull_and_front_face_drop_exactly_the_intended_facing() {
    ...
    assert_eq!(cull_scene_pixel(0, 1), BLACK, "cull FRONT drops the front triangle");
    assert_eq!(cull_scene_pixel(0, 2), WHITE, "cull BACK keeps the front triangle");
    // front_face=1 (CW): the same winding is now BACK.
    assert_eq!(cull_scene_pixel(1, 1), WHITE, "cull FRONT keeps the (now) back triangle");
    assert_eq!(cull_scene_pixel(1, 2), BLACK, "cull BACK drops the (now) back triangle");
}
...
fn copy_buffer_to_texture_lays_out_rows() {
    ...
    let (exec, s) = run(&[
        ...
        Cmd::CreateTexture(1, tex(2, 2, TextureFormat::Rgba8Unorm, texture_usage::COPY_DST | texture_usage::COPY_SRC)),
        Cmd::Submit(CommandBuffer {
            encoder: vec![Enc::CopyBufferToTexture { src: 1, src_offset: 0, bytes_per_row: 8, dst: 1, mip: 0, width: 2, height: 2 }],
            signal: None,
        }),
    ]);
    assert_eq!(readback(&exec, &s, 1, 16), src, "buffer bytes copied verbatim into the tight texture plane");
}
...
fn copy_texture_to_texture_moves_only_the_named_region() {
    ...
    let (exec, s) = run(&[
        Cmd::CreateTexture(1, tex(2, 2, TextureFormat::Rgba8Unorm, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC)),
        Cmd::CreateTexture(2, tex(2, 2, TextureFormat::Rgba8Unorm, texture_usage::COPY_DST | texture_usage::COPY_SRC)),
        Cmd::Submit(CommandBuffer {
            encoder: vec![
    ...
    assert_eq!(&px[0..12], &[0u8; 12], "the three unaddressed texels stay zeroed");
    assert_eq!(&px[12..16], &[255, 0, 0, 255], "dst texel (1,1) received the copied red texel");
}
...
fn blit_downscale(filter: Filter) -> [u8; 4] {
    ...
    let (exec, s) = run(&[
        ...
        Cmd::CreateTexture(1, tex(2, 1, TextureFormat::Rgba8Unorm, texture_usage::COPY_DST | texture_usage::COPY_SRC)),
        Cmd::CreateTexture(2, tex(1, 1, TextureFormat::Rgba8Unorm, texture_usage::COPY_DST | texture_usage::COPY_SRC)),
        Cmd::Submit(CommandBuffer {
            encoder: vec![
                Enc::CopyBufferToTexture { src: 1, src_offset: 0, bytes_per_row: 8, dst: 1, mip: 0, width: 2, height: 1 },
                Enc::BlitTexture {
                    src: 1,
                    src_sub: TextureSubresource::base(),
                    src_origin: Origin3d::default(),
                    src_extent: Extent3d { width: 2, height: 1, depth: 1 },
                    dst: 2,
                    dst_sub: TextureSubresource::base(),
...
fn blit_nearest_selects_and_linear_averages() {
    assert_eq!(blit_downscale(Filter::Nearest), [0, 0, 255, 255], "nearest picks the src texel the centre lands on (blue)");
    assert_eq!(blit_downscale(Filter::Linear), [128, 0, 128, 255], "linear averages the two src texels (red+blue)");
}
...
fn resolve_multisample_averages_samples_into_single_sample_dst() {
    let (exec, s) = run(&[
        Cmd::CreateTexture(1, tex_ms(1, 1, 4, TextureFormat::Rgba8Unorm, texture_usage::COPY_SRC)),
        Cmd::CreateTexture(2, tex(1, 1, TextureFormat::Rgba8Unorm, texture_usage::COPY_DST | texture_usage::COPY_SRC)),
        Cmd::Submit(CommandBuffer {
    ...
    assert_eq!(readback(&exec, &s, 2, 4), vec![0, 0, 0, 0], "resolve wrote the per-sample mean into the dst");
}
