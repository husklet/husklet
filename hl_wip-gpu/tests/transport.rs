...
fn sample_batch() -> Vec<Cmd> {
    vec![
        Cmd::CreateBuffer(1, BufferDesc { size: 64, usage: buffer_usage::STORAGE, label: "b".into() }),
        Cmd::CreateFence(2),
        ...
        Cmd::Present { surface: 9, texture: 3 },
    ]
}
...
fn remote_sink_submits_a_batch_a_server_decodes_identically() {
    ...
    let surface = Surface { id: 9, width: 640, height: 480, stride: 2560, fd: -1, generation: 0 };
    ...
    assert_eq!(decoded, batch, "server decoded the exact submitted command sequence");
    // Bytes: the payload the server saw re-encodes to the same bytes the sink encoded.
    assert_eq!(encode_stream(&decoded), expected_payload, "payload bytes are byte-identical");
    // The transport-private 16-byte header carried the surface id/geometry.
    assert_eq!((header.surface_id, header.width, header.height), (9, 640, 480));
...
fn remote_sink_and_recording_sink_agree_at_the_command_boundary() {
    ...
    assert_eq!(socketed, recorded, "socketed boundary == in-memory boundary");
...
fn remote_sink_negotiates_against_the_host_handshake() {
    ...
    let caps = sink.negotiate(&feature_request()).expect("compatible negotiation");
    ...
    let bad = FeatureRequest { wire_version: WIRE_VERSION + 1, ..feature_request() };
    assert!(sink.negotiate(&bad).is_err(), "incompatible request fails cleanly at negotiation");
...
fn a_failure_ack_makes_submit_return_err() {
    ...
    assert!(result.is_err(), "RemoteCommandSink treated a NACK as success");
...
fn reconnect_replays_acknowledged_residency_once_before_new_work() {
    ...
    let upload = vec![
        Cmd::CreateBuffer(4, BufferDesc { size: 4, usage: buffer_usage::COPY_DST, label: "resident".into() }),
    ...
    let surface = Surface { id: 9, width: 8, height: 8, stride: 32, fd: -1, generation: 0 };
    let mut sink = RemoteCommandSink::with_surface(sock.path(), surface);
    sink.submit(&upload).expect("residency frame acknowledged");
    sink.submit(&draw).expect("reconnect recovered residency and executor ACKed");

    let (first_body, recovered) = server.join().unwrap();
    assert_eq!(decode_stream(&first_body).unwrap(), upload, "first executor saw the upload");
    let recovered = decode_stream(&recovered).unwrap();
    assert_eq!(&recovered[..upload.len()], upload.as_slice(), "residency first, exactly once");
    assert_eq!(&recovered[upload.len()..], draw.as_slice(), "new work follows the replay");
    assert_eq!(sink.generation(), 2, "one reconnect advanced the generation");
    assert!(!sink.take_residency_reset(), "successful replay consumed the reset generation");
}
