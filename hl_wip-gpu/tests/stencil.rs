...
//! the stencil model is always exercised by `hl_wip-gpu`'s own suite.
//!
...
fn fstri(c: [f32; 4]) -> Vec<u8> {
    [(-1.0, -1.0), (3.0, -1.0), (-1.0, 3.0)].iter().flat_map(|(x, y)| vtx(*x, *y, c)).collect()
}
...
fn tex(fmt: TextureFormat, usage: u32) -> TextureDesc {
    TextureDesc { width: N, height: N, depth: 1, mip_levels: 1, sample_count: 1, dim: TextureDim::D2, format: fmt, usage, label: String::new() }
}
...
fn run(cmds: &[Cmd]) -> Vec<u8> {
    ...
    let mut s = hl_gpu::Session::new(limits, hl_gpu::GlobalLedger::unbounded(), Box::new(hl_gpu::FakeClock::new(0)));
...
fn mark_then_test(
    ...
    let mut cmds = vec![
        ...
        Cmd::CreateTexture(2, tex(TextureFormat::Depth24PlusStencil8, texture_usage::RENDER_TARGET)),
        Cmd::CreateBuffer(1, buf(quad(GREEN).len() as u64, buffer_usage::VERTEX | buffer_usage::COPY_DST)),
        Cmd::CreateBuffer(2, buf(fstri(GREEN).len() as u64, buffer_usage::VERTEX | buffer_usage::COPY_DST)),
        Cmd::WriteBuffer { id: 1, offset: 0, data: quad(GREEN) },
        Cmd::WriteBuffer { id: 2, offset: 0, data: fstri(GREEN) },
    ];
    let mark_pipe = { next_pipe += 1; next_pipe };
    ...
    let test_pipe = { next_pipe += 1; next_pipe };
    ...
    let mut enc = vec![
        ...
        Enc::SetVertexBuffer { slot: 0, buffer: mark_buffer, offset: 0 },
        Enc::Draw { vertex_count: mark_verts, instance_count: 1, first_vertex: 0, first_instance: 0 },
        Enc::EndRenderPass,
    ];
    ...
    enc.extend([
        ...
        Enc::BeginRenderPass {
            color: vec![ColorAttachment { texture: 1, load: LoadOp::Clear, clear: BLUE, store: true }],
            depth: Some(DepthAttachment { texture: 2, load: LoadOp::Load, clear_depth: 1.0, clear_stencil: 0 }),
        },
    ...
    cmds.push(Cmd::Submit(CommandBuffer { encoder: enc, signal: None }));
    run(&cmds)
}
...
fn stencil_equal_gates_the_draw_to_the_marked_rect() {
    ...
    for y in 0..N {
        for x in 0..N {
            ...
            assert_eq!(at(&px, x, y), want, "pixel ({x},{y}) inside_rect={} — stencil EQUAL must gate the draw", inside_rect(x, y));
        }
    }
    let inside = ((3 * N / 4 - N / 4) * (3 * N / 4 - N / 4)) as usize; // 4x4 = 16 for N=8
    assert_eq!(count(&px, GREEN_PX), inside, "exactly the marked rect is green");
    assert_eq!(count(&px, BLUE_PX), (N * N) as usize - inside, "the rest is the blue clear");
    ...
    assert_eq!(count(&control, GREEN_PX), (N * N) as usize, "stencil DISABLED (ALWAYS) → the fullscreen draw covers everything");
}
...
fn stencil_increment_accumulates_then_gates() {
    ...
    let px = mark_then_test(
        ...
        Some(depth_stencil(compare::ALWAYS, stencil_op::INCREMENT_CLAMP, 0xFF)),
        depth_stencil(compare::EQUAL, stencil_op::KEEP, 0x00),
    );
    assert_eq!(
        count(&px, GREEN_PX),
    ...
    assert_eq!(count(&once, BLUE_PX), (N * N) as usize, "one INCREMENT → stencil 1, EQUAL(2) gates out everything");
}
