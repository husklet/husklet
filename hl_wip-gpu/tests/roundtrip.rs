...
fn rt(cmds: Vec<Cmd>) {
    assert_eq!(decode_stream(&encode_stream(&cmds)).unwrap(), cmds, "stream must round-trip: {cmds:?}");
}
...
fn every_enum_value_round_trips_through_to_from_u32() {
    ...
    for d in [TextureDim::D1, TextureDim::D2, TextureDim::D3, TextureDim::Cube] {
    ...
    for t in [Topology::PointList, Topology::LineList, Topology::LineStrip, Topology::TriangleList, Topology::TriangleStrip] {
    ...
    for a in [TextureAspect::All, TextureAspect::DepthOnly, TextureAspect::StencilOnly] {
        assert_eq!(TextureAspect::from_u32(a.to_u32()).unwrap(), a);
    }
    for m in [AddressMode::ClampToEdge, AddressMode::Repeat, AddressMode::MirrorRepeat] {
...
fn every_texture_format_and_dim_round_trips_in_a_create_texture() {
    ...
    for fmt in formats {
        for dim in [TextureDim::D1, TextureDim::D2, TextureDim::D3, TextureDim::Cube] {
            rt(vec![Cmd::CreateTexture(1, TextureDesc {
                width: 4, height: 4, depth: 1, mip_levels: 1, sample_count: 1,
                dim, format: fmt, usage: 0x3F, label: "t".into(),
            })]);
        }
    }
}
...
fn every_sampler_enum_combination_round_trips() {
    ...
    for filter in [Filter::Nearest, Filter::Linear] {
        for addr in [AddressMode::ClampToEdge, AddressMode::Repeat, AddressMode::MirrorRepeat] {
            rt(vec![Cmd::CreateSampler(1, SamplerDesc {
                min_filter: filter, mag_filter: filter, mip_filter: filter,
                address_u: addr, address_v: addr, address_w: addr,
            })]);
        }
    }
}
...
fn every_encoder_enum_value_round_trips_in_its_op() {
    ...
    for topo in [Topology::PointList, Topology::LineList, Topology::LineStrip, Topology::TriangleList, Topology::TriangleStrip] {
        rt(vec![Cmd::CreateRenderPipeline(1, RenderPipelineDesc {
            ...
            depth: None,
            topology: topo, cull: 0, front_face: 0, sample_count: 1, label: String::new(),
        })]);
    }
    for load in [LoadOp::Load, LoadOp::Clear, LoadOp::DontCare] {
        rt(vec![Cmd::Submit(CommandBuffer {
            encoder: vec![
                Enc::BeginRenderPass { color: vec![ColorAttachment { texture: 1, load, clear: [0.0; 4], store: true }], depth: None },
                Enc::EndRenderPass,
            ],
            signal: None,
        })]);
    ...
    for fmt in [IndexFormat::U16, IndexFormat::U32] {
        rt(vec![Cmd::Submit(CommandBuffer {
            encoder: vec![Enc::SetIndexBuffer { buffer: 1, offset: 0, format: fmt }],
            signal: None,
        })]);
    }
    for filter in [Filter::Nearest, Filter::Linear] {
        rt(vec![Cmd::Submit(CommandBuffer {
            encoder: vec![Enc::BlitTexture {
                src: 1, src_sub: TextureSubresource::base(), src_origin: Origin3d::default(), src_extent: Extent3d { width: 2, height: 2, depth: 1 },
                dst: 2, dst_sub: TextureSubresource::base(), dst_origin: Origin3d::default(), dst_extent: Extent3d { width: 2, height: 2, depth: 1 },
                filter,
            }],
            signal: None,
        })]);
    }
    for aspect in [TextureAspect::All, TextureAspect::DepthOnly, TextureAspect::StencilOnly] {
        rt(vec![Cmd::Submit(CommandBuffer {
            encoder: vec![Enc::CopyTextureToTexture {
                src: 1, src_sub: TextureSubresource { mip: 0, layer: 0, aspect }, src_origin: Origin3d::default(),
                dst: 2, dst_sub: TextureSubresource { mip: 0, layer: 0, aspect }, dst_origin: Origin3d::default(),
...
fn every_shader_payload_kind_classifies_deterministically() {
    ...
    use hl_gpu::protocol::model::kernel::{GlslDescriptor, KernelDescriptor, glsl_stage, SPIRV_MAGIC};
    let kd = KernelDescriptor { ptx: "ret;".into(), entry: "k".into(), block: [1, 1, 1] };
    let gd = GlslDescriptor { stage: glsl_stage::VERTEX, entry: "v".into(), source: "x".into() };
    let magic_led = vec![
        (Cmd::CreateShader { id: 1, kind: ShaderPayloadKind::SpirV, spirv: vec![SPIRV_MAGIC, 0] }, ShaderPayloadKind::SpirV),
        (Cmd::CreateShader { id: 2, kind: ShaderPayloadKind::PtxKernel, spirv: kd.to_words() }, ShaderPayloadKind::PtxKernel),
        (Cmd::CreateShader { id: 3, kind: ShaderPayloadKind::Glsl, spirv: gd.to_words() }, ShaderPayloadKind::Glsl),
    ];
    for (cmd, want) in magic_led {
        let back = decode_stream(&encode_stream(&[cmd.clone()])).unwrap();
        match &back[0] {
            Cmd::CreateShader { kind, .. } => assert_eq!(*kind, want, "magic-led payload must reclassify to {want:?}"),
    ...
    for kind in [ShaderPayloadKind::DemoBuiltin, ShaderPayloadKind::LegacyMsl] {
        let back = decode_stream(&encode_stream(&[Cmd::CreateShader { id: 1, kind, spirv: vec![0x4141_4141, 0x4242_4242] }])).unwrap();
        assert!(matches!(back[0], Cmd::CreateShader { kind: ShaderPayloadKind::LegacyMsl, .. }), "magic-less payload classifies as LegacyMsl");
    }
}
