...
fn remote_sink_reads_a_written_buffer_back_over_the_socket() {
    ...
    let got = sink.read_buffer(BufferId(1), 0, data.len()).expect("readback over socket");
    assert_eq!(got, data, "socket readback returned the exact written bytes");

    // A partial-window readback returns exactly that slice.
    let mid = sink.read_buffer(BufferId(1), 2, 3).expect("partial readback");
...
fn in_process_and_remote_readback_are_byte_identical() {
    ...
    let inproc_bytes =
        CommandSink::read_buffer(&mut inproc, BufferId(1), 0, data.len()).expect("in-process readback");
    ...
    let remote_bytes = remote.read_buffer(BufferId(1), 0, data.len()).expect("remote readback");
    drop(remote);
    server.join().unwrap();

    assert_eq!(inproc_bytes, data, "in-process readback bytes");
    assert_eq!(remote_bytes, data, "remote readback bytes");
    assert_eq!(inproc_bytes, remote_bytes, "in-process and remote readback are byte-identical");
}
...
fn submit_only_serve_connection_fails_readback() {
    ...
    assert!(sink.read_buffer(BufferId(1), 0, 4).is_err(), "submit-only host fails readback cleanly");
    drop(sink);
    server.join().unwrap();
}
