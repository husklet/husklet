...
fn large_frame_roundtrip_many_buffers_byte_exact() {
    with_watchdog(30, || {
        ...
        let content = |b: u32| -> Vec<u8> {
            (0..BUF_LEN).map(|i| (b as usize * 31 + i * 2654435761usize) as u8).collect()
        };
        ...
        for b in 1..=N_BUF {
            ...
            batch.push(Cmd::WriteBuffer { id: b, offset: 0, data });
        }
        for f in 0..3000u32 {
        ...
        sink.submit(&batch).expect("multi-MiB frame acknowledged over the socket");

        // Read every buffer back over the SAME connection and assert byte-exact — and a partial window too.
        for b in 1..=N_BUF {
            let want = content(b);
            let got = sink.read_buffer(BufferId(b), 0, BUF_LEN).expect("readback whole buffer");
            assert_eq!(got, want, "buffer {b} round-tripped byte-exact through the multi-MiB frame");
            let mid = sink.read_buffer(BufferId(b), 1000, 256).expect("partial readback");
...
fn fragmented_frame_reassembles_through_the_serve_loop() {
    with_watchdog(30, || {
        ...
        let header = SubmitHeader { surface_id: 1, width: 0, height: 0, len: payload.len() as u32 };
        ...
        let mut ack = [0u8; 1];
        (&client).read_exact(&mut ack).expect("host acked the reassembled fragmented frame");
        assert_eq!(ack[0], ACK_OK, "a frame split across single-byte writes must reassemble + ACK");
        ...
        assert_eq!(got, data, "the fragmented frame produced the same result as a single write");
...
fn disconnect_mid_frame_is_graceful_and_a_fresh_connection_works() {
    with_watchdog(30, || {
        ...
        // #1: connect, consume handshake, promise 64 payload bytes but send only 20, then drop.
        {
            let client = UnixStream::connect(&sock.0).unwrap();
            drain_handshake(&client);
            let header = SubmitHeader { surface_id: 1, width: 0, height: 0, len: 64 };
            let mut w = &client;
        ...
        let mut sink = RemoteCommandSink::new(sock.path());
        sink.submit(&write_program(2, &data)).expect("fresh connection submits after the disconnect");
        let got = sink.read_buffer(BufferId(2), 0, data.len()).expect("fresh connection readback works");
        assert_eq!(got, data, "the connection AFTER a mid-frame disconnect is fully functional");
        drop(sink);

        let (first_errored, second) = server.join().unwrap();
        assert!(first_errored, "a mid-frame disconnect surfaces a typed transport error, not a panic/hang");
        assert!(second.is_ok(), "the serve loop recovered: the next connection served to a clean EOF");
    });
}
    ...
    with_watchdog(30, || {
        ...
        for k in 0..N {
            ...
            clients.push(thread::spawn(move || {
                ...
                sink.submit(&write_program(1, &data)).expect("per-connection submit");
                let got = sink.read_buffer(BufferId(1), 0, LEN).expect("per-connection readback");
        ...
        for c in clients {
            c.join().expect("a concurrent client thread panicked (isolation broken)");
        }
        server.join().unwrap();
    });
}
...
fn malformed_length_prefix_is_capped_without_oom_and_recovers() {
    with_watchdog(30, || {
        ...
        assert!(u32::MAX > MAX_FRAME_BYTES, "the forged length must exceed the transport cap");
        ...
        // #1: forge the absurd length prefix, send NO body, drop.
        {
            let client = UnixStream::connect(&sock.0).unwrap();
            drain_handshake(&client);
            let header = SubmitHeader { surface_id: 1, width: 0, height: 0, len: u32::MAX };
            let mut w = &client;
            w.write_all(&header.to_bytes()).unwrap();
            w.flush().ok();
        ...
        let mut sink = RemoteCommandSink::new(sock.path());
        sink.submit(&write_program(3, &data)).expect("fresh connection works after a malformed prefix");
        let got = sink.read_buffer(BufferId(3), 0, data.len()).expect("readback after malformed prefix");
        assert_eq!(got, data, "the connection after a malformed length prefix is fully functional");
        drop(sink);

        let (first_errored, second) = server.join().unwrap();
        assert!(first_errored, "an over-cap wire length is refused (drained to EOF) as a typed error, not OOM");
        assert!(second.is_ok(), "the serve loop recovered: the next connection served cleanly");
    });
}
...
fn pipelined_burst_does_not_deadlock_under_backpressure() {
    with_watchdog(45, || {
        ...
        // Frame 0: create the destination buffer. Wait for its ack before pipelining writes into it.
        {
            ...
            let hdr = SubmitHeader { surface_id: 1, width: 0, height: 0, len: create.len() as u32 };
        ...
        // is a known pattern. Backpressure engages when the in-flight bytes exceed the OS socket buffer.
        {
            ...
            for f in 0..FRAMES {
                ...
                let batch = [Cmd::WriteBuffer { id: 1, offset: (f * CHUNK) as u64, data: chunk }];
                let payload = encode_stream(&batch);
                let hdr = SubmitHeader { surface_id: 1, width: 0, height: 0, len: payload.len() as u32 };
        ...
        assert_eq!(oks, FRAMES, "every pipelined frame was processed and acked — no deadlock, no drop");
        ...
        assert_eq!(got.len(), total, "the whole buffer was written by the burst");
        for f in 0..FRAMES {
