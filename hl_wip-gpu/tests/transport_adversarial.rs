...
use hl_gpu::transport::{serve_connection, serve_connection_with_handler, ConnectionHandler, Verdict};
...
fn read_frame_reassembles_a_full_frame() {
    ...
    let header = SubmitHeader { surface_id: 7, width: 64, height: 32, len: 5 };
...
fn read_frame_returns_none_on_clean_eof() {
    ...
    assert!(read_frame(&b).unwrap().is_none(), "a clean close is Ok(None), not an error");
}
...
fn read_frame_errors_on_a_truncated_payload() {
    ...
    let (a, b) = UnixStream::pair().unwrap();
    let header = SubmitHeader { surface_id: 1, width: 0, height: 0, len: 16 };
    let mut bytes = header.to_bytes().to_vec();
    bytes.extend_from_slice(&[0xAB; 4]); // only 4 of the promised 16 payload bytes
    let mut writer = a;
    writer.write_all(&bytes).unwrap();
    drop(writer); // EOF mid-payload
    assert!(read_frame(&b).is_err(), "a truncated payload is a transport error");
}
...
fn read_frame_reassembles_across_interleaved_partial_writes() {
    ...
    let (a, b) = UnixStream::pair().unwrap();
    let header = SubmitHeader { surface_id: 3, width: 1, height: 1, len: 8 };
    ...
    let frame = read_frame(&b).unwrap().expect("frame reassembled from partial writes");
...
fn readback_response_fail_is_a_typed_error_not_empty_bytes() {
    ...
    assert!(read_readback_response(&b).is_err(), "a FAIL readback response is an error");
}
...
fn server_nacks_a_malformed_submit_payload_without_calling_the_handler() {
    ...
    assert_eq!(handler_calls, 0, "the handler never saw the malformed frame");
}
...
fn readback_frame_routes_to_readback_never_to_submit() {
    ...
    let submit_hdr = SubmitHeader { surface_id: 5, width: 0, height: 0, len: submit_payload.len() as u32 };
    ...
    assert_eq!(bytes, vec![0x10, 0x20, 0x30], "readback returned the host's bytes");
    ...
    assert!(read_readback_response(&client).is_err(), "unknown-buffer readback fails cleanly");
    ...
    assert_eq!(submits, 1, "only the real submit was counted; readback frames bypassed submit");
}
...
fn serve_loop_returns_cleanly_when_the_peer_closes() {
    ...
    drop(client);
    assert!(server.join().unwrap().is_ok(), "serve_loop returns Ok on a clean peer close");
}
