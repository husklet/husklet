...
fn assert_no_panic(bytes: &[u8]) {
    ...
    assert!(r.is_ok(), "decode_stream PANICKED on {} bytes: {:02x?}", bytes.len(), bytes);
}

fn representative_streams() -> Vec<Vec<Cmd>> {
    vec![
        vec![
            Cmd::CreateBuffer(1, BufferDesc { size: 256, usage: buffer_usage::VERTEX | buffer_usage::COPY_DST, label: "b".into() }),
        ...
        vec![
            Cmd::CreateTexture(2, TextureDesc {
                ...
                format: TextureFormat::Rgba8Unorm, dim: TextureDim::D2,
                usage: texture_usage::COPY_DST | texture_usage::SAMPLED, label: "t".into(),
            }),
            Cmd::Present { surface: 2, texture: 2 },
...
fn decode_never_panics_on_bitflipped_valid_streams() {
    for stream in representative_streams() {
        ...
        assert!(decode_stream(&good).is_ok(), "valid stream failed to decode");
