...
use hl_gpu::{
    Cmd, CommandSink, CpuExecutor, GpuError, GpuExecutor, InProcessCommandSink,
};
...
fn every_resource_kind_creates_looks_up_and_destroys_through_the_runtime() {
    ...
    let mut sink = InProcessCommandSink::with_session(
        {
            ...
            hl_gpu::Session::new(limits, hl_gpu::GlobalLedger::unbounded(), Box::new(hl_gpu::FakeClock::new(0)))
        },
        exec,
    );
    ...
    let create = vec![
        Cmd::CreateBuffer(1, BufferDesc { size: 4096, usage: buffer_usage::STORAGE, label: String::new() }),
        Cmd::CreateTexture(1, TextureDesc {
            width: 4, height: 4, depth: 1, mip_levels: 1, sample_count: 1,
            dim: TextureDim::D2, format: TextureFormat::Rgba8Unorm, usage: texture_usage::SAMPLED, label: String::new(),
        }),
        ...
        Cmd::CreateShader { id: 1, kind: hl_gpu::ShaderPayloadKind::PtxKernel, spirv: vec![KERNEL_MAGIC, 0] },
        Cmd::CreateRenderPipeline(1, RenderPipelineDesc {
            vertex: ShaderRef { module: 1, entry: "k".into() },
            fragment: None,
            vertex_buffers: vec![],
        ...
        Cmd::CreateComputePipeline(2, ComputePipelineDesc { compute: ShaderRef { module: 1, entry: "k".into() }, label: String::new() }),
        Cmd::CreateBindGroup(1, BindGroupDesc {
            set: 0,
            entries: vec![BindEntry { binding: 0, resource: BindResource::Buffer { id: 1, offset: 0, size: 4096 } }],
        }),
        Cmd::CreateSurface(1, SurfaceDesc { width: 4, height: 4, format: TextureFormat::Bgra8Unorm, hlp_surface: 1 }),
        Cmd::CreateFence(1),
    ];
    sink.submit(&create).expect("every-kind create batch runs cleanly");

    // LOOKUP: each id is live in its own kind's table (the runtime is the singular id -> native owner).
    let r = &sink.session().resources;
    assert!(r.buffers.contains(1) && r.textures.contains(1) && r.samplers.contains(1), "buffer/texture/sampler live");
    assert!(r.shaders.contains(1) && r.pipelines.contains(1) && r.pipelines.contains(2), "shader + both pipelines live");
    assert!(r.bind_groups.contains(1) && r.surfaces.contains(1) && r.fences.contains(1), "bind group/surface/fence live");
    ...
    assert_eq!(sink.session().residency_bytes(), expect_bytes, "aggregate per-kind residency charge");
    assert_eq!(sink.session().object_count(), 9);
    assert_eq!(sink.session().compiled_cache_bytes(), 4096 + 4096, "only the two pipelines meter the compiled cache");
    ...
    sink.submit(&destroy).expect("every-kind destroy batch runs cleanly");
    ...
    assert_eq!(sink.session().residency_bytes(), 0, "residency fully refunded");
    assert_eq!(sink.session().object_count(), 0);
    assert_eq!(sink.session().compiled_cache_bytes(), 0, "compiled cache refunded on pipeline destroy");
}
...
fn dispatch_routes_draw_and_compute_work_to_the_executor() {
    ...
    sink.submit(&[
        Cmd::CreateShader { id: 1, kind: hl_gpu::ShaderPayloadKind::PtxKernel, spirv: vec![KERNEL_MAGIC, 0] },
        Cmd::CreateTexture(1, TextureDesc {
            width: 2, height: 2, depth: 1, mip_levels: 1, sample_count: 1,
            dim: TextureDim::D2, format: TextureFormat::Rgba8Unorm, usage: texture_usage::RENDER_TARGET, label: String::new(),
        }),
        Cmd::CreateRenderPipeline(1, RenderPipelineDesc {
            vertex: ShaderRef { module: 1, entry: "k".into() },
            fragment: None,
        ...
        Cmd::CreateComputePipeline(2, ComputePipelineDesc { compute: ShaderRef { module: 1, entry: "k".into() }, label: String::new() }),
        Cmd::CreateBuffer(1, BufferDesc { size: 16, usage: buffer_usage::STORAGE, label: String::new() }),
    ...
    sink.submit(&[Cmd::Submit(CommandBuffer {
        encoder: vec![
            Enc::BeginRenderPass { color: vec![hl_gpu::protocol::model::descriptor::ColorAttachment { texture: 1, load: hl_gpu::protocol::model::enums::LoadOp::Load, clear: [0.0; 4], store: true }], depth: None },
            Enc::SetPipeline(1),
    ...
    assert_eq!(sink.executor().draws, 1, "the draw op routed to the executor");
    assert_eq!(sink.executor().dispatches, 1, "the compute dispatch routed to the executor");
}
...
fn negotiate_accepts_a_compatible_request() {
    ...
    assert!(caps.negotiate(&req).is_ok(), "a fully-covered request negotiates cleanly");
    ...
    assert!(caps.negotiate(&full_req).is_ok(), "the maximal in-surface request negotiates cleanly");
}
...
fn negotiate_rejects_each_incompatible_axis() {
    ...
    assert_eq!(
        caps.negotiate(&FeatureRequest { wire_version: caps.wire_version + 1, ..base.clone() }).unwrap_err(),
    ...
    assert_eq!(
        caps.negotiate(&FeatureRequest { shader_payloads: 1 << 31, ..base.clone() }).unwrap_err(),
        GpuError::Unsupported("capability: shader payload not supported"),
    );
    // a command tag the backend does not replay (bit 63).
    assert_eq!(
        caps.negotiate(&FeatureRequest { command_bits: 1 << 63, ..base.clone() }).unwrap_err(),
    ...
    assert_eq!(
        caps.negotiate(&FeatureRequest { texture_formats: depth_bit, ..base.clone() }).unwrap_err(),
...
fn negotiate_through_the_runtime_service_records_or_rejects() {
    ...
    let negotiated = service::negotiate::negotiate(&mut s, &exec, &req).expect("compatible negotiate");
    assert_eq!(negotiated.name, "hl-cpu");
    assert!(s.caps.is_some(), "a successful negotiation records the caps on the session");
    ...
    let bad = FeatureRequest { wire_version: caps2.wire_version, shader_payloads: shader_payload::SPIRV, command_bits: 0, texture_formats: 0 };
    assert_eq!(
