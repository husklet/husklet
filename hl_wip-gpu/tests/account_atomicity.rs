...
fn duplicate_create_over_live_id_is_typed_and_charges_nothing() {
    ...
    assert_eq!(snapshot(&s, &global), before, "a rejected duplicate create must not move the account");
}
...
fn repeated_duplicate_creates_never_drift_the_account() {
    ...
    s.submit(&[buffer(1, 4096), buffer(2, 2048), Cmd::CreateFence(9)]).unwrap();
    let before = snapshot(&s, &global);
    ...
    for i in 0..N {
        ...
        let dup = if i % 2 == 0 { buffer(1, (i as u64 + 1) * 8192) } else { Cmd::CreateFence(9) };
        ...
        assert_eq!(snapshot(&s, &global), before, "attempt {i} drifted the account");
    }
    ...
    assert_eq!(err, GpuError::DuplicateId { kind: "buffer", id: 1 });
    assert_eq!(snapshot(&s, &global), before, "a mixed frame that rejects must charge nothing at all");
}
...
fn legal_create_after_rejections_still_charges_and_dispatches() {
    ...
    for _ in 0..8 {
        assert!(matches!(
            ...
            GpuError::DuplicateId { kind: "buffer", id: 1 }
        ));
    }
    assert_eq!(snapshot(&s, &global), before, "rejections left the account untouched");
    ...
    assert_eq!(after.conn_bytes, before.conn_bytes + 2048, "the legal create charged exactly its size");
    ...
    assert_eq!(s.session().resources.live_count(), 2, "the legal create really dispatched");
    ...
    assert_eq!(s.read_buffer(hl_gpu::BufferId(1), 0, 128).unwrap(), vec![0u8; 128]);
}
