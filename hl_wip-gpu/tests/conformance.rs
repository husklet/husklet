...
use hl_gpu::protocol::model::descriptor::{
    BindEntry, BindGroupDesc, BindResource, BufferDesc, ColorAttachment, ComputePipelineDesc, ShaderRef,
    TextureDesc,
};
...
fn buffer_partial_readback_window() {
    ...
    let mut out = [0u8; 3];
    exec.read_buffer(&s.resources, BufferId(1), 2, &mut out).unwrap();
    assert_eq!(out, [2, 3, 4]);
}
...
fn buffer_to_buffer_copy_full() {
    ...
    let mut out = [0u8; 4];
    exec.read_buffer(&s.resources, BufferId(2), 0, &mut out).unwrap();
...
fn texture_clear_midgray_rounds_to_128() {
    ...
    let s = run_batch(
        &mut exec,
        &[
            Cmd::CreateTexture(1, tex(1, 1, TextureFormat::Rgba8Unorm, texture_usage::RENDER_TARGET)),
...
fn vecadd_program() -> KernelProgram {
    KernelProgram {
        ...
        block: [4, 1, 1],
        params: vec![
            Param { width: 8, offset: 0, is_ptr: true, region: 0 },  // a
            Param { width: 8, offset: 8, is_ptr: true, region: 1 },  // b
            Param { width: 8, offset: 16, is_ptr: true, region: 2 }, // c
            Param { width: 4, offset: 24, is_ptr: false, region: 0 }, // n (scalar)
        ],
        param_bytes: 28,
        num_regions: 3,
        shared_bytes: 0,
        ...
        insts: vec![
            ...
            Inst::MovSReg { d: 4, sreg: SR_NTID_X },  // r3 = ntid.x
            Inst::MovSReg { d: 5, sreg: SR_CTAID_X }, // r4 = ctaid.x
            Inst::MovSReg { d: 6, sreg: SR_TID_X },   // r5 = tid.x
            Inst::IMad { d: 7, a: Op::Reg(5), b: Op::Reg(4), c: Op::Reg(6) }, // r1 = r4*r3 + r5
            Inst::Setp { d: 8, a: Op::Reg(7), b: Op::Reg(3), cmp: CMP_GE, unsigned: false }, // p1 = i>=n
            Inst::Bra { target: 21, pred: Some((8, false)) }, // @p1 -> ret
            Inst::Cvta { d: 9, s: 0 },  // rd4 = global(a)
            Inst::IMul { d: 10, a: Op::Reg(7), b: Op::ImmI(4), wide: true, unsigned: false }, // rd5 = i*4
            Inst::IAdd { d: 11, a: Op::Reg(9), b: Op::Reg(10), wide: true }, // rd6 = &a[i]
            Inst::Cvta { d: 12, s: 1 }, // rd7 = global(b)
            Inst::IAdd { d: 13, a: Op::Reg(12), b: Op::Reg(10), wide: true }, // rd8 = &b[i]
            Inst::LdGlobal { d: 14, addr: 13, off: 0, ty: gty::F32 }, // f1 = b[i]
            Inst::LdGlobal { d: 15, addr: 11, off: 0, ty: gty::F32 }, // f2 = a[i]
            Inst::FAdd { d: 16, a: Op::Reg(15), b: Op::Reg(14) }, // f3 = a[i]+b[i]
            Inst::Cvta { d: 17, s: 2 }, // rd9 = global(c)
            Inst::IAdd { d: 18, a: Op::Reg(17), b: Op::Reg(10), wide: true }, // rd10 = &c[i]
            Inst::StGlobal { addr: 18, off: 0, src: Op::Reg(16), ty: gty::F32 }, // c[i] = f3
            Inst::Ret,
        ],
    }
}
...
fn compute_dispatch_writes_constant_into_buffer() {
    ...
    let mut out = [0u8; 4];
    exec.read_buffer(&s.resources, BufferId(2), 0, &mut out).unwrap();
    assert_eq!(f32::from_le_bytes(out), 1.0, "kernel must store 1.0f into region 0");
}
...
fn compute_vecadd_elementwise() {
    ...
    let mut out = [0u8; 16];
    exec.read_buffer(&s.resources, BufferId(4), 0, &mut out).unwrap();
    let got: Vec<f32> =
