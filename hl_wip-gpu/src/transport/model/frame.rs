...
impl Frame {
    ...
    pub fn new(surface_id: u32, width: u32, height: u32, payload: Vec<u8>) -> Self {
        let header = SubmitHeader { surface_id, width, height, len: payload.len() as u32 };
        Self { header, payload }
    }
...
mod tests {
    ...
    fn frame_bytes_are_header_then_payload() {
        ...
        assert_eq!(SubmitHeader::from_bytes(bytes[..SubmitHeader::SIZE].try_into().unwrap()), f.header);
    }
}
