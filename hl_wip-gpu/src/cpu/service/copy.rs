...
pub(crate) fn buffer_slice_bounds(buf_len: usize, offset: u64, size: u64) -> Result<(usize, usize)> {
...
pub(crate) fn check_region_in_texture(t: &Texture, origin: &Origin3d, extent: &Extent3d) -> Result<()> {
...
pub(crate) fn blit_texture(
    ...
    let (sw, bpt, src_fmt) = {
        let t = texture(res, src)?;
        (t.desc.width as usize, texel_bytes(t.desc.format)?, t.desc.format)
    };
    ...
    for y in 0..extent.height as usize {
        for x in 0..extent.width as usize {
            ...
            for channel in 0..bpt {
                let sum: u32 =
                    (0..samples).map(|sample| source[sp + sample * bpt + channel] as u32).sum();
