...
import { ContainerDetailsSource, ExecutionDetailsSource, ImageDetailsSource } from './model.js';
...
const session = await connect({
  onRows(request, channel) {
    const window = imageDetails.answer(request) ?? containerDetails.answer(request) ?? executionDetails.answer(request);
...
surface = render(React.createElement(WorkspaceManager, { api: workspace(session), selections: providerSelections, containerDetails, executionDetails, imageDetails }), session, {
  title: 'Resources',
});
