...
import { ContainerDetailsSource, ImageDetailsSource } from './model.js';
...
const session = await connect({
  onRows(request, channel) {
    const window = imageDetails.answer(request) ?? containerDetails.answer(request);
...
surface = render(React.createElement(WorkspaceManager, { api: workspace(session), selections: providerSelections, containerDetails, imageDetails }), session, {
  title: 'Resources',
});
