...
import { ContainerDetailsSource, ExecutionDetailsSource, ImageDetailsSource, NetworkDetailsSource } from './model.js';
...
const session = await connect({
  onRows(request, channel) {
    const window = imageDetails.answer(request) ?? containerDetails.answer(request) ?? executionDetails.answer(request) ?? networkDetails.answer(request);
...
surface = render(React.createElement(WorkspaceManager, { api: workspace(session), selections: providerSelections, containerDetails, executionDetails, imageDetails, networkDetails }), session, {
  title: 'Resources',
});
