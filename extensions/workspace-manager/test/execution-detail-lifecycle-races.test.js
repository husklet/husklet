...
test('same-id execution replacement fences detail output wait and consent', { timeout: 8_000 }, async () => {
  ...
  const server = net.createServer((socket) => { const reader = new Reader(); socket.write(encode({ channel: 0, kind: KIND.request, payload: { protocol: 1, extension: 'execution-lifecycle-test', granted: ['container-read', 'container-write'] } })); socket.on('data', (chunk) => { for (const frame of reader.take(chunk)) {
  ...
  try {
    ...
    stage = host(); stage.render(h(WorkspaceManager, { api: { ...framed, containers: { ...framed.containers, execution: race.containers.execution, executionLogs: race.containers.executionLogs, waitExecution: race.containers.waitExecution }, subscribe: undefined, unsubscribe: undefined }, executionDetails: new ExecutionDetailsSource(async (mutation) => mutations.push(mutation)), initial: { containers: [], images: [], volumes: [], networks: [] } }));
