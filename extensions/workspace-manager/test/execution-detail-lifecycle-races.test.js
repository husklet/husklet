...
test('same-id execution replacement fences detail output wait and consent', { timeout: 8_000 }, async () => {
  ...
  const server = net.createServer((socket) => { const reader = new Reader(); socket.write(encode({ channel: 0, kind: KIND.open, payload: { protocol: 1, extension: 'execution-lifecycle-test', granted: ['container-read', 'container-write'] } })); socket.on('data', (chunk) => { for (const frame of reader.take(chunk)) {
