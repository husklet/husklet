...
export function tools(api) {
  const definitions = [
    ...
    define('husklet_extension_provider_list', 'List bounded manifest-declared providers for enabled extension records, independently of pane occupancy; status is lifecycle policy, not sidecar readiness.', empty, () => api.extensions.providers(), inventory('providers')),
    define('husklet_extension_provider_catalogue_wait', 'Wait for one exact installed extension name/digest/status cursor to change, then return the bounded enabled declaration catalogue.', z.object({ after: extensionInventoryCursor, timeout_ms: z.number().int().min(1).max(30_000).default(30_000) }).strict(), ({ after, timeout_ms }) => api.extensions.waitForProviders(after, { timeoutMs: timeout_ms }), detailResult),
