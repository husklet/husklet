...
export function tools(api) {
  const definitions = [
    ...
    define('husklet_extension_acquisition', 'Read bounded acquisition progress, candidate digest, installed digest observed for consent, manifest identity, and requested grants.', z.object({ job: extensionJob }).strict(), ({ job }) => api.extensions.acquisition(job)),
    ...
    define('husklet_container_processes', 'Read a bounded timestamped initial-process snapshot bound to the complete immutable container ID actually sampled; PIDs are snapshot-local and reusable.', z.object({ id }).strict(), ({ id: value }) => api.containers.processes(value)),
    ...
    define('husklet_execution_signal', 'Signal one immutable execution ID without signaling its owning container; snapshot PIDs are never accepted.', z.object({ id: executionIdentity, signal: signalName }).strict(), async ({ id: value, signal }) => { await api.containers.signalExecution(value, signal); return { done: true }; }),
    ...
    define('husklet_terminal_switch_occupant', 'Switch one exact observed pane generation between its retained terminal and a live named extension provider.', z.object({ slot: id, generation: acquisitionRevision, target: z.union([z.object({ kind: z.literal('terminal') }).strict(), z.object({ kind: z.literal('surface'), extension: extensionName, provider: extensionName }).strict()]) }).strict(), async ({ slot: value, generation, target }) => { await api.terminal.switchOccupant(value, generation, target); return { done: true }; }),
    define('husklet_terminal_close', 'Close only the exact observed pane generation and revision after explicit confirmation.', z.object({ slot: id, generation: acquisitionRevision, revision: acquisitionRevision, confirm: z.literal(true) }).strict(), async ({ slot: value, generation, revision }) => { await api.terminal.closeObserved(value, generation, revision); return { done: true }; }),
