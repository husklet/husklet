...
test('semantic XML escapes every XML metacharacter and remains structurally bounded', () => {
  ...
  const plan=semanticXml({slot:'plan',revision:1,truncated:false,root:{id:0,role:'QueryPlan',label:'plan & <x>',value:'bounded source',disabled:false,destructive:false,actions:[],children:[{id:1,role:'QueryPlanNode',label:'hash_join',value:'state=hot detail="slow&wide"',disabled:false,destructive:false,actions:[],children:[{id:2,role:'QueryPlanMetric',label:'duration_us',value:'42',disabled:false,destructive:false,actions:[],children:[]}]}]}});assert.match(plan,/role="QueryPlan"/);assert.match(plan,/&amp;/);assert.match(plan,/&lt;x&gt;/);assert.equal((plan.match(/<node /g)??[]).length,3);assert(!plan.includes('<truncated/>'));
