...
`husklet_terminal_spawn` also requires both fields and will not run its bounded
...
requires `confirm: true`, targets one complete immutable execution ID under
`ContainerControl`, and accepts only a 1..=32 byte signal name. Container stop, removal, and signaling likewise
...
document: terminal panes include screen lines plus cursor and grid dimensions from one
screen snapshot, alongside focus and tab metadata;
