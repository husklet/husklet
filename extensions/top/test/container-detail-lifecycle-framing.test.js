...
test(
  'authoritative container inventory replacement invalidates prior detail lifecycle',
  { timeout: 5_000 },
  async () => {
    ...
    const server = net.createServer((socket) => {
      ...
      socket.write(
        encode({
          ...
          payload: {
            protocol: 1,
            extension: 'container-detail-lifecycle-test',
          ...
          if (call === 'container_list') {
            ...
            payload = {
              ...
              with: [
                {
                  id,
                  name: listAttempts === 1 ? 'before-refresh' : 'after-refresh',
                  image: listAttempts === 1 ? 'old:image' : 'new:image',
                  state: 'running',
            ...
            payload = {
              ...
              with: {
                id,
                name: inspectAttempts === 1 ? 'before-refresh' : 'after-refresh',
                image: inspectAttempts === 1 ? 'old:image' : 'new:image',
                state: 'running',
                created: inspectAttempts,
              },
            };
          }
    ...
    try {
      ...
      assert.ok(
        ...
        'refresh unmounts old detail',
      );
      assert.ok(
        refreshPatches.some((patch) => patch.SetProp?.value?.Text === 'Details'),
        'replacement returns closed',
      );
      assert.equal(
        refreshPatches.some((patch) => patch.SetProp?.value?.Text === 'Hide details'),
        false,
      ...
      assert.equal(
        ...
        1,
        'inventory replacement does not republish old detail',
      );

      invoke(stage, 'Details');
      await until(() => lengths(mutations).length === 2);
      assert.deepEqual(lengths(mutations)[1], {
        source: CONTAINER_DETAIL_SOURCE,
        version: 2,
        rows: 5,
      });
