...
test('same-name volume generation replacement invalidates detail authority', { timeout: 5_000 }, async () => {
  ...
  try {
    ...
    stage = host(); stage.render(h(Top, { api: { ...framed, volumes: { ...framed.volumes, inspect: inspectionApi.volumes.inspect }, subscribe: undefined, unsubscribe: undefined }, volumeDetails: new VolumeDetailsSource(async (mutation) => mutations.push(mutation)), initial: { containers: [], executions: [], images: [], networks: [] } }));
