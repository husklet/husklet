...
test('authoritative network replacement invalidates detail and identity consent', { timeout: 5_000 }, async () => {
  ...
  try {
    ...
    stage = host(); stage.render(h(Top, { api: { ...framed, networks: { ...framed.networks, inspect: inspectionApi.networks.inspect }, subscribe: undefined, unsubscribe: undefined }, networkDetails: new NetworkDetailsSource(async (m) => mutations.push(m)), initial: { containers: [], executions: [], images: [], volumes: [] } }));
