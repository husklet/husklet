...
- Virtual table column schemas may mark a column `importance: "Optional"`; a narrow renderer moves those cells into bounded per-row details. `identity: true` marks the stable identifying column and keeps it visible at every supported width. This changes presentation only: row identities, source generations, and bounded windows remain authoritative.
