...
- `host.containers.execStreaming(id, generation, options, onPage)` — executes by immutable container ID, optionally resolves credential keys directly into process environment variables, reports the live execution ID through an awaited `options.onStarted` hook, writes bounded `options.input` chunks with backpressure and explicit EOF before consuming output, delivers bounded pages with callback backpressure, cancels on abort or callback failure, and returns the completed summary without auto-removing the record.
