...
groups
  ...
  .push(
    ...
    '- `host.containers.execStreaming(id, generation, options, onPage)` — executes by immutable container ID, optionally resolves credential keys directly into process environment variables, reports the live execution ID through an awaited `options.onStarted` hook, concurrently writes bounded `options.input` chunks and drains bounded output pages so neither pipe can deadlock the other, preserves backpressure and explicit EOF in both directions, cancels on abort or callback failure, and returns the completed summary without auto-removing the record.',
