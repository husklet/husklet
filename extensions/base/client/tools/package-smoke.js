...
async function runPackedStarter(starter, installedClient, architecture, signal, hostEof = false) {
  const wire = await import(new URL('dist/wire.js', `file://${installedClient}/`));
      ...
      for (const frame of reader.take(chunk)) {
        ...
        stream.write(
          wire.encode({
            ...
            payload: {
              reply: 'workspace',
              with: { name: 'project', architecture, image: 'alpine:3.20' },
            },
          }),
          () => {
      ...
      child.exitCode !== null
        ...
        : await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error('packed client starter did not stop')), 2_000),
            ),
          ]);
    assert.deepEqual(exit, hostEof ? { code: 1, signal: null } : { code: 0, signal: null });
    assert.equal(
      stdout,
      `${JSON.stringify({ name: 'project', architecture, image: 'alpine:3.20' })}\n`,
    );
    assert.equal(
      stderr,
      hostEof ? 'client-starter: host connection ended: extension host connection closed\n' : '',
    );
  } finally {
    peer?.destroy();
...
async function runPackedProtocolRefusal(starter, installedClient) {
  ...
  const server = net.createServer((stream) => {
    ...
    stream.on('data', (chunk) => {
      spoke = true;
      for (const frame of reader.take(chunk)) {
        if (frame.payload?.call === 'workspace_info')
          stream.write(
            wire.encode({
              ...
              payload: {
                reply: 'workspace',
                with: { name: 'project', architecture: 'x86_64', image: 'alpine:3.20' },
              },
            }),
          );
      }
  ...
  try {
    const exit = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error('packed client starter did not refuse incompatible protocol')),
          2_000,
        ),
      ),
    ]);
    ...
    assert.equal(
      spoke,
      false,
      'incompatible protocol must be refused before the starter sends a request',
    );
    assert.equal(stdout, '');
    assert.equal(
      stderr,
      'client-starter: startup failed: host speaks protocol 2, this extension speaks 1\n',
    );
  } finally {
    peer?.destroy();
    if (child.exitCode === null) child.kill('SIGKILL');
...
async function runPackedTruncatedGreeting(starter, installedClient) {
  ...
  try {
    const exit = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error('packed client starter did not reject a truncated greeting')),
          2_000,
        ),
      ),
    ]);
    ...
    assert.equal(
      spoke,
      false,
      'truncated greeting must be rejected before the starter sends a request',
    );
    assert.equal(stdout, '');
    assert.match(
      stderr,
      /^client-starter: startup failed: extension host closed with an unfinished frame \([1-9][0-9]* bytes buffered\)\n$/,
    );
  } finally {
    peer?.destroy();
    if (child.exitCode === null) child.kill('SIGKILL');
...
async function runPackedOversizedGreeting(starter, installedClient) {
  ...
  try {
    const exit = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error('packed client starter did not reject an oversized greeting')),
          2_000,
        ),
      ),
    ]);
    ...
    assert.equal(
      spoke,
      false,
      'oversized greeting must be rejected before the starter sends a request',
    );
    assert.equal(stdout, '');
    assert.equal(
      stderr,
      `client-starter: startup failed: frame declares ${wire.PAYLOAD_LIMIT + 1} bytes, above the ${wire.PAYLOAD_LIMIT} limit\n`,
    );
  } finally {
    peer?.destroy();
    if (child.exitCode === null) child.kill('SIGKILL');
  ...
  try {
    const exit = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () =>
            reject(new Error('packed client starter did not reject an illegal greeting header')),
          2_000,
        ),
      ),
    ]);
    ...
    assert.equal(
      spoke,
      false,
      'illegal greeting header must be rejected before the starter sends a request',
    );
    assert.equal(stdout, '');
...
async function runPackedGreetingTimeout(starter) {
  ...
  const child = spawn(process.execPath, ['main.js'], {
    ...
    env: {
      ...process.env,
      HUSKLET_EXTENSION_SOCKET: socket,
      HUSKLET_EXTENSION_CONNECT_TIMEOUT_MS: '40',
    },
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  let stdout = '';
  ...
  try {
    const exit = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error('packed client starter did not enforce its greeting deadline')),
          2_000,
        ),
      ),
    ]);
    ...
    assert.equal(
      stderr,
      'client-starter: startup failed: extension host handshake timed out after 40ms\n',
    );
  } finally {
    peer?.destroy();
    if (child.exitCode === null) child.kill('SIGKILL');
...
async function runPackedMalformedReply(starter, installedClient) {
  ...
  try {
    const exit = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error('packed client starter did not reject a malformed reply')),
          2_000,
        ),
      ),
    ]);
    ...
    assert.equal(
      requested,
      true,
      'packed starter must cross the handshake and issue its exact workspace request',
    );
    assert.equal(stdout, '', 'malformed reply must not become invented workspace output');
    assert.match(
      stderr,
      /^client-starter: host connection ended: frame payload is not valid UTF-8 JSON: .{1,256}\n$/,
    );
  } finally {
    peer?.destroy();
    if (child.exitCode === null) child.kill('SIGKILL');
...
try {
  ...
  for (const required of [
    ...
    'dist/index.js',
    'dist/api.d.ts',
    'dist/generated-protocol.js',
    'dist/generated-protocol.d.ts',
    'dist/semantic.js',
    'dist/session.js',
    'dist/wire.js',
    ...
    'examples/agent-control.mjs',
    'examples/agent-container-control.mjs',
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(consumer, 'node_modules/@husklet/client/package.json')),
  );
  assert.equal(
    manifest.dependencies,
    undefined,
    'framework-neutral client gained an undeclared runtime dependency',
  );
  for (const field of ['cpu', 'os', 'libc']) {
    assert.equal(
      manifest[field],
      undefined,
      `framework-neutral client gained an architecture restriction in ${field}`,
    );
  }
  assert.equal(manifest.exports['.'].types, './dist/api.d.ts');
  assert.equal(manifest.exports['./protocol'].types, './dist/generated-protocol.d.ts');
  ...
  for (const name of ['generated-protocol.js', 'generated-protocol.d.ts']) {
    assert.deepEqual(
      fs.readFileSync(path.join(installedClient, 'dist', name)),
      fs.readFileSync(path.join(root, 'src', name)),
      `packed ${name} drifted from the Rust-fingerprinted artifact`,
    );
  }
  ...
  assert.equal(
    starterManifest.engines.node,
    manifest.engines.node,
    'starter Node requirement drifted from the installed client',
  );
  assert.equal(
    starterDockerignore,
  ...
  assert.equal(
    starterManifest.scripts.test,
    "node --check main.js && node --input-type=module --eval \"await Promise.all([import('@husklet/client'), import('@husklet/client/protocol')])\"",
  );
  execFileSync(
    'npm',
    [
  ...
  assert.equal(
    missingSocket.stderr,
    'client-starter: startup failed: HUSKLET_EXTENSION_SOCKET is not set; an extension runs inside a workspace\n',
  );
  ...
  assert(
    !dockerfile.includes('--platform='),
    'framework-neutral starter must inherit the selected image architecture',
  );
  assert.match(dockerfile, /COPY --chown=node:node package\.json/);
  assert.match(
    dockerfile,
    /COPY --chown=node:node node_modules\/@husklet\/client \.\/node_modules\/@husklet\/client/,
  );
  assert.match(dockerfile, /require\('@husklet\/client\/package\.json'\)\.version/);
  assert.doesNotMatch(
    dockerfile,
    /^RUN npm (?:ci|install)/m,
    'image build must not resolve the npm registry',
  );
