...
test('packaged external agent restores container after exact execution lifecycle over Unix framing', async () => {
  ...
  const server = net.createServer((socket) => {
    ...
    socket.write(encode({ channel: CONTROL, kind: KIND.open, payload: { protocol: 1, peer: 'container-agent', granted: ['workspaces:read', 'containers:read', 'containers:create', 'containers:execute', 'containers:lifecycle', 'containers:remove'] } }));
  });
