...
test('packaged external agent restores container after exact execution lifecycle over Unix framing', async () => {
  ...
  const server = net.createServer((socket) => {
    ...
    socket.on('data', (chunk) => {
      for (const frame of reader.take(chunk)) {
        ...
        if (call === 'workspace_info') {
          ...
          socket.write(
            encode({
              channel: 2,
              kind: KIND.response,
              payload: { reply: 'container', with: container('exited') },
            }),
          );
          ...
          assert.deepEqual(frame.payload.with, { id: containerId, generation: 4 });
          socket.write(
            encode({
              channel: call === 'container_start' ? 210 : 211,
              kind: KIND.event,
              payload: {
                snapshot: 'containers',
                of: [container(call === 'container_start' ? 'running' : 'exited')],
              },
            }),
          );
          socket.write(encode({ channel: 2, kind: KIND.response, payload: { reply: 'done' } }));
        ...
        } else if (call === 'execution_close_input') {
  ...
  try {
    const child = spawn(
      process.execPath,
      [example, JSON.stringify({ path: socketPath, containerId, command: ['printf', 'ok'] })],
    ...
    assert.equal(result.container, containerId);
    assert.equal(result.execution.id, executionId);
    assert.deepEqual(result.output.stdout, [111, 107]);
