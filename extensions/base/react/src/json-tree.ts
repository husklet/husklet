...
export const JSON_TREE_DEFAULTS = Object.freeze({
  maxDepth: 12,
  maxNodes: 500,
  maxStringLength: 512,
});
...
function scalar(value: unknown, type: string, limit: number) {
  ...
  if (type === 'number' && !Number.isFinite(value))
    return { display: String(value), truncated: false };
...
export function inspectJson(
  ...
  const maxStringLength = limitedInteger(
  ...
  const visit = (
    ...
    if (expandable && typeof current === 'object' && current !== null && seen.has(current)) {
      rows.push({
        ...
        type: 'circular',
        value: current,
        display: `[Circular → ${seen.get(current)}]`,
        expandable: false,
        childCount: 0,
        truncated: true,
      });
    ...
    const bounded = expandable
      ? {
          ...
          truncated: depth >= maxDepth && entries.length > 0,
        }
    ...
    rows.push({
      ...
      expandable: expandable && entries.length > 0 && depth < maxDepth,
  ...
  if (nodeLimit) {
    ...
    rows.push({
      ...
      type: 'truncated',
      value: undefined,
      display: `Node limit reached; showing at most ${maxNodes} rows.`,
      expandable: false,
      childCount: 0,
      truncated: true,
    });
  ...
  return {
    rows,
    limits: { maxDepth, maxNodes, maxStringLength },
    truncated: nodeLimit || rows.some((row) => row.truncated),
  };
}
...
export function JsonTree({
  ...
  return h(
    ...
    model.truncated
      ? h(InlineMessage, {
          label: `Inspection is bounded to ${model.limits.maxNodes} nodes, depth ${model.limits.maxDepth}, and ${model.limits.maxStringLength} characters per string. Truncated values are marked.`,
          tone: 'warning',
        })
      : null,
    rows.length === 0
      ? h(EmptyState, { label: 'No matching values', detail: 'Clear or broaden the filter.' })
      : null,
    h(
      Scroll,
      { height, grow: true },
        ...
        ...rows.map((row) =>
          h(
            Row,
            {
              key: row.path,
              gap: 1,
              pad: { start: { step: Math.min(row.depth, 16) } },
              align: 'center',
              wrap: true,
            },
            row.expandable
              ? h(Button, {
                  label: `${expanded.has(row.path) ? 'Collapse' : 'Expand'} ${row.path}`,
                  variant: 'ghost',
                  onInvoke: () => toggle(row.path),
                })
              : h(Text, { label: row.path, color: 'text-dim' }),
            h(Badge, { label: row.type, tone: tone(row.type) }),
            h(Button, {
              label: row.display,
              variant: 'ghost',
              onInvoke: () => onSelect?.({ path: row.path, type: row.type, value: row.value }),
            }),
            h(Button, {
              label: `Copy ${row.path}`,
              enabled: typeof onCopy === 'function',
              onInvoke: () =>
                onCopy?.({ path: row.path, type: row.type, value: row.value, text: copyText(row) }),
            }),
