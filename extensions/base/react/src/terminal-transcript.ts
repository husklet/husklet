...
export function TerminalTranscript({
  ...
  for (const [sourceIndex, source] of lines.slice(-TERMINAL_TRANSCRIPT_LINE_LIMIT).entries()) {
    ...
    if (line === null || typeof line !== 'object')
      throw new TypeError('TerminalTranscript lines must be strings or objects');
    ...
    retained.push({
      ...
      text,
      sourceIndex:
        lines.length - Math.min(lines.length, TERMINAL_TRANSCRIPT_LINE_LIMIT) + sourceIndex,
    });
  }
