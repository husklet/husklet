...
function rank(command: CommandPaletteItem, query: string): number | null {
  ...
  if (needle === '') return 0;
  const haystack =
    `${command.title} ${command.group ?? ''} ${(command.keywords ?? []).join(' ')}`.toLocaleLowerCase();
  let at = 0;
  let score = 0;
  let previous = -2;
...
export function filterCommands(
  ...
  return commands
    .slice(0, COMMAND_PALETTE_ITEM_LIMIT)
    .map((command, order) => {
      if (
        command === null ||
        typeof command !== 'object' ||
        String(command.id ?? '').length === 0
      ) {
      ...
      if (normalized.title.trim() === '')
        throw new TypeError('every command requires a nonblank title');
...
export function CommandPaletteView({
  ...
  const key = (event: unknown) => {
    ...
    if (pressed === 'ArrowDown')
      ...
      setActive((current) =>
        selectable.length === 0 ? 0 : (current + selectable.length - 1) % selectable.length,
      );
  ...
  return React.createElement(
    ...
    matches.length === 0
      ? React.createElement(EmptyState, {
          label: bounded(emptyLabel, COMMAND_PALETTE_TEXT_BYTE_LIMIT),
        })
      : React.createElement(
          List,
          { grow: true },
          ...[...groups].flatMap(([group, commandsInGroup]) => [
