...
export function CommandPaletteStory() {
  ...
  return h(Column, { gap: 2, grow: true, width: { maximum: { chars: 76 } } },
    h(Heading, { label: 'Keyboard-first workspace commands', scale: 'title' }),
    h(Text, { label: 'Fuzzy matching keeps grouped results, stable identities, disabled authority, and destructive semantics visible.', wrap: true }),
    h(CommandPaletteView, {
      commands,
