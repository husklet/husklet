...
export function boundedContainers(containers: readonly ContainerInput[]): BoundedContainer[] {
  return containers
    ...
    .map((container) => ({
      id: text(container.id, 80),
      name: text(container.name, 64),
      image: text(container.image, 120),
      ...
      processes: (container.processes ?? []).slice(0, PROCESS_LIMIT).map((process) => ({
        pid:
          typeof process.pid === 'number' && Number.isSafeInteger(process.pid) && process.pid >= 0
            ? process.pid
            : 0,
        command: text(process.command, 160),
        user: text(process.user, 48),
      })),
    }))
...
const sample = boundedContainers([
  {
    id: 'sha256:api-generation-42',
    name: 'api',
    image: 'team/api:1.8.2',
    state: 'running',
    logs: 'ready on :8080\nGET /health 200\n',
    processes: [
      { pid: 1, user: 'node', command: 'node server.js' },
      { pid: 31, user: 'node', command: 'worker --queue events' },
    ],
  },
  {
    id: 'sha256:worker-generation-19',
    name: 'worker',
    image: 'team/worker:1.8.2',
    state: 'paused',
    logs: 'queue lag: 14\n',
    processes: [{ pid: 1, user: 'node', command: 'worker --queue default' }],
  },
  {
    id: 'sha256:db-generation-7',
    name: 'database',
...
export function ContainerOperationsStory({
  containers = sample,
}: {
  ...
  const [status, setStatus] = useState(
    'Select a container, then inspect its bounded runtime state.',
  );
  ...
  return (
    <Column gap={2} grow={true}>
      ...
      <Text
        label={
          'Immutable container identity stays visible across inspection and control. Destructive stop requires a separate confirmation.'
        }
        wrap={true}
      />
      <Row gap={2} wrap={true} grow={true}>
            ...
            {inventory.map((container) => (
              <ListItemButton
                ...
                onInvoke={() => {
                  ...
                  setInspected(false);
                  setStatus(`Selected ${container.name}.`);
                }}
              />
            ))}
          </List>
        ...
        {selected ? (
              ...
              <Row gap={2} wrap={true}>
                <Button
                  ...
                  onInvoke={() => {
                    setInspected(true);
                    setStatus(
                      `Loaded ${selected.processes.length} bounded processes for ${selected.name}.`,
                    );
                  }}
                />
                <Button
          ...
          <InlineMessage label={'No containers are available.'} tone={'neutral'} />
        )}
      </Row>
