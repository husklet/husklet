...
export function DragReorderStory() {
  ...
  const record = (message: string) =>
    setEvents((current) => [...current, message].slice(-EVENT_LIMIT));
  ...
  return (
    <Scroll width={'fill'} height={'fill'}>
      <Column gap={2} grow={true}>
        ...
        <Text
          label={
            'Drag a card onto another card, or use Move up and Move down. Both paths apply the same bounded reorder.'
          }
          wrap={true}
        />
        {items.map((item, index) => (
          <Container
            ...
            onDrag={() => {
              setSource(item.id);
              record(`Drag source: ${item.id}`);
            }}
            onDrop={(event) => {
              if (!source) {
                record(`Drop ignored on ${item.id}: no source`);
                return;
              }
        ...
        <Column gap={1}>
          ...
          {(events.length ? events : ['No interactions yet.']).map((event, index) => (
