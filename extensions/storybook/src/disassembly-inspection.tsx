  ...
  return instructions
    .filter((instruction): instruction is Instruction => {
      ...
      return (
        ...
        Boolean(mnemonic.trim())
      );
    })
    .slice(0, INSTRUCTION_LIMIT)
    .map(
      ({ address, bytes, mnemonic, operands = '' }) =>
        `${address.toString(16).padStart(16, '0')}\t${bytes.map((byte) => byte.toString(16).padStart(2, '0')).join(' ')}\t${mnemonic.replace(/[\t\r\n]/g, ' ')}\t${String(operands).replace(/[\t\r\n]/g, ' ')}`,
    )
    .join('\n');
}
...
export function DisassemblyInspectionStory() {
  const value = boundedInstructions([
    ...
    {
      address: 0x401004,
      bytes: [0xe8, 0x27, 0, 0, 0],
      mnemonic: 'call',
      operands: '0x401030 <serve>',
    },
  ...
  return (
    <Column gap={2} grow={true}>
      ...
      <Text label={'Addresses, original bytes, mnemonics, and operands remain selectable.'} />
