...
export interface PullLayerInput {
  id?: unknown;
  current?: unknown;
  total?: unknown;
}
...
export function boundedPull(pull: PullInput): BoundedPull {
  ...
  return {
    ...
    platform: PLATFORMS.includes(pull.platform as Platform)
      ? (pull.platform as Platform)
      : PLATFORMS[0],
...
const initial = boundedPull({
  job: '42',
  reference: 'ghcr.io/team/api:1.8.2',
...
export function ImagePullStory() {
  ...
  const [message, setMessage] = useState(
    'Job 42 is pulling a manifest-selected platform with bounded layer progress.',
  );
  ...
  const fail = () => {
    setPull({
      ...pull,
      state: 'failed',
      error: 'registry authorization expired; no partial image became visible',
    });
  ...
  return (
    <Column gap={2} grow={true}>
      ...
      <Text
        label={
          'One explicit reference resolves to one workspace architecture. Progress, cancellation, errors, and the final digest remain tied to job 42.'
        }
        wrap={true}
      />
      <Select
        ...
        onChange={({ value }) =>
          choosePlatform(PLATFORMS.includes(value as Platform) ? (value as Platform) : PLATFORMS[0])
        }
      />
      <Card label={pull.reference} variant={'outline'}>
        <CardContent gap={2}>
          <CodeView value={`${pull.state} · ${pull.platform} · job ${pull.job}`} monospace={true} />
          {pull.layers.map((layer) => (
            <Column key={layer.id} gap={1}>
              <CodeView
                value={`${layer.id} · ${layer.current}/${layer.total} bytes`}
                monospace={true}
              />
