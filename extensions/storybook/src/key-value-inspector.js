...
export function KeyValueInspectorStory({ source }) {
  ...
  return h(Column, { gap: 2, grow: true },
    h(Heading, { label: 'Image manifest inspector', scale: 'title', wrap: true }),
    h(Text, { label: '256 logical properties; GTK requests at most 32 rows at a time.', wrap: true }),
    h(KeyValueTable, { source: KEY_VALUE_SOURCE, schema: KEY_VALUE_SCHEMA, grow: true }),
    h(Row, { gap: 2, wrap: true },
      h(Button, { label: 'Refresh metadata', onInvoke: () => setRefreshes((count) => count + 1) })),
    h(InlineMessage, {
