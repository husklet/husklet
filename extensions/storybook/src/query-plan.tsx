...
export const QUERY_PLAN_MODES = Object.freeze({
  full: 'Full plan',
  hotspot: 'Hotspots',
  mismatch: 'Estimate mismatches',
});
  ...
  children: [
    {
      id: 'join',
      operator: 'hash_join',
      label: 'Join customers to recent orders',
      relation: '',
      state: 'normal',
      ...
      children: [
        {
          id: 'customers',
          operator: 'index_scan',
          label: 'Customers by account',
          relation: 'customers_account_id_idx',
          state: 'normal',
          detail: 'account_id = $1',
          metrics: {
            estimated_rows: 1200,
        ...
        },
        {
          id: 'orders-hash',
          operator: 'hash',
          label: 'Hash recent orders',
          relation: '',
          state: 'normal',
          detail: 'batches=8 memory=4 MiB',
          metrics: {
            estimated_rows: 24000,
            actual_rows: 289440,
            cost: 702.8,
            duration_us: 11620,
            loops: 1,
    ...
    {
      id: 'preferences',
      operator: 'subquery_scan',
      label: 'Preference summary',
      relation: 'preferences',
      state: 'estimate_mismatch',
      detail: 'actual rows are 24× estimate',
      metrics: { estimated_rows: 50, actual_rows: 1184, cost: 29.8, duration_us: 230, loops: 1 },
      children: [],
    },
  ],
});
...
function selected(node: QueryPlanRecord, mode: QueryPlanMode): boolean {
  return (
    mode === 'full' ||
    (mode === 'hotspot' && node.state === 'hot') ||
    (mode === 'mismatch' && node.state === 'estimate_mismatch')
  );
}

/** Keeps matching operators and their complete root path, but no unrelated siblings. */
export function filterQueryPlan(
  node: QueryPlanRecord,
  mode: QueryPlanMode,
): QueryPlanRecord | null {
...
function count(node: QueryPlanRecord): number {
  ...
  return (
    ...
    >
      {Object.entries(node.metrics).map(([label, value]) => (
        <QueryPlanMetric key={label} label={label} value={value} />
      ))}
...
export function QueryPlanStory() {
  ...
  return (
    <Column gap={2}>
      ...
      <Row gap={1} wrap={true}>
        {(Object.entries(QUERY_PLAN_MODES) as [QueryPlanMode, string][]).map(([key, label]) => (
          <Button key={key} label={label} enabled={mode !== key} onInvoke={() => setMode(key)} />
        ))}
      </Row>
      <InlineMessage
        label={
          mode === 'full'
            ? 'Showing the complete captured plan.'
            : `Showing ${QUERY_PLAN_MODES[mode].toLowerCase()} with their ancestor paths.`
        }
      />
      <QueryPlan
