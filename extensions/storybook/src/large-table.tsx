...
export function LargeDataTableStory({ source }) {
  ...
  return (
    <Column gap={2} grow={true}>
      <Heading label={'100,000 logical records'} scale={'title'} wrap={true} />
      <Text
        label={'Only host-requested 128-row windows exist in memory. Resize or scroll to request another window.'}
        wrap={true} />
      <Row gap={2} wrap={true}>
        <Entry
          value={filter}
      ...
      {interactions.length === 0
        ? [<InlineMessage key={'interaction-empty'}
