...
export function boundedGraph(value: unknown): BoundedGraph {
  ...
  for (const value of rawNodes) {
    ...
    nodes.push({
      ...
      version: clean(node.version, 64),
      state: clean(node.state, 40),
      detail: clean(node.detail, 160),
    });
  }
  ...
  for (const value of rawEdges) {
    ...
    edges.push({
      source,
      target,
      relation: clean(edge.relation, 64),
      requirement: clean(edge.requirement, 120),
    });
  }
  ...
  const cycles = rawCycles
    .filter(
      (value): value is string[] =>
        Array.isArray(value) &&
        value.length > 0 &&
        value.length <= MEMBER_LIMIT &&
        value.every((id) => typeof id === 'string' && ids.has(id)) &&
        value.every((id, index) =>
          edges.some(
            (edge) => edge.source === id && edge.target === value[(index + 1) % value.length],
          ),
        ),
    )
    .slice(0, CYCLE_LIMIT)
  ...
  return {
    ...
    cycles,
    totals: {
      nodes: total(totals?.nodes, nodes.length),
      edges: total(totals?.edges, edges.length),
      cycles: total(totals?.cycles, cycles.length),
    },
  };
}
...
const data = boundedGraph({
  nodes: [
    { id: 'app', label: 'app', version: '1.0', state: 'resolved', detail: 'root' },
    {
      id: 'react',
      label: 'react',
      version: '18.3',
      state: 'conflict',
      detail: '18 and 19 requested',
    },
    {
      id: 'scheduler',
      label: 'scheduler',
  ...
  edges: [
    { source: 'app', target: 'react', relation: 'runtime', requirement: '^18' },
    { source: 'react', target: 'scheduler', relation: 'peer', requirement: '^0.23' },
    { source: 'scheduler', target: 'react', relation: 'runtime', requirement: '*' },
  ],
  cycles: [['react', 'scheduler']],
  totals: { nodes: 87, edges: 240, cycles: 2 },
});
    ...
    <Column gap={2} grow={true}>
      ...
      >
        {nodes.map((node) => (
          <DependencyNode
            key={node.id}
            label={`${node.label}@${node.version}`}
            value={`id=${node.id} state=${node.state} detail=${node.detail}`}
          >
            {data.edges
        ...
        {data.cycles.map((cycle, index) => (
          <DependencyCycle
            key={`c${index}`}
            label={`cycle ${index + 1}`}
            detail={`${cycle.length} members`}
          >
            {cycle.map((member, position) => (
              <DependencyCycleMember key={position} label={member} value={`position=${position}`} />
            ))}
          </DependencyCycle>
        ))}
      </DependencyGraph>
