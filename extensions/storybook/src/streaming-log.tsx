...
export function StreamingLogStory() {
  ...
  const [status, setStatus] = useState(
    'Initial history exceeds retention so the oldest text is evicted.',
  );
  const append = () => {
    sequence.current += 1;
    ...
    setChunk(next);
    setStatus(
      `Appended batch ${sequence.current}; ${next.length}/${LOG_CHUNK_LIMIT} wire characters.`,
    );
  };
  return (
    <Column gap={2} grow={true}>
      ...
      <Text
        label={
          'Each update sends only new text. GTK retains the newest 4,096 characters and follows the tail.'
        }
        wrap={true}
      />
