...
const initial: readonly TerminalTranscriptLine[] = [
  ...
  },
  {
    id: 'warning',
    number: 421,
    timestamp: '12:04:08.414',
    text: 'warning: output retained from bounded tail',
    stream: 'stderr',
  },
...
export function TerminalTranscriptStory() {
  ...
  return (
    <Column gap={2} grow={true}>
      ...
      <Text
        label={
          'A bounded native projection with exact cursor placement, stream tone, selection, and explicit actions.'
        }
        wrap={true}
      />
      <FormControlLabel label={'Show timestamps'}>
        <Switch checked={timestamps} onToggle={(event) => setTimestamps(Boolean(event.value))} />
      </FormControlLabel>
      <TerminalTranscript
        lines={initial}
        lineNumbers={true}
        ...
        droppedLines={413}
        onSelect={(line) => {
          setSelected(String(line.id ?? line.number ?? ''));
          setStatus(`Selected immutable line ${line.number}.`);
        }}
        actions={[
          {
            id: 'copy',
            label: 'Copy visible',
            onInvoke: () => setStatus('Copied the bounded visible projection.'),
          },
          {
            id: 'clear',
            ...
            tone: 'danger',
            destructive: true,
            onInvoke: () => setStatus('Clear requested; confirmation remains a host concern.'),
          },
        ]}
      />
