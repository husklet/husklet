...
export function EntryWorkbench() {
  ...
  const valid = /^[a-z][a-z0-9-]{2,31}$/.test(value);
  return (
    <ComponentDocument
      name="Entry"
    ...
    >
      <DocumentationSection title="Overview">
        <Code
          value={'<Entry value={name} placeholder="extension-name" onChange={setName} />'}
          wrap
        />
        <FieldSpecimen label="Extension name" helper={event} width={widths[width]}>
          <Entry
            ...
            width={widths[width]}
            align="start"
            tone={tone === 'danger' || !valid ? 'danger' : 'neutral'}
            enabled={enabled}
            secret={secret}
      ...
      <DocumentationSection title="States">
        <SpecimenGrid>
          <FieldSpecimen label="Empty with placeholder" width={{ chars: 30 }}>
            <Entry value="" placeholder="extension-name" width={{ chars: 30 }} />
          </FieldSpecimen>
          <FieldSpecimen
            label="Focused"
            helper="Keyboard focus keeps a visible ring"
            width={{ chars: 30 }}
          >
            <Entry value="workspace-api" tooltip="Focused extension name" width={{ chars: 30 }} />
          </FieldSpecimen>
          <FieldSpecimen label="Valid" helper="Name is available" width={{ chars: 30 }}>
            <Entry value="workspace-api" width={{ chars: 30 }} />
          </FieldSpecimen>
          <FieldSpecimen
            label="Error"
            ...
            width={{ chars: 30 }}
          >
            <Entry value="Workspace API" tone="danger" width={{ chars: 30 }} />
          </FieldSpecimen>
          <FieldSpecimen
            label="Disabled"
            ...
            width={{ chars: 30 }}
          >
            <Entry value="workspace-api" enabled={false} width={{ chars: 30 }} />
          </FieldSpecimen>
          <FieldSpecimen label="Secret" helper="Value remains concealed" width={{ chars: 30 }}>
            <Entry value="token-value" secret width={{ chars: 30 }} />
          </FieldSpecimen>
        </SpecimenGrid>
      </DocumentationSection>
