...
export function ConfirmationStory() {
  ...
  return h(Column, { gap: 2 },
    h(Heading, { label: 'Remove cache volume', scale: 'title' }),
    h(Text, { label: 'The immutable volume generation is shown before any destructive request.', wrap: true }),
    removed
      ? h(InlineMessage, { label: 'Volume cache was removed.', tone: 'positive' })
      : h(ConfirmAction, {
