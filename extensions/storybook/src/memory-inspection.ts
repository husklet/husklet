...
export function MemoryInspectionStory() {
  ...
  return h(Column, { gap: 2, grow: true },
    h(Heading, { label: 'Process address space', scale: 'title' }),
    h(Text, { label: 'Exact ranges, access permissions, byte sizes, and mappings remain selectable.' }),
    h(MemoryMap, { value, tone: 'accent', grow: true }),
    h(InlineMessage, { label: `Showing 4 of at most ${REGION_LIMIT} regions` }),
  );
}
