...
function render(node) {
  ...
  }, ...Object.entries(node.metrics).map(([label, value]) => h(QueryPlanMetric, { key: label, label, value })), ...node.children.map(render));
}
...
export function QueryPlanStory() {
  ...
  return h(Column, { gap: 2 },
    ...
    h(Row, { gap: 1, wrap: true }, ...Object.entries(QUERY_PLAN_MODES).map(([key, label]) => h(Button, {
      key, label, enabled: mode !== key, onInvoke: () => setMode(key),
    }))),
    h(InlineMessage, { label: mode === 'full' ? 'Showing the complete captured plan.' : `Showing ${QUERY_PLAN_MODES[mode].toLowerCase()} with their ancestor paths.` }),
    h(QueryPlan, { label: `${shown} plan operators`, detail: `bounded source: showing ${shown} of 84 operators` }, render(filtered)));
}
