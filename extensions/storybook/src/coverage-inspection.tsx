...
export const COVERAGE_STORY = 'Inspect source coverage';
export const COVERAGE_LIMIT = 512;
export const SOURCE_LIMIT = 512;
...
export function boundedCoverage(lines: readonly unknown[], total: number = lines.length): string {
  ...
  const kept = lines
    .filter((entry): entry is CoverageLine => {
  ...
  if (Number.isSafeInteger(total) && total > kept.length)
    rows.push(`…\t\t… showing ${kept.length} of ${total} lines …`);
  return rows.join('\n');
}
export function CoverageInspectionStory() {
  const value = boundedCoverage(
    [
      { line: 41, hits: 8, source: 'match request {' },
      { line: 42, hits: 8, source: '    Request::Ready => serve(),' },
      { line: 43, hits: 0, source: '    Request::Retry => retry(),' },
      { line: 44, hits: 8, source: '}' },
    ],
    287,
  );
  return (
    <Column gap={2} grow={true}>
      ...
      <Text label={'Covered and missed lines remain selectable with exact hit counts.'} />
