...
export class FileSource {
  ...
  async publish() {
    await this.send('source_resize', {
      mutation: { Length: { source: FILE_SOURCE, version: this.version, rows: this.files.length } },
    });
  }
  ...
  answer(value: unknown) {
    ...
    const count = Math.min(
      request.range.count,
      FILE_WINDOW_LIMIT,
      Math.max(0, this.files.length - request.range.start),
    );
    const rows = this.files
      .slice(request.range.start, request.range.start + count)
      .map((file, offset) => ({
