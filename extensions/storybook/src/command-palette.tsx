...
const commands = [
  ...
  },
  {
    ...
    title: 'Restart selected container',
    group: 'Containers',
    keywords: ['reboot'],
  },
  {
  ...
  },
  {
    ...
    group: 'Remote',
    disabled: true,
    detail: 'No remote authority granted.',
  },
  {
    id: 'workspace.remove',
    ...
    group: 'Danger',
    destructive: true,
    detail: 'Requires explicit confirmation.',
  },
];
...
export function CommandPaletteStory() {
  ...
  return (
    <Column gap={2} grow={true} width={{ maximum: { chars: 76 } }}>
      ...
      <Text
        label={
          'Fuzzy matching keeps grouped results, stable identities, disabled authority, and destructive semantics visible.'
        }
        wrap={true}
      />
      <CommandPaletteView
        ...
        onQueryChange={(query) =>
          setStatus(query ? `Filtering for “${query}”` : 'Showing all commands.')
        }
        onSelect={(command) =>
          setStatus(
            command.destructive
              ? `Review required for ${command.title}.`
              : `Invoked ${command.title}.`,
          )
        }
      />
