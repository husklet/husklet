...
export function WorkspaceFileEditStory() {
  ...
  const save = () => {
    if (!selected) return;
    setFiles((current) =>
      current.map((file) => (file.path === selected.path ? { ...file, content: draft } : file)),
    );
  ...
  const rename = () => {
    ...
    setFiles((current) =>
      current.map((file) => (file.path === selected.path ? { ...file, path: next } : file)),
    );
  ...
  return (
    <Column gap={2} grow={true}>
      ...
      <Text
        label={
          'Every operation stays inside the selected workspace and uses a normalized relative path. Content and inventory are independently bounded.'
        }
        wrap={true}
      />
            ...
            {files.map((file) => (
              <ListItemButton
                ...
                onInvoke={() => {
                  ...
                  setStatus(`Read ${file.content.length} bounded bytes from ${file.path}.`);
                }}
              />
            ))}
          </List>
          ...
          <Column gap={2} grow={true}>
            ...
            <Row gap={2} wrap={true}>
              ...
              <ConfirmAction
                ...
                onConfirm={async () =>
                  setStatus(`Deletion confirmed for confined path ${selected.path}.`)
                }
              />
            </Row>
          </Column>
