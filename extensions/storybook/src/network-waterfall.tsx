...
function validPhase(value: unknown): value is NetworkPhaseRecord {
  ...
  return (
    ...
    (PHASES as readonly string[]).includes(phase.kind) &&
    Number.isSafeInteger(phase.offsetUs) &&
    Number(phase.offsetUs) >= 0 &&
    Number.isSafeInteger(phase.durationUs) &&
    Number(phase.durationUs) > 0 &&
...
function validRequest(value: unknown): value is NetworkRequestRecord {
  ...
  if (
    ...
    !clean(request.url).trim() ||
    !Number.isSafeInteger(request.startUs) ||
    Number(request.startUs) < 0 ||
    !Number.isSafeInteger(request.durationUs) ||
    Number(request.durationUs) <= 0 ||
    ...
    Number(request.bytes) > 2 ** 40 ||
    !Array.isArray(request.phases) ||
    request.phases.length > PHASE_LIMIT ||
    !request.phases.every(validPhase)
  )
    return false;
  const phases = request.phases;
  return phases.every(
    (phase, index) =>
      phase.offsetUs + phase.durationUs <= Number(request.durationUs) &&
...
export function boundedRequests(
  ...
  totalRequests: number = requests.length,
) {
  const valid = requests.filter(validRequest);
  const declaredTotal =
    Number.isSafeInteger(totalRequests) && totalRequests >= 0 ? totalRequests : valid.length;
...
const fixture = boundedRequests(
  [
    {
      method: 'GET',
      url: 'https://api.example.test/users?limit=20',
      startUs: 0,
      durationUs: 184000,
      status: 200,
      bytes: 18432,
      ...
      phases: [
        { kind: 'dns', offsetUs: 0, durationUs: 12000 },
        { kind: 'connect', offsetUs: 12000, durationUs: 24000 },
        { kind: 'tls', offsetUs: 36000, durationUs: 31000 },
        { kind: 'request', offsetUs: 67000, durationUs: 4000 },
        { kind: 'wait', offsetUs: 71000, durationUs: 98000 },
        { kind: 'download', offsetUs: 169000, durationUs: 15000 },
      ],
    },
    {
      method: 'POST',
      ...
      phases: [
        { kind: 'connect', offsetUs: 0, durationUs: 18000 },
        { kind: 'request', offsetUs: 18000, durationUs: 9000 },
        { kind: 'wait', offsetUs: 27000, durationUs: 69000 },
      ],
    },
    {
      method: 'GET',
      ...
      status: 304,
      bytes: 0,
      detail: 'cache validated',
      phases: [
        { kind: 'request', offsetUs: 0, durationUs: 3000 },
        { kind: 'wait', offsetUs: 3000, durationUs: 72000 },
      ],
    },
  ],
  47,
...
export function NetworkWaterfallStory() {
  ...
  const shown = failedOnly
    ? fixture.requests.filter((request) => (request.status ?? 0) >= 400)
    : fixture.requests;
  return (
    <Column gap={2} grow={true}>
      <Heading label={'Network request waterfall'} scale={'title'} />
      <Text label={'Inspect exact request timing, transfer metadata, and ordered phases.'} />
      <Row gap={1}>
        <Button
