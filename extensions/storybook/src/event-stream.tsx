...
export class TimelineSource {
  ...
  async publish() {
    await this.send('source_resize', {
      mutation: {
        Length: { source: EVENT_SOURCE, version: this.version, rows: EVENT_RETENTION_LIMIT },
      },
    });
  }
  ...
  answer(request: unknown) {
    if (
      !isWindowRequest(request) ||
      request.source !== EVENT_SOURCE ||
  ...
  row(index: number) {
    ...
    return {
      ...
      cells: [
        ...
        {
          Badge: {
            label: sequence % 5 ? 'completed' : 'warning',
            tone: sequence % 5 ? 'positive' : 'warning',
          },
        },
...
export function EventStreamStory({ source }: { source: TimelineSource }) {
  ...
  return (
    <Column gap={2} grow={true}>
      ...
      <Text
        label={
          'The source retains 10,000 logical events; GTK requests and renders only a 64-row window.'
        }
        wrap={true}
      />
      ...
      <InlineMessage
        label={
          acknowledged
            ? `Acknowledged newest event ${acknowledged} time${acknowledged === 1 ? '' : 's'}.`
            : 'No event acknowledged.'
        }
