...
export function DisassemblyInspectionStory() {
  ...
  return h(Column, { gap: 2, grow: true },
    h(Heading, { label: 'Decoded entry point', scale: 'title' }),
    h(Text, { label: 'Addresses, original bytes, mnemonics, and operands remain selectable.' }),
    h(DisassemblyView, { value, tone: 'accent', grow: true }),
    h(InlineMessage, { label: `Showing 4 of at most ${INSTRUCTION_LIMIT} instructions` }),
  );
}
