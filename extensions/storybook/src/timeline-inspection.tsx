...
export function boundedEvents(events: readonly unknown[]): string {
  ...
  return events
    .filter((event): event is TimelineEvent => {
      if (event === null || typeof event !== 'object') return false;
      const { timestampMs, label } = event as Record<string, unknown>;
      return (
        Number.isSafeInteger(timestampMs) && typeof label === 'string' && Boolean(label.trim())
      );
    })
    .slice(0, TIMELINE_LIMIT)
    .map(
      ({ timestampMs, category = '', label, detail = '' }) =>
        `${timestampMs}\t${clean(category)}\t${clean(label)}\t${clean(detail)}`,
    )
    .join('\n');
}
