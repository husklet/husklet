 ...
 */
export async function mountConcurrentSurfaces(session: Session) {
  let overview!: RenderHandle;
  overview = render(<Counter name={'Overview'} close={() => overview.close()} />, session, {
    title: 'Concurrent surfaces',
  });
  const overviewSlot = await overview.ready;
  const details = render(<Counter name={'Details'} />, session, {
