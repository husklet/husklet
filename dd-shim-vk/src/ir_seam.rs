...
//! from the Vulkan object/command model onto the shared [`dd_shim_common::ir`] vocabulary (which is
//! `dd_gpu::ir` re-exported verbatim — the guest producer and host executor share ONE Rust type and
...
mod tests {
    use super::*;
    use dd_shim_common::ir::{decode_stream, encode_stream};
    ...
    /// compute stream with the guest producer, then decode it with the HOST's own `dd_gpu::ir`
