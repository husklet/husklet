//! Default-stub bookkeeping for the generated long-tail `cu*` entry points.
//!
//! A stub keeps the ABI intact (the app links and runs) but does no work and returns `CUDA_SUCCESS`.
//! To make the long tail *visible* (so we know which entry points a given CUDA app actually calls, and
//! in what order, to prioritize porting), each stub reports itself once when `DD_SHIM_DEBUG` is set.
//! Identical mechanism to dd-shim-gl's `stub.rs`.
