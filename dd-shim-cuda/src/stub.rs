...
//!     reports the unsupported call here. With `DD_SHIM_STRICT=1` the process aborts at the first such
...
/// `DD_SHIM_STRICT=1` (any non-empty value) is set — abort at the first unsupported CUDA call.
...
/// `DD_SHIM_DEBUG`, and — under `DD_SHIM_STRICT=1` — prints the full report and aborts the process
