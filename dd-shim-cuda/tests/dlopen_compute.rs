...
//! binaries but not the cdylib; a prior `cargo build -p dd-shim-cuda`, as the deploy/CI does, produces
...
fn deployed_libcuda_so_runs_vecadd_end_to_end() {
    let Some(lib) = cdylib() else {
        eprintln!("[dlopen_compute] libcuda cdylib not built (run `cargo build -p dd-shim-cuda`); skipping");
        return;
    };
