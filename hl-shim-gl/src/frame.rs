...
fn build_single_draw_frame(s: &GlState) -> Option<Vec<u8>> {
    ...
    if has_bg {
        ...
        if has_u {
            entries.push(hl_shim::ir::BindEntry {
                binding: 1,
                resource: hl_shim::ir::BindResource::Buffer { id: 11, offset: 0, size: prog.ubuf_size as u64 },
            });
        }
        for (k, &t) in texlist.iter().enumerate() {
            entries.push(hl_shim::ir::BindEntry {
                binding: k as u32,
                resource: hl_shim::ir::BindResource::Texture { id: tex_ir_id(t) },
            });
            entries.push(hl_shim::ir::BindEntry {
                binding: k as u32,
                resource: hl_shim::ir::BindResource::Sampler { id: crate::wireenc::sampler_ir_id(t) },
            });
        }
        cmds.push(Cmd::CreateBindGroup(40, hl_shim::ir::BindGroupDesc { set: 0, entries }));
    }
    ...
    if has_bg {
        for (d_i, d) in draws.iter().enumerate() {
            ...
            if draw_has_u {
                entries.push(hl_shim::ir::BindEntry {
                    binding: 1,
                    resource: hl_shim::ir::BindResource::Buffer { id: 1000 + d_i as u32, offset: 0, size: dpr.ubuf_size as u64 },
                });
            }
            for (k, &ti) in dtex.iter().enumerate() {
                let gltex = texlist[ti];
                entries.push(hl_shim::ir::BindEntry { binding: k as u32, resource: hl_shim::ir::BindResource::Texture { id: tex_ir_id(gltex) } });
                entries.push(hl_shim::ir::BindEntry { binding: k as u32, resource: hl_shim::ir::BindResource::Sampler { id: crate::wireenc::sampler_ir_id(gltex) } });
            }
            cmds.push(Cmd::CreateBindGroup(40 + d_i as u32, hl_shim::ir::BindGroupDesc { set: 0, entries }));
        }
    }
