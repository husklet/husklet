...
//! `hl_gpu::ir::Cmd` sequence `gl_shim.c` emits for it at swap. These are pure functions (state in,
...
use hl_shim::ir::{buffer_usage, texture_usage, BufferDesc, Cmd, SamplerDesc, TextureDesc};
use hl_shim::ir::{AddressMode, Filter, TextureDim, TextureFormat};
...
mod tests {
    use super::*;
    use hl_shim::ir::{encode_stream, Cmd};
    use hl_shim::wire::{Decoder, Encoder};
