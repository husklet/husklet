...
#
# This is the flagged cutover selector: a validation run sets HL_SHIM_IMPL=rust to swap in the Rust
...
if [ "${HL_SHIM_IMPL:-}" != "rust" ]; then
    echo "HL_SHIM_IMPL != rust — leaving the default (C shim) GLES libs in place."
    exit 0
fi
