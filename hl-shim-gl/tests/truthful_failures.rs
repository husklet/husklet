...
use hl_shim_gl::gles::{glGetError, glGetString};
...
fn stub_raises_error_and_initializes_outputs() {
    ...
    hl_shim_gl::glGetProgramInterfaceiv(1, 0x92E1 /* GL_UNIFORM_BLOCK */, 0, &mut params as *mut i32);
    ...
    hl_shim_gl::glGenProgramPipelines(2, ids.as_mut_ptr());
    ...
    let prog = hl_shim_gl::glCreateShaderProgramv(0x8B31 /* GL_VERTEX_SHADER */, 1, core::ptr::null());
    ...
    let cname = b"Blk\0";
    let idx = hl_shim_gl::glGetProgramResourceIndex(
        1,
    ...
    let img = hl_shim_gl::eglCreateImage(
        core::ptr::null_mut(),
        core::ptr::null_mut(),
        0,
        core::ptr::null_mut(),
...
fn advertised_gl_version_matches_inventory_profile() {
    ...
    assert_eq!(tokens.len(), hl_shim_gl::ADVERTISED_GL_EXTENSION_COUNT);
    ...
    for (i, tok) in tokens.iter().enumerate() {
        let p = hl_shim_gl::gles::glGetStringi(0x1F03, i as u32);
    ...
    let oob = hl_shim_gl::gles::glGetStringi(0x1F03, tokens.len() as u32);
