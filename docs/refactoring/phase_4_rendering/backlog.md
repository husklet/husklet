...
| R5 | default multi-process Chrome content | window/browser UI renders, but renderer content stays white; engine micro-gate localizes an intermittent level-triggered epoll re-registration lost-wakeup | implement and prove the engine fix in [`chrome-fix-plan.md`](chrome-fix-plan.md), then repeated no-argument Chrome content/input/menu/resize/recovery/shutdown journey |
