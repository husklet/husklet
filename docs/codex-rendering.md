...
| `compositor_surface_teardown_reclaims_cpu_gpu_window_and_fence_state` | missing | Smithay surface/client destruction leaves no complete presentation cleanup path | Idempotent per-surface cleanup plus client-owned GPU resource teardown |
| `compositor_surface_keys_include_client_identity_and_generation` | contradictory | global caches use client-local wl_surface protocol ids, allowing cross-client collisions | Owned generational surface key plus deliberate same-id two-client test |
