...
pub struct HlState {
    ...
    pub(crate) visibility: HashMap<u32, hl_display::present::SurfaceVisibility>,
...
mod zero_copy_release_tests {
    ...
    fn map_and_commit(
        ...
        let fd = hl_display::keymap::anon_fd_with(&vec![0x20u8; size]).unwrap();
        let pool = c.alloc();
...
mod region_occlusion_and_pacing_tests {
    ...
    impl Harness {
        ...
        fn shm_buffer(&mut self, w: i32, h: i32) -> u32 {
            let stride = w * 4;
            let size = (stride * h) as usize;
            let fd = hl_display::keymap::anon_fd_with(&vec![0x30u8; size]).unwrap();
...
mod xwayland_window_model_tests {
    ...
    fn adopted_x11_window_presents_focuses_and_withdraws() {
        ...
        let stride = w * 4;
        let size = (stride * h) as usize;
        let fd = hl_display::keymap::anon_fd_with(&vec![0x50u8; size]).unwrap();
        let pool = alloc!();
...
mod input_routing_tests {
    ...
    fn split_client_routing_and_geometry_mirror() {
        ...
        let stride = w * 4;
        let size = (stride * h) as usize;
        let fd = hl_display::keymap::anon_fd_with(&vec![0u8; size]).unwrap();
        let pool = b.alloc();
