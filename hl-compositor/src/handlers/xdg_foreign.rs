...
//! and hl supplies exactly that: Smithay mints a real 32-char export handle, tracks the exported surface,
//! resolves `import_toplevel(handle)` back to it, and applies `set_parent_of` through the same
//! `XdgToplevelSurfaceData.parent` field the xdg-shell window model already honours. hl's contribution is
