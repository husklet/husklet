...
//! `hl_display::explicit_sync_bridge` (an `MTLSharedEvent`); that interop is mac-gated and not exercised
...
/// `MTLSharedEvent` wait on the Metal queue (see `hl_display::explicit_sync_bridge`).
