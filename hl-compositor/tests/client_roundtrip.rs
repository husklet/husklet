...
//! client (built on `hl_display::wire`) connects over a `socketpair` that we hand to Smithay's
...
fn globals_advertise_frame_presents_feedback_and_cursor_shape_wire() {
    ...
    let mfd = hl_display::keymap::anon_fd_with(&pixels).expect("anon shm fd");
    ...
    *cursor_hidden.lock().unwrap() = None;
    ...
    let cmfd = hl_display::keymap::anon_fd_with(&cpixels).expect("cursor anon shm fd");
    let cpool = c.alloc();
...
impl hl_display::present::Presenter for RecordingPresenter {
    fn present(
        &mut self,
        surf: &hl_display::present::SurfaceBuffer,
    ) -> Result<hl_display::present::PresentOutcome, hl_display::present::PresentError> {
        self.frames += 1;
        *self.last_damage.lock().unwrap() = Some(surf.damage);
        *self.last_present_sid.lock().unwrap() = Some(surf.sid);
        Ok(hl_display::present::PresentOutcome::Delivered {
