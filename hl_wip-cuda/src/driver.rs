...
impl Cuda {
    ...
    fn staged(&self, family: &str, soname: &str) -> PathBuf {
        self.spec.stage_root.join(family).join(self.spec.arch.dir()).join(soname)
    }
}
...
impl Driver for Cuda {
    fn device_request(&self, guest_env: &[String]) -> DeviceRequest {
        ...
        let mut mounts = vec![
            DeviceMount::ro(
                self.staged("cuda", "libcuda.so.1").to_string_lossy().into_owned(),
                format!("{libdir}/libcuda.so.1"),
            ),
            DeviceMount::ro(
                self.staged("cuda", "libcudart.so.1").to_string_lossy().into_owned(),
                format!("{libdir}/libcudart.so.1"),
            ),
            DeviceMount::ro(
                self.staged("nvml", "libnvidia-ml.so.1").to_string_lossy().into_owned(),
        ...
        DeviceRequest { mounts, env, render_node: false }
    }
...
mod tests {
    ...
    fn device_request_binds_the_three_sonames_and_socket() {
        ...
        let want = [
            ("/opt/hlroot/cuda/aarch64/libcuda.so.1", format!("{libdir}/libcuda.so.1"), true),
            ("/opt/hlroot/cuda/aarch64/libcudart.so.1", format!("{libdir}/libcudart.so.1"), true),
            ("/opt/hlroot/nvml/aarch64/libnvidia-ml.so.1", format!("{libdir}/libnvidia-ml.so.1"), true),
        ];
        for (host, container, ro) in want {
            assert!(
                req.mounts.iter().any(|m| m.host == host && m.container == container && m.read_only == ro),
        ...
        assert!(req
            .mounts
            .iter()
            .any(|m| m.host == "/tmp/hl-gpu.sock" && m.container == "/run/hl-gpu.sock" && !m.read_only));
    }
