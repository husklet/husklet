...
//! ([`hl_gpu::protocol::model::kernel`]) whose interpreter lives in the `hl_wip-gpu` executor — and that
...
//! through the kernel interpreter; that would require an `hl_wip-gpu` change (out of this crate's scope).
//!
...
impl SamplerDesc {
    ...
    pub fn point_clamp() -> Self {
        Self { filter: FilterMode::Point, ..Self::linear_clamp() }
    }
}
