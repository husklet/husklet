...
pub fn add_memcpy_htod_node(graph: &mut Graph, dst: DevicePtr, data: &[u8]) -> usize {
    graph.add(GraphNode::MemcpyHtoD { dst, data: data.to_vec() })
}
...
pub fn add_memset_node(graph: &mut Graph, dst: DevicePtr, pattern: &[u8]) -> usize {
    graph.add(GraphNode::Memset { dst, pattern: pattern.to_vec() })
}
...
pub fn add_kernel_node(
    ...
    graph.add(GraphNode::Kernel { func, grid, block, args })
}
...
pub fn instantiate(graph: &Graph) -> ExecGraph {
    ExecGraph { nodes: graph.nodes.clone() }
}
...
pub fn launch_graph(ctx: &mut CudaContext, sink: &mut dyn CommandSink, exec: &ExecGraph) -> Result<()> {
    ...
    for node in &exec.nodes {
        match node {
            ...
            GraphNode::Kernel { func, grid, block, args } => {
