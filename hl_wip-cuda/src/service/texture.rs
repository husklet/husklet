...
pub fn malloc_array(width: u32, height: u32) -> Result<CudaArray> {
    ...
    Ok(CudaArray { id: next_handle(), width, height, texels: vec![0.0; n] })
}
...
pub fn memcpy_to_array(array: &mut CudaArray, src: &[f32]) -> Result<()> {
    ...
    if src.len() != expect {
        return Err(GpuError::Invalid("cudaMemcpy2DToArray: source length != array texel count"));
    }
...
pub fn create_texture_object(array: &CudaArray, desc: SamplerDesc) -> TextureObject {
    TextureObject { id: next_handle(), array: array.clone(), desc }
}
