...
const SHIMS: &[Shim] = &[
    Shim { dir: "shim/cuda", lib: "libhl_cuda_guest.so", family: "cuda", soname: "libcuda.so.1" },
    Shim { dir: "shim/cudart", lib: "libhl_cudart_guest.so", family: "cuda", soname: "libcudart.so.1" },
    Shim { dir: "shim/nvml", lib: "libhl_nvml_guest.so", family: "nvml", soname: "libnvidia-ml.so.1" },
];
...
fn main() {
    ...
    for s in SHIMS {
        println!("cargo:rerun-if-changed={}", manifest_dir.join(s.dir).join("src").display());
        println!("cargo:rerun-if-changed={}", manifest_dir.join(s.dir).join("registry").display());
        println!("cargo:rerun-if-changed={}", manifest_dir.join(s.dir).join("build.rs").display());
    }
    ...
    for (triple, linker, arch_dir) in ARCHES {
        ...
        if !std_available(&sysroot, triple) {
            ...
            println!(
                "cargo:warning=hl_wip-cuda: rust std for {triple} not installed (no rustup on this host); \
            ...
            match build_shim(&cargo, &manifest_dir, &shim_target, s, triple, linker) {
                Ok(()) => {
                    if let Err(e) = stage(&shim_target, &stage_root, s, triple, arch_dir) {
                        ...
                        println!("cargo:warning=hl_wip-cuda: staging {} for {triple} failed: {e}", s.soname);
                        built_ok = false;
                    }
                }
                Err(e) => {
                    ...
                    println!("cargo:warning=hl_wip-cuda: building {} for {triple} failed: {e}", s.dir);
                    built_ok = false;
                    break;
                }
            }
        ...
        if built_ok {
            println!(
                "cargo:warning=hl_wip-cuda: staged guest shims for {triple} -> {}",
                stage_root.display()
            );
        }
    }
