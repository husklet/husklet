...
fn batched_strided_gemm_exact() {
    ...
    let a: Vec<i32> = (0..batch * sa).map(|i| (i as i32 * 7 + 3).rem_euclid(19) - 9).collect();
    let b: Vec<i32> = (0..batch * sb).map(|i| (i as i32 * 5 + 1).rem_euclid(19) - 9).collect();
    ...
    assert_eq!(got, want, "strided-batched GEMM: every element of every batch exact");
...
fn conv2d_nchw_multichannel_exact() {
    ...
    let weight: Vec<i32> = (0..cout * cin * 9).map(|i| (i as i32 * 7 + 2) % 9 - 4).collect();
    ...
    launch::launch(&mut ctx, &mut sink, func, (1, 1, cout as u32), (8, 8, 1), &args).unwrap();
...
fn softmax_rowwise_fixedpoint_exact() {
    ...
    let args = vec![KernelArg::Ptr(d_in), KernelArg::Ptr(d_w), KernelArg::Ptr(d_sum), sc(cols as i32)];
    launch::launch(&mut ctx, &mut sink, func, (rows as u32, 1, 1), (cols as u32, 1, 1), &args).unwrap();
    ...
    for r in 0..rows {
        assert!(want_w[r * cols..(r + 1) * cols].contains(&(1i32 << Q)), "row max lane weights 1<<Q");
    }
}
...
fn layernorm_stats_fixedpoint_exact() {
    ...
    launch::launch(&mut ctx, &mut sink, func, (rows as u32, 1, 1), (n as u32, 1, 1), &args).unwrap();
...
fn relu_and_gelu_cubic_exact() {
    ...
    launch::launch(&mut ctx, &mut sink, relu_fn, (grid, 1, 1), (128, 1, 1), &args_r).unwrap();
    let args_g = vec![KernelArg::Ptr(d_in), KernelArg::Ptr(d_gelu), sc(n as i32)];
    launch::launch(&mut ctx, &mut sink, gelu_fn, (grid, 1, 1), (128, 1, 1), &args_g).unwrap();
    ...
    assert_eq!(got_gelu, want_gelu, "fixed-point GELU-cubic elementwise exact");
    assert!(want_relu.iter().any(|&v| v == 0) && want_relu.iter().any(|&v| v > 0), "ReLU exercises both sides");
    assert!(want_gelu.iter().any(|&v| v < 0), "GELU-cubic produces negative outputs too");
}
...
fn gemv_and_argmax_exact() {
    ...
    let a: Vec<i32> = (0..m * n).map(|i| (i as i32 * 7 + 1).rem_euclid(11) - 5).collect();
    let x: Vec<i32> = (0..n).map(|i| (i as i32 * 3 + 2).rem_euclid(11) - 5).collect();
    ...
    let args_gemv = vec![KernelArg::Ptr(d_a), KernelArg::Ptr(d_x), KernelArg::Ptr(d_y), sc(m as i32), sc(n as i32)];
    launch::launch(&mut ctx, &mut sink, gemv_fn, (grid, 1, 1), (256, 1, 1), &args_gemv).unwrap();

    let args_max = vec![KernelArg::Ptr(d_y), KernelArg::Ptr(d_max), sc(m as i32)];
    launch::launch(&mut ctx, &mut sink, max_fn, (grid, 1, 1), (256, 1, 1), &args_max).unwrap();

    let args_arg = vec![KernelArg::Ptr(d_y), KernelArg::Ptr(d_max), KernelArg::Ptr(d_idx), sc(m as i32)];
    launch::launch(&mut ctx, &mut sink, arg_fn, (grid, 1, 1), (256, 1, 1), &args_arg).unwrap();
    ...
    assert_eq!(got_y[got_idx as usize], got_max, "arg-max index indeed points at the max value");
}
...
fn im2col_and_embedding_gather_exact() {
    ...
    launch::launch(&mut ctx, &mut sink, im2col_fn, (1, 1, 1), (ow as u32, oh as u32, 1), &args_im).unwrap();
    ...
    let args_em = vec![KernelArg::Ptr(d_tab), KernelArg::Ptr(d_ix), KernelArg::Ptr(d_out), sc(dim as i32)];
    launch::launch(&mut ctx, &mut sink, embed_fn, (tokens as u32, 1, 1), (dim as u32, 1, 1), &args_em).unwrap();
    ...
    assert_eq!(&got_emb[2 * dim..3 * dim], &got_emb[4 * dim..5 * dim], "repeated index gathers the same row");
}
