...
fn tiled_matmul_shared_memory_exact() {
    ...
    let a: Vec<i32> = (0..n * k).map(|i| (i as i32 * 7 + 3).rem_euclid(19) - 9).collect();
    let b: Vec<i32> = (0..k * n).map(|i| (i as i32 * 5 + 1).rem_euclid(19) - 9).collect();
    ...
    assert!(want.iter().any(|&v| v != 0), "reference must be non-degenerate");
...
fn prefix_scan_inclusive_and_exclusive_exact() {
    ...
    let args1 = vec![KernelArg::Ptr(d_in), KernelArg::Ptr(d_out), KernelArg::Ptr(d_sums), sc(n as i32)];
    launch::launch(&mut ctx, &mut sink, scan_fn, (grid as u32, 1, 1), (block as u32, 1, 1), &args1).unwrap();
    ...
    launch::launch(&mut ctx, &mut sink, add_fn, (grid as u32, 1, 1), (block as u32, 1, 1), &args3).unwrap();
    ...
    assert_eq!(got_inclusive, want_incl, "multi-block inclusive prefix scan, every element exact");
    ...
    assert_eq!(got_exclusive, want_excl, "exclusive prefix scan, every element exact");
    assert_eq!(want_incl[n - 1], input.iter().sum::<i32>(), "final inclusive = total sum");
    assert_eq!(sink.executor().dispatches, 2, "two kernel dispatches: block-scan + add-offset");
}
...
fn histogram_atomic_global_and_shared_exact() {
    ...
    assert!(want.iter().filter(|&&c| c > 0).count() >= 8, "distribution must actually spread across bins");
    ...
    assert_eq!(got_g.iter().sum::<i32>(), n as i32, "every input counted exactly once");
...
fn conv1d_box_shared_halo_exact() {
    ...
    launch::launch(&mut ctx, &mut sink, func, (grid, 1, 1), (block as u32, 1, 1), &args).unwrap();
    ...
    assert_eq!(got, want, "1D box convolution (shared-halo), every element exact");
}
...
fn conv2d_box_blur_exact() {
    ...
    let args = vec![KernelArg::Ptr(d_in), KernelArg::Ptr(d_out), sc(w as i32), sc(h as i32)];
...
fn reduce_segmented_sum_and_max_exact() {
    ...
    transfer::memset(&mut ctx, &mut sink, d_max, &i32s_to_bytes(&vec![i32::MIN; nseg])).unwrap();
    ...
    assert_eq!(got_sum, want_sum, "per-segment sum exact (cross-block atomic accumulation)");
    assert_eq!(got_max, want_max, "per-segment signed max exact");
    assert_eq!(got_sum.iter().sum::<i32>(), input.iter().sum::<i32>(), "segment sums cover every element");
}
...
fn transpose_shared_memory_coalesced_exact() {
    ...
    assert_eq!(got, want, "shared-memory tiled transpose, every element exact");
}
...
fn bitonic_sort_power_of_two_exact() {
    ...
    let input: Vec<i32> = (0..n).map(|i| ((i * 1103515245 + 12345) % 1000) as i32).collect();
...
fn special_register_as_operand_computes_global_index() {
    ...
    assert_eq!(got_operand, want, "sreg-as-operand global index, every thread exact");
    assert!(got_operand.iter().any(|&v| v != 0), "guards against the all-zero silent-wrong footgun");
    // …and it is bit-identical to the mov-first spelling.
    assert_eq!(got_operand, got_movfirst, "operand form == mov-first form, bit-exact");
}
...
fn sreg_operand_ir_matches_mov_first() {
    ...
    assert_eq!(count_movsreg(&op), 3, "operand form materializes ntid/ctaid/tid via MovSReg prelude");
    assert_eq!(count_movsreg(&mv), 3, "mov-first form materializes ntid/ctaid/tid via MovSReg");
}
...
fn unknown_special_register_operand_errors_not_silent_zero() {
    ...
    for bad in [
        ...
        assert!(r.is_err(), "unknown special register must error, got Ok for `{bad}`");
    }
    ...
    assert!(ptx::compile(src, "k", [8, 1, 1]).is_err(), "unknown sreg in mov must error too");
