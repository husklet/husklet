...
fn saxpy_multiblock_exact() {
    ...
    let want: Vec<f32> = x.iter().zip(&y).map(|(xi, yi)| alpha.mul_add(*xi, *yi)).collect();
    assert_eq!(got, want, "saxpy y = a*x + y, all {n} elements across 4 blocks");
...
fn reduction_sum_and_max_multiblock_exact() {
    ...
    let sum_args = vec![KernelArg::Ptr(d_in), KernelArg::Ptr(d_sum), KernelArg::Scalar((n as i32).to_le_bytes().to_vec())];
    launch::launch(&mut ctx, &mut sink, sum_fn, (8, 1, 1), (128, 1, 1), &sum_args).unwrap();
    ...
    assert_eq!(got_sum, -500, "closed-form: sum_{{i=0}}^{{999}}(i-500) = -500");
    ...
    let max_args = vec![KernelArg::Ptr(d_in), KernelArg::Ptr(d_max), KernelArg::Scalar((n as i32).to_le_bytes().to_vec())];
    launch::launch(&mut ctx, &mut sink, max_fn, (8, 1, 1), (128, 1, 1), &max_args).unwrap();
    ...
    assert_eq!(sink.executor().dispatches, 2, "two dispatches: one sum, one max");
}
...
fn elementwise_mul_add_min_exact() {
    ...
    let args = |x: DevicePtr, y: DevicePtr, z: DevicePtr| {
        vec![KernelArg::Ptr(x), KernelArg::Ptr(y), KernelArg::Ptr(z), KernelArg::Scalar((n as i32).to_le_bytes().to_vec())]
    };
    ...
    launch::launch(&mut ctx, &mut sink, min_fn, grid, block, &args(dai, dbi, dci)).unwrap();
...
fn shared_memory_block_reduction_exact() {
    ...
    let args = vec![KernelArg::Ptr(d_in), KernelArg::Ptr(d_out), KernelArg::Scalar((n as i32).to_le_bytes().to_vec())];
    launch::launch(&mut ctx, &mut sink, func, (grid as u32, 1, 1), (block as u32, 1, 1), &args).unwrap();

    let partials = bytes_to_i32s(&readback(&mut sink, &ctx, d_out, grid * 4));

    // Reference: each block sums its own contiguous slice.
    let want_partials: Vec<i32> =
        (0..grid).map(|b| input[b * block..(b + 1) * block].iter().sum()).collect();
    assert_eq!(partials, want_partials, "per-block shared-memory partials, each exact");

    // And the host-summed total equals the whole-array sum.
    let total: i32 = partials.iter().sum();
    assert_eq!(total, input.iter().sum::<i32>(), "grid total from block partials");
    assert_eq!(sink.executor().dispatches, 1);
}
