...
fn alloc_zeroed_i32(sink: &mut InProcessCommandSink<CpuExecutor>, ctx: &mut CudaContext, n: usize) -> DevicePtr {
...
fn merge_sort_multiblock_u32_exact() {
    ...
    while (runlen as usize) < n {
        ...
        let args = vec![KernelArg::Ptr(buf_a), KernelArg::Ptr(buf_b), sc(n as i32), sc(runlen as i32)];
        launch::launch(&mut ctx, &mut sink, func, (grid, 1, 1), (block, 1, 1), &args).unwrap();
    ...
    want.sort_unstable();
    assert_eq!(got, want, "multi-block merge sort produces the exact ascending order");
    ...
    assert_eq!(got_sorted, want, "output is a permutation of the input (no keys lost/duplicated)");
    assert!(want.iter().any(|&k| k > i32::MAX as u32), "high-half keys present → unsigned compare tested");
    assert_eq!(passes, 10, "ceil(log2(1000)) = 10 ping-pong passes");
    assert_eq!(sink.executor().dispatches, 10, "one dispatch per merge pass");
}
...
fn dft_fixed_point_integer_twiddle_exact() {
    ...
    let cos_t: Vec<i32> = (0..n)
        .map(|m| ((2.0 * std::f64::consts::PI * m as f64 / n as f64).cos() * q as f64).round() as i32)
        .collect();
    let sin_t: Vec<i32> = (0..n)
        .map(|m| ((2.0 * std::f64::consts::PI * m as f64 / n as f64).sin() * q as f64).round() as i32)
        .collect();
    // A non-trivial small integer signal (a couple of tones + DC), values in a modest range.
    let x: Vec<i32> = (0..n).map(|i| 3 + (i as i32 % 4) - ((i as i32 / 4) % 3) + 2 * ((i as i32) % 2)).collect();
    ...
    assert_eq!(want_re[0], q as i32 * x.iter().sum::<i32>(), "k=0 real bin = Q·Σx");
    assert_eq!(want_im[0], 0, "k=0 imag bin = 0");
    assert!(want_im.iter().any(|&v| v != 0), "spectrum has non-zero imaginary content");
}
...
fn bfs_frontier_expansion_step_exact() {
    ...
    assert_eq!(got_cnt, want_cnt, "BFS frontier count exact (cross-block atomic)");
    ...
    assert_eq!(got_dist[3], -1, "vertex 3 is two hops away — still unvisited after one step");
    assert_eq!(got_front, vec![0, 1, 1, 0, 0, 0, 0, 0], "frontier is exactly {{1,2}}");
}
...
fn stream_compaction_predicate_scan_scatter_exact() {
    ...
    launch::launch(&mut ctx, &mut sink, pred_fn, (grid as u32, 1, 1), (block as u32, 1, 1), &args_p).unwrap();

    // (b) inclusive scan of flags → incl (per-block scan, host offsets, add_offset)
    let args_s = vec![KernelArg::Ptr(d_flag), KernelArg::Ptr(d_incl), KernelArg::Ptr(d_sums), sc(n as i32)];
    launch::launch(&mut ctx, &mut sink, scan_fn, (grid as u32, 1, 1), (block as u32, 1, 1), &args_s).unwrap();
    ...
    launch::launch(&mut ctx, &mut sink, add_fn, (grid as u32, 1, 1), (block as u32, 1, 1), &args_a).unwrap();
    ...
    launch::launch(&mut ctx, &mut sink, scat_fn, (grid as u32, 1, 1), (block as u32, 1, 1), &args_c).unwrap();
    ...
    assert_eq!(&out[..count], &want[..], "compacted prefix equals the filtered elements, in order");
    // Anti-false-pass: the predicate actually filtered (some kept, some dropped).
    assert!(count > 0 && count < n, "predicate is non-trivial: 0 < count < n");
    assert!(out[..count].iter().all(|v| v & 1 == 0), "every compacted element passes the predicate");
}
...
fn monte_carlo_pi_deterministic_hit_count_exact() {
    ...
    assert_eq!(got, want, "Monte-Carlo quarter-circle hit count exact vs the identical CPU LCG");
    // Anti-false-pass: a non-degenerate count (roughly π/4 of the samples, but asserted EXACT above).
    let total = threads * iters;
    assert!(got > 0 && got < total, "hit count is a real interior count: 0 < hits < samples");
    // Sanity band only (NOT the assertion): π/4 ≈ 0.785 of samples land inside.
    assert!(got > total / 2, "roughly π/4 of samples are hits (sanity band, not the exactness check)");
}
...
fn kmeans_step_assign_and_accumulate_exact() {
    ...
    assert_eq!(got_assign, want_assign, "k-means point→cluster assignment exact");
    assert_eq!(got_sumx, want_sumx, "per-cluster x-sum exact (cross-block atomic)");
    assert_eq!(got_sumy, want_sumy, "per-cluster y-sum exact (cross-block atomic)");
    ...
    assert_eq!(got_count.iter().sum::<i32>(), n as i32, "every point assigned exactly once");
    assert!(got_count.iter().all(|&c| c > 0), "no empty cluster — assignment is non-degenerate");
}
...
fn running_minmax_prefix_scan_exact() {
    ...
    let args = vec![KernelArg::Ptr(d_in), KernelArg::Ptr(d_omin), KernelArg::Ptr(d_omax)];
    launch::launch(&mut ctx, &mut sink, func, (1, 1, 1), (n as u32, 1, 1), &args).unwrap();
    ...
    assert!(want_min.windows(2).all(|w| w[1] <= w[0]), "running min is non-increasing");
    assert!(want_max.windows(2).all(|w| w[1] >= w[0]), "running max is non-decreasing");
    assert!(want_min[n - 1] < want_min[0], "running min genuinely decreased over the sequence");
    assert!(want_max[n - 1] > want_max[0], "running max genuinely increased over the sequence");
    assert_eq!(want_min[n - 1], *input.iter().min().unwrap(), "final running min = global min");
    assert_eq!(want_max[n - 1], *input.iter().max().unwrap(), "final running max = global max");
}
