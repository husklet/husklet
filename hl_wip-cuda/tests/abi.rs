...
//!      symbol list exactly (no missing, no extra) and the count matches (132 / 49 / 62), and
...
fn built_exports(shim: &Shim, shim_target: &Path) -> BTreeSet<String> {
    ...
    assert!(status.success(), "aarch64 build of {} must succeed", shim.dir);
...
fn shim_export_surfaces_match_the_golden_abi() {
    ...
    for shim in shims() {
        ...
        assert_eq!(golden.len(), shim.expected, "golden {} has an unexpected count", shim.golden);
        ...
        assert!(missing.is_empty(), "{}: golden symbols missing from the .so: {missing:?}", shim.dir);
        assert!(extra.is_empty(), "{}: .so exports symbols not in the golden: {extra:?}", shim.dir);
        assert_eq!(exports.len(), shim.expected, "{}: exported symbol count drifted", shim.dir);
    }
}
