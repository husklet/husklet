...
fn managed_alloc_emits_create_buffer_and_flags_managed() {
    ...
    assert!(matches!(sink.batches.last().unwrap()[0], Cmd::CreateBuffer(1, _)));
...
fn host_get_device_pointer_maps_to_a_device_buffer() {
    ...
    assert_eq!(sink.batches.len(), batches, "cached mapping emits no new CreateBuffer");
...
fn dtoh_async_reads_bytes_back_through_the_sink() {
    ...
    let out = transfer::read_dtoh_async(&c, &mut sink, s, hl_cuda::DevicePtr(base.0 + 4), 16).unwrap();
    assert_eq!(out.len(), 16);
    // the readback request was recorded against the resolved (buffer, offset, len).
    assert_eq!(sink.reads.last().copied(), Some((hl_gpu::BufferId(1), 4, 16)));
...
fn ptx_parse_recovers_global_declarations() {
    ...
    assert_eq!(
        m.globals,
        vec![
            GlobalVar { name: "gCounters".into(), size: 256 },
            GlobalVar { name: "kCoeff".into(), size: 16 },
        ]
    );
    // A `ld.global`/`st.global` instruction inside a kernel body is NOT mistaken for a declaration.
    assert!(!PtxModule::parse(ptx::VECADD_PTX).globals.iter().any(|_| true));
}
...
fn module_get_global_returns_backing_buffer_and_size() {
    ...
    let (ptr, size) = load_module::module_get_global(&mut c, &mut sink, m, "gCounters").unwrap().unwrap();
    assert_eq!(size, 256);
    match sink.batches.last().unwrap().as_slice() {
        [Cmd::CreateBuffer(id, desc)] => {
            assert_eq!(*id, 1);
    ...
    let batches = sink.batches.len();
    let (ptr2, size2) = load_module::module_get_global(&mut c, &mut sink, m, "gCounters").unwrap().unwrap();
    assert_eq!((ptr2, size2), (ptr, 256));
    assert_eq!(sink.batches.len(), batches, "cached global emits no new CreateBuffer");

    // a second, distinct global gets its own backing buffer.
    let (cptr, csize) = load_module::module_get_global(&mut c, &mut sink, m, "kCoeff").unwrap().unwrap();
    assert_eq!(csize, 16);
    assert_ne!(cptr, ptr);

    // an undeclared symbol is Ok(None) → NOT_FOUND at the ABI seam (no fake pointer, no submit).
    let batches = sink.batches.len();
    assert_eq!(load_module::module_get_global(&mut c, &mut sink, m, "missing").unwrap(), None);
