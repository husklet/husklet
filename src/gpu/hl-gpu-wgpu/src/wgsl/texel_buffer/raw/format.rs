    ...
    let shape = match format {
        ...
        | TextureFormat::R8Sint => (
            if format == TextureFormat::R8Uint {
                Uint
            } else if format == TextureFormat::R8Sint {
                Sint
            } else {
                Float
            },
        ...
        | TextureFormat::Rg8Sint => (
            if format == TextureFormat::Rg8Uint {
                Uint
            } else if format == TextureFormat::Rg8Sint {
                Sint
            } else {
                Float
            },
        ...
        TextureFormat::R16Float | TextureFormat::R16Uint | TextureFormat::R16Sint => (
            if format == TextureFormat::R16Uint {
                Uint
            } else if format == TextureFormat::R16Sint {
                Sint
            } else {
                Float
            },
        ...
        TextureFormat::Rg16Uint | TextureFormat::Rg16Sint => (
            if format == TextureFormat::Rg16Uint {
                Uint
            } else {
                Sint
            },
        ...
        TextureFormat::Rgba16Uint | TextureFormat::Rgba16Sint => (
            if format == TextureFormat::Rgba16Uint {
                Uint
            } else {
                Sint
            },
        ...
        TextureFormat::Rg32Float | TextureFormat::Rg32Uint | TextureFormat::Rg32Sint => (
            if format == TextureFormat::Rg32Uint {
                Uint
            } else if format == TextureFormat::Rg32Sint {
                Sint
            } else {
                Float
            },
...
pub(super) fn decode(
    ...
    let value = match format {
        TextureFormat::Rgba8Unorm | TextureFormat::Bgra8Unorm | TextureFormat::Rgba8Snorm => {
            function.expressions.append(
                Expression::Math {
                    fun: if format == TextureFormat::Rgba8Snorm {
                        naga::MathFunction::Unpack4x8snorm
                    } else {
                        naga::MathFunction::Unpack4x8unorm
                    },
                    arg: words[0],
                    arg1: None,
                    arg2: None,
        ...
        | TextureFormat::Rg8Unorm
        | TextureFormat::Rg8Snorm => {
            let count = if matches!(format, TextureFormat::R8Unorm | TextureFormat::R8Snorm) {
                1
            } else {
                2
            };
            let mut components = (0..count)
                .map(|component| {
                    normalized_byte(
                        ...
                        count,
                        component,
                        matches!(format, TextureFormat::R8Snorm | TextureFormat::Rg8Snorm),
                    )
                })
                .collect::<Vec<_>>();
        ...
        | TextureFormat::R8Sint
        | TextureFormat::Rg8Sint => {
            let count = if matches!(format, TextureFormat::R8Uint | TextureFormat::R8Sint) {
                1
            } else {
                2
            };
            ...
            let mut components = (0..count)
                .map(|component| {
                    packed_byte_component(function, words[0], index, count, component, signed)
                })
                .collect::<Vec<_>>();
            let kind = if signed {
        ...
        TextureFormat::Rgb10a2Unorm | TextureFormat::Rgb10a2Uint => {
            ...
            for component in 0..4 {
                let shift = function.expressions.append(
                    Expression::Literal(naga::Literal::U32(shifts[component])),
                    Span::default(),
                );
                ...
                let mask = function.expressions.append(
                    Expression::Literal(naga::Literal::U32(max)),
                    Span::default(),
                );
                let bits = function.expressions.append(
                    Expression::Binary {
                ...
                let value = if format == TextureFormat::Rgb10a2Uint {
                    ...
                    let float = function.expressions.append(
                        Expression::As {
                            expr: bits,
                    ...
                    let max = function.expressions.append(
                        Expression::Literal(naga::Literal::F32(max as f32)),
                        Span::default(),
                    );
                    function.expressions.append(
                        Expression::Binary {
        ...
        TextureFormat::R16Float => {
            ...
            function.expressions.append(
                Expression::Compose {
                    ty: vec4,
                    components: vec![lane, zero, zero, one],
                },
                Span::default(),
            )
        }
        ...
        | TextureFormat::Rgba16Sint => {
            let signed = matches!(
                format,
                TextureFormat::R16Sint | TextureFormat::Rg16Sint | TextureFormat::Rgba16Sint
            );
            let count = if matches!(format, TextureFormat::R16Uint | TextureFormat::R16Sint) {
                1
            } else if matches!(format, TextureFormat::Rg16Uint | TextureFormat::Rg16Sint) {
                2
            } else {
                4
            };
            let mut components = (0..count)
                .map(|component| {
                    packed_half_component(
                        function,
                        words[component as usize / 2],
                        index,
                        count * 2,
                        component % 2,
                        signed,
    ...
    if format == TextureFormat::Bgra8Unorm {
        let components = [2, 1, 0, 3]
            ...
            .to_vec();
        Ok(function.expressions.append(
            Expression::Compose {
                ty: vec4,
                components,
            },
...
fn decode_unsigned_float(
    ...
    let shift = function.expressions.append(
        Expression::Literal(naga::Literal::U32(shift)),
        Span::default(),
    );
    let bits = function.expressions.append(
        Expression::Binary {
    ...
    let mask = function.expressions.append(
        Expression::Literal(naga::Literal::U32(mantissa_mask)),
        Span::default(),
    );
    let mantissa = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::And,
            left: bits,
            right: mask,
    ...
    let exp_shift = function.expressions.append(
        Expression::Literal(naga::Literal::U32(mantissa_bits)),
        Span::default(),
    );
    let exponent = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::ShiftRight,
            left: bits,
            right: exp_shift,
    ...
    let exp_mask = function
        .expressions
        .append(Expression::Literal(naga::Literal::U32(31)), Span::default());
    let exponent = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::And,
            left: exponent,
    ...
    let mantissa_f = function.expressions.append(
        Expression::As {
            expr: mantissa,
            kind: naga::ScalarKind::Float,
            convert: Some(4),
    ...
    let exponent_f = function.expressions.append(
        Expression::As {
            expr: exponent,
            kind: naga::ScalarKind::Float,
            convert: Some(4),
    ...
    let scale = function.expressions.append(
        Expression::Literal(naga::Literal::F32((1u32 << mantissa_bits) as f32)),
        Span::default(),
    );
    let fraction = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::Divide,
            left: mantissa_f,
            right: scale,
    ...
    let significand = function.expressions.append(
        Expression::Binary {
    ...
    let bias = function.expressions.append(
        Expression::Literal(naga::Literal::F32(15.0)),
        Span::default(),
    );
    let power = function.expressions.append(
        Expression::Binary {
    ...
    let two = function.expressions.append(
        Expression::Literal(naga::Literal::F32(2.0)),
        Span::default(),
    );
    let factor = function.expressions.append(
        Expression::Math {
    ...
    let normal = function.expressions.append(
        Expression::Binary {
    ...
    let denorm_scale = function.expressions.append(
        Expression::Literal(naga::Literal::F32(denorm_scale)),
        Span::default(),
    );
    let denorm = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::Multiply,
            left: mantissa_f,
            right: denorm_scale,
    ...
    let zero_u = function
        .expressions
        .append(Expression::Literal(naga::Literal::U32(0)), Span::default());
    let exp_zero = function.expressions.append(
        Expression::Binary {
    ...
    let finite = function.expressions.append(
        Expression::Select {
            condition: exp_zero,
            accept: denorm,
            reject: normal,
    ...
    let mant_shift = function.expressions.append(
        Expression::Literal(naga::Literal::U32(23 - mantissa_bits)),
        Span::default(),
    );
    let special_mantissa = function.expressions.append(
        Expression::Binary {
    ...
    let infinity = function.expressions.append(
        Expression::Literal(naga::Literal::U32(0x7f80_0000)),
        Span::default(),
    );
    let special_bits = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::InclusiveOr,
            left: infinity,
            right: special_mantissa,
        },
        Span::default(),
    );
    let special = function.expressions.append(
        Expression::As {
            expr: special_bits,
            kind: naga::ScalarKind::Float,
            convert: None,
    ...
    let thirty_one = function
        .expressions
        .append(Expression::Literal(naga::Literal::U32(31)), Span::default());
    let exp_special = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::Equal,
            left: exponent,
            right: thirty_one,
...
fn half_lane(
    ...
    let lane = function.expressions.append(
        Expression::Binary {
...
fn packed_half_component(
    ...
    let bytes = function.expressions.append(
    ...
    let component = function.expressions.append(
        Expression::Literal(naga::Literal::U32(component * 2)),
        Span::default(),
    );
    let offset = function.expressions.append(
        Expression::Binary {
    ...
    let eight = function
        .expressions
        .append(Expression::Literal(naga::Literal::U32(8)), Span::default());
    let shift = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::Multiply,
            left: offset,
    ...
    let mask = function.expressions.append(
        Expression::Literal(naga::Literal::U32(0xffff)),
        Span::default(),
    );
    let half = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::And,
            left: shifted,
            right: mask,
    ...
    let shift_16 = function
        .expressions
        .append(Expression::Literal(naga::Literal::U32(16)), Span::default());
    ...
    let signed = function.expressions.append(
        Expression::As {
            expr: left,
...
fn byte_component(
    ...
    if signed {
        let shift_24 = function
            .expressions
            .append(Expression::Literal(naga::Literal::U32(24)), Span::default());
        let left = function.expressions.append(
            Expression::Binary {
                op: naga::BinaryOperator::ShiftLeft,
                left: byte,
...
fn packed_byte_component(
    ...
    let bytes_expr = function.expressions.append(
        Expression::Literal(naga::Literal::U32(bytes)),
        Span::default(),
    );
    let byte = function.expressions.append(
    ...
    let component = function.expressions.append(
        Expression::Literal(naga::Literal::U32(component)),
        Span::default(),
    );
    let byte = function.expressions.append(
        Expression::Binary {
...
fn normalized_byte(
    ...
    let max = function.expressions.append(
        Expression::Literal(naga::Literal::F32(if signed { 127.0 } else { 255.0 })),
        Span::default(),
    );
    ...
    let neg_one = function.expressions.append(
        Expression::Literal(naga::Literal::F32(-1.0)),
        Span::default(),
    );
    function.expressions.append(
...
fn scalar_literal(
    ...
    function
        .expressions
        .append(Expression::Literal(literal), Span::default())
}
...
pub(super) fn encode(
    ...
    let packed = match format {
        ...
        | TextureFormat::Rg8Sint => {
            ...
            vec![encode_partial_bytes(
                function, format, index, value, current[0], bytes,
            )]
        }
        TextureFormat::Rgba8Unorm | TextureFormat::Bgra8Unorm | TextureFormat::Rgba8Snorm => {
            ...
            vec![function.expressions.append(
                Expression::Math {
                    fun: if format == TextureFormat::Rgba8Snorm {
                        naga::MathFunction::Pack4x8snorm
                    } else {
                        naga::MathFunction::Pack4x8unorm
                    },
                    arg: source,
                    arg1: None,
                    arg2: None,
        ...
        TextureFormat::Rgba8Uint | TextureFormat::Rgba8Sint => {
            let mut word = function
                ...
                .append(Expression::Literal(naga::Literal::U32(0)), Span::default());
            for component in 0..4 {
                let part = function.expressions.append(
                    Expression::AccessIndex {
                        base: value,
                        index: component,
                    },
                ...
                word = function.expressions.append(
                    Expression::Binary {
                        op: naga::BinaryOperator::InclusiveOr,
                        left: word,
                        right: part,
        ...
        .collect(),
        TextureFormat::Rgba16Uint | TextureFormat::Rgba16Sint => (0..2)
            .map(|pair| {
                let mut word = function
                    .expressions
                    .append(Expression::Literal(naga::Literal::U32(0)), Span::default());
                for lane in 0..2 {
                    ...
                    let shift = function.expressions.append(
                        Expression::Literal(naga::Literal::U32(lane * 16)),
                        Span::default(),
                    );
                    let component = function.expressions.append(
                        Expression::Binary {
        ...
        | TextureFormat::Rgba32Sint => (0..if matches!(
            format,
            TextureFormat::Rg32Float | TextureFormat::Rg32Uint | TextureFormat::Rg32Sint
        ) {
            2
...
pub(super) fn encode_partial_bytes(
    ...
    if !matches!(
        ...
        for component in 0..bytes {
            ...
            let byte_mask = function.expressions.append(
                Expression::Literal(naga::Literal::U32(0xff)),
                Span::default(),
            );
            let part = function.expressions.append(
            ...
            packed = function.expressions.append(
                Expression::Binary {
                    op: naga::BinaryOperator::InclusiveOr,
                    left: packed,
                    right: part,
    ...
    let byte_offset = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::Multiply,
            left: index,
            right: bytes_expr,
    ...
    let low_mask = function.expressions.append(
        Expression::Literal(naga::Literal::U32((1u32 << (bytes * 8)) - 1)),
        Span::default(),
    );
    let mask = function.expressions.append(
        Expression::Binary {
    ...
    let inverse = function.expressions.append(
        Expression::Unary {
    ...
    let preserved = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::And,
            left: current,
            right: inverse,
    ...
    let packed = function.expressions.append(
        Expression::Binary {
            op: naga::BinaryOperator::And,
            left: packed,
            right: low_mask,
