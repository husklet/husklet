...
fn _hl_cubic_level(t: texture_2d<f32>, uv: vec2<f32>, level: i32, au: u32, av: u32) -> vec4<f32> {
    ...
    for (var y = 0; y < 4; y++) { for (var x = 0; x < 4; x++) {
        ...
        out += textureLoad(t, q, level) * w[0][x] * w[1][y];
    }}
    return out;
}
fn _hl_sample_auto(t: texture_2d<f32>, s: sampler, uv: vec2<f32>, mn:u32, mg:u32, mm:u32, au:u32, av:u32, aw:u32, lmin:u32, lmax:u32) -> vec4<f32> {
    ...
    if mm == 1u {
        ...
        return mix(_hl_cubic_level(t,uv,lo,au,av), _hl_cubic_level(t,uv,hi,au,av), fract(lod));
    }
