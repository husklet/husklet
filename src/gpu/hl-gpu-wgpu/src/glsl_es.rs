...
fn strip_trailing_break(body: &[Tok]) -> String {
    let sig: Vec<usize> = (0..body.len())
        .filter(|&j| is_significant(&body[j]))
        .collect();
    if sig.len() >= 2 {
...
fn normalize_directives_and_precision(toks: &mut Vec<Tok>) {
    ...
    while i < toks.len() {
        match &toks[i] {
            ...
            Tok::Pp(p) => {
                let rewritten =
                    rewrite_io_macro_def(p).unwrap_or_else(|| rewrite_vertex_builtins(p));
...
fn classify_io_tail(
    ...
    let agg = if let Some((cols, rows)) = parse_matrix(&base) {
        Some(AggTy::Matrix {
            tok: base.clone(),
            cols,
            col_ty: format!("vec{rows}"),
        })
    } else {
        count.map(|c| AggTy::Array {
            elem: base.clone(),
            count: c,
        })
    };
...
fn parse_index_qualifier(quals: &str) -> Option<(String, u32)> {
    ...
    for (k, t) in toks.iter().enumerate() {
        if let Tok::Word(w) = t {
            if (w == "location" || w == "index")
                && next_significant(&toks, k + 1).map(|e| toks[e] == Tok::Punct('=')) == Some(true)
            {
