...
pub(crate) struct SubmitCommit {
    pub(crate) refusal: Option<GpuError>,
...
impl WgpuExecutor {
            ...
            let outcome = (|| -> Result<()> {
                match &ops[i] {
                    ...
                    Enc::BeginComputePass => {
                        ...
                        let mut encoder = self.gpu.device.create_command_encoder(
                            &wgpu::CommandEncoderDescriptor {
                                label: Some("hl-command-buffer-compute-pass"),
                            },
                        );
                        let started = std::time::Instant::now();
                        self.run_compute_pass(res, &ops[i + 1..end], &mut encoder)?;
                        if let Some(profile) = self.profile.borrow_mut().as_mut() {
                            profile.compute_passes.add(started.elapsed());
                        }
                        // See the render-pass arm: validation failure discards only this private encoder.
                        native = Some(encoder);
                    }
                    Enc::ClearRect {
                        texture,
                    ...
                    Enc::CopyBufferToTextureRegion {
                        ...
                        let tight =
                            if source_stride == tight_row && source_rows_per_image == image_rows {
                                source
                            } else {
                    ...
                    Enc::CopyTextureToBufferRegion {
                        ...
                        for layer in 0..extent.depth as usize {
                            for row in 0..image_rows as usize {
                                let destination_row =
                                    layer * destination_rows_per_image as usize + row;
                    ...
                    Enc::CopyBufferToTexture {
                        ...
                        let tight = if src_stride == row {
                            ...
                            for r in 0..rows {
                                let offset =
                                    r.checked_mul(src_stride).ok_or(GpuError::OutOfBounds)?;
                                let start =
    ...
    fn submit_encoded(&self, encoder: &mut Option<wgpu::CommandEncoder>) {
        ...
        let remaining = self.diagnostic_sentinel_submits.get();
        if remaining != 0 {
