...
fn render(exec: &mut WgpuExecutor, sample_count: u32) -> Vec<u8> {
    ...
    let read_id = if sample_count > 1 {
        let mut cmds = vec![
            ...
            Cmd::CreateTexture(
                2,
                tex(1, texture_usage::RENDER_TARGET | texture_usage::COPY_SRC),
            ),
        ];
        cmds.extend(shaders);
...
fn msaa_edge_coverage_is_quantized_to_the_four_sample_levels() {
    ...
    assert_eq!(
        interior,
        [255, 255, 255, 255],
        "MSAA interior must be EXACT white (full coverage)"
    );
    assert_eq!(
        exterior,
        [0, 0, 0, 255],
        "MSAA exterior must be EXACT black (zero coverage)"
    );
    ...
    for y in 0..H {
        for x in 0..W {
            ...
            assert_eq!(
                p[3], 255,
                "pixel ({x},{y}) alpha must stay opaque (all samples are opaque)"
            );
            assert!(
