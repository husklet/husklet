...
// GL/protocol blend-factor + op wire numbering (`hl-gl` `blend_factor_wire`/`blend_op_wire`):
...
fn quad_uniform(rect: [f32; 4], color: [f32; 4]) -> Vec<u8> {
    le_f32(&[
        rect[0], rect[1], rect[2], rect[3], color[0], color[1], color[2], color[3],
    ])
}
...
fn run(exec: &mut WgpuExecutor, fg_blend: Option<BlendState>) -> Vec<u8> {
    ...
    let fg_target = ColorTargetState {
        format: TextureFormat::Rgba8Unorm,
        blend: fg_blend,
        write_mask: 0xF,
    };
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::WriteBuffer {
                id: 1,
                offset: 0,
                data: quad_uniform([-1.0, -1.0, 1.0, 1.0], f(BG, 1.0)),
            },
            ...
            Cmd::WriteBuffer {
                id: 2,
                offset: 0,
                data: quad_uniform([-0.5, -0.5, 0.5, 0.5], f(FG, FG_ALPHA)),
            },
            Cmd::CreateShader {
                id: 1,
