...
/// the read-back plane. A missing GPU adapter is a hard failure.
#[allow(clippy::type_complexity)]
fn run_with_scissor(scissor: Enc) -> (hl_gpu::Result<()>, Vec<u8>) {
...
fn overhanging_scissor_clips_to_the_target_and_does_not_nack() {
    let (result, img) = run_with_scissor(Enc::SetScissor {
        x: 10,
        y: 8,
        w: 100,
        h: 100,
