...
fn two_color_attachments_each_hold_their_own_output() {
    ...
    for (i, out) in t0.chunks_exact(4).enumerate() {
        assert!(
            near(out.try_into().unwrap(), A),
            "target0 px {i}: {out:?} != A {A:?}"
        );
    }
    for (i, out) in t1.chunks_exact(4).enumerate() {
        assert!(
            near(out.try_into().unwrap(), B),
            "target1 px {i}: {out:?} != B {B:?}"
        );
    }
    assert!(
        !near(A, B),
        "the two attachment colors must differ for the demo to prove anything"
    );
