...
fn partial_refusal_commits_resource_for_the_next_command_buffer() {
    ...
    let mut session = session(Limits::from_capabilities(caps), GlobalLedger::unbounded());
...
fn refused_fence_lifecycle_is_not_committed() {
    ...
    let outcome =
        hl_gpu::runtime::submit_outcome(&mut session, &mut executor, 0, &[Cmd::CreateFence(9)])
...
fn successful_presentation_survives_a_later_refusal() {
    ...
    impl GpuExecutor for PresentPartial {
        ...
        fn execute(
            &mut self,
            _: &mut SessionResources,
            batch: &[Cmd],
        ) -> Result<hl_gpu::Execution> {
            Ok(hl_gpu::Execution::partial(
                vec![hl_gpu::Presentation {
                    surface: SurfaceId(1),
                    texture: TextureId(2),
...
fn partial_delta_carries_only_the_fence_signal_that_was_scheduled() {
    ...
    let outcome = hl_gpu::runtime::submit_outcome(&mut session, &mut executor, 0, &batch).unwrap();
    assert_eq!(
        outcome.committed.replay_commands().collect::<Vec<_>>(),
        [&Cmd::CreateFence(5)]
    );
...
fn committed_submit_does_not_imply_its_fence_signal_was_scheduled() {
    ...
    let batch = [Cmd::Submit(CommandBuffer {
        encoder: Vec::new(),
        signal: Some((9, 4)),
    })];
    let outcome = hl_gpu::runtime::submit_outcome(&mut session, &mut executor, 0, &batch).unwrap();
...
fn refused_shared_buffer_destroy_keeps_export_and_charge() {
    ...
    let export = hl_gpu::runtime::service::dispatch::export_buffer(
        &mut session,
        &executor,
        hl_gpu::BufferId(1),
    )
    .unwrap();
    hl_gpu::runtime::submit_outcome(&mut session, &mut executor, 0, &[Cmd::DestroyBuffer(1)])
        .unwrap();
...
fn refused_shared_texture_destroy_keeps_export_and_charge() {
    ...
    let export =
        hl_gpu::runtime::service::dispatch::export_texture(&mut session, &executor, TextureId(2))
            .unwrap();
    hl_gpu::runtime::submit_outcome(&mut session, &mut executor, 0, &[Cmd::DestroyTexture(2)])
        .unwrap();
    assert!(exports.is_live(export));
    ...
    fn execute(
        ...
        match &batch[0] {
            Cmd::DestroyBuffer(id) => {
                resources.buffers.remove(*id)?;
            }
            Cmd::DestroyTexture(id) => {
                resources.textures.remove(*id)?;
            }
...
fn committed_importer_buffer_destroy_does_not_preserve_a_refused_replacement() {
    ...
    let mut owner = session(Limits::from_capabilities(caps.clone()), global.clone())
        .with_exports(exports.clone());
    let mut importer =
        session(Limits::from_capabilities(caps.clone()), global).with_exports(exports.clone());
    let mut executor = DestroyThenRefuse(caps);
...
fn committed_importer_texture_destroy_does_not_preserve_a_refused_replacement() {
    ...
    let export =
        hl_gpu::runtime::service::dispatch::export_texture(&mut owner, &executor, TextureId(2))
            .unwrap();
    hl_gpu::runtime::service::dispatch::import_texture(
        &mut importer,
        &executor,
        TextureId(2),
        export,
