...
fn residency_nack_then_teardown_does_not_climb() {
    ...
    sink.submit(&[buffer(1, 1024), buffer(2, 1024)])
        .expect("working set fits under the cap");
    ...
    for attempt in 0..5 {
        ...
        assert_eq!(
            err,
            GpuError::ResourceLimit("connection residency"),
            "attempt {attempt}"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            2,
            "attempt {attempt}: object count unchanged"
        );
    }
    ...
    sink.submit(&[Cmd::DestroyBuffer(1), Cmd::DestroyBuffer(2)])
        .expect("teardown destroys succeed");
    assert_eq!(
        sink.session().residency_bytes(),
        0,
    ...
    sink.submit(&[buffer(20, 1024), buffer(21, 1024)])
        .expect("a fresh working set fits post-teardown");
...
fn recreate_teardown_cycles_stay_bounded() {
    ...
    for cycle in 0u32..16 {
        let base = cycle * 10 + 1;
        sink.submit(&[buffer(base, 4096), buffer(base + 1, 4096)])
            .expect("cycle working set fits");
        high_water = high_water.max(sink.session().residency_bytes());
        sink.submit(&[Cmd::DestroyBuffer(base), Cmd::DestroyBuffer(base + 1)])
            .expect("teardown");
        assert_eq!(
            sink.session().residency_bytes(),
            0,
            "cycle {cycle}: ledger back to zero"
        );
    }
    assert_eq!(
        high_water, 8192,
        "the ledger never held more than a SINGLE cycle's working set"
    );
}
