...
fn render(depth: bool) -> [[u8; 4]; 2] {
    ...
    let pipeline = RenderPipelineDesc {
        ...
        depth: depth
            .then(|| DepthState::depth_only(TextureFormat::Depth32Float, true, compare::LESS)),
...
fn depth_test_less_lets_nearer_triangle_win_the_overlap() {
    ...
    assert_eq!(
        pixel0, GREEN_PX,
        "overlap: nearer (green) must survive the later far draw"
    );
    ...
    assert_eq!(
        pixel1, RED_PX,
        "far-only region: farther (red) must be written"
    );
}
...
fn depth_disabled_control_is_painters_order() {
    ...
    assert_eq!(
        pixel0, RED_PX,
        "no depth: later draw (red) wins the overlap (painter's)"
    );
...
fn depth_buffer_stores_written_depth() {
    ...
    exec.read_texture(&s.resources, TextureId(2), &mut raw)
        .unwrap();
    let z0 = f32::from_le_bytes(raw[0..4].try_into().unwrap());
    let z1 = f32::from_le_bytes(raw[4..8].try_into().unwrap());
    assert!(
        (z0 - 0.3).abs() < 1e-6,
        "pixel0 depth should be the near 0.3, got {z0}"
    );
    assert!(
        (z1 - 0.7).abs() < 1e-6,
        "pixel1 depth should be the far 0.7, got {z1}"
    );
}
