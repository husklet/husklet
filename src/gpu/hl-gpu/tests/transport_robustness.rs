...
use hl_gpu::{
    serve_connection_with_handler, Capabilities, Cmd, CommandSink, ConnectionHandler,
    ReadbackRequest, RemoteCommandSink,
};
...
fn large_multi_mb_many_cmd_frame_round_trips_over_the_socket() {
    ...
    sink.submit(&batch)
        .expect("large frame acknowledged over the socket");
    ...
    assert_eq!(
        decoded.len(),
        batch.len(),
        "every command in the large frame arrived"
    );
    ...
    assert_eq!(
        read, big,
        "the multi-MB upload round-tripped client -> host -> readback intact"
    );
...
fn a_nacked_frame_leaves_the_same_connection_serving() {
    ...
    sink.submit(&[Cmd::CreateFence(7)])
        .expect("the same connection still serves after a NACK");
    assert_eq!(
        sink.connects(),
        1,
        "the good frame reused the SAME connection — no reconnect, no close"
    );

    let got = batch_rx.recv().unwrap();
    assert_eq!(
        got,
        vec![Cmd::CreateFence(7)],
        "the host served the good frame after the NACK"
    );
...
fn an_over_cap_frame_is_drained_and_nacked_without_closing_the_connection() {
    ...
    (&stream)
        .read_exact(&mut ack)
        .expect("host answered the over-cap frame instead of closing");
    ...
    assert_eq!(
        ack[0], ACK_OK,
        "the good frame ACKs on the connection that survived the over-cap frame"
    );
