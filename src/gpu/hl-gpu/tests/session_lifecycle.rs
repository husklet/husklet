...
fn concurrent_sessions_isolated() {
    ...
    assert_eq!(
        a.read_buffer(hl_gpu::BufferId(1), 0, 16).unwrap(),
        vec![0xAA; 16]
    );
    assert_eq!(
        b.read_buffer(hl_gpu::BufferId(1), 0, 16).unwrap(),
        vec![0xBB; 16]
    );
...
fn teardown_mid_stream() {
...

    {
        ...
        assert!(
            global.residency_bytes() >= 4096 + 2048,
            "mid-stream resources are charged"
        );
    ...
    assert_eq!(
        ...
        0,
        "drop refunded the whole connection contribution"
    );
    ...
    assert_eq!(
        fresh.read_buffer(hl_gpu::BufferId(1), 0, 16).unwrap(),
        vec![0x5A; 16]
    );
}
...
fn submit_to_closed_sink() {
    ...
    assert_eq!(
        s.submit(&[buffer(2, 16)]).unwrap_err(),
        GpuError::Invalid("session closed")
    );
    assert_eq!(
        s.wait(hl_gpu::FenceId(1), 1).unwrap_err(),
        GpuError::Invalid("session closed")
    );
    assert_eq!(
...
fn cleanup_on_drop() {
    ...
    assert_eq!(
        global.residency_bytes(),
        0,
        "no residency accumulated across create/drop cycles"
    );
    assert_eq!(
        global.object_count(),
        0,
        "no object count accumulated across create/drop cycles"
    );
}
...
fn double_teardown_and_use_after_teardown() {
    ...
    assert_eq!(
        ...
        0,
        "close reclaimed live resources"
    );
    ...
    assert_eq!(
        s.submit(&[buffer(3, 64)]).unwrap_err(),
        GpuError::Invalid("session closed")
    );
...
fn explicit_release_is_refund_exact_once() {
    ...
    assert_eq!(
        global.residency_bytes(),
        4096,
        "close A refunded only A's contribution"
    );
    ...
    assert_eq!(
        global.residency_bytes(),
        4096,
        "double-close did not double-refund"
    );
    ...
    assert_eq!(
        global.residency_bytes(),
        4096,
        "drop after close did not double-refund"
    );

    // B is untouched and still usable.
    b.submit(&[write(1, 0x11, 4)]).unwrap();
    assert_eq!(
        b.read_buffer(hl_gpu::BufferId(1), 0, 4).unwrap(),
        vec![0x11; 4]
    );
    drop(b);
    assert_eq!(
