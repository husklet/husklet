...
async fn typed_event_stream_replays_create_and_destroy_from_real_handlers() {
    ...
    let destroy = tokio::time::timeout(std::time::Duration::from_secs(2), stream.next())
        ...
        .unwrap()
        .unwrap();
    assert_eq!((create.action.as_str(), destroy.action.as_str()), ("create", "destroy"));
...
async fn client_reaches_real_server_and_observes_headless_state() {
    ...
    containers
        ...
        .unwrap();
    let daemon = Daemon::new(containers);
    assert_eq!(daemon.headless().containers().list().await.unwrap().len(), 1);
    ...
    let logs = client.containers().logs("renamed", true, true).await.unwrap();
    ...
    let removed = tokio::spawn(async move { waiter.containers().wait_for(&id, WaitCondition::Removed).await });
    tokio::task::yield_now().await;
    client.containers().remove("renamed", false, false).await.unwrap();
