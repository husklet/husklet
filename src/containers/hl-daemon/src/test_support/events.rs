...
async fn events_until_in_the_past_closes_immediately() {
    ...
    assert!(
        body.is_empty(),
        "a past --until must yield an empty, closed stream, got {body:?}"
    );
}
...
async fn network_connect_disconnect_emit_events() {
    ...
    let evs = drain_events(&mut rx);
    assert!(
        evs.contains(&("network".into(), "connect".into())),
        "{evs:?}"
    );
    assert!(
        evs.contains(&("network".into(), "disconnect".into())),
        "{evs:?}"
    );
}
...
async fn container_rename_emits_rename_event() {
    ...
    let evs = drain_events(&mut rx);
    assert!(
        evs.contains(&("container".into(), "rename".into())),
        "{evs:?}"
    );
}
...
async fn image_prune_removes_dangling_unreferenced_image() {
    let app = test_app();
    {
        ...
        g.images.push(Image {
            name: String::new(),
            rootfs: "/store/dangle".into(),
            ..Default::default()
        });
        g.images.push(Image {
            name: "keep:latest".into(),
            rootfs: "/store/keep".into(),
            ..Default::default()
        });
    }
    ...
    let g = app.inner.lock().await;
    assert!(
        !g.images.iter().any(|i| i.rootfs == "/store/dangle"),
        "dangling image removed"
    );
    assert!(
        g.images.iter().any(|i| i.name == "keep:latest"),
        "tagged image kept"
    );
}
...
async fn version_tracks_crate_version() {
    ...
    assert_ne!(
        v.version, "0.1.0-hl",
        "version must not be the stale hardcoded value"
    );
}
...
async fn system_df_counts_are_consistent() {
    ...
    assert_eq!(
        ...
        "one active image, not the container count"
    );
    assert_eq!(
        df["VolumeUsage"]["ActiveCount"], 1,
        "the mounted volume is active"
    );
    assert_eq!(
        df["Volumes"][0]["UsageData"]["RefCount"], 1,
        "mounted volume RefCount is live"
    );
    // Build-cache TotalCount must match the Items length (no phantom count).
    let bc = &df["BuildCacheUsage"];
    assert_eq!(
        bc["TotalCount"].as_i64().unwrap(),
        bc["Items"].as_array().unwrap().len() as i64
    );
}
