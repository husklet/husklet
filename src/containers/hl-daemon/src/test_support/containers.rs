...
async fn kill_exited_container_is_409_and_unchanged() {
    ...
    let resp =
        crate::containers::containers_kill(State(app.clone()), Path("c1".into()), Query(empty_q()))
            .await;
...
async fn stop_already_exited_is_304_and_unchanged() {
    ...
    seed_container(&app, "c1", "exited").await;
    let resp =
        crate::containers::containers_stop(State(app.clone()), Path("c1".into()), Query(empty_q()))
            .await;
...
async fn containers_json_includes_paused_without_all() {
    ...
    assert!(
        ids.contains(&"ppaused"),
        "paused must be listed (ps-paused fix)"
    );
    assert!(
...
async fn containers_json_wire_shape_and_all_filter() {
    ...
    assert!(
        !obj.contains_key("SizeRw"),
        "SizeRw must be omitted without --size"
    );
...
async fn containers_inspect_wire_shape() {
    ...
    assert_eq!(
        ...
        "exited container is not Running"
    );
...
async fn flow_container_name_conflict_keeps_single() {
    ...
    assert_eq!(
        r.status(),
        StatusCode::CONFLICT,
        "duplicate --name web is 409"
    );
    let msg = to_body_string(r).await;
    assert!(
        msg.contains("already in use"),
        "409 body names the conflict: {msg}"
    );
    assert_eq!(
        app.inner
            .lock()
...
async fn flow_container_rename_then_inspect_lookup_and_update_noop() {
    ...
    assert_eq!(
        app.inner.lock().await.containers[&cid].name,
        "app",
        "name rebound to app"
    );
    ...
    assert_eq!(
        r.status(),
        StatusCode::OK,
        "lookup by the NEW name resolves"
    );
    ...
    assert_eq!(
        ...
        "old name `web` is freed by rename"
    );
...
async fn flow_rename_onto_existing_name_creates_duplicate_divergence() {
    ...
    let g = app.inner.lock().await;
    assert_eq!(
        g.containers.values().filter(|c| c.name == "app").count(),
        1,
        "no duplicate name"
    );
    assert_eq!(
        g.containers[&cid_web].name, "web",
        "the refused rename left web unchanged"
    );
}
...
async fn flow_restart_policy_stop_sets_manual_stop_flag() {
    ...
    let r =
        crate::containers::containers_stop(State(app.clone()), Path("rc".into()), Query(empty_q()))
            .await;
    assert_eq!(r.status(), StatusCode::NO_CONTENT);
    let g = app.inner.lock().await;
    ...
    assert_eq!(
        c.status, "exited",
        "stop flips a running container to exited"
    );
    assert!(
        c.manually_stopped,
    ...
    assert_eq!(
        c.restart_policy.name, "always",
        "restart policy is not cleared by stop"
    );
}
...
async fn flow_containers_prune_removes_only_stopped_keeps_running() {
    ...
    assert_eq!(
        deleted.len(),
        2,
        "exactly two containers pruned: {deleted:?}"
    );
    assert!(
        deleted.contains("exitidA00000"),
        "exited A reported deleted: {deleted:?}"
    );
    assert!(
        deleted.contains("exitidB00000"),
        "exited B reported deleted: {deleted:?}"
    );
    assert!(
    ...
    // In-memory state: only the running container remains.
    {
        let g = app.inner.lock().await;
        assert!(
            g.containers.contains_key("runid0000000"),
            "running container kept"
        );
    ...
    assert_eq!(
        list.len(),
        1,
        "ps all=1 lists only the surviving running container"
    );
...
async fn flow_container_name_reusable_after_remove() {
    ...
    assert!(
        g.containers.contains_key(&cid2),
        "the recreated web is present"
    );
}
...
async fn logs_since_until_filter_per_chunk_not_per_coalesced_run() {
    ...
    {
        let live = Live::new();
        live.log_chunks.lock().await.extend([
            (100i64, 1u8, b"early\n".to_vec()),
            (200i64, 1u8, b"late\n".to_vec()),
        ]);
...
async fn container_inspect_round_trips_full_create_config() {
    ...
    let envs: Vec<String> = cfg["Env"]
        ...
        .collect();
    assert_eq!(
        envs.iter().filter(|e| e.starts_with("FOO=")).count(),
        1,
        "one FOO= after dedup"
    );
    ...
    assert!(v["NetworkSettings"]["Ports"]
        .as_object()
        .unwrap()
...
async fn top_on_stopped_container_is_409() {
    ...
    assert_eq!(
        resp.status(),
        StatusCode::CONFLICT,
        "top on a stopped container is a conflict"
    );
...
async fn inspect_dead_status_sets_dead_boolean() {
    ...
    assert_eq!(
        v["State"]["Dead"], true,
        "State.Dead must agree with a dead status"
    );
}
...
async fn wait_on_created_container_blocks_until_exit() {
    ...
    let handle = tokio::spawn(async move {
        let resp = crate::containers::containers_wait(
            State(app2),
            Path("waitcreated0".into()),
            Query(waitq),
        )
        .await;
        axum::body::to_bytes(resp.into_body(), usize::MAX)
    ...
    assert!(
        !handle.is_finished(),
        "wait must block a created container, not return immediately"
    );
    ...
    let body = tokio::time::timeout(std::time::Duration::from_secs(2), handle)
        .await
        .expect("wait completes once exited")
        .unwrap();
...
async fn wait_condition_removed_blocks_until_removed() {
    ...
    assert!(
        !handle.is_finished(),
        "condition=removed must not complete while the container exists"
    );
...
async fn failed_spawn_persists_terminal_exit_state() {
    ...
    assert_eq!(
        c.status, "exited",
        "failed spawn marks the container exited"
    );
    ...
    assert_eq!(
        status, "exited",
        "terminal state must be durably saved, not left running"
    );
}
...
fn app_with_failing_state() -> App {
    let app = test_app();
    App {
        state_path: app.volumes_dir.clone(),
        ..app
    } // volumes_dir is a dir -> rename onto it fails
}
...
async fn create_with_missing_network_is_404_and_records_nothing() {
    ...
    assert_eq!(
        r.status(),
        StatusCode::NOT_FOUND,
        "create on a missing network is 404"
    );
    let g = app.inner.lock().await;
    ...
    assert!(
        rx.try_recv().is_err(),
        "no create/volume event emitted on the failed create"
    );
}
...
async fn create_rolls_back_and_500s_when_state_cannot_persist() {
    ...
    assert_eq!(
        ...
        "a failed durable save fails the create"
    );
    assert!(
        app.inner.lock().await.containers.is_empty(),
        "the partial container is rolled back"
    );
    ...
    assert!(
        !saw_create,
        "no container/create event when the state save failed"
    );
}
...
async fn failed_spawn_removes_spent_live_entry() {
    ...
    app.inner
        ...
        .await
        .live
        .insert("spent0000000".into(), live.clone());
    live.started
...
async fn start_installs_starting_health_state_synchronously() {
    ...
    assert_eq!(
        h.status, "starting",
        "health must be visible as starting from the start transition"
    );
}
...
async fn create_rejects_image_with_missing_store_rootfs() {
    ...
    let gone = std::path::Path::new(&app.images_dir)
        .join("gone/rootfs")
        .to_string_lossy()
        .into_owned();
    {
    ...
    assert_eq!(
        r.status(),
        StatusCode::NOT_FOUND,
        "create on a missing store rootfs is 404"
    );
    assert!(
        app.inner.lock().await.containers.is_empty(),
        "no container recorded"
    );
    assert!(
        rx.try_recv().is_err(),
        "no create event on the rejected create"
    );
}
...
async fn create_fails_when_anon_volume_root_is_unusable() {
    ...
    let app = App {
        volumes_dir: volfile.to_string_lossy().into_owned(),
        ..app0
    };
    ...
    assert_eq!(
        r.status(),
        StatusCode::INTERNAL_SERVER_ERROR,
        "unusable volumes root fails the create"
    );
    let g = app.inner.lock().await;
...
async fn published_port_bind_conflict_fails_start() {
    ...
    let c = Container {
        ...
        arch: Some(hl_jit::Guest::LinuxAarch64),
    ...
    app.inner
        ...
        .await
        .live
        .insert(c.id.clone(), live.clone());
    let ok = crate::runtime::spawn_live(&app, &c, &[], live).await;
    assert!(
        !ok,
        "start must fail when the published host port is already bound"
    );
    let g = app.inner.lock().await;
    let cc = g.containers.get("portfail0000").unwrap();
    assert_eq!(
        cc.status, "exited",
        "a bind conflict leaves the container exited, not running"
    );
...
async fn logs_follow_delivers_all_chunks_no_drop() {
    ...
    app.inner
        ...
        .await
        .live
        .insert("logsfollow00".into(), live.clone());
