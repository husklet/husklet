...
async fn build_inherits_base_labels_and_child_overrides() {
    ...
    assert_eq!(
        img.labels.get("org.example.base").map(String::as_str),
        Some("kept"),
        "base label survives"
    );
    assert_eq!(
        img.labels.get("shared").map(String::as_str),
        Some("from-child"),
        "child overrides matching key"
    );
    assert_eq!(
        img.labels.get("child").map(String::as_str),
        Some("yes"),
        "child label added"
    );
}
...
async fn build_env_interpolation_uses_prior_env() {
    ...
    assert!(
        img.env.contains(&"B=one".to_string()),
        "ENV B=${{A}} -> B=one; env={:?}",
        img.env
    );
}
...
async fn build_pre_from_arg_does_not_leak_into_stage() {
    ...
    assert!(
        !err2,
        "re-declared ARG V after FROM restores the value: {_b2}"
    );
}
...
async fn build_replays_base_onbuild_triggers() {
    ...
    assert!(
        img.env.contains(&"TRIGGERED=yes".to_string()),
        "ONBUILD ENV replayed; env={:?}",
        img.env
    );
}
...
async fn build_unknown_target_errors() {
    ...
    assert!(
        built(&app, "tgt:latest").await.is_none(),
        "no image registered for an unknown target"
    );
}
...
async fn build_rejects_non_string_exec_form() {
    ...
    assert!(
        err,
        "non-string exec-form element must fail the build: {body}"
    );
    assert!(
        built(&app, "execbad:latest").await.is_none(),
        "no image for a rejected Dockerfile"
    );
}
...
async fn build_allows_bracket_shell_command() {
    ...
    assert!(
        !err,
        "a `[`-prefixed shell command must not be rejected as bad exec form: {body}"
    );
}
...
async fn build_from_local_honors_tag() {
    ...
    let which =
        std::fs::read_to_string(PathBuf::from(&img.rootfs).join("which")).unwrap_or_default();
...
async fn build_workdir_dotdot_normalized() {
    ...
    assert_eq!(
        img.workdir, "/a/c",
        "WORKDIR ../c from /a/b normalizes to /a/c"
    );
}
...
async fn build_copy_from_external_local_image() {
    ...
    let got =
        std::fs::read_to_string(PathBuf::from(&img.rootfs).join("got.bin")).unwrap_or_default();
    assert_eq!(
        got, "external-content",
        "file copied from the external image's rootfs"
    );
}
...
async fn build_dockerignore_excludes_secret() {
    ...
    assert!(
        ...
        ".dockerignore'd secret.txt must NOT be copied"
    );
}
...
async fn build_base_config_change_invalidates_cache() {
    ...
    let _ = crate::build::images_build(State(app.clone()), Query(q), axum::body::Bytes::from(body))
        .await;
    let first = built(&app, "cfgchild:latest").await.unwrap();
    assert!(
        first.env.contains(&"FOO=one".to_string()),
        "first build inherits FOO=one: {:?}",
        first.env
    );
    ...
    let q2: crate::build::BuildQ =
        serde_json::from_value(serde_json::json!({"t": "cfgchild:latest"})).unwrap();
    let _ = crate::build::images_build(
        State(app.clone()),
        Query(q2),
