...
async fn network_connect_missing_container_is_404_no_phantom() {
    ...
    assert_eq!(
        ...
        "connect of a missing container is 404"
    );
    assert_eq!(
        net_members(&app, "mynet").await.len(),
        0,
        "no phantom membership"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        "net with no real endpoints deletes"
    );
    ...
    assert_eq!(
        r.status(),
        StatusCode::NOT_FOUND,
        "connect to a missing network is 404"
    );
}
...
async fn network_disconnect_missing_container_is_404_noop() {
    ...
    assert_eq!(
        r.status(),
        StatusCode::NOT_FOUND,
        "disconnect of a missing container is 404"
    );
    ...
    assert_eq!(
        r.status(),
        StatusCode::NOT_FOUND,
        "disconnect on a missing network is 404"
    );
}
...
async fn flow_network_endpoint_refcount_lifecycle() {
    ...
    assert_eq!(
        ...
        "delete with an endpoint is 403"
    );
    assert!(
        app.inner
            .lock()
    ...
    assert_eq!(
        net_members(&app, "mynet").await.len(),
        0,
        "membership cleared on disconnect"
    );
    assert_eq!(
        net_endpoint_count(&app, "mynet").await,
        0,
    ...
    assert_eq!(
        r.status(),
        StatusCode::NO_CONTENT,
        "delete after disconnect is 204"
    );
    assert!(
        !app.inner
            .lock()
...
async fn live_network_connect_disconnect_refreshes_reach_by_name_table() {
    ...
    let after_connect = std::fs::read_to_string(&names_path)
        .expect("connect must WRITE the live reach-by-name file");
    assert!(
...
async fn flow_network_duplicate_predefined_and_prune_selectivity() {
    ...
    assert_eq!(
        r.status(),
        StatusCode::FORBIDDEN,
        "predefined bridge delete is 403"
    );
    assert!(app
        .inner
        .lock()
    ...
    assert!(
        pruned.contains("emptyuser"),
        "empty user net is pruned: {pruned:?}"
    );
    assert!(
        pruned.contains("mynet"),
        "the other empty user net is pruned: {pruned:?}"
    );
    assert!(
        !pruned.contains("busyuser"),
        "a net with an endpoint is NOT pruned"
    );
    ...
    assert!(
        !names.contains("emptyuser") && !names.contains("mynet"),
        "empty user nets gone"
    );
}
...
async fn flow_network_connect_idempotent_no_refcount_leak() {
    ...
    assert_eq!(
        net_endpoint_count(&app, "mynet").await,
        1,
        "first connect allocates one endpoint"
    );
    ...
    assert_eq!(
        r.status(),
        StatusCode::OK,
        "hl re-connect is idempotent 200 (docker would 403/500)"
    );
    assert_eq!(
        net_endpoint_count(&app, "mynet").await,
        1,
    ...
    assert_eq!(
        net_members(&app, "mynet").await.len(),
        0,
        "membership cleared in one disconnect"
    );
    ...
    assert_eq!(
        r.status(),
        StatusCode::OK,
        "hl double-disconnect is idempotent 200 (docker would 500)"
    );
    assert_eq!(
        net_endpoint_count(&app, "mynet").await,
        0,
        "still zero endpoints"
    );
    ...
    assert_eq!(
        r.status(),
        StatusCode::NO_CONTENT,
        "empty net deletes (refcount reached zero)"
    );
}
...
async fn network_create_fails_when_state_cannot_persist() {
    ...
    let app = App {
        state_path: base.volumes_dir.clone(),
        ..base
    };
    let body = axum::Json(
        serde_json::from_value::<crate::networks::NetCreateBody>(
