...
async fn save_load_round_trips_lifecycle_metadata() {
    ...
    let rootfs = std::path::PathBuf::from(&app.images_dir)
        .join("lifecycle")
        .join("rootfs");
...
async fn image_id_is_stable_sha256_shared_by_list_and_inspect() {
    ...
    assert_eq!(
        list_id, inspect_id,
        "list id == inspect id for the same image"
    );
...
async fn images_json_wire_shape() {
    ...
    for key in [
        ...
        assert!(
            ...
            "image summary missing wire key {key}"
        );
    }
    assert!(
    ...
    assert_eq!(
        img["RepoTags"].as_array().unwrap()[0],
        "alpine:3.19",
...
async fn system_df_wire_shape_and_counts() {
    ...
    for key in [
        ...
        assert!(
            obj.contains_key(key),
            "system df missing top-level key {key}"
        );
    }
    ...
    assert_eq!(
        v["ContainerUsage"]["ActiveCount"], 1,
...
async fn image_tag_missing_source_is_404() {
    ...
    let resp = crate::images::image_tag(State(app.clone()), Path("ghost".into()), Query(q)).await;
...
async fn image_rmi_shared_rootfs_untags_only_and_keeps_sibling() {
    ...
    let resp = crate::images::image_delete(
        State(app.clone()),
        Path("alpine:3.19".into()),
        Query(empty_q()),
    )
    .await;
...
async fn image_rmi_missing_is_404() {
    let app = test_app();
    let resp =
        crate::images::image_delete(State(app.clone()), Path("ghost".into()), Query(empty_q()))
            .await;
    assert_eq!(resp.status(), StatusCode::NOT_FOUND);
}
...
async fn flow_image_tag_untag_delete_shared_rootfs_lifecycle() {
    ...
    assert_eq!(r.status(), StatusCode::CREATED);
    {
        ...
        let a2 = g
            ...
            .expect("a:2 aliased");
        assert_eq!(
            a2.rootfs, "/store/shared-R/rootfs",
            "alias shares the source rootfs"
        );
    }
    ...
    assert!(
        tags.contains("a:1") && tags.contains("a:2"),
        "both tags listed: {tags:?}"
    );

    // Step 3: `rmi a:1` — 200, an UNTAG only (a:2 still references the rootfs) — no Deleted record.
    let r =
        crate::images::image_delete(State(app.clone()), Path("a:1".into()), Query(empty_q())).await;
    assert_eq!(r.status(), StatusCode::OK);
    let report = to_body_json(r).await;
    let arr = report.as_array().unwrap();
    assert!(
        arr.iter().any(|x| x.get("Untagged").is_some()),
        "a:1 untag: {report}"
    );
    assert!(
    ...
    );
    {
        ...
        assert!(
            g.images.iter().any(|i| i.name == "a:2"),
            "sibling a:2 survives"
        );
    }

    // Step 4: `rmi a:2` — now the LAST reference, so a true Deleted (sha256) record.
    let r =
        crate::images::image_delete(State(app.clone()), Path("a:2".into()), Query(empty_q())).await;
    assert_eq!(r.status(), StatusCode::OK);
    let report = to_body_json(r).await;
    let arr = report.as_array().unwrap();
    assert!(
    ...
    assert!(
        image_repotags(&app).await.is_empty(),
        "store empty after both tags removed"
    );
}
...
async fn flow_rmi_image_in_use_by_container_diverges_from_docker_409() {
    ...
    assert_eq!(
        app.inner.lock().await.containers[&cid].image,
        "busy:1",
    ...
    let g = app.inner.lock().await;
    assert!(
        !g.images.iter().any(|i| i.name == "busy:1"),
        "forced rmi removed the image"
    );
    assert!(
        g.containers.contains_key(&cid),
        "the container remains (now dangling)"
    );
}
...
async fn flow_image_three_tag_chain_delete_middle_first() {
    ...
    let r =
        crate::images::image_delete(State(app.clone()), Path("a:3".into()), Query(empty_q())).await;
    assert_eq!(r.status(), StatusCode::OK);
    let arr = to_body_json(r).await;
    let arr = arr.as_array().unwrap();
    assert!(
    ...
    assert!(
        image_repotags(&app).await.is_empty(),
        "store empty after the last tag is removed"
    );
}
...
async fn nonforced_rmi_last_alias_409_when_container_uses_rootfs_under_old_tag() {
    ...
    let r = crate::images::image_delete(State(app.clone()), Path("new:1".into()), Query(empty_q()))
        .await;
    assert_eq!(
        r.status(),
        StatusCode::CONFLICT,
        "rmi must refuse while a container uses the rootfs"
    );
    assert!(
        app.inner
            .lock()
...
async fn commit_captures_upper_writes_and_user() {
    ...
    let img = g
        .images
        .iter()
        .find(|i| i.name == "committed:v1")
        .expect("committed image registered");
    ...
    assert!(
        std::path::Path::new(&img.rootfs).join("added").exists(),
        "commit captures new files"
    );
...
async fn discover_images_restores_labels_and_arch_from_sidecar() {
    ...
    let img = imgs
        ...
        .expect("discovered");
    assert_eq!(
        img.labels.get("com.example.k").map(String::as_str),
        Some("v"),
        "sidecar labels restored"
    );
    assert_eq!(
        img.arch,
        hl_jit::Guest::LinuxX86_64,
        "sidecar arch restored (not arm64 fallback)"
    );
}
...
async fn create_healthcheck_none_disables_probe() {
    ...
    assert!(
        c.healthcheck.is_none(),
        "Test=[NONE] must disable the healthcheck (no fake health monitor)"
    );
}
...
async fn export_includes_upper_layer_writes() {
    ...
    assert!(
        contains(b"written.txt"),
        "export must include the container's writable-layer write"
    );
    let _ = std::fs::remove_dir_all(&base);
}
...
async fn tag_alias_survives_discovery() {
    ...
    std::fs::write(
        dir.join("hl-image.json"),
        serde_json::json!({"name":"base:latest","arch":"aarch64","os":"linux"}).to_string(),
    )
    .unwrap();
    {
        let mut g = app.inner.lock().await;
        g.images.push(Image {
            name: "base:latest".into(),
            rootfs: rootfs.clone(),
            ..Default::default()
        });
    }
    ...
    let q: crate::images::TagQ =
        serde_json::from_value(serde_json::json!({"repo":"alias","tag":"v2"})).unwrap();
    let r =
    ...
    assert!(
        rediscovered
            .iter()
            .any(|i| i.name == "alias:v2" && i.rootfs == rootfs),
...
async fn stats_num_procs_agrees_with_pids_and_memory_shape() {
    ...
    let q: crate::containers::StatsQ =
        serde_json::from_value(serde_json::json!({"stream":"false"})).unwrap();
    let resp = crate::containers::containers_stats(
        State(app.clone()),
        Path("statsone0000".into()),
        Query(q),
    )
    .await;
    let v = to_body_json(resp).await;
    assert_eq!(
        v["num_procs"], v["pids_stats"]["current"],
        "num_procs must equal pids_stats.current"
    );
    let mem = v["memory_stats"].as_object().unwrap();
    assert!(
        mem.contains_key("max_usage"),
        "memory_stats must carry max_usage"
    );
    assert!(
        mem.contains_key("failcnt"),
        "memory_stats must carry failcnt"
    );
}
...
async fn distribution_inspect_is_honest_not_fabricated() {
    ...
    assert_eq!(
        ...
        "no truthful descriptor -> honest 404"
    );
    let v = to_body_json(resp).await;
    assert!(
    ...
    assert!(
        v.get("Descriptor").is_none(),
        "must not fabricate a manifest descriptor"
    );
    assert!(
