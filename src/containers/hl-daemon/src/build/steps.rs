...
mod tests {
    ...
    fn add_local_tar_is_extracted_into_destination() {
        ...
        assert!(
            rootfs.join("out/inside.txt").is_file(),
            "archive contents extracted into dest"
        );
        assert!(
            !rootfs.join("out/payload.tar").exists(),
            "archive file itself must not be copied"
        );
    ...
    fn copy_local_tar_is_not_extracted() {
        ...
        assert!(
            rootfs.join("out/payload.tar").is_file(),
            "COPY copies the tar verbatim"
        );
        assert!(
            !rootfs.join("out/inside.txt").exists(),
            "COPY must not extract"
        );
        let _ = std::fs::remove_dir_all(&base);
    }
    ...
    fn copy_does_not_follow_symlinked_destination() {
        ...
        assert!(
            ...
            "copy must not write through the dest symlink"
        );
        let md = std::fs::symlink_metadata(rootfs.join("dstlink")).unwrap();
        assert!(
            !md.file_type().is_symlink(),
            "dest symlink replaced by a real entry"
        );
        assert!(
            rootfs.join("dstlink").is_file(),
            "payload landed at the literal dest path"
        );
        let _ = std::fs::remove_dir_all(&base);
    }
    ...
    fn copy_chmod_applies_mode_to_destination() {
        ...
        let mode = std::fs::metadata(rootfs.join("bin/tool"))
            .unwrap()
            .permissions()
            .mode();
        assert_eq!(
            mode & 0o777,
            0o755,
            "destination has the requested 0755 mode"
        );
        let _ = std::fs::remove_dir_all(&base);
    }
    ...
    fn copy_chown_numeric_is_parsed_and_copy_succeeds() {
        ...
        .expect("COPY --chown");
        assert!(
            rootfs.join("tool").is_file(),
            "the file is copied regardless of chown privilege"
        );
...
pub(super) async fn run_step(
    ...
    // `HL_GUEST_ENV` — the launch_config mapper only translates known `HL_*`/`HL_JIT_*` keys and would drop
...
pub(super) fn copy_step(
    ...
    if std::fs::symlink_metadata(&dst_host)
