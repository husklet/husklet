...
fn unpack_build_context(
    ...
    if let Err(e) = crate::util::Archive::validate(&ctar) {
        let _ = std::fs::remove_file(&ctar);
        return Err(e);
    }
    let ok = matches!(
...
struct BuildContext;
impl BuildContext {
    fn dockerfile(ctx: &std::path::Path, dfname: &str) -> Option<String> {
    ...
    fn wildcard(pat: &str, s: &str) -> bool {
        ...
        impl Wildcard {
            ...
            fn matches(p: &[u8], s: &[u8]) -> bool {
                ...
                match p[0] {
                    b'*' => {
                        Self::matches(&p[1..], s) || (!s.is_empty() && Self::matches(p, &s[1..]))
                    }
                    b'?' => !s.is_empty() && Self::matches(&p[1..], &s[1..]),
                    c => !s.is_empty() && s[0] == c && Self::matches(&p[1..], &s[1..]),
                }
            }
        }
        Wildcard::matches(pat.as_bytes(), s.as_bytes())
    }
    ...
    fn segments(p: &[&str], t: &[&str]) -> bool {
        ...
        if p[0] == "**" {
            ...
            (0..=t.len()).any(|i| Self::segments(&p[1..], &t[i..]))
        } else if t.is_empty() {
            false
        } else if Self::wildcard(p[0], t[0]) {
            Self::segments(&p[1..], &t[1..])
        } else {
            false
        }
    }
    ...
    fn pattern(pat: &str, rel: &str) -> bool {
        ...
        (1..=t.len()).any(|k| Self::segments(&p, &t[..k]))
    }

    /// A parsed `.dockerignore`: `(negated, pattern)` in file order. `!` re-includes (last match wins).
    fn dockerignore(ctx: &std::path::Path) -> Vec<(bool, String)> {
    ...
    fn excluded(rel: &str, patterns: &[(bool, String)]) -> bool {
        let mut ex = false;
        for (neg, pat) in patterns {
            if Self::pattern(pat, rel) {
                ex = !neg;
            }
        }
        ex
    ...
    fn collect_dirs(dir: &std::path::Path, dirs: &mut Vec<std::path::PathBuf>) {
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
                let path = entry.path();
                if path.is_dir() && !path.is_symlink() {
    ...
    fn apply(ctx: &std::path::Path, dfname: &str) {
        let patterns = Self::dockerignore(ctx);
        ...
        Self::collect_files(ctx, &mut files);
        for f in files {
...
pub(crate) async fn images_build(
    ...
    // `docker run org/app:v2` finds it. A bare component alone would collapse it to `app`, dropping the
    // tag from `RepoTags` and colliding distinct images that share a short name.
    let name = ImageRef::with_tag(&raw_tag, "").short();
    let dfname = q
        .dockerfile
        .filter(|d| !d.is_empty())
    ...
    let buildargs: HashMap<String, String> = match q
        .buildargs
        .as_deref()
        ...
        .transpose()
    {
        Ok(arguments) => arguments.map(BuildArgs::into_map).unwrap_or_default(),
        Err(error) => return BuildResponse::error(log, format!("invalid buildargs: {error}")),
    };
    ...
    if std::fs::create_dir_all(&ctx).is_err() {
        return BuildResponse::error(log, "cannot create build dir".into());
    }
    ...
    if let Err(e) = unpack_build_context(&body[..], &ctx, &a.images_dir) {
        cleanup(&ctx);
        return BuildResponse::error(log, e);
    }
    let dockerfile = match BuildContext::dockerfile(&ctx, &dfname) {
        Some(d) => d,
        None => {
            cleanup(&ctx);
            return BuildResponse::error(
                log,
                format!("Cannot locate specified Dockerfile: {dfname}"),
            );
        }
    };
    ...
    let mut steps = match dockerfile.parse::<Dockerfile>() {
        Ok(dockerfile) => dockerfile.into_instructions(),
        Err(error) => {
            cleanup(&ctx);
            return BuildResponse::error(log, format!("invalid Dockerfile: {error}"));
        }
    };
    ...
    if !target.is_empty() {
        let known_stages: Vec<String> = steps
            ...
            .filter_map(|instruction| {
                let w: Vec<&str> = instruction.arguments.split_whitespace().collect();
                w.iter()
        ...
        if !known_stages.iter().any(|s| s == &target) {
            ...
            return BuildResponse::error(
                log,
                format!("target stage \"{target}\" could not be found"),
            );
        }
    }
    ...
    for instruction in &steps {
        if matches!(
            ...
            if let Err(e) = instruction.arguments.parse::<DockerCommand>() {
                cleanup(&ctx);
                return BuildResponse::error(log, format!("{}: {e}", instruction.name));
            }
        }
    }
    ...
    let safe: String = hl_images::Key::from_name(&raw_tag).as_str().to_owned();
    ...
    let mut parent_id: Option<CacheId> = None; // previous step identity, seeded from the base at FROM
    let mut cache_ok = false; // false after the first miss: no more hits this stage (Docker rule)
    let mut pending_fs: Option<CacheId> = None; // a cache-hit fs layer whose rootfs restore is deferred (lazy)
    ...
    let mut i = 0usize;
    while i < steps.len() {
        let instruction = steps[i].clone();
        let inst = instruction.name.as_str();
        ...
        let args = match instruction.substitute(&vars) {
            Ok(arguments) => arguments,
            Err(error) => return fail(log, format!("{inst}: {error}"), &ctx),
        };
        log.push(
        ...
        history.push(crate::model::HistoryEntry {
            ...
            empty_layer: !instruction.mutates_filesystem(),
        });
        ...
        let mut current_cid: Option<CacheId> = None;
        if use_cache && from_done && inst != "FROM" {
            ...
            let cid = CacheId::from_step(parent_id.as_ref(), &desc);
            if inst == "ARG" {
                ...
                if let Some(meta) = hit {
                    ...
                    labels = meta
                        ...
                        .unwrap_or_default();
                    if instruction.mutates_filesystem() {
        ...
        match inst {
            "FROM" => {
                ...
                let mut found = {
                    ...
                    DiscoveredImages::new(&g.images).find(&base).map(&pick)
                };
                if found.is_none() {
                    ...
                    let (dir, archs) = (a.images_dir.clone(), ImagePlatform::arches(None));
                ...
                if use_cache {
                    let base_digest =
                        match Sha256Digest::build_path(std::path::Path::new(&b.rootfs)) {
                            Ok(digest) => digest,
                            Err(error) => {
                                return fail(
                                    log,
                                    format!("cannot digest base image rootfs: {error}"),
                                    &ctx,
                                );
                            }
                        };
                    ...
                    parent_id = Some(CacheId::from_step(None, &seed));
                    cache_ok = true;
                    pending_fs = None;
                }
                ...
                if !b.onbuild.is_empty() {
                    ...
                    for trigger in &b.onbuild {
                        let parsed = match trigger.parse::<Dockerfile>() {
                            Ok(parsed) => parsed,
                            Err(error) => {
                                return fail(log, format!("invalid ONBUILD trigger: {error}"), &ctx)
                            }
                        };
                        triggers.extend(parsed.into_instructions());
                    }
            ...
            "COPY" | "ADD" => {
                    ...
                    .find_map(|p| p.strip_prefix("--from="))
                {
                    if !eff_names.contains_key(fromv) {
                        let ext = {
                            let g = a.inner.lock().await;
                            DiscoveredImages::new(&g.images)
                                .find(fromv)
                                .map(|im| im.rootfs.clone())
                        };
            ...
            "ENV" => {
                ...
                let pairs = match parsed.pairs() {
                    Ok(pairs) => pairs,
                    Err(error) => return fail(log, format!("ENV: {error}"), &ctx),
                };
                for (k, v) in pairs {
            ...
            "WORKDIR" => {
                ...
                workdir = BuildContext::normalize(&joined);
            ...
            "CMD" => match args
                .parse::<DockerCommand>()
                .and_then(|command| command.resolve(&shell))
            {
                Ok(arguments) => cmd = arguments,
                Err(error) => return fail(log, format!("CMD: {error}"), &ctx),
            },
            "ENTRYPOINT" => match args
                .parse::<DockerCommand>()
                .and_then(|command| command.resolve(&shell))
            {
                Ok(arguments) => entrypoint = arguments,
                Err(error) => return fail(log, format!("ENTRYPOINT: {error}"), &ctx),
            },
            "LABEL" => {
                let parsed = Instruction {
                    name: "LABEL".into(),
                ...
                let pairs = match parsed.pairs() {
                    Ok(pairs) => pairs,
                    Err(error) => return fail(log, format!("LABEL: {error}"), &ctx),
                };
                for (k, v) in pairs {
                    labels.insert(k, v);
                }
            ...
            "SHELL" => match args.parse::<DockerCommand>() {
                Ok(DockerCommand::Exec(arguments)) if !arguments.is_empty() => shell = arguments,
                Ok(_) => {
                    return fail(
                        log,
                        "SHELL requires a non-empty exec-form array".into(),
                        &ctx,
                    )
                }
                Err(error) => return fail(log, format!("SHELL: {error}"), &ctx),
            },
    ...
    if use_cache {
        if let Some(fsid) = pending_fs.take() {
            if !bc.materialize(&fsid, &rootfs) {
                ...
                return BuildResponse::error(
                    log,
                    "build cache: failed to restore the final layer".into(),
                );
            }
        }
    }
    ...
    let id = {
        let rootfs_digest = match Sha256Digest::build_path(&rootfs) {
            Ok(digest) => digest,
            Err(error) => {
                return fail(log, format!("cannot digest image rootfs: {error}"), &ctx);
            }
        };
        ...
        let manifest = format!(
            "dockerfile:\n{dockerfile}\nrootfs:{}\ncmd:{}\nentrypoint:{}\nenv:{}\nworkdir:{workdir}\nlabels:\n{labels_str}",
            rootfs_digest, cmd.join("\u{1}"), entrypoint.join("\u{1}"), env.join("\u{1}"));
        Sha256Digest::from_bytes(manifest.as_bytes()).to_string()
    };
    {
        let mut g = a.inner.lock().await;
        g.images
            .retain(|im| ImageRef::from(&im.name).short() != ImageRef::from(&name).short());
        g.images.push(Image {
            name: name.clone(),
    ...
    BuildResponse::stream(log)
}

#[cfg(test)]
mod handler_tests {
    use super::{unpack_build_context, BuildContext};
    ...
    fn read_context_dockerfile_rejects_symlink_escape() {
        ...
        assert_eq!(
            BuildContext::dockerfile(&ctx, "Dockerfile").as_deref(),
            Some("FROM inside\n")
        );
        ...
        .unwrap();
        assert_eq!(
            BuildContext::dockerfile(&ctx, "Dockerfile.link"),
            None,
