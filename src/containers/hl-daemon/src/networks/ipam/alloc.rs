...
impl Net {
    ...
    pub(crate) fn allocate_ip(&self) -> Option<String> {
        let net = self;
    ...
    pub(crate) fn join_network(
        ...
        Net::join_network_ex(nets, net_name, cid, cname, None, &[])
    }
    ...
    pub(crate) fn join_network_ex(
        ...
        let ip = match req_ip.filter(|ip| !ip.is_empty() && !taken(ip)) {
            ...
            None => n.allocate_ip()?, // subnet exhausted -> fail the join rather than double-issue an address
        };
    ...
    pub(crate) fn leave(&mut self, cid: &str) {
        self.containers.retain(|c| c != cid);
        self.endpoints.remove(cid);
    }

    pub(crate) fn json(&self) -> crate::api::NetworkJson {
        let n = self;
        ...
        NetworkJson {
            ...
            scope: n.scope.clone(),
            containers,
            created: Timestamp::seconds(n.created).to_string(),
            enable_ipv6: false,
            internal: false,
            ipam: Ipam {
                driver: "default",
    ...
    pub(crate) fn matches(&self, id: &str) -> bool {
        self.id == id || self.name == id || self.id.starts_with(id)
    }
}
...
mod tests {
    ...
    fn alloc_subnet_skips_occupied() {
        ...
        let (sub, gw) = Net::alloc_subnet(&nets);
    ...
    fn alloc_ip_first_is_dot2() {
        ...
        assert_eq!(n.allocate_ip().as_deref(), Some("172.18.0.2"));
    }
    ...
    fn alloc_ip_skips_used() {
        ...
        assert_eq!(n.allocate_ip().as_deref(), Some("172.18.0.4"));
    }
    ...
    fn alloc_ip_continues_past_first_octet_into_the_16() {
        ...
        assert_eq!(n.allocate_ip().as_deref(), Some("172.18.1.0"));
    }
    ...
    fn rename_endpoints_reales_across_joined_networks() {
        ...
        let updated = Net::rename_endpoints(&mut nets, "cid1", "app");
    ...
    fn ip_mac_deterministic() {
        assert_eq!(Net::ip_mac("172.18.0.2"), "02:42:ac:12:00:02");
    }

    #[test]
    fn ip_mac_malformed_input() {
        // Not four dotted octets -> the fixed placeholder.
        assert_eq!(Net::ip_mac("not-an-ip"), "02:42:00:00:00:00");
    }
}
