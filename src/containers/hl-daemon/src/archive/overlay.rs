...
pub(crate) struct MountLayout;
impl MountLayout {
    pub(crate) fn binds(mounts: &[Mount]) -> Vec<String> {
        mounts
            .iter()
    ...
    pub(crate) fn readonly(binds: &[String], mounts: &[Mount]) -> Vec<String> {
        let mut targets = Vec::new();
        for b in binds {
            if let Some((_, dst, true)) = crate::containers::Bind::parse(b) {
    ...
    pub(crate) fn contains(path: &str, ro_targets: &[String]) -> bool {
...
pub(crate) fn archive_host_path(
    ...
    if let Some((host, cont)) = best {
        return ContainedPath::join(&host, &path[cont.len()..]);
    }
    ContainedPath::join(rootfs, path)
}
...
struct Overlay;
impl Overlay {
    fn copy(src: &std::path::Path, dst: &std::path::Path) -> std::io::Result<()> {
...
pub(crate) fn stage_overlay_merge(
    ...
    if lower.is_dir() && !opaque {
        for e in std::fs::read_dir(lower)?.flatten() {
            ...
            Overlay::copy(&e.path(), &dest.join(&name))?;
        }
    }
...
impl StagedGet {
    fn temporary(prefix: &str) -> Option<std::path::PathBuf> {
...
pub(crate) fn merged_overlay_dir(
    ...
    if up != ContainedPath::join(upper, path) {
        return None;
    }
    let low = ContainedPath::join(lower, path);
    ...
    let tmp_root = StagedGet::temporary("hl-cp-get")?;
...
pub(crate) struct ContainedPath;
impl ContainedPath {
    pub(crate) fn join(base: &str, rel: &str) -> std::path::PathBuf {
...
impl PathStat {
    pub(crate) fn header(host: &std::path::Path) -> Option<String> {
        ...
        let stat = json!({"name": name, "size": md.len(), "mode": u32::from(FileMode::from(&md)),
        "mtime": Timestamp::seconds(md.mtime()).to_string(), "linkTarget": link});
        Some(Base64::encode(stat.to_string().as_bytes()))
    }
}
...
mod tests {
    ...
    fn readonly_targets_cover_ro_binds_and_mounts() {
        ...
        let t = MountLayout::readonly(&binds, &mounts);
    ...
    fn stage_overlay_merge_unions_layers_and_honors_whiteouts_and_opaque() {
        ...
        assert!(
            dest.join("alpine-release").exists(),
            "lower-only entry dropped from merge"
        );
        assert!(
            dest.join("hosts").exists(),
            "upper-only entry missing from merge"
        );
        assert_eq!(
            std::fs::read(dest.join("shadowed")).unwrap(),
            b"upper\n",
            "upper must shadow lower"
        );
        assert!(
            !dest.join("secret").exists(),
            "whiteout did not hide lower entry"
        );
        assert!(
            !dest.join(".wh.secret").exists(),
            "whiteout control file leaked into tar view"
        );
        ...
        assert!(
            !od.join("fromlower").exists(),
            "opaque dir must hide all lower entries"
        );
        assert!(
            !od.join(".wh..wh..opq").exists(),
            "opaque marker leaked into tar view"
        );
    ...
    fn path_under_readonly_mount_matches_target_and_children() {
        ...
        assert!(MountLayout::contains("/data", &ro), "the mount root itself");
        assert!(
            MountLayout::contains("/data/sub/x.txt", &ro),
            "a child path"
        );
        assert!(MountLayout::contains("/data/", &ro), "trailing slash");
        assert!(
            !MountLayout::contains("/database", &ro),
            "sibling prefix must NOT match"
        );
        assert!(!MountLayout::contains("/other", &ro));
    }
}
