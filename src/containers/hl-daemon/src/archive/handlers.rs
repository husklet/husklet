...
pub(crate) async fn archive_head(
    ...
    let binds: Vec<String> = c
        ...
        .iter()
        .cloned()
        .chain(MountLayout::binds(&c.mounts))
        .collect();
    match PathStat::header(&overlay_host_path(
        &c.upper,
        &c.rootfs,
        &binds,
        &a.volumes_dir,
...
pub(crate) async fn archive_get(
    ...
    let Some(stat) = PathStat::header(&host) else {
        return (
            StatusCode::NOT_FOUND,
            Json(crate::api::ErrorMessage {
                message: format!("Could not find the file {} in container {id}", q.path),
    ...
    let out = std::process::Command::new("tar")
        .args(Tar::create_args(&parent, &base))
        .output();
...
pub(crate) async fn archive_put(
    ...
    let (cid, upper, rootfs, binds, ro_targets) = {
        ...
        (
            ...
            c.binds
                ...
                .chain(MountLayout::binds(&c.mounts))
                .collect::<Vec<_>>(),
            MountLayout::readonly(&c.binds, &c.mounts),
        )
    };
    ...
    if MountLayout::contains(&q.path, &ro_targets) {
        return (
            StatusCode::FORBIDDEN,
            Json(crate::api::ErrorMessage {
                message: format!(
                    "cannot copy to {}: mounted path is marked read-only",
                    q.path
                ),
            }),
        )
    ...
    if !host.is_dir() {
        ...
        crate::util::Generation::bump(&cid);
        return (
    ...
    let out = Tar::extract(&host, body.as_ref());
    ...
    crate::util::Generation::bump(&cid);
    match out {
        Ok(o) if o.status.success() => (StatusCode::OK, Json(crate::api::Empty {})).into_response(),
        Ok(o) => {
            eprintln!(
...
struct Tar;
impl Tar {
    fn create_args(parent: &std::path::Path, base: &str) -> Vec<std::ffi::OsString> {
    ...
    fn extract_args(host: &std::path::Path) -> Vec<std::ffi::OsString> {
        use std::ffi::OsString;
        vec![
            OsString::from("-x"),
            OsString::from("-f"),
    ...
    fn entries(body: &[u8]) -> std::io::Result<Vec<String>> {
        use std::io::Write;
        ...
        match child.wait_with_output() {
            Ok(o) if o.status.success() => Ok(String::from_utf8_lossy(&o.stdout)
                ...
                .map(|l| l.to_string())
                .collect()),
            Ok(_) => Err(std::io::Error::other("invalid tar archive")),
            Err(error) => Err(error),
        }
    }
    ...
    fn scrub(host: &std::path::Path, entries: &[String]) {
    ...
    fn extract(host: &std::path::Path, body: &[u8]) -> std::io::Result<std::process::Output> {
        Self::scrub(host, &Self::entries(body)?);
        use std::io::Write;
        use std::process::Stdio;
        let mut child = std::process::Command::new("tar")
            .args(Self::extract_args(host))
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()?;
...
mod tests {
    ...
    fn put_tar_argv_requests_numeric_owner_and_preserve() {
        let s = as_strings(&Tar::extract_args(std::path::Path::new("/dest")));
        assert!(s.iter().any(|a| a == "-x"), "must extract: {s:?}");
        assert!(
            s.iter().any(|a| a == "--numeric-owner"),
            "missing --numeric-owner: {s:?}"
        );
        assert!(
            s.iter().any(|a| a == "-p"),
            "missing -p (preserve perms): {s:?}"
        );
    }
    ...
    fn get_tar_argv_requests_posix_format_for_nanosecond_mtimes() {
        let s = as_strings(&Tar::create_args(std::path::Path::new("/p"), "base"));
        assert!(
            s.iter().any(|a| a == "--format=posix"),
            "missing --format=posix: {s:?}"
        );
    ...
    fn put_does_not_escape_through_existing_destination_symlink() {
        ...
        let out = Tar::extract(&target, &tar.stdout).unwrap();
        assert!(
            out.status.success(),
            "tar stderr: {}",
        ...
        assert!(
            ...
            "write ESCAPED the target via the symlink"
        );
        let md = std::fs::symlink_metadata(target.join("linkout")).unwrap();
        assert!(
            md.file_type().is_dir(),
            "destination component is still a traversing symlink"
        );
        assert_eq!(
            std::fs::read(target.join("linkout/file.txt")).unwrap(),
            b"pwned"
        );
