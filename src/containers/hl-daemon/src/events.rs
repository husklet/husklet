...
impl Filters {
    ...
    fn matches(&self, ev: &Value) -> bool {
        ...
        if !self.actions.is_empty() && !self.actions.iter().any(|a| a == action || a == action_key)
        {
            return false;
        }
        ...
        if !self.images.is_empty() && !self.images.iter().any(|i| i == image || i == name) {
            return false;
        }
...
mod history_tests {
    ...
    fn history_records_events_without_a_subscriber() {
        ...
        assert_eq!(
            all.len(),
            2,
            "both events recorded despite zero live receivers"
        );
        assert_eq!(all[0]["Type"], "container");
        assert_eq!(all[0]["Action"], "create");
        // A `since` in the far future excludes past events.
        assert!(
            bus.history_since(i64::MAX).is_empty(),
            "since in the future replays nothing"
        );
    }
}
...
mod filter_tests {
    ...
    fn malformed_filters_json_is_an_error() {
        assert!(
            Filters::parse(&Some("{\"type\":[\"container\"".into())).is_err(),
            "truncated JSON"
        );
    ...
    fn action_filter_matches_health_status_key() {
        ...
        assert!(
            f.matches(&ev),
            "health_status must match the compound action key"
        );
    }
    ...
    fn label_network_volume_scope_filters_narrow() {
        ...
        assert!(
            !f.matches(&miss),
            "label filter must not leak a non-matching event"
        );
        ...
        assert!(
            !fnet.matches(&unrelated),
            "network filter must not leak unrelated container events"
        );
        ...
        let local_ev = json!({"Type":"container","Action":"start","scope":"local",
            "Actor":{"ID":"c1","Attributes":{}}});
        assert!(
            !fscope.matches(&local_ev),
            "scope=swarm must not match a local event"
        );
    }
}
