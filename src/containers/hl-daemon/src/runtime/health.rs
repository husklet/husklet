...
pub(super) async fn health_monitor(
    ...
    loop {
        ...
        let (next, streak) =
            apply_probe(&h.status, h.failing_streak, code, retries, in_start_period);
        h.status = next;
...
mod tests {
    ...
    fn success_is_healthy_and_clears_streak() {
        assert_eq!(
            apply_probe("starting", 2, 0, 3, false),
            ("healthy".into(), 0)
        );
        assert_eq!(
            apply_probe("unhealthy", 5, 0, 3, false),
            ("healthy".into(), 0)
        );
        // a success DURING the start period is immediately healthy too.
        assert_eq!(
            apply_probe("starting", 0, 0, 3, true),
            ("healthy".into(), 0)
        );
    }
    ...
    fn failures_during_start_period_are_not_counted() {
        ...
        assert_eq!(
            apply_probe("starting", 0, 1, 3, true),
            ("starting".into(), 0)
        );
        assert_eq!(
            apply_probe("starting", 0, 1, 3, true),
        ...
        assert_eq!(
            apply_probe("starting", 0, 1, 3, false),
            ("starting".into(), 1)
        );
    }
