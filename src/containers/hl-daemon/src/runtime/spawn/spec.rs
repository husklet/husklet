...
pub(super) fn resolve_mount_src(src: &str, volumes_dir: &str, vols: &[Vol]) -> String {
    if src.starts_with('/') {
        ...
        let normalized = RuntimePath::normalize(&root.join(src));
...
impl Resources {
    pub(super) fn cpus(nano_cpus: i64) -> u32 {
        if nano_cpus > 0 {
...
impl Sandbox {
    pub(super) fn requested(security_opt: &[String]) -> bool {
...
mod tests {
        ...
        for bad in ["../escaped", "../../etc", "a/../../b"] {
            ...
            assert!(
                got.starts_with("/vols/")
                    && !RuntimePath::normalize(std::path::Path::new(&got)).starts_with("/etc"),
    ...
    fn nano_cpus_to_cpus_ceils_and_zeroes() {
        assert_eq!(Resources::cpus(0), 0); // unset -> unlimited
        assert_eq!(Resources::cpus(-1), 0); // negative -> unlimited
        assert_eq!(Resources::cpus(1_000_000_000), 1); // exactly 1 CPU
        assert_eq!(Resources::cpus(1_500_000_000), 2); // 1.5 -> 2 whole CPUs
        assert_eq!(Resources::cpus(1), 1); // any positive fraction -> at least 1
    }

    #[test]
    fn wants_sandbox_matches_case_insensitive_substring() {
        assert!(Sandbox::requested(&["seccomp=x".into(), "SANDBOX".into()]));
        assert!(Sandbox::requested(&["label=Untrusted".into()]));
        assert!(!Sandbox::requested(&["seccomp=unconfined".into()]));
        assert!(!Sandbox::requested(&[]));
    }
}
