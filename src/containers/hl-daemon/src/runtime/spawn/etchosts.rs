...
impl Hosts {
    pub(super) fn names(own_name: &str, hostname: &str) -> String {
    ...
    pub(super) fn render(own: Option<(&str, &str)>, peers: &[(String, String)]) -> String {
    ...
    pub(super) fn hostname(id: &str, hostname: &str) -> String {
...
mod tests {
    ...
    fn render_etc_hosts_own_then_peers_in_order() {
        ...
        let out = Hosts::render(Some(("172.18.0.2", "web myhost")), &peers);
        assert_eq!(
            out,
            "127.0.0.1\tlocalhost\n\
    ...
    fn render_etc_hosts_own_no_peers() {
        let out = Hosts::render(Some(("172.18.0.2", "web")), &[]);
        assert_eq!(out, "127.0.0.1\tlocalhost\n172.18.0.2\tweb\n");
    }

    #[test]
    fn eff_hostname_prefers_user_value_else_short_id() {
        assert_eq!(Hosts::hostname("0123456789abcdef0000", "myhost"), "myhost");
        // empty hostname -> first 12 chars of the id.
        assert_eq!(Hosts::hostname("0123456789abcdef0000", ""), "0123456789ab");
        // a short id (< 12 chars) is used whole, never panics on the slice bound.
        assert_eq!(Hosts::hostname("abc", ""), "abc");
    }
}
