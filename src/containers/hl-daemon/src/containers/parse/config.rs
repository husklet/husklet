...
impl Bind {
    pub(crate) fn parse(b: &str) -> Option<(&str, &str, bool)> {
...
impl Signal {
    pub(crate) fn parse(s: &str, default: i32) -> i32 {
        let t = s.trim();
        if t.is_empty() {
            return default;
        }
...
pub(crate) struct QueryFlag;
impl QueryFlag {
    pub(crate) fn is_true(s: &Option<String>) -> bool {
...
mod tests {
    ...
    #[test]
    fn bind_src_dst() {
        assert_eq!(Bind::parse("/h:/c"), Some(("/h", "/c", false)));
    }
    #[test]
    fn bind_ro_flag() {
        assert_eq!(Bind::parse("/h:/c:ro"), Some(("/h", "/c", true)));
        // `ro` may appear among a comma-list of options.
        assert_eq!(Bind::parse("vol:/c:ro,z"), Some(("vol", "/c", true)));
    }
    #[test]
    fn bind_rw_flag() {
        assert_eq!(Bind::parse("/h:/c:rw"), Some(("/h", "/c", false)));
        assert_eq!(Bind::parse("/h:/c:rw,z"), Some(("/h", "/c", false)));
    }
    #[test]
    fn bind_empty_dst_is_none() {
        assert_eq!(Bind::parse("/h:"), None);
    }
    #[test]
    fn bind_splitn3_keeps_extra_colons_in_opts() {
        ...
        assert_eq!(Bind::parse("a:b:c:d"), Some(("a", "b", false)));
    }
    #[test]
    fn bind_no_colon_is_none() {
        assert_eq!(Bind::parse("justsrc"), None);
    }

    // ---- parse_signal -------------------------------------------------------
    #[test]
    fn signal_named_with_prefix() {
        assert_eq!(Signal::parse("SIGTERM", 0), libc::SIGTERM);
    }
    #[test]
    fn signal_named_without_prefix() {
        assert_eq!(Signal::parse("TERM", 0), libc::SIGTERM);
        assert_eq!(Signal::parse("kill", 0), libc::SIGKILL); // case-insensitive
    }
    #[test]
    fn signal_numeric_verbatim() {
        assert_eq!(Signal::parse("15", 0), 15);
        assert_eq!(Signal::parse("9", 0), 9);
    }
    #[test]
    fn signal_junk_falls_back_to_default() {
        assert_eq!(Signal::parse("NOPE", 7), 7);
        assert_eq!(Signal::parse("", 7), 7);
    }
}
