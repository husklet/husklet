...
impl Container {
    pub(super) fn mounts_json(&self, vols: &[Vol]) -> Vec<Value> {
        let c = self;
        let vol_mp = |name: &str| {
            vols.iter()
                .find(|v| v.name == name)
        ...
        for b in &c.binds {
            if let Some((src, dst, ro)) = Bind::parse(b) {
                if src.starts_with('/') {
...
mod tests {
    ...
    fn mount_type_volume_carries_name_and_driver() {
        ...
        let out = c.mounts_json(&[vol("data", "/mp/data")]);
        let m = &out[0];
        assert_eq!(m["Type"], "volume");
