    ...
    pub(crate) async fn inspect(State(a): State<App>, Path(id): Path<String>) -> Response {
        ...
        };
        {
            ...
            let started_at = if c.started_at == 0 {
                ...
                Timestamp::nanos(c.started_at_ns).to_string()
            } else {
                Timestamp::seconds(c.started_at).to_string()
            };
            let finished_at = if c.finished_at == 0 {
                "0001-01-01T00:00:00Z".to_string()
            } else if c.finished_at_ns > 0 {
                Timestamp::nanos(c.finished_at_ns).to_string()
            } else {
                Timestamp::seconds(c.finished_at).to_string()
            };
            ...
            Json(crate::api::ContainerInspect {
                id: c.id.clone(),
                image: c.image.clone(),
                created: Timestamp::seconds(c.created).to_string(),
                name: format!(
                    "/{}",
                    if c.name.is_empty() {
                ...
                mounts: c.mounts_json(&g.volumes),
                host_config,
                ...
                network_settings: crate::api::NetworkSettingsJson {
                    ports: {
                        ...
                        let mut ports = Publish::new(&c.publish).map();
