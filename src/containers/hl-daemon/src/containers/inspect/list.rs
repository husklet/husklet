...
impl Containers {
    pub(crate) async fn list(
        ...
        let want_size = QueryFlag::is_true(&q.size);
        ...
        let v: Vec<crate::api::ContainerSummary> = matched
            ...
            .map(|c| {
                ...
                crate::api::ContainerSummary {
                    ...
                    status: c.human_status(),
                    exit_code: c.exit_code,
                    ports: Publish::new(&c.publish).summaries(),
                    labels: c.labels.clone(),
                    mounts: c.mounts_json(&vols_snapshot),
                    names: vec![format!(
                        "/{}",
                        if c.name.is_empty() {
                            c.id[..12.min(c.id.len())].to_string()
