...
//! - `arch`   — docker `--platform` ↔ hl-arch mapping (`guest_of`/`manifest_arch`/`docker_arch`/`ImagePlatform::arch(s)`).
...
pub(crate) async fn images_create(
    ...
    if name.is_empty() {
        return ErrorMessage::bad_request("fromImage is required");
    }
    ...
    let want = ImageRef::with_tag(&name, &tag).short();
    let want_arch = ImagePlatform::arch(q.platform.as_deref());
    let creds = Progress::auth(&headers);
    let archs = ImagePlatform::arches(q.platform.as_deref());
    if a.inner.lock().await.images.iter().any(|i| {
        ImageRef::from(&i.name).short() == want
            && want_arch.map_or(true, |a| ImagePlatform::docker(i.arch) == a)
    }) {
    ...
    Progress::pull_stream(a, name, tag, want, creds, archs)
}
