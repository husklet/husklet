...
impl Images {
    pub(crate) async fn save(State(a): State<App>, Query(q): Query<SaveQ>) -> Response {
        let names = q.names.unwrap_or_default();
        if names.is_empty() {
            return ErrorMessage::bad_request("names is required");
        }
        ...
        let img = {
            ...
            DiscoveredImages::new(&g.images).find(&names).cloned()
        };
        let Some(img) = img else {
            return ErrorMessage::no_such_image(&names);
        };
        ...
        match hl_images::Store::new(&a.images_dir)
