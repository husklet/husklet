...
impl ImageApi {
    pub(crate) async fn list(State(a): State<App>) -> Json<Vec<ImageSummary>> {
    ...
    pub(crate) async fn history(State(a): State<App>, Path(name): Path<String>) -> Response {
        let g = a.inner.lock().await;
        match DiscoveredImages::new(&g.images).find(&name) {
            Some(i) => {
                let total = PathSize::image(&i.rootfs, &i.name);
                if i.history.is_empty() {
                    return Json(vec![HistoryLayer {
                        id: i.id(),
                        created: i.created,
                        created_by: "hl import".to_string(),
                        tags: vec![ImageRef::from(&i.name).short()],
                        size: total,
                        comment: "",
                        empty_layer: false,
                    }])
                ...
                let rows: Vec<HistoryLayer> = i
                    ...
                    .map(|(pos, h)| HistoryLayer {
                        ...
                        tags: if pos == 0 {
                            vec![ImageRef::from(&i.name).short()]
                        } else {
                            vec![]
                        },
            ...
            None => ErrorMessage::no_such_image(&name),
        }
    }
    ...
    pub(crate) async fn prune(State(a): State<App>) -> Json<PruneReport> {
        ...
        for img in &dangling {
            ...
            if !shared {
                ...
                deleted.push(json!({ "Deleted": img.id() }));
            }
            deleted.push(json!({ "Untagged": img.id() }));
        }
    ...
    pub(crate) async fn distribution(Path(name): Path<String>) -> Response {
        (
    ...
    pub(crate) async fn inspect(State(a): State<App>, Path(name): Path<String>) -> Response {
        ...
        if DiscoveredImages::new(&a.inner.lock().await.images)
            .find(&name)
            .is_none()
        {
        ...
        match DiscoveredImages::new(&g.images).find(&name) {
            Some(i) => {
                let tag = ImageRef::from(&i.name).short();
                let size = PathSize::image(&i.rootfs, &i.name);
                ...
                Json(ImageInspect {
                    ...
                    repo_digests: vec![],
                    architecture: ImagePlatform::docker(i.arch).to_string(),
                    os: i.arch.os().to_string(),
                    size,
                    virtual_size: size,
                    // RFC3339 string shape strict clients (bollard) expect; `created` is unix secs.
                    created: Timestamp::seconds(i.created).to_string(),
                    config: ImageConfig {
                        image: tag,
                        cmd: i.cmd.clone(),
                        entrypoint,
