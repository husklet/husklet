...
static void handler(int sig, siginfo_t *si, void *uc) {
    caught++; last_sig = sig; last_code = si->si_code;
    siglongjmp(pad, 1);
}
...
__attribute__((noinline)) static long deep(long n) { volatile char pad[512]; pad[0]=(char)n; long r = n <= 0 ? 0 : deep(n-1); return r + pad[0]; }
int main(void) {
    ...
    sigaction(SIGFPE, &sa, NULL); sigaction(SIGILL, &sa, NULL);
    long acc = 0;
    for (int i = 0; i < 256; i++) {
        volatile long ok = i;
        if (sigsetjmp(pad, 1) == 0) acc += faulting(&ok, i, i+1, i+2, i+3);
        if (sigsetjmp(pad, 1) == 0) acc += faulting((volatile long *)(intptr_t)8, i, i, i, i);
        else acc += 1000;
    }
    ...
    sa.sa_flags = SA_SIGINFO | SA_ONSTACK; sa.sa_sigaction = handler;
