...
__attribute__((noinline)) static uint64_t f0(uint64_t x){ return x*3+1; }
__attribute__((noinline)) static uint64_t f1(uint64_t x){ return x^0x5555555555555555ull; }
__attribute__((noinline)) static uint64_t f2(uint64_t x){ return (x<<7)|(x>>57); }
__attribute__((noinline)) static uint64_t f3(uint64_t x){ return x - 0x9E3779B9ull; }
...
int main(void) {
    ...
    uint64_t h = 0;
    for (int i = 0; i < 256; i++) table[i] = (uint64_t)i * 0x9E3779B97F4A7C15ull;
    ...
    for (int r = 0; r < 4096; r++) h = fns[r & 3](h) ^ (h >> 3);
    // string ops
    ...
    for (int r = 0; r < 4096; r++) {
        ...
        unsigned char c; __asm__ volatile("bt %2,%1\n\tsetc %0" : "=r"(c) : "r"(x), "r"((uint64_t)(r & 63)) : "cc");
        uint64_t m = x; __asm__ volatile("xadd %0,%1" : "+r"(y), "+r"(m) :: "cc");
