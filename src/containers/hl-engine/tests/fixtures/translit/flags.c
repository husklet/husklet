...
__attribute__((noinline)) static int all_cc(uint64_t a, uint64_t b) {
    ...
    __asm__ volatile("cmp %[b],%[a]\n\t"
                     ...
                     "setbe %6\n\tseta %7\n\tsets %8\n\tsetns %9\n\tsetp %10\n\tsetnp %11\n\t"
                     "setl %12\n\tsetge %13\n\tsetle %14\n\tsetg %15\n"
                     : "=m"(r[0]),"=m"(r[1]),"=m"(r[2]),"=m"(r[3]),"=m"(r[4]),"=m"(r[5]),
                       "=m"(r[6]),"=m"(r[7]),"=m"(r[8]),"=m"(r[9]),"=m"(r[10]),"=m"(r[11]),
                       "=m"(r[12]),"=m"(r[13]),"=m"(r[14]),"=m"(r[15])
                     : [a]"r"(a), [b]"r"(b) : "cc");
    int v = 0;
    for (int i = 0; i < 16; i++) v |= (r[i] & 1) << i;
    return v;
}
...
__attribute__((noinline)) static void barrier(void) { sink++; }
...
__attribute__((noinline)) static uint64_t df_copy(void) {
    ...
    for (int i = 0; i < 64; i++) { h ^= (unsigned char)dst[i]; h *= 1099511628211ull; }
    return h;
}
...
int main(void) {
    ...
    printf("flags h=%016llx df=%016llx sink=%llu\n", (unsigned long long)h,
           (unsigned long long)df_copy(), (unsigned long long)sink);
    return 0;
}
