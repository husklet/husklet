...
static void emit(unsigned char *p, int k) {
    ...
    body[5]=(unsigned char)(k&0xff); body[6]=(unsigned char)((k>>8)&0xff);
    body[7]=(unsigned char)((k>>16)&0xff); body[8]=(unsigned char)((k>>24)&0xff);
...
int main(void) {
    ...
    size_t sz = 4096;
    unsigned char *rwx = mmap(NULL, sz, PROT_READ|PROT_WRITE|PROT_EXEC,
                              MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
    if (rwx == MAP_FAILED) { perror("mmap rwx"); return 1; }
    long acc = 0;
    for (int round = 0; round < 64; round++) {
        emit(rwx, round * 3 + 1);
        fn f = (fn)rwx;
        for (int i = 0; i < 16; i++) acc += f(i);
    }
    printf("rwx acc=%ld\n", acc);
    ...
    if (rw == MAP_FAILED) { perror("mmap rw"); return 1; }
    long acc2 = 0;
    for (int round = 0; round < 64; round++) {
        if (mprotect(rw, sz, PROT_READ|PROT_WRITE) != 0) { perror("mprotect rw"); return 1; }
        emit(rw, round * 5 + 2);
        if (mprotect(rw, sz, PROT_READ|PROT_EXEC) != 0) { perror("mprotect rx"); return 1; }
        fn f = (fn)rw;
        for (int i = 0; i < 16; i++) acc2 += f(i);
    }
    printf("rx  acc=%ld\n", acc2);
