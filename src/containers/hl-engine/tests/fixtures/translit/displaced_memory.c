...
static volatile int8_t signed_cell = -7;
...
int main(void) {
    const void *identity;
    uint64_t before, rcx_load;
    int64_t signed_load;
    __asm__ volatile("lea cell(%%rip),%0" : "=r"(identity));
    __asm__ volatile("mov cell(%%rip),%0" : "=a"(before) : : "memory");
    __asm__ volatile("mov cell(%%rip),%0" : "=c"(rcx_load) : : "memory");
    __asm__ volatile("movsbq signed_cell(%%rip),%0" : "=d"(signed_load) : : "memory");
    ...
    printf("displaced ptr=%p before=%016llx rcx=%016llx signed=%lld base=%016llx after=%016llx text=%s\n", identity,
           (unsigned long long)before, (unsigned long long)rcx_load, (long long)signed_load,
           (unsigned long long)base_load(&cell), (unsigned long long)cell, destination);
    return strcmp(destination, source) != 0 || rcx_load != before || signed_load != -7;
}
