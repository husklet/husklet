...
static void *worker(void *arg) {
    ...
    for (uint64_t i = 0; i < 200000; i++) { h = mix(h + i); shared += (h & 1); }
    return (void *)(uintptr_t)h;
}
static int clone_child(void *arg) { (void)arg; _exit(37); }
int main(int argc, char **argv) {
    ...
    if (argc > 1) { printf("exec ok\n"); return 0; }
    // threads: each guest thread needs its own %gs published
    pthread_t t[8]; uint64_t acc = 0;
    for (int i = 0; i < 8; i++) pthread_create(&t[i], NULL, worker, (void *)(uintptr_t)(i + 1));
    for (int i = 0; i < 8; i++) { void *r; pthread_join(t[i], &r); acc = acc * 131 + (uint64_t)(uintptr_t)r; }
    ...
    for (int i = 0; i < 8; i++) {
        pid_t p = fork();
        if (p == 0) { uint64_t h = mix(i); for (int k=0;k<50000;k++) h = mix(h+k); _exit((int)(h & 0x3f)); }
        int st; waitpid(p, &st, 0); total += WEXITSTATUS(st);
    }
    printf("fork total=%d\n", total);
    // vfork + execve, into this same static-PIE image
    for (int i = 0; i < 4; i++) {
        pid_t p = vfork();
        if (p == 0) { execl(argv[0], argv[0], "child", (char *)NULL); _exit(99); }
        int st; waitpid(p, &st, 0); total += WEXITSTATUS(st);
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (p < 0) { cs = -1; break; }
        int st; waitpid(p, &st, 0); cs += WEXITSTATUS(st);
    }
