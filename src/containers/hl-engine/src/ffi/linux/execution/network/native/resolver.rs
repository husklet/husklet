...
impl Resolver {
    pub(super) fn accepts(address: &hl_network::SocketAddress) -> bool {
        matches!(
            address,
    ...
    pub(super) fn answer(query: &[u8]) -> Option<Vec<u8>> {
        ...
        let (name, end) = Self::question_name(query, HEADER)?;
        if end + 4 > query.len() {
            return None;
        }
...
mod tests {
    ...
    fn rejects_malformed_compressed_and_oversized_questions() {
        ...
        assert!(Resolver::answer(&compressed).is_none());

        let mut oversized = vec![0; MESSAGE_LIMIT + 1];
        oversized[5] = 1;
        assert!(Resolver::answer(&oversized).is_none());
    }

    #[test]
    fn queue_admission_bounds_packets_and_total_bytes() {
        assert!(Resolver::queue_available(QUEUE_LIMIT - 1, 0, 512));
        assert!(!Resolver::queue_available(QUEUE_LIMIT, 0, 1));
        assert!(!Resolver::queue_available(0, BYTE_LIMIT, 1));
        assert!(!Resolver::queue_available(0, 0, BYTE_LIMIT + 1));
    }
}
