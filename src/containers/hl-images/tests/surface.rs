...
fn public_snapshot_and_platform_surface_is_composable() {
    ...
    let active = snapshots
        .prepare(Id::new("layer-1").unwrap(), None)
        .unwrap();
    ...
    assert_eq!(
        snapshots.view(committed.id()).unwrap().path(),
        committed.path()
    );
    ...
    assert_eq!(
        std::fs::read_to_string(reopened.path().join("hello")).unwrap(),
        "world"
    );
    drop(reopened);
    assert!(leases.get(&lease_id).unwrap().is_some());
...
fn snapshot_fork_preserves_unreadable_files() {
    ...
    let child = snapshots
        .prepare(Id::new("child").unwrap(), Some(parent.id()))
        .unwrap();
    assert_eq!(
...
fn snapshot_removal_handles_readonly_directories() {
    ...
    let snapshots = Snapshots::open(temp.path()).unwrap();
    let draft = snapshots
        .prepare(Id::new("readonly").unwrap(), None)
        .unwrap();
    ...
    let snapshot = draft
        .commit(Id::new("readonly-committed").unwrap())
        .unwrap();
