...
impl Client {
    ...
    pub fn pull(
        ...
        let layers = manifest["layers"]
            .as_array()
            .ok_or_else(|| Error::Manifest("manifest has no layers array".to_string()))?;
        ...
        for layer in layers {
            ...
            if !MediaType::new(media).supports_layer() {
            ...
            let id = LayerId::from_digest(&digest).as_str().to_owned();
        ...
        let install = Install::new(rootfs);
        let staging = install.sibling("staging");
        LayerRootfs::new(&staging).reset()?;
        let staged = (|| {
    ...
    pub(super) fn resolve_manifest(&mut self, want_archs: &[&str]) -> Result<Value, Error> {
        ...
        let Some(list) = man["manifests"].as_array() else {
            return Manifest(man).validate(); // already a single manifest
        };
        let digest = want_archs
            .iter()
            .find_map(|arch| Platforms(list).select(arch))
            .ok_or_else(|| {
    ...
    pub(super) fn config_blob(&mut self, manifest: &Value) -> Result<Value, Error> {
        ...
        verify_digest(
            "config",
            digest,
            &crate::image::digest::Sha256Digest::from_bytes(&bytes).to_string(),
        )?;
    ...
    fn unpack_layer(
        ...
        let out = (|| {
            ...
            verify_digest(
                &format!("layer {id}"),
                digest,
                &crate::image::digest::Sha256Digest::file(&tmp)?.oci(),
            )?;
            if let Some(want) = diff_id {
                verify_digest(
                    &format!("layer {id} diff_id"),
                    want,
                    &crate::image::digest::Sha256Digest::gzip_file(&tmp)?.oci(),
                )?;
            }
            ...
            LayerRootfs::new(rootfs).clear_opaque(&LayerRootfs::opaque_dirs_in_tar(&tmp));
            crate::image::archive::Archive::new(&tmp).extract_layer(rootfs)
        })();
        let _ = std::fs::remove_file(&tmp);
        out?;
        LayerRootfs::new(rootfs).apply_whiteouts()
    }
}
...
struct Platforms<'a>(&'a [Value]);
impl Platforms<'_> {
    fn select(&self, arch: &str) -> Option<String> {
...
impl Manifest {
    fn validate(self) -> Result<Value, Error> {
        let man = self.0;
        ...
        if let Some(media) = man["mediaType"].as_str() {
            if !MediaType::new(media).supports_manifest() {
                return Err(Error::Manifest(format!(
...
impl<'a> Install<'a> {
    ...
    fn sibling(&self, tag: &str) -> PathBuf {
        let parent = self
            .target
            .parent()
    ...
    fn commit(&self, staging: &Path) -> Result<(), Error> {
        let final_ = self.target;
...
mod tests {
    ...
    fn scratch(tag: &str) -> PathBuf {
        let n = uniq();
        let p =
            std::env::temp_dir().join(format!("hl-pull-test-{}-{}-{}", tag, std::process::id(), n));
    ...
    fn make_layer(files: &[(&str, &[u8])]) -> (Vec<u8>, String, String) {
        ...
        let digest = crate::image::digest::Sha256Digest::from_bytes(&bytes).oci();
        let diff_id = crate::image::digest::Sha256Digest::gzip_file(&tar)
            .unwrap()
            .oci();
    ...
    fn good_routes() -> (Routes, String, Vec<u8>) {
        ...
        let cdigest = crate::image::digest::Sha256Digest::from_bytes(&cfg).oci();
        let manifest = json!({
            "schemaVersion": 2,
        ...
        r.insert(
            ...
            (200, MEDIA_MANIFEST.to_string(), manifest.clone()),
        );
    ...
    fn pull_rejects_invalid_config() {
        ...
        let cdigest = crate::image::digest::Sha256Digest::from_bytes(&garbage).oci();
        ...
        routes.insert(
            MANIFEST_PATH.to_string(),
            (200, MEDIA_MANIFEST.to_string(), manifest),
        );
        routes.insert(
            blob_path(&cdigest),
            (200, MEDIA_CONFIG.to_string(), garbage),
        );
    ...
    fn pull_rejects_schema_version_1() {
        ...
        r.insert(
            MANIFEST_PATH.to_string(),
            (200, "application/json".to_string(), manifest),
        );
        let m = Mock::start(r);
    ...
    fn pull_accepts_zero_layer_scratch() {
        ...
        m.client()
            .pull(&rootfs, &["amd64"], &mut |_| {})
            .expect("scratch pull ok");
        assert!(rootfs.is_dir(), "empty rootfs created");
        assert_eq!(
            std::fs::read_dir(&rootfs).unwrap().count(),
            0,
            "rootfs is empty"
        );
        let _ = std::fs::remove_dir_all(rootfs.parent().unwrap());
    ...
    fn failed_pull_preserves_previous_rootfs() {
        ...
        assert!(
            !rootfs.join("first.txt").exists(),
            "no partial layer in final rootfs"
        );
    ...
    fn select_platform_honors_variant() {
        ...
        assert_eq!(
            Platforms(&list).select("arm/v7"),
            Some("sha256:v7".to_string())
        );
        assert_eq!(
            Platforms(&list).select("arm/v6"),
            Some("sha256:v6".to_string())
        );
        // a bare arch (no variant) matches arch+os and takes the first entry.
        assert_eq!(
            Platforms(&list).select("arm"),
            Some("sha256:v6".to_string())
        );
        // an unrequestable variant yields nothing.
        assert_eq!(Platforms(&list).select("arm/v9"), None);
    }
}
