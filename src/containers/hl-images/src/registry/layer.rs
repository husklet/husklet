...
mod tests {
    ...
    fn whiteout_of_directory_removes_whole_subtree() {
        ...
        assert!(
            !root.join("d").exists(),
            "directory subtree should be removed"
        );
    ...
    fn bare_wh_prefix_marker_does_not_delete_parent() {
        ...
        assert!(
            dir.exists(),
            "parent dir must NOT be wiped by a bare .wh. marker"
        );
        assert!(dir.join("safe").exists(), "sibling must survive");
        assert!(
            !dir.join(".wh.").exists(),
            "the malformed marker is still cleaned up"
        );
    }
    ...
    fn opaque_marker_is_dropped_without_deleting_siblings() {
        ...
        assert!(
            !root.join(".wh..wh..opq").exists(),
            "opaque marker itself is removed"
        );
    }
    ...
    fn tar_pack_argv_has_sparse_flag() {
        ...
        assert!(argv
            .iter()
            .any(|a| a == std::ffi::OsStr::new("/some/rootfs")));
    }
    ...
    fn opaque_dir_escaping_rootfs_is_not_cleared() {
        ...
        assert!(
            ...
            "opaque `..` escape must not delete outside the rootfs"
        );
    ...
    fn make_dirs_writable_recovers_readonly_subtree_and_reset_dir_clears_it() {
        ...
        for d in [&root, &root.join("ro"), &inner] {
            ...
            assert!(
                mode & 0o200 != 0,
                "{} should be owner-writable, mode={:o}",
                d.display(),
                mode
            );
        }
    ...
    fn make_dirs_writable_does_not_follow_symlinks() {
        ...
        assert_eq!(
            mode & 0o200,
            0,
            "a symlinked-to external dir must NOT be made writable"
        );
