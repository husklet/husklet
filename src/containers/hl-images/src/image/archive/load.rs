...
impl Store {
    ...
    fn load_docker_format(&self, staging: &Path) -> Result<LoadedImage, Error> {
        ...
        let entry = mval.as_array().and_then(|a| a.first()).ok_or_else(|| {
            Error::Manifest("docker manifest.json is not a non-empty array".to_string())
        })?;
...
mod tests {
    ...
    fn load_rootfs_only_archive_falls_back() {
        ...
        let loaded = store
            .load_archive(&bytes)
            .expect("load_archive tolerates no manifest");

        assert_eq!(loaded.name, "loaded", "generic fallback name");
        assert_eq!(
            loaded.arch,
            Arch::LinuxAarch64,
            "arch fallback with no sniffable binary"
        );
        assert_eq!(
    ...
    fn load_restores_arch_and_labels_from_manifest_without_elf() {
        ...
        let loaded = Store::new(store_dir.to_str().unwrap())
            .load_archive(&bytes)
            .unwrap();
        assert_eq!(
            loaded.arch,
            Arch::LinuxX86_64,
            "manifest arch restores amd64 (not the arm64 fallback)"
        );
        assert_eq!(
            loaded.labels.get("org.example").map(String::as_str),
            Some("yes"),
            "labels round-trip"
        );
        let _ = std::fs::remove_dir_all(&src);
    ...
    fn load_non_dd_archive_errors_and_leaves_no_leftover() {
        ...
        let err = store
            .load_archive(&bytes)
            .expect_err("non-hl archive must be rejected");
        assert!(
    ...
    fn load_docker_save_archive_flattens_layers_and_whiteouts() {
        ...
        let outer = tar_members(
            &arc,
            &["manifest.json", "config.json", "layer1.tar", "layer2.tar"],
        );
        ...
        let loaded = store
            .load_archive(&outer)
            .expect("docker save archive loads");
        ...
        assert_eq!(
            std::fs::read_to_string(rootfs.join("c.txt")).unwrap(),
            "from-l2\n"
        );
        assert!(
            !rootfs.join("a.txt").exists(),
            "a.txt should be whiteouted away"
        );
        assert!(
            !rootfs.join("keep/inside.txt").exists(),
            "opaque should clear the lower file"
        );
        assert_eq!(
            std::fs::read_to_string(rootfs.join("keep/new.txt")).unwrap(),
            "upper\n"
        );
        assert!(
            !rootfs.join(".wh.a.txt").exists(),
            "whiteout marker must not leak"
        );
        assert!(
            !rootfs.join("keep/.wh..wh..opq").exists(),
            "opaque marker must not leak"
        );
    ...
    fn load_malformed_dd_manifest_errors() {
        ...
        assert!(
            err.to_string().contains("malformed hl-manifest.json"),
            "err: {err}"
        );
    ...
    fn load_unsupported_os_manifest_errors() {
        ...
        let err = store
            .load_archive(&bytes)
            .expect_err("windows os must error");
        assert!(
            err.to_string().contains("unsupported image os"),
            "err: {err}"
        );
    ...
    fn load_same_name_swaps_without_leftovers() {
        ...
        assert!(
            loaded.rootfs.join("new-sentinel").exists(),
            "new content installed"
        );
        assert!(
            !loaded.rootfs.join("old-sentinel").exists(),
    ...
    fn concurrent_loads_into_one_store_do_not_collide() {
        ...
        assert_eq!(
            std::fs::read_to_string(la.rootfs.join("marker")).unwrap(),
            "AAAA\n"
        );
        assert_eq!(
            std::fs::read_to_string(lb.rootfs.join("marker")).unwrap(),
            "BBBB\n"
        );
