...
impl Store {
    ...
    pub fn save_archive(&self, rootfs: &Path, manifest: &Manifest) -> Result<Vec<u8>, Error> {
        ...
        let manifest_json =
            serde_json::to_string(manifest).map_err(|e| Error::Archive(e.to_string()))?;
...
mod tests {
    ...
    fn save_flags_request_xattrs_sparse_and_posix() {
        assert!(
            SAVE_FLAGS.contains(&"--xattrs"),
            "xattrs round-trip (finding 3)"
        );
        assert!(
            SAVE_FLAGS.contains(&"--sparse"),
            "sparse files not expanded (finding 4)"
        );
        assert!(
            SAVE_FLAGS.contains(&"--format=posix"),
            "pax/ns mtimes (finding 13)"
        );
    }
    ...
    fn save_then_load_roundtrips_config_and_files() {
        ...
        assert_eq!(
            loaded.healthcheck,
            Some(serde_json::json!({"Test": ["CMD", "true"]}))
        );
