...
mod tests {
    ...
    fn remove_image_dir_deletes_dir_under_store_root() {
        ...
        assert!(
            !img_dir.exists(),
            "image dir under store root should be removed"
        );
    ...
    fn remove_image_dir_leaves_dir_whose_parent_is_store_root() {
        ...
        assert!(
            rootfs.exists(),
            "rootfs whose parent is the store root must be left untouched"
        );
    ...
    fn dir_for_contains_path_traversal_names() {
        ...
        for name in ["../../evil", "..", ".", "a/../../b", "", "/etc/passwd"] {
            ...
            assert_eq!(
                ...
                1,
                "{name:?} -> {d:?} is not a single component"
            );
            let comp = rel.components().next().unwrap().as_os_str();
            assert!(
                comp != ".." && comp != ".",
                "{name:?} -> {d:?} is a traversal component"
            );
        }
    }
    ...
    fn dir_for_is_injective_for_colliding_refs() {
        ...
        assert_ne!(
            store.dir_for("owner/app:1_2"),
            store.dir_for("owner/app_1:2")
        );
        assert_ne!(store.dir_for("a:b/c"), store.dir_for("a/b:c"));
        // ordinary names remain stable + readable (no separators to escape)
        assert_eq!(
            store.dir_for("busybox_latest"),
            PathBuf::from("/store/busybox_latest")
        );
    }
    ...
    fn install_dir_success_swaps_and_failure_preserves_old() {
        ...
        let err = store
            .install_dir(&missing, &target)
            .expect_err("missing staged must fail");
        assert!(err.to_string().contains("No such file") || !err.to_string().is_empty());
        assert!(
            target.join("rootfs/old-sentinel").exists(),
            "old image destroyed on failed install"
        );
        ...
        assert!(
            asides.is_empty(),
            "aside copy left behind after restore: {asides:?}"
        );
        ...
        store
            .install_dir(&staged, &target)
            .expect("install succeeds");
        assert!(
            target.join("rootfs/new-sentinel").exists(),
            "new content not installed"
        );
        assert!(
            !target.join("rootfs/old-sentinel").exists(),
        ...
        assert!(
            asides2.is_empty(),
            "aside copy not cleaned after success: {asides2:?}"
        );
    ...
    fn remove_image_dir_leaves_dir_outside_store_root() {
        ...
        assert!(
            img_dir.exists(),
            "rootfs outside the store root must be left untouched"
        );
