...
"#;
    ...
    let process = if matches!(mode, Mode::CwdRelative | Mode::LiveNative) {
        ...
        Process::new("/bin/sh").args([
            "-c",
            "printf 'extern int unit_127(int); int main(void){return unit_127(1)==128?0:1;}\\n' >/tmp/main.c && COMPILER_PATH=/usr/libexec/gcc/x86_64-alpine-linux-musl/15.2.0/:/usr/x86_64-alpine-linux-musl/bin/ gcc -O2 /tmp/main.c /work/src/unit_127.c -o /tmp/toolchain && /tmp/toolchain && sha256sum /tmp/toolchain",
        ])
