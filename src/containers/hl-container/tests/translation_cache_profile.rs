...
fn require_durable_cache_receipts(mode: Mode, names: &[Vec<u8>]) -> Result<(), Error> {
    let has = |marker: &[u8]| {
        names
            .iter()
            .any(|name| name.windows(marker.len()).any(|part| part == marker))
    };
    match mode {
        Mode::CacheCold => require(
            has(b".published-"),
            "cold cache arm emitted no identity-bound publication receipt",
        ),
        Mode::CacheValid => {
            for (marker, missing) in [
                (
                    &b".published-"[..],
                    "warm cache arm lost its identity-bound cold publication receipt",
                ),
                (
                    &b".hit-fixed-image-"[..],
                    "warm cache arm emitted no fixed-image HIT receipt",
                ),
                (
                    &b".hit-eager-snapshot-"[..],
                    "warm cache arm emitted no eager-snapshot receipt",
                ),
                (
                    &b".library-activated-"[..],
                    "warm cache arm emitted no library-activation receipt",
                ),
            ] {
                require(has(marker), missing)?;
            }
            Ok(())
        }
        _ => Ok(()),
...
#[test]
fn durable_cache_receipt_requirement_is_non_vacuous() {
    assert!(require_durable_cache_receipts(Mode::CacheCold, &[]).is_err());
    assert!(require_durable_cache_receipts(Mode::CacheCold, &[b"key.published-1".to_vec()]).is_ok());
    let complete = [
        b"key.published-1".to_vec(),
        b"key.hit-fixed-image-1".to_vec(),
        b"key.hit-eager-snapshot-1".to_vec(),
        b"key.library-activated-1".to_vec(),
    ];
    assert!(require_durable_cache_receipts(Mode::CacheValid, &complete).is_ok());
    for removed in 0..complete.len() {
        let mutation = complete
            ...
            .collect::<Vec<_>>();
        assert!(require_durable_cache_receipts(Mode::CacheValid, &mutation).is_err());
    }
}
