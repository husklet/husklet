...
pub(crate) struct Memory {
    ...
    #[cfg(test)]
    fail_container_replace: std::sync::atomic::AtomicU64,
}
...
impl Memory {
    ...
    pub(crate) fn fail_next_container_replace(&self) {
        self.fail_container_replace
...
impl Containers for Memory {
    ...
    async fn replace(&self, container: &Container) -> Result<()> {
        ...
        if self
            ...
            .is_ok()
        {
            return Err(Error::Corrupt("injected container replace failure".into()));
        }
        let mut values = self.values.write().await;
        if !values.contains_key(&container.id) {
