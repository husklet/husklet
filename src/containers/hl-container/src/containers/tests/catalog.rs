...
#[tokio::test]
async fn devices_reach_runtime_as_typed_launch_requirements() {
    let runtime = Arc::new(FakeRuntime::new(ExitStatus::Code(0)));
    let root = std::env::temp_dir().join(format!("hl-container-device-{}", uuid::Uuid::new_v4()));
    let containers = build_with(
        ...
        root.join("volumes"),
        root.join("runtime"),
        crate::Devices::new().with(Clock),
    )
    .await
    .unwrap();
    containers.create(spec("device-launch")).await.unwrap();

    containers.start("device-launch").await.unwrap();

    assert_eq!(
        runtime.programs.lock().unwrap()[0].env["CLOCK_PROVIDER"],
        "host"
    );
    assert_eq!(
        runtime.extensions.lock().unwrap()[0][0].provider.as_str(),
        "test.clock"
    );
    containers.wait("device-launch").await.unwrap();
}

/// A device names its own guest endpoints, so its value wins over a requested one. Every other
/// requested variable reaches the process unchanged, including one the device does not claim.
#[tokio::test]
async fn device_environment_overrides_only_the_variables_it_owns() {
    let runtime = Arc::new(FakeRuntime::new(ExitStatus::Code(0)));
    let root = std::env::temp_dir().join(format!("hl-container-device-{}", uuid::Uuid::new_v4()));
    let containers = build_with(
        Arc::new(Memory::default()),
    ...
    .await
    .unwrap();
    let mut requested = spec("device-environment");
    requested.process = requested
        .process
        .env("CLOCK_PROVIDER", "guest")
        .env("CLOCK_FORMAT", "iso8601");
    containers.create(requested).await.unwrap();

    containers.start("device-environment").await.unwrap();

    let programs = runtime.programs.lock().unwrap();
    assert_eq!(programs[0].env["CLOCK_PROVIDER"], "host");
    assert_eq!(programs[0].env["CLOCK_FORMAT"], "iso8601");
    assert_eq!(programs[0].env["A"], "B");
    drop(programs);
    containers.wait("device-environment").await.unwrap();
}
