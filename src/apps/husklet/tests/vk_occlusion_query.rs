...
fn real_vulkan_occlusion_query_reflects_drawn_coverage_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-vulkan/tests/fixtures/vk_occlusion_query.c"
        ))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
    ...
    assert!(
        stdout.contains("VK_OCCLUSION_OK"),
        "guest self-checked the occlusion counts"
    );
    assert!(
        exec.submit_count() > 0,
    ...
    assert_eq!(
        q0, 2048,
        "visible LEFT-half draw must count 32*64 = 2048 samples"
    );
    assert_eq!(
        q1, 0,
        "fully-scissored draw must count 0 samples (occluded)"
    );
    ...
    assert!(
        is_green(16, 32),
        "Q0 LEFT-half draw must be GREEN, got {:?}",
        at(16, 32)
    );
    assert!(
        is_green(2, 2),
        "Q0 LEFT-half draw must reach the corner, got {:?}",
        at(2, 2)
    );
    assert!(
        is_black(48, 32),
        "the occluded Q1 draw must leave the RIGHT half the BLACK clear, got {:?}",
        at(48, 32)
    );
    assert!(
        is_black(60, 60),
        "the RIGHT half stays the BLACK clear, got {:?}",
        at(60, 60)
    );
    ...
    assert_eq!(
        green as u64, q0,
        "the GREEN pixel count must equal Q0's occlusion sample count"
    );
