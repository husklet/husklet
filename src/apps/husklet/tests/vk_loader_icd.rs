...
//!
//! We `gcc` `../../surface/hl-vulkan/tests/fixtures/vk_compute.c` against the REAL Khronos Vulkan-Headers and link the REAL loader
...
fn real_vulkan_loader_enumerates_our_icd_and_drives_it() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-vulkan/tests/fixtures/vk_compute.c"
        ))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
    ...
    assert!(
        stdout.contains("PHYSICAL_DEVICE_COUNT: 1"),
        "our ICD reported exactly one device"
    );
    assert!(
        stdout.contains("VK_DRIVE_OK"),
        "the program drove create/pipeline/dispatch/submit"
    );
