...
//! hl stack and capture its rendered window pixels off our compositor.
//!
...
fn weston_simple_egl_composites_through_the_full_stack() {
    ...
    let app_bin = match which_weston() {
        ...
        None => {
            eprintln!(
                "weston-simple-egl not found on PATH — skipping the milestone (install `weston`)."
            );
            return;
        }
    };
    ...
    let runtime_dir =
        std::env::temp_dir().join(format!("hl-wip-weston-xdg-{}", std::process::id()));
