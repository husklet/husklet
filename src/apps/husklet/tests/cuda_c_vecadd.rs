...
//!
//! At test time we `gcc` `../../surface/hl-cuda/tests/fixtures/cuda_vecadd.c` against a faithful minimal `cuda.h` and link `-lcuda` from
...
fn real_c_cuda_program_computes_vecadd_through_libcuda() {
    ...
    assert!(
        ...
        "staged libcuda missing at {cuda_dir:?} — build hl-cuda's shim first"
    );

    let manifest = env!("CARGO_MANIFEST_DIR");
    let csrc = format!("{manifest}/../../surface/hl-cuda/tests/fixtures/cuda_vecadd.c");
    ...
    let compile = Command::new("gcc")
        .arg(&csrc)
        .arg("-I")
        .arg(format!("{manifest}/../../surface/hl-cuda/tests/fixtures"))
        .arg("-L")
        .arg(&cuda_dir)
        .arg("-lcuda")
        .arg("-o")
    ...
    assert!(
        stdout.contains("RESULT: 11.0 22.0 33.0 44.0"),
        "C program read back a + b over the socket"
    );
    assert!(
        stdout.contains("VECADD_OK"),
        "C program asserted the sum itself"
    );
