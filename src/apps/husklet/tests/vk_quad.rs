...
//! positions + a per-vertex `@location(0)` color the rasterizer interpolates. `../../surface/hl-vulkan/tests/fixtures/vk_quad.c` is compiled
...
//! `vkCmdBindIndexBuffer`/`vkCmdDrawIndexed` correctly (`hl-vulkan/src/service/record.rs`), but the wgpu
//! executor's render-pass replay only issues `Enc::Draw` — `Enc::DrawIndexed` / `Enc::SetIndexBuffer` /
//! `Enc::SetVertexBuffer` fall through unhandled in `hl-gpu-wgpu/src/submit.rs::run_render_pass`, so an
//! indexed draw would rasterize NOTHING. (Vertex-buffer layouts are also rejected outright at
//! `hl-gpu-wgpu/src/pipeline.rs::create_render_pipeline`.) See the mission report for the precise gap.
...
fn real_vulkan_colored_quad_interpolates_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-vulkan/tests/fixtures/vk_quad.c"
        ))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
    ...
    assert!(
        dominant_red(tl),
        "top-left corner should be ~RED (its vertex color), got {tl:?}"
    );
    assert!(
        dominant_green(tr),
        "top-right corner should be ~GREEN, got {tr:?}"
    );
    assert!(
        dominant_blue(bl),
        "bottom-left corner should be ~BLUE, got {bl:?}"
    );
    assert!(
        dominant_yellow(br),
        "bottom-right corner should be ~YELLOW, got {br:?}"
    );
    ...
    assert_eq!(
        black, 0,
        "the quad must fully cover the framebuffer — found {black} clear (black) pixels"
    );
