...
fn real_gles3_instanced_grid_places_each_instance_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-gl/tests/fixtures/gl_instanced.c"
        ))
    ...
    assert!(
        stdout.contains("GL_INSTANCED_OK"),
        "guest asserted the 4-instance grid over the socket"
    );
    assert!(
        exec.submit_count() > 0,
    ...
    write_png(
        &demo_png_dir().join("gl_instanced.png"),
        W,
        H,
        &bgra_to_rgba(px),
    );
