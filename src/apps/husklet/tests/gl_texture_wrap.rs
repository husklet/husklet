...
fn real_gles2_texture_wrap_repeat_vs_clamp_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-gl/tests/fixtures/gl_texture_wrap.c"
        ))
    ...
    assert!(
        stdout.contains("GL_TEXWRAP_OK"),
        "guest asserted REPEAT-vs-CLAMP taps over the socket"
    );
    assert!(
        exec.submit_count() > 0,
    ...
    write_png(
        &demo_png_dir().join("gl_texture_wrap.png"),
        W,
        H,
        &bgra_to_rgba(px),
    );
    ...
    assert!(
        red(20, 32),
        "REPEAT u≈1.28 (PAST 1.0) must WRAP back to RED"
    );
    ...
    assert!(
        green(52, 32),
        "CLAMP u≈1.28 (PAST 1.0) must HOLD the GREEN edge (no wrap)"
    );
    ...
    assert!(
        red(20, 32) && green(52, 32),
        "the same past-1.0 UV must differ: REPEAT->RED, CLAMP->GREEN"
    );
