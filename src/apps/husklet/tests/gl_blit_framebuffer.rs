...
//!
//! Exercises the hl-gl blit path (record.rs blit_framebuffer records a BlitOp; frame.rs
...
fn real_gles_blit_framebuffer_subrect_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-gl/tests/fixtures/gl_blit_framebuffer.c"
        ))
    ...
    assert!(
        stdout.contains("GL_BLIT_OK"),
        "guest asserted the copied sub-rect + untouched border over the socket"
    );
    assert!(
        exec.submit_count() > 0,
    ...
    assert!(
        red(at(44, 44)),
        "inside the blitted square (far corner) must be RED"
    );
