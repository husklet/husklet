...
//!
//! Exercises the hl-gl MRT path (framebuffer multi-attachment + build_mrt_geometry_frame + multi-`out`
...
fn real_gles3_mrt_two_attachments_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-gl/tests/fixtures/gl_mrt.c"
        ))
    ...
    assert!(
        stdout.contains("GL_MRT_OK"),
        "guest asserted the two attachments' distinct values over the socket"
    );
    assert!(
        exec.submit_count() > 0,
    ...
    let is_solid = |px: &[u8], r: u8, g: u8, b: u8| {
        px.chunks_exact(4)
            .all(|t| near(t[0], r) && near(t[1], g) && near(t[2], b)) // RGBA
    };
    ...
    assert_eq!(
        n_targets, 2,
        "MRT must materialize exactly two 64x64 attachment textures, found {n_targets}"
    );
    assert!(
        red_seen,
        "attachment 0 must be solid RED (its own fragment output o0)"
    );
    assert!(
        green_seen,
        "attachment 1 must be solid GREEN (its own fragment output o1)"
    );
