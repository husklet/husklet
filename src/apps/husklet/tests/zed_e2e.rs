...
//! the Wayland build) end to end through the ENTIRE hl stack and capture its rendered window off our
//! compositor.
//!
...
fn zed_composites_through_the_full_stack() {
        ...
        None => {
            eprintln!(
                "zed-editor (the real Zed Linux Wayland/Vulkan build) not found — skipping the milestone. \
                 Stage the Linux aarch64 tarball under hl/target/zed-dl (zed.app/libexec/zed-editor) or \
                 install Zed."
            );
            return;
        }
    ...
    if !Path::new(wl_client).exists() {
        eprintln!(
            "SKIP: libwayland-client {wl_client} not present — Zed cannot open a Wayland display."
        );
        return;
    }
    ...
    std::fs::write(
        project_dir.join("hello.txt"),
        "hello from the hl_wip zed milestone\n",
    )
    .ok();
    ...
    let our_adapter_selected =
        combined.contains("Selected GPU adapter: \"hl Metal (Vulkan)") && !our_adapter_rejected;
    ...
    if !through_our_stack || frames.is_empty() {
        ...
        // owning crate (host executor `hl-gpu-wgpu`, outside this test's write scope):
        //
        ...
        //   stack.
        //   ROOT CAUSE (host executor, `hl-gpu-wgpu`): wgpu's indirect-validation shader is a plain
        ...
        let fell_back_to_non_hl =
            our_adapter_rejected || combined.contains("Selected GPU adapter: \"llvmpipe");
        eprintln!(
    ...
    if spread > 40 {
        ...
        assert!(
            png.exists(),
            "a real PNG of the composited Zed frame was written at {png:?}"
        );
        eprintln!(
        ...
        // so it's the swapchain-image readback / present path (hl-vulkan present.rs + wayland_app.rs),
        ...
        eprintln!(
            "MILESTONE (through-our-stack) REACHED — Zed renders on our Vulkan stack (adapter={adapter}, \
             ...
             (hl-vulkan present.rs / adapter/wayland_app.rs), NOT the render path (0 rejected ops).",
            frames.len(),
        );
    }
...
fn classify_stop(log: &str, submits: u64) -> String {
    ...
    {
        return "LAYER = HOST EXECUTOR compute path (hl-gpu-wgpu): wgpu accepts our device, then builds \
    ...
    if l.contains("max_buffer_size") && l.contains("allowed 0") {
        return "LAYER = VULKAN ICD device LIMITS (hl-vulkan): wgpu discovered our adapter \
                ...
                vkGetPhysicalDeviceProperties2 (hl-vulkan/shim/vulkan/src/instance.rs) does NOT fill \
    ...
    if l.contains("wayland")
        && (l.contains("global") || l.contains("no such") || l.contains("protocol"))
        || l.contains("wl_") && l.contains("not") && l.contains("support")
        || l.contains("xdg_wm_base")
        || l.contains("wp_viewporter")
        || l.contains("wl_compositor")
    {
        return "LAYER = COMPOSITOR / wl-protocol (hl-compositor): Zed's GPUI Wayland backend needs a \
                global our compositor does not advertise, or hit a wl protocol error. Fix belongs in \
                hl-compositor/src/adapter/smithay/state.rs (advertise the missing global)."
            .to_string();
    }
    ...
    {
        return format!(
            "LAYER = VULKAN ICD (hl-vulkan): blade-graphics rejected our physical device — a required \
             Vulkan feature/extension is not advertised. Our device advertises ONLY VK_KHR_swapchain + \
             VK_KHR_dynamic_rendering (hl-vulkan/src/model/capability.rs::DEVICE_EXTENSIONS) and a \
             ...
             indexing, and buffer-device-address. gpu_submits={submits}. Fix belongs in hl-vulkan \
             (advertise + back the feature chain blade gates device creation on)."
        );
    }
    if submits > 0 {
        return "LAYER = PRESENT / readback (hl-vulkan present path): Zed DID submit Vulkan work \
                (gpu_submits>0) but no frame was presented onto its wl_surface. Inspect \
                hl-vulkan/src/service/present.rs (swapchain image readback) + \
                hl-vulkan/src/adapter/wayland_app.rs (wl_shm attach onto the app surface)."
            .to_string();
    }
...
/// Locate the real Zed Linux binary. Prefer the staged tarball under `hl/target/zed-dl`, then PATH
...
fn which_zed() -> Option<PathBuf> {
    ...
    let staged: &[&str] = &[
        ...
        "../hl/target/zed-dl/zed.app/libexec/zed-editor",
    ];
