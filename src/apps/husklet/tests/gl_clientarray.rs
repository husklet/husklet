...
/// Compile the surface-owned `tests/fixtures/<name>.c` against the staged GL shim, run it on the
/// software-Vulkan WgpuExecutor, and
...
fn run_client_array_program(name: &str, ok_marker: &str) {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-gl/tests/fixtures/{name}.c"
        ))
