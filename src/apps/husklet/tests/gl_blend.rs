...
//! `hl-gpu-wgpu` builds every color target with `blend: None` (pipeline.rs), so a pipeline blend state
...
fn real_gles2_src_over_blend_composites_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-gl/tests/fixtures/gl_blend.c"
        ))
    ...
    assert!(
        stdout.contains("GL_BLEND_OK"),
        "guest asserted the src-over composite over the socket"
    );
    assert!(
        exec.submit_count() > 0,
    ...
    write_png(
        &demo_png_dir().join("gl_blend.png"),
        W,
        H,
        &bgra_to_rgba(px),
    );
    ...
    let blended = px
        ...
        .count();
    assert!(
        blended > (W * H / 8) as usize,
        "the blended color must cover the overlap, only {blended} matched"
    );
