...
//!
//! This is NOT the in-process lowering (`hl-cuda/tests/e2e.rs` covers that). Here the "app" `dlopen()`s
//! the real, separately-compiled guest shared object that `hl-cuda`'s build.rs staged at
...
unsafe fn sym<T>(handle: *mut c_void, name: &str) -> T {
    ...
    assert!(
        !p.is_null(),
        "dlsym({name}) returned null — staged libcuda.so.1 is missing the symbol"
    );
...
fn real_libcuda_shim_runs_vecadd_over_the_socket() {
    ...
    assert!(
        ...
        "staged shim missing at {so_path} — build it first: \
         cargo build --manifest-path hl-cuda/shim/cuda/Cargo.toml"
    );
    ...
    unsafe {
        ...
        assert_eq!(
            (cu.cu_ctx_create)(&mut ctx, 0, dev),
            CUDA_SUCCESS,
            "cuCtxCreate"
        );
        ...
        let mut dc: u64 = 0;
        assert_eq!(
            (cu.cu_mem_alloc)(&mut da, bytes),
            CUDA_SUCCESS,
            "cuMemAlloc a"
        );
        assert_eq!(
            (cu.cu_mem_alloc)(&mut db, bytes),
            CUDA_SUCCESS,
            "cuMemAlloc b"
        );
        assert_eq!(
            (cu.cu_mem_alloc)(&mut dc, bytes),
            CUDA_SUCCESS,
            "cuMemAlloc c"
        );
        ...
        assert_eq!(
            ...
            "vecadd result read back over the socket"
        );
