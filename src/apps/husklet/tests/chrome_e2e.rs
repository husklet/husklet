...
fn chrome_first_light_through_the_full_stack() {
    ...
    match &preload {
        ...
        None => eprintln!(
            "no fd-ownership preload (HL_CHROME_PRELOAD unset and tests/fixtures/chrome_fdguard.c did not compile) — \
    ...
    let best = frames
        .iter()
        .max_by_key(|f| (orange_fraction(f) * 1e6) as u64);
    ...
    if !rendered {
        let stage = if fd_ownership_abort {
            "GAP #0 / NEUTRALIZER DID NOT APPLY — Chromium still aborts in early ChromeMain with a 'FD \
             ...
             PINNED and normally NEUTRALIZED by the LD_PRELOAD shim `tests/fixtures/chrome_fdguard.c`: Chromium's own \
            ...
            "GAP #0c / POST-NEUTRALIZE downstream CHECK — GAP #0 (fd-ownership) AND GAP #0b \
             ...
             tests/fixtures/chrome_fdguard.c by rerouting those two jump-table slots to the benign skip/continue \
    ...
    assert!(
        png.exists(),
        "a real PNG of the composited Chrome frame was written at {png:?}"
    );
    eprintln!(
...
/// fd-ownership enforcement (see the GAP #0 note above and `tests/fixtures/chrome_fdguard.c`). Without it the real
/// arm64 Chromium aborts in early ChromeMain before any Wayland/EGL/GL is reached. Prefers a prebuilt path
/// in `$HL_CHROME_PRELOAD`; otherwise compiles `tests/fixtures/chrome_fdguard.c` with the system `cc` into `out_dir`.
