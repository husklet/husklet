...
impl Harness {
    fn start(tag: &str) -> Harness {
        let runtime_dir =
            std::env::temp_dir().join(format!("hl-wip-interact-{tag}-{}", std::process::id()));
...
fn chrome_interaction_input_roundtrip() {
    ...
    let base_frame = wait_frame(&mut queue, &mut app, &h.captures, 5, |f| {
        f.width == W && f.pixel(1, 1) == Some(BASE)
    })
    ...
    let t = h.inject(
        "ptr_enter",
        InputCommand::PointerMotion { x: 100.0, y: 90.0 },
    );
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| a.events[s1_from..]
            .iter()
            .any(|r| matches!(r.ev, Ev::PtrEnter { .. }))),
        "pointer enter never delivered"
    );
    let lat_enter = latency_of(&app, s1_from, &t, |e| matches!(e, Ev::PtrEnter { .. }));
    assert!(
        lat_enter < LAT_BUDGET,
        "pointer-enter latency {lat_enter:?} exceeded budget {LAT_BUDGET:?}"
    );

    // A second motion inside the surface is a real wl_pointer.motion.
    let t = h.inject(
        "ptr_motion",
        InputCommand::PointerMotion { x: 120.0, y: 90.0 },
    );
    let before = app.events.len();
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| a.events[before..]
            .iter()
            .any(|r| matches!(r.ev, Ev::PtrMotion { .. }))),
        "pointer motion never delivered"
    );
    let lat_motion = latency_of(&app, before, &t, |e| matches!(e, Ev::PtrMotion { .. }));
    assert!(
        lat_motion < LAT_BUDGET,
        "pointer-motion latency {lat_motion:?} exceeded budget"
    );
    ...
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| {
            a.events[before..]
                .iter()
                .filter(|r| matches!(r.ev, Ev::PtrButton { .. }))
                .count()
                >= 2
        }),
    ...
    assert!(
        lat_button < LAT_BUDGET,
        "pointer-button latency {lat_button:?} exceeded budget"
    );
    ...
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| a.events[before..]
            .iter()
            .any(|r| matches!(r.ev, Ev::PtrAxisV(_)))),
        "smooth scroll axis never delivered"
    );
    let lat_scroll = latency_of(&app, before, &t, |e| matches!(e, Ev::PtrAxisV(_)));
    assert!(
        lat_scroll < LAT_BUDGET,
        "smooth-scroll latency {lat_scroll:?} exceeded budget"
    );
    let smooth_v = app.events[before..]
        .iter()
        .find_map(|r| match r.ev {
            Ev::PtrAxisV(v) => Some(v),
            _ => None,
        })
        .unwrap();
    assert!(
        smooth_v > 0.0,
        "smooth scroll delivered a positive vertical value, got {smooth_v}"
    );
    ...
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| a.events[before..]
            .iter()
            .any(|r| matches!(r.ev, Ev::PtrV120(_)))),
        "discrete scroll value120 never delivered"
    );
    let lat_notch = latency_of(&app, before, &t, |e| matches!(e, Ev::PtrV120(_)));
    assert!(
        lat_notch < LAT_BUDGET,
        "discrete-scroll latency {lat_notch:?} exceeded budget"
    );
    let v120 = app.events[before..]
        .iter()
        .find_map(|r| match r.ev {
            Ev::PtrV120(v) => Some(v),
            _ => None,
    ...
    assert!(
        app.events[before..]
            .iter()
            .any(|r| r.ev == Ev::PtrAxisSource(u32::from(AxisSource::Wheel))),
    ...
    let ptr_order: Vec<&Ev> = app.events[s1_from..]
        ...
        .collect();
    assert!(
        matches!(ptr_order[0], Ev::PtrEnter { .. }),
        "first pointer event is enter"
    );
    assert!(matches!(ptr_order[1], Ev::PtrMotion { .. }), "then motion");
    assert!(
        matches!(ptr_order[2], Ev::PtrButton { pressed: true, .. }),
        "then button press"
    );
    assert!(
        matches!(ptr_order[3], Ev::PtrButton { pressed: false, .. }),
        "then button release"
    );
    assert!(
        matches!(ptr_order[4], Ev::PtrAxisV(_)),
        "then smooth scroll"
    );
    assert!(
        matches!(ptr_order[5], Ev::PtrV120(_)),
        "then discrete notch"
    );

    // ---- serials strictly increase across the serial-bearing pointer events. ----
    let ptr_serials: Vec<u32> = app.events[s1_from..]
        .iter()
    ...
    for w in ptr_serials.windows(2) {
        assert!(
            w[0] < w[1],
            "pointer wire serials strictly increase: {} < {}",
            w[0],
            w[1]
        );
    }
    ...
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| a.events[s2_from..]
            .iter()
            .any(|r| matches!(r.ev, Ev::KbdEnter(_)))),
        "keyboard enter never delivered on focus"
    );
    let lat_kbd_enter = latency_of(&app, s2_from, &t, |e| matches!(e, Ev::KbdEnter(_)));
    assert!(
        lat_kbd_enter < LAT_BUDGET,
        "keyboard-enter latency {lat_kbd_enter:?} exceeded budget"
    );
    pump_until(&mut queue, &mut app, 2, |a| a.last_mods().is_some());
    assert_eq!(
        app.last_mods(),
        Some(0),
        "initial modifiers on focus are 0 (nothing held)"
    );
    ...
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| a.last_mods()
            == Some(MOD_SHIFT)),
        "mods_depressed did not latch to Shift"
    );
    let lat_mod = latency_of(&app, s2_from, &t, |e| {
        matches!(
            e,
            Ev::KbdMods {
                depressed: MOD_SHIFT
    ...
    assert!(
        lat_mod < LAT_BUDGET,
        "modifier-latch latency {lat_mod:?} exceeded budget"
    );
    ...
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| a.events[key_from..]
            .iter()
            .any(|r| matches!(
                &r.ev,
                Ev::KbdKey {
    ...
    let lat_key = latency_of(&app, key_from, &t, |e| {
        matches!(
            e,
            Ev::KbdKey {
                key: KEY_A,
    ...
    assert!(
        lat_key < LAT_BUDGET,
        "key latency {lat_key:?} exceeded budget"
    );
    assert_eq!(
        app.last_mods(),
        Some(MOD_SHIFT),
        "Shift still reported held during the letter key"
    );
    ...
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| a.last_mods() == Some(0)),
    ...
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| a.last_mods() == Some(MOD_CTRL)),
        "mods did not latch to Control"
    );
    let _ = h.inject(
        "ctrl_up",
        InputCommand::Key {
    ...
    assert!(
        pump_until(&mut queue, &mut app, 5, |a| a.events[leave_from..]
            .iter()
            .any(|r| matches!(r.ev, Ev::KbdLeave(_)))),
        "keyboard leave never delivered on focus clear"
    );
    let lat_kbd_leave = latency_of(&app, leave_from, &t, |e| matches!(e, Ev::KbdLeave(_)));
    assert!(
        lat_kbd_leave < LAT_BUDGET,
        "keyboard-leave latency {lat_kbd_leave:?} exceeded budget"
    );
    ...
    while app.configure_count <= cfg_before {
        assert!(
            ...
            "set_maximized produced NO new configure (configure lost)"
        );
        queue
            .blocking_dispatch(&mut app)
            .expect("dispatch resize configure");
    }
    let configure_turnaround = t_cfg.elapsed();
    assert!(
        configure_turnaround < LAT_BUDGET,
        "configure turnaround {configure_turnaround:?} exceeded budget"
    );
    let serial = app
        .pending_configure
        .expect("resize configure carried a serial");
    assert_eq!(
        app.tl_size,
        Some(OUTPUT_LOGICAL),
        "resize configure carries the new (output logical) size"
    );
    ...
    surface.commit();
    let resized = wait_frame(&mut queue, &mut app, &h.captures, 5, |f| {
        f.width == W && f.pixel(W / 2, H / 2) == Some(RESIZE_COL)
    })
    .expect("the committed buffer after ack_configure never presented (lost configure/commit)");
    assert!(
        resized.serial > base_frame.serial,
        "post-configure frame presents after the mapped frame"
    );
    ...
    let _ = h.inject(
        "burst_prime",
        InputCommand::PointerMotion { x: 10.0, y: 90.0 },
    );
    settle(&mut queue, &mut app);
    ...
    for i in 0..BURST {
        ...
        h.input_tx
            .send(InputCommand::PointerMotion { x, y: 90.0 })
            .expect("burst motion");
    }
    assert!(
        pump_until(&mut queue, &mut app, 10, |a| a.motion_xs.len()
            - motions_before
            >= BURST),
        "burst not fully drained: delivered {} of {BURST}",
        app.motion_xs.len() - motions_before
    );
    ...
    assert_eq!(
        delivered, BURST,
        "every one of {BURST} rapid motions was delivered (no drops/coalescing)"
    );
    ...
    for (i, &x) in got_xs.iter().enumerate() {
        assert_eq!(
            x,
            20.0 + i as f64,
            "burst motion #{i} arrived out of order (x={x})"
        );
    }
    ...
    assert!(
        burst_max_lat < LAT_BUDGET,
        "worst-case burst delivery {burst_max_lat:?} exceeded budget {LAT_BUDGET:?}"
    );
...
impl Dispatch<WlKeyboard, ()> for App {
    fn event(
        ...
        match e {
            ...
            wl_keyboard::Event::Modifiers { mods_depressed, .. } => app.push(Ev::KbdMods {
