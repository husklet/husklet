...
//! `gtk-4-examples`) end to end through the ENTIRE hl stack and capture its rendered window off our
...
//!     -> our compositor (`hl_compositor::adapter::smithay::run_auto`, a real Smithay Wayland server on
...
fn gtk4_composites_through_the_full_stack() {
    ...
    // reconstructed by the driver (hl-gl `service::frame::gsk_globals_std140`), so `push.mvp` actually
    ...
    if !has_real_content {
        ...
        //   * libepoxy GL-context classification: FIXED in hl-gl/shim/egl/src/driver.rs
        ...
        //   (GL name, content generation) — hl-gl/src/model/context.rs `sampled_texture_ir` /
        ...
        //   WGSL through the pure-Rust ES→desktop lowering in hl-gpu-wgpu/src/glsl_es.rs plus two
        //   naga-module post-passes in hl-gpu-wgpu/src/wgsl.rs. The six constructs that blocked naga-24,
        ...
        //   "stride 26032 exceeds the limit 48" NACK. hl-gl/src/service/frame.rs now hoists the whole-stride
        ...
        //   not match (3)"). hl-gl now recovers the host `k` per bound sampler
        ...
        //   only ~2 submits with 0 presents. hl-gl now has a program-keyed shader/pipeline residency cache
        ...
        //   hl-gl/src/service/frame.rs `lower_draw` now NEVER skips a declared sampler: it binds the
        ...
        eprintln!(
             ...
             * GskGpu fragment shader compilation: FIXED — the `#version 320 es` GskGpu fragment programs now \
                compile to WGSL (hl-gpu-wgpu glsl_es.rs ES→desktop lowering: __VERSION__ seed + UBO \
             ...
             * GskGpu instanced vertex layout: FIXED — Device::create_render_pipeline now succeeds. GskGpu \
                ...
                the glVertexAttribPointer offset; hl-gl/src/service/frame.rs now hoists that whole-stride \
             ...
             * GskGpu bind-group binding alignment: FIXED — the driver binds each texture/sampler at the HOST \
                ...
                it to 1 entry vs the layout's 3 ('bindings (1) does not match (3)'). hl-gl now recovers the \
             ...
             * GskGpu per-frame shader recompile: FIXED — GskGL reuses each of its ~11 programs across every \
                ...
                failures) and burned the whole frame budget (~2 submits, 0 presents). hl-gl now caches a \
             ...
             * GskGpu bind-group COMPLETENESS: FIXED — Device::create_bind_group now succeeds. A GskGpu \
                ...
                layout NACKed ('bindings (3) does not match (7)'). hl-gl's lower_draw now NEVER skips a \
        ...
        panic!(
            "GTK4 composited {presented} frame(s) through the full stack but did NOT render PROPER content \
             ...
             reconstructed in hl-gl service::frame), so a blank frame is a REGRESSION, not a diagnosed \
    ...
    assert!(
        png.exists(),
        "a real PNG of the composited GTK frame was written at {png:?}"
    );
