...
fn real_gles2_share_group_texture_crosses_contexts_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-gl/tests/fixtures/gl_share_group.c"
        ))
    ...
    assert!(
        stdout.contains("GL_SHARE_OK"),
        "guest asserted B saw A's four texels over the socket"
    );
    assert!(
        exec.submit_count() > 0,
    ...
    write_png(
        &demo_png_dir().join("gl_share_group.png"),
        W,
        H,
        &bgra_to_rgba(px),
    );
    ...
    for &c in &quads {
        assert!(
            c >= 0 && c <= 3,
            "a screen quadrant was not one of A's four texels: {quads:?}"
        );
        assert!(
            !seen[c as usize],
            "A texel class {c} appeared twice — not a clean permutation: {quads:?}"
        );
        seen[c as usize] = true;
    }
    assert!(
        seen.iter().all(|&s| s),
        "all four of A's texels (RED,GREEN,BLUE,WHITE) must cross into B: {quads:?}"
    );
