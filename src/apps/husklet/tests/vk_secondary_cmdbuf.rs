...
fn real_vulkan_secondary_cmdbuf_executes_into_primary_pass_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-vulkan/tests/fixtures/vk_secondary_cmdbuf.c"
        ))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
    ...
    assert!(
        stdout.contains("VK_SECONDARY_OK"),
        "guest recorded + executed the secondary command buffer"
    );
    assert!(
        exec.submit_count() > 0,
    ...
    assert!(
        is(16, 32, 255, 0, 0),
        "secondary draw #1 LEFT region must be RED, got {:?}",
        at(16, 32)
    );
    assert!(
        is(48, 32, 0, 0, 255),
        "secondary draw #2 RIGHT region must be BLUE, got {:?}",
        at(48, 32)
    );
    assert!(
        is(32, 32, 0, 0, 0),
    ...
    assert!(
        red > 300 && blue > 300,
        "both secondary-recorded draws must cover real area (red={red}, blue={blue})"
    );
