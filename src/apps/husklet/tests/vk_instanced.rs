...
fn real_vulkan_instanced_grid_places_each_instance_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-vulkan/tests/fixtures/vk_instanced.c"
        ))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
    ...
    assert!(
        is(8, 32, 255, 0, 0),
        "instance 0 cell must be RED, got {:?}",
        at(8, 32)
    );
    assert!(
        is(24, 32, 0, 255, 0),
        "instance 1 cell must be GREEN, got {:?}",
        at(24, 32)
    );
    assert!(
        is(40, 32, 0, 0, 255),
        "instance 2 cell must be BLUE, got {:?}",
        at(40, 32)
    );
    assert!(
        is(56, 32, 255, 255, 0),
        "instance 3 cell must be YELLOW, got {:?}",
        at(56, 32)
    );
    ...
    assert!(
        ...
        "gap between instances 0/1 must be BLACK, got {:?}",
        at(16, 32)
    );
