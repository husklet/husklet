...
use hl_gpu::protocol::model::descriptor::{
    ...
    ColorTargetState, DepthAttachment, DepthState, RenderPipelineDesc, ShaderRef, StencilFaceState,
    TextureDesc,
};
...
// GL/protocol blend-factor + op wire numbering (matches hl-gl `blend_factor_wire`/`blend_op_wire`,
...
fn depth_stencil_blend_mrt_combine_in_one_frame() {
    ...
    hl_gpu::runtime::submit(
        ...
        &[
            ...
            Cmd::WriteBuffer {
                id: 1,
                offset: 0,
                data: uniform(BG_A, 1.0, BG_B, 1.0, 0.5),
            }, // BG (opaque, mid)
            Cmd::CreateBuffer(
                2,
                BufferDesc {
            ...
            Cmd::WriteBuffer {
                id: 2,
                offset: 0,
                data: uniform(FG_A, 0.5, FG_B, 0.5, 0.3),
            }, // FG (translucent, near)
            Cmd::CreateBuffer(
                3,
                BufferDesc {
            ...
            Cmd::WriteBuffer {
                id: 3,
                offset: 0,
                data: uniform([255, 0, 0], 0.5, [255, 0, 0], 0.5, 0.9),
            }, // FAR red (occluded)
            Cmd::CreateShader {
                id: 1,
    ...
    let a0 = exec
        .read_texture(&s.resources, 1)
        .expect("read attachment 0");
    let a1 = exec
        .read_texture(&s.resources, 2)
        .expect("read attachment 1");
    write_png(
        &demo_png_dir().join("feature_combo_attach0.png"),
        N * 8,
        N * 8,
        &upscale(&a0, 8),
    );
    write_png(
        &demo_png_dir().join("feature_combo_attach1.png"),
        N * 8,
        N * 8,
        &upscale(&a1, 8),
    );
    ...
    assert_ne!(
        comp0, comp1,
        "the two attachments must composite to DIFFERENT colors for MRT to prove anything"
    );
    // Blend actually composited (not an opaque overwrite of FG, not a no-op leaving BG).
    assert!(
        comp0 != [FG_A[0], FG_A[1], FG_A[2], comp0[3]]
            && comp0 != [BG_A[0], BG_A[1], BG_A[2], comp0[3]],
    ...
    for y in 0..N {
        for x in 0..N {
            ...
            assert!(
                near(p0, want0),
                "attach0 ({x},{y}) inside_rect={}: got {p0:?} want {want0:?}",
                inside_rect(x, y)
            );
            assert!(
                near(p1, want1),
                "attach1 ({x},{y}) inside_rect={}: got {p1:?} want {want1:?}",
                inside_rect(x, y)
            );
            if near(p0, comp0) {
                comp0_px += 1;
            ...
            if p0[0] > 150 && p0[1] < 100 && p0[2] < 100 {
                red_px += 1;
            }
            if p1[0] > 150 && p1[1] < 100 && p1[2] < 100 {
                red_px += 1;
            }
        }
    }
    ...
    assert_eq!(
        comp0_px, 256,
        "attachment 0: exactly the 16x16 marked rect must composite (stencil gate)"
    );
    assert_eq!(
        comp1_px, 256,
        "attachment 1: exactly the 16x16 marked rect must composite (stencil gate)"
    );
    // The far red draw was occluded by the depth test — it must not survive anywhere.
    assert_eq!(
        red_px, 0,
        "the z=0.9 red draw must be occluded by the depth test (near FG at z=0.3 wins)"
    );
