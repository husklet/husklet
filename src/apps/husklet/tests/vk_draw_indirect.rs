...
fn real_vulkan_draw_indirect_sources_params_from_buffer_on_lavapipe() {
    ...
    let out_dir =
        std::env::temp_dir().join(format!("hl-demo-vkdrawindirect-{}", std::process::id()));
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-vulkan/tests/fixtures/vk_draw_indirect.c"
        ))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
    ...
    assert!(
        stdout.contains("VK_DRAW_INDIRECT_OK"),
        "guest drove the indirect draw"
    );
    assert!(
        exec.submit_count() > 0,
    ...
    assert!(
        is(16, 32, 255, 0, 0),
        "indirect cmd0 (firstVertex 0) LEFT region must be RED, got {:?}",
        at(16, 32)
    );
    assert!(
        is(48, 32, 0, 0, 255),
        "indirect cmd1 (firstVertex 6) RIGHT region must be BLUE, got {:?}",
        at(48, 32)
    );
    assert!(
        is(32, 32, 0, 0, 0),
    ...
    assert!(
        red > 300 && blue > 300,
        "both indirect draws must cover real area (red={red}, blue={blue})"
    );
