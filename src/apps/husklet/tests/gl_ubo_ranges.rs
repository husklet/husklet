...
//! `hl-gl`'s multi-block UBO routing (`src/service/record.rs::assemble_multi_block_ubo_bytes` +
...
fn real_gles3_bind_buffer_range_routes_two_blocks_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-gl/tests/fixtures/gl_ubo_ranges.c"
        ))
    ...
    write_png(
        &demo_png_dir().join("gl_ubo_ranges.png"),
        W,
        H,
        &bgra_to_rgba(px),
    );
    ...
    assert_eq!(
        hit,
        (W * H) as usize,
        "the whole frame must be the combined color, got {hit}/{}",
        W * H
    );
