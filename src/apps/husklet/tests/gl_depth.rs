...
fn real_gles2_depth_test_occludes_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-gl/tests/fixtures/gl_depth.c"
        ))
    ...
    assert!(
        stdout.contains("GL_DEPTH_OK"),
        "guest asserted the depth occlusion over the socket"
    );
    assert!(
        exec.submit_count() > 0,
    ...
    write_png(
        &demo_png_dir().join("gl_depth.png"),
        W,
        H,
        &bgra_to_rgba(px),
    );
