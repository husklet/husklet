...
fn all_three_drivers_lower_onto_one_shared_runtime_and_cpu_executor() {
    ...
    // =============================================================================================
    {
        ...
        assert_eq!(
            sink.resources().buffers.len(),
            0,
            "cuda: buffer freed from the shared session"
        );
    }
    ...
    // =============================================================================================
    {
        ...
        assert_eq!(
            sink.resources().buffers.len(),
            1,
            "vulkan: buffer is live in the shared session"
        );
        ...
        assert_eq!(
            bytes,
            vec![0u8; SIZE as usize],
            "vulkan: buffer read back off the shared CPU executor"
        );
        ...
        assert_eq!(
            sink.resources().buffers.len(),
            0,
            "vulkan: buffer freed from the shared session"
        );
    }
    ...
    // =============================================================================================
    {
        ...
        assert!(
            swap::swap_buffers(&mut rec_ctx, &mut rec).unwrap(),
            "gl: a frame was presented"
        );
        ...
        sink.submit(&f.cmds)
            .expect("gl: frame accepted + executed by the shared runtime");
        ctx.reset_frame();
        ...
        assert_eq!(
            center,
            [255, 0, 0, 255],
            "gl: target cleared to blue on the shared CPU executor (BGRA)"
        );
        assert!(
    ...
    assert_eq!(
        sink.resources().buffers.len(),
        0,
        "no buffers leaked across the three drivers"
    );
    assert!(
        sink.resources().surfaces.len() >= 1,
        "the GL frame's surface is resident on the shared runtime"
    );
    assert!(
        sink.resources().textures.len() >= 1,
        "the GL frame's render target is resident on the shared runtime"
    );
}
