...
fn run_variant(bin: &std::path::Path, gl_dir: &std::path::Path, disable: bool) -> Vec<u8> {
    let exec = WgpuExecutorServer::start(if disable {
        "glstencil-off"
    } else {
        "glstencil-on"
    });
    ...
    assert!(
        stdout.contains("GL_STENCIL_OK"),
        "guest asserted its stencil result over the socket"
    );
    assert!(
        exec.submit_count() > 0,
...
fn real_gles2_stencil_test_gates_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-gl/tests/fixtures/gl_stencil.c"
        ))
    ...
    let is_blue =
        |t: (u8, u8, u8, u8)| near(t.0, 255) && near(t.1, 0) && near(t.2, 0) && t.3 == 255;

    write_png(
        &demo_png_dir().join("gl_stencil.png"),
        W,
        H,
        &bgra_to_rgba(&px),
    );
    ...
    assert!(
        is_red(at(&px, 32, 32)),
        "center must be the gated RED quad, got {:?}",
        at(&px, 32, 32)
    );
    assert!(
        is_red(at(&px, 20, 20)),
        "inside-rect must be RED, got {:?}",
        at(&px, 20, 20)
    );
    assert!(
        is_red(at(&px, 44, 44)),
    ...
    assert!(
        is_blue(at(&px, 4, 4)),
        "corner must stay BLUE (gated out), got {:?}",
        at(&px, 4, 4)
    );
    assert!(
        is_blue(at(&px, 60, 4)),
    ...
    assert!(
        is_blue(at(&px, 32, 4)),
        "top-edge must stay BLUE (gated out), got {:?}",
        at(&px, 32, 4)
    );
    ...
    let red = px
        .chunks_exact(4)
        .filter(|t| is_red((t[0], t[1], t[2], t[3])))
        .count();
    let blue = px
        .chunks_exact(4)
        .filter(|t| is_blue((t[0], t[1], t[2], t[3])))
        .count();
    ...
    assert!(
        blue > total / 2,
        "BLUE border must dominate the frame: {blue}/{total}"
    );
    ...
    assert!(
        blue_off < total / 16,
        "disabled run must not leave a BLUE border: {blue_off}/{total}"
    );
