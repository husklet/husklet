...
fn real_vulkan_scissor_clips_fullscreen_draw_on_lavapipe() {
    ...
    let compile = Command::new("gcc")
        .arg(format!(
            "{manifest}/../../surface/hl-vulkan/tests/fixtures/vk_scissor.c"
        ))
        .args(["-I", VK_HEADERS])
        .arg("-ldl")
        .arg("-o")
    ...
    assert!(
        is_green(32, 32),
        "scissor interior must be GREEN, got {:?}",
        at(32, 32)
    );
    assert!(
        is_green(17, 17),
        "scissor top-left interior must be GREEN, got {:?}",
        at(17, 17)
    );
    assert!(
        is_green(46, 46),
        "scissor bottom-right interior must be GREEN, got {:?}",
        at(46, 46)
    );
    assert!(
        is_blue(8, 8),
        "outside (top-left) must be the BLUE clear, got {:?}",
        at(8, 8)
    );
    assert!(
        is_blue(56, 56),
        "outside (bottom-right) must be the BLUE clear, got {:?}",
        at(56, 56)
    );
    assert!(
        is_blue(8, 56),
        "outside (bottom-left) must be the BLUE clear, got {:?}",
        at(8, 56)
    );
    assert!(
        is_blue(56, 8),
        "outside (top-right) must be the BLUE clear, got {:?}",
        at(56, 8)
    );
    assert!(
        is_blue(32, 8),
        "above the scissor must be the BLUE clear, got {:?}",
        at(32, 8)
    );
    ...
    let green = px
        ...
        .count();
    assert!(
        (820..1220).contains(&green),
        "scissor-clipped GREEN must be ~1024 px, got {green}"
    );
