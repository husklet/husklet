...
pub enum WorkspaceCmd {
    ...
    Create {
        ...
        cuda: Option<String>,
        /// Render this workspace's GUI apps on the Mac (docs/ideas/RENDERING_PLAN.md): hl starts the
        /// compositor, mounts its Wayland socket into the guest, and sets `WAYLAND_DISPLAY` and
        /// `XDG_RUNTIME_DIR`, so a Linux GUI app can draw without a custom image.
...
pub fn run(action: WorkspaceCmd) {
    match action {
        ...
        WorkspaceCmd::Create {
            ...
            cuda,
            gui,
        } => create(name, image, arch, vpn, cuda, gui),
...
fn build_workspace(
    ...
    let arch = match arch {
        Some(s) => {
            Arch::parse(s).ok_or_else(|| format!("unknown arch {s:?} (use arm64 | amd64)"))?
        }
    ...
    let vpn_cfg = match vpn {
        ...
        Some(s) => Some(VpnConfig::parse(s).ok_or_else(|| {
            format!("invalid --vpn {s:?} (use e.g. `socks5:127.30.0.1:1080` or a bare `host:port`)")
        })?),
    };
    ...
    let cuda_cfg = match cuda {
        ...
        Some(s) => Some(CudaDevice::parse(s).ok_or_else(|| {
            format!(
                "invalid --cuda {s:?} (use `--cuda`, `--cuda \"Name|8.6|8192\"`, or `--cuda off`)"
            )
        })?),
    };
    ...
    let mut ws = prior
        .cloned()
        .unwrap_or_else(|| WorkspaceConfig::new(name, image, arch));
...
fn create(
    ...
    let vpn_note = ws
        .vpn
        .as_ref()
        .map(|c| format!("  vpn={}", c.to_spec()))
        .unwrap_or_default();
    let cuda_note = ws
        .cuda
        .as_ref()
        .map(|c| format!("  cuda={}", c.to_spec()))
        .unwrap_or_default();
...
fn launch(name: String, restore: bool, cwd: Option<String>, slot: Option<String>) {
    ...
    let launched = match crate::hl_launcher::launch_ex(
        &ws,
        cols,
        rows,
        restore,
...
mod tests {
    ...
    fn recreate_without_arch_keeps_prior_arch() {
        ...
        let got =
            build_workspace(Some(&prior), "api", "node:20", None, None, None, Some("on")).unwrap();
        assert_eq!(
            got.arch,
            Arch::Amd64,
            "omitted --arch must not reset an amd64 workspace to arm64"
        );
        assert!(got.gui, "the flag the user DID pass still applies");
        // Passing --arch explicitly still changes it.
        let got2 = build_workspace(
            Some(&prior),
            "api",
            "node:20",
            Some("arm64"),
    ...
    fn vpn_three_way_semantics() {
        ...
        // off → clear
        let cleared =
            build_workspace(Some(&prior), "w", "img", None, Some("off"), None, None).unwrap();
        assert!(cleared.vpn.is_none());
        // explicit kind → set
        let set = build_workspace(
            None,
            "w",
            "img",
            None,
    ...
    fn cuda_three_way_semantics() {
        ...
        let cleared =
            build_workspace(Some(&prior), "w", "img", None, None, Some("none"), None).unwrap();
        ...
        let spec =
            build_workspace(None, "w", "img", None, None, Some("My GPU|7.5|8192"), None).unwrap();
    ...
    fn gui_three_way_semantics() {
        ...
        prior.gui = true;
        assert!(
            build_workspace(Some(&prior), "w", "img", None, None, None, None)
                .unwrap()
                .gui,
            "absent preserves on"
        );
        assert!(
            !build_workspace(Some(&prior), "w", "img", None, None, None, Some("off"))
                .unwrap()
                .gui,
            "off clears"
        );
        assert!(
            build_workspace(None, "w", "img", None, None, None, Some("on"))
                .unwrap()
                .gui,
            "on sets"
        );
        assert!(
            !build_workspace(None, "w", "img", None, None, None, Some("false"))
                .unwrap()
                .gui,
            "false clears"
        );
    ...
    fn bad_input_is_err_not_panic() {
        assert!(
            build_workspace(None, "w", "img", Some("sparc"), None, None, None).is_err(),
            "bad arch"
        );
        // A `--vpn` that VpnConfig::parse rejects. An empty host:port (`socks5:`) has no endpoint.
        assert!(
            build_workspace(None, "w", "img", None, Some("socks5:"), None, None).is_err(),
            "bad vpn"
        );
    }
}
