...
use hl_gpu::{
    encode_stream, BufferId, Cmd, ConnectionHandler, CpuExecutor, GlobalLedger, GpuExecutor,
...
impl Gpu {
    ...
    pub fn with_cuda(mut self, cuda: CudaDevice) -> Self {
        self.cuda = Some(cuda);
        self
    }
    ...
    fn drivers(&self) -> Drivers {
        ...
        if let Some(cuda) = &self.cuda {
            let bytes = u64::from(cuda.vram_mb).saturating_mul(1024 * 1024);
            let spec = CudaSpec::new(self.arch.cuda(), &self.socket)
                .stage_root(&self.stage_root)
                .advertise(&cuda.name, &cuda.compute_capability, bytes);
            drivers.add(Cuda::new(spec));
        }
...
impl Backend {
    pub fn configured() -> io::Result<Self> {
        match std::env::var("HL_GPU_BACKEND").as_deref() {
            Ok("cpu") => Ok(Self::Cpu),
            Ok("wgpu") | Err(std::env::VarError::NotPresent) => Ok(Self::Wgpu),
    ...
    fn executor(self) -> Result<Box<dyn GpuExecutor>, String> {
        match self {
            Self::Cpu => {
                ...
                executor.set_kernel_compiler(compile_ptx);
                Ok(Box::new(executor))
            }
            Self::Wgpu => {
...
fn compile_ptx(descriptor: &KernelDescriptor) -> hl_gpu::Result<KernelProgram> {
...
impl Connection {
    fn new(executor: Box<dyn GpuExecutor>) -> Self {
...
impl ConnectionHandler for Connection {
    fn submit(&mut self, _header: &SubmitHeader, batch: &[Cmd]) -> Verdict {
        let bytes = encode_stream(batch).len();
        let trace = std::env::var_os("HL_GPU_TRACE").is_some();
        if trace {
            eprintln!(
...
impl Service {
    pub fn start(socket: impl Into<PathBuf>, backend: Backend) -> io::Result<Self> {
                ...
                match listener.accept() {
                    Ok((stream, _)) => {
                        thread::spawn(move || {
                            ...
                            let mut connection = Connection::new(executor);
                            let _ = hl_gpu::serve_connection_with_handler(
                                &stream,
                                &capabilities,
                                &mut connection,
                            );
...
mod tests {
    ...
    fn headless_cuda_does_not_attach_display_drivers() {
        let gpu =
            Gpu::new(GuestArch::Aarch64, "/stage", "/host/hl-gpu.sock").with_cuda(CudaDevice {
                name: "test".into(),
    ...
    fn service_publishes_and_removes_a_ready_endpoint() {
        ...
        let service = Service::start(&path, Backend::Cpu).unwrap();
