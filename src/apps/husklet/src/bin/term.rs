...
impl WsDefaults {
    ...
    fn load() -> WsDefaults {
        ...
        let Ok(text) = std::fs::read_to_string(WsDefaults::path()) else {
            return d;
        };
    ...
    fn save(&self) -> std::io::Result<()> {
        ...
        let mut s =
            String::from("# hl — defaults for newly-created workspaces (edited in Settings)\n\n");
...
fn save_term_config(cfg: &TermConfig) -> std::io::Result<()> {
    ...
    s.push_str(&format!(
        "font_size = {}\n",
        if cfg.font_size.fract().abs() < f64::EPSILON {
            format!("{}", cfg.font_size as i64)
        } else {
            format!("{}", cfg.font_size)
        }
    ));
    ...
    }
    {
        ...
        save.connect_clicked(move |_| match save_settings(&ui) {
            Ok(()) => {
                ...
                status
                    .set_text("Saved — appearance is live now; defaults apply to new workspaces.");
            }
            Err(e) => {
...
fn settings_appearance(ui: &Rc<SettingsUi>) -> gtk::Box {
    ...
    p.append(&field(
        "TERMINAL FONT",
        &ui.font,
        Some("Any installed monospace family, e.g. Menlo, SF Mono, JetBrains Mono."),
    ));
    ...
    p.append(&field(
        "BACKGROUND (#rrggbb)",
        &ui.bg,
        Some("Terminal background. hl ships near-black #1a1d23."),
    ));
...
fn settings_defaults(ui: &Rc<SettingsUi>) -> gtk::Box {
    ...
    p.append(&field(
        "DEFAULT IMAGE",
        &ui.d_image,
        Some("Docker image reference a new workspace defaults to."),
    ));
...
fn settings_device(ui: &Rc<SettingsUi>) -> gtk::Box {
    ...
    p.append(&field(
        "VRAM (MB)",
        &ui.d_cuda_vram,
        Some("Reported device memory in MB (carved from unified memory)."),
    ));
...
fn settings_rendering(ui: &Rc<SettingsUi>) -> gtk::Box {
    ...
    let hint = gtk::Label::new(Some(
        "Only affects workspaces that launch a graphical app. See docs/ideas/RENDERING_PLAN.md.",
    ));
    hint.add_css_class("fhint");
    hint.set_xalign(0.0);
    hint.set_wrap(true);
...
fn open_image_picker(parent: &gtk::Window, form: &Rc<Form>) {
    ...
    let sub = gtk::Label::new(Some(&format!(
        "for {arch} — or Cancel and type a custom image reference"
    )));
...
fn pane_resources(form: &Rc<Form>) -> gtk::Box {
    ...
    p.append(&field(
        "TERMINAL SCROLLBACK (blank = unlimited)",
        &form.scrollback,
        Some("Lines of history each shell retains; blank or 0 = unlimited."),
    ));
    p
}
...
fn open_terminal_window(app: &gtk::Application, ws: &WorkspaceConfig) {
    ...
    // freeze fails has just its own slot discarded (so it reopens fresh); the others are unaffected.
    {
        ...
        window.connect_close_request(move |_| {
            ...
            let mut freezes: Vec<(String, Option<std::process::Child>)> = live
                ...
                .map(|(slot, _pid)| {
                    let child =
                        hlcli_command(&["workspace", "checkpoint", &tw.ws.name, "--slot", slot])
                            .spawn()
                            .ok();
                    (slot.clone(), child)
                })
            ...
            for (slot, child) in &mut freezes {
                ...
                if !ok {
                    eprintln!(
                        "[hl-term] freeze of {:?} slot {slot} failed — discarding that slot",
                        tw.ws.name
                    );
    ...
    if let Some(n) = std::env::var("HL_TERM_TABS")
        .ok()
...
fn wire_search(tw: &Rc<TermWin>) {
    {
        ...
        tw.search
            .entry
            .clone()
            .connect_changed(move |_| search_update(&tw));
    }
...
fn save_session(tw: &Rc<TermWin>) {
    ...
    let entries: Vec<(String, String)> = {
        ...
        es.iter()
            .skip(1)
            .map(|e| (e.name.clone(), tab_title(&e.button)))
            .collect()
    };
...
fn snapshot_node(
    ...
    if let Some(t) = w.downcast_ref::<vte4::Terminal>() {
        let cwd = t
            ...
            .and_then(|u| session::cwd_from_uri(&u));
    ...
    if let Some(paned) = w.downcast_ref::<gtk::Paned>() {
        ...
        let a = paned
            .start_child()
            .and_then(|c| snapshot_node(tw, &c, storage, hist_idx));
        let b = paned
            .end_child()
            .and_then(|c| snapshot_node(tw, &c, storage, hist_idx));
        return match (a, b) {
...
fn split_focused(tw: &Rc<TermWin>, orient: gtk::Orientation) {
    let Some(old) = tw.focused.borrow().clone() else {
        return;
    };
...
fn close_terminal_pane(tw: &Rc<TermWin>, term: &vte4::Terminal) {
    ...
    if let Some(paned) = parent.downcast_ref::<gtk::Paned>() {
        ...
        if let Some(bx) = pparent.downcast_ref::<gtk::Box>() {
            ...
            let paned_is_start =
                pp.start_child().as_ref() == Some(paned.upcast_ref::<gtk::Widget>());
            if paned_is_start {
...
fn make_terminal_ex(
    ...
    let dbgcmd = dbg.as_ref().map(|p| {
        format!(
            "exec '{}' workspace launch '{}' --slot '{}' > '{}' 2>&1",
            hl, tw.ws.name, slot, p
        )
    });
    ...
    let mut launch_args: Vec<&str> = vec![
        hl.as_str(),
        "workspace",
        "launch",
        tw.ws.name.as_str(),
...
fn open_url(url: &str) {
    let full = if url.starts_with("www.") {
        format!("https://{url}")
    } else {
        url.to_string()
    };
...
fn spawn_dashboard_poller(
    ws_name: String,
    shell: String,
    data: std::sync::Arc<std::sync::Mutex<DashData>>,
) {
...
fn query_volumes(sock: &std::path::Path) -> Vec<Vec<String>> {
    ...
    arr(list)
        .iter()
        .map(|vo| vec![s(vo, "Name"), s(vo, "Driver")])
        .collect()
}
...
fn maybe_shot(window: &gtk::ApplicationWindow, tag: &str) {
    let Ok(path) = std::env::var("HL_TERM_SHOT") else {
        return;
    };
    if std::env::var("HL_TERM_VIEW").unwrap_or_else(|_| "manager".into()) != tag {
        return;
    }
    let ms: u64 = std::env::var("HL_TERM_SHOT_MS")
        .ok()
...
fn hlcli_command(args: &[&str]) -> std::process::Command {
    ...
    c.env(
        "PATH",
        "/Users/x/.local/bin:/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin",
    );
    for k in [
        "HOME",
        "USER",
...
fn slot_has_checkpoint(ws: &Workspace, slot: &str) -> bool {
    ws.checkpoint_slot_dir(&hl_root(), slot)
        .join("MANIFEST")
        .exists()
}
...
mod macshim {
    ...
    pub fn force_dark() {
        ...
        unsafe {
            ...
            let Some(dark) = NSAppearance::appearanceNamed(&name) else {
                return;
            };
...
99999     1       00:01 grep workspace launch general";
    ...
    fn finds_launchers_and_their_forks() {
        ...
        assert!(
            !pids.contains(&"43405"),
            "hl-term itself is not a workspace process"
        );
