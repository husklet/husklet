...
#[derive(Subcommand)]
enum Cmd {
    /// Internal native-compositor service owned by a workspace launch.
    #[cfg(target_os = "macos")]
    #[command(name = "__compositor", hide = true)]
    Compositor {
        #[arg(long)]
...
fn main() {
    ...
    let code = match cli.cmd {
        #[cfg(target_os = "macos")]
        Cmd::Compositor { socket } => match hl::compositor::run_native(&socket) {
            Ok(()) => 0,
            Err(error) => {
                eprintln!("hl compositor: {error}");
                1
            }
        },
...
mod tests {
    ...
    fn workspace_create_flag_arities() {
        ...
        .unwrap();
        let Cmd::Workspace {
            action: WorkspaceCmd::Create { cuda, gui, .. },
        } = bare.cmd
        else {
            panic!()
        };
        assert_eq!(
            cuda.as_deref(),
            Some("default"),
            "bare --cuda => default_missing_value"
        );
        assert_eq!(
            gui.as_deref(),
            Some("on"),
            "bare --gui => default_missing_value"
        );
    ...
    fn run_passes_through_trailing_args_and_hyphens() {
        ...
        assert_eq!(
            args,
            vec!["ubuntu", "--platform", "linux/amd64", "echo", "-n", "hi"]
        );
    }
    ...
    fn bare_image_is_external_subcommand_shorthand() {
        ...
        let Cmd::Image(args) = cli.cmd else {
            panic!("expected external-subcommand Image")
        };
