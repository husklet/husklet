...
pub struct Options {
    ...
    candidate_engine_options: Vec<String>,
...
pub fn run(options: Options) -> Result<(), Error> {
    ...
    let null_arm = options.candidate.is_none() && options.candidate_engine_options.is_empty();
    let base = Arm {
        label: "base",
        binary: options.base.clone(),
        options: options.engine_options.clone(),
    };
    let candidate = Arm {
        label: "candidate",
        ...
        options: options
            .engine_options
            .iter()
            .chain(&options.candidate_engine_options)
            .cloned()
            .collect(),
    };
    ...
    for name in &names {
        ...
        if worst.as_ref().is_none_or(|(held, _)| (ratio - 1.0).abs() > (held - 1.0).abs()) {
    ...
    notes.push_str(&null_arm_verdict(null_arm, worst.as_ref()));
...
fn null_arm_verdict(null_arm: bool, worst: Option<&(f64, String)>) -> String {
    let Some((ratio, name)) = worst else {
        return "# null-arm\tNO-PHASES\n".to_owned();
    };
    if !null_arm {
        return format!(
            "# null-arm\tNOT-RUN\tthis run compared two arms; the floor is unknown here.\n\
    ...
    let mut line =
        format!("# null-arm\t{verdict}\tworst={name}\tratio={ratio:.4}\ttolerance={NULL_ARM_TOLERANCE}\n");
    if !resolved {
...
mod tests {
    ...
    fn phase_lines_are_read_and_other_output_ignored() {
        ...
        assert!(parse("PHASE compute ok=1\n").is_empty(), "a phase without us is not a sample");
    }
    ...
    fn every_run_ends_with_a_null_arm_line() {
        ...
        let resolved = null_arm_verdict(true, Some(&worst));
        ...
        let unresolved = null_arm_verdict(true, Some(&drifted));
        ...
        let compared = null_arm_verdict(false, Some(&worst));
    ...
    fn a_null_arm_is_the_same_binary_with_the_same_options() {
        ...
        assert!((1.017_f64 - 1.0).abs() > tolerance, "the reading this driver replaces must fail");
