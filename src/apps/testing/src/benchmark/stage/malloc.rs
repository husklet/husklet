...
pub(super) fn layouts(source: &Path, rootfs: &Path, output: &Path) -> [Layout; 2] {
    [
        layout("plain", false, source, rootfs, output),
        layout("sqlite", true, source, rootfs, output),
    ]
}

fn layout(name: &'static str, sqlite: bool, source: &Path, rootfs: &Path, output: &Path) -> Layout {
