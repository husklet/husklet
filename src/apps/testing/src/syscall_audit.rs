...
impl Audit {
    ...
    fn inventory(&self) -> Result<Vec<Entry>, String> {
        ...
        if entries != checked {
            return Err("syscall-numbers.tsv differs from production C number.c; run testing syscall-audit --regenerate src/runtime/native/retained/src/linux_abi/number.c".into());
        }
