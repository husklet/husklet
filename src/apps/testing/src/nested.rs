...
// tools/process.c forks one child, reports exec failure separately, bounds its
// capture, waits for teardown, and returns the leaf status/stdout. The C gate
...
"#,
    ...
    fn manifest_accepts_sixteen_layers_and_rejects_seventeen() {
        ...
        assert!(load(root.path(), &definition).unwrap_err().to_string().contains("invalid nested chain"));
    }
