...
//! `.oracle()` check holds hl byte-identical to native (aarch64) / qemu (x86_64) on BOTH Linux engines.
...
pub fn groups() -> Vec<Group> {
    vec![group(
        ...
        vec![
            ...
            // connect01/bind01/sendto02: EBADF/ENOTSOCK/EFAULT/EINVAL error paths. hl re-derives the Linux
            ...
            // resolved: hl byte-exact vs native on BOTH arches; connect01/bind01 LTP binaries pass 7/7 each.
            ...
            // error-path surface. hl folded a bad/non-dir dirfd into a host path via g_fdpath (missing EBADF/
