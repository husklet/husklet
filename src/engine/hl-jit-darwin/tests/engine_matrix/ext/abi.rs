...
fn abi_float() -> Group {
    group(
        ...
        vec![
            ...
            s("simd-syscall", "ext_abi/simd_syscall.c"), // vector regs survive syscall block-exits (slim spill)
            s("simd-syscall-crypto", "ext_abi/simd_syscall_crypto.c"), // inline 0F38 shuffle glue marks vdirty (pshufb result survives a slim syscall exit)
        ],
    )
}
