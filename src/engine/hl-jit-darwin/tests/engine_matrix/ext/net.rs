...
fn ext_net() -> Group {
    group(
        ...
        vec![
            ...
            // hl models lo + eth0; with no HL_IP in the bare matrix eth0 is the synthetic 172.17.0.2/16.
            ...
            // `netstat -ln` inside the container list it. hl synthesized only the header -> every listener was
            ...
            // Fixed golden (hl's IPv4-only contract deliberately differs from a raw v6-capable host, matching
            // a real Docker default-bridge container); same syscall path serves both Linux engines.
            src("v6-unreach", "ext_net/net_v6_unreach.c").out("v6_connect enetunreach=1 fast=1\n"),
