...
fn compile_probe(arch: &str) -> String {
    let src = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("testdata/guests/overlay/probe.c");
...
fn build_layers(arch: &str) -> (PathBuf, PathBuf) {
    ...
    // M-series fixtures: a nested lower-only directory (mkdir/rename must copy up its parents),
    ...
    // N-series: a lower-backed directory removed at runtime must HIDE its read-only lower
    ...
    // bottom-up; rmcopy = a dir with a copied-up child + a lower child, emptied then rmdir'd.
...
fn run_probe(arch: &str) -> std::collections::HashMap<String, String> {
    let guest = match arch {
        "x86_64" => hl_jit::Guest::LinuxX86_64,
        _ => hl_jit::Guest::LinuxAarch64,
    };
    ...
    let mut cfg = hl_jit::SpawnConfig::new(String::new(), upper.to_string_lossy().into_owned());
    ...
    assert!(
        ...
        "[{arch}] probe did not finish; stdout=\n{stdout}\nstderr=\n{}",
        String::from_utf8_lossy(&out.stderr)
    );
    stdout
        .lines()
...
fn check(arch: &str) {
    ...
    let g = |k: &str| {
        m.get(k)
            .cloned()
            .unwrap_or_else(|| format!("<missing {k}>"))
    };
    ...
    assert_eq!(
        g("g2_dst_f1"),
        "content-f1",
        "[{arch}] G2 lost /g2dst/f1 (data loss)"
    );
    assert_eq!(
        g("g2_dst_f2"),
        "content-f2",
        "[{arch}] G2 lost /g2dst/f2 (data loss)"
    );
    assert_eq!(
        g("g2_dst_nested"),
        "deep",
        "[{arch}] G2 lost nested subtree (data loss)"
    );
    assert_eq!(
        g("g2_src_gone"),
        "1",
        "[{arch}] G2 source must be gone after rename"
    );
    ...
    assert_eq!(
        g("g3_mode"),
        "4755",
        "[{arch}] G3 copy-up must preserve setuid+mode (got {})",
        g("g3_mode")
    );
    assert_eq!(
        g("g3_mtime"),
        "1000000000",
        "[{arch}] G3 copy-up must preserve mtime (got {})",
        g("g3_mtime")
    );
    ...
    assert_eq!(
        g("g5_get"),
        "hello",
        "[{arch}] G5 getxattr must return what setxattr stored"
    );
    assert_eq!(
        g("g5_list_has_user_a"),
        "1",
        "[{arch}] G5 listxattr must include the set attr"
    );
    assert_eq!(
        g("g5_copyup_get"),
        "v1",
        "[{arch}] G5 setxattr on a lower file must persist after copy-up"
    );
    assert_eq!(
        g("g5_copyup_bytes"),
        "xf-lower",
        "[{arch}] G5 copy-up must preserve the file's bytes"
    );
    ...
    assert_eq!(
        g("g6_dir_visible"),
        "1",
        "[{arch}] G6 recreated dir must be visible (stale .wh. cleared)"
    );
    assert_eq!(
        g("g1r_readdir"),
        "fresh",
        "[{arch}] G1r opaque: readdir must show only new content, not lower stale* (got {})",
        g("g1r_readdir")
    );
    assert_eq!(
        g("g1r_stale_lookup"),
        "1",
        "[{arch}] G1r opaque: lower child must not be looked up through the recreated dir"
    );

    // whiteout/unlink: rmdir of a non-empty lower-backed dir must fail ENOTEMPTY and keep its children.
    assert_eq!(
        g("g4b_rmdir_nonempty"),
        "1",
        "[{arch}] rmdir of a non-empty lower-backed dir must fail ENOTEMPTY"
    );
    assert_eq!(
        g("g4b_child_kept"),
        "kept",
        "[{arch}] rmdir ENOTEMPTY must not have removed the lower child"
    );
    ...
    assert_eq!(
        g("g7_exchange_ret"),
        "0",
        "[{arch}] G7 exchange must succeed"
    );
    assert_eq!(
        g("g7_a"),
        "BBB",
        "[{arch}] G7 exchange: /ex_a must hold the upper's old content"
    );
    assert_eq!(
        g("g7_b"),
        "AAA",
        "[{arch}] G7 exchange: /ex_b must hold the lower's old content"
    );
    assert_eq!(
        g("g7_exchange2_ret"),
        "0",
        "[{arch}] G7 exchange of two lower-only files must succeed"
    );
    assert_eq!(
        g("g7_c"),
        "DDD",
        "[{arch}] G7 exchange (both lower): /ex_c must swap to /ex_d's content"
    );
    assert_eq!(
        g("g7_d"),
        "CCC",
        "[{arch}] G7 exchange (both lower): /ex_d must swap to /ex_c's content"
    );

    // ---- M-series: directory-CREATION ops through the overlay must return the real-overlayfs
    ...
    assert_eq!(
        g("m1_mkdir_new"),
        "0",
        "[{arch}] M1 plain mkdir into the upper must succeed (not EPERM)"
    );
    assert_eq!(
        g("m1_mkdir_again"),
        "17",
        "[{arch}] M1 mkdir of an existing upper name must be EEXIST"
    );
    assert_eq!(
        g("m2_mode"),
        "755",
        "[{arch}] M2 mkdir mode must honor umask 022 (0777 -> 0755)"
    );
    assert_eq!(
        g("m3_mkdirat"),
        "0",
        "[{arch}] M3 mkdirat via an explicit dir-fd must succeed"
    );
    assert_eq!(
        g("m3_visible"),
        "1",
        "[{arch}] M3 mkdirat result must be visible"
    );
    assert_eq!(
        g("m4_nested_lower_parent"),
        "0",
        "[{arch}] M4 mkdir under a lower-only parent chain must succeed (copy-up)"
    );
    assert_eq!(
        g("m4_visible"),
        "1",
        "[{arch}] M4 nested mkdir result must be visible"
    );
    assert_eq!(
        g("m5_enoent"),
        "1",
        "[{arch}] M5 mkdir under a missing parent must be ENOENT (got errno {})",
        g("m5_enoent")
    );
    ...
    assert_eq!(
        g("m7_mkdir_over_wh"),
        "0",
        "[{arch}] M7 mkdir over a whiteout (recreate a removed name) must succeed"
    );
    assert_eq!(
        g("m7_isdir"),
        "1",
        "[{arch}] M7 recreated name must be a directory"
    );
    assert_eq!(
        g("m8_creat"),
        "0",
        "[{arch}] M8 creat into an overlay upper dir must succeed"
    );
    assert_eq!(
        g("m8_symlink"),
        "0",
        "[{arch}] M8 symlink into an overlay upper dir must succeed"
    );
    assert_eq!(
        g("m8_mknod_fifo"),
        "0",
        "[{arch}] M8 mknod(FIFO) into an overlay upper dir must succeed"
    );
    assert_eq!(
        g("m9_rmdir"),
        "0",
        "[{arch}] M9 rmdir of an upper-created empty dir must succeed"
    );
    assert_eq!(
        g("m9_rmdir_gone"),
        "1",
        "[{arch}] M9 rmdir of an already-removed dir must be ENOENT"
    );
    assert_eq!(
        g("m10_rmdir_file"),
        "1",
        "[{arch}] M10 rmdir of a regular file must be ENOTDIR (got errno {})",
        g("m10_rmdir_file")
    );
    assert_eq!(
        g("m10_unlinkat_rmdir"),
        "0",
        "[{arch}] M10 unlinkat AT_REMOVEDIR of an empty dir must succeed"
    );
    assert_eq!(
        g("m11_rename_into_overlay"),
        "0",
        "[{arch}] M11 rename of an upper file into a lower-only (copied-up) dir must succeed"
    );
    assert_eq!(
        g("m11_dst"),
        "REN",
        "[{arch}] M11 renamed file must be readable at its destination"
    );
    assert_eq!(
        g("m12_mkdir_over_lowerdir"),
        "1",
        "[{arch}] M12 mkdir over a lower-only DIR must be EEXIST (got errno {})",
        g("m12_mkdir_over_lowerdir")
    );
    ...
    assert_eq!(
        g("m13_symlink_eexist"),
        "1",
        "[{arch}] M13 symlink over a lower-only name must be EEXIST (got errno {})",
        g("m13_symlink_eexist")
    );
    assert_eq!(
        g("m13_openexcl_eexist"),
        "1",
        "[{arch}] M13 open(O_CREAT|O_EXCL) over a lower-only name must be EEXIST (got errno {})",
        g("m13_openexcl_eexist")
    );
    assert_eq!(
        g("m14_rmdir_lower_empty"),
        "0",
        "[{arch}] M14 rmdir of a lower-only EMPTY dir must succeed (whiteout)"
    );
    assert_eq!(
        g("m14_gone"),
        "1",
        "[{arch}] M14 rmdir'd lower dir must then be gone"
    );
    assert_eq!(
        g("m15_mkdirat_lower_dirfd"),
        "0",
        "[{arch}] M15 mkdirat under a lower-only (copied-up) dir-fd must succeed"
    );
    assert_eq!(
        g("m15_visible"),
        "1",
        "[{arch}] M15 mkdirat-under-lower-dirfd result must be visible"
    );

    // ---- N-series: after a lower-backed directory is removed, its read-only lower children
    ...
    assert_eq!(
        g("n1_parent_gone"),
        "1",
        "[{arch}] N1 removed lower-backed dir must be gone"
    );
    ...
    assert_eq!(
        g("n2_leaf_gone"),
        "1",
        "[{arch}] #239/#269: deep descendant of a removed lower subtree must not stale-resolve"
    );
    assert_eq!(
        g("n2_midc_gone"),
        "1",
        "[{arch}] #239/#269: mid-level dir of a removed lower subtree must not stale-resolve"
    );
    assert_eq!(
        g("n2_top_gone"),
        "1",
        "[{arch}] N2 removed subtree top must be gone"
    );
    assert_eq!(
        g("n3_rmdir"),
        "0",
        "[{arch}] #269: rmdir of an emptied copied-up lower dir must succeed (not ENOTEMPTY)"
    );
    assert_eq!(
        g("n3_child_gone"),
        "1",
        "[{arch}] #269: child of an rmdir'd copied-up lower dir must be ENOENT afterward"
    );
}
