...
fn coverage_static_is_fatal_when_runtime_tree_is_missing() {
    ...
    assert_ne!(
        code, 0,
        "a missing runtime tree must be fatal, got exit {code}"
    );
}
...
fn coverage_report_does_not_greenlight_a_broken_scan() {
    ...
    assert_ne!(
        code, 0,
        "report against a zero-handler tree must be fatal, got exit {code}"
    );
}
...
fn coverage_dynamic_fails_when_required_engines_are_missing() {
    ...
    assert_ne!(
        code, 0,
        "dynamic coverage with no engines must be fatal, got exit {code}"
    );
}
...
fn test_ci_matrix_fails_dark_engine_lanes() {
    ...
    assert!(
        f.iter()
            .any(|m| m.contains("MISSING") && m.contains("x86_64")),
...
fn test_ci_matrix_fails_empty_or_all_skipped() {
    ...
    let cells: Vec<Cell> = Engine::ALL
        ...
        .map(|&e| Cell {
            name: "g/c".into(),
            engine: e,
            status: Status::Skip("no bin".into()),
        })
        .collect();
    let f = gate_failures(&Engine::ALL, &all_available, &cells);
    assert!(
...
fn test_ci_matrix_xfail_and_skip_stay_green() {
    ...
    cells.push(Cell {
        name: "g/x".into(),
        engine: Engine::LinuxAarch64,
        status: Status::Xfail("known".into()),
    });
    cells.push(Cell {
        name: "g/s".into(),
        engine: Engine::LinuxAarch64,
        status: Status::Skip("n/a".into()),
    });
    let f = gate_failures(&Engine::ALL, &all_available, &cells);
    assert!(
        f.is_empty(),
        "xfail/skip alongside passes must stay green, got: {f:?}"
    );
}
...
fn test_every_gui_matrix_probe_is_gated_or_documented() {
    let dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("testdata/guests/gui_matrix");
...
fn perf_matrix_rechecks_timed_invocation_success() {
    ...
    assert_eq!(
        crate::support::run_guarded("true", &[], 5),
        Some(true),
        "a clean run is a success"
    );
    assert_eq!(
...
fn perf_matrix_hang_guard_wraps_timed_jit_runs() {
    ...
    assert_eq!(
        ok,
        Some(false),
        "a hang must be caught by the timeout guard, not timed as success"
    );
    assert!(
        start.elapsed().as_secs() < 5,
        "the hang guard must return promptly, not wait out the sleep"
    );
}
