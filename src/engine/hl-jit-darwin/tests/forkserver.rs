...
fn compile_probe(arch: &str) -> Option<PathBuf> {
    ...
    let src = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("testdata/guests/forkserver/probe.c");
...
fn engine_lane(guest: hl_jit::Guest) {
    if !hl_jit::available(guest) {
        eprintln!(
...
fn forkserver_equivalence_aarch64() {
    engine_lane(hl_jit::Guest::LinuxAarch64);
}

#[test]
fn forkserver_equivalence_x86_64() {
    engine_lane(hl_jit::Guest::LinuxX86_64);
}
