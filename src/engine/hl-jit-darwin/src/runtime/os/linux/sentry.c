...
static void sentry_service_one(struct sentry_ring *R) {
    ...
    int64_t local_ret = 0;
    {
        ...
        if (p) switch (snr) {
            ...
            case 436: { // close_range: operate on THIS process's virtual table, never host fd numbers.
                unsigned first = (unsigned)G_A0(&tmp), last = (unsigned)G_A1(&tmp);
                int flags = (int)G_A2(&tmp);
                handled_local = 1;
                if ((flags & ~(2 | 4)) || first > last) {
                    local_ret = -EINVAL;
                    break;
                }
                // CLOSE_RANGE_UNSHARE is already inherent here: every sentry_proc owns an independent
                // descriptor table. Values beyond the bounded virtual table are simply unopened.
                unsigned end = last < SENTRY_VFD_MAX ? last : SENTRY_VFD_MAX - 1;
                if (first >= SENTRY_VFD_MAX) {
                    local_ret = 0;
                    break;
                }
                for (unsigned v = first; v <= end; v++) {
                    if (p->real[v] < 0) continue;
                    if (flags & 4) {
                        p->cloexec[v] = 1;
                        // Keep the backing descriptor consistent for sentry-side helpers that inspect it.
                        int fdflags = fcntl(p->real[v], F_GETFD);
                        if (fdflags >= 0) (void)fcntl(p->real[v], F_SETFD, fdflags | FD_CLOEXEC);
                    } else {
...
static void syscall_route(struct cpu *c) {
    ...
    case 25: // fcntl(fd, cmd, arg): POSIX (5..7) and OFD (36..38) locks carry a flock* in a2. Always redir a2
    ...
    case 22: // epoll_pwait: ret ready events (SENTRY_EPEV_SZ each, per guest arch) landed at buf[0]
        ...
        break;
    case 25: // fcntl F_GETLK/F_OFD_GETLK: the conflicting lock was written back into the ring flock
        if (((int)G_A1(c) == 5 || (int)G_A1(c) == 36) && ret >= 0 && G_A2(c))
            memcpy((void *)G_A2(c), R->buf, SENTRY_FLOCKSZ);
        break;
