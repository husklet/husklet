...
// Registration-time readiness prime. Linux reports an fd that is ALREADY readable/writable at
// EPOLL_CTL_ADD/MOD time for both edge- and level-triggered interest. A macOS kqueue registration can miss
// that current readiness when the filter is re-added under cross-thread changelist pressure, especially
// when a peer is already blocked in kevent(). Stash one synthetic readiness event whenever the fd polls
// ready at registration; ep_prime_push de-duplicates it by fd/filter. The following epoll_wait consumes the
// prime once. A still-ready level-triggered fd is then naturally reported again by kqueue, while EPOLLET
// waits for its next transition. Tables are indexed by epoll fd (<HL_NFD); larger fds cannot be primed.
...
static uint8_t g_ep_has_mojo[HL_NFD];
...
// ep_mem_set and the sibling [HL_NFD] interest tables; a Chromium renderer registers hundreds of fds,
...
// hl-tracked epoll fds (< HL_NFD, g_epoll set) get this surface -- a dup'd/large epfd keeps the existing
...
static int svc_event(struct cpu *c, uint64_t nr, uint64_t a0, uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4,
    ...
    case 21: {
        ...
        // (2) EBADF: epfd must be an open fd. A hl-tracked epoll (g_epoll set) is known-valid -> only an
        ...
        // (7/8/9) EEXIST (ADD an already-registered fd) / ENOENT (MOD|DEL an absent fd) on a hl-tracked epoll
        ...
        if (w7_on() && fd >= 0 && fd < HL_NFD && (g_sock_fam[fd] == AF_UNIX || g_eventfd_peer[fd])) {
            if (epfd >= 0 && epfd < HL_NFD && op != 2) g_ep_has_mojo[epfd] = 1;
        ...
        // Prime current readiness for ADD/MOD regardless of edge/level mode (see g_ep_prime).
        if (op != 2) {
    ...
    case 22: {
        ...
        for (;;) {
            ...
            if (w7_on() && ep >= 0 && ep < HL_NFD && g_ep_has_mojo[ep] && tp == NULL)
            ...
            ts_wait_leave();
            if (w7_on() && ep >= 0 && ep < HL_NFD && g_ep_has_mojo[ep] && r > 0) {
                char ids[240];
                int p = 0;
    ...
    case 86: {
        ...
        // file). Our timerfds are hl-tracked kqueues (< HL_NFD, g_timerfd set); a larger valid fd is left to
