...
static int svc_io(struct cpu *c, uint64_t nr, uint64_t a0, uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4,
    ...
    case 25: {
        ...
        // F_GETLK/SETLK/SETLKW and their OFD variants: translate the Linux flock and command. The
        // in-engine lock table is also used for OFD locks; unlike passing Linux command 36..38 to
        // macOS fcntl (which returns ENOTTY), this preserves exclusion and blocking semantics.
        if (lcmd == 5 || lcmd == 6 || lcmd == 7 || lcmd == 36 || lcmd == 37 || lcmd == 38) {
            int lockcmd = lcmd >= 36 ? lcmd - 31 : lcmd;
            // macOS F_GETLK=7,SETLK=8,SETLKW=9
            int mc = lockcmd == 5 ? F_GETLK : lockcmd == 6 ? F_SETLK : F_SETLKW;
            ...
            // as a real F_SETLKW returns. poslk_op returns 0 only for non-regular fds -> host path below.
            {
                int pout = 0, claimed;
                for (;;) {
                    claimed = poslk_op((int)a0, lockcmd, (uint8_t *)a2, &pout);
                    if (!claimed) break; // not a regular file -> fall through to the host fcntl path
                    if (lockcmd == 7 && pout == -EAGAIN) {
