...
static int svc_fs(struct cpu *c, uint64_t nr, uint64_t a0, uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4,
    ...
    case 452: {
        ...
        // fchmodat2(fd, "", mode, AT_EMPTY_PATH) operates on the descriptor itself. systemd-sysusers
        // uses this when copying metadata to its atomic /etc/passwd and /etc/group replacement files.
        // Treating every empty path as ENOENT made desktop package installation fail after taking the
        // password lock successfully.
        if (nr == 452 && ((const char *)a1)[0] == '\0' && (a3 & 0x1000)) {
            int r = fchmod((int)a0, (mode_t)a2);
            if (r == 0 && (int)a0 >= 0 && (int)a0 < HL_NFD && g_fdpath[(int)a0][0])
                fc_evict_path(g_fdpath[(int)a0]);
    ...
    case 54: {
        ...
        if (a1 && ((const char *)a1)[0] == '\0' && (a4 & 0x1000)) {
            if (fchown((int)a0, (uid_t)a2, (gid_t)a3) < 0 && errno == EBADF) {
                G_RET(c) = (uint64_t)(int64_t)(-EBADF);
                break;
            }
            chown_xattr_set_fd((int)a0, (int)(int32_t)(uint32_t)a2, (int)(int32_t)(uint32_t)a3);
            if ((int)a0 >= 0 && (int)a0 < HL_NFD && g_fdpath[(int)a0][0]) fc_evict_path(g_fdpath[(int)a0]);
            G_RET(c) = 0;
            break;
        }
        if (jail_ro_at((int)a0, (const char *)a1)) {
