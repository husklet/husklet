...
// ---------------- JIT code cache ----------------
// Large desktop applications touch substantially more code than command-line workloads. Chromium's cold
// start alone can exceed the old 64 MiB translated arena; wholesale eviction then discards its hot startup
// and event-loop blocks and turns launch into continuous retranslation. Keep the arena strictly below
// AArch64's +/-128 MiB B/BL range because translated blocks are directly chained across the arena. A
// larger arena needs branch islands or segmented chaining, not a constant increase.
#define CACHE_SZ (120u << 20)
#if CACHE_SZ >= (128u << 20)
#error "CACHE_SZ must fit the signed AArch64 B/BL displacement; use branch islands before enlarging it"
#endif
// base, bump pointer
...
// exit, ~90 host words ~360B) cap at ~350K live blocks in a full cache, so 2^20 slots keeps the load
// factor under ~35% (short linear-probe chains) and guarantees map_put never silently fails mid-run. A
...
// ============================================================================
// The single-threaded wholesale flush (dispatch.c) reuses the arena in place: it resets the bump
...
// retained VA (no per-flush arena leak) AND removes the old unsafe reuse-in-place-on-alloc-failure path
