...
mod tests {
    ...
    fn frozen_prefix_offsets_never_move() {
        ...
        assert_eq!(
            pool_len as usize,
            wire.len() - std::mem::size_of::<WireHeader>(),
            "pool_len@4"
        );
    ...
    fn wire_pool_offsets_point_at_expected_strings() {
        ...
        let cfg = LaunchConfig {
            ...
            volumes: vec![
                ("/data".into(), "/hostdata".into(), true),
                ("/cfg".into(), "/hcfg".into(), false),
            ],
        ...
        let hdr: WireHeader =
            unsafe { std::ptr::read_unaligned(wire.as_ptr() as *const WireHeader) };
        ...
        .to_wire();
        let nphdr: WireHeader =
            unsafe { std::ptr::read_unaligned(np.as_ptr() as *const WireHeader) };
        assert_eq!(nphdr.nopcache, 1);
        // No VPN configured (the default) -> egress_off is 0 == "" -> the engine keeps direct egress.
        assert_eq!(
            nphdr.egress_off, 0,
            "unset egress must read as the empty string (direct egress)"
        );
