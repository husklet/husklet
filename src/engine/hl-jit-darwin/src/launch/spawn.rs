...
pub fn spawn_io(guest: Guest, cfg: &LaunchConfig, io: SpawnIo) -> std::io::Result<u32> {
    let engine = guest.jit_path().ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no engine built for guest")
    })?;
    let engine_c =
        CString::new(engine).map_err(|_| std::io::Error::from(std::io::ErrorKind::InvalidInput))?;
