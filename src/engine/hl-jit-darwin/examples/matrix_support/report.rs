...
pub fn table(rows: &[Row]) -> String {
    ...
    let _ = writeln!(
        s,
        "\x1b[1m{:<wg$}  {:<wt$}  {:<wa$}  {:>wo$}  {:>wj$}  {:>wr$}  {:<ws$}\x1b[0m",
        "Group",
        "Test",
        "Arch",
        "Oracle(ms)",
    ...
    for r in &rows {
        ...
        let _ = writeln!(
            s,
            "{:<wg$}  {:<wt$}  {:<wa$}  {:>wo$}  {:>wj$}  {:>wr$}  \x1b[{sc}m{:<ws$}\x1b[0m",
            g,
            t,
            r.arch,
            fmt_oracle(r.oracle_ms),
            r.jit_ms,
            fmt_ratio(r.ratio()),
            r.status,
            wg = w_group,
...
pub fn summary(rows: &[Row]) -> String {
    ...
    let _ = writeln!(
        s,
        "  cases timed      : {timed}   (rated: {rated}, jit-only/oracle-less: {jit_only})"
    );
    if rated > 0 {
        ...
        let _ = writeln!(
            s,
            "  ratio× (jit/native): median {med:.2}×   p90 {p90:.2}×   geomean {gm:.2}×"
        );
    } else {
        let _ = writeln!(
            s,
            "  ratio× (jit/native): n/a (no oracle-checked cases in this run)"
        );
    }
    ...
    for r in top.iter().take(15) {
        let _ = writeln!(
            s,
            "    {:>8}  {:<20} {:<12} jit {:>6}ms  oracle {:>6}",
            fmt_ratio(r.ratio()),
            format!("{}/{}", r.group, r.test),
            r.arch,
            r.jit_ms,
            fmt_oracle(r.oracle_ms)
        );
...
pub fn write_machine(
    ...
    for r in &rows {
        ...
        let _ = writeln!(
            ...
            csv_field(&r.test),
            csv_field(&r.arch),
            r.oracle_ms.map(|v| v.to_string()).unwrap_or_default(),
            r.jit_ms,
            ratio,
            r.status
        );
