#!/usr/bin/env bash
# hl-tests/tools/coverage.sh -- find syscalls/opcodes the engines DON'T implement.
#
...
# per syscall family:
#     svc_sysv/svc_mem/svc_signal/svc_time/svc_io/svc_aio/svc_fs/svc_proc/svc_net/svc_event/svc_misc/svc_rare
#     (os/linux/syscall/{sysv,mem,signal,time,io,aio,fs,proc,net,event,misc,rare}.c)
...
#   dynamic  run a corpus (the compiled hl-tests guests + busybox applets) through each engine and
...
#
#   bash hl-tests/tools/coverage.sh [static|dynamic|all|report]   (default: all)
...
RT="${HLCOV_RT:-$CRATE_ROOT/src/runtime}"   # HLCOV_RT overrides for tests/CI probing
...
# NOTE: `aio` is #included into dispatch.c (svc_aio, canonical 0-4: io_setup/io_destroy/io_submit/
# io_cancel/io_getevents) and MUST be scanned or those look unimplemented (#335). Keep this list in sync
# with the family modules dispatch.c fans out to; helpers.c has no `switch (nr)` so it is intentionally
# absent (a module with no nr-switch simply contributes nothing).
...
# Without this a stale/wrong RT path (e.g. the pre-refactor hl-jit/src/runtime tree) makes the static
...
# Brace-depth tracked from each module's `switch (nr)` so nested switches (ioctl cmd / prctl op / fcntl
...
# A module may hold MORE THAN ONE top-level `switch (nr)` (e.g. net.c: an AF_NETLINK-guarded switch
# followed by the main socket-family switch 198-210); the whole file is scanned and EVERY nr-switch is
# unioned, not just the first (#335 -- the socket family lived in the second switch and looked missing).
handled_one() {
    awk '
        { line=$0; sub(/\/\/.*/,"",line) }               # drop // comments
        !instr && line ~ /switch[ \t]*\([ \t]*nr[ \t]*\)/ { instr=1; depth=0; seenopen=0 }
        instr {
            ...
            if (n>0) seenopen=1
            if (depth<=0 && seenopen) instr=0            # left this nr-switch; keep scanning for the next
        }
    ' "$1"
}
handled_syscalls() {
...
dynamic() {
    ...
    [ -z "$A" ] && { echo "FATAL: engines not built (no target/debug/build/hl-jit-darwin-*/out/hljit-linux_aarch64); run \`make jit\` first" >&2; exit 4; }
    local log; log="$(mktemp)"
    # bare aarch64 + x86 guests
    for b in "$ROOT"/target/hl-tests/aarch64/*; do [ -f "$b" ] && dyn_one "$A" "'$b'" >>"$log" 2>&1; done
    for b in "$ROOT"/target/hl-tests/x86_64/*; do [ -f "$b" ] && dyn_one "$X" "'$b'" >>"$log" 2>&1; done
...
report() {
    ...
    }
    {
        ...
        echo ""
        echo "_Auto-generated by \`bash hl-tests/tools/coverage.sh report\`. Do not hand-edit; rerun instead._"
        echo ""
        echo "dd services Linux syscalls in \`os/linux/syscall/dispatch.c\`, which fans out to one handler"
        echo "module per family: \`svc_{sysv,mem,signal,time,io,aio,fs,proc,net,event,misc,rare}()\`. Every"
        ...
        echo "| metric | count |"
        echo "| --- | --- |"
        echo "| distinct canonical syscall numbers handled (12 modules) | **$handled_n** |"
        ...
        echo ""
        echo "Run \`bash hl-tests/tools/coverage.sh static\` for the per-arch named gap list."
        echo ""
        ...
        echo "these are hit by the hl-tests corpus or the busybox/alpine real-software runs."
        echo ""
