...
use super::*;
use hl_jit_darwin::{Guest, PortMap, SpawnConfig, Volume};
...
mod tests {
    ...
    fn build_rejects_empty_argv_element() {
        ...
        assert!(matches!(
            err,
            Error::Invalid("container argv contains empty argument")
        ));
    }

    #[test]
    fn builder_dialect_matches_daemon_keys() {
        // Every promoted HL_*/HL_JIT_* key encodes exactly as the daemon's spawn_cfg did.
    ...
    fn apply_device_binds_mounts_and_render_node() {
        ...
        let vols: Vec<(String, String, bool)> = c
            .cfg
            .volumes
            .iter()
        ...
        assert_eq!(
            ...
            vec![
                ...
                (
                    "/usr/lib/x/lib.so".to_string(),
                    "/host/lib.so".to_string(),
                    true
                ),
            ]
        );
        ...
        assert!(!c
            ...
            .env
            .iter()
            .any(|(k, _)| k == "IGNORED_BY_APPLY_DEVICE"));
    }
