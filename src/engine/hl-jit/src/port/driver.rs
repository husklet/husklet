...
impl Drivers {
    ...
    pub fn add_provider(
        ...
        p: impl DeviceProvider + Send + 'static,
    ) -> &mut Self {
...
mod tests {
    ...
    fn requests_returns_each_drivers_mounts_and_env_additively_in_order() {
        ...
        assert_eq!(
            reqs[0].mounts,
            vec![DeviceMount::ro("/host/libcuda.so", "/usr/lib/libcuda.so")]
        );
        assert_eq!(reqs[0].env, vec!["LD_LIBRARY_PATH=/usr/lib".to_string()]);
        // Second driver's request, in order.
        assert_eq!(
            reqs[1].mounts,
            vec![DeviceMount::rw("/host/wayland.sock", "/run/wayland.sock")]
        );
        assert_eq!(
            reqs[1].env,
            vec!["WAYLAND_DISPLAY=wayland.sock".to_string()]
        );
    }
    ...
    fn add_provider_adapts_a_bare_device_provider() {
        ...
        assert_eq!(
            drivers.requests(&[])[0].env,
            vec!["FROM_PROVIDER=1".to_string()]
        );
    }
}
