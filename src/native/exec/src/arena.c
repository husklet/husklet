...
static int services_valid(const hl_native_memory *memory) {
  return memory != NULL && memory->abi == HL_NATIVE_ABI &&
         memory->size >= sizeof(*memory) && memory->reserve != NULL &&
         memory->release != NULL && memory->publish != NULL &&
         memory->repair != NULL && memory->write_begin != NULL &&
         memory->write_end != NULL;
}

static int mapping_valid(const hl_native_mapping *mapping, uint64_t capacity,
                         uint64_t alignment) {
  return mapping->abi == HL_NATIVE_ABI && mapping->size >= sizeof(*mapping) &&
         mapping->handle != 0 && mapping->writable != 0 &&
         mapping->executable != 0 && mapping->capacity == capacity &&
         mapping->content <= capacity && mapping->writable % alignment == 0 &&
         mapping->executable % alignment == 0 &&
         mapping->writable <= UINTPTR_MAX - capacity &&
         mapping->executable <= UINTPTR_MAX - capacity;
}
...
hl_native_status hl_native_arena_create(hl_native_arena *arena,
                                        const hl_native_config *config) {
  hl_native_status status;
  uint32_t dual;
  if (arena == NULL || config == NULL || config->abi != HL_NATIVE_ABI ||
      config->size < sizeof(*config) ||
      config->capacity != HL_NATIVE_ARENA_BYTES ||
      !power_of_two(config->alignment) ||
      config->alignment > HL_NATIVE_ALIGNMENT_MAX ||
      !services_valid(config->memory) ||
      (config->flags &
       ~(HL_NATIVE_DUAL_PREFERRED | HL_NATIVE_DUAL_REQUIRED |
         HL_NATIVE_DIAGNOSTICS | HL_NATIVE_A64_NO_WRITE_RESERVE |
         HL_NATIVE_A64_NO_WRITE_COMMIT | HL_NATIVE_A64_RUNTIME_WRITE_RESERVE |
  ...
  status = arena->memory.reserve(arena->memory.context, config->capacity,
                                 config->alignment, dual != 0, &arena->mapping);
  if (status != HL_NATIVE_OK && dual == HL_NATIVE_DUAL_PREFERRED)
    status = arena->memory.reserve(arena->memory.context, config->capacity,
                                   config->alignment, 0, &arena->mapping);
  if (status != HL_NATIVE_OK)
    return status;
  if (!mapping_valid(&arena->mapping, config->capacity, config->alignment)) {
    if (arena->mapping.handle != 0)
...
hl_native_status hl_native_arena_begin(hl_native_arena *arena) {
  hl_native_status status;
  if (arena == NULL || arena->memory.reserve == NULL || arena->writing)
    return HL_STATE("arena begin while writing");
...
hl_native_status hl_native_arena_allocate(hl_native_arena *arena, uint64_t size,
                                          uint64_t alignment,
                                          hl_native_span *span) {
  uintptr_t current, aligned, limit;
  if (arena == NULL || span == NULL || !arena->writing || size == 0 ||
...
hl_native_status hl_native_arena_publish(hl_native_arena *arena,
                                         const hl_native_span *span,
                                         uint64_t used) {
  hl_native_status status;
  uintptr_t expected;
  if (arena == NULL || span == NULL || !arena->writing || used == 0 ||
      used > span->capacity)
    return HL_NATIVE_ARGUMENT;
  expected = (uintptr_t)arena->writable + span->offset;
  if ((uintptr_t)span->writable != expected ||
      (uintptr_t)span->writable < (uintptr_t)arena->cursor ||
      span->offset > arena->mapping.capacity ||
      used > arena->mapping.capacity - span->offset ||
      span->executable != arena->executable + span->offset)
    return HL_NATIVE_ARGUMENT;
  status = arena->memory.publish(arena->memory.context, arena->mapping.handle,
                                 span->offset, used);
  if (status != HL_NATIVE_OK)
    return status;
...
hl_native_status hl_native_arena_end(hl_native_arena *arena) {
  hl_native_status status;
  if (arena == NULL || arena->memory.reserve == NULL || !arena->writing)
    return HL_STATE("arena end while not writing");
...
hl_native_status hl_native_arena_repair(hl_native_arena *arena,
                                        uint32_t preserve) {
  hl_native_status status;
  if (arena == NULL || arena->memory.reserve == NULL || arena->writing ||
      preserve > 1)
    return HL_STATE("arena repair while writing");
  status =
      arena->memory.repair(arena->memory.context, &arena->mapping, preserve);
  if (status != HL_NATIVE_OK)
    return status;
  if (!mapping_valid(&arena->mapping, arena->capacity, arena->alignment))
    return HL_NATIVE_PLATFORM;
  bind(arena);
  return HL_NATIVE_OK;
}
...
hl_native_status hl_native_arena_rotate(hl_native_arena *arena) {
  hl_native_status status;
  if (arena == NULL || arena->memory.reserve == NULL || !arena->writing)
    return HL_STATE("arena rotate while not writing");
  status = arena->memory.write_end(arena->memory.context);
  if (status != HL_NATIVE_OK)
    return status;
  arena->writing = 0;
  ...
  if (status != HL_NATIVE_OK ||
      !mapping_valid(&arena->mapping, arena->capacity, arena->alignment))
...
void hl_native_arena_destroy(hl_native_arena *arena) {
  if (arena == NULL || arena->memory.reserve == NULL)
    return;
  if (arena->writing &&
      arena->memory.write_end(arena->memory.context) == HL_NATIVE_OK)
    arena->write_transitions++;
  arena->memory.release(arena->memory.context, arena->mapping.handle);
  memset(arena, 0, sizeof(*arena));
}
