...
_Static_assert(ATOMIC_LLONG_LOCK_FREE == 2,
               "executor admission requires lock-free uint64 atomics");
...
static hl_native_status run_lifecycle_enter(hl_native_executor *executor) {
  uint64_t current =
      atomic_load_explicit(&executor->fork_runs, memory_order_acquire);
  for (;;) {
    if ((current & FORK_RUNS_CLOSED) != 0)
      return HL_STATE("fork run admission closed");
    if ((current & FORK_RUNS_COUNT_MASK) == FORK_RUNS_COUNT_MASK)
      return HL_NATIVE_CAPACITY;
    if (atomic_compare_exchange_weak_explicit(&executor->fork_runs, &current,
...
static void run_lifecycle_leave(hl_native_executor *executor) {
  uint64_t prior =
      atomic_fetch_sub_explicit(&executor->fork_runs, 1, memory_order_release);
  (void)prior;
}
...
static void a64_fallback_word(hl_native_executor *executor, uint32_t word,
  ...
  if (!executor->diagnostics)
    return;
  atomic_fetch_add_explicit(entry_rejection
                                ? &executor->a64_fallback_entry_rejection
...
#define ADMISSION(state, count)                                                \
  (((uint64_t)(state) << ADMISSION_STATE_SHIFT) | (uint32_t)(count))
...
static uint64_t direct_generation(hl_native_executor *executor) {
  ...
  if (executor->direct_generation == 0)
    executor->direct_generation = 1;
...
static uint64_t authority_identity(hl_native_executor *executor) {
  ...
  if (executor->next_authority_identity == 0)
    executor->next_authority_identity = 1;
  return executor->next_authority_identity;
}

uint64_t
hl_native_certificate_cache_identity_issue(hl_native_executor *executor) {
  if (executor == NULL)
    return 0;
  uint64_t current = atomic_load_explicit(
      &executor->next_certificate_cache_identity, memory_order_relaxed);
  for (;;) {
...
hl_native_status
hl_native_direct_register(hl_native_executor *executor,
  ...
  *output = NULL;
  if (executor == NULL || authority == NULL ||
      authority->abi != HL_NATIVE_ABI ||
      ...
      authority->host_first >
          UINT64_MAX - (authority->guest_last - authority->guest_first))
    return HL_NATIVE_ARGUMENT;
  ...
  if (executor->direct_authority != NULL) {
    ...
    free(token);
    return HL_STATE("direct authority already held");
  }
  token->owner = executor;
  token->generation = direct_generation(executor);
  token->authority = *authority;
  if (!executor->retained_authority_valid ||
      memcmp(&executor->retained_authority, authority, sizeof(*authority)) !=
          0) {
...
int hl_native_direct_validate(const hl_native_executor *executor,
  ...
  if (executor == NULL || token == NULL || output == NULL ||
      ...
      token->generation == 0 ||
      token->generation != executor->direct_generation)
    return 0;
...
uint64_t hl_native_direct_generation(const hl_native_executor *executor,
                                     const hl_native_direct_token *token) {
  return executor != NULL && token != NULL &&
                 executor->direct_authority == token &&
                 token->owner == executor &&
                 token->generation == executor->direct_generation
             ? token->generation
...
uint64_t hl_native_direct_identity(const hl_native_executor *executor,
                                   const hl_native_direct_token *token) {
  return hl_native_direct_generation(executor, token) != 0 ? token->identity
                                                           : 0;
}
...
int hl_native_direct_request_valid(const hl_native_executor *executor,
  ...
  if (hl_native_direct_generation(executor, token) != generation ||
      identity == 0 || token->identity != identity || projection == NULL ||
  ...
  const hl_native_projection_view *view =
      &projection->views[projection->active];
  return authority->mapping_incarnation == projection->mapping_incarnation &&
         ...
         authority->guest_first == view->guest_first &&
         authority->guest_last == view->guest_last &&
...
int hl_native_direct_request_snapshot(const hl_native_executor *executor,
                                      ...
                                      const hl_native_projection *projection,
                                      hl_native_direct_authority *output) {
  if (output == NULL || !hl_native_direct_request_valid(
                            executor, token, generation, identity, projection))
    return 0;
  *output = token->authority;
  return 1;
}
...
hl_native_status hl_native_direct_unregister(hl_native_executor *executor,
                                             hl_native_direct_token *token) {
  hl_native_status status;
  if (executor == NULL || token == NULL || token->owner != executor)
    return HL_NATIVE_ARGUMENT;
  status = hl_native_executor_gate_enter(executor);
  if (status != HL_NATIVE_OK)
    return status;
  if (executor->direct_authority == token &&
      token->generation == executor->direct_generation) {
...
hl_native_status
hl_native_interrupt_create(hl_native_interrupt_token **output) {
  if (output == NULL)
    return HL_NATIVE_ARGUMENT;
...
hl_native_status hl_native_interrupt_set(hl_native_interrupt_token *token,
                                         uint64_t value) {
  if (token == NULL)
    return HL_NATIVE_ARGUMENT;
...
static void ibtc_clear(hl_native_executor *executor) {
  ...
  if (madvise(executor->ibtc, bytes, HL_MADV_DONTNEED) == 0)
    return;
#endif
...
static void cache_observe(void *context, hl_native_cache_event event) {
  ...
  if (executor == NULL)
    return;
  switch (event) {
  case HL_NATIVE_CACHE_RELOCATION_COLD_TARGET:
    counter = &executor->relocation_cold_targets;
    break;
  case HL_NATIVE_CACHE_RELOCATION_CYCLE:
    counter = &executor->relocation_cycles;
    break;
  case HL_NATIVE_CACHE_RELOCATION_CAPACITY:
    counter = &executor->relocation_capacity;
    break;
  case HL_NATIVE_CACHE_RELOCATION_INVALIDATION:
    counter = &executor->relocation_invalidations;
    break;
  default:
    return;
  }
...
static void ibtc_invalidate(hl_native_executor *executor, uint64_t first,
                            uint64_t last) {
  if (executor == NULL || executor->ibtc == NULL || last <= first)
    return;
...
static void ibtc_publish(hl_native_ibtc_entry *entry, uint64_t target,
                         void *body) {
#if defined(__aarch64__)
  __asm__ volatile("dmb ish\n\tstp %1, %2, [%0]"
                   :
                   : "r"(entry), "r"(target), "r"(body)
                   : "memory");
#else
  entry->body = body;
...
void hl_native_ibtc_fill_shared(hl_native_executor *executor, uint64_t target,
                                void *body) {
  ibtc_publish(&executor->ibtc[(target >> 2) & (HL_NATIVE_IBTC_COUNT - 1)],
               target, body);
}

#if defined(__aarch64__)
hl_native_status hl_native_ibtc_fill(hl_native_executor *executor,
                                     hl_native_aarch64_cpu *cpu,
                                     uint64_t target,
                                     const hl_native_code *code) {
  ...
  if (site == 0)
    return HL_NATIVE_OK;
  if ((site & 15u) != 0)
    return HL_STATE("ibtc site alignment");
  ...
  if (executor->diagnostics) {
    ...
    executor->ibtc_fills++;
    atomic_fetch_add_explicit(&executor->ibtc_site_misses, 1,
                              memory_order_relaxed);
    atomic_fetch_add_explicit(&executor->ibtc_shared_misses, 1,
                              memory_order_relaxed);
    if (previous != 0 && previous != target)
      executor->ibtc_site_collisions++;
    if (shared->target != 0 && shared->target != target)
      executor->ibtc_shared_collisions++;
  }
  int64_t branch_delta;
  memcpy(&branch_delta, executor->arena.writable + (site - rx) + 8,
         sizeof(branch_delta));
  ...
  uint32_t patched = 0;
  if (status == HL_NATIVE_OK)
    status = hl_native_cache_relocate_site(
        executor->cache, (const void *)site, branch_delta, target,
        code->instruction_epoch, code->identity_token, &patched);
  if (status == HL_NATIVE_OK) {
    ...
    status = executor->arena.memory.publish(executor->arena.memory.context,
                                            executor->arena.mapping.handle,
                                            offset, 8);
  }
  if (status == HL_NATIVE_OK)
    hl_native_ibtc_fill_shared(executor, target, code->body);
  if (writing) {
...
hl_native_status hl_native_executor_gate_enter(hl_native_executor *executor) {
  ...
  if (!atomic_compare_exchange_strong_explicit(
          &executor->admission, &expected, ADMISSION(MUTATION_ACTIVE, 0),
...
void hl_native_executor_gate_leave(hl_native_executor *executor) {
  if (executor == NULL)
    return;
  atomic_store_explicit(&executor->admission, ADMISSION(MUTATION_OPEN, 0),
...
static uint64_t activation_generation(hl_native_executor *executor) {
  ...
  for (;;) {
    ...
    uint64_t desired = current + 1;
    if (atomic_compare_exchange_weak_explicit(
            &executor->activation_generation, &current, desired,
            memory_order_relaxed, memory_order_relaxed))
      return desired;
  }
}

hl_native_status hl_native_execution_enter(hl_native_executor *executor,
                                           hl_native_execution *execution) {
  if (executor == NULL || execution == NULL || execution->owner != NULL)
    return HL_NATIVE_ARGUMENT;
  uint64_t current =
      atomic_load_explicit(&executor->admission, memory_order_acquire);
  for (;;) {
    if (ADMISSION_STATE(current) != MUTATION_OPEN)
      return HL_STATE("execution admission not open");
    if (ADMISSION_COUNT(current) == UINT32_MAX)
      return HL_NATIVE_CAPACITY;
    uint64_t desired = ADMISSION(MUTATION_OPEN, ADMISSION_COUNT(current) + 1);
    if (atomic_compare_exchange_weak_explicit(&executor->admission, &current,
...
uint64_t hl_native_execution_generation(const hl_native_execution *execution) {
  return execution != NULL && execution->owner != NULL ? execution->generation
                                                       : 0;
}

hl_native_status
hl_native_execution_bind_certificate(hl_native_execution *execution,
                                     uint64_t *token) {
...
hl_native_status hl_native_execution_leave(hl_native_execution *execution) {
  ...
  if (execution == NULL || execution->owner == NULL)
    return HL_NATIVE_ARGUMENT;
  ...
  uint64_t current =
      atomic_load_explicit(&executor->admission, memory_order_relaxed);
  for (;;) {
    if (ADMISSION_STATE(current) != MUTATION_OPEN ||
        ADMISSION_COUNT(current) == 0)
...
hl_native_status hl_native_fault_scope_enter(hl_native_executor *executor,
                                             hl_native_cpu *cpu,
                                             hl_native_fault_scope *scope) {
  hl_native_execution execution = {0};
  if (executor == NULL || cpu == NULL || scope == NULL ||
      scope->executor != NULL || scope->reserved != 0 ||
      cpu->abi != HL_NATIVE_ABI || cpu->size != sizeof(*cpu) ||
      cpu->state.opaque == NULL ||
      (cpu->architecture != HL_NATIVE_AARCH64 &&
       cpu->architecture != HL_NATIVE_X86_64))
    return HL_NATIVE_ARGUMENT;
...
hl_native_status hl_native_fault_scope_leave(hl_native_fault_scope *scope) {
  if (scope == NULL || scope->abi != HL_NATIVE_ABI ||
      scope->size != sizeof(*scope) || scope->reserved != 0 ||
...
int hl_native_fault_scope_provenance(const hl_native_fault_scope *scope,
                                     uint64_t host_pc,
                                     hl_native_provenance *output) {
  if (scope == NULL || output == NULL || scope->abi != HL_NATIVE_ABI ||
      scope->size != sizeof(*scope) || scope->reserved > 1 ||
      scope->executor == NULL || scope->cpu == NULL || host_pc == 0)
    return 0;
  return hl_native_cache_provenance_record(
      scope->executor->cache, (const void *)(uintptr_t)host_pc, output);
}

int hl_native_fault_scope_contains(const hl_native_fault_scope *scope,
                                   uint64_t host_pc) {
  hl_native_provenance record;
  return hl_native_fault_scope_provenance(scope, host_pc, &record);
}

int hl_native_fault_scope_prepare_return(const hl_native_fault_scope *scope,
                                         uint64_t host_pc, uint64_t host_fault,
                                         void *host_context) {
  hl_native_provenance record;
  if (host_context == NULL ||
      !hl_native_fault_scope_provenance(scope, host_pc, &record))
    return 0;
#if defined(__linux__) && defined(__aarch64__)
  if (scope->architecture == HL_NATIVE_AARCH64) {
    hl_a64_host_context context;
    if (!hl_a64_linux_context(host_context, &context) ||
        context.program != host_pc)
      return 0;
    return hl_a64_linux_fault_return(scope->cpu->state.aarch64, host_context,
                                     &record, host_fault);
  }
#elif defined(__APPLE__) && defined(__aarch64__)
  if (scope->architecture == HL_NATIVE_AARCH64) {
    hl_a64_host_context context;
    if (!hl_a64_darwin_context(host_context, &context) ||
        context.program != host_pc)
      return 0;
    return hl_a64_darwin_fault_return(scope->cpu->state.aarch64, host_context,
                                      &record, host_fault);
  }
#else
  (void)host_fault;
...
hl_native_status hl_native_execution_exit(hl_native_execution *execution,
                                          hl_native_exit *output, uint32_t kind,
                                          uint32_t access, uint64_t instruction,
  ...
  if (execution == NULL || execution->owner == NULL)
    return HL_NATIVE_ARGUMENT;
  status = hl_native_exit_build(output, kind, access, instruction, next,
                                address, code);
  release = hl_native_execution_leave(execution);
  return status == HL_NATIVE_OK ? release : status;
}

hl_native_status hl_native_create(const hl_native_config *config,
                                  hl_native_executor **output) {
  hl_native_executor *executor;
  hl_native_status status;
  if (output == NULL)
    return HL_NATIVE_ARGUMENT;
  *output = NULL;
   ...
   * that selected continuation before it became the default. */
  executor->dirty_overflow_continue =
      (config->flags & HL_NATIVE_A64_DIRTY_OVERFLOW_EXIT) == 0;
  ...
  hl_native_cache_observer observer = {
      executor, executor->diagnostics ? cache_observe : NULL};
  status = hl_native_cache_create(&executor->cache, &executor->arena, 1u << 19,
                                  1u << 18, 0, 0, &observer);
  if (status != HL_NATIVE_OK) {
...
hl_native_status hl_native_before_fork(hl_native_executor *executor) {
  ...
  if (executor == NULL)
    return HL_NATIVE_ARGUMENT;
  if (!atomic_compare_exchange_strong_explicit(
          &executor->fork_runs, &runs_expected, FORK_RUNS_CLOSED,
          memory_order_seq_cst, memory_order_relaxed))
    return HL_STATE("fork close: runs outstanding");
  if (!atomic_compare_exchange_strong_explicit(
          &executor->admission, &expected, ADMISSION(MUTATION_FORK, 0),
          memory_order_seq_cst, memory_order_relaxed)) {
...
hl_native_status hl_native_after_fork(hl_native_executor *executor,
                                      uint32_t preserve) {
  hl_native_status status;
  uint64_t expected = ADMISSION(MUTATION_FORK, 0);
  if (executor == NULL || preserve > 1)
    return HL_NATIVE_ARGUMENT;
  if (!atomic_compare_exchange_strong_explicit(
          &executor->admission, &expected, ADMISSION(MUTATION_ACTIVE, 0),
  ...
  if (status == HL_NATIVE_OK && preserve == 0)
    status = hl_native_cache_reset(executor->cache, 0);
  if (status == HL_NATIVE_OK && preserve != 0) {
    int writing = 0;
    status = hl_native_cache_write_begin(executor->cache);
    if (status == HL_NATIVE_OK)
      writing = 1;
    if (status == HL_NATIVE_OK)
      status = hl_native_cache_relocations_restore(executor->cache);
    if (writing) {
      hl_native_status end = hl_native_cache_write_end(executor->cache);
      if (status == HL_NATIVE_OK)
        status = end;
...
hl_native_status hl_native_diagnose(const hl_native_executor *executor,
                                    hl_native_diagnostics *output) {
  ...
  if (output->size >=
      offsetof(hl_native_diagnostics, x86_syscall_vector_dirty)) {
  ...
  if (output->size >=
      offsetof(hl_native_diagnostics, relocation_cold_targets)) {
    output->a64_guard_fast =
        atomic_load_explicit(&executor->a64_guard_fast, memory_order_relaxed);
    output->a64_guard_full =
        atomic_load_explicit(&executor->a64_guard_full, memory_order_relaxed);
    output->a64_guard_fallback = atomic_load_explicit(
        &executor->a64_guard_fallback, memory_order_relaxed);
    output->a64_dirty_reserved = atomic_load_explicit(
        &executor->a64_dirty_reserved, memory_order_relaxed);
    output->a64_dirty_overflow = atomic_load_explicit(
        &executor->a64_dirty_overflow, memory_order_relaxed);
    output->a64_dirty_committed = atomic_load_explicit(
        &executor->a64_dirty_committed, memory_order_relaxed);
    output->a64_dirty_merged =
        atomic_load_explicit(&executor->a64_dirty_merged, memory_order_relaxed);
    output->x86_cold_builds =
        atomic_load_explicit(&executor->x86_cold_builds, memory_order_relaxed);
    output->x86_cold_quota_exits = atomic_load_explicit(
        &executor->x86_cold_quota_exits, memory_order_relaxed);
  }
  ...
  if (output->size >= sizeof(hl_native_diagnostics)) {
    output->x86_guard_fast =
        atomic_load_explicit(&executor->x86_guard_fast, memory_order_relaxed);
    output->x86_guard_full =
        atomic_load_explicit(&executor->x86_guard_full, memory_order_relaxed);
    output->x86_guard_fallback = atomic_load_explicit(
        &executor->x86_guard_fallback, memory_order_relaxed);
    output->x86_dirty_merged =
        atomic_load_explicit(&executor->x86_dirty_merged, memory_order_relaxed);
    output->x86_dirty_committed = atomic_load_explicit(
        &executor->x86_dirty_committed, memory_order_relaxed);
    output->x86_dirty_overflow = atomic_load_explicit(
        &executor->x86_dirty_overflow, memory_order_relaxed);
    output->x86_write_cache_hit = atomic_load_explicit(
        &executor->x86_write_cache_hit, memory_order_relaxed);
    output->x86_write_cache_miss = atomic_load_explicit(
        &executor->x86_write_cache_miss, memory_order_relaxed);
  }
  if (output->size >=
      offsetof(hl_native_diagnostics, a64_fallback_guard_read)) {
    output->relocation_cold_targets = atomic_load_explicit(
        &executor->relocation_cold_targets, memory_order_relaxed);
    output->relocation_cycles = atomic_load_explicit(
        &executor->relocation_cycles, memory_order_relaxed);
    output->relocation_capacity = atomic_load_explicit(
        &executor->relocation_capacity, memory_order_relaxed);
    output->relocation_invalidations = atomic_load_explicit(
        &executor->relocation_invalidations, memory_order_relaxed);
    output->ibtc_site_misses =
        atomic_load_explicit(&executor->ibtc_site_misses, memory_order_relaxed);
    output->ibtc_shared_misses = atomic_load_explicit(
        &executor->ibtc_shared_misses, memory_order_relaxed);
  }
  if (output->size >=
      offsetof(hl_native_diagnostics, a64_fallback_entry_rejection)) {
    output->a64_fallback_guard_read = atomic_load_explicit(
        &executor->a64_fallback_guard_read, memory_order_relaxed);
    output->a64_fallback_guard_write = atomic_load_explicit(
        &executor->a64_fallback_guard_write, memory_order_relaxed);
    output->a64_fallback_simd_fp = atomic_load_explicit(
        &executor->a64_fallback_simd_fp, memory_order_relaxed);
    output->a64_fallback_memory = atomic_load_explicit(
        &executor->a64_fallback_memory, memory_order_relaxed);
    output->a64_fallback_control = atomic_load_explicit(
        &executor->a64_fallback_control, memory_order_relaxed);
    output->a64_fallback_other = atomic_load_explicit(
        &executor->a64_fallback_other, memory_order_relaxed);
  }
  if (output->size >= offsetof(hl_native_diagnostics, x86_public_epochs)) {
    output->a64_fallback_entry_rejection = atomic_load_explicit(
        &executor->a64_fallback_entry_rejection, memory_order_relaxed);
    output->a64_fallback_generated = atomic_load_explicit(
        &executor->a64_fallback_generated, memory_order_relaxed);
    output->a64_fallback_call = atomic_load_explicit(
        &executor->a64_fallback_call, memory_order_relaxed);
    output->a64_fallback_return = atomic_load_explicit(
        &executor->a64_fallback_return, memory_order_relaxed);
    output->a64_fallback_indirect = atomic_load_explicit(
        &executor->a64_fallback_indirect, memory_order_relaxed);
    output->a64_fallback_system = atomic_load_explicit(
        &executor->a64_fallback_system, memory_order_relaxed);
    output->a64_fallback_form_memory = atomic_load_explicit(
        &executor->a64_fallback_form_memory, memory_order_relaxed);
    output->a64_fallback_form_other = atomic_load_explicit(
        &executor->a64_fallback_form_other, memory_order_relaxed);
  }
  if (output->size >= offsetof(hl_native_diagnostics, a64_branch_exhaustion)) {
    output->x86_public_epochs = executor->x86_public_epochs;
  }
  if (output->size >=
      offsetof(hl_native_diagnostics, ibtc_authenticated_entries)) {
    output->a64_branch_exhaustion = atomic_load_explicit(
        &executor->a64_branch_exhaustion, memory_order_relaxed);
    output->a64_branch_cold_relocation = atomic_load_explicit(
        &executor->a64_branch_cold_relocation, memory_order_relaxed);
    output->a64_branch_nonrelocatable = atomic_load_explicit(
        &executor->a64_branch_nonrelocatable, memory_order_relaxed);
    output->a64_branch_unidentified = atomic_load_explicit(
        &executor->a64_branch_unidentified, memory_order_relaxed);
    uint64_t sample_form = atomic_load_explicit(
        &executor->a64_branch_sample_form, memory_order_acquire);
    if (sample_form != 0) {
      output->a64_branch_sample_pc = executor->a64_branch_sample_pc;
      output->a64_branch_sample_source_first =
          executor->a64_branch_sample_source_first;
      output->a64_branch_sample_source_last =
...
hl_native_status hl_native_changed(hl_native_executor *executor,
                                   const hl_native_change *changes,
                                   size_t count) {
  if (executor == NULL || changes == NULL || count == 0 || count > 1024)
    return HL_NATIVE_ARGUMENT;
  int replaced = 0;
  for (size_t index = 0; index < count; index++) {
    const hl_native_change *change = &changes[index];
    if (change->abi != HL_NATIVE_ABI || change->size < sizeof(*change) ||
        change->reserved != 0 ||
        (change->kind != HL_NATIVE_REPLACE &&
         change->kind != HL_NATIVE_INVALIDATE) ||
        (change->kind == HL_NATIVE_INVALIDATE &&
         change->last <= change->first) ||
        (change->kind == HL_NATIVE_REPLACE &&
         (change->first != 0 || change->last != 0)))
      return HL_NATIVE_ARGUMENT;
  }
  hl_native_status status = hl_native_executor_gate_enter(executor);
  if (status != HL_NATIVE_OK)
  ...
  for (size_t index = 0; index < count; index++) {
    ...
    if (change->kind == HL_NATIVE_REPLACE) {
      ...
      status = (change->mapping_epoch == 0 ||
                hl_native_cache_address_identity_matches(
                    executor->cache, change->mapping_epoch,
                    executor->memory_mode, executor->authority_generation))
                   ? hl_native_cache_relocations_invalidate(
                         executor->cache, change->first, change->last)
                   : HL_STATE("change outside cache address identity");
      if (status == HL_NATIVE_OK)
        status = hl_native_cache_invalidate(executor->cache, change->first,
                                            change->last, NULL);
    }
  }
  if (writing) {
...
hl_native_status hl_native_resolve_fault(const hl_native_executor *executor,
                                         hl_native_fault *fault) {
  uint64_t guest_pc;
  if (executor == NULL || fault == NULL || fault->abi != HL_NATIVE_ABI ||
      fault->size < sizeof(*fault) || fault->access > HL_NATIVE_ACCESS_EXECUTE)
    return HL_NATIVE_ARGUMENT;
  fault->precise =
      hl_native_fault_guest(executor->cache, fault->host_pc, &guest_pc);
  fault->guest_pc = fault->precise ? guest_pc : 0;
  return HL_NATIVE_OK;
}

static hl_native_status run_exit(hl_native_execution *execution,
                                 hl_native_exit *output, uint32_t kind,
                                 uint64_t instruction) {
  return hl_native_execution_exit(execution, output, kind,
                                  HL_NATIVE_ACCESS_UNKNOWN, instruction,
                                  instruction, 0, 0);
}
...
static hl_native_status run_fatal(hl_native_execution *execution,
                                  hl_native_exit *output, uint64_t code) {
  return hl_native_execution_exit(execution, output, HL_NATIVE_EXIT_FATAL,
                                  HL_NATIVE_ACCESS_UNKNOWN, 0, 0, 0,
                                  code == 0 ? 1 : code);
}
#endif

hl_native_status hl_native_synchronize_epoch(hl_native_executor *executor,
                                             uint64_t mapping_epoch,
                                             uint64_t instruction_epoch,
                                             uint64_t memory_mode,
                                             uint64_t authority_generation) {
  hl_native_status status;
  if (hl_native_cache_epoch_matches(executor->cache, mapping_epoch,
                                    instruction_epoch, memory_mode,
  ...
  if (!hl_native_cache_epoch_matches(executor->cache, mapping_epoch,
                                     instruction_epoch, memory_mode,
                                     authority_generation))
    status = hl_native_cache_reset_identity(executor->cache, mapping_epoch,
                                            instruction_epoch, memory_mode,
                                            authority_generation);
  if (status == HL_NATIVE_OK)
    ibtc_clear(executor);
...
hl_native_status
hl_native_synchronize_direct(hl_native_executor *executor,
                             uint64_t mapping_epoch, uint64_t instruction_epoch,
                             const hl_native_direct_token *token,
                             uint64_t generation, uint64_t identity,
                             const hl_native_projection *projection) {
  hl_native_status status = hl_native_executor_gate_enter(executor);
  if (status != HL_NATIVE_OK)
    return status;
  if (!hl_native_direct_request_valid(executor, token, generation, identity,
                                      projection)) {
    status = HL_STATE("direct synchronize request invalid");
  } else if (!hl_native_cache_epoch_matches(executor->cache, mapping_epoch,
                                            instruction_epoch, 1, identity)) {
    status = hl_native_cache_reset_identity(executor->cache, mapping_epoch,
                                            instruction_epoch, 1, identity);
    if (status == HL_NATIVE_OK)
      ibtc_clear(executor);
...
hl_native_status hl_native_executor_rollover(hl_native_executor *executor,
                                             uint64_t mapping_epoch,
                                             uint64_t instruction_epoch) {
  hl_native_status status;
  if (executor == NULL ||
  ...
  status = hl_native_cache_reset_identity(
      executor->cache, mapping_epoch, instruction_epoch, executor->memory_mode,
 ...
 * crossing, is 168 ms of a 48.65 s phase: 0.35%. So the widest possible
 * run-view cache buys 0.13% of sqlite, while the extra linear scan it imposes
 * on a64_guard_full's 73,085,374 executions costs more than that outright.
...
static void run_view_promote(hl_a64_run_views *cache, size_t index) {
  ...
  if (cache == NULL || index >= cache->count || index == 0)
    return;
  selected = cache->entries[index];
  memmove(&cache->entries[1], &cache->entries[0],
          index * sizeof(cache->entries[0]));
...
static int run_view_resolve(hl_a64_run_views *cache, hl_native_aarch64_cpu *cpu,
  ...
  for (index = 0; index < cache->count; index++) {
    ...
    if (!run_view_contains(&cache->entries[index], address, size, required))
      continue;
...
static void run_view_install(hl_a64_run_views *cache, const hl_a64_view *view) {
  ...
  if (cache->count < HL_A64_RUN_VIEW_COUNT)
    cache->count++;
  if (cache->count > 1)
    memmove(&cache->entries[1], &cache->entries[0],
...
void hl_a64_run_view_publish(const hl_a64_run_views *cache,
                             hl_native_aarch64_cpu *cpu,
  ...
  if (mapping_incarnation == 0 || count > HL_A64_RUN_VIEW_COUNT)
    return;
  for (size_t index = 0; index < count; ++index) {
    const hl_a64_view *view = &cache->entries[index];
    if (view->guest_last <= view->guest_first ||
        view->write_policy != HL_NATIVE_WRITE_EXACT ||
        view->mapping_incarnation != mapping_incarnation ||
...
static void active_view_publish(hl_native_aarch64_cpu *cpu,
                                uint64_t mapping_incarnation,
                                uint64_t authority) {
  active_view_clear(cpu);
  if (mapping_incarnation == 0 || authority == 0 ||
      cpu->memory_last <= cpu->memory_first || cpu->memory_permissions == 0 ||
      (cpu->memory_permissions & ~UINT64_C(7)) != 0)
    return;
...
static hl_native_status a64_execution_enter(hl_native_executor *executor,
  ...
  if (status != HL_NATIVE_OK)
    return status;
  status =
      hl_native_execution_bind_certificate(execution, &cpu->certificate_token);
  if (status != HL_NATIVE_OK)
    (void)hl_native_execution_leave(execution);
  return status;
}

static hl_native_status run_aarch64(hl_native_executor *executor,
                                    hl_native_cpu *cpu_handle,
                                    hl_native_aarch64_cpu *cpu,
                                    const hl_native_run_request *request,
  ...
  if ((quantum_poll == NULL) != (quantum_grant == 0))
    return HL_NATIVE_ARGUMENT;
  if ((fault_publish == NULL) != (fault_unpublish == NULL))
    return HL_NATIVE_ARGUMENT;
  ...
  if ((memory_mode == 0) != (authority_generation == 0) ||
      (memory_mode == 0) != (authority_identity == 0))
    return HL_NATIVE_ARGUMENT;
  expected_authority =
      memory_mode != 0 ? authority_identity : request->mapping_epoch;
  if (expected_authority == 0)
    return HL_NATIVE_ARGUMENT;
  hl_native_status epoch_status =
      memory_mode == 0
          ? hl_native_synchronize_epoch(executor, source->mapping_incarnation,
                                        0, 0, expected_authority)
          : hl_native_synchronize_direct(executor, source->mapping_incarnation,
                                         0, direct_token, authority_generation,
                                         authority_identity, projection);
  if (epoch_status != HL_NATIVE_OK)
    return epoch_status;
  if (projection != NULL) {
  ...
  for (;;) {
    ...
    if (execution.owner == NULL) {
      ...
      if (status != HL_NATIVE_OK)
        return status;
      if (memory_mode != 0 &&
          !hl_native_direct_request_snapshot(
              executor, direct_token, authority_generation, authority_identity,
              projection, &direct_authority)) {
        (void)hl_native_execution_leave(&execution);
        return HL_STATE("direct authority snapshot refused");
      }
    }
    if (cpu->interrupt != 0)
      return run_exit(&execution, output, HL_NATIVE_EXIT_INTERRUPT,
                      instruction);
    if (budget == 0) {
    ...
    if (limit > HL_A64_TRACE_MAX_WORDS)
      limit = HL_A64_TRACE_MAX_WORDS;
    if (!extendable && budget < limit)
      limit = (size_t)budget;
    ...
    if (count == 0 && translated && resolver != NULL) {
      ...
      if (resolver(resolver_context, instruction, source->mapping_incarnation,
        ...
        resolved_source =
            (hl_a64_source){&resolved_span, 1, source->mapping_incarnation,
                            resolved_span.instruction_epoch};
        if (!hl_a64_source_validate(&resolved_source))
          return HL_NATIVE_ARGUMENT;
    ...
    hl_native_lookup found =
        hl_native_translation_lookup(executor, &key, &code);
    ...
    if (found != HL_NATIVE_HIT || code.instruction_count > budget) {
      ...
      if (resolver != NULL && active_source == source && count < limit &&
          ...
          hl_a64_source_fetch(active_source, instruction + (count - 1) * 4, 1,
                              &tail) &&
          (tail.words[0] & UINT32_C(0xfc000000)) == UINT32_C(0x14000000)) {
        int64_t displacement =
            (int64_t)((uint64_t)(tail.words[0] & UINT32_C(0x03ffffff)) << 38) >>
            36;
        uint64_t branch_pc = instruction + (count - 1) * 4;
        uint64_t magnitude = displacement < 0
                                 ? UINT64_C(0) - (uint64_t)displacement
                                 : (uint64_t)displacement;
        int target_valid = displacement < 0
                               ? magnitude <= branch_pc
                               : magnitude <= UINT64_MAX - branch_pc;
        uint64_t target =
            displacement < 0 ? branch_pc - magnitude : branch_pc + magnitude;
        hl_native_source_span successor = {0};
        if (target_valid && target != instruction &&
            resolver(resolver_context, target, source->mapping_incarnation,
          ...
          while (insert < active_source->span_count &&
                 region_spans[insert].guest_first < successor.guest_first)
            insert++;
          memmove(&region_spans[insert + 1], &region_spans[insert],
                  (active_source->span_count - insert) *
                      sizeof(region_spans[0]));
          region_spans[insert] = successor;
          region_source = (hl_a64_source){
              region_spans, active_source->span_count + 1,
              source->mapping_incarnation, source->instruction_epoch};
          if (hl_a64_source_validate(&region_source)) {
            size_t successor_count =
                hl_a64_source_available(&region_source, target, limit - count);
      ...
      status = hl_a64_trace_cache_direct(
          executor, build_source, instruction, build_count, scratch,
          sizeof(scratch), memory_mode != 0 ? &direct_authority : NULL,
          expected_authority, &code, &hit);
      if (status != HL_NATIVE_OK) {
        if (executor->diagnostics) {
        ...
        if (status != HL_NATIVE_OK)
          return status;
        return run_exit(&execution, output, HL_NATIVE_EXIT_FALLBACK,
                        instruction);
      }
      status = a64_execution_enter(executor, &execution, cpu);
      if (status != HL_NATIVE_OK)
      ...
      if (code.instruction_count > budget && extendable) {
        if (!quantum_poll(quantum_context, cpu->executed, cumulative_budget))
          return run_exit(&execution, output, HL_NATIVE_EXIT_YIELD,
                          instruction);
        if (cumulative_budget > UINT64_MAX - quantum_grant ||
            cpu->executed > cumulative_budget)
          return run_fatal(&execution, output, A64_FATAL_QUANTUM_GRANT);
    ...
    cpu->active_authority =
        memory_mode != 0 ? authority_identity : request->mapping_epoch;
    active_view_publish(cpu, request->mapping_epoch, expected_authority);
    if (fault_publish != NULL &&
        fault_publish(fault_context, &fault_scope) != HL_NATIVE_OK) {
      cpu->active_authority = 0;
      active_view_clear(cpu);
      status = hl_native_execution_leave(&execution);
      return status == HL_NATIVE_OK ? HL_STATE("fault scope publish refused")
                                    : status;
    }
    cpu->diagnostic_guard_fast = cpu->diagnostic_guard_full =
        cpu->diagnostic_guard_fallback = 0;
    cpu->diagnostic_dirty_reserved = cpu->diagnostic_dirty_overflow =
        cpu->diagnostic_dirty_committed = 0;
    ...
    if (fault_unpublish != NULL)
      fault_unpublish(fault_context, &fault_scope);
    cpu->active_authority = 0;
    active_view_clear(cpu);
    if (cpu->budget > cumulative_budget)
      return run_fatal(&execution, output, A64_FATAL_ENTER_BUDGET);
    cpu->executed = cumulative_budget - cpu->budget;
    if (executor->diagnostics) {
      atomic_fetch_add_explicit(&executor->a64_guard_fast,
                                cpu->diagnostic_guard_fast,
                                memory_order_relaxed);
      atomic_fetch_add_explicit(&executor->a64_guard_full,
                                cpu->diagnostic_guard_full,
                                memory_order_relaxed);
      atomic_fetch_add_explicit(&executor->a64_guard_fallback,
                                cpu->diagnostic_guard_fallback,
                                memory_order_relaxed);
      atomic_fetch_add_explicit(&executor->a64_dirty_reserved,
                                cpu->diagnostic_dirty_reserved,
                                memory_order_relaxed);
      atomic_fetch_add_explicit(&executor->a64_dirty_overflow,
                                cpu->diagnostic_dirty_overflow,
                                memory_order_relaxed);
      atomic_fetch_add_explicit(&executor->a64_dirty_committed,
                                cpu->diagnostic_dirty_committed,
                                memory_order_relaxed);
      atomic_fetch_add_explicit(&executor->a64_dirty_merged,
                                cpu->diagnostic_dirty_merged,
                                memory_order_relaxed);
      cpu->diagnostic_guard_fast = cpu->diagnostic_guard_full =
          cpu->diagnostic_guard_fallback = 0;
      cpu->diagnostic_dirty_reserved = cpu->diagnostic_dirty_overflow =
    ...
    if ((cpu->execution_identity & 3u) == 1u) {
      if (!hl_native_cache_execution(executor->cache, cpu->execution_identity,
                                     &executed_code))
    ...
    if (executor->diagnostics) {
      ...
      case HL_NATIVE_EXIT_BRANCH: {
        ...
        if (!executed_identity) {
          atomic_fetch_add_explicit(&executor->a64_branch_unidentified, 1,
                                    memory_order_relaxed);
          ...
          atomic_fetch_add_explicit(&executor->a64_branch_exhaustion, 1,
                                    memory_order_relaxed);
          form = HL_NATIVE_A64_BRANCH_FORM_EXHAUSTION;
        } else if (executed_code.relocation_count != 0) {
          atomic_fetch_add_explicit(&executor->a64_branch_cold_relocation, 1,
                                    memory_order_relaxed);
          form = HL_NATIVE_A64_BRANCH_FORM_COLD_RELOCATION;
        } else {
          atomic_fetch_add_explicit(&executor->a64_branch_nonrelocatable, 1,
                                    memory_order_relaxed);
        ...
        if (atomic_compare_exchange_strong_explicit(
                &executor->a64_branch_sample_claim, &unclaimed, 1,
                memory_order_relaxed, memory_order_relaxed)) {
          ...
          atomic_store_explicit(&executor->a64_branch_sample_form, form,
                                memory_order_release);
        }
        break;
      }
      case HL_NATIVE_EXIT_SYSCALL:
        executor->boundary_syscall++;
        break;
      case HL_NATIVE_EXIT_FALLBACK:
        executor->boundary_fallback++;
        break;
      case HL_NATIVE_EXIT_YIELD:
        executor->boundary_yield++;
        break;
      default:
        break;
      }
    ...
    if (cpu->reason == HL_NATIVE_EXIT_FALLBACK && cpu->fault_access != 0 &&
      ...
      if (executor->diagnostics) {
        ...
        atomic_fetch_add_explicit(counter, 1, memory_order_relaxed);
        atomic_fetch_add_explicit(&executor->a64_fallback_generated, 1,
                                  memory_order_relaxed);
        atomic_fetch_add_explicit(&executor->a64_fallback_form_memory, 1,
                                  memory_order_relaxed);
        /* The word classifier credits both taxonomies together; a guard fault
         * must too, or form_memory reads as 100% while memory reads as 0 for
         * the same exit. */
        atomic_fetch_add_explicit(&executor->a64_fallback_memory, 1,
                                  memory_order_relaxed);
      }
    }
    if (cpu->reason == HL_NATIVE_EXIT_FALLBACK && cpu->fault_access != 0 &&
      ...
      if (++operand_retry_count > HL_A64_OPERAND_RETRY_LIMIT)
        return run_exit(&execution, output, HL_NATIVE_EXIT_FALLBACK,
                        cpu->program);
      if (completed > budget)
        return run_fatal(&execution, output, A64_FATAL_OPERAND_COMPLETED);
      ...
      if (HL_A64_RUN_VIEW_CACHE &&
          run_view_resolve(&operand_views, cpu, cpu->fault_address,
                           cpu->fault_size, (uint32_t)cpu->fault_access)) {
        if (executor->diagnostics)
          executor->operand_cache_hits++;
        ...
        hl_a64_run_view_publish(&operand_views, cpu,
      ...
      if (cpu->interrupt != 0 || budget == 0)
        continue;
      if (executor->diagnostics)
        executor->operand_callbacks++;
      result = operand_resolver(operand_context, cpu->fault_address,
                                cpu->fault_size, (uint32_t)cpu->fault_access,
                                executed_code.mapping_epoch,
                                executed_code.instruction_epoch, &view);
      if (result == HL_NATIVE_OPERAND_RESOLVED) {
        resolved_projection =
            (hl_a64_projection){&view, 1, source->mapping_incarnation, 0};
        if (!hl_a64_projection_validate(&resolved_projection) ||
            !hl_a64_projection_resolve(&resolved_projection, cpu,
                                       cpu->fault_address, cpu->fault_size,
                                       (uint32_t)cpu->fault_access))
          return HL_NATIVE_ARGUMENT;
        if (HL_A64_RUN_VIEW_CACHE)
          run_view_install(&operand_views, &view);
        hl_a64_run_view_publish(&operand_views, cpu,
                                source->mapping_incarnation);
        continue;
      }
      ...
      if (result == HL_NATIVE_OPERAND_FAULT)
        return hl_native_execution_exit(
            &execution, output, HL_NATIVE_EXIT_FAULT,
    ...
    if (cpu->reason == HL_NATIVE_EXIT_FAULT)
      return hl_native_execution_exit(&execution, output, HL_NATIVE_EXIT_FAULT,
                                      (uint32_t)cpu->fault_access, cpu->program,
                                      cpu->program, cpu->fault_address, 1);
    /* A dirty-record overflow is the one epoch the generated guard raises; it
     * asks the host to drain the record table, not to give up on this entry. */
    if (cpu->reason != HL_NATIVE_EXIT_SYSCALL &&
        cpu->reason != HL_NATIVE_EXIT_FALLBACK &&
        cpu->reason != HL_NATIVE_EXIT_INTERRUPT &&
        cpu->reason != HL_NATIVE_EXIT_YIELD &&
        cpu->reason != HL_NATIVE_EXIT_EPOCH)
      return run_exit(&execution, output, HL_NATIVE_EXIT_FALLBACK, instruction);
    if (executor->diagnostics && cpu->reason == HL_NATIVE_EXIT_FALLBACK &&
        cpu->fault_access == 0) {
...
static hl_native_status run_inner(hl_native_executor *executor,
                                  hl_native_cpu *cpu,
                                  const hl_native_run_request *request,
                                  hl_native_exit *output) {
  hl_native_execution execution = {0};
  ...
  if (request->size >= offsetof(hl_native_run_request, direct_token) &&
      request->memory_mode != 0) {
    const hl_native_direct_token *token =
        request->size >= offsetof(hl_native_run_request, authority_identity)
            ? request->direct_token
            : NULL;
    uint64_t identity =
        request->size >= offsetof(hl_native_run_request, certificate)
            ? request->authority_identity
            : 0;
    if (!hl_native_direct_request_valid(executor, token,
                                        request->authority_generation, identity,
                                        request->projection)) {
      (void)hl_native_execution_leave(&execution);
...
hl_native_status hl_native_run(hl_native_executor *executor, hl_native_cpu *cpu,
  ...
  if (executor == NULL || cpu == NULL || request == NULL || output == NULL ||
      cpu->abi != HL_NATIVE_ABI || cpu->size < sizeof(*cpu) ||
      cpu->reserved != 0 || request->abi != HL_NATIVE_ABI ||
      request->size < offsetof(hl_native_run_request, source) ||
      request->reserved != 0 || output->abi != HL_NATIVE_ABI ||
