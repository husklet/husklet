...
static void fault_chain(int signal, siginfo_t *information, void *context,
    ...
    if (disposition == (uintptr_t)SIG_IGN) return;
...
static void fault_dispatch(int signal, siginfo_t *information, void *context) {
    uint64_t address = information == NULL ? 0 : (uint64_t)(uintptr_t)information->si_addr;
    uint64_t program = 0;
#if defined(__aarch64__)
    if (context != NULL) program = ((ucontext_t *)context)->uc_mcontext.pc;
#elif defined(__x86_64__)
    if (context != NULL) program = ((ucontext_t *)context)->uc_mcontext.gregs[REG_RIP];
#endif
    if (program != 0 && hl_native_fault_thread_prepare(program, address, context)) return;
    size_t index = signal == SIGBUS ? 1u : 0u;
    fault_chain(signal, information, context, &hl_native_fault_process.prior[index]);
}
...
hl_native_status hl_native_fault_process_acquire(void) {
    ...
    if (hl_native_fault_atfork_status != 0) return HL_NATIVE_PLATFORM;
    if (pthread_mutex_lock(&hl_native_fault_process.lock) != 0) return HL_NATIVE_PLATFORM;
    ...
    for (; count < 2; count++) {
        ...
        installed.sa_flags = SA_SIGINFO | SA_ONSTACK |
            (hl_native_fault_process.prior[count].sa_flags & (SA_NODEFER | SA_RESTART));
...
#else
hl_native_status hl_native_fault_process_acquire(void) { return HL_NATIVE_PLATFORM; }
hl_native_status hl_native_fault_process_release(void) { return HL_NATIVE_PLATFORM; }
#endif
