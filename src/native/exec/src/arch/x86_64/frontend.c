...
static void decode_block(const hl_x86_a64_request *request, decode *block) {
    ...
    while (cursor < request->guest_size && block->count < request->max_instructions) {
        ...
        if (vex != 0u && vex_map == 1u && opcode == 0x5bu && vex_pp <= 2u &&
            ...
            if ((modrm >> 6) == 3u) {
                ...
                item->source = 16u;
                   ...
                   cursor < request->guest_size && request->guest_bytes[cursor] == 0x2au) {
            uint8_t modrm;
            /* F3 produces a single, F2 a double; REX.W picks the integer source width. */
        ...
        } else if ((semantic_prefix == 0xf3u || semantic_prefix == 0xf2u) && opcode == 0x0fu &&
            ...
            if ((modrm >> 6) == 3u) {
                ...
                /* A memory source zeroes the upper bits, so the plain sized load is exact. */
                if (!hl_x86_decode_address(request, block, item, rex, 0, address_32, start, &cursor)) break;
                item->operation = OP_VECTOR;
                item->width = lane;
                item->vector_kind = VECTOR_COPY;
