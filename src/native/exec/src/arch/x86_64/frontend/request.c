...
uint32_t hl_x86_a64_unknown_flags(uint32_t flags) {
  return flags & ~(uint32_t)HL_X86_A64_FLAGS;
}
...
int hl_x86_request_valid(const hl_x86_a64_request *request,
  ...
  return request != NULL && result != NULL &&
         ...
         request->size == sizeof *request && request->guest_bytes != NULL &&
         request->host_words != NULL && request->provenance != NULL &&
         request->guest_size != 0 && request->max_instructions != 0 &&
