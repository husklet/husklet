...
int hl_x86_decode_shift(const hl_x86_a64_request *request, decode *block,
  ...
  if (kind > 3u && (operand_16 != 0u || opcode == 0xc0u || opcode == 0xd0u ||
                    opcode == 0xd2u || (modrm >> 6) != 3u))
    return fail(block, cursor, start,
                operand_16 != 0u ? HL_X86_A64_FLAGS_ABI_REQUIRED
...
static uint32_t shift_word(unsigned destination, unsigned source,
                           unsigned count, unsigned kind, int wide) {
  unsigned bits = wide ? 64u : 32u;
  uint32_t base = kind == 7u
                      ? (wide ? UINT32_C(0x93400000) : UINT32_C(0x13000000))
...
/* Words for the shifted value alone, i.e. what remains once the flag word is
 * elided. */
static uint32_t value_words(const instruction *item) {
  if (item->variable_count)
    return 5u + constant_words(item->width == 8u ? 63u : 31u);
  return item->shift_count == 0u ? 1u : 3u;
}

uint32_t hl_x86_shift_words(const instruction *item) {
  if (item->nzcv_dead && item->pfaf_dead)
    return value_words(item);
  if (item->variable_count)
    return item->shift_kind == 4u ? 46u : 43u;
  if (item->shift_count == 0u)
    return 1u;
  if (item->shift_count != 1u)
    return 27u;
  return item->shift_kind == 4u ? 32u : 30u;
}

static uint32_t variable_shift(unsigned destination, unsigned source,
                               unsigned count, unsigned kind, int wide) {
  uint32_t base = kind == 4u   ? UINT32_C(0x1ac02000)
                  : kind == 5u ? UINT32_C(0x1ac02400)
                               : UINT32_C(0x1ac02800);
  return base | (wide ? UINT32_C(0x80000000) : 0) | count << 16 | source << 5 |
         destination;
}
...
static void emit_variable(uint32_t *words, uint32_t *cursor,
  ...
  words[(*cursor)++] = UINT32_C(0x6a00001f) |
                       (wide ? UINT32_C(0x80000000) : 0) | 18u << 16 | 18u << 5;
...
/* The value half of a shift, emitting the same instructions the full form does
 * for the result. */
static void emit_value_only(uint32_t *words, uint32_t *cursor,
                            const instruction *item) {
  unsigned bits = item->width == 8u ? 64u : 32u;
  int wide = item->width == 8u;
  ...
  if (!item->variable_count) {
    if (item->shift_count == 0u) {
      words[(*cursor)++] =
          move_register(item->destination, item->destination, wide);
      return;
    }
    words[(*cursor)++] = move_register(16, item->destination, wide);
    words[(*cursor)++] =
        shift_word(18, 16, item->shift_count, item->shift_kind, wide);
    words[(*cursor)++] = move_register(item->destination, 18, wide);
    return;
  }
  words[(*cursor)++] = move_register(16, item->destination, wide);
...
void hl_x86_emit_shift(uint32_t *words, uint32_t *cursor,
  ...
  clear = HL_X86_RFLAGS_CF | HL_X86_RFLAGS_PF | HL_X86_RFLAGS_ZF |
          HL_X86_RFLAGS_SF | (count == 1u ? HL_X86_RFLAGS_OF : 0);
  ...
  words[(*cursor)++] = UINT32_C(0xaa000000) | 22u << 16 | 20u << 5 | 20u;
  words[(*cursor)++] =
      hl_x86_bit(22, 16, item->shift_kind == 4u ? bits - count : count - 1u);
  words[(*cursor)++] = UINT32_C(0xaa000000) | 22u << 16 | 20u << 5 | 20u;
  words[(*cursor)++] = move_register(22, 18, 1);
  words[(*cursor)++] =
      UINT32_C(0xca400000) | 22u << 16 | 4u << 10 | 22u << 5 | 22u;
