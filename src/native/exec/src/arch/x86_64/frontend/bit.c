...
int hl_x86_decode_bit(const hl_x86_a64_request *request, decode *block,
  ...
  if (immediate) {
    ...
    item->operand_immediate =
        request->guest_bytes[(*cursor)++] & (uint8_t)(width * 8u - 1u);
  }
  return 1;
}
...
static void publish_carry(uint32_t *words, uint32_t *cursor) {
  ...
  words[(*cursor)++] =
      UINT32_C(0xb3400020) | 21u << 5 | 20u; /* bfi x20,x21,#0,#1 */
...
void hl_x86_emit_bit(uint32_t *words, uint32_t *cursor,
  ...
  /* The memory form addresses the containing byte, so only the register form
     keeps the decoder's already width-masked bit index. */
  if (item->bit_immediate != 0u)
    ...
    emit_constant(words, cursor, 19u,
                  item->memory_operand != 0u ? 7u : item->width * 8u - 1u);
  ...
  if (item->condition != 0u) {
    ...
    if (item->condition == 1u)
      words[(*cursor)++] =
          (wide ? UINT32_C(0xaa000000) : UINT32_C(0x2a000000)) | 22u << 16 |
          old << 5 | 20u;
    else if (item->condition == 2u)
      words[(*cursor)++] =
          (wide ? UINT32_C(0x8a200000) : UINT32_C(0x0a200000)) | 22u << 16 |
          old << 5 | 20u;
    else
      words[(*cursor)++] =
          (wide ? UINT32_C(0xca000000) : UINT32_C(0x4a000000)) | 22u << 16 |
          old << 5 | 20u;
    if (item->memory_operand != 0u) {
      memory = *item;
      ...
      words[(*cursor)++] =
          (wide ? UINT32_C(0xaa0003e0) : UINT32_C(0x2a0003e0)) | 20u << 16 |
          item->destination;
    }
  }
