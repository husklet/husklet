...
static void load_target(uint32_t *words, uint32_t *cursor,
                        const instruction *item, int effective) {
  ...
  below = &words[(*cursor)++];
  words[(*cursor)++] = UINT32_C(0xb1002212);
  overflow = &words[(*cursor)++]; /* adds x18,x16,#8 */
  words[(*cursor)++] =
...
uint32_t hl_x86_control_words(const instruction *item) {
  if (item->operation == OP_PUSH) {
    instruction store = {.pc = item->pc,
                         .immediate = (uint64_t)(-(int64_t)item->width),
                         .operation = OP_STORE,
                         .source = item->has_immediate ? 20u : item->source,
                         .width = item->width,
                         .address_base = 4u,
                         .address_index = UINT8_MAX,
                         .live_chain = item->live_chain};
    return hl_x86_store_words(&store) +
           (item->has_immediate ? constant_words(item->immediate) : 0u) + 2u;
  }
  if (item->operation == OP_POP) {
    instruction load = {.pc = item->pc,
                        .operation = OP_LOAD,
                        .destination = 21u,
                        .width = item->width,
                        .address_base = 4u,
  ...
  return 130u + hl_x86_read_cache_words(8u) +
         (item->live_chain != 0u ? 19u : 0u) +
         (item->source_high != 0u ? hl_x86_address_words(item) : 0u) +
         constant_words(item->pc) + constant_words(item->pc + item->length) +
         constant_words(item->immediate);
}
...
void hl_x86_emit_control(uint32_t *words, uint32_t *cursor,
  ...
  if (item->operation == OP_PUSH) {
    ...
    words[(*cursor)++] = UINT32_C(0xd1000084) |
                         (uint32_t)item->width << 10; /* sub x4,x4,#width */
    return;
  }
  if (item->operation == OP_POP) {
    instruction load = {.pc = item->pc,
    ...
    words[(*cursor)++] = UINT32_C(0x91000084) |
                         (uint32_t)item->width << 10; /* add x4,x4,#width */
    if (item->width == 2u) {
  ...
  if (item->operation == OP_RETURN) {
    ...
    words[(*cursor)++] = store_word(
        4, offsetof(hl_native_x86_64_cpu, registers) + 4u * sizeof(uint64_t));
    emit_constant(words, cursor, 18, item->pc);
    words[(*cursor)++] =
        store_word(18, offsetof(hl_native_x86_64_cpu, indirect_site));
    words[(*cursor)++] =
        store_word(21, offsetof(hl_native_x86_64_cpu, program));
    return;
  }
  ...
  if (item->has_immediate != 0u)
    ...
    words[(*cursor)++] = UINT32_C(0xaa0003f5) | (uint32_t)item->source
                                                    << 16; /* mov x21,xN */
  ...
  instruction push = {.pc = item->pc,
                      .immediate = UINT64_MAX - 7u,
                      .operation = OP_STORE,
                      .source = 20u,
                      .width = 8u,
                      .address_base = 4u,
  ...
  words[(*cursor)++] = store_word(4, offsetof(hl_native_x86_64_cpu, registers) +
                                         4u * sizeof(uint64_t));
