...
static void zero_test(uint32_t *instruction, uint32_t *target,
                      unsigned source) {
  int64_t distance = target - instruction;
  *instruction = UINT32_C(0xb4000000) |
                 ((uint32_t)distance & UINT32_C(0x7ffff)) << 5 | source;
}
...
/* Diagnostic counters for the write path. x17 is scratch at every site below
 * and the sequence is absent entirely when diagnostics are off, so a production
 * block is word-for-word what it was. Sizing reads the same flag. */
static _Thread_local int write_diagnostics;

void hl_x86_set_write_diagnostics(int enabled) {
  write_diagnostics = enabled != 0;
}

static void diagnostic_increment(uint32_t *words, uint32_t *cursor,
                                 size_t field) {
...
_Static_assert(offsetof(hl_native_x86_64_cpu, dirty_count) - HL_X86_WINDOW <=
                   504u,
...
static uint32_t window_load(unsigned destination, size_t field) {
  return UINT32_C(0xf9400000) | window_word(field) << 10 |
         HL_X86_WINDOW_BASE << 5 | destination;
}

static uint32_t window_store(unsigned source, size_t field) {
  return UINT32_C(0xf9000000) | window_word(field) << 10 |
         HL_X86_WINDOW_BASE << 5 | source;
}

static uint32_t pair_load(unsigned first, unsigned second, unsigned base,
                          uint32_t slot) {
  return UINT32_C(0xa9400000) | (slot & UINT32_C(0x7f)) << 15 | second << 10 |
         base << 5 | first;
}

static uint32_t pair_store(unsigned first, unsigned second, unsigned base,
                           uint32_t slot) {
  return UINT32_C(0xa9000000) | (slot & UINT32_C(0x7f)) << 15 | second << 10 |
         base << 5 | first;
}

/* The entry trampolines bound the published count against this capacity as a
 * literal, so it must keep tracking the record. */
_Static_assert(HL_X86_READ_VIEWS ==
                   sizeof(((hl_native_x86_64_cpu *)0)->read_views) /
                       sizeof(((hl_native_x86_64_cpu *)0)->read_views[0]),
               "x86 entry trampolines validate read_count against a literal "
               "capacity of four");

static void jump(uint32_t *instruction, uint32_t *target) {
  int64_t distance = target - instruction;
  *instruction =
      UINT32_C(0x14000000) | ((uint32_t)distance & UINT32_C(0x03ffffff));
}
...
void hl_x86_emit_read_cache(uint32_t *words, uint32_t *cursor, unsigned width,
  ...
  words[(*cursor)++] = UINT32_C(0xb1000000) | width << 10 | 16u << 5 |
                       18u; /* adds x18,x16,#width */
  overflow = &words[(*cursor)++];
  words[(*cursor)++] = UINT32_C(0x91000393) |
                       (uint32_t)offsetof(hl_native_x86_64_cpu, read_views)
                           << 10;            /* x19 = views */
  words[(*cursor)++] = UINT32_C(0xaa1d03f6); /* x22 = validated count */
  uint32_t *loop = &words[*cursor];
  empty = &words[(*cursor)++];
  words[(*cursor)++] =
      pair_load(20, 17, 19, 0); /* ldp x20,x17,[x19]: view first,last */
  words[(*cursor)++] = UINT32_C(0xeb14021f); /* cmp x16,x20 */
  below = &words[(*cursor)++];
  words[(*cursor)++] = UINT32_C(0xeb11025f); /* cmp x18,x17 */
  above = &words[(*cursor)++];
  words[(*cursor)++] =
      pair_load(20, 17, 19, 2); /* ldp x20,x17,[x19,#16]: delta,permissions */
  permission = &words[(*cursor)++];
  words[(*cursor)++] = UINT32_C(0x8b140211); /* add x17,x16,x20 */
  words[(*cursor)++] = vector ? ((width == 4u   ? UINT32_C(0xbd400220)
                                  : width == 8u ? UINT32_C(0xfd400220)
                                                : UINT32_C(0x3dc00220)) |
                                 destination)
                              : ((width == 1u   ? UINT32_C(0x39400220)
                                  : width == 2u ? UINT32_C(0x79400220)
                                  : width == 4u ? UINT32_C(0xb9400220)
                                                : UINT32_C(0xf9400220)) |
                                 destination);
  if (vector && width == 32u)
    words[(*cursor)++] =
        UINT32_C(0x3dc00620) | (destination + 1u); /* ldr qD+1,[x17,#16] */
...
void hl_x86_emit_vector_upper_load(uint32_t *words, uint32_t *cursor,
  ...
  words[(*cursor)++] =
      UINT32_C(0x91000000) | (uint32_t)offset << 10 | 28u << 5 | 20u;
...
void hl_x86_emit_vector_upper_zero(uint32_t *words, uint32_t *cursor,
                                   unsigned guest) {
  size_t offset = offsetof(hl_native_x86_64_cpu, vector_upper) + guest * 16u;
...
void hl_x86_patch_read_hit(uint32_t *hit, uint32_t *target) {
  jump(hit, target);
}
...
static void emit_write_cache(uint32_t *words, uint32_t *cursor, unsigned width,
  ...
  words[(*cursor)++] =
      pair_load(20, 21, 19, 0); /* ldp x20,x21,[x19]: view first,last */
  ...
  words[(*cursor)++] = UINT32_C(0x91000000) | (uint32_t)HL_X86_WINDOW << 10 |
                       28u << 5 | HL_X86_WINDOW_BASE;
  words[(*cursor)++] =
      pair_load(17, 19, HL_X86_WINDOW_BASE,
  ...
  words[(*cursor)++] =
      window_load(17, offsetof(hl_native_x86_64_cpu, dirty_first));
  words[(*cursor)++] =
      UINT32_C(0xb100063f); /* cmn first,#1: UINT64_MAX means empty */
  uint32_t *empty = &words[(*cursor)++];
  words[(*cursor)++] =
      window_load(21, offsetof(hl_native_x86_64_cpu, dirty_count));
  words[(*cursor)++] = UINT32_C(0x91000013) |
                       (uint32_t)offsetof(hl_native_x86_64_cpu, dirty_records)
                           << 10 |
  ...
  words[(*cursor)++] =
      window_load(20, offsetof(hl_native_x86_64_cpu, dirty_view_first));
  words[(*cursor)++] = UINT32_C(0xeb14023f); /* cmp x17,x20 */
  uint32_t *next_owner_first = &words[(*cursor)++];
  words[(*cursor)++] = UINT32_C(0xf9400671); /* ldr x17,[x19,#8] */
  words[(*cursor)++] =
      window_load(20, offsetof(hl_native_x86_64_cpu, dirty_view_last));
  words[(*cursor)++] = UINT32_C(0xeb14023f); /* cmp x17,x20 */
  uint32_t *next_owner_last = &words[(*cursor)++];
  words[(*cursor)++] =
      UINT32_C(0xf9400a71); /* ldr x17,[x19,#16]: recorded first */
  words[(*cursor)++] =
      window_load(20, offsetof(hl_native_x86_64_cpu, dirty_last));
  words[(*cursor)++] =
      UINT32_C(0xeb14023f); /* cmp recorded first,current last */
  uint32_t *next_above = &words[(*cursor)++];
  words[(*cursor)++] =
      UINT32_C(0xf9400e71); /* ldr x17,[x19,#24]: recorded last */
  words[(*cursor)++] =
      window_load(20, offsetof(hl_native_x86_64_cpu, dirty_first));
  words[(*cursor)++] =
      UINT32_C(0xeb14023f); /* cmp recorded last,current first */
  uint32_t *next_below = &words[(*cursor)++];
  words[(*cursor)++] = UINT32_C(0xf9400a71); /* recorded first */
  words[(*cursor)++] =
      UINT32_C(0xeb11029f); /* cmp current first,recorded first */
  ...
  words[(*cursor)++] =
      window_load(20, offsetof(hl_native_x86_64_cpu, dirty_last));
  words[(*cursor)++] =
      UINT32_C(0xeb11029f); /* cmp current last,recorded last */
  uint32_t *keep_last = &words[(*cursor)++];
  words[(*cursor)++] = UINT32_C(0xf9000e74); /* str current last,[x19,#24] */
  uint32_t *merged = &words[*cursor];
  diagnostic_increment(words, cursor,
                       offsetof(hl_native_x86_64_cpu, diagnostic_dirty_merged));
  words[(*cursor)++] = ones_word(20);
  words[(*cursor)++] =
      pair_store(20, 31, HL_X86_WINDOW_BASE,
  ...
  diagnostic_increment(
      words, cursor,
      offsetof(hl_native_x86_64_cpu, diagnostic_dirty_committed));
  words[(*cursor)++] =
      window_load(17, offsetof(hl_native_x86_64_cpu, dirty_count));
  ...
  words[(*cursor)++] =
      pair_load(20, 21, HL_X86_WINDOW_BASE,
  ...
  words[(*cursor)++] = UINT32_C(0x91000631); /* add x17,x17,#1 */
  words[(*cursor)++] =
      window_store(17, offsetof(hl_native_x86_64_cpu, dirty_count));
  words[(*cursor)++] = ones_word(20);
  words[(*cursor)++] =
      pair_store(20, 31, HL_X86_WINDOW_BASE,
                 window_word(offsetof(hl_native_x86_64_cpu, dirty_first)));
  ...
  words[(*cursor)++] =
      pair_store(20, 21, HL_X86_WINDOW_BASE,
                 window_word(offsetof(hl_native_x86_64_cpu, memory_first)));
  ...
  diagnostic_increment(
      words, cursor,
      offsetof(hl_native_x86_64_cpu, diagnostic_write_cache_hit));
  if (hit != NULL) {
    words[(*cursor)++] = UINT32_C(0xa9415bf5);
    words[(*cursor)++] = UINT32_C(0xa8c253f3);
    diagnostic_increment(words, cursor,
                         offsetof(hl_native_x86_64_cpu, diagnostic_guard_fast));
    words[(*cursor)++] = UINT32_C(0x91000000) | width << 10 | 16u << 5 |
                         18u; /* add x18,x16,#width */
    *hit = &words[(*cursor)++];
  }
  ...
  diagnostic_increment(
      words, cursor, offsetof(hl_native_x86_64_cpu, diagnostic_dirty_overflow));
  uint32_t *miss_target = &words[*cursor];
  diagnostic_increment(
      words, cursor,
      offsetof(hl_native_x86_64_cpu, diagnostic_write_cache_miss));
  uint32_t *active = &words[*cursor];
  words[(*cursor)++] = UINT32_C(0xa9415bf5);
  words[(*cursor)++] = UINT32_C(0xa8c253f3);
...
void hl_x86_emit_dirty(uint32_t *words, uint32_t *cursor) {
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, dirty_first));
  words[(*cursor)++] = UINT32_C(0xeb11021f); /* cmp x16,x17 */
  words[(*cursor)++] = UINT32_C(0x9a913211); /* csel x17,x16,x17,lo */
  words[(*cursor)++] =
      store_word(17, offsetof(hl_native_x86_64_cpu, dirty_first));
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, dirty_last));
  words[(*cursor)++] = UINT32_C(0xeb11025f); /* cmp x18,x17 */
  words[(*cursor)++] = UINT32_C(0x9a918251); /* csel x17,x18,x17,hi */
  words[(*cursor)++] =
      store_word(17, offsetof(hl_native_x86_64_cpu, dirty_last));
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, memory_first));
  words[(*cursor)++] =
      store_word(17, offsetof(hl_native_x86_64_cpu, dirty_view_first));
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, memory_last));
  words[(*cursor)++] =
      store_word(17, offsetof(hl_native_x86_64_cpu, dirty_view_last));
}

uint32_t hl_x86_load_words(const instruction *item) {
  uint32_t access_width =
      item->load_width != 0u ? item->load_width : item->width;
  uint32_t words = hl_x86_address_words(item) - 1u + 23u +
                   (item->live_chain != 0u ? 19u : 0u) + item->memory_write +
                   constant_words(access_width) + constant_words(item->pc);
  if (item->memory_write == 0u)
    words += hl_x86_read_cache_words(access_width);
  else
    words += HL_X86_WRITE_CACHE_WORDS + cache_diagnostic_words(0);

  if (item->load_width != 0u && item->signed_extend != 0u && item->width <= 2u)
    ++words;

  return item->conditional != 0u ? words - 1u + hl_x86_cmov_words(item) : words;
}

void hl_x86_emit_load(uint32_t *words, uint32_t *cursor,
  ...
  below = &words[(*cursor)++];
  words[(*cursor)++] =
      UINT32_C(0xb1000000) | access_width << 10 | 16u << 5 | 18u;
  overflow = &words[(*cursor)++];
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, memory_last));
  ...
  above = &words[(*cursor)++];
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, memory_permissions));
  permission = &words[(*cursor)++];
  if (item->memory_write != 0u)
    permission_write = &words[(*cursor)++];
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, memory_delta));
  words[(*cursor)++] = UINT32_C(0x8b110211);
  words[(*cursor)++] = access_width == 1u   ? UINT32_C(0x39400232)
                       : access_width == 2u ? UINT32_C(0x79400232)
                       : access_width == 4u ? UINT32_C(0xb9400232)
                                            : UINT32_C(0xf9400232);
  skip = &words[(*cursor)++];
  ...
  if (permission_write != NULL)
    test(permission_write, miss, 1u);
  words[(*cursor)++] =
      store_word(16, offsetof(hl_native_x86_64_cpu, fault_address));
  words[(*cursor)++] =
      item->memory_write != 0u ? UINT32_C(0xd2800071) : UINT32_C(0xd2800031);
  words[(*cursor)++] =
      store_word(17, offsetof(hl_native_x86_64_cpu, fault_access));
  emit_constant(words, cursor, 17, access_width);
  words[(*cursor)++] =
      store_word(17, offsetof(hl_native_x86_64_cpu, fault_size));
  ...
  if (item->conditional != 0u)
    ...
    if (item->signed_extend != 0u) {
      ...
      words[(*cursor)++] =
          (item->width == 8u ? UINT32_C(0x93400000) : UINT32_C(0x13000000)) |
    ...
    if (item->width == 1u)
      words[(*cursor)++] =
          (item->destination_high != 0u ? UINT32_C(0xb3781c00)
                                        : UINT32_C(0xb3401c00)) |
          source << 5 | item->destination;
    else if (item->width == 2u)
      words[(*cursor)++] =
          UINT32_C(0xb3403c00) | source << 5 | item->destination;
    else if (item->signed_extend == 0u)
      words[(*cursor)++] =
          (item->width == 8u ? UINT32_C(0xaa0003e0) : UINT32_C(0x2a0003e0)) |
...
uint32_t hl_x86_store_words(const instruction *item) {
  return hl_x86_address_words(item) - 1u + (item->source_high == 8u ? 1u : 0u) +
         ...
         (write_diagnostics ? 6u : 0u) + (item->live_chain != 0u ? 19u : 0u) +
         constant_words(item->pc) +
         (item->has_immediate != 0u ? constant_words(item->operand_immediate)
                                    : 0u);
}

void hl_x86_emit_store(uint32_t *words, uint32_t *cursor,
                       const instruction *item) {
  uint32_t *below;
  uint32_t *overflow;
  uint32_t *above;
  ...
  hl_x86_emit_address(words, cursor, item);
  --*cursor;
  /* x18 carries the end-of-access address through the bounds checks below, so
   * the high-byte source must land in x20 instead.  Both forms of that value
   * are materialised before the cache, whose hit exit branches past the guard,
   * and survive it in the temporaries the cache spills and restores. */
  if (item->has_immediate != 0u)
    emit_constant(words, cursor, 20u, item->operand_immediate);
  if (item->source_high == 8u) {
    words[(*cursor)++] =
        UINT32_C(0x53083c00) | source << 5 | 20u; /* ubfx w20,wS,#8,#8 */
    source = 20u;
  }
  emit_write_cache(words, cursor, item->width, &cache_hit);
  diagnostic_increment(words, cursor,
                       offsetof(hl_native_x86_64_cpu, diagnostic_guard_full));
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, memory_first));
  words[(*cursor)++] = UINT32_C(0xeb11021f);
  below = &words[(*cursor)++];
  words[(*cursor)++] =
      UINT32_C(0xb1000000) | (uint32_t)item->width << 10 | 16u << 5 | 18u;
  overflow = &words[(*cursor)++];
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, memory_last));
  words[(*cursor)++] = UINT32_C(0xeb11025f);
  ...
  words[(*cursor)++] = (item->width == 1u   ? UINT32_C(0x39000220)
                        ...
                        : item->width == 4u ? UINT32_C(0xb9000220)
                                            : UINT32_C(0xf9000220)) |
                       source;
  words[(*cursor)++] = UINT32_C(0xd2800031);
  words[(*cursor)++] =
      store_word(17, offsetof(hl_native_x86_64_cpu, memory_written));
  hl_x86_emit_dirty(words, cursor);
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, memory_permissions));
  words[(*cursor)++] =
      load_word(18, offsetof(hl_native_x86_64_cpu, executable_written));
  words[(*cursor)++] =
      UINT32_C(0xaa120231); /* orr x17,x17,x18: sticky permission latch */
  words[(*cursor)++] =
      store_word(17, offsetof(hl_native_x86_64_cpu, executable_written));
  skip = &words[(*cursor)++];
  uint32_t *miss = &words[*cursor];
  branch(below, miss, 3u);
  branch(overflow, miss, 2u);
  ...
  diagnostic_increment(
      words, cursor, offsetof(hl_native_x86_64_cpu, diagnostic_guard_fallback));
  words[(*cursor)++] =
      store_word(16, offsetof(hl_native_x86_64_cpu, fault_address));
...
uint32_t hl_x86_xchg_words(const instruction *item) {
  return hl_x86_address_words(item) - 1u + HL_X86_WRITE_CACHE_WORDS +
         cache_diagnostic_words(0) + 60u + constant_words(item->width) +
...
void hl_x86_emit_exchange(uint32_t *words, uint32_t *cursor,
                          const instruction *item) {
...
void hl_x86_emit_xchg(uint32_t *words, uint32_t *cursor,
                      const instruction *item) {
  ...
  if (item->width != 1u) {
    ...
    words[(*cursor)++] =
        UINT32_C(0xea00001f) | 19u << 16 | 16u << 5; /* tst x16,x19 */
  ...
  words[(*cursor)++] = UINT32_C(0x8b110211); /* add x17,x16,x17 */
  {
    uint32_t size = item->width == 8u   ? UINT32_C(0xc0000000)
                    ...
                    : item->width == 2u ? UINT32_C(0x40000000)
                                        : 0u;
  ...
  if (item->width == 1u)
    words[(*cursor)++] =
        (item->destination_high ? UINT32_C(0xb3781c00) : UINT32_C(0xb3401c00)) |
        18u << 5 | item->destination;
  else if (item->width == 2u)
    words[(*cursor)++] = UINT32_C(0xb3403c00) | 18u << 5 | item->destination;
  else
  ...
  words[(*cursor)++] =
      UINT32_C(0x91000000) | (uint32_t)item->width << 10 | 16u << 5 | 18u;
  hl_x86_emit_dirty(words, cursor);
  words[(*cursor)++] =
      load_word(17, offsetof(hl_native_x86_64_cpu, memory_permissions));
  words[(*cursor)++] =
  ...
  branch(below, fault, 3u);
  branch(overflow, fault, 2u);
  branch(above, fault, 8u);
  test(readable, fault, 0u);
  test(writable, fault, 1u);
  words[(*cursor)++] =
      store_word(16, offsetof(hl_native_x86_64_cpu, fault_address));
  ...
  if (unaligned != NULL)
    branch(unaligned, fallback, 1u); /* b.ne */
  emit_constant(words, cursor, 17u, item->pc);
  words[(*cursor)++] = store_word(17, offsetof(hl_native_x86_64_cpu, program));
...
int hl_x86_decode_xadd(const hl_x86_a64_request *request, decode *block,
                       instruction *item, uint8_t opcode, uint8_t rex,
                       uint8_t operand_16, uint8_t address_32, uint8_t lock,
  ...
  if ((modrm >> 6) == 3u) {
    ...
    ++*cursor;
    item->destination = (uint8_t)(rm | ((rex & 1u) << 3));
    if (opcode == 0xc0u && rex == 0u && rm >= 4u) {
      item->destination = (uint8_t)(rm - 4u);
  ...
  if (!hl_x86_decode_address(request, block, item, rex, 0, address_32, start,
                             cursor))
    return 0;
  item->operation = OP_XADD;
...
void hl_x86_emit_xadd(uint32_t *words, uint32_t *cursor,
                      const instruction *item) {
  instruction add = *item;
  if (!item->memory_operand) {
    words[(*cursor)++] = item->width == 1u
                             ? (item->destination_high ? UINT32_C(0xd3483c00)
                                                       : UINT32_C(0xd3401c00)) |
                                   item->destination << 5 | 19u
                             : (item->width == 8u ? UINT32_C(0xaa0003f3)
                                                  : UINT32_C(0x2a0003f3)) |
                                   item->destination << 16;
    words[(*cursor)++] = item->width == 1u
                             ? (item->source_high ? UINT32_C(0xd3483c00)
                                                  : UINT32_C(0xd3401c00)) |
                                   item->source << 5 | 21u
                             : (item->width == 8u ? UINT32_C(0xaa0003f5)
                                                  : UINT32_C(0x2a0003f5)) |
                                   item->source << 16;
    words[(*cursor)++] = UINT32_C(0xaa1303f9); /* preserve pre-image in x25 */
    add.destination = 19u;
    add.source = 21u;
    add.destination_high = 0u;
    add.source_high = 0u;
    add.memory_operand = 0u;
    add.memory_write = 0u;
    add.alu_kind = 0u;
    add.flags_only = 1u;
    ...
    if (item->width == 1u)
      words[(*cursor)++] =
          (item->source_high ? UINT32_C(0xb3781c00) : UINT32_C(0xb3401c00)) |
          25u << 5 | item->source;
    else if (item->width == 2u)
      ...
      words[(*cursor)++] =
          ...
          25u << 16 | item->source;
    if (item->width == 1u)
      words[(*cursor)++] = (item->destination_high ? UINT32_C(0xb3781c00)
                                                   : UINT32_C(0xb3401c00)) |
                           18u << 5 | item->destination;
    else if (item->width == 2u)
      words[(*cursor)++] = UINT32_C(0xb3403c00) | 18u << 5 | item->destination;
  ...
  }
  {
    ...
    words[(*cursor)++] =
        load_word(17, offsetof(hl_native_x86_64_cpu, memory_delta));
    words[(*cursor)++] = UINT32_C(0x8b110211);
    words[(*cursor)++] = UINT32_C(
        0xaa1103f8); /* preserve admitted host EA across flag lowering */
    {
      uint32_t size = item->width == 8u   ? UINT32_C(0xc0000000)
                      : item->width == 4u ? UINT32_C(0x80000000)
                      : item->width == 2u ? UINT32_C(0x40000000)
      ...
      /* SUB adds the two's complement and AND clears the inverted mask; x21
       * keeps the original operand so the flags below stay the exact x86
       * result. */
      if (kind == 5u) {
        words[(*cursor)++] =
            (item->width == 8u ? UINT32_C(0xcb0003e0) : UINT32_C(0x4b0003e0)) |
            source << 16 | 22u;
        source = 22u;
      } else if (kind == 4u) {
        words[(*cursor)++] =
            (item->width == 8u ? UINT32_C(0xaa2003e0) : UINT32_C(0x2a2003e0)) |
            source << 16 | 22u;
        source = 22u;
        opc = UINT32_C(0x1000); /* LDCLRAL */
      } else if (kind == 1u)
        opc = UINT32_C(0x3000); /* LDSETAL */
      else if (kind == 6u)
        opc = UINT32_C(0x2000); /* LDEORAL */
      words[(*cursor)++] =
          UINT32_C(0x38e00000) | size | opc | source << 16 | 17u << 5 | 19u;
    }
    add.destination = 19u;
    add.source = 21u;
    add.destination_high = 0u;
    ...
    words[(*cursor)++] =
        UINT32_C(0x91000012) | (uint32_t)item->width << 10 | 16u << 5;
    hl_x86_emit_dirty(words, cursor);
    words[(*cursor)++] =
        load_word(17, offsetof(hl_native_x86_64_cpu, memory_permissions));
    words[(*cursor)++] =
    ...
    done = &words[(*cursor)++];
    {
      ...
      emit_constant(words, cursor, 17u, item->pc);
      words[(*cursor)++] =
          store_word(17, offsetof(hl_native_x86_64_cpu, program));
      words[(*cursor)++] = UINT32_C(0xd2800071);
      words[(*cursor)++] =
          store_word(17, offsetof(hl_native_x86_64_cpu, reason));
      if (item->live_chain) {
        hl_x86_finish_chain(words, cursor);
        hl_x86_spill_all(words, cursor);
      }
 ...
 * the accumulator. */
void hl_x86_emit_cmpxchg(uint32_t *words, uint32_t *cursor,
                         const instruction *item) {
  uint32_t *below, *overflow, *above, *readable, *writable, *unaligned, *done,
      *published, *matched;
  unsigned source = item->source;
  ...
  words[(*cursor)++] = UINT32_C(0x8b110211);
  words[(*cursor)++] =
      UINT32_C(0xaa1003f8); /* preserve validated guest EA in x24 */
  ...
  if (item->width == 1u)
    ...
    words[(*cursor)++] =
        (item->width == 8u ? UINT32_C(0xaa0003f4) : UINT32_C(0x2a0003f4));
  {
    uint32_t base = item->width == 8u   ? UINT32_C(0xc8e0fc00)
                    : item->width == 4u ? UINT32_C(0x88e0fc00)
                    : item->width == 2u ? UINT32_C(0x48e0fc00)
                                        : UINT32_C(0x08e0fc00);
    words[(*cursor)++] =
        base | 20u << 16 | 17u << 5 | 21u; /* casal expected,new,[host] */
  }
  words[(*cursor)++] =
      UINT32_C(0xaa1403f9); /* mov x25,x20: returned old value */
  compare.memory_operand = 0u;
  compare.memory_write = 0u;
  compare.destination = 0u;
  compare.destination_high = 0u;
  compare.source = 25u;
  compare.source_high = 0u;
  compare.has_immediate = 0u;
  compare.alu_kind = 7u;
  compare.flags_only = 1u;
  compare.preserve_carry = 0u;
  compare.preserve_flags = 0u;
  compare.unary_neg = 0u;
  ...
  if (item->width == 1u)
    ...
    words[(*cursor)++] =
        UINT32_C(0xb3403f20) | 25u << 5; /* bfi x0,x25,#0,#16 */
  else
    words[(*cursor)++] =
        (item->width == 8u ? UINT32_C(0xaa0003e0) : UINT32_C(0x2a0003e0)) |
        25u << 16;
...
static void trunc_scalar_constants(const instruction *item, uint64_t *threshold,
                                   uint64_t *indefinite) {
  *threshold =
      item->vector_lane == 4u
          ? (item->width == 8u ? UINT64_C(0x5f000000) : UINT64_C(0x4f000000))
          : (item->width == 8u ? UINT64_C(0x43e0000000000000)
                               : UINT64_C(0x41e0000000000000));
  *indefinite =
      item->width == 8u ? UINT64_C(0x8000000000000000) : UINT64_C(0x80000000);
}

static uint32_t vector_operation_words(const instruction *item) {
  switch (item->vector_kind) {
  case VECTOR_BYTE_MASK:
    return 7u;
  ...
  case VECTOR_SCALAR_COMPARE_DOUBLE:
    return 13u;
  ...
  case VECTOR_SHUFFLE_FLOAT:
    return 5u;
  case VECTOR_SHUFFLE_WORD:
    return 6u;
  case VECTOR_SHUFFLE_DOUBLE:
    return 3u;
  ...
  case VECTOR_BLEND_VARIABLE:
    return 3u;
  case VECTOR_MULTIPLY_EVEN_SIGNED_DWORD:
    return 3u;
  case VECTOR_HORIZONTAL_SUBTRACT:
  case VECTOR_HORIZONTAL_SATURATING:
    return 3u;
  case VECTOR_SIGN:
    return 6u;
  case VECTOR_ABSOLUTE:
    return 1u;
  case VECTOR_SIGNED_DWORD_TO_FLOAT:
    return 1u;
  case VECTOR_MERGE_FROM_INTEGER:
    return item->memory_operand != 0u ? 3u : 2u;
  case VECTOR_AES_ENCRYPT:
    return 5u;
  case VECTOR_AES_ENCRYPT_LAST:
    return 4u;
  case VECTOR_PACK_SIGNED:
  case VECTOR_PACK_UNSIGNED:
    return 3u;
  ...
  case VECTOR_SHUFFLE_BYTE:
    return 3u;
  case VECTOR_SHIFT_IMMEDIATE:
    return 1u;
  case VECTOR_INSERT_WORD:
    return 1u;
  case VECTOR_MULTIPLY_HIGH_WORD:
  case VECTOR_MULTIPLY_EVEN_DWORD:
    return 3u;
  case VECTOR_SUM_ABSOLUTE_DIFFERENCES_BYTE:
    return 4u;
  default:
    return 1u;
  }
...
/* The address decoder overwrites item->width with the GPR width, so the VEX
 * lane count of a memory form only survives in vector_memory_width. */
static uint32_t vex_vector_width(const instruction *item) {
  return item->vector_memory_width != 0u ? item->vector_memory_width
                                         : item->width;
}
...
uint32_t hl_x86_vector_words(const instruction *item) {
  ...
  if (item->vector_vex != 0u) {
    ...
    uint32_t two_source =
        item->vector_kind == VECTOR_SHUFFLE_FLOAT ||
    ...
    if (vex_width == 32u && item->vector_kind == VECTOR_BLEND_IMMEDIATE &&
      ...
      upper.vector_immediate =
          (uint8_t)(item->vector_immediate >> (16u / item->vector_lane));
      upper_operation = vector_operation_words(&upper);
    }
    operation += vex_width == 32u ? upper_operation + 2u +
                                        (item->memory_operand == 0u ? 2u : 0u) +
                                        (two_source != 0u ? 2u : 0u)
                                  : 2u;
    if (vex_width == 32u && item->vector_kind == VECTOR_BLEND_VARIABLE)
      operation += 2u;
  }
  uint32_t width =
      item->vector_memory_width != 0u ? item->vector_memory_width : item->width;
  uint32_t aligned =
      item->vector_aligned == 0u
          ? 0u
          : 6u + constant_words(item->pc) + (item->live_chain != 0u ? 19u : 0u);
  if (item->memory_operand == 0u)
    return operation;
  uint32_t words =
      hl_x86_address_words(item) - 1u +
      (item->memory_write != 0u
           ? 42u + HL_X86_WRITE_CACHE_WORDS + cache_diagnostic_words(0)
           : 24u) +
      (item->memory_write == 0u ? hl_x86_read_cache_words(width) : 0u) +
      (item->live_chain != 0u ? 19u : 0u) + constant_words(item->pc) +
      (item->vector_kind != VECTOR_COPY && item->memory_write == 0u ? operation
                                                                    : 0u) +
      aligned +
      /* Only VECTOR_COPY reassembles the loaded upper half; every other kind
       * just rescues it out of the read cache's v17 for the VEX completion. */
      (width != 32u                       ? 0u
       : item->memory_write != 0u         ? 3u
       : item->vector_kind == VECTOR_COPY ? 4u
       : item->vector_vex != 0u           ? 2u
                                          : 1u);
  return words;
}

static void emit_vector_operation(uint32_t *words, uint32_t *cursor,
                                  const instruction *item) {
  uint32_t destination = item->destination;
  uint32_t source = item->source;
  uint32_t first =
      item->vector_vex != 0u ? item->vector_source_one : destination;
...
#define EMIT_MASK(vector, target)                                              \
  do {                                                                         \
    ...
    words[(*cursor)++] =                                                       \
        UINT32_C(0x0e003c00) | 17u << 16 | 17u << 5 | (target);                \
    words[(*cursor)++] = UINT32_C(0x2a000000) | (target) << 16 | 8u << 10 |    \
                         16u << 5 | (target);                                  \
  } while (0)
    if (equal_any == 0u)
      EMIT_MASK(18u, 19u);
    ...
    if (equal_any != 0u) {
      ...
      words[(*cursor)++] =
          UINT32_C(0x4e201c00) | 23u << 16 | destination << 5 | 24u;
      words[(*cursor)++] = UINT32_C(0x6e201c00) | 25u << 16 | 25u << 5 | 25u;
      for (lane_index = 0; lane_index < 16u; ++lane_index) {
        words[(*cursor)++] = UINT32_C(0x4e000400) |
                             ((lane_index << 1) | 1u) << 16 | 24u << 5 | 26u;
        words[(*cursor)++] =
            UINT32_C(0x6e208c00) | 26u << 16 | source << 5 | 26u;
...
#undef EMIT_MASK
    ...
    if ((immediate & 0x40u) == 0u) {
      ...
      words[(*cursor)++] =
          UINT32_C(0x1a800000) | 16u << 16 | 0u << 12 | 25u << 5 | 1u;
    }
    ...
    words[(*cursor)++] =
        UINT32_C(0x2a000000) | 25u << 16 | 7u << 10 | 20u << 5 | 20u;
    words[(*cursor)++] = UINT32_C(0x710042ff); /* cmp w23,#16 */
    words[(*cursor)++] = UINT32_C(0x1a9f27f9); /* cset w25,lo */
    words[(*cursor)++] =
        UINT32_C(0x2a000000) | 25u << 16 | 6u << 10 | 20u << 5 | 20u;
    words[(*cursor)++] = UINT32_C(0x12000239); /* and w25,w17,#1 */
    words[(*cursor)++] =
        UINT32_C(0x2a000000) | 25u << 16 | 11u << 10 | 20u << 5 | 20u;
    ...
    uint32_t base =
        item->vector_kind == VECTOR_SCALAR_SQRT_DOUBLE  ? UINT32_C(0x1e61c000)
    ...
    words[(*cursor)++] =
        UINT32_C(0x1e602000) | 18u << 16 | 18u << 5; /* fcmp d18,d18 */
    uint32_t ordered = (*cursor)++;
    emit_constant(words, cursor, 16u, item->pc);
    words[(*cursor)++] =
        store_word(16u, offsetof(hl_native_x86_64_cpu, program));
    emit_constant(words, cursor, 16u, HL_NATIVE_EXIT_FALLBACK);
    words[(*cursor)++] =
        store_word(16u, offsetof(hl_native_x86_64_cpu, reason));
    if (item->live_chain != 0u) {
      hl_x86_finish_chain(words, cursor);
      hl_x86_spill_all(words, cursor);
    }
    ...
    words[ordered] = UINT32_C(0x54000000) |
                     ...
                     7u; /* b.vc */
    words[(*cursor)++] = UINT32_C(0x6e080400) | 18u << 5 |
                         destination; /* ins vd.d[0],v18.d[0] */
    ...
    if (item->condition != 0u) {
      base = item->vector_subopcode == 0x58u   ? UINT32_C(0x1e202800)
             ...
             : item->vector_subopcode == 0x5cu ? UINT32_C(0x1e203800)
                                               : UINT32_C(0x1e201800);
      if (item->vector_lane == 8u)
        base |= UINT32_C(0x00400000);
      words[(*cursor)++] = base | source << 16 | destination << 5 | 18u;
      ...
      base = item->vector_subopcode == 0x58u   ? UINT32_C(0x4e20d400)
             ...
             : item->vector_subopcode == 0x5cu ? UINT32_C(0x4ea0d400)
                                               : UINT32_C(0x6e20fc00);
      if (item->vector_lane == 8u)
        base |= UINT32_C(0x00400000);
      words[(*cursor)++] = base | source << 16 | destination << 5 | 18u;
      ...
      words[(*cursor)++] =
          UINT32_C(0x6e31a800) | 19u << 5 | 19u; /* uminv b19,v19.16b */
      words[(*cursor)++] =
          UINT32_C(0x1e260000) | 19u << 5 | 16u; /* fmov w16,s19 */
      ordered = (*cursor)++;
    }
    emit_constant(words, cursor, 16u, item->pc);
    words[(*cursor)++] =
    ...
    words[(*cursor)++] = UINT32_C(0xd65f03c0);
    words[ordered] =
        (item->condition != 0u ? UINT32_C(0x54000000) : UINT32_C(0x35000000)) |
        (((*cursor - ordered) & UINT32_C(0x7ffff)) << 5) |
    ...
    if (item->condition != 0u)
      ...
      words[(*cursor)++] =
          UINT32_C(0x4ea01c00) | 18u << 16 | 18u << 5 | destination;
  } else if (item->vector_kind == VECTOR_SCALAR_COMPARE_DOUBLE) {
    uint64_t clear = UINT64_C(1) | UINT64_C(1) << 2 | UINT64_C(1) << 4 |
                     UINT64_C(1) << 6 | UINT64_C(1) << 7 | UINT64_C(1) << 11;
    words[(*cursor)++] = (item->vector_lane == 4u ? UINT32_C(0x1e202000)
                                                  : UINT32_C(0x1e602000)) |
                         (item->condition != 0u ? UINT32_C(0x10) : 0u) |
                         source << 16 |
                         destination << 5; /* fcmp/fcmpe sd,ss or dd,ds */
    words[(*cursor)++] =
        UINT32_C(0x9a9f07e0) | (4u ^ 1u) << 12 | 19u; /* cset x19,mi: less */
    words[(*cursor)++] =
        UINT32_C(0x9a9f07e0) | (0u ^ 1u) << 12 | 20u; /* cset x20,eq */
    words[(*cursor)++] = UINT32_C(0x9a9f07e0) | (6u ^ 1u) << 12 |
                         21u; /* cset x21,vs: unordered */
    words[(*cursor)++] = load_word(22u, offsetof(hl_native_x86_64_cpu, flags));
    emit_constant(words, cursor, 23u, clear);
    words[(*cursor)++] = UINT32_C(0x8a200000) | 23u << 16 | 22u << 5 |
                         22u; /* bic flags,flags,mask */
    words[(*cursor)++] = UINT32_C(0xaa000000) | 21u << 16 | 19u << 5 |
                         19u; /* less | unordered */
    words[(*cursor)++] =
        UINT32_C(0xaa000000) | 19u << 16 | 22u << 5 | 22u; /* CF */
    words[(*cursor)++] = UINT32_C(0xaa000000) | 21u << 16 | 20u << 5 |
                         20u; /* equal | unordered */
    words[(*cursor)++] =
        UINT32_C(0xaa000000) | 20u << 16 | 6u << 10 | 22u << 5 | 22u; /* ZF */
    words[(*cursor)++] =
        UINT32_C(0xaa000000) | 21u << 16 | 2u << 10 | 22u << 5 | 22u; /* PF */
    ...
    trunc_scalar_constants(item, &threshold, &indefinite);
    uint32_t convert =
        item->width == 8u ? UINT32_C(0x9e780000) : UINT32_C(0x1e780000);
    if (single != 0u)
      convert &=
          ~UINT32_C(0x00400000); /* type field 01 (double) -> 00 (single) */
    words[(*cursor)++] = convert | source << 5 |
                         destination; /* fcvtzs x/w destination,s/d source */
    emit_constant(words, cursor, 20u, threshold);
    words[(*cursor)++] =
        (single != 0u ? UINT32_C(0x1e270000) : UINT32_C(0x9e670000)) |
        20u << 5 | 19u; /* fmov s/d 19,w/x 20 */
    words[(*cursor)++] =
        (single != 0u ? UINT32_C(0x1e202000) : UINT32_C(0x1e602000)) |
        19u << 16 | source << 5; /* fcmp source,19 */
    emit_constant(words, cursor, 20u, indefinite);
    words[(*cursor)++] =
        (item->width == 8u ? UINT32_C(0x9a800000) : UINT32_C(0x1a800000)) |
        destination << 16 | 2u << 12 | 20u << 5 |
        destination; /* csel result,indef,result,cs */
    ...
    words[(*cursor)++] =
        UINT32_C(0x6e201c00) | 19u << 16 | 19u << 5 | 19u; /* eor v19,v19,v19 */
    words[(*cursor)++] =
        UINT32_C(0x4e284800) | 19u << 5 | 18u; /* aese v18.16b,v19.16b */
    if (item->vector_kind == VECTOR_AES_ENCRYPT)
      words[(*cursor)++] =
          UINT32_C(0x4e286800) | 18u << 5 | 18u; /* aesmc v18.16b,v18.16b */
    words[(*cursor)++] =
        UINT32_C(0x6e201c00) | source << 16 | 18u << 5 | destination;
    ...
    if (item->memory_operand != 0u) {
      ...
      words[(*cursor)++] =
          (wide != 0u ? UINT32_C(0x9e660000) : UINT32_C(0x1e260000)) |
          16u << 5 | 20u; /* fmov w/x 20,s/d 16 */
      from = 20u;
    }
    if (wide != 0u)
      convert |= UINT32_C(0x80000000);
    if (single == 0u)
      convert |= UINT32_C(0x00400000);
    words[(*cursor)++] = convert | from << 5 | 19u; /* scvtf s/d 19,w/x from */
    words[(*cursor)++] =
        (single != 0u ? UINT32_C(0x6e040400) : UINT32_C(0x6e080400)) |
        19u << 5 | destination; /* ins vd.s/d[0],v19.s/d[0] */
  } else if (item->vector_kind == VECTOR_MERGE_LOW) {
    words[(*cursor)++] =
        (item->width == 4u ? UINT32_C(0x6e040400) : UINT32_C(0x6e080400)) |
        source << 5 | destination; /* ins vd.s/d[0],vn.s/d[0] */
  } else if (item->vector_kind == VECTOR_SIGNED_DWORD_TO_FLOAT) {
    words[(*cursor)++] =
        UINT32_C(0x4e21d800) | source << 5 | destination; /* scvtf .4s */
  } else if (item->vector_kind == VECTOR_FLOAT_TO_SIGNED_DWORD ||
             item->vector_kind == VECTOR_TRUNC_FLOAT_TO_SIGNED_DWORD) {
    /* Capture before writing so an in-place conversion still compares the float
     * source. */
    words[(*cursor)++] =
        UINT32_C(0x4ea01c00) | source << 16 | source << 5 | 17u;
    ...
    words[(*cursor)++] =
        UINT32_C(0x4e040c00) | 16u << 5 | 19u; /* dup indefinite */
    if (item->vector_kind == VECTOR_FLOAT_TO_SIGNED_DWORD) {
      words[(*cursor)++] =
          UINT32_C(0x6e219800) | 17u << 5 | destination; /* frintx */
      words[(*cursor)++] =
          UINT32_C(0x4ea1b800) | destination << 5 | destination;
    } else {
      words[(*cursor)++] =
          UINT32_C(0x4ea1b800) | 17u << 5 | destination; /* fcvtzs */
    }
    words[(*cursor)++] =
        UINT32_C(0x6e20e400) | 18u << 16 | 17u << 5 | 21u; /* >= 2^31 */
    words[(*cursor)++] =
        UINT32_C(0x4e20e400) | 17u << 16 | 17u << 5 | 22u;      /* ordered */
    words[(*cursor)++] = UINT32_C(0x6e205800) | 22u << 5 | 22u; /* NaN mask */
    words[(*cursor)++] = UINT32_C(0x4ea01c00) | 22u << 16 | 21u << 5 | 21u;
    words[(*cursor)++] =
        UINT32_C(0x6e601c00) | destination << 16 | 19u << 5 | 21u;
    words[(*cursor)++] =
        UINT32_C(0x4ea01c00) | 21u << 16 | 21u << 5 | destination;
    ...
    words[(*cursor)++] =
        UINT32_C(0x4ea01c00) | first << 16 | first << 5 | scratch;
    for (output = 0; output < count; ++output) {
      if (((item->vector_immediate >> output) & 1u) != 0u) {
        ...
        words[(*cursor)++] = UINT32_C(0x6e000400) | imm5 << 16 | imm4 << 11 |
                             source << 5 | scratch;
      }
    }
    words[(*cursor)++] =
    ...
    words[(*cursor)++] =
        UINT32_C(0x6e601c00) | first << 16 | source << 5 | scratch;
    words[(*cursor)++] =
        UINT32_C(0x4ea01c00) | scratch << 16 | scratch << 5 | destination;
  } else if (item->vector_kind == VECTOR_FROM_INTEGER)
    words[(*cursor)++] =
        (item->width == 8u ? UINT32_C(0x9e670000) : UINT32_C(0x1e270000)) |
        source << 5 | destination;
  else if (item->vector_kind == VECTOR_TO_INTEGER)
    words[(*cursor)++] =
        (item->width == 8u ? UINT32_C(0x9e660000) : UINT32_C(0x1e260000)) |
        source << 5 | destination;
  else if (item->vector_kind == VECTOR_ZERO_LOW)
    words[(*cursor)++] =
        UINT32_C(0x0ea01c00) | source << 16 | source << 5 | destination;
  else if (item->vector_kind == VECTOR_UNPACK_LOW ||
           item->vector_kind == VECTOR_UNPACK_HIGH) {
    uint32_t base = lane == 1u   ? UINT32_C(0x4e003800)
                    : lane == 2u ? UINT32_C(0x4e403800)
                    : lane == 4u ? UINT32_C(0x4e803800)
                                 : UINT32_C(0x4ec03800);
    if (item->vector_kind == VECTOR_UNPACK_HIGH)
      base += UINT32_C(0x4000);
    ...
    if (count == 0u) {
      ...
      words[(*cursor)++] =
          (item->vector_subopcode == 4u ? UINT32_C(0x4f000400)
                                        : UINT32_C(0x6f000400)) |
    ...
    words[(*cursor)++] = UINT32_C(0x4e001c00) |
                         ((insert_lane << 2) | 2u) << 16 | source << 5 |
                         destination;
    ...
    if (item->vector_kind == VECTOR_COMPARE_EQUAL)
      base = lane == 1u   ? UINT32_C(0x6e208c00)
             : lane == 2u ? UINT32_C(0x6e608c00)
             : lane == 4u ? UINT32_C(0x6ea08c00)
                          : UINT32_C(0x6ee08c00);
    else
      base = lane == 1u   ? UINT32_C(0x4e203400)
             : lane == 2u ? UINT32_C(0x4e603400)
             : lane == 4u ? UINT32_C(0x4ea03400)
                          : UINT32_C(0x4ee03400);
    words[(*cursor)++] = base | source << 16 | first << 5 | destination;
    ...
    uint32_t is_maximum = item->vector_kind == VECTOR_MAXIMUM_SIGNED ||
                          item->vector_kind == VECTOR_MAXIMUM_UNSIGNED;
    uint32_t base =
        (is_unsigned != 0u ? UINT32_C(0x6e206c00) : UINT32_C(0x4e206c00)) -
        (is_maximum != 0u ? UINT32_C(0x800) : 0u);
    base |= lane == 1u   ? 0u
            : lane == 2u ? UINT32_C(0x00400000)
                         : UINT32_C(0x00800000);
    words[(*cursor)++] = base | source << 16 | first << 5 | destination;
  } else if (item->vector_kind == VECTOR_XOR) {
    words[(*cursor)++] =
        UINT32_C(0x6e201c00) | source << 16 | destination << 5 | destination;
  } else if (item->vector_kind == VECTOR_AND) {
    words[(*cursor)++] =
        UINT32_C(0x4e201c00) | source << 16 | destination << 5 | destination;
  } else if (item->vector_kind == VECTOR_OR) {
    words[(*cursor)++] =
        UINT32_C(0x4ea01c00) | source << 16 | destination << 5 | destination;
  } else if (item->vector_kind == VECTOR_AND_NOT) {
    words[(*cursor)++] =
        UINT32_C(0x4e601c00) | destination << 16 | source << 5 | destination;
  } else if (item->vector_kind == VECTOR_ADD ||
             item->vector_kind == VECTOR_SUBTRACT) {
    uint32_t base = item->vector_kind == VECTOR_ADD ? UINT32_C(0x4e208400)
                                                    : UINT32_C(0x6e208400);
    base |= lane == 1u   ? 0u
            : lane == 2u ? UINT32_C(0x00400000)
            : lane == 4u ? UINT32_C(0x00800000)
                         : UINT32_C(0x00c00000);
    words[(*cursor)++] = base | source << 16 | first << 5 | destination;
    ...
    words[(*cursor)++] = UINT32_C(0x4e609c00) | source << 16 | first << 5 |
                         destination; /* mul vd.8h,vfirst.8h,vs.8h */
  } else if (item->vector_kind == VECTOR_MULTIPLY_HIGH_WORD) {
    uint32_t low =
        item->condition != 0u ? UINT32_C(0x0e60c000) : UINT32_C(0x2e60c000);
    uint32_t high =
        item->condition != 0u ? UINT32_C(0x4e60c000) : UINT32_C(0x6e60c000);
    words[(*cursor)++] = low | source << 16 | first << 5 | 17u;
    words[(*cursor)++] = high | source << 16 | first << 5 | 18u;
    words[(*cursor)++] =
        UINT32_C(0x4e405800) | 18u << 16 | 17u << 5 | destination;
    /* UZP2 selects bits 31:16 from each widened product in lane order. */
  } else if (item->vector_kind == VECTOR_MULTIPLY_EVEN_DWORD) {
    words[(*cursor)++] = UINT32_C(0x4e801800) | first << 16 | first << 5 |
                         17u; /* uzp1 even first-source lanes */
    words[(*cursor)++] = UINT32_C(0x4e801800) | source << 16 | source << 5 |
                         18u; /* uzp1 even source lanes */
    words[(*cursor)++] =
        UINT32_C(0x2ea0c000) | 18u << 16 | 17u << 5 | destination;
  } else if (item->vector_kind == VECTOR_MULTIPLY_EVEN_SIGNED_DWORD) {
    words[(*cursor)++] = UINT32_C(0x4e801800) | first << 16 | first << 5 |
                         17u; /* uzp1 even first-source lanes */
    words[(*cursor)++] = UINT32_C(0x4e801800) | source << 16 | source << 5 |
                         18u; /* uzp1 even second-source lanes */
    words[(*cursor)++] =
        UINT32_C(0x0ea0c000) | 18u << 16 | 17u << 5 | destination;
  } else if (item->vector_kind == VECTOR_MULTIPLY_LOW_DWORD) {
    words[(*cursor)++] = UINT32_C(0x4ea09c00) | source << 16 | first << 5 |
                         destination; /* mul vd.4s,vn.4s,vm.4s */
    ...
    words[(*cursor)++] =
        UINT32_C(0x6e207400) | source << 16 | first << 5 | destination;
    ...
    words[(*cursor)++] = base | source << 16 | first << 5 | destination;
             ...
             item->vector_kind == VECTOR_HORIZONTAL_SATURATING) {
    uint32_t arrangement =
        lane == 2u ? UINT32_C(0x00400000) : UINT32_C(0x00800000);
    uint32_t base = item->vector_kind == VECTOR_HORIZONTAL_SUBTRACT
                        ? UINT32_C(0x6e208400)
                    : item->condition != 0u ? UINT32_C(0x4e202c00)
                                            : UINT32_C(0x4e200c00);
    words[(*cursor)++] = UINT32_C(0x4e001800) | arrangement | source << 16 |
                         first << 5 | 17u; /* uzp1: adjacent even lanes */
    words[(*cursor)++] = UINT32_C(0x4e005800) | arrangement | source << 16 |
                         first << 5 | 18u; /* uzp2: adjacent odd lanes */
    words[(*cursor)++] =
        base | arrangement | 18u << 16 | 17u << 5 | destination;
  } else if (item->vector_kind == VECTOR_SIGN) {
    uint32_t size = lane == 1u   ? 0u
                    : lane == 2u ? UINT32_C(0x00400000)
                                 : UINT32_C(0x00800000);
    words[(*cursor)++] =
        UINT32_C(0x4e20a800) | size | source << 5 | 21u; /* cmlt control,#0 */
    words[(*cursor)++] =
        UINT32_C(0x4e208800) | size | source << 5 | 22u; /* cmgt control,#0 */
    words[(*cursor)++] =
        UINT32_C(0x6e20b800) | size | first << 5 | 23u; /* neg first */
    words[(*cursor)++] = UINT32_C(0x4e201c00) | first << 16 | 22u << 5 | 22u;
    words[(*cursor)++] = UINT32_C(0x4e201c00) | 23u << 16 | 21u << 5 | 21u;
    words[(*cursor)++] =
        UINT32_C(0x4ea01c00) | 22u << 16 | 21u << 5 | destination;
  } else if (item->vector_kind == VECTOR_ABSOLUTE) {
    uint32_t size = lane == 1u   ? 0u
                    : lane == 2u ? UINT32_C(0x00400000)
                                 : UINT32_C(0x00800000);
    words[(*cursor)++] =
        UINT32_C(0x4e20b800) | size | source << 5 | destination;
  } else if (item->vector_kind == VECTOR_BYTE_MASK) {
    words[(*cursor)++] =
        UINT32_C(0x6f090400) | source << 5 | 17u; /* ushr v17.16b,source,#7 */
    words[(*cursor)++] = UINT32_C(0x6f001400) | 25u << 16 | 17u << 5 | 17u;
    words[(*cursor)++] = UINT32_C(0x6f001400) | 50u << 16 | 17u << 5 | 17u;
    words[(*cursor)++] = UINT32_C(0x6f001400) | 100u << 16 | 17u << 5 | 17u;
    words[(*cursor)++] = UINT32_C(0x0e013e30); /* umov w16,v17.b[0] */
    words[(*cursor)++] =
        UINT32_C(0x0e113e20) | destination; /* umov wd,v17.b[8] */
    words[(*cursor)++] = UINT32_C(0x2a000000) | destination << 16 | 8u << 10 |
                         16u << 5 | destination;
  } else
    words[(*cursor)++] =
        UINT32_C(0x4ea01c00) | source << 16 | source << 5 | destination;
...
static void emit_vex_completion(uint32_t *words, uint32_t *cursor,
                                const instruction *item,
  ...
  upper = *item;
  upper.width = 16u;
  upper.vector_memory_width = 0u;
  upper.memory_operand = 0u;
  upper.destination = 20u;
  if (memory_upper != 0u)
    upper.source = 29u;
  else {
    hl_x86_emit_vector_upper_load(words, cursor, 29u, item->source);
    upper.source = 29u;
  }
  if (item->vector_kind == VECTOR_SHUFFLE_FLOAT ||
      item->vector_kind == VECTOR_SHUFFLE_DOUBLE ||
    ...
    upper.vector_source_one = upper.source;
...
void hl_x86_emit_vector(uint32_t *words, uint32_t *cursor,
                        const instruction *item) {
  uint32_t *below;
  uint32_t *overflow;
  uint32_t *above;
  ...
  if (item->memory_write != 0u) {
    if (width == 32u)
      hl_x86_emit_vector_upper_load(words, cursor, 17u, item->source);
    words[(*cursor)++] =
        UINT32_C(0xd5033abf); /* dmb ishst: preserve x86 StoreStore ordering */
    words[(*cursor)++] = (width == 4u   ? UINT32_C(0xbd000220)
                          : width == 8u ? UINT32_C(0xfd000220)
                                        : UINT32_C(0x3d800220)) |
                         item->source;
    if (width == 32u)
    ...
    words[(*cursor)++] = (width == 2u   ? UINT32_C(0x79400220)
                          ...
                          : width == 8u ? UINT32_C(0xfd400220)
                                        : UINT32_C(0x3dc00220)) |
                         (width == 32u                       ? 16u
                          : item->vector_kind == VECTOR_COPY ? item->destination
                                                             : 16u);
    if (width == 32u)
    ...
    words[(*cursor)++] =
        UINT32_C(0xd50339bf); /* dmb ishld: preserve x86 LoadLoad/LoadStore */
    if (width == 32u && item->vector_kind == VECTOR_COPY) {
      words[(*cursor)++] =
          UINT32_C(0x4ea01c00) | 16u << 16 | 16u << 5 | item->destination;
  ...
  words[(*cursor)++] =
      store_word(16, offsetof(hl_native_x86_64_cpu, fault_address));
  words[(*cursor)++] =
      required == 2u ? UINT32_C(0xd2800051) : UINT32_C(0xd2800031);
  words[(*cursor)++] =
      store_word(17, offsetof(hl_native_x86_64_cpu, fault_access));
