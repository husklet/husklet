...
void hl_x86_spill(uint32_t *words, uint32_t *cursor, uint32_t dirty) {
    ...
    for (index = 0; index < 16; ++index) {
        if ((dirty & (UINT32_C(1) << index)) != 0)
            words[(*cursor)++] =
                store_word(index, offsetof(hl_native_x86_64_cpu, registers) + index * sizeof(uint64_t));
    }
}
...
void hl_x86_emit_exit(const hl_x86_a64_request *request, const decode *block, uint32_t *cursor) {
    if (block->exit == HL_X86_A64_DYNAMIC_BRANCH || block->exit == HL_X86_A64_DIRECT_CALL) return;
    if (block->exit == HL_X86_A64_CONDITIONAL_BRANCH) {
        ...
        if (control->condition == 0xau || control->condition == 0xbu) {
            request->host_words[(*cursor)++] = load_word(20, offsetof(hl_native_x86_64_cpu, flags));
        ...
        if (self_loop) {
            uint32_t condition = control->condition == 0xau ? 1u : control->condition == 0xbu ? 0u :
                                                                                              arm_condition(control->condition);
            uint32_t taken_branch = (*cursor)++;
            emit_constant(request->host_words, cursor, 16, block->next_pc);
            request->host_words[(*cursor)++] = store_word(16, offsetof(hl_native_x86_64_cpu, program));
            request->host_words[(*cursor)++] = load_word(17, offsetof(hl_native_x86_64_cpu, loop_completed));
            request->host_words[(*cursor)++] = UINT32_C(0x91000400) | 17u << 5 | 17u;
            request->host_words[(*cursor)++] = store_word(17, offsetof(hl_native_x86_64_cpu, loop_completed));
            ...
            request->host_words[taken_branch] = UINT32_C(0x54000000) |
                ((taken - taken_branch) & UINT32_C(0x7ffff)) << 5 | condition;
            request->host_words[(*cursor)++] = load_word(17, offsetof(hl_native_x86_64_cpu, loop_completed));
            request->host_words[(*cursor)++] = UINT32_C(0x91000400) | 17u << 5 | 17u;
            request->host_words[(*cursor)++] = store_word(17, offsetof(hl_native_x86_64_cpu, loop_completed));
            request->host_words[(*cursor)++] = load_word(17, offsetof(hl_native_x86_64_cpu, loop_remaining));
            request->host_words[(*cursor)++] = UINT32_C(0xd1000400) | 17u << 5 | 17u;
            request->host_words[(*cursor)++] = store_word(17, offsetof(hl_native_x86_64_cpu, loop_remaining));
            ...
            request->host_words[(*cursor)++] = load_word(16, offsetof(hl_native_x86_64_cpu, interrupt));
            uint32_t irq_branch = (*cursor)++;
            int64_t body_delta = -(int64_t)(*cursor);
            request->host_words[(*cursor)++] = UINT32_C(0x14000000) | ((uint32_t)body_delta & UINT32_C(0x03ffffff));
            uint32_t done = *cursor;
            request->host_words[zero_branch] = UINT32_C(0xb4000000) |
                ((done - zero_branch) & UINT32_C(0x7ffff)) << 5 | 17u;
            request->host_words[irq_branch] = UINT32_C(0xb5000000) |
                ((done - irq_branch) & UINT32_C(0x7ffff)) << 5 | 16u;
            request->host_words[executable_branch] = UINT32_C(0x37000000) |
        ...
        request->host_words[(*cursor)++] = UINT32_C(0x9a800000) | 17u << 16 |
                                           (control->condition == 0xau ? 1u : control->condition == 0xbu ? 0u :
                                                                                                           arm_condition(control->condition)) <<
                                               12 |
                                           16u << 5 | 16u;
        if (control->condition != 0xau && control->condition != 0xbu)
        ...
        emit_constant(request->host_words, cursor, 16,
                      block->exit == HL_X86_A64_DIRECT_BRANCH ? block->target : block->next_pc);
    }
    request->host_words[(*cursor)++] = store_word(16, offsetof(hl_native_x86_64_cpu, program));
}
