...
uint32_t hl_x86_cmov_words(const instruction *item) {
  uint32_t condition =
      item->condition == 0xau || item->condition == 0xbu ? 3u : 15u;
...
void hl_x86_emit_cmov(uint32_t *words, uint32_t *cursor,
  ...
  if (item->width == 2u) {
    words[(*cursor)++] = UINT32_C(0x1a800000) |
    ...
    words[(*cursor)++] =
        ...
        (uint32_t)item->destination << 16 | condition << 12 | source << 5 |
        item->destination;
  }
}
...
uint32_t hl_x86_set_words(const instruction *item) {
  uint32_t condition =
      item->condition == 0xau || item->condition == 0xbu ? 4u : 16u;
...
void hl_x86_emit_set(uint32_t *words, uint32_t *cursor,
  ...
  words[(*cursor)++] =
      UINT32_C(0x1a9f07e0) | ((condition ^ 1u) & 15u) << 12 | 20u;
  if (item->memory_operand != 0u) {
    instruction store = *item;
    store.operation = OP_STORE;
    store.source = 20u;
...
/* Counted by emitting into a scratch buffer: the four forms differ by several
   words each and a hand-kept constant has to be an exact match, not an upper
   bound. */
...
void hl_x86_emit_bitscan(uint32_t *words, uint32_t *cursor,
  ...
  if (item->condition == 0u) {
    words[(*cursor)++] = (wide ? UINT32_C(0xdac00000) : UINT32_C(0x5ac00000)) |
                         source << 5 | scan;
    words[(*cursor)++] =
        (wide ? UINT32_C(0xdac01000) : UINT32_C(0x5ac01000)) | scan << 5 | scan;
  } else if (item->conditional != 0u) {
    words[(*cursor)++] = (wide ? UINT32_C(0xdac01000) : UINT32_C(0x5ac01000)) |
                         source << 5 | scan;
  } else {
    words[(*cursor)++] = (wide ? UINT32_C(0xdac01000) : UINT32_C(0x5ac01000)) |
                         source << 5 | 20u;
    /* The halfword source was already zero-extended into a 32-bit scratch, so
       its leading zero count is the 32-bit one and the index it subtracts from
       is 31, not 15. */
  ...
  if (item->conditional != 0u && item->width == 2u) {
    if (item->condition != 0u) {
      ...
      words[(*cursor)++] =
          UINT32_C(0x1a800000) | 19u << 16 | 9u << 12 | target << 5 | target;
    }
  }
  if (item->conditional == 0u && item->width == 2u) {
    /* A 32-bit test of the zero-extended halfword always reports N=0, so shift
       the halfword's own sign bit up before the test that publishes SF. */
  ...
  if (item->conditional != 0u) {
    ...
    if (item->width == 2u)
      words[(*cursor)++] =
          UINT32_C(0xb3403c00) | target << 5 | item->destination;
    ...
    if (item->width == 2u)
      words[(*cursor)++] =
          UINT32_C(0xb3403c00) | target << 5 | item->destination;
    hl_x86_emit_nzcv(words, cursor);
    /* The NZCV transfer reads ARM C as a borrow, so a logical test lands as
       CF=1; bsf and bsr clear CF, and PF and AF stay as the interpreter leaves
       them. */
    words[(*cursor)++] = load_word(20u, offsetof(hl_native_x86_64_cpu, flags));
    words[(*cursor)++] = UINT32_C(0x927ff800) | 20u << 5 | 20u;
    words[(*cursor)++] = store_word(20u, offsetof(hl_native_x86_64_cpu, flags));
...
static instruction string_memory(const instruction *item, unsigned operation,
                                 unsigned base, unsigned reg,
                                 unsigned segment) {
  instruction memory = {0};
  memory.pc = item->pc;
  memory.length = item->length;
  memory.operation = operation;
  memory.destination = reg;
  memory.source = reg;
  memory.width = item->width;
  memory.load_width = operation == OP_LOAD ? item->width : 0u;
  memory.address_32 = item->address_32;
  memory.address_base = base;
  memory.address_index = UINT8_MAX;
  memory.segment = segment;
  memory.live_chain = item->live_chain;
  return memory;
}
...
uint32_t hl_x86_string_words(const instruction *item) {
  ...
  if (item->condition != 1u) {
    instruction load = string_memory(
        item, OP_LOAD, 6u, item->condition == 2u ? 0u : 18u, item->segment);
  ...
  if (item->condition != 2u) {
    instruction store =
        string_memory(item, OP_STORE, 7u, item->condition == 1u ? 0u : 19u, 0u);
...
void hl_x86_emit_string(uint32_t *words, uint32_t *cursor,
  ...
  if (item->condition != 1u) {
    ...
    if (item->condition == 0u)
      words[(*cursor)++] =
          UINT32_C(0xaa1203f3); /* mov x19,x18: preserve across store guards */
  }
  if (item->condition != 2u) {
    instruction store =
        string_memory(item, OP_STORE, 7u, item->condition == 1u ? 0u : 19u, 0u);
  ...
  uint32_t sub =
      item->address_32 != 0u ? UINT32_C(0x51000000) : UINT32_C(0xd1000000);
  uint32_t add =
      item->address_32 != 0u ? UINT32_C(0x11000000) : UINT32_C(0x91000000);
  if (item->condition != 1u)
    words[(*cursor)++] = sub | (uint32_t)item->width << 10 | 6u << 5 | 6u;
  if (item->condition != 2u)
    words[(*cursor)++] = sub | (uint32_t)item->width << 10 | 7u << 5 | 7u;
  uint32_t *advanced = &words[(*cursor)++];
  uint32_t forward_index = *cursor;
  if (item->condition != 1u)
    words[(*cursor)++] = add | (uint32_t)item->width << 10 | 6u << 5 | 6u;
  if (item->condition != 2u)
    words[(*cursor)++] = add | (uint32_t)item->width << 10 | 7u << 5 | 7u;
  uint32_t advanced_index = *cursor;
  *forward = UINT32_C(0x36000000) | 10u << 19 |
             ((forward_index - (uint32_t)(forward - words)) & UINT32_C(0x3fff))
                 << 5 |
             17u;
  *advanced =
      UINT32_C(0x14000000) |
  ...
  if (item->conditional != 0u) {
    words[(*cursor)++] =
        (item->address_32 != 0u ? UINT32_C(0x51000421) : UINT32_C(0xd1000421));
