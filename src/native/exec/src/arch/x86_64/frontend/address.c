...
int hl_x86_decode_address(const hl_x86_a64_request *request, decode *block,
  ...
  if (mod == 2u)
    displacement_size = 4u;
  if (displacement_size > request->guest_size - *cursor ||
      displacement_size > 15u - (*cursor - start))
    return fail(block, cursor, start, HL_X86_A64_TRUNCATED);
  item->immediate = displacement_size == 0u
                        ? 0u
                        : (uint64_t)load_signed(&request->guest_bytes[*cursor],
                                                displacement_size);
...
static uint32_t fold_displacement(uint64_t immediate, uint32_t *low,
  ...
  *negative = value < 0;
  ...
  if (magnitude > UINT64_C(0xffffff))
    return 0;
...
static uint32_t add_immediate(unsigned destination, unsigned source,
                              uint32_t value, int shift, int negative,
                              int wide) {
  return (negative ? (wide ? UINT32_C(0xd1000000) : UINT32_C(0x51000000))
                   : (wide ? UINT32_C(0x91000000) : UINT32_C(0x11000000))) |
         (shift != 0 ? UINT32_C(0x400000) : 0u) | value << 10 | source << 5 |
         destination;
}
...
static uint64_t address_constant(const instruction *item) {
  uint64_t value =
      (item->rip_relative ? item->pc + item->length : 0u) + item->immediate;
...
uint32_t hl_x86_address_words(const instruction *item) {
  ...
  int negative;
  uint32_t folded =
      item->address_base != UINT8_MAX && item->immediate != 0u
  ...
  if (item->address_base == UINT8_MAX)
    ...
    words = 1u +
            (item->immediate != 0u ? constant_words(item->immediate) + 1u : 0u);

  if (item->address_index != UINT8_MAX)
    ++words;
  if (item->bit_memory_offset != 0u)
    words += item->bit_immediate != 0u
                 ? constant_words(item->operand_immediate >> 3) + 1u
                 : 3u; /* extend/move, asr, add - three words at every operand
                          width */
...
void hl_x86_emit_address(uint32_t *words, uint32_t *cursor,
  ...
  if (item->address_base == UINT8_MAX) {
    ...
    if (low != 0u)
      words[(*cursor)++] = add_immediate(16, source, low, 0, negative, wide);
  } else {
    words[(*cursor)++] = move_register(16, item->address_base, wide);
    if (item->immediate != 0u) {
      emit_constant(words, cursor, 17, item->immediate);
      words[(*cursor)++] =
          (wide ? UINT32_C(0x8b000000) : UINT32_C(0x0b000000)) | 17u << 16 |
          16u << 5 | 16u;
    }
  }
  if (item->address_index != UINT8_MAX)
    words[(*cursor)++] =
        (item->address_32 != 0u ? UINT32_C(0x0b000000) : UINT32_C(0x8b000000)) |
        (uint32_t)item->address_index << 16 |
        (uint32_t)item->address_scale << 10 | 16u << 5 | 16u;
  if (item->segment != 0u) {
    words[(*cursor)++] =
        load_word(17, item->segment == 1u ? offsetof(hl_native_x86_64_cpu, fs)
  ...
  if (item->bit_memory_offset != 0u) {
    if (item->bit_immediate != 0u) {
      ...
      if (item->bit_operand_width == 2u)
        words[(*cursor)++] = UINT32_C(0x93403c00) |
                             (uint32_t)item->bit_index << 5 | 17u; /* sxth */
      else if (item->bit_operand_width == 4u)
        words[(*cursor)++] = UINT32_C(0x93407c00) |
                             (uint32_t)item->bit_index << 5 | 17u; /* sxtw */
      else
        words[(*cursor)++] =
            UINT32_C(0xaa0003e0) | (uint32_t)item->bit_index << 16 | 17u;
      words[(*cursor)++] =
          UINT32_C(0x9343fc00) | 17u << 5 | 17u; /* asr x17,x17,#3 */
    }
    words[(*cursor)++] = UINT32_C(0x8b110210); /* add x16,x16,x17 */
  }
