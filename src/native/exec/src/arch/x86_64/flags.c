...
uint32_t hl_x86_rflags_to_nzcv(uint64_t flags) {
    ...
    if ((flags & HL_X86_RFLAGS_SF) != 0) nzcv |= UINT32_C(1) << 31;
    if ((flags & HL_X86_RFLAGS_ZF) != 0) nzcv |= UINT32_C(1) << 30;
    if ((flags & HL_X86_RFLAGS_CF) == 0) nzcv |= UINT32_C(1) << 29;
    if ((flags & HL_X86_RFLAGS_OF) != 0) nzcv |= UINT32_C(1) << 28;
    return nzcv;
}
...
uint64_t hl_x86_nzcv_to_rflags(uint32_t nzcv, uint64_t preserved) {
    ...
    if ((nzcv & (UINT32_C(1) << 31)) != 0) flags |= HL_X86_RFLAGS_SF;
    if ((nzcv & (UINT32_C(1) << 30)) != 0) flags |= HL_X86_RFLAGS_ZF;
    if ((nzcv & (UINT32_C(1) << 29)) == 0) flags |= HL_X86_RFLAGS_CF;
    if ((nzcv & (UINT32_C(1) << 28)) != 0) flags |= HL_X86_RFLAGS_OF;
    return flags;
}
...
int hl_x86_condition_holds(uint8_t condition, uint64_t flags) {
    ...
    switch ((condition >> 1) & 7u) {
        ...
        case 4: value = sign; break;
        case 5: value = parity; break;
        case 6: value = sign != overflow; break;
        default: value = zero || sign != overflow; break;
    }
