...
static int valid(uint32_t word) {
    uint64_t mask;
    return (word & 0x1F800000u) == 0x12000000u && hl_a64_logical_mask(word, &mask);
}
...
int hl_a64_logical_mask(uint32_t word, uint64_t *output) {
    ...
    uint64_t element;
    uint64_t mask = 0;
    if (output == NULL || (width == 32 && n) || pattern < 2) return 0;
    ...
    if (size != 64) element &= (UINT64_C(1) << size) - 1u;
    for (unsigned offset = 0; offset < width; offset += size) mask |= element << offset;
    *output = mask;
    return 1;
}
...
int hl_a64_logical_body(hl_a64_assembler *assembler, uint32_t word) {
    ...
    hl_a64_emit32(assembler, (word & ~0x3FFu) | (native_source << 5) | native_destination);
    if (destination != 31 && stolen(destination))
        hl_a64_str(assembler, 17, CPU, (int)destination * 8);
    return hl_a64_assembler_ok(assembler);
}

int hl_a64_logical_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_LOGICAL_MAX_BYTES || !valid(word)) return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_logical_body(assembler, word)) return 0;
