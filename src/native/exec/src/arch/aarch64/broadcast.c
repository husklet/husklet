...
static int valid(uint32_t word) {
  ...
  unsigned size;
  if ((!element(word) && !general(word)) || immediate == 0)
    return 0;
...
int hl_a64_broadcast_emit(hl_a64_assembler *assembler, uint32_t word,
                          uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_BROADCAST_MAX_BYTES ||
      !valid(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_broadcast_body(assembler, word))
    return 0;
