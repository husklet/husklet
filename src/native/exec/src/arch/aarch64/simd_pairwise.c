...
static int valid(uint32_t word) {
    ...
    if ((word & UINT32_C(0xdf3e0c00)) != UINT32_C(0x5e300800)) return 0;
    if (!u) return opcode == 0x1bu && size == 3u;
...
int hl_a64_simd_pairwise_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_SIMD_PAIRWISE_MAX_BYTES ||
        !valid(word))
        return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_simd_pairwise_body(assembler, word)) return 0;
