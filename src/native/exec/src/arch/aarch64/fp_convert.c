...
int hl_a64_fp_convert_emit(hl_a64_assembler *assembler, uint32_t word,
                           uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_FP_CONVERT_MAX_BYTES ||
      !valid(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_fp_convert_body(assembler, word))
    return 0;
