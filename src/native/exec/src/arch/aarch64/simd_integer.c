...
static int valid(uint32_t word) {
    ...
    if (!q && size == 3u) return 0;
    switch (opcode) {
        case 0x10u: return 1;                       /* ADD / SUB */
        case 0x0cu:                                /* SMAX / UMAX */
        case 0x0du:                                /* SMIN / UMIN */
        case 0x12u: return size != 3u;              /* MLA / MLS */
        case 0x13u: return u ? size == 0u : size != 3u; /* PMUL / MUL */
        default: return 0;
    }
}
...
int hl_a64_simd_integer_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_SIMD_INTEGER_MAX_BYTES ||
        !valid(word))
        return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_simd_integer_body(assembler, word)) return 0;
