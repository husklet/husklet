...
static int valid(uint32_t word) {
    ...
    if ((word & UINT32_C(0x8f3e0c00)) != UINT32_C(0x0e200800) || size == 3u) return 0;
    if (scalar && !q) return 0;
...
int hl_a64_saturating_narrow_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (!assembler || hl_a64_assembler_remaining(assembler) < HL_A64_SATURATING_NARROW_MAX_BYTES ||
        !valid(word)) return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_saturating_narrow_body(assembler, word)) return 0;
