...
static int integer_conversion(uint32_t word) {
    unsigned q = (word >> 30) & 1u;
    unsigned size = (word >> 22) & 1u;
    if ((word & UINT32_C(0x9fbffc00)) != UINT32_C(0x0e21d800)) return 0;
    return !size || q;
}
...
int hl_a64_simd_float_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_SIMD_FLOAT_MAX_BYTES ||
        !valid(word))
        return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_simd_float_body(assembler, word)) return 0;
