...
int hl_a64_shift_body(hl_a64_assembler *assembler, uint32_t word) {
    ...
    if (stolen(value)) hl_a64_ldr(assembler, 16, CPU, (int)value * 8);
    if (stolen(amount)) hl_a64_ldr(assembler, 17, CPU, (int)amount * 8);
    hl_a64_emit32(assembler, (word & ~((31u << 16) | (31u << 5) | 31u)) |
                             (native_amount << 16) | (native_value << 5) | native_destination);
...
int hl_a64_shift_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_SHIFT_MAX_BYTES || !valid(word)) return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_shift_body(assembler, word)) return 0;
