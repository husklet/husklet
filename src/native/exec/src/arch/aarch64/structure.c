...
static void guest_register(hl_a64_assembler *assembler, unsigned output,
                           unsigned reg) {
  if (reg == 31)
...
static void writeback(hl_a64_assembler *assembler,
...
static int transfer_bytes(uint32_t word, uint64_t *bytes) {
  ...
  if (((word >> 24) & 1u) == 0) {
    ...
    if (((word >> 10) & 3u) == 3u && q == 0 && registers > 1)
      return 0;
...
static int valid(uint32_t word, uint64_t pc, hl_a64_memory *memory) {
  ...
  if (!hl_a64_memory_decode(word, pc, memory) ||
      memory->kind != HL_A64_MEMORY_STRUCTURE || !transfer_bytes(word, &bytes))
    return 0;
  memory->bytes = bytes;
  return 1;
}

int hl_a64_structure_body(hl_a64_assembler *assembler, uint32_t word,
  ...
  *guard = (hl_a64_guard){.pc = pc};
  ...
  if (memory.writeback)
    writeback(assembler, &memory);
  return hl_a64_assembler_ok(assembler);
}

int hl_a64_structure_emit(hl_a64_assembler *assembler, uint32_t word,
                          uint64_t pc) {
  hl_a64_memory memory;
  hl_a64_guard guard;
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_STRUCTURE_MAX_BYTES ||
      !valid(word, pc, &memory))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_structure_body(assembler, word, pc, &guard, NULL))
    return 0;
