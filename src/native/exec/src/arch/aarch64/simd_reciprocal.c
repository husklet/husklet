...
static int valid(uint32_t word) {
    ...
    if ((word & UINT32_C(0x8f3e0c00)) == UINT32_C(0x0e200800)) {
        if (opcode == 0x1cu) return !scalar && size == 2u;
        if (opcode != 0x1du || size < 2u) return 0;
...
int hl_a64_simd_reciprocal_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_SIMD_RECIPROCAL_MAX_BYTES ||
        !valid(word))
        return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_simd_reciprocal_body(assembler, word)) return 0;
