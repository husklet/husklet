...
int hl_a64_reverse_body(hl_a64_assembler *assembler, uint32_t word) {
  ...
  hl_a64_emit32(assembler, (word & ~((31u << 5) | 31u)) | (native_source << 5) |
                               native_destination);
...
int hl_a64_reverse_emit(hl_a64_assembler *assembler, uint32_t word,
                        uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_REVERSE_MAX_BYTES ||
      !valid(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_reverse_body(assembler, word))
    return 0;
