...
#define OFFSET_PERMISSIONS                                                     \
  ((int)offsetof(hl_native_aarch64_cpu, memory_permissions))
#define OFFSET_FAULT_ADDRESS                                                   \
  ((int)offsetof(hl_native_aarch64_cpu, fault_address))
...
#define OFFSET_FAULT_COMPLETED                                                 \
  ((int)offsetof(hl_native_aarch64_cpu, fault_completed))
...
#define OFFSET_READ_INCARNATION                                                \
  ((int)offsetof(hl_native_aarch64_cpu, read_incarnation))
#define OFFSET_READ_COUNT ((int)offsetof(hl_native_aarch64_cpu, read_count))
#define OFFSET_READ_VALID_COUNT                                                \
  ((int)offsetof(hl_native_aarch64_cpu, read_valid_count))
#define OFFSET_READ_VIEWS ((int)offsetof(hl_native_aarch64_cpu, read_views))
#define OFFSET_READ_VIEW_PUBLICATION                                           \
  ((int)offsetof(hl_native_aarch64_cpu, read_view_publication))
#define OFFSET_WRITE_POLICY                                                    \
  ((int)offsetof(hl_native_aarch64_cpu, memory_write_policy))
#define OFFSET_WRITE_INDEX                                                     \
  ((int)offsetof(hl_native_aarch64_cpu, memory_write_index))
#define OFFSET_WRITTEN ((int)offsetof(hl_native_aarch64_cpu, memory_written))
#define OFFSET_EXECUTABLE_WRITTEN                                              \
  ((int)offsetof(hl_native_aarch64_cpu, executable_written))
#define OFFSET_DIRTY_VIEW_FIRST                                                \
  ((int)offsetof(hl_native_aarch64_cpu, dirty_view_first))
#define OFFSET_DIRTY_VIEW_LAST                                                 \
  ((int)offsetof(hl_native_aarch64_cpu, dirty_view_last))
...
#define OFFSET_DIRTY_RECORDS                                                   \
  ((int)offsetof(hl_native_aarch64_cpu, dirty_records))
#define OFFSET_DIRTY_OVERFLOW                                                  \
  ((int)offsetof(hl_native_aarch64_cpu, dirty_overflow))
#define OFFSET_RESERVE_FILTER                                                  \
  ((int)offsetof(hl_native_aarch64_cpu, reserve_filter))
...
_Static_assert(DIRTY_CAPACITY < 4096,
               "aarch64 dirty capacity exceeds cmp imm12");
/* The gate reads its slot with an ldrb whose unsigned immediate is unscaled. */
_Static_assert(OFFSET_RESERVE_FILTER + (int)HL_NATIVE_A64_SATURATION_SLOTS <
                   4096,
...
static uint64_t overflow_report(const hl_a64_assembler *assembler,
                                uint64_t pc) {
...
static int patch_offset(hl_a64_assembler *assembler, uint32_t *instruction,
  ...
  int64_t limit;
  if (!hl_a64_assembler_ok(assembler) || instruction == NULL ||
      target == NULL || words == NULL)
    goto invalid;
...
static void condition(hl_a64_assembler *assembler, uint32_t *instruction,
  ...
  int64_t words;
  if (!patch_offset(assembler, instruction, target, 19u, &words))
    return;
...
static void compare_zero(hl_a64_assembler *assembler, uint32_t *instruction,
  ...
  if (!patch_offset(assembler, instruction, target, 19u, &words))
    return;
  *instruction =
      0x34000000u | (((uint32_t)words & 0x7ffffu) << 5) | (uint32_t)reg;
}

static void branch(hl_a64_assembler *assembler, uint32_t *instruction,
                   const uint8_t *target);
static void link(hl_a64_assembler *assembler, uint32_t *instruction,
                 const uint8_t *target);
...
static void archive_dirty_body(hl_a64_assembler *assembler, uint64_t pc) {
  uint32_t *none, *empty, *different[2], *before, *after, *keep_first,
      *keep_last, *merged, *overflow, *success;
  hl_a64_ldr(assembler, 17, CPU, OFFSET_DIRTY_FIRST);
  hl_a64_emit32(assembler,
                UINT32_C(0xb100063f)); /* cmn x17,#1: empty sentinel */
  ...
  hl_a64_ldr(assembler, 9, CPU, OFFSET_DIRTY_FIRST);
  hl_a64_emit32(assembler,
                UINT32_C(0xeb11013f)); /* cmp active_first,record_first */
  ...
  hl_a64_emit32(assembler,
                UINT32_C(0xf100021f) |
                    (DIRTY_CAPACITY << 10)); /* cmp count,#capacity */
  ...
  if (overflow_continue != NULL)
    branch(assembler, overflow_continue, done);
}
...
void hl_a64_guard_write_begin(hl_a64_assembler *assembler, uint64_t bytes,
                              uint64_t pc, hl_a64_guard *guard) {
  uint32_t *empty, *not_contiguous, *contiguous, *different_view[2], *above,
      *below, *safe;
  uint32_t *gate = NULL;
  ...
  if (!assembler->write_reserve || assembler->dirty_overflow_continue)
    return;
  if (assembler->runtime_write_reserve) {
    ...
    hl_a64_emit32(
        assembler,
        0x39400000u |
            ((uint32_t)(OFFSET_RESERVE_FILTER + (int)site_slot(pc)) << 10) |
            ((uint32_t)CPU << 5) | 17u);
  ...
  hl_a64_ldr(assembler, 9, CPU, 9 * 8);
  if (gate != NULL)
    compare_zero(assembler, gate, assembler->cursor, 17);
}

void hl_a64_guard_written(hl_a64_assembler *assembler, uint64_t bytes) {
  uint32_t *empty, *not_contiguous, *contiguous, *different_view[2], *above,
      *below, *after_merge, *after_range, *after_contiguous, *full,
      *overflow_clear, *overflow_done;
  ...
  hl_a64_emit32(assembler, UINT32_C(0xf100023f) |
                               (DIRTY_CAPACITY << 10)); /* cmp x17,#capacity */
...
static void test(hl_a64_assembler *assembler, uint32_t *instruction,
  ...
  int64_t words;
  if (!patch_offset(assembler, instruction, target, 14u, &words))
    return;
...
static void branch(hl_a64_assembler *assembler, uint32_t *instruction,
                   const uint8_t *target) {
  int64_t words;
  if (!patch_offset(assembler, instruction, target, 26u, &words))
    return;
...
/* The view payload is immutable for a whole entry, so the scan is a
 * count-driven loop over x30 rather than four unrolled copies of the same
 * twelve words. */
static void read_cache(hl_a64_assembler *assembler, uint64_t bytes,
                       uint32_t **hit) {
  uint32_t *active[1];
  uint32_t *exhausted;
  uint32_t *below;
  ...
  diagnostic_increment(
      assembler, (int)offsetof(hl_native_aarch64_cpu, diagnostic_guard_fast));
  hl_a64_ldr(assembler, 17, CPU, OFFSET_FLAGS);
  hl_a64_emit32(assembler, 0xD51B4200u | 17u);
  hl_a64_ldr(assembler, 9, CPU, 9 * 8);
...
static void write_cache(hl_a64_assembler *assembler, uint64_t bytes,
                        uint64_t pc, const uint8_t *resume, uint8_t **archive) {
  uint32_t *inactive[1];
  uint32_t *exhausted;
  uint32_t *below;
  uint32_t *above;
...
void hl_a64_guard_begin(hl_a64_assembler *assembler, uint64_t bytes,
  ...
  diagnostic_increment(
      assembler, (int)offsetof(hl_native_aarch64_cpu, diagnostic_guard_full));
  guard->required = required;
  guard->bytes = bytes;
}

void hl_a64_guard_direct_begin(hl_a64_assembler *assembler, uint64_t bytes,
                               uint32_t required, hl_a64_guard *guard) {
  hl_a64_str(assembler, 9, CPU, 9 * 8);
  hl_a64_emit32(assembler, 0xD53B4209u);
  hl_a64_str(assembler, 9, CPU, OFFSET_FLAGS);
...
void hl_a64_guard_finish(hl_a64_assembler *assembler,
                         const hl_a64_guard *guard) {
  ...
  if (guard->below == NULL) {
    if (guard->overflow == NULL && guard->above == NULL &&
        guard->permission == NULL && guard->resume == NULL &&
        guard->required == 0 && guard->bytes == 0)
