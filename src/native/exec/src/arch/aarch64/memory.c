...
static int casp(uint32_t word) {
    return (word & 0xBFA07C00u) == 0x08207C00u;
}

static uint64_t memory_bytes(uint32_t word) {
    if (casp(word)) return (word & (1u << 30)) ? 16 : 8;
    ...
    if ((word & 0x3F000000u) == 0x08000000u && !(word & (1u << 23)) && (word & (1u << 21))) bytes *= 2;
    return pair ? bytes * 2 : bytes;
}

static int prefetch(uint32_t word) {
    if ((word & 0xFF000000u) == 0xD8000000u) return 1;
    if ((word & 0x04000000u) != 0 || ((word >> 30) & 3) != 3 || ((word >> 22) & 3) != 2) return 0;
    return (word & 0x3B000000u) == 0x39000000u || (word & 0x3B200000u) == 0x38000000u ||
...
static uint64_t structure_bytes(uint32_t word) {
    ...
    if (!((word >> 24) & 1)) {
        ...
        switch ((word >> 12) & 15u) {
            ...
            case 6: regs = 3; break;
            case 8:
            case 10: regs = 2; break;
            case 7: regs = 1; break;
            default: regs = 0; break;
        }
...
int hl_a64_memory_decode(uint32_t word, uint64_t pc, hl_a64_memory *output) {
    ...
    if ((word & 0x3B000000u) == 0x18000000u || (word & 0xFF000000u) == 0x98000000u ||
        (word & 0x3F000000u) == 0x1C000000u || (word & 0xFF000000u) == 0xD8000000u) {
        output->kind = prefetch(word) ? HL_A64_MEMORY_PREFETCH : HL_A64_MEMORY_LITERAL;
        ...
        output->offset = output->postindex ? 0 : sign_extend((word >> 15) & 0x7fu, 7) *
                                                   (int64_t)(memory_bytes(word) / 2);
    } else if ((word & 0x3F000000u) == 0x08000000u) {
        output->kind = casp(word) || (word & (1u << 23)) ? HL_A64_MEMORY_ATOMIC : HL_A64_MEMORY_EXCLUSIVE;
    } else if ((word & 0x3B200C00u) == 0x38200000u) {
        output->kind = HL_A64_MEMORY_ATOMIC;
    } else if ((word & 0x3B000000u) == 0x39000000u) {
        output->kind = prefetch(word) ? HL_A64_MEMORY_PREFETCH : HL_A64_MEMORY_UNSIGNED;
        output->offset = (int64_t)((word >> 10) & 0xfffu) << __builtin_ctzll(memory_bytes(word));
    } else if ((word & 0x3B200C00u) == 0x38200800u) {
        output->kind = prefetch(word) ? HL_A64_MEMORY_PREFETCH : HL_A64_MEMORY_REGISTER;
    } else if ((word & 0x3B200000u) == 0x38000000u) {
        output->kind = prefetch(word) ? HL_A64_MEMORY_PREFETCH : HL_A64_MEMORY_UNSCALED;
        mode = (word >> 10) & 3u;
        output->postindex = mode == 1;
        output->writeback = mode == 1 || mode == 3;
        output->offset = output->postindex ? 0 : sign_extend((word >> 12) & 0x1ffu, 9);
    } else {
        return 0;
    }
    ...
    if (output->kind != HL_A64_MEMORY_PREFETCH) {
        if (output->kind == HL_A64_MEMORY_LITERAL)
            output->read = 1;
        else if (output->kind == HL_A64_MEMORY_UNSIGNED || output->kind == HL_A64_MEMORY_UNSCALED ||
            ...
            output->read = output->vector
                ? (uint8_t)(((word >> 22) & 1u) != 0)
    ...
    output->stolen = (uint8_t)(stolen(output->base) || (!output->vector && stolen(output->target)) ||
                                (output->kind == HL_A64_MEMORY_PAIR && !output->vector && stolen(output->target2)) ||
                                ((output->kind == HL_A64_MEMORY_REGISTER ||
                                  (output->kind == HL_A64_MEMORY_STRUCTURE && output->writeback &&
                                   output->index != 31)) && stolen(output->index)) ||
                                (output->kind == HL_A64_MEMORY_ATOMIC && stolen(output->index)));
    return 1;
}
