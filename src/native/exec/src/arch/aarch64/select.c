...
int hl_a64_select_body(hl_a64_assembler *assembler, uint32_t word) {
    ...
    if (stolen(preferred)) hl_a64_ldr(assembler, 16, CPU, (int)preferred * 8);
    if (stolen(alternative)) hl_a64_ldr(assembler, 17, CPU, (int)alternative * 8);
    hl_a64_emit32(assembler, (word & ~((31u << 16) | (31u << 5) | 31u)) |
                             (native_alternative << 16) | (native_preferred << 5) | native_destination);
    if (destination != 31 && stolen(destination))
        hl_a64_str(assembler, 16, CPU, (int)destination * 8);
    return hl_a64_assembler_ok(assembler);
}

int hl_a64_select_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_SELECT_MAX_BYTES || !valid(word)) return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_select_body(assembler, word)) return 0;
