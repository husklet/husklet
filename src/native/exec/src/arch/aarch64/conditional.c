...
static void patch(hl_a64_assembler *assembler, uint32_t *instruction,
                  const uint8_t *target, uint32_t word, predicate_kind kind) {
  uint32_t distance;
  if (target == NULL || !hl_a64_assembler_wrote(assembler, instruction))
    return;
...
int hl_a64_conditional_chain(hl_a64_assembler *assembler, uint32_t word,
                             uint64_t pc, uint32_t **fall_patch,
                             uint64_t *fall_target, uint32_t **taken_patch,
                             uint64_t *taken_target) {
  predicate_kind kind;
  int64_t displacement;
  uint32_t *branch;
  if (assembler == NULL || !decode(word, &kind, &displacement))
    return 0;

  if (kind != PREDICATE_CONDITION)
    source(assembler, word & 31u);
  ...
  if (taken_patch != NULL)
    *taken_patch = taken_reservation;
  if (taken_target != NULL)
    *taken_target = pc + (uint64_t)displacement;
  hl_a64_stub_exit(assembler, HL_NATIVE_EXIT_BRANCH,
                   pc + (uint64_t)displacement);
  return hl_a64_assembler_ok(assembler);
}

int hl_a64_conditional_stitch(hl_a64_assembler *assembler, uint32_t word,
                              uint64_t pc, uint32_t **taken_patch,
                              uint64_t *taken_target, uint32_t **refund_patch) {
  predicate_kind kind;
  int64_t displacement;
  uint32_t *branch;
  uint32_t inverted;
  ...
  if (kind != PREDICATE_CONDITION && stolen(word & 31u))
    return 0;
  ...
  if (refund_patch != NULL)
    *refund_patch = (uint32_t *)assembler->cursor;
...
int hl_a64_conditional_body(hl_a64_assembler *assembler, uint32_t word,
                            uint64_t pc) {
  return hl_a64_conditional_chain(assembler, word, pc, NULL, NULL, NULL, NULL);
}

int hl_a64_conditional_emit(hl_a64_assembler *assembler, uint32_t word,
                            uint64_t pc) {
  predicate_kind kind;
  int64_t displacement;
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_CONDITIONAL_MAX_BYTES ||
      !decode(word, &kind, &displacement))
    return 0;
  (void)kind;
  (void)displacement;
