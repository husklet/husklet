...
int hl_a64_divide_body(hl_a64_assembler *assembler, uint32_t word) {
  ...
  if (assembler == NULL || !valid(word))
    return 0;
  if (stolen(dividend))
    hl_a64_ldr(assembler, 16, CPU, (int)dividend * 8);
  if (stolen(divisor))
    hl_a64_ldr(assembler, 17, CPU, (int)divisor * 8);
  hl_a64_emit32(assembler, (word & ~((31u << 16) | (31u << 5) | 31u)) |
                               (native_divisor << 16) | (native_dividend << 5) |
                               native_destination);
...
int hl_a64_divide_emit(hl_a64_assembler *assembler, uint32_t word,
                       uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_DIVIDE_MAX_BYTES ||
      !valid(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_divide_body(assembler, word))
    return 0;
