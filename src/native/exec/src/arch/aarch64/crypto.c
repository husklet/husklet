...
static enum crypto_feature feature(uint32_t word) {
  if ((word & UINT32_C(0xff3e0c00)) == UINT32_C(0x4e280800)) {
    ...
    return ((word >> 22) & 3u) == 0 && opcode >= 4u && opcode <= 7u
               ? CRYPTO_AES
               : CRYPTO_NONE;
  }
...
int hl_a64_crypto_host_supports(uint32_t word) {
  ...
  if (required == CRYPTO_NONE)
    return 0;
#if defined(__linux__) && defined(__aarch64__)
  unsigned long capabilities = getauxval(AT_HWCAP);
  if (required == CRYPTO_AES)
    return (capabilities & HWCAP_AES) != 0;
  if (required == CRYPTO_SHA1)
    return (capabilities & HWCAP_SHA1) != 0;
  return (capabilities & HWCAP_SHA2) != 0;
#elif defined(__APPLE__) && defined(__aarch64__)
  if (required == CRYPTO_AES)
    return apple_feature("hw.optional.arm.FEAT_AES");
  if (required == CRYPTO_SHA1)
    return apple_feature("hw.optional.arm.FEAT_SHA1");
...
int hl_a64_crypto_body(hl_a64_assembler *assembler, uint32_t word) {
  if (assembler == NULL || !hl_a64_crypto_host_supports(word))
    return 0;
  hl_a64_emit32(assembler, word);
  return hl_a64_assembler_ok(assembler);
}

int hl_a64_crypto_emit(hl_a64_assembler *assembler, uint32_t word,
                       uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_CRYPTO_MAX_BYTES ||
      !hl_a64_crypto_host_supports(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_crypto_body(assembler, word))
    return 0;
