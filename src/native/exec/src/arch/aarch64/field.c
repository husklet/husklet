...
int hl_a64_field_body(hl_a64_assembler *assembler, uint32_t word) {
    ...
    if (extract && stolen(right)) hl_a64_ldr(assembler, 17, CPU, (int)right * 8);
...
int hl_a64_field_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_FIELD_MAX_BYTES || !valid(word)) return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_field_body(assembler, word)) return 0;
