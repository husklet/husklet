...
static int source_byte(const hl_a64_source *source, uint64_t guest, uint8_t *value) {
    for (size_t index = 0; index < source->span_count; index++) {
        const hl_a64_source_span *span = &source->spans[index];
...
size_t hl_a64_source_available(const hl_a64_source *source, uint64_t pc, size_t limit) {
    size_t count = 0;
    uint64_t instruction = pc;
    uint8_t byte;
    if (!hl_a64_source_validate(source) || limit == 0 || limit > HL_A64_SOURCE_MAX_WORDS ||
        (pc & 3u) != 0)
        return 0;
    while (count < limit && instruction <= UINT64_MAX - 4) {
        size_t index;
        for (index = 0; index < 4; index++)
            if (!source_byte(source, instruction + index, &byte)) break;
        if (index != 4) break;
        count++;
        instruction += 4;
    }
...
int hl_a64_source_fetch(const hl_a64_source *source, uint64_t pc, size_t count, hl_a64_fetch_result *output) {
    if (output == NULL) return 0;
    memset(output, 0, sizeof(*output));
    output->fault_pc = pc;
    if (!hl_a64_source_validate(source) || count == 0 || count > HL_A64_SOURCE_MAX_WORDS ||
        (pc & 3u) != 0 || pc > UINT64_MAX - count * 4)
        return 0;
    ...
    for (size_t word = 0; word < count; word++) {
        ...
        for (size_t byte = 0; byte < sizeof(bytes); byte++)
            if (!source_byte(source, word_pc + byte, &bytes[byte])) return 0;
