...
static int stack_operand(uint32_t word, unsigned fields) {
    if ((word & 0x1f000000u) != 0x11000000u && (word & 0x1f200000u) != 0x0b200000u) return 0;
    if ((fields & 1u) != 0 && (word & 31u) == 31u) return 1;
    if ((fields & 2u) != 0 && ((word >> 5) & 31u) == 31u) return 1;
    return (fields & 4u) != 0 && ((word >> 16) & 31u) == 31u;
}

static int field_reserved(uint32_t word, unsigned fields) {
    if ((fields & 1u) != 0 && reserved_register(word & 31u)) return 1;
    if ((fields & 2u) != 0 && reserved_register((word >> 5) & 31u)) return 1;
    if ((fields & 4u) != 0 && reserved_register((word >> 16) & 31u)) return 1;
    return (fields & 8u) != 0 && reserved_register((word >> 10) & 31u);
}

static unsigned immediate_fields(uint32_t word) {
    if ((word & 0x1f000000u) == 0x10000000u) return 0; /* ADR/ADRP need guest-PC relocation. */
    if ((word & 0x1f800000u) == 0x12800000u) return 1; /* move wide */
    if ((word & 0x1f800000u) == 0x13800000u) return 1 | 2 | 4; /* EXTR */
    return 1 | 2;
}

static unsigned register_fields(uint32_t word) {
    if ((word & 0x1f000000u) == 0x1b000000u) return 1 | 2 | 4 | 8; /* three source */
    if ((word & 0x5fe00000u) == 0x5ac00000u) return 1 | 2; /* one source */
...
static int integer_immediate(uint32_t word, unsigned *fields) {
    if ((word & 0x1f000000u) != 0x11000000u && (word & 0x1f800000u) != 0x12000000u &&
        (word & 0x1f800000u) != 0x12800000u && (word & 0x1f800000u) != 0x13000000u &&
...
static hl_a64_control register_control(uint32_t word) {
    if ((word & 0xfffffc1fu) == 0xd61f0000u) return HL_A64_INDIRECT_BRANCH;
    if ((word & 0xfffffc1fu) == 0xd63f0000u) return HL_A64_INDIRECT_CALL;
    if ((word & 0xfffffc1fu) == 0xd65f0000u) return HL_A64_RETURN;
...
static uint64_t predicate_target(uint32_t word, uint64_t guest_pc, unsigned bits) {
...
static uint64_t pcrel_value(uint32_t word, uint64_t guest_pc) {
    uint64_t immediate = ((uint64_t)((word >> 5) & 0x7ffffu) << 2) | ((word >> 29) & 3u);
...
static int integer_register(uint32_t word, unsigned *fields) {
    if ((word & 0x1f200000u) != 0x0b000000u && (word & 0x1f200000u) != 0x0b200000u &&
        (word & 0x1fe0fc00u) != 0x1a000000u && (word & 0x1f000000u) != 0x0a000000u &&
        (word & 0x1f000000u) != 0x1b000000u && (word & 0x7ffffc00u) != 0x5ac00000u &&
        !((word & 0x7fff0000u) == 0x5ac00000u && ((word >> 10) & 0x3fu) >= 1u &&
          ((word >> 10) & 0x3fu) <= 3u) &&
        (word & 0x7ffffc00u) != 0x5ac01000u && (word & 0x1fe0f000u) != 0x1ac02000u &&
        (word & 0x1fe0f800u) != 0x1ac00800u && (word & 0x1fe00000u) != 0x1a400000u &&
...
static hl_a64_terminal classify(uint32_t word) {
    if ((word & 0xffe0001fu) == 0xd4000001u) return HL_A64_SYSCALL;
...
static void emit_movconst(uint8_t *output, unsigned destination, uint64_t value) {
    unsigned half;
    emit_word(output, UINT32_C(0xd2800000) | (uint32_t)((value & 0xffffu) << 5) | destination);
...
int hl_a64_emit_block(const hl_a64_block_input *input, hl_a64_block_output *output) {
    size_t index;
    size_t required_code = 0;
    if (output == NULL) return 0;
    memset(output, 0, sizeof(*output));
    output->terminal = HL_A64_UNSUPPORTED;
    if (input == NULL || input->instructions == NULL || input->code == NULL || input->provenance == NULL ||
        input->instruction_count == 0 || input->instruction_count > HL_A64_BLOCK_MAX_INSTRUCTIONS ||
        input->instruction_count > SIZE_MAX / (A64_WORD_BYTES * A64_MOVCONST_WORDS) ||
        input->provenance_capacity < input->instruction_count ||
        input->instructions[0].guest_pc > UINT64_MAX - (input->instruction_count - 1) * A64_WORD_BYTES)
        return 0;
    for (index = 0; index < input->instruction_count; index++) {
        uint64_t expected = input->instructions[0].guest_pc + index * A64_WORD_BYTES;
        if (input->instructions[index].reserved != 0 || input->instructions[index].guest_pc != expected ||
    ...
    for (index = 0; index < input->instruction_count; index++) {
        ...
        } else if (integer_immediate(word, &fields) || integer_register(word, &fields)) {
            words = 1;
        } else {
            break;
        }
        if (field_reserved(word, fields) || stack_operand(word, fields)) break;
        required_code += words * A64_WORD_BYTES;
    }
    if (input->code_capacity < required_code || input->host_base > UINT64_MAX - required_code) return 0;

    for (index = 0; index < input->instruction_count; index++) {
        const hl_a64_instruction *instruction = &input->instructions[index];
        unsigned fields = 0;
        int pcrel = pcrel_integer(instruction->word);
        int copy = pcrel || integer_immediate(instruction->word, &fields) || integer_register(instruction->word, &fields);
        size_t emitted = pcrel ? A64_WORD_BYTES * A64_MOVCONST_WORDS : A64_WORD_BYTES;
        if (pcrel) fields = 1;
        if (!copy || field_reserved(instruction->word, fields) || stack_operand(instruction->word, fields)) {
            ...
            if (immediate_branch(instruction->word)) {
                ...
                output->control_target = branch_target(instruction->word, instruction->guest_pc);
                ...
                output->control_target = predicate_target(instruction->word, instruction->guest_pc, 19u);
                output->control_fallthrough = instruction->guest_pc + A64_WORD_BYTES;
                output->control_condition = (uint8_t)(instruction->word & 15u);
            } else if ((compare_branch(instruction->word) || test_branch(instruction->word)) &&
                ...
                output->control_target =
                    predicate_target(instruction->word, instruction->guest_pc, compare ? 19u : 14u);
                output->control_fallthrough = instruction->guest_pc + A64_WORD_BYTES;
                ...
                if (compare) {
                    ...
                    output->control_bit =
                        (uint8_t)(((instruction->word >> 31) << 5) | ((instruction->word >> 19) & 31u));
                    output->control_wide = (uint8_t)(output->control_bit >= 32u);
                }
            } else if (indirect != HL_A64_CONTROL_NONE && !reserved_register((instruction->word >> 5) & 31u)) {
                output->terminal = HL_A64_CONTROL;
        ...
        input->provenance[index].host_last = input->provenance[index].host_first + emitted;
