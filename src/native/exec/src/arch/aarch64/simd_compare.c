...
static int zero_form(uint32_t word) {
  ...
  if ((word & 0x9F3E0C00u) != 0x0E200800u)
    return 0;
  if (opcode < 0x08u || opcode > 0x0Au || (u && opcode == 0x0Au))
    return 0;
  return size != 3u || wide;
}
...
int hl_a64_simd_compare_emit(hl_a64_assembler *assembler, uint32_t word,
                             uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_SIMD_COMPARE_MAX_BYTES ||
      !valid(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_simd_compare_body(assembler, word))
    return 0;
