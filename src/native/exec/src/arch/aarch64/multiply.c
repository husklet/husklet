...
int hl_a64_multiply_body(hl_a64_assembler *assembler, uint32_t word) {
  ...
  unsigned native_destination =
      destination != 31 && (stolen(destination) || destination == 9)
          ? 16u
          : destination;
  uint32_t native;
...
int hl_a64_multiply_emit(hl_a64_assembler *assembler, uint32_t word,
                         uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_MULTIPLY_MAX_BYTES ||
      !valid(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_multiply_body(assembler, word))
    return 0;
