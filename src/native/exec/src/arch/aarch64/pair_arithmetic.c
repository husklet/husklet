...
static int valid(uint32_t word) {
  ...
  if (opcode == 0x17u)
    return !u && (q || size != 3u);
  if (opcode == 0x14u || opcode == 0x15u)
    return size != 3u;
  if (!u)
    return 0;
  if (opcode == 0x1au)
    return size < 2u && (size == 0u || q);
  if (opcode == 0x18u || opcode == 0x1eu)
    return (size & 1u) == 0u || q;
  return 0;
}
...
int hl_a64_pair_arithmetic_emit(hl_a64_assembler *assembler, uint32_t word,
                                uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) <
          HL_A64_PAIR_ARITHMETIC_MAX_BYTES ||
      !valid(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_pair_arithmetic_body(assembler, word))
    return 0;
