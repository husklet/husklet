...
static int valid(uint32_t word) {
  ...
  if ((word & UINT32_C(0x9ff80400)) != UINT32_C(0x0f000400))
    return 0;
  ...
  if (selector != 7u)
    return o2 == 0u;
  if ((low == 0u || operation != 0u) && o2 != 0u)
    return 0;
  if (low != 0u && operation != 0u)
    return wide != 0u;
  return 1;
}
...
int hl_a64_simd_immediate_emit(hl_a64_assembler *assembler, uint32_t word,
                               uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_SIMD_IMMEDIATE_MAX_BYTES ||
      !valid(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_simd_immediate_body(assembler, word))
    return 0;
