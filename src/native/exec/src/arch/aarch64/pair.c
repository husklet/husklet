...
int hl_a64_pair_body(hl_a64_assembler *assembler, uint32_t word, uint64_t pc,
  ...
  *guard = (hl_a64_guard){.pc = pc};
  ...
  writeback =
      sign_extend((word >> 15) & 0x7fu, 7) * (int64_t)(memory.bytes / 2);
  ...
  native = word & ~((UINT32_C(0x7f) << 15) | (3u << 23) | (31u << 10) |
                    (31u << 5) | 31u);
  native |= (2u << 23) | (16u << 5) | first_native | (second_native << 10);
  if (!memory.read)
  ...
  if (!memory.read) {
    ...
    if (second_stolen)
      hl_a64_ldr(assembler, (int)second_native, CPU, memory.target2 * 8);
  }
  if (sites != NULL) {
    sites->count = 1;
  ...
  if (memory.read) {
    ...
    if (second_stolen)
      hl_a64_str(assembler, (int)second_native, CPU, memory.target2 * 8);
  }
  if (memory.writeback) {
...
int hl_a64_pair_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
  ...
  hl_a64_guard guard;
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_PAIR_MAX_BYTES ||
      !valid(word, pc, &memory))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_pair_body(assembler, word, pc, &guard, NULL))
    return 0;
