...
static int record_valid(const hl_native_provenance *record) {
    if (record == NULL || record->code_size != sizeof(uint32_t) ||
        (record->guest & 3u) != 0 ||
        (record->access != HL_NATIVE_ACCESS_READ && record->access != HL_NATIVE_ACCESS_WRITE) ||
...
int hl_a64_fault_reconstruct(hl_native_aarch64_cpu *cpu, const hl_a64_host_context *host,
                             const hl_native_provenance *record) {
    if (cpu == NULL || host == NULL || host->vectors_valid != 1 || host->reserved != 0 ||
        !record_valid(record) || host->registers[28] != (uint64_t)(uintptr_t)cpu)
        return 0;
...
int hl_a64_fault_prepare(hl_native_aarch64_cpu *cpu, const hl_a64_host_context *host,
                         const hl_native_provenance *record, uint64_t host_fault) {
    ...
    if (record->address.displacement >= 0) {
        ...
        if (host->registers[16] > UINT64_MAX - displacement) return 0;
        access_first = host->registers[16] + displacement;
    } else {
        uint64_t displacement = (uint64_t)(-(record->address.displacement + 1)) + 1;
        if (host->registers[16] < displacement) return 0;
        access_first = host->registers[16] - displacement;
    }
    if (!width_valid(record->width) || access_first > UINT64_MAX - record->width ||
        host_fault < access_first || host_fault >= access_first + record->width)
        return 0;
    if (!hl_a64_fault_reconstruct(cpu, host, record)) return 0;
...
int hl_a64_linux_context(const void *opaque, hl_a64_host_context *output) {
    if (opaque == NULL || output == NULL) return 0;
    const ucontext_t *context = opaque;
    hl_a64_host_context captured = {0};
    memcpy(captured.registers, context->uc_mcontext.regs, sizeof(captured.registers));
    ...
    while ((size_t)(end - cursor) >= sizeof(struct _aarch64_ctx)) {
        ...
        if (header.magic == FPSIMD_MAGIC) {
            if (header.size < sizeof(struct fpsimd_context)) return 0;
...
int hl_a64_linux_fault_reconstruct(hl_native_aarch64_cpu *cpu, const void *context,
                                   const hl_native_provenance *record) {
    hl_a64_host_context host;
    return hl_a64_linux_context(context, &host) && hl_a64_fault_reconstruct(cpu, &host, record);
}

#if defined(__linux__) && defined(__aarch64__)
int hl_a64_linux_fault_return(hl_native_aarch64_cpu *cpu, void *opaque,
                              const hl_native_provenance *record, uint64_t host_fault) {
    if (cpu == NULL || opaque == NULL || record == NULL) return 0;
    hl_a64_host_context host;
    if (!hl_a64_linux_context(opaque, &host) ||
        !hl_a64_fault_prepare(cpu, &host, record, host_fault)) return 0;
