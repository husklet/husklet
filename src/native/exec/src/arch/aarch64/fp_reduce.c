...
static int valid(uint32_t word) {
    ...
    if ((word & UINT32_C(0xef3e0c00)) != UINT32_C(0x6e300800)) return 0;
    if (opcode != 0x0cu && opcode != 0x0fu) return 0;
...
int hl_a64_simd_fp_reduce_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_SIMD_FP_REDUCE_MAX_BYTES ||
        !valid(word))
        return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_simd_fp_reduce_body(assembler, word)) return 0;
