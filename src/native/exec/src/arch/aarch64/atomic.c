...
static int atomic_memory(uint32_t word) {
    if ((word & UINT32_C(0x3f200c00)) != UINT32_C(0x38200000)) return 0;
...
int hl_a64_atomic_host_supports(void) {
...
#elif defined(__APPLE__) && defined(__aarch64__)
    {
        ...
        value = sysctlbyname("hw.optional.arm.FEAT_LSE", &present, &size, NULL, 0) == 0 &&
...
int hl_a64_atomic_definitions(uint32_t word, uint32_t *definitions) {
    atomic_form form;
    if (definitions == NULL || !decode(word, &form)) return 0;
    *definitions = 0;
    if (form.writes_source) *definitions |= UINT32_C(1) << form.source;
    if (form.writes_target) *definitions |= UINT32_C(1) << form.target;
    return 1;
}
...
int hl_a64_atomic_body(hl_a64_assembler *assembler, uint32_t word, uint64_t pc,
    ...
    if (form.target != 31u && stolen(form.target)) return 0;
    ...
    if (source_stolen) hl_a64_ldr(assembler, 17, CPU, (int)form.source * 8);
    if (sites != NULL) {
        sites->count = 1;
    ...
    hl_a64_emit32(assembler, (word & ~((31u << 16) | (31u << 5) | 31u)) |
                                 (native_source << 16) | (16u << 5) | form.target);
...
int hl_a64_atomic_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    hl_a64_guard guard;
    atomic_form form;
    if (assembler == NULL ||
        hl_a64_assembler_remaining(assembler) < HL_A64_ATOMIC_MAX_BYTES ||
        !decode(word, &form) || !hl_a64_atomic_host_supports() ||
        (form.target != 31u && stolen(form.target))) return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_atomic_body(assembler, word, pc, &guard, NULL)) return 0;
