...
int hl_a64_direct_body(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    return hl_a64_direct_chain(assembler, word, pc, NULL, NULL);
}

int hl_a64_direct_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_DIRECT_MAX_BYTES || !valid(word)) return 0;
