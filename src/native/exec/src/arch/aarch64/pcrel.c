...
int hl_a64_pcrel_body(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    ...
    if (destination != 31) hl_a64_movconst(assembler, (int)native_destination, value);
...
int hl_a64_pcrel_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_PCREL_MAX_BYTES || !valid(word)) return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_pcrel_body(assembler, word, pc)) return 0;
