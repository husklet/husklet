...
static int immediate(uint32_t word) { return (word & 0x1F800000u) == 0x11000000u; }

static int shifted(uint32_t word) { return (word & 0x1F200000u) == 0x0B000000u; }
...
int hl_a64_add_body(hl_a64_assembler *assembler, uint32_t word) {
    ...
    if (shifted(word)) return shifted_body(assembler, word);
    if (!immediate(word)) return 0;
...
int hl_a64_add_emit(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
    if (assembler == NULL || hl_a64_assembler_remaining(assembler) < HL_A64_ADD_MAX_BYTES ||
        (!immediate(word) && !shifted(word))) return 0;
    hl_a64_stub_prologue(assembler);
    if (!hl_a64_add_body(assembler, word)) return 0;
