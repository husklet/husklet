...
| 6 | Mutation/fork retirement of cached backing | Registry pins CPU identity; stop-the-world clears tuples before reclamation and refreshes after publication | The run lease pins backing, and the release-published `read_token`/`read_incarnation` window qualifies every cached view by mapping incarnation | **Implemented.** The removed `certificate_valid/delta` pair carried no such identity. |
