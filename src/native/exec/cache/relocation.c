...
static uint32_t cold_word(const hl_native_relocation *relocation,
                          uint32_t index) {
  return relocation->span.word_count == 0 ? relocation->expected
                                          : relocation->span.cold[index];
}

static int span_matches(const uint32_t *words,
                        const hl_native_relocation *relocation,
                        uint32_t first) {
...
static int hot_span_matches(const uint32_t *words,
                            const resolved_relocation *resolved) {
  uint32_t count = span_words(&resolved->relocation);
  for (uint32_t index = 0; index < count; index++)
    if (words[index] != resolved->patched[index])
      return 0;
  return 1;
}
...
static uint32_t a64_branch(uint64_t source, uint64_t target) {
  int64_t displacement = target >= source ? (int64_t)((target - source) / 4)
...
static uint32_t a64_edge_refund(const uint32_t *cold, uint32_t *refund) {
  if ((cold[0] & UINT32_C(0xffc003ff)) != UINT32_C(0x910003de))
    return 0;
...
static int a64_edge_admission(uint32_t *hot, uint64_t source_offset,
  ...
  int64_t displacement =
      target->admitted_offset >= branch_offset
  ...
  if (count == 0 || count > UINT32_C(0xfff) ||
      charge > (int64_t)UINT32_C(0xfff) || charge < -(int64_t)UINT32_C(0xfff) ||
      ((target->admitted_offset | branch_offset) & 3u) != 0 ||
      displacement < -(1 << 25) || displacement >= (1 << 25))
    return 0;
  hot[0] = UINT32_C(0xf9400000) |
           ((uint32_t)(offsetof(hl_native_aarch64_cpu, interrupt) / 8) << 10) |
           (28u << 5) | 16u;
  hot[1] = a64_imm19(UINT32_C(0xb5000010),
                     (int64_t)cold_word_index - 1); /* cbnz x16,cold */
  hot[2] =
      UINT32_C(0xf9400000) |
      ((uint32_t)(offsetof(hl_native_aarch64_cpu, interrupt_token) / 8) << 10) |
      (28u << 5) | 16u;
  hot[3] = a64_imm19(UINT32_C(0xb4000010), 3); /* cbz x16,budget */
  hot[4] = UINT32_C(0xc8dffe10);               /* ldar x16,[x16] */
  hot[5] = a64_imm19(UINT32_C(0xb5000010),
                     (int64_t)cold_word_index - 5); /* cbnz x16,cold */
  hot[6] = charge >= 0
               ? UINT32_C(0xd10003d1) |
                     ((uint32_t)charge << 10) /* sub x17,x30,#charge */
               : UINT32_C(0x910003d1) |
                     ((uint32_t)(-charge) << 10); /* add x17,x30,#-charge */
  hot[7] = a64_imm14(UINT32_C(0xb7f80011),
                     (int64_t)cold_word_index - 7); /* tbnz x17,#63,cold */
  ...
  for (uint32_t index = 11; index < HL_NATIVE_RELOCATION_SPAN_WORDS; index++)
    hot[index] =
        UINT32_C(0xd503201f); /* padding the taken edge never reaches */
  if (stitched)
    hot[11] = saved;
  return 1;
}

static void span_restore(uint32_t *words,
                         const hl_native_relocation *relocation) {
  uint32_t count = span_words(relocation);
  for (uint32_t index = 0; index < count; index++)
    words[index] = cold_word(relocation, index);
}
...
static int cycle_requires_exit(const hl_native_cache *cache,
                               const cache_entry *source,
                               const cache_entry *target) {
  cycle_node frontier[CYCLE_FRONTIER_CAPACITY] = {
      {target->guest, target->instruction_epoch}};
  ...
  while (consumed < count) {
    ...
    if (current_slot < 0 ||
        cache->entries[current_slot].instruction_epoch != current.epoch ||
        cache->entries[current_slot].cycle_safe == 0)
      safe = 0;
    if (same_node(current, destination))
      closes = 1;
    for (uint32_t index = 0; index < cache->resolved_count; ++index) {
      const resolved_relocation *edge = &cache->resolved[index];
      if (edge->generation != cache->generation ||
          edge->source_guest != current.guest ||
          edge->source_instruction_epoch != current.epoch)
        continue;
      cycle_node next = {edge->relocation.target_guest,
                         edge->relocation.target_instruction_epoch};
      uint32_t node = 0;
      while (node < count && !same_node(frontier[node], next))
        node++;
      if (node < count)
        continue;
      if (count == CYCLE_FRONTIER_CAPACITY)
        return 1;
      frontier[count++] = next;
    }
  }
...
static hl_native_status
patch(hl_native_cache *cache, const cache_entry *source,
  ...
  if (!span_matches(word, relocation, cold_word(relocation, 0)))
    return HL_NATIVE_STATE;
  if ((target->body_offset | source_offset) % 4 != 0)
    return HL_NATIVE_ARGUMENT;
  ...
  for (uint32_t index = 0; index < words; index++)
    patched[index] = cold_word(relocation, index);
  if (words == HL_NATIVE_RELOCATION_SPAN_WORDS) {
    if (!a64_edge_admission(patched, source_offset, target))
      return HL_NATIVE_ARGUMENT;
  } else {
    patched[0] =
        UINT32_C(0x14000000) | ((uint32_t)displacement & UINT32_C(0x03ffffff));
  }
  for (uint32_t index = 0; index < words; index++)
    word[index] = patched[index];
  hl_native_status status = cache->arena->memory.publish(
      cache->arena->memory.context, cache->arena->mapping.handle, source_offset,
      words * 4u);
...
static hl_native_status
remember(hl_native_cache *cache, const cache_entry *source,
         const cache_entry *target, const hl_native_relocation *relocation,
  ...
  cache->resolved[cache->resolved_count - 1]
      .relocation.target_instruction_count = target->instruction_count;
  return HL_NATIVE_OK;
}
...
hl_native_status
hl_native_cache_relocate_site(hl_native_cache *cache, const void *site,
  ...
  if (patched != NULL)
    *patched = 0;
  if (cache == NULL || site == NULL || !cache->arena->writing ||
      (branch_delta & 3) != 0)
    return HL_NATIVE_ARGUMENT;
  ...
  if ((site_offset & 15u) != 0 ||
      (branch_delta < 0 &&
       UINT64_C(0) - (uint64_t)branch_delta > site_offset) ||
      (branch_delta >= 0 &&
       (uint64_t)branch_delta > cache->arena->mapping.content - site_offset))
    return HL_NATIVE_STATE;
  branch_offset = branch_delta < 0
                      ? site_offset - (UINT64_C(0) - (uint64_t)branch_delta)
                      : site_offset + (uint64_t)branch_delta;
  if ((branch_offset & 3u) != 0 ||
      branch_offset > cache->arena->mapping.content - 4)
    return HL_NATIVE_STATE;
  ...
  for (uint32_t index = 0; index < cache->resolved_count; index++) {
    ...
    if (old.source_guest != source->guest ||
        old.source_instruction_epoch != source->instruction_epoch ||
        old.source_code_offset != source->code_offset ||
        old.relocation.code_offset != branch_offset - source->code_offset)
      continue;
    int old_target_slot =
        hl_native_cache_find(cache, old.relocation.target_guest);
    int old_target_valid =
        old_target_slot >= 0 &&
        cache->entries[old_target_slot].certificate_identity ==
            old.target_certificate_identity;
    if (!old_target_valid)
    ...
    if (!hot_span_matches(word, &old))
      return HL_NATIVE_STATE;
    span_restore(word, &old.relocation);
    hl_native_status status = cache->arena->memory.publish(
        cache->arena->memory.context, cache->arena->mapping.handle,
        branch_offset, span_words(&old.relocation) * 4u);
    if (status != HL_NATIVE_OK)
      return status;
  ...
  if (target_slot < 0 ||
      cache->entries[target_slot].instruction_epoch != target_epoch ||
  ...
  hl_native_status status =
      patch(cache, source, &cache->entries[target_slot], &relocation, value);
  if (status == HL_NATIVE_CAPACITY)
    return HL_NATIVE_OK;
  if (status == HL_NATIVE_OK)
    status = remember(cache, source, &cache->entries[target_slot], &relocation,
                      value, 0);
  if (status == HL_NATIVE_OK && patched != NULL)
    *patched = 1;
  return status;
}
...
hl_native_status hl_native_cache_relocations_invalidate(hl_native_cache *cache,
  ...
  for (uint32_t index = 0; index < cache->relocation_count; index++) {
    ...
    if (source != NULL &&
        source->instruction_epoch == pending.source_instruction_epoch &&
  ...
  for (uint32_t index = 0; index < cache->resolved_count; index++) {
    ...
    int target_slot =
        hl_native_cache_find(cache, resolved.relocation.target_guest);
    cache_entry *source = source_slot < 0 ? NULL : &cache->entries[source_slot];
    cache_entry *target = target_slot < 0 ? NULL : &cache->entries[target_slot];
    int source_removed =
        source == NULL ||
        source->instruction_epoch != resolved.source_instruction_epoch ||
        source->code_offset != resolved.source_code_offset ||
        source->certificate_identity != resolved.source_certificate_identity ||
        overlaps(source, first, last);
    int target_removed =
        target == NULL ||
        target->instruction_epoch !=
            resolved.relocation.target_instruction_epoch ||
        target->certificate_identity != resolved.target_certificate_identity ||
        overlaps(target, first, last);
    ...
    if (source != NULL &&
      ...
      hl_native_status status = cache->arena->memory.publish(
          cache->arena->memory.context, cache->arena->mapping.handle, offset,
          words * 4u);
      if (status != HL_NATIVE_OK) {
        cache->poisoned = 1;
        return status;
...
hl_native_status
hl_native_cache_relocate(hl_native_cache *cache, uint64_t source_guest,
                         uint64_t source_instruction_epoch,
                         const hl_native_relocation *relocations,
                         uint32_t count) {
  int source_slot;
  if (cache == NULL || relocations == NULL || count == 0 ||
      count > cache->relocation_capacity)
    return HL_NATIVE_ARGUMENT;
  if (!cache->arena->writing)
    return HL_NATIVE_STATE;
  source_slot = hl_native_cache_find(cache, source_guest);
  if (source_slot < 0)
    return HL_NATIVE_STATE;
  cache_entry *source = &cache->entries[source_slot];
  if (source->instruction_epoch != source_instruction_epoch)
    return HL_NATIVE_STATE;
  for (uint32_t index = 0; index < count; index++) {
    ...
    if (target_slot >= 0 &&
        cache->entries[target_slot].conditional_self_loop != 0)
      continue;
    if (target_slot >= 0 &&
        cycle_requires_exit(cache, source, &cache->entries[target_slot])) {
      hl_native_cache_observe(cache, HL_NATIVE_CACHE_RELOCATION_CYCLE);
      continue;
    }
    if (target_slot >= 0 && (!relocation->target_epoch_known ||
                             cache->entries[target_slot].instruction_epoch ==
                                 relocation->target_instruction_epoch)) {
      hl_native_relocation bound = *relocation;
      bound.target_instruction_epoch =
          cache->entries[target_slot].instruction_epoch;
      ...
      hl_native_status status =
          patch(cache, source, &cache->entries[target_slot], &bound, patched);
      if (status == HL_NATIVE_OK)
        status = remember(cache, source, &cache->entries[target_slot], &bound,
                          patched, !relocation->target_epoch_known);
      if (status != HL_NATIVE_OK)
        return status;
      continue;
    }
...
hl_native_status hl_native_cache_resolve(hl_native_cache *cache,
  ...
  target_slot = hl_native_cache_find(cache, target_guest);
  if (target_slot < 0 ||
      cache->entries[target_slot].instruction_epoch != target_instruction_epoch)
    return HL_NATIVE_STATE;
  if (cache->entries[target_slot].conditional_self_loop != 0) {
    for (uint32_t index = 0; index < cache->relocation_count; index++) {
      pending_relocation pending = cache->relocations[index];
  ...
  for (uint32_t index = 0; index < cache->relocation_count; index++) {
    ...
    if (pending.relocation.target_guest != target_guest ||
        (pending.relocation.target_epoch_known &&
         pending.relocation.target_instruction_epoch !=
             target_instruction_epoch)) {
      cache->relocations[retained++] = pending;
      continue;
    }
    int source_slot = hl_native_cache_find(cache, pending.source_guest);
    if (source_slot < 0 ||
        cache->entries[source_slot].instruction_epoch !=
            pending.source_instruction_epoch ||
        cache->entries[source_slot].code_offset != pending.source_code_offset ||
        cache->entries[source_slot].certificate_identity !=
            pending.source_certificate_identity)
      continue;
    if (cycle_requires_exit(cache, &cache->entries[source_slot],
                            &cache->entries[target_slot])) {
      hl_native_cache_observe(cache, HL_NATIVE_CACHE_RELOCATION_CYCLE);
      continue;
    }
    if (cache->resolved_count == cache->resolved_capacity) {
    ...
    uint32_t patched[HL_NATIVE_RELOCATION_SPAN_WORDS];
    hl_native_status status =
        patch(cache, &cache->entries[source_slot], &cache->entries[target_slot],
              &bound, patched);
    if (status == HL_NATIVE_OK)
      status = remember(cache, &cache->entries[source_slot],
