#include "../src/arch/aarch64/arithmetic.h"
#include "../src/arch/aarch64/entry.h"
#include "../include/executor.h"

#include <stdio.h>
#include <string.h>
#include <sys/mman.h>
#include <unistd.h>

#define CHECK(x) do { if (!(x)) { fprintf(stderr, "arithmetic:%d: %s\n", __LINE__, #x); return 1; } } while (0)
#define ARITH(sf, sub, flags, shift, rm, amount, rn, rd) \
    (((uint32_t)(sf) << 31) | ((uint32_t)(sub) << 30) | ((uint32_t)(flags) << 29) | 0x0b000000u | \
     ((uint32_t)(shift) << 22) | ((uint32_t)(rm) << 16) | ((uint32_t)(amount) << 10) | \
     ((uint32_t)(rn) << 5) | (rd))
#define EXT(sf, sub, flags, rm, option, amount, rn, rd) \
    (((uint32_t)(sf) << 31) | ((uint32_t)(sub) << 30) | ((uint32_t)(flags) << 29) | 0x0b200000u | \
     ((uint32_t)(rm) << 16) | ((uint32_t)(option) << 13) | ((uint32_t)(amount) << 10) | \
     ((uint32_t)(rn) << 5) | (rd))

static void execute(hl_native_aarch64_cpu *cpu, void *address) {
    void (*entry)(void);
    memcpy(&entry, &address, sizeof(entry));
    hl_native_aarch64_enter(cpu, entry);
}

int main(void) {
#if !defined(__aarch64__)
    return 0;
#else
    const unsigned stolen[] = {16, 17, 18, 28, 30};
    uint32_t words[15];
    size_t offsets[15];
    size_t count = 0;
    for (size_t i = 0; i < sizeof(stolen) / sizeof(stolen[0]); i++)
        words[count++] = ARITH(1, 0, 0, 0, 2, 0, 1, stolen[i]);
    words[count++] = ARITH(1, 0, 0, 0, 16, 0, 17, 18); /* swapped stolen sources */
    words[count++] = ARITH(0, 0, 0, 0, 3, 31, 2, 1);  /* add w1,w2,w3,lsl#31 */
    words[count++] = ARITH(1, 0, 0, 1, 3, 63, 2, 1);  /* lsr#63 */
    words[count++] = ARITH(1, 0, 0, 2, 3, 63, 2, 1);  /* asr#63 */
    words[count++] = ARITH(1, 1, 1, 0, 3, 0, 2, 1);   /* subs */
    words[count++] = ARITH(1, 1, 1, 0, 3, 0, 30, 31); /* cmp x30,x3 */
    words[count++] = ARITH(1, 0, 1, 0, 3, 0, 30, 31); /* cmn x30,x3 */
    words[count++] = ARITH(1, 0, 0, 0, 31, 0, 31, 31);/* add xzr,xzr,xzr */
    words[count++] = EXT(1, 1, 0, 27, 2, 0, 26, 14);  /* sub x14,x26,w27,uxtw */
    words[count++] = EXT(1, 0, 0, 17, 6, 1, 16, 18);  /* add x18,x16,w17,sxtw#1 */

    long page = sysconf(_SC_PAGESIZE);
    size_t capacity = (size_t)page * 2;
    uint8_t *code = mmap(NULL, capacity, PROT_READ | PROT_WRITE,
                         MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    hl_a64_assembler assembler;
    CHECK(code != MAP_FAILED && count == sizeof(words) / sizeof(words[0]));
    CHECK(hl_a64_assembler_begin(&assembler, code, code, capacity));
    for (size_t i = 0; i < count; i++) {
        offsets[i] = hl_a64_assembler_size(&assembler);
        CHECK(hl_a64_arithmetic_emit(&assembler, words[i], 0x8000 + i * 4));
    }
    CHECK(mprotect(code, capacity, PROT_READ | PROT_EXEC) == 0);

    hl_native_aarch64_cpu cpu = {0};
    cpu.registers[1] = 20;
    cpu.registers[2] = 22;
    for (size_t i = 0; i < sizeof(stolen) / sizeof(stolen[0]); i++) {
        execute(&cpu, code + offsets[i]);
        CHECK(cpu.registers[stolen[i]] == 42);
    }
    cpu.registers[17] = 9;
    cpu.registers[16] = 4;
    execute(&cpu, code + offsets[5]);
    CHECK(cpu.registers[18] == 13);
    cpu.registers[2] = 1;
    cpu.registers[3] = 1;
    execute(&cpu, code + offsets[6]);
    CHECK(cpu.registers[1] == UINT64_C(0x80000001));
    cpu.registers[2] = 5;
    cpu.registers[3] = UINT64_C(0x8000000000000000);
    execute(&cpu, code + offsets[7]);
    CHECK(cpu.registers[1] == 6);
    cpu.registers[3] = UINT64_MAX;
    execute(&cpu, code + offsets[8]);
    CHECK(cpu.registers[1] == 4);
    cpu.registers[2] = 0;
    cpu.registers[3] = 1;
    execute(&cpu, code + offsets[9]);
    CHECK(cpu.registers[1] == UINT64_MAX && cpu.flags == UINT64_C(0x80000000));
    cpu.registers[2] = UINT64_C(0x8000000000000000);
    execute(&cpu, code + offsets[9]);
    CHECK(cpu.registers[1] == UINT64_C(0x7fffffffffffffff));
    CHECK(cpu.flags == UINT64_C(0x30000000));
    cpu.registers[30] = 7;
    cpu.registers[3] = 7;
    execute(&cpu, code + offsets[10]);
    CHECK(cpu.flags == UINT64_C(0x60000000));
    cpu.registers[30] = UINT64_MAX;
    cpu.registers[3] = 1;
    execute(&cpu, code + offsets[11]);
    CHECK(cpu.flags == UINT64_C(0x60000000));
    cpu.registers[30] = UINT64_C(0x7fffffffffffffff);
    execute(&cpu, code + offsets[11]);
    CHECK(cpu.flags == UINT64_C(0x90000000));
    cpu.flags = UINT64_C(0xa0000000);
    execute(&cpu, code + offsets[12]);
    CHECK(cpu.flags == UINT64_C(0xa0000000));
    cpu.registers[26] = 100;
    cpu.registers[27] = UINT64_C(0xaaaa000000000003);
    execute(&cpu, code + offsets[13]);
    CHECK(cpu.registers[14] == 97);
    cpu.registers[16] = 10;
    cpu.registers[17] = UINT64_C(0xaaaaaaaaffffffff);
    execute(&cpu, code + offsets[14]);
    CHECK(cpu.registers[18] == 8);
    CHECK(munmap(code, capacity) == 0);

    uint8_t short_buffer[HL_A64_ARITHMETIC_MAX_BYTES - 1];
    memset(short_buffer, 0xa5, sizeof(short_buffer));
    CHECK(hl_a64_assembler_begin(&assembler, short_buffer, short_buffer, sizeof(short_buffer)));
    CHECK(!hl_a64_arithmetic_emit(&assembler, words[0], 0x9000));
    CHECK(hl_a64_assembler_size(&assembler) == 0);
    CHECK(hl_a64_assembler_begin(&assembler, short_buffer, short_buffer, sizeof(short_buffer)));
    CHECK(!hl_a64_arithmetic_emit(&assembler, ARITH(1, 0, 0, 3, 1, 0, 2, 3), 0x9000));
    CHECK(!hl_a64_arithmetic_emit(&assembler, ARITH(0, 0, 0, 0, 1, 32, 2, 3), 0x9000));
    CHECK(!hl_a64_arithmetic_emit(&assembler, EXT(1, 0, 0, 1, 2, 5, 2, 3), 0x9000));
    CHECK(hl_a64_assembler_size(&assembler) == 0);
    return 0;
#endif
}
