...
int main(void) {
    ...
    CHECK(setenv("HL_A64_DIRTY_OVERFLOW_CONTINUE", "1", 1) == 0);
    ...
    size_t reserve_offset = hl_a64_assembler_size(&assembler);
    hl_a64_stub_prologue(&assembler);
    hl_a64_ldr(&assembler, 16, 28, 16 * 8);
    hl_a64_guard reserve = {0};
    hl_a64_guard_write_begin(&assembler, 8, 0x5000, &reserve);
    /* Reaching this marker proves a full archive returned to the guarded
     * operation instead of leaving through an epoch exit. */
    hl_a64_movconst(&assembler, 17, UINT64_C(0x5eed5eed5eed5eed));
    hl_a64_str(&assembler, 17, 28, (int)offsetof(hl_native_aarch64_cpu, read_token));
    hl_a64_stub_exit(&assembler, HL_NATIVE_EXIT_BRANCH, 0x5004);
    ...
    execute(&cpu, code + reserve_offset);
