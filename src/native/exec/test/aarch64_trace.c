...
int main(void) {
    ...
    const hl_a64_source_span ret_provider_b_span = {
        0x8210, (const uint8_t *)&provider_ret, sizeof(provider_ret), 7, 8};
    const hl_a64_source ret_provider_b_source = {&ret_provider_b_span, 1, 7, 8};
    ...
    /* A cold, distinct site reaches the already-published live target through
     * the shared table without returning to the dispatcher. */
    run_request.source = &ret_provider_b_source;
    run_state.program = 0x8210;
    register_sentinels(&run_state, 0x9200);
    CHECK(hl_native_run(run_executor, &run_cpu, &run_request, &run_output) == HL_NATIVE_OK);
    CHECK(run_output.kind == HL_NATIVE_EXIT_SYSCALL && run_output.instruction == 0x9200);
    CHECK(registers_match(&run_state, 0x9200));
    ...
    CHECK(ibtc_after.ibtc_local_hits == ibtc_before.ibtc_local_hits);
