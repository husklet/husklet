...
| --- | --- | --- | --- |
| `core/{cli,config,launch}.c`, `core/target/run.c` | `hl-engine/src/{cli.rs,launcher/{wire,plan}.rs,c_execution/{wire,process,worker}.rs}` | Standalone-only `hl_engine_entry -> hl_cli_route_parse / hl_run_config_file_with -> hl_standalone_run -> hl_native_engine_run`; the product worker instead enters `CGuestExecutor -> hl_c_backend_create/hl_c_backend_run -> hl_engine_create_with_options/hl_engine_run` | **1: remove from the product archive.** Assert the four objects and `hl_engine_entry` are absent, run lifecycle/worker tests, then balanced product benchmarks to detect binary-layout movement. |
| `core/options.c` | `hl-engine/src/options.rs` and the validated `RuntimePlan` worker wire | `CGuestExecutor::create_with_streams -> hl_c_backend_create -> hl_options_set -> hl_engine_create_with_options`; guest setup and many retained subsystems then read the immutable store through `hl_options_get` | **2: keep the C read view, eliminate C parsing/mutation policy.** Introduce one bounded immutable option table at the shim only after differential tests cover absent versus empty, overwrite, integer bounds, environment exclusion, and all option consumers. |
