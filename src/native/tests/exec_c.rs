...
/// a listed program that starts passing fails the run so the list cannot rot.
const KNOWN_FAILING: &[&str] = &["a64_cycles", "aarch64_addressing", "aarch64_trace", "run", "translation"];
...
fn evaluate(source: &Path, scratch: &Path, helpers: &[PathBuf]) -> Outcome {
    ...
    if !built.status.success() {
        ...
        return Outcome::Failed(format!("compile failed:\n{report}"));
    }
...
fn exec_c_programs_pass() {
    ...
    for (name, outcome) in &results {
        let known = KNOWN_FAILING.contains(&name.as_str());
        match outcome {
            Outcome::Passed if known => unexpected_passes.push(name.clone()),
            Outcome::Passed => passed += 1,
            Outcome::Skipped(reason) => eprintln!("skip {name}: {reason}"),
            Outcome::Failed(report) if known => eprintln!("known failure {name}: {report}"),
            Outcome::Failed(report) => failures.push(format!("{name}: {report}")),
        }
    }
