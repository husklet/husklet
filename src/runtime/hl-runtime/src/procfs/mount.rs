...
/// Returns the retained engine's fixed base mount namespace.
///
/// Runtime bind mounts require a separate namespace-owned producer. They are
/// deliberately absent until that port can provide their identity and lifetime.
pub(super) fn view(mounts: &[MountSnapshot]) -> Result<ProcfsMountView, ProcfsError> {
    let mut entries = vec![
        entry(
            23,
            0,
