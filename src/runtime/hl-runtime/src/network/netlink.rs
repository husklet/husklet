...
impl RouteSocket {
    pub(crate) fn new(interfaces: Vec<NamespaceInterface>, port: u32) -> Arc<Self> {
        Arc::new(Self { interfaces, port, replies: Mutex::new(VecDeque::new()), readiness: ReadinessRegistry::new() })
    }

    pub(crate) fn port(&self) -> u32 { self.port }

    pub(crate) fn ready(&self) -> bool {
        !self.replies.lock().unwrap_or_else(|error| error.into_inner()).is_empty()
    }
    ...
    pub(crate) fn receive(&self, output: &mut [u8], peek: bool) -> Result<(usize, usize), ObjectError> {
        let mut replies = self.replies.lock().unwrap_or_else(|error| error.into_inner());
        let Some(reply) = replies.front() else { return Err(ObjectError::WouldBlock) };
        let full = reply.len();
        let count = full.min(output.len());
        output[..count].copy_from_slice(&reply[..count]);
        if !peek { replies.pop_front(); }
        Ok((count, full))
    }

    fn reply(&self, request: &[u8], kind: u16, flags: u16, sequence: u32) -> Vec<u8> {
        if kind >= 16 && kind % 4 != 2 { return self.error(request, sequence, -1); }
        if kind == 18 && flags & 0x300 == 0 { return self.one_link(request, sequence); }
        let mut output = Vec::new();
        match kind {
            18 => for interface in &self.interfaces { self.link(&mut output, sequence, interface, true); },
            22 => {
                if let Some(loopback) = self.interfaces.first() {
                    self.address(&mut output, sequence, loopback, 2, &loopback.ipv4, 254, Some(&loopback.name));
                    let mut ipv6 = [0_u8; 16]; ipv6[15] = 1;
                    self.address(&mut output, sequence, loopback, 10, &ipv6, 254, None);
                }
                for interface in self.interfaces.iter().skip(1) {
                    self.address(&mut output, sequence, interface, 2, &interface.ipv4, 0, Some(&interface.name));
                }
            }
            _ => {}
        }
    ...
    fn one_link(&self, request: &[u8], sequence: u32) -> Vec<u8> {
        let index = request.get(20..24).map(|bytes| i32::from_ne_bytes(bytes.try_into().unwrap())).unwrap_or(0);
        let mut name = None;
        let mut offset = 32;
        while offset + 4 <= request.len() {
            ...
            if length < 4 || offset + length > request.len() { break; }
            if kind == 3 {
    ...
    fn address(&self, output: &mut Vec<u8>, sequence: u32, interface: &NamespaceInterface, family: u8, address: &[u8], scope: u8, label: Option<&[u8]>) {
        let start = output.len();
        ...
        if let Some(label) = label { Self::attribute(output, 3, &[label, &[0]].concat()); }
        let length = (output.len() - start) as u32;
        output[start..start + 4].copy_from_slice(&length.to_ne_bytes());
    }

    fn header(output: &mut Vec<u8>, kind: u16, flags: u16, sequence: u32, port: u32, length: u32) {
        output.extend_from_slice(&length.to_ne_bytes()); output.extend_from_slice(&kind.to_ne_bytes());
        output.extend_from_slice(&flags.to_ne_bytes()); output.extend_from_slice(&sequence.to_ne_bytes());
        output.extend_from_slice(&port.to_ne_bytes());
    }

    fn attribute(output: &mut Vec<u8>, kind: u16, data: &[u8]) {
        let length = 4 + data.len();
        output.extend_from_slice(&(length as u16).to_ne_bytes()); output.extend_from_slice(&kind.to_ne_bytes());
        output.extend_from_slice(data); output.resize((output.len() + 3) & !3, 0);
    }
}
