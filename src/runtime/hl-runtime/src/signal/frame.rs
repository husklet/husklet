...
impl<M: hl_linux::GuestMemory> RuntimeProcessSyscalls<M> {
    ...
    pub fn deliver_forced_frame(&self) -> LinuxResult {
        ...
        let action = match self.tasks.action(self.process, info.signal) {
            Ok(action) => action,
        ...
        let thread = match self.tasks.deliver_thread_state(self.thread) {
            Ok(thread) => thread,
            Err(_) => return LinuxResult::Error(Errno::ESRCH),
        };
        let machine = match port.snapshot(self.thread) {
            Ok(machine) => machine,
        ...
        let mut publication = match port.prepare_install(self.thread, &image) {
    ...
    pub(crate) fn rt_sigreturn(&self) -> LinuxResult {
        ...
        let current = match port.snapshot(self.thread) {
            Ok(machine) => machine,
            Err(_) => return LinuxResult::Error(Errno::EIO),
        };
        ...
        let bytes = match port.read_frame(self.thread, address, length) {
            Ok(bytes) => bytes,
            Err(_) => return LinuxResult::Error(Errno::EFAULT),
        };
        ...
        let mut publication = match port.prepare_restore(self.thread, &restore.machine) {
            Ok(publication) => publication,
            Err(_) => return LinuxResult::Error(Errno::EFAULT),
        };
        if publication.publish().is_err() {
