...
struct NetworkForkReservation {
    reservation: u64,
    child: Option<Arc<NetworkCatalog>>,
}

struct NetworkForkState {
    next: u64,
    staged: BTreeMap<u64, NetworkForkReservation>,
    children: BTreeMap<u64, Arc<NetworkCatalog>>,
}

/// Prepares the child network artifact without reconstructing any socket.
///
/// Forked descriptor tables share socket open-file descriptions. Those
/// descriptions retain generation-qualified socket IDs in this exact catalog,
/// so the child must retain the same catalog identity.
pub struct NetworkForkParticipant {
    parent: Arc<NetworkCatalog>,
    state: Mutex<NetworkForkState>,
}

impl NetworkForkParticipant {
    #[must_use]
    pub fn new(parent: Arc<NetworkCatalog>) -> Self {
        Self {
            parent,
            state: Mutex::new(NetworkForkState {
                next: 1,
                staged: BTreeMap::new(),
...
impl ForkParticipant for NetworkForkParticipant {
