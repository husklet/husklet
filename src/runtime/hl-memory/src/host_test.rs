...
#[test]
fn coarse_private_policy_requires_private_nonexecutable_alias_free_backing() {
    let coordinator = MappingCoordinator::new(FakeHost::failing(usize::MAX));
    let private = |address, protection| MapRequest {
    ...
    coordinator
        .map(private(0x1000, Protection::READ.union(Protection::WRITE)))
        .unwrap();
    let lease = coordinator
        .project_contiguous(GuestAddress::new(0x1000), 8, Protection::WRITE, 1)
        .unwrap();
    assert_eq!(lease.primary_view().publication, crate::WritePublication::CoarsePrivate);
    drop(lease);

    coordinator.map(private(0x3000, Protection::EXECUTE)).unwrap();
    let lease = coordinator
        .project_contiguous(GuestAddress::new(0x1000), 8, Protection::WRITE, 2)
        .unwrap();
    assert_eq!(lease.primary_view().publication, crate::WritePublication::Exact);
    drop(lease);
    ...
    coordinator
        ...
        .unwrap();
    let lease = coordinator
        .project_contiguous(GuestAddress::new(0x1000), 8, Protection::WRITE, 3)
        .unwrap();
    assert_eq!(lease.primary_view().publication, crate::WritePublication::CoarsePrivate);
    drop(lease);

    let mut shared = private(0x5000, Protection::READ.union(Protection::WRITE));
    shared.backing = Backing::Anonymous {
        identity: 902,
        shared: true,
    };
    coordinator.map(shared).unwrap();
    let lease = coordinator
        .project_contiguous(GuestAddress::new(0x5000), 8, Protection::WRITE, 4)
        .unwrap();
    assert_eq!(lease.primary_view().publication, crate::WritePublication::Exact);
}

#[test]
fn coarse_private_publication_commits_touched_view_and_invalidates_exclusive() {
    let coordinator = MappingCoordinator::new(FakeHost::failing(usize::MAX));
    let private = |address, identity| MapRequest {
        placement: Placement::Fixed(GuestAddress::new(address)),
        length: 4096,
        alignment: 4096,
    ...
    coordinator.map(private(0x1000, 911)).unwrap();
    coordinator.map(private(0x3000, 912)).unwrap();
    let (_, reservation) = coordinator
        .load_exclusive(GuestAddress::new(0x1000), 8, false, AtomicOrder::Acquire)
        .unwrap();
    let mut lease = coordinator
        .project_contiguous(GuestAddress::new(0x1000), 8, Protection::WRITE, 7)
        .unwrap();
    let additional = lease
        .project_additional(GuestAddress::new(0x3000), 8, Protection::WRITE)
        .unwrap();
    assert_eq!(additional.index, 1);
    assert_eq!(additional.publication, crate::WritePublication::CoarsePrivate);
    let generation = lease.generation();
    lease.publish_native_writes(generation, &[], &[0]).unwrap();

    assert!(
        !coordinator
            .store_exclusive(reservation, AtomicValue { low: 1, high: 0 }, AtomicOrder::Release)
            .unwrap()
    );
    let transcript = &coordinator.host.state.lock().unwrap().transcript;
    assert_eq!(
...
#[test]
fn coarse_private_policy_reclassifies_after_remap_and_fork_generation() {
    let coordinator = MappingCoordinator::new(FakeHost::failing(usize::MAX));
    let request_at = |address, protection| MapRequest {
        placement: Placement::Fixed(GuestAddress::new(address)),
        length: 4096,
        alignment: 4096,
        protection,
    ...
    coordinator
        .map(request_at(0x1000, Protection::READ.union(Protection::WRITE)))
        .unwrap();
    coordinator
        .remap(range(), request_at(0x3000, Protection::EXECUTE), true)
        .unwrap();
    let parent = coordinator
        .project_contiguous(GuestAddress::new(0x1000), 8, Protection::WRITE, 9)
        .unwrap();
    assert_eq!(parent.primary_view().publication, crate::WritePublication::Exact);
    drop(parent);

    let child = coordinator.fork_restore(FakeHost::failing(usize::MAX)).unwrap();
    let inherited = child
        .project_contiguous(GuestAddress::new(0x1000), 8, Protection::WRITE, 10)
        .unwrap();
    assert_eq!(inherited.primary_view().publication, crate::WritePublication::Exact);
    drop(inherited);
    child
        .protect(
            AddressRange::nonempty(GuestAddress::new(0x3000), 4096).unwrap(),
        ...
        .unwrap();
    let reclassified = child
        .project_contiguous(GuestAddress::new(0x1000), 8, Protection::WRITE, 11)
        .unwrap();
    assert_eq!(
        reclassified.primary_view().publication,
        crate::WritePublication::CoarsePrivate
    );
}

#[test]
fn mixed_exact_and_coarse_publication_retains_per_view_semantics() {
    let coordinator = MappingCoordinator::new(FakeHost::failing(usize::MAX));
    let mut primary = request();
    primary.protection = Protection::READ.union(Protection::WRITE);
    primary.backing = Backing::Anonymous {
        identity: 931,
        shared: false,
    };
    coordinator.map(primary).unwrap();
    ...
    coordinator.map(executable).unwrap();
    let executable_range = AddressRange::nonempty(GuestAddress::new(0x3002), 2).unwrap();
    let token = coordinator.executable_token(executable_range, 1);
    let mut lease = coordinator
        .project_contiguous(GuestAddress::new(0x1000), 8, Protection::WRITE, 12)
        .unwrap();
    assert_eq!(lease.primary_view().publication, crate::WritePublication::CoarsePrivate);
    let additional = lease
        .project_additional(GuestAddress::new(0x3000), 8, Protection::WRITE)
        .unwrap();
    assert_eq!(additional.index, 1);
    assert_eq!(additional.publication, crate::WritePublication::Exact);
    let generation = lease.generation();
    lease
        .publish_native_writes(generation, &[executable_range], &[0])
        .unwrap();
    assert_ne!(coordinator.executable_token(executable_range, 1), token);
    let transcript = &coordinator.host.state.lock().unwrap().transcript;
    assert_eq!(
        transcript
            .iter()
...
#[test]
fn coarse_private_publication_rejects_stale_generation_and_rolls_back() {
    let coordinator = MappingCoordinator::new(FakeHost::failing(usize::MAX));
    let mut writable = request();
    writable.protection = Protection::READ.union(Protection::WRITE);
    ...
    coordinator.map(writable).unwrap();
    let lease = coordinator
        .project_contiguous(GuestAddress::new(0x1000), 8, Protection::WRITE, 13)
        .unwrap();
    let mut stale = lease.generation();
    stale.mappings = stale.mappings.wrapping_add(1);
    assert_eq!(
        lease.publish_native_writes(stale, &[], &[0]),
        Err(MemoryError::InvariantViolation)
    );
    let state = coordinator.host.state.lock().unwrap();
    assert!(state.live.is_empty());
