...
impl Audit {
    ...
    fn inventory(&self) -> Result<Vec<Entry>, String> {
        ...
        let production = fs::read_to_string(self.root.join("src/runtime/native/retained/src/linux_abi/number.c"))
        ...
        if entries != checked {
            return Err("syscall-numbers.tsv differs from production C number.c; run hl-syscall-audit --regenerate src/runtime/native/retained/src/linux_abi/number.c".into());
        }
    ...
    fn numbers_path(&self) -> PathBuf {
        self.root.join("src/runtime/hl-syscall-audit/syscall-numbers.tsv")
    }
