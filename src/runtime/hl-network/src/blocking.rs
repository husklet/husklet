...
impl WaitGate {
    pub(crate) fn generation(&self) -> u64 {
        *self.generation.lock().unwrap_or_else(|error| error.into_inner())
    }

    fn notify_waiters(&self) {
        let mut value = self.generation.lock().unwrap_or_else(|error| error.into_inner());
    ...
    pub(crate) fn wait(&self, observed: u64) {
        ...
        while *value == observed {
            value = self.wake.wait(value).unwrap_or_else(|error| error.into_inner());
        }
    }
}
