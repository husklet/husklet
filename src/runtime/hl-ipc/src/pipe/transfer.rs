...
impl PipeTransfer {
    ...
    fn attempt(
        ...
        if source_key < target_key {
            let mut source_state = source.pipe.state.lock().unwrap_or_else(|error| error.into_inner());
            let mut target_state = target.pipe.state.lock().unwrap_or_else(|error| error.into_inner());
