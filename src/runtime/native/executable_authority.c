...
hl_status hl_c_backend_executable_open(const hl_host_services *services,
  ...
  if (services == NULL || services->file == NULL ||
      services->file->open_relative == NULL || host_path == NULL ||
      host_path[0] == '\0') {
    return HL_STATUS_INVALID_ARGUMENT;
  }

  hl_host_result opened = services->file->open_relative(
      services->context, HL_HOST_HANDLE_CWD, host_path, strlen(host_path),
      HL_HOST_FILE_READ, 0, 0);
  if (opened.status != HL_STATUS_OK) {
    return (hl_status)opened.status;
  }
  if (opened.value == HL_HOST_HANDLE_INVALID) {
...
void hl_c_backend_executable_discard(const hl_host_services *services,
                                     hl_engine_executable *executable) {
  if (executable == NULL) {
    return;
  }
  if (executable->host_handle != HL_HOST_HANDLE_INVALID && services != NULL &&
      services->file != NULL && services->file->close != NULL) {
