...
void hl_lint_config_init(LintConfig *config) {
  *config = (LintConfig){
      ...
      .clang_tidy_checks =
          ...
          "bugprone-infinite-loop,bugprone-not-null-terminated-result,bugprone-"
          "posix-return,"
          "bugprone-signal-handler,bugprone-sizeof-expression,bugprone-"
          "suspicious-memory-comparison,"
          "bugprone-suspicious-memset-usage,bugprone-undefined-memory-"
          "manipulation",
  };
...
static void print_usage(const char *program) {
  fprintf(stdout,
          "usage: %s [options] [--source-dir <path>]... [--source-file "
          "<path>]...\n",
          program);
  fprintf(stdout, "options:\n");
  fprintf(stdout, "  --source-dir PATH         add recursive source directory "
                  "(default: src)\n");
  fprintf(stdout, "  --source-file PATH        add explicit source file\n");
  fprintf(
      stdout,
      "  --clang-tidy-source-file PATH analyze a compiled translation unit\n");
  fprintf(stdout,
          "  --include-dir PATH        add include directory for cppcheck\n");
  fprintf(stdout, "  --compile-commands-dir DIR directory containing "
  ...
  fprintf(stdout, "  --clang-tidy-checks LIST  clang-tidy checks (default: "
                  "bugprone-*,clang-analyzer-*,performance-*)\n");
  fprintf(stdout, "  --max-function-lines N    opt in to lexical "
                  "function-length warnings\n");
  fprintf(
      stdout,
      "  --max-nesting N           opt in to lexical brace-depth warnings\n");
  fprintf(stdout,
          "  --max-line-length N       opt in to line-length warnings\n");
  ...
  fprintf(stdout,
          "  --skip-custom             disable custom heuristics stage\n");
  fprintf(stdout, "  --allow-getenv-file PATH  allow direct environment access "
                  "in this source file\n");
  fprintf(stdout, "  --allow-stdio-file PATH   temporarily allow direct "
                  "console output in this file\n");
  fprintf(stdout, "  --allow-shell-file PATH   temporarily allow shell "
...
static bool parse_integer_option(const char *option, const char *value,
                                 int *result) {
  char *end = NULL;
...
HlLintCliResult hl_lint_cli_parse(LintConfig *config, int argc, char **argv) {
  for (int index = 1; index < argc; index++) {
    ...
    if (strcmp(argument, "--source-dir") == 0 ||
    ...
    } else if (strcmp(argument, "--include-dir") == 0 ||
               strcmp(argument, "-I") == 0) {
      if ((value = option_value(&index, argc, argv)) == NULL)
        return HL_LINT_CLI_EXIT_ERROR;
      hl_lint_list_append(&config->include_dirs, value);
    } else if (strcmp(argument, "--compile-commands-dir") == 0) {
      if ((config->compile_db_dir = option_value(&index, argc, argv)) == NULL)
        return HL_LINT_CLI_EXIT_ERROR;
    } else if (strcmp(argument, "--clang-format-bin") == 0) {
      if ((config->clang_format_bin = option_value(&index, argc, argv)) == NULL)
        return HL_LINT_CLI_EXIT_ERROR;
    } else if (strcmp(argument, "--clang-tidy-bin") == 0) {
      if ((config->clang_tidy_bin = option_value(&index, argc, argv)) == NULL)
        return HL_LINT_CLI_EXIT_ERROR;
    } else if (strcmp(argument, "--cppcheck-bin") == 0) {
      if ((config->cppcheck_bin = option_value(&index, argc, argv)) == NULL)
        return HL_LINT_CLI_EXIT_ERROR;
    } else if (strcmp(argument, "--clang-tidy-checks") == 0) {
      if ((config->clang_tidy_checks = option_value(&index, argc, argv)) ==
          NULL)
        return HL_LINT_CLI_EXIT_ERROR;
      ...
      config->strict = true;
    } else if (strcmp(argument, "--skip-clang-format") == 0 ||
               strcmp(argument, "--clang-format-no-check") == 0) {
      config->run_clang_format = false;
    } else if (strcmp(argument, "--skip-clang-tidy") == 0 ||
               strcmp(argument, "--clang-tidy-no-check") == 0) {
      config->run_clang_tidy = false;
    } else if (strcmp(argument, "--skip-cppcheck") == 0 ||
               strcmp(argument, "--cppcheck-no-check") == 0) {
