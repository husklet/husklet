...
static bool is_source_file(const char *path) {
  return has_extension(path, ".c") || has_extension(path, ".h") ||
         has_extension(path, ".m") || has_extension(path, ".mm");
}
...
static char *path_from_wide(const wchar_t *path) {
  int count = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, path, -1, NULL,
                                  0, NULL, NULL);
  if (count == 0)
    return NULL;
  ...
  if (utf8 == NULL)
    return NULL;
  if (WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, path, -1, utf8, count,
                          NULL, NULL) == 0) {
    free(utf8);
    return NULL;
  }
