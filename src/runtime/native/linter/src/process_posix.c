...
static int append_output(HlLintProcessResult *result, const char *data,
  ...
  size_t available =
      result->output_size < limit ? limit - result->output_size : 0;
...
int hl_lint_process_run(const char *const argv[], size_t output_limit,
  ...
  if (set_close_on_exec(output_pipe[0]) != 0 ||
      set_close_on_exec(output_pipe[1]) != 0) {
    result->platform_error = errno;
    close(output_pipe[0]);
    close(output_pipe[1]);
    return -1;
  ...
  if (error == 0) {
    error = posix_spawn_file_actions_adddup2(&actions, output_pipe[1],
                                             STDOUT_FILENO);
  }
  if (error == 0) {
    error = posix_spawn_file_actions_adddup2(&actions, output_pipe[1],
  ...
  if (error == 0) {
    error = posix_spawn_file_actions_addclose(&actions, output_pipe[1]);
  }
  if (error != 0) {
  ...
  pid_t child = -1;
  error = posix_spawnp(&child, argv[0], &actions, NULL,
  ...
  for (;;) {
    ...
    if (count > 0) {
      if (append_output(result, buffer, (size_t)count, limit, &capacity) != 0 &&
          read_error == 0) {
        read_error = errno;
      }
      continue;
