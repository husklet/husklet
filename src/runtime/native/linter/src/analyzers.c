...
static int run_clang_format(const LintConfig *cfg, const StringList *files,
                            LintStats *stats) {
  int rc = 0;
  if (!cfg->run_clang_format)
    return 0;
  ...
  for (size_t i = 0; i < files->count; i++) {
    ...
    const char *const argv[] = {
        cfg->clang_format_bin, "--dry-run", "--Werror", "--style=file",
...
static int run_clang_tidy(const LintConfig *cfg, const StringList *files,
                          LintStats *stats) {
  int rc = 0;
  if (!cfg->run_clang_tidy)
    return 0;
  ...
  for (size_t i = 0; i < files->count; i++) {
    ...
    char *checks = xdup_format(
        "--checks=%s", cfg->clang_tidy_checks
...
static int run_cppcheck(const LintConfig *cfg, const StringList *files,
                        LintStats *stats) {
  (void)files;
  if (!cfg->run_cppcheck)
    return 0;
  ...
  char *project = cfg->compile_db_dir
                      ? xdup_format("--project=%s/compile_commands.json",
                                    cfg->compile_db_dir)
                      : NULL;
  if (!project) {
  ...
  if (rc == 0)
    return 0;
  emit_diag("warn", cfg->compile_db_dir, 0, 0, "cppcheck",
...
int hl_lint_analyzers_run(const LintConfig *config, const StringList *files,
                          const StringList *clang_tidy_files,
                          LintStats *stats) {
  int result = 0;
  if (config->run_clang_format)
    result = run_clang_format(config, files, stats);
  if (result == 0 && config->run_clang_tidy)
    result = run_clang_tidy(config, clang_tidy_files, stats);
  if (result == 0 && config->run_cppcheck)
    result = run_cppcheck(config, files, stats);
  return result;
}
