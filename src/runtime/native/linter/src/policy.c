...
static bool is_getenv_allowed_in_file(const LintConfig *cfg, const char *path) {
  ...
  for (size_t i = 0; i < cfg->allow_getenv_files.count; i++) {
    if (path_matches(path, cfg->allow_getenv_files.items[i]))
      return true;
  }
  return false;
}
...
static bool is_stdio_allowed_in_file(const LintConfig *cfg, const char *path) {
  for (size_t i = 0; i < cfg->allow_stdio_files.count; i++) {
    if (path_matches(path, cfg->allow_stdio_files.items[i]))
      return true;
  }
  return false;
}
...
static bool is_shell_allowed_in_file(const LintConfig *cfg, const char *path) {
  for (size_t i = 0; i < cfg->allow_shell_files.count; i++) {
    if (path_matches(path, cfg->allow_shell_files.items[i]))
      return true;
  }
  return false;
}

static void emit_diag(const char *severity, const char *path, int line, int col,
                      const char *rule, const char *message) {
  if (line > 0) {
    fprintf(stdout, "%s:%d:%d: [%s] %s: %s\n", path, line, col, severity, rule,
            message);
  } else {
...
static bool word_starts_token(const char *line, const char *found, size_t len) {
  ...
  bool left_ok = offset == 0 || !(isalnum((unsigned char)line[offset - 1]) ||
                                  line[offset - 1] == '_');
...
static bool line_has_direct_console_output(const char *line) {
  if (line_has_call(line, "printf") || line_has_call(line, "vprintf") ||
      line_has_call(line, "puts") || line_has_call(line, "putchar") ||
      line_has_call(line, "perror")) {
    return true;
  }
  if (line_has_call(line, "dprintf") || line_has_call(line, "vdprintf"))
    return true;
  if ((line_has_call(line, "fprintf") || line_has_call(line, "vfprintf") ||
       line_has_call(line, "fputs") || line_has_call(line, "fputc")) &&
...
static bool line_has_control_prefix(const char *line) {
  ...
  for (size_t i = 0; k_controls[i]; i++) {
    ...
    if (strncmp(s, kw, klen) == 0 && (isspace((unsigned char)s[klen]) ||
                                      s[klen] == '(' || s[klen] == '\0')) {
      return true;
    }
  }
...
static bool looks_like_function_signature(const char *sig) {
  ...
  if (strchr(s, ';'))
    return false;
  if (line_has_control_prefix(s))
    return false;
  if (strstr(s, "static_assert") || strstr(s, "typedef"))
    return false;
  if (strstr(s, "sizeof("))
    return false;
  if (strstr(s, "alignas("))
...
static void sanitize_for_parse(const char *src, char *dst, size_t dst_len,
                               ScanState *state) {
  size_t j = 0;
...
static void check_file_custom(const LintConfig *cfg, const char *path,
                              LintStats *stats) {
  FILE *fp = fopen(path, "r");
  if (!fp) {
    emit_diag("error", path, 0, 0, "fs",
              "failed to open file for custom checks");
    stats->errors++;
    return;
  }
  ...
  while (fgets(raw, sizeof(raw), fp)) {
    ...
    if (line_has_direct_environment_access(clean)) {
      if (!is_getenv_allowed_in_file(cfg, path)) {
        emit_diag("error", path, lineno, 1, "api",
                  "direct environment access is only allowed in explicitly "
                  "whitelisted files");
        stats->errors++;
      }
    }
    ...
    if (line_has_platform_debug_output(clean) &&
        !is_stdio_allowed_in_file(cfg, path)) {
      emit_diag("error", path, lineno, 1, "logging",
                "platform debug output is forbidden; use tagged logging");
      stats->errors++;
    }
    if ((line_has_call(clean, "system") || line_has_call(clean, "popen")) &&
    ...
    if (!in_function && brace_depth == 0) {
      ...
      } else if (strchr(clean, '(') && !line_has_control_prefix(clean) &&
                 strncmp(clean, "#", 1) != 0) {
        signature[0] = '\0';
        snprintf(signature, sizeof(signature), "%s", clean);
        sig_start_line = lineno;
    ...
    if (in_function && lineno != func_start_line)
      func_lines++;
    ...
    for (size_t i = 0; i < strlen(clean); i++) {
      ...
      if (c == '{') {
        ...
        if (in_function) {
          ...
          if (nesting > func_max_nesting)
            func_max_nesting = nesting;
        }
        continue;
      }
      if (c == '}') {
        ...
        if (in_function && brace_depth < func_base_depth + 1) {
          if (cfg->max_function_lines > 0 &&
            ...
            emit_diag("warn", path, func_start_line, 1, "complexity",
                      "function exceeds max lines");
            stats->warnings++;
          }
          if (cfg->max_nesting_depth > 0 &&
              func_max_nesting > cfg->max_nesting_depth) {
            emit_diag("warn", path, func_start_line, 1, "complexity",
                      "function exceeds max nesting depth");
            stats->warnings++;
          }
          in_function = false;
          break;
...
void hl_lint_policy_run(const LintConfig *cfg, const StringList *files,
                        LintStats *stats) {
  if (!cfg->run_custom)
    return;
