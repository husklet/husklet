...
#define HL_UINT64_LOCK_FREE                                                    \
  _Generic((uint64_t)0,                                                        \
      ...
      default: 0)
_Static_assert(HL_UINT64_LOCK_FREE == 2,
               "fault provenance requires lock-free uint64_t atomics");
_Static_assert(ATOMIC_INT_LOCK_FREE == 2,
               "fault provenance requires lock-free 32-bit atomics");

enum { RELOCATION_CAPACITY = 1u << 16 };

static uint64_t address_shape(const hl_native_address *address) {
  return (uint64_t)address->kind | (uint64_t)address->bits << 8 |
         (uint64_t)address->base << 16 | (uint64_t)address->index << 24 |
         (uint64_t)address->shift << 32 | (uint64_t)address->extend << 40;
}

static void provenance_store(provenance_entry *entry, uint64_t first,
                             uint64_t last, const hl_native_provenance *record,
                             uint32_t generation) {
  uint64_t sequence =
  ...
  atomic_store_explicit(&entry->address_displacement,
                        (uint64_t)record->address.displacement,
                        memory_order_relaxed);
  atomic_store_explicit(&entry->address_constant, record->address.constant,
                        memory_order_relaxed);
  atomic_store_explicit(&entry->address_shape, address_shape(&record->address),
                        memory_order_relaxed);
...
static int address_valid(const hl_native_address *address) {
  if (address->kind > HL_NATIVE_ADDRESS_INDEXED ||
      (address->bits != 0 && address->bits != 32 && address->bits != 64) ||
      address->shift > 63 || address->extend > HL_NATIVE_EXTEND_S32 ||
      address->reserved != 0)
    return 0;
  if (address->kind == HL_NATIVE_ADDRESS_NONE)
...
static int insertion(const hl_native_cache *cache, uint64_t guest) {
  ...
  for (uint32_t probe = 0; probe < cache->capacity; probe++) {
    ...
    if (cache->entries[slot].generation != cache->generation ||
        cache->entries[slot].state == ENTRY_EMPTY)
      return tombstone == UINT32_MAX ? (int)slot : (int)tombstone;
    if (cache->entries[slot].state == ENTRY_TOMBSTONE &&
        tombstone == UINT32_MAX)
      tombstone = slot;
    if (cache->entries[slot].state != ENTRY_TOMBSTONE &&
        cache->entries[slot].guest == guest)
      return -1;
  }
...
hl_native_status
hl_native_cache_create(hl_native_cache **output, hl_native_arena *arena,
                       uint32_t capacity, uint32_t provenance_capacity,
                       uint32_t hash_shift, uint64_t mapping_epoch,
                       const hl_native_cache_observer *observer) {
  hl_native_cache *cache;
  if (output == NULL)
    return HL_NATIVE_ARGUMENT;
  *output = NULL;
  if (arena == NULL || arena->memory.reserve == NULL ||
      !power_of_two(capacity) || capacity > (1u << 19) ||
      !power_of_two(provenance_capacity) || provenance_capacity > (1u << 18) ||
      hash_shift > 2)
    return HL_NATIVE_ARGUMENT;
  ...
  cache->certificates =
      calloc(HL_NATIVE_CERTIFICATE_CAPACITY, sizeof(*cache->certificates));
  cache->certificate_valid =
      calloc(HL_NATIVE_CERTIFICATE_CAPACITY, sizeof(*cache->certificate_valid));
  if (cache->entries == NULL || cache->provenance == NULL ||
      cache->live == NULL || cache->relocations == NULL ||
      cache->resolved == NULL || cache->certificates == NULL ||
      cache->certificate_valid == NULL) {
 ...
 */
#define PROBE_KEY_VALID(EPOCH_SINK, MISS_SINK, HIT_SINK)                       \
  do {                                                                         \
    if (mapping_epoch != cache->mapping_epoch ||                               \
        memory_mode != cache->memory_mode ||                                   \
        authority_generation != cache->authority_generation) {                 \
      EPOCH_SINK;                                                              \
      return HL_NATIVE_EPOCH;                                                  \
    }                                                                          \
    slot = hl_native_cache_find_identity(cache, guest, memory_mode,            \
...
hl_native_lookup
hl_native_cache_lookup_key(hl_native_cache *cache, uint64_t guest,
                           uint64_t mapping_epoch, uint64_t instruction_epoch,
                           uint64_t memory_mode, uint64_t authority_generation,
                           hl_native_code *output) {
  int slot;
  cache_entry *entry;
  if (!hl_native_cache_available(cache) || output == NULL)
    return HL_NATIVE_MISS;
  memset(output, 0, sizeof(*output));
  cache->stats.lookups++;
  PROBE_KEY_VALID(cache->stats.epoch_rejections++, cache->stats.misses++,
                  cache->stats.hits++);
}

hl_native_lookup
hl_native_cache_probe_key(hl_native_cache *cache, uint64_t guest,
                          uint64_t mapping_epoch, uint64_t instruction_epoch,
                          uint64_t memory_mode, uint64_t authority_generation,
                          hl_native_code *output) {
  if (!hl_native_cache_available(cache) || output == NULL)
...
hl_native_lookup hl_native_cache_probe_key_counted(
    hl_native_cache *cache, hl_native_cache_lookup_counts *counts,
    uint64_t guest, uint64_t mapping_epoch, uint64_t instruction_epoch,
    uint64_t memory_mode, uint64_t authority_generation,
    hl_native_code *output) {
  if (!hl_native_cache_available(cache) || counts == NULL || output == NULL)
    return HL_NATIVE_MISS;
  memset(output, 0, sizeof(*output));
  ...
  PROBE_KEY_VALID(count_saturating(&counts->epoch_rejections),
                  count_saturating(&counts->misses),
                  count_saturating(&counts->hits));
}

#undef PROBE_KEY_VALID

hl_native_lookup hl_native_cache_lookup(hl_native_cache *cache, uint64_t guest,
                                        uint64_t mapping_epoch,
                                        hl_native_code *output) {
  return hl_native_cache_lookup_key(cache, guest, mapping_epoch, 0, 0, 0,
                                    output);
}
...
static void certificate_revoke(hl_native_cache *cache, uint64_t identity) {
  if (cache == NULL || identity == 0)
    return;
  uint32_t used =
      atomic_load_explicit(&cache->certificate_used, memory_order_acquire);
  for (uint32_t index = 0; index < used; ++index)
    if (cache->certificates[index].identity == identity) {
      atomic_store_explicit(&cache->certificate_valid[index], 0,
...
static uint64_t
certificate_reserve(hl_native_cache *cache,
                    const hl_native_certificate_record *record) {
  if (record == NULL || record->identity == 0)
    return 0;
  uint32_t index =
      atomic_load_explicit(&cache->certificate_used, memory_order_relaxed);
  if (index == HL_NATIVE_CERTIFICATE_CAPACITY)
    return 0;
  cache->certificates[index] = *record;
  atomic_store_explicit(&cache->certificate_used, index + 1,
                        memory_order_release);
...
static void certificate_publish(hl_native_cache *cache, uint64_t identity) {
  ...
  for (uint32_t index = 0; index < used; ++index)
    if (cache->certificates[index].identity == identity) {
      atomic_store_explicit(&cache->certificate_valid[index], 1,
                            memory_order_release);
      return;
    }
}
...
int hl_native_cache_certificate(const hl_native_cache *cache, uint64_t identity,
  ...
  if (cache == NULL || identity == 0 || output == NULL)
    return 0;
  uint32_t used =
      atomic_load_explicit(&cache->certificate_used, memory_order_acquire);
  for (uint32_t index = 0; index < used; ++index) {
    if (cache->certificates[index].identity != identity)
      continue;
    if (atomic_load_explicit(&cache->certificate_valid[index],
                             memory_order_acquire) == 0)
      return 0;
    *output = cache->certificates[index];
    return atomic_load_explicit(&cache->certificate_valid[index],
                                memory_order_acquire) != 0;
  }
  return 0;
}
...
hl_native_status hl_native_cache_write_begin(hl_native_cache *cache) {
  if (!hl_native_cache_available(cache))
...
hl_native_status hl_native_cache_reserve_key(
    hl_native_cache *cache, uint64_t guest, uint64_t mapping_epoch,
    uint64_t instruction_epoch, uint64_t memory_mode,
    uint64_t authority_generation, uint64_t source_first, uint64_t source_last,
    uint64_t maximum, const hl_native_certificate_record *certificate,
  ...
  if (cache == NULL || mapping_epoch != cache->mapping_epoch ||
      memory_mode != cache->memory_mode ||
      authority_generation != cache->authority_generation ||
      source_last <= source_first || maximum == 0)
    return HL_NATIVE_ARGUMENT;
  if (cache->active_token != 0)
    return HL_NATIVE_STATE;
  if (hl_native_cache_find_identity(cache, guest, memory_mode,
                                    authority_generation) >= 0)
    return HL_NATIVE_STATE;
  slot = insertion(cache, guest);
  if (slot < 0 || cache->live_count >= cache->capacity)
    return HL_NATIVE_CAPACITY;
  hl_native_status status =
      hl_native_arena_allocate(cache->arena, maximum, 16, &span);
  if (status != HL_NATIVE_OK)
    return status;
...
hl_native_status hl_native_cache_reserve(hl_native_cache *cache, uint64_t guest,
                                         uint64_t mapping_epoch,
  ...
  return hl_native_cache_reserve_key(cache, guest, mapping_epoch, 0, 0, 0,
                                     source_first, source_last, maximum, NULL,
                                     block);
}

hl_native_status hl_native_cache_writable(hl_native_cache *cache,
                                          const hl_native_block *block,
                                          void **bytes, uint64_t *capacity) {
  cache_entry *entry;
  if (cache == NULL || block == NULL || bytes == NULL || capacity == NULL ||
      block->slot >= cache->capacity)
    return HL_NATIVE_ARGUMENT;
  *bytes = NULL;
  *capacity = 0;
  entry = &cache->entries[block->slot];
  if (entry->generation != cache->generation ||
      entry->state != ENTRY_RESERVED || entry->token != block->token ||
      entry->code_offset != block->code_offset ||
      entry->code_size != block->code_size ||
      cache->active_token != block->token)
    return HL_NATIVE_STATE;
...
hl_native_status
hl_native_cache_publish_map(hl_native_cache *cache, hl_native_block *block,
                            uint64_t used, uint64_t body_offset,
                            const hl_native_provenance *records,
                            uint32_t record_count) {
  hl_native_span span;
  cache_entry *entry;
  hl_native_status status;
  if (cache == NULL || block == NULL || block->slot >= cache->capacity ||
  ...
  for (uint32_t index = 0; index < record_count; index++) {
    if (records[index].code_size == 0 || records[index].code_offset >= used ||
        ...
        records[index].code_offset < previous_end ||
        records[index].guest < block->source_first ||
        records[index].guest >= block->source_last ||
        records[index].access > HL_NATIVE_ACCESS_EXECUTE ||
        records[index].width > 64 || !address_valid(&records[index].address) ||
        ((records[index].access == HL_NATIVE_ACCESS_UNKNOWN) !=
         (records[index].width == 0)) ||
        (records[index].access != HL_NATIVE_ACCESS_UNKNOWN &&
         records[index].address.kind == HL_NATIVE_ADDRESS_NONE))
      return HL_NATIVE_ARGUMENT;
  ...
  if (entry->generation != cache->generation ||
      entry->state != ENTRY_RESERVED || entry->token != block->token ||
      entry->guest != block->guest ||
      entry->mapping_epoch != cache->mapping_epoch ||
  ...
  for (uint32_t index = 0; index < record_count; index++) {
    provenance_entry *provenance =
        &cache->provenance[cache->provenance_next++ &
                           (cache->provenance_capacity - 1)];
    uint64_t first = block->code_offset + records[index].code_offset;
    provenance_store(provenance, first, first + records[index].code_size,
                     &records[index], cache->generation);
  }
  entry->state = ENTRY_LIVE;
  cache->live[cache->live_count++] = block->slot;
  cache->stats.publications++;
  cache->active_token = 0;
  atomic_store_explicit(&cache->published_generation, cache->generation,
                        memory_order_release);
...
hl_native_status hl_native_cache_publish(hl_native_cache *cache,
                                         hl_native_block *block, uint64_t used,
  ...
  return hl_native_cache_publish_map(cache, block, used, body_offset, &record,
                                     1);
}
...
void hl_native_cache_cancel(hl_native_cache *cache, hl_native_block *block) {
  ...
  if (cache->active_token == block->token)
    cache->active_token = 0;
  memset(block, 0, sizeof(*block));
}

int hl_native_cache_epoch_matches(const hl_native_cache *cache,
                                  uint64_t mapping_epoch,
                                  uint64_t instruction_epoch,
                                  uint64_t memory_mode,
                                  uint64_t authority_generation) {
  return cache != NULL &&
         atomic_load_explicit(&cache->mapping_epoch, memory_order_acquire) ==
             mapping_epoch &&
         atomic_load_explicit(&cache->instruction_epoch,
                              memory_order_acquire) == instruction_epoch &&
         atomic_load_explicit(&cache->memory_mode, memory_order_acquire) ==
             memory_mode &&
         atomic_load_explicit(&cache->authority_generation,
                              memory_order_acquire) == authority_generation;
}
...
int hl_native_cache_execution(const hl_native_cache *cache, uint64_t identity,
                              hl_native_code *output) {
  uintptr_t executable;
  if (cache == NULL || output == NULL || (identity & 3u) != 1u)
    return 0;
  memset(output, 0, sizeof(*output));
...
hl_native_status hl_native_cache_reset_identity(hl_native_cache *cache,
                                                uint64_t mapping_epoch,
                                                uint64_t instruction_epoch,
                                                uint64_t memory_mode,
                                                uint64_t authority_generation) {
  ...
  atomic_store_explicit(&cache->published_generation, 0, memory_order_release);
  atomic_fetch_add_explicit(&cache->provenance_epoch, 1, memory_order_acq_rel);
  // Entries past provenance_next were never published in this generation, so
  // only the written prefix needs clearing; a generation wrap recycles stale
  // tags and needs all.
  uint32_t written = cache->provenance_next < cache->provenance_capacity
                         ? cache->provenance_next
  ...
  atomic_store_explicit(&cache->mapping_epoch, mapping_epoch,
                        memory_order_release);
  atomic_store_explicit(&cache->instruction_epoch, instruction_epoch,
                        memory_order_release);
  atomic_store_explicit(&cache->memory_mode, memory_mode, memory_order_release);
  atomic_store_explicit(&cache->authority_generation, authority_generation,
                        memory_order_release);
  cache->live_count = 0;
...
hl_native_status hl_native_cache_reset_epoch(hl_native_cache *cache,
                                             uint64_t mapping_epoch,
                                             uint64_t instruction_epoch) {
  return hl_native_cache_reset_identity(cache, mapping_epoch, instruction_epoch,
                                        0, 0);
}

hl_native_status hl_native_cache_reset(hl_native_cache *cache,
                                       uint64_t mapping_epoch) {
  return hl_native_cache_reset_epoch(cache, mapping_epoch, 0);
}

hl_native_status hl_native_cache_invalidate(hl_native_cache *cache,
                                            uint64_t first, uint64_t last,
                                            uint32_t *removed) {
  uint32_t retained = 0, count = 0;
  if (cache == NULL || last <= first)
    return HL_NATIVE_ARGUMENT;
  if (!hl_native_cache_available(cache))
    return HL_NATIVE_STATE;
  if (cache->active_token != 0)
  ...
  for (uint32_t index = 0; index < cache->live_count; index++) {
    ...
    cache_entry *entry = &cache->entries[slot];
    if (!hl_native_cache_live(cache, slot))
      continue;
...
int hl_native_cache_provenance_record(const hl_native_cache *cache,
  ...
  int found = 0;
  if (cache == NULL || executable == NULL)
    return 0;
  pc = (uintptr_t)executable;
  base = (uintptr_t)cache->arena->executable;
  if (pc < base || pc >= base + cache->arena->mapping.capacity)
    return 0;
  offset = pc - base;
  uint64_t epoch =
      atomic_load_explicit(&cache->provenance_epoch, memory_order_acquire);
  uint32_t generation;
  if ((epoch & 1) != 0)
    return 0;
  generation =
      atomic_load_explicit(&cache->published_generation, memory_order_acquire);
  for (uint32_t index = 0; index < cache->provenance_capacity; index++) {
    ...
    candidate.address.displacement = (int64_t)atomic_load_explicit(
        &entry->address_displacement, memory_order_relaxed);
    candidate.address.constant =
        atomic_load_explicit(&entry->address_constant, memory_order_relaxed);
    ...
    candidate.access =
        atomic_load_explicit(&entry->access, memory_order_relaxed);
    candidate.width = atomic_load_explicit(&entry->width, memory_order_relaxed);
    entry_generation =
        atomic_load_explicit(&entry->generation, memory_order_relaxed);
    if (before != atomic_load_explicit(&entry->sequence, memory_order_acquire))
      continue;
    if (entry_generation == generation && first <= offset && offset < last &&
        first >= best) {
      best = first;
      source = candidate;
      found = 1;
...
int hl_native_cache_provenance(const hl_native_cache *cache,
                               const void *executable, uint64_t *guest) {
  hl_native_provenance record;
  if (!hl_native_cache_provenance_record(cache, executable, &record))
    return 0;
  if (guest != NULL)
    *guest = record.guest;
  return 1;
...
int hl_native_address_reconstruct(const hl_native_address *address,
                                  const uint64_t *registers,
                                  uint32_t register_count, uint64_t *output) {
  uint64_t value, index;
  if (address == NULL || output == NULL || !address_valid(address) ||
      address->kind == HL_NATIVE_ADDRESS_NONE)
    return 0;
  if (address->kind == HL_NATIVE_ADDRESS_CONSTANT) {
    value = address->constant;
  } else {
    if (registers == NULL || address->base >= register_count)
      return 0;
    value = registers[address->base];
    if (address->kind == HL_NATIVE_ADDRESS_INDEXED) {
      if (address->index >= register_count)
        return 0;
      index = registers[address->index];
      if (address->extend == HL_NATIVE_EXTEND_U32)
        index = (uint32_t)index;
      if (address->extend == HL_NATIVE_EXTEND_S32)
...
void hl_native_cache_diagnose(const hl_native_cache *cache,
                              hl_native_cache_stats *output) {
  if (cache == NULL || output == NULL)
    return;
  *output = cache->stats;
  output->live_blocks = cache->live_count;
  output->generation = cache->generation;
  output->mapping_epoch =
      atomic_load_explicit(&cache->mapping_epoch, memory_order_acquire);
}
