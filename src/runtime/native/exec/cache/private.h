...
_Static_assert(sizeof(pending_relocation) == 160,
               "pending relocation footprint drifted");
_Static_assert(sizeof(resolved_relocation) == 224,
               "resolved relocation footprint drifted");
...
struct hl_native_cache {
  ...
  uint32_t *live;
  uint32_t capacity, provenance_capacity, provenance_next, live_count,
      generation;
...
static inline void hl_native_cache_observe(hl_native_cache *cache,
                                           hl_native_cache_event event) {
  if (cache->observer.observe != NULL)
    cache->observer.observe(cache->observer.context, event);
}

static inline int hl_native_cache_live(const hl_native_cache *cache,
                                       uint32_t slot) {
  return cache->entries[slot].generation == cache->generation &&
         cache->entries[slot].state == ENTRY_LIVE;
}

static inline int hl_native_cache_occupied(const hl_native_cache *cache,
                                           uint32_t slot) {
  return cache->entries[slot].generation == cache->generation &&
...
static inline uint32_t hl_native_cache_home(const hl_native_cache *cache,
                                            uint64_t guest) {
  return (uint32_t)(((guest >> cache->hash_shift) *
                     UINT64_C(0x9E3779B97F4A7C15)) >>
                    cache->hash_rotate) &
         (cache->capacity - 1);
}

static inline int hl_native_cache_find_identity(const hl_native_cache *cache,
                                                uint64_t guest,
  ...
  for (uint32_t probe = 0; probe < cache->capacity; probe++) {
    ...
    if (!hl_native_cache_occupied(cache, slot))
      return -1;
...
static inline int hl_native_cache_find(const hl_native_cache *cache,
                                       uint64_t guest) {
