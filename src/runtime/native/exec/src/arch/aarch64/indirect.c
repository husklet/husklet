...
static void patch_cbnz(hl_a64_assembler *assembler, uint32_t *branch,
...
static void patch_adr(hl_a64_assembler *assembler, uint32_t *instruction,
                      const uint8_t *target) {
  int64_t distance;
  if (!hl_a64_assembler_wrote(assembler, instruction))
    return;
...
static void patch_literal(hl_a64_assembler *assembler, uint32_t *instruction,
                          const uint8_t *target) {
  int64_t distance;
  if (!hl_a64_assembler_wrote(assembler, instruction))
    return;
...
static void guest_register(hl_a64_assembler *assembler, int output,
                           unsigned reg) {
  if (stolen(reg))
...
static int valid(uint32_t word) {
  ...
  int authenticated_return =
      (word & UINT32_C(0xfffffbff)) == UINT32_C(0xd65f0bff);
  return authenticated_return ||
         (source != 31 &&
          (form == 0xD61F0000u || form == 0xD63F0000u || form == 0xD65F0000u));
}

int hl_a64_indirect_body(hl_a64_assembler *assembler, uint32_t word,
                         uint64_t pc, void *ibtc) {
  uint32_t form = word & 0xFFFFFC1Fu;
  ...
  if (ibtc != NULL) {
    ...
    hl_a64_emit32(
        assembler,
        UINT32_C(0xd503201f)); /* patched b body; nop falls to shared */
...
int hl_a64_indirect_emit(hl_a64_assembler *assembler, uint32_t word,
                         uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_INDIRECT_MAX_BYTES ||
      !valid(word))
    return 0;
