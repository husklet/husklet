...
static int decode(uint32_t word, unsigned *bytes, unsigned *load,
  ...
  if ((word & UINT32_C(0x3fbf7c00)) != UINT32_C(0x089f7c00))
    return 0;
  *bytes = 1u << ((word >> 30) & 3u);
  *load = (word >> 22) & 1u;
  *base = (word >> 5) & 31u;
...
int hl_a64_ordered_body(hl_a64_assembler *assembler, uint32_t word, uint64_t pc,
  ...
  if (assembler == NULL || guard == NULL ||
      !decode(word, &bytes, &load, &base, &target))
    return 0;
  ...
  hl_a64_guard_begin(assembler, bytes,
                     ...
                     guard);
  if (!load)
    hl_a64_guard_write_begin(assembler, bytes, pc, guard);
  ...
  hl_a64_emit32(assembler,
                (word & ~UINT32_C(0x3ff)) | (16u << 5) | native_target);
  if (!load)
    hl_a64_guard_written(assembler, bytes);
  if (load && target_stolen)
...
int hl_a64_ordered_emit(hl_a64_assembler *assembler, uint32_t word,
  ...
  if (!hl_a64_ordered_body(assembler, word, pc, &guard, NULL))
    return 0;
