...
static void publish_execution_identity(hl_a64_assembler *assembler,
                                       int scratch);
...
#define OFFSET_INTERRUPT_TOKEN                                                 \
  ((int)offsetof(hl_native_aarch64_cpu, interrupt_token))
...
void hl_a64_stub_prologue(hl_a64_assembler *assembler) {
  ...
  for (int reg = 1; reg <= 30; reg++)
    if (!stolen(reg))
      hl_a64_ldr(assembler, reg, 0, reg * 8);
...
static void spill(hl_a64_assembler *assembler) {
  ...
  for (int vector = 0; vector < 32; vector += 2)
    hl_a64_stp_q(assembler, vector, vector + 1, CPU,
                 OFFSET_VECTOR + vector * 16);
  for (int reg = 0; reg <= 30; reg++)
    if (!stolen(reg))
      hl_a64_str(assembler, reg, CPU, reg * 8);
...
void hl_a64_stub_exit_register(hl_a64_assembler *assembler, uint32_t kind,
                               int target) {
  if (assembler->diagnostics && kind == HL_NATIVE_EXIT_BRANCH)
...
static void patch_condition(hl_a64_assembler *assembler, uint32_t *branch,
                            const uint8_t *target) {
  uint32_t distance;
  if (!hl_a64_assembler_wrote(assembler, branch))
    return;
  distance = (uint32_t)((target - (const uint8_t *)branch) / 4);
  *branch |= (distance & UINT32_C(0x7ffff)) << 5;
}

static void patch_branch(hl_a64_assembler *assembler, uint32_t *branch,
                         const uint8_t *target) {
  uint32_t distance;
  if (!hl_a64_assembler_wrote(assembler, branch))
    return;
...
static void publish_execution_identity(hl_a64_assembler *assembler,
  ...
  hl_a64_emit32(assembler,
                UINT32_C(0x10000000) | (uint32_t)scratch); /* adr scratch,. */
...
void hl_a64_stub_budget_begin(hl_a64_assembler *assembler, uint64_t pc,
                              hl_a64_budget_guard *guard) {
  ...
  hl_a64_emit32(assembler, UINT32_C(0xb6f80000) | (UINT32_C(2) << 5) |
                               17u); /* tbz x17,#63,+2 */
...
void hl_a64_stub_budget_finish(hl_a64_assembler *assembler,
                               hl_a64_budget_guard *guard, uint32_t count) {
  if (hl_a64_assembler_wrote(assembler, guard->subtract))
