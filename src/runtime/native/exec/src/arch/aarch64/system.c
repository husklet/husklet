...
int hl_a64_system_body(hl_a64_assembler *assembler, uint32_t word) {
  ...
  if (assembler == NULL ||
      (!read_tls(word) && !write_tls(word) && !barrier(word) &&
       !pointer_authentication_hint(word)))
    return 0;
  if (pointer_authentication_hint(word)) {
    ...
    if (reg != 31 && stolen(reg))
      hl_a64_str(assembler, 16, CPU, (int)reg * 8);
  } else if (reg == 31) {
...
int hl_a64_system_emit(hl_a64_assembler *assembler, uint32_t word,
                       uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_SYSTEM_MAX_BYTES ||
      (!read_tls(word) && !write_tls(word) && !barrier(word) &&
       !pointer_authentication_hint(word)))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_system_body(assembler, word))
    return 0;
