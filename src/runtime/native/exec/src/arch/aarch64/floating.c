...
static int valid(uint32_t word) {
  ...
  if ((word & 0x5f000000u) == 0x1f000000u)
    return 1;
...
int hl_a64_floating_emit(hl_a64_assembler *assembler, uint32_t word,
                         uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_FLOATING_MAX_BYTES ||
      !valid(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_floating_body(assembler, word))
    return 0;
