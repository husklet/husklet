...
static void density_add(uint64_t *value, uint64_t add,
...
static void density_record(hl_a64_trace_density *density,
                           hl_a64_density_family family, size_t words,
                           int instruction) {
  if (density == NULL)
    return;
  if (instruction)
    density_add(&density->families[family].guest_instructions, 1, density);
...
static hl_a64_density_family density_family(uint32_t word, uint64_t pc,
  ...
  if (sites->count == 0)
    return HL_A64_DENSITY_OTHER;
  if (hl_a64_memory_decode(word, pc, &memory) &&
      memory.kind == HL_A64_MEMORY_PAIR)
    return memory.read ? HL_A64_DENSITY_PAIR_READ : HL_A64_DENSITY_PAIR_WRITE;
  return HL_A64_DENSITY_SCALAR_MEMORY;
}

static int append_unknown(hl_a64_trace_result *output, size_t first,
                          size_t last, uint64_t guest) {
  if (last <= first || output->provenance_count >= HL_NATIVE_PROVENANCE_MAX)
    return 0;
...
static int append_memory(hl_a64_trace_result *output,
                         const hl_a64_memory_sites *sites, uint64_t guest) {
  ...
  for (uint32_t index = 0; index < sites->count; ++index) {
    ...
    if (site->access != HL_NATIVE_ACCESS_READ &&
        site->access != HL_NATIVE_ACCESS_WRITE)
      return 0;
    output->provenance[output->provenance_count++] = (hl_native_provenance){
...
static int hint_body(hl_a64_assembler *assembler, uint32_t word) {
  if ((word & 0xFFFFF01Fu) != 0xD503201Fu)
    return 0;
...
static int body(hl_a64_assembler *assembler, uint32_t word, uint64_t pc) {
  return hint_body(assembler, word) || hl_a64_move_body(assembler, word) ||
         hl_a64_add_body(assembler, word) ||
         hl_a64_logical_body(assembler, word) ||
         hl_a64_select_body(assembler, word) ||
         hl_a64_arithmetic_body(assembler, word) ||
         hl_a64_bitwise_body(assembler, word) ||
         ...
         hl_a64_pcrel_body(assembler, word, pc) ||
         hl_a64_field_body(assembler, word) ||
         hl_a64_multiply_body(assembler, word) ||
         hl_a64_divide_body(assembler, word) ||
         hl_a64_pair_arithmetic_body(assembler, word) ||
         hl_a64_shift_body(assembler, word) ||
         hl_a64_reverse_body(assembler, word) ||
         hl_a64_simd_compare_body(assembler, word) ||
         hl_a64_compare_body(assembler, word) ||
         hl_a64_simd_difference_body(assembler, word) ||
         hl_a64_simd_float_body(assembler, word) ||
         hl_a64_simd_fp_reduce_body(assembler, word) ||
         hl_a64_simd_integer_body(assembler, word) ||
         hl_a64_simd_logic_body(assembler, word) ||
...
static uint32_t register_definition(unsigned reg) { return UINT32_C(1) << reg; }
...
static int body_accepts(uint32_t word, uint64_t pc, hl_a64_memory *memory) {
  ...
  if (!hl_a64_assembler_begin(&assembler, storage, storage, sizeof(storage)))
    return 0;
  if (body(&assembler, word, pc) ||
    ...
    if (memory != NULL)
      hl_a64_memory_decode(word, pc, memory);
...
hl_a64_instruction_effect hl_a64_trace_effect(uint32_t word, uint64_t pc) {
  ...
  if ((word & 0xff000010u) == 0x54000000u ||
      ...
      (word & 0x7e000000u) == 0x36000000u || indirect_branch || indirect_call ||
      indirect_return) {
  ...
  if (memory.kind != HL_A64_MEMORY_NONE) {
    ...
    if (memory.kind == HL_A64_MEMORY_EXCLUSIVE ||
        memory.kind == HL_A64_MEMORY_ATOMIC) {
...
static int self_add_immediate(uint32_t word, unsigned *reg, uint64_t *value,
                              int subtract_flags) {
  uint32_t opcode = word & UINT32_C(0xffc00000);
  uint32_t expected =
      subtract_flags ? UINT32_C(0xf1000000) : UINT32_C(0x91000000);
  unsigned rd = word & 31u, rn = (word >> 5) & 31u;
  if (opcode != expected || rd != rn || rd == 31 || ((word >> 22) & 1u) != 0)
    return 0;
  *reg = rd;
...
int hl_a64_trace_loop_plan(const uint32_t *words, size_t count, uint64_t pc,
  ...
  if ((branch_word & UINT32_C(0xff00001f)) != UINT32_C(0x54000008))
    return 0; /* B.HI */
  int64_t displacement =
      loop_sign_extend((branch_word >> 5) & UINT32_C(0x7ffff), 19) * 4;
  uint64_t branch_pc = pc + (count - 1) * UINT64_C(4), target;
  if (!add_signed_checked(branch_pc, displacement, &target) || target != pc)
    return 0;
  ...
  for (size_t index = 0; index + 1 < count; ++index) {
    ...
    if (hl_a64_memory_decode(words[index], instruction, &memory)) {
      hl_a64_instruction_effect effect =
          hl_a64_trace_effect(words[index], instruction);
      if (memory.writeback || memory.kind == HL_A64_MEMORY_LITERAL ||
          memory.kind == HL_A64_MEMORY_REGISTER ||
          memory.kind == HL_A64_MEMORY_EXCLUSIVE ||
          memory.kind == HL_A64_MEMORY_ATOMIC ||
          memory.kind == HL_A64_MEMORY_STRUCTURE ||
          memory.kind == HL_A64_MEMORY_PREFETCH ||
          (memory.kind != HL_A64_MEMORY_PAIR &&
           memory.kind != HL_A64_MEMORY_UNSIGNED &&
           memory.kind != HL_A64_MEMORY_UNSCALED) ||
          memory.bytes == 0 || memory.bytes > INT64_MAX ||
          memory.offset > INT64_MAX - (int64_t)memory.bytes)
        return 0;
      if (effect.terminal || effect.control ||
          effect.gpr_definitions == UINT32_MAX)
        return 0;
      memory_definitions |= effect.gpr_definitions;
      unsigned range = 0;
      while (range < plan.range_count && plan.ranges[range].base != memory.base)
        range++;
      if (range == plan.range_count) {
        if (plan.range_count == 2)
          return 0;
        ...
        plan.range_count++;
      } else if (closed[range])
        return 0;
      if (memory.offset < plan.ranges[range].minimum)
        plan.ranges[range].minimum = memory.offset;
      ...
      plan.ranges[range].permissions |=
          memory.write ? HL_NATIVE_ACCESS_WRITE : 0;
      plan.memory_count++;
      continue;
    }
    if (self_add_immediate(words[index], &reg, &value, 0)) {
      unsigned range = 0;
      while (range < plan.range_count && plan.ranges[range].base != reg)
        range++;
      if (range == plan.range_count || closed[range] ||
          plan.ranges[range].step != 0)
        return 0;
    ...
    if (index + 2 == count &&
        self_add_immediate(words[index], &reg, &value, 1)) {
  ...
  if (plan.memory_count < 2 || plan.range_count == 0 || plan.trip == 31 ||
      plan.decrement == 0)
    return 0;
  for (unsigned range = 0; range < plan.range_count; ++range) {
    if (!closed[range] || plan.ranges[range].step == 0 ||
        plan.ranges[range].base == plan.trip)
      return 0;
    if ((plan.ranges[range].permissions & HL_NATIVE_ACCESS_WRITE) != 0) {
      if (!write_seen[range] || write_cursor[range] <= write_first[range] ||
          (uint64_t)(write_cursor[range] - write_first[range]) !=
              plan.ranges[range].step)
        return 0;
      plan.ranges[range].writes_contiguous = 1;
    }
  }
  if ((memory_definitions & register_definition(plan.trip)) != 0)
    return 0;
  for (unsigned range = 0; range < plan.range_count; ++range)
    if ((memory_definitions & register_definition(plan.ranges[range].base)) !=
        0)
      return 0;
  plan.loop_pc = pc;
...
hl_a64_trace_loop_preflight(const hl_native_aarch64_cpu *cpu,
                            const hl_a64_loop_plan *plan,
                            uint64_t mapping_incarnation, uint64_t authority,
                            uint64_t budget, hl_a64_loop_entry *output) {
  hl_a64_loop_entry entry = {0};
  if (output == NULL)
    return HL_A64_LOOP_PREFLIGHT_REJECT;
  memset(output, 0, sizeof(*output));
  if (cpu == NULL || plan == NULL || mapping_incarnation == 0 ||
      authority == 0 || cpu->active_authority != authority ||
      plan->range_count == 0 || plan->range_count > 2 || plan->trip >= 31 ||
      plan->decrement == 0 || plan->instruction_count == 0 ||
  ...
  entry.iterations =
      entry.trip / entry.decrement + (entry.trip % entry.decrement != 0);
  if (entry.iterations == 0 ||
      entry.iterations > UINT64_MAX / entry.instruction_count)
    return HL_A64_LOOP_PREFLIGHT_REJECT;
  entry.budget_iterations = budget / entry.instruction_count;
  if (entry.budget_iterations == 0)
    return HL_A64_LOOP_PREFLIGHT_REJECT;
  if (entry.budget_iterations > entry.iterations)
    entry.budget_iterations = entry.iterations;
  ...
  for (uint32_t range = 0; range < plan->range_count; ++range) {
    ...
    if (source->base >= 31 || source->minimum >= source->maximum ||
        source->step == 0 || (entry.iterations - 1) > UINT64_MAX / source->step)
      return HL_A64_LOOP_PREFLIGHT_REJECT;
    advance = (entry.iterations - 1) * source->step;
    if (cpu->registers[source->base] > UINT64_MAX - advance)
      return HL_A64_LOOP_PREFLIGHT_REJECT;
    last_base = cpu->registers[source->base] + advance;
    if (!add_signed_checked(cpu->registers[source->base], source->minimum,
                            &first) ||
    ...
    for (uint64_t index = 0; index < cpu->read_count; ++index) {
      ...
      if (candidate[1] > candidate[0] && candidate[3] != 0 &&
          (candidate[3] & ~UINT64_C(7)) == 0 && first >= candidate[0] &&
          last <= candidate[1] &&
    ...
    if ((source->permissions & HL_NATIVE_ACCESS_WRITE) != 0) {
      if (++write_ranges > 1 || !source->writes_contiguous)
        return HL_A64_LOOP_PREFLIGHT_REJECT;
      if ((view[3] & UINT64_C(4)) != 0)
        entry.executable = 1;
  ...
  *output = entry;
  return entry.executable ? HL_A64_LOOP_PREFLIGHT_EPOCH
...
int hl_a64_trace_range_plan(const uint32_t *words, size_t count, uint64_t pc,
  ...
  for (size_t index = 0; index < count; index++) {
    ...
    if (effect.terminal || effect.control)
      break;
    if (effect.gpr_definitions == UINT32_MAX)
      return 0;
    definitions |= effect.gpr_definitions;
    if (!hl_a64_memory_decode(words[index], instruction, &memory))
      continue;
    if (memory.vector || memory.writeback ||
        memory.kind == HL_A64_MEMORY_LITERAL ||
        memory.kind == HL_A64_MEMORY_REGISTER ||
        memory.kind == HL_A64_MEMORY_PAIR ||
        memory.kind == HL_A64_MEMORY_EXCLUSIVE ||
        memory.kind == HL_A64_MEMORY_ATOMIC ||
    ...
    if (!have_base) {
      ...
      if (memory.base != plan.base)
        return 0;
      if (memory.offset < plan.minimum)
        plan.minimum = memory.offset;
...
static int add_signed_checked(uint64_t base, int64_t offset, uint64_t *result) {
  if (offset >= 0) {
    if (base > UINT64_MAX - (uint64_t)offset)
      return 0;
...
int hl_a64_trace_certificate_check(const hl_native_aarch64_cpu *cpu,
                                   uint64_t base, int64_t minimum,
                                   int64_t maximum, uint32_t required,
...
static int
trace_build(const hl_a64_source *source, uint64_t pc, size_t count,
            void *buffer, size_t capacity, hl_a64_trace_result *output,
            void *ibtc, const hl_native_direct_authority *authority,
            uint64_t expected_authority, hl_a64_trace_density *density_output,
            uint32_t diagnostics, uint32_t write_reserve, uint32_t write_commit,
            uint32_t runtime_write_reserve, uint32_t dirty_overflow_continue) {
  ...
  if (output == NULL || expected_authority == 0)
    return 0;
  memset(output, 0, sizeof(*output));
  ...
  while (planned < count) {
    ...
    if (!hl_a64_source_fetch(source, cursor, 1, &one))
      break;
    ...
    if (!casp) {
      if ((word & 0x3fc00000u) == 0x08400000u)
        planning_exclusive = 1;
      else if (planning_exclusive && (word & 0x3fc00000u) == 0x08000000u)
        planning_exclusive = 0;
    }
    if (cursor > UINT64_MAX - 4)
      break;
    uint64_t next = cursor + 4;
    int followed = 0;
    if (!planning_exclusive &&
        (word & UINT32_C(0xfc000000)) == UINT32_C(0x14000000)) {
      int64_t displacement =
          (int64_t)((uint64_t)(word & UINT32_C(0x03ffffff)) << 38) >> 36;
      uint64_t target;
      if (!add_signed_checked(cursor, displacement, &target))
        break;
      int seen = 0;
      for (size_t index = 0; index < block_head_count; ++index)
        if (block_heads[index] == target)
          seen = 1;
      if (!seen && block_head_count < 16 &&
          hl_a64_source_available(source, target, 1) != 0) {
        next = target;
        followed = 1;
    ...
    cursor = next;
    hl_a64_instruction_effect effect =
        hl_a64_trace_effect(word, planned_pcs[planned - 1]);
    ...
    if (conditional && !followed) {
      if (planned_conditionals >= HL_A64_TRACE_MAX_CONDITIONALS)
        break;
      planned_conditionals++;
      continue;
    }
    if ((effect.terminal || effect.control) && !followed)
      break;
  }
  if (planned == 0)
    return 0;
  ...
  for (size_t index = 1; index < count; ++index) {
    if (planned_pcs[index] < hull_first)
      hull_first = planned_pcs[index];
    if (planned_pcs[index] + 4 > hull_last)
      hull_last = planned_pcs[index] + 4;
  }
  ...
  for (size_t index = 0; index < count; index++) {
    ...
    if (planned_words[index] == 0xD4000001u) {
      ...
      if (!append_unknown(output, begin, hl_a64_assembler_size(&assembler),
                          instruction))
        return 0;
      density_record(
          &density, HL_A64_DENSITY_CONTROL,
    ...
    if (!exclusive && index + 1 < count &&
        stitched < HL_A64_TRACE_MAX_CONDITIONALS &&
        hl_a64_conditional_stitch(&assembler, word, instruction, &taken_patch,
                                  &taken_target, &refund_patches[stitched])) {
      ...
      hl_native_relocation *relocation =
          &output->relocations[output->relocation_count - 1];
    ...
    int followed_direct = index + 1 < count &&
                          planned_pcs[index + 1] != instruction + 4 &&
                          (word & UINT32_C(0xfc000000)) == UINT32_C(0x14000000);
    if (followed_direct) {
    ...
    if (hl_a64_direct_chain(&assembler, word, instruction, &direct_patch,
                            &direct_target) ||
        hl_a64_indirect_body(&assembler, planned_words[index], instruction,
                             ibtc) ||
        hl_a64_conditional_chain(&assembler, planned_words[index], instruction,
                                 &direct_patch, &direct_target, &taken_patch,
                                 &taken_target)) {
      output->source_last = instruction + 4;
      ...
      if (direct_patch != NULL) {
        output->relocations[output->relocation_count++] =
            (hl_native_relocation){
                .code_offset =
                    (uint64_t)((uint8_t *)direct_patch - (uint8_t *)buffer),
        ...
        hl_native_relocation *relocation =
            &output->relocations[output->relocation_count - 1];
        relocation->span.word_count = HL_A64_EDGE_SPAN_WORDS;
        memcpy(relocation->span.cold, direct_patch,
      ...
      if (taken_patch != NULL) {
        output->relocations[output->relocation_count++] =
            (hl_native_relocation){
                .code_offset =
                    (uint64_t)((uint8_t *)taken_patch - (uint8_t *)buffer),
                .target_guest = taken_target,
                .expected = *taken_patch,
            };
        hl_native_relocation *relocation =
            &output->relocations[output->relocation_count - 1];
        relocation->span.word_count = HL_A64_EDGE_SPAN_WORDS;
        memcpy(relocation->span.cold, taken_patch,
               sizeof(relocation->span.cold));
      }
      break;
    }
    if (!body(&assembler, planned_words[index], instruction) &&
        !hl_a64_single_direct_body(&assembler, planned_words[index],
                                   instruction, authority, &guards[guard_count],
                                   &memory_sites) &&
        !hl_a64_zero_body(&assembler, planned_words[index], instruction,
                          &guards[guard_count], &memory_sites) &&
        !hl_a64_ordered_body(&assembler, planned_words[index], instruction,
                             &guards[guard_count], &memory_sites) &&
        !hl_a64_atomic_body(&assembler, planned_words[index], instruction,
                            &guards[guard_count], &memory_sites) &&
        !hl_a64_simd_immediate_body(&assembler, planned_words[index]) &&
        !hl_a64_single_body(&assembler, planned_words[index], instruction,
                            &guards[guard_count], &memory_sites) &&
        !hl_a64_pair_body(&assembler, planned_words[index], instruction,
                          &guards[guard_count], &memory_sites) &&
        !hl_a64_structure_body(&assembler, planned_words[index], instruction,
                               &guards[guard_count], &memory_sites)) {
      if (index == 0) {
    ...
    hl_a64_density_family family =
        density_family(planned_words[index], instruction, &memory_sites);
    density_record(
        &density, family,
        (hl_a64_assembler_size(&assembler) - begin) / sizeof(uint32_t), 1);
    ...
    if (memory_sites.count != 0) {
      if (!append_memory(output, &memory_sites, instruction))
        return 0;
    } else if (!append_unknown(output, begin, hl_a64_assembler_size(&assembler),
                               instruction)) {
      return 0;
    }
  }
  ...
  output->instruction_count =
      charged_prefix != 0 ? charged_prefix : owned_count;
  hl_a64_stub_budget_finish(&assembler, &budget_guard,
                            (uint32_t)output->instruction_count);
  for (uint32_t index = 0; index < stitched; index++) {
    uint32_t refund =
        (uint32_t)output->instruction_count - refund_prefixes[index];
    *refund_patches[index] |= refund << 10;
    ...
    hl_native_relocation *relocation =
        &output->relocations[refund_relocations[index]];
    memcpy(relocation->span.cold,
           (const uint8_t *)buffer + relocation->code_offset,
           sizeof(relocation->span.cold));
  ...
  for (size_t index = 0; index < guard_count; index++) {
    ...
    density_add(&density.families[guard_families[index]].cold_words,
                (hl_a64_assembler_size(&assembler) - begin) / sizeof(uint32_t),
                &density);
  }
  if (!hl_a64_assembler_ok(&assembler)) {
    memset(buffer, 0, capacity);
  ...
  density.overhead_words =
      attributed <= density.total_words ? density.total_words - attributed : 0;
  if (density_output != NULL)
    *density_output = density;
  return 1;
}

int hl_a64_trace_build(const hl_a64_source *source, uint64_t pc, size_t count,
...
int hl_a64_trace_build_density(const hl_a64_source *source, uint64_t pc,
...
int hl_a64_trace_build_direct(const hl_a64_source *source, uint64_t pc,
                              size_t count, void *buffer, size_t capacity,
                              const hl_native_direct_authority *authority,
                              uint64_t expected_authority,
                              hl_a64_trace_result *output) {
  return trace_build(source, pc, count, buffer, capacity, output, NULL,
                     authority, expected_authority, NULL, 0, 1, 1, 0, 1);
}

hl_native_status hl_a64_trace_cache_direct(
    hl_native_executor *executor, const hl_a64_source *source, uint64_t pc,
    size_t count, void *buffer, size_t capacity,
  ...
  if (executor == NULL || source == NULL || code == NULL || hit == NULL ||
      count == 0 || count > HL_A64_TRACE_MAX_WORDS ||
      pc > UINT64_MAX - count * 4 || expected_authority == 0 ||
  ...
  key = (hl_native_translation_key){
      ...
      .direct_token = authority == NULL
                          ? 0
                          : (uint64_t)(uintptr_t)executor->direct_authority,
  ...
  if (hl_native_translation_lookup(executor, &key, code) == HL_NATIVE_HIT) {
    ...
    if (gate == HL_NATIVE_OK)
      gate =
          hl_native_cache_relocations_invalidate(executor->cache, pc, pc + 4);
    if (gate == HL_NATIVE_OK)
  ...
  *hit = 0;
  ...
  while (!trace_build(source, pc, admitted, buffer, capacity, &trace,
                      executor->ibtc, authority, expected_authority, NULL,
                      executor->diagnostics, executor->write_reserve,
                      executor->write_commit, executor->runtime_write_reserve,
  ...
  if (trace.provenance_count == 0)
    return HL_STATE("trace produced no provenance");
  emission = (hl_native_emission){.bytes = buffer,
                                  .size = trace.code_size,
                                  .body_offset = trace.body_offset,
                                  ...
                                  .instruction_count =
                                      (uint32_t)trace.instruction_count,
                                  .cycle_safe = 1};
  ...
  hl_native_status status =
      hl_native_translation_publish(executor, &key, &emission);
  if (status != HL_NATIVE_OK)
    return status;
  return hl_native_translation_lookup(executor, &key, code) == HL_NATIVE_HIT
             ? HL_NATIVE_OK
             : HL_STATE("published trace missed lookup");
}

hl_native_status hl_a64_trace_cache(hl_native_executor *executor,
                                    const hl_a64_source *source, uint64_t pc,
                                    size_t count, void *buffer, size_t capacity,
                                    hl_native_code *code, uint32_t *hit) {
  return hl_a64_trace_cache_direct(
      executor, source, pc, count, buffer, capacity, NULL,
