...
static int valid(uint32_t word) {
  ...
  if ((word & 0x5F20FC00u) != 0x1E200000u || (opcode != 6u && opcode != 7u))
    return 0;
...
int hl_a64_fp_move_emit(hl_a64_assembler *assembler, uint32_t word,
                        uint64_t pc) {
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_FP_MOVE_MAX_BYTES ||
      !valid(word))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_fp_move_body(assembler, word))
    return 0;
