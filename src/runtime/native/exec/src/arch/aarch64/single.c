...
static void add_immediate(hl_a64_assembler *assembler, int destination,
                          int source, int64_t value) {
  uint64_t magnitude =
  ...
  if (destination != source && value == 0)
    hl_a64_movr(assembler, destination, source);
}
...
static void effective_address(hl_a64_assembler *assembler, uint32_t word,
                              const hl_a64_memory *memory) {
  ...
  if (memory->kind == HL_A64_MEMORY_REGISTER) {
    ...
    unsigned shift =
        ((word >> 12) & 1u) ? (unsigned)__builtin_ctzll(memory->bytes) : 0;
    if (stolen(memory->index)) {
      hl_a64_ldr(assembler, 17, CPU, memory->index * 8);
      hl_a64_emit32(assembler, 0x8B200000u | (17u << 16) | (option << 13) |
                                   (shift << 10) | (16u << 5) | 16u);
    } else {
      hl_a64_emit32(assembler, 0x8B200000u | ((uint32_t)memory->index << 16) |
                                   (option << 13) | (shift << 10) | (16u << 5) |
                                   16u);
    }
...
static int valid(uint32_t word, uint64_t pc, hl_a64_memory *memory) {
  if (!hl_a64_memory_decode(word, pc, memory))
    return 0;
  if (!memory->vector &&
      ((word & UINT32_C(0x3b200c00)) == UINT32_C(0x38200400) ||
  ...
  if ((memory->kind != HL_A64_MEMORY_LITERAL &&
    ...
    return 0;
  return !(memory->writeback && memory->base != 31 && memory->read &&
           memory->base == memory->target);
}

int hl_a64_single_body(hl_a64_assembler *assembler, uint32_t word, uint64_t pc,
  ...
  int target_stolen;
  int64_t writeback = 0;
  if (assembler == NULL || guard == NULL || !valid(word, pc, &memory))
    return 0;
  ...
  if (!memory.read && target_stolen)
    hl_a64_ldr(assembler, 17, CPU, memory.target * 8);
  if (sites != NULL) {
    sites->count = 1;
  ...
  if (memory.read && target_stolen)
    hl_a64_str(assembler, 17, CPU, memory.target * 8);
  if (memory.writeback) {
    if (memory.base == 31) {
...
int hl_a64_single_direct_body(hl_a64_assembler *assembler, uint32_t word,
                              uint64_t pc,
                              const hl_native_direct_authority *authority,
                              hl_a64_guard *guard, hl_a64_memory_sites *sites) {
  hl_a64_memory memory;
  if (assembler == NULL || authority == NULL || guard == NULL ||
      sites == NULL || !valid(word, pc, &memory) || memory.vector ||
      memory.writeback || memory.bytes == 0 || memory.bytes > 8 ||
      (memory.kind != HL_A64_MEMORY_LITERAL &&
       memory.kind != HL_A64_MEMORY_UNSIGNED &&
       memory.kind != HL_A64_MEMORY_UNSCALED &&
       memory.kind != HL_A64_MEMORY_REGISTER) ||
      (authority->permissions &
       (memory.read ? HL_NATIVE_ACCESS_READ : HL_NATIVE_ACCESS_WRITE)) == 0)
    return 0;
  memset(guard, 0, sizeof(*guard));
  guard->pc = pc;
  if (memory.kind == HL_A64_MEMORY_LITERAL) {
    uint64_t address;
    if (memory.offset >= 0) {
      if (pc > UINT64_MAX - (uint64_t)memory.offset)
        return 0;
    ...
    if (address < authority->guest_first ||
        address > UINT64_MAX - memory.bytes ||
...
int hl_a64_single_emit(hl_a64_assembler *assembler, uint32_t word,
                       uint64_t pc) {
  hl_a64_memory memory;
  hl_a64_guard guard;
  if (assembler == NULL ||
      hl_a64_assembler_remaining(assembler) < HL_A64_SINGLE_MAX_BYTES ||
      !valid(word, pc, &memory))
    return 0;
  hl_a64_stub_prologue(assembler);
  if (!hl_a64_single_body(assembler, word, pc, &guard, NULL))
    return 0;
