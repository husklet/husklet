...
static int mergeable(const hl_native_aarch64_cpu *cpu) {
  if (cpu == NULL || cpu->dirty_first == UINT64_MAX ||
      cpu->dirty_first >= cpu->dirty_last ||
  ...
  for (uint64_t index = cpu->dirty_count; index != 0; --index) {
    ...
    if (record[0] == cpu->dirty_view_first &&
        record[1] == cpu->dirty_view_last && record[3] >= cpu->dirty_first &&
        cpu->dirty_last >= record[2])
      return (int)(index - 1);
  }
  return -1;
}
...
static const hl_a64_view *containing(const hl_a64_projection *projection,
                                     uint64_t address) {
  for (size_t index = 0; index < projection->count; index++) {
    const hl_a64_view *view = &projection->views[index];
...
int hl_a64_projection_resolve(const hl_a64_projection *projection,
  ...
  if (cpu == NULL || !hl_a64_projection_validate(projection) || size == 0 ||
