...
/* ADD, OR, AND, SUB and XOR are the ALU kinds with a single-instruction LSE
 * form. */
...
static int atomic_alu_register_opcode(uint8_t opcode) {
  return opcode < 0x40u && (opcode & 7u) <= 1u &&
         atomic_alu_kind((uint8_t)((opcode >> 3) & 7u));
}

/* The only register-only vector lowerings that can leave the block are the
   double and packed float arithmetic forms, which bail to the interpreter on an
   unordered result. */
...
static int may_fallback(const instruction *item) {
  return item->memory_operand != 0u || item->operation == OP_LOAD ||
         item->operation == OP_STORE || item->operation == OP_STRING ||
         item->operation == OP_MUL || item->operation == OP_DIV ||
         (item->operation == OP_IMUL && item->source_high != 0u) ||
         item->operation == OP_CALL ||
         (item->operation == OP_JUMP && item->source_high != 0u) ||
...
/* 1 iff the item redefines every flag the NZCV transfer publishes while reading
   none. adc/sbb read CF, inc/dec preserve it, and a memory operand can fault
   before the redefinition. imul qualifies: it redefines CF and OF, and this
   lowering stores SF and ZF too rather than leaving them undefined.
   comisd/ucomisd clear CF, PF, AF, ZF, SF and OF and then rewrite them from the
   comparison. */
static int kills_nzcv(const instruction *item) {
  if (item->operation == OP_IMUL)
    return item->source_high == 0u && item->memory_operand == 0u;
  if (item->operation == OP_VECTOR)
    return item->vector_kind == VECTOR_SCALAR_COMPARE_DOUBLE &&
           item->memory_operand == 0u;
  return item->operation == OP_ALU && item->memory_operand == 0u &&
         item->preserve_carry == 0u && item->preserve_flags == 0u &&
         item->alu_kind != 2u && item->alu_kind != 3u;
}

/* 1 iff the item redefines both PF and AF while reading neither, which adc/sbb
   and inc/dec also do even though they read or preserve CF. imul is absent
   because this lowering leaves PF and AF alone. */
static int kills_pfaf(const instruction *item) {
  if (item->operation == OP_VECTOR)
    return item->vector_kind == VECTOR_SCALAR_COMPARE_DOUBLE &&
           item->memory_operand == 0u;
  return item->operation == OP_ALU && item->memory_operand == 0u &&
         item->preserve_flags == 0u;
}

/* Whitelist of items with no flag semantics at all: they read no flag, write
   none, and cannot leave the block through anything that observes one. Anything
   unlisted counts as a reader, because missing a reader silently produces a
   wrong branch while missing a killer only costs words. The float arithmetic
   forms are listed even though they can bail to the interpreter: the bail
   resumes the identical guest stream at the same pc, so it reaches this scan's
   killer before any reader exactly as the translated block would. */
...
/* Additionally transparent for the NZCV half: a producer whose own NZCV is
   already dead emits no flag word at all, and rcl/rcr are excluded because they
   read CF into the rotated value. */
static int transparent_nzcv(const instruction *item) {
  ...
  if (item->memory_operand != 0u)
    return 0;
  if (item->operation == OP_IMUL)
    return item->source_high == 0u && item->nzcv_dead != 0u;
  if (item->operation == OP_ROTATE)
    return item->nzcv_dead != 0u &&
           (item->shift_kind == 0u || item->shift_kind == 1u);
  if (item->operation == OP_SHIFT)
    return item->nzcv_dead != 0u && item->pfaf_dead != 0u;
  return 0;
}

/* Additionally transparent for the PF/AF half: imul and the rotates redefine no
   part of it and read none of it, whatever they do to CF and OF. */
...
/* 1 iff no reader sees the half before something redefines it outright. End of
 * block is a reader. */
static int half_is_dead(const decode *block, uint32_t index,
                        int (*kills)(const instruction *),
...
/* A register ALU, shift, rotate or imul publishes nothing observable besides
   cpu->flags, so a later instruction redefining a half it wrote lets it skip
   that half's materialization. The two halves are tracked apart because their
   killer sets differ, and each producer consumes the ones it writes. The walk
   runs backwards so an already-elided producer is known to be crossable when it
   is reached. */
static void mark_dead_flags(decode *block) {
  ...
  for (index = block->count; index-- > 0;) {
    ...
    if (item->memory_operand != 0u)
      continue;
    if (item->operation == OP_IMUL) {
      /* This lowering leaves PF and AF alone, so only the NZCV half is its to
       * elide. */
      if (item->source_high != 0u)
        continue;
      item->nzcv_dead =
          (uint8_t)half_is_dead(block, index, kills_nzcv, transparent_nzcv);
      continue;
    }
    if (item->operation != OP_ALU && item->operation != OP_SHIFT &&
        item->operation != OP_ROTATE)
      continue;
    ...
    item->nzcv_dead =
        (uint8_t)half_is_dead(block, index, kills_nzcv, transparent_nzcv);
    item->pfaf_dead =
        (uint8_t)half_is_dead(block, index, kills_pfaf, transparent_pfaf);
  }
}
...
void hl_x86_prepare_vector_immediate(instruction *item, uint8_t modrm,
                                     uint8_t rex, uint8_t immediate,
                                     uint8_t destructive_rm) {
  item->vector_immediate_form = destructive_rm != 0u
...
static void decode_block(const hl_x86_a64_request *request, decode *block) {
  ...
  while (cursor < request->guest_size &&
         block->count < request->max_instructions) {
    size_t start = cursor;
    ...
    for (;;) {
      ...
      if (prefix == 0x66u)
        ...
        address_32 = 1;
      else if (prefix == 0x64u)
        segment = 1;
      else if (prefix == 0x65u)
        segment = 2;
    ...
    if ((request->guest_bytes[cursor] & 0xf0u) == 0x40u)
      rex = request->guest_bytes[cursor++];
    if (cursor >= request->guest_size) {
      cursor = start;
      block->status = HL_X86_A64_TRUNCATED;
      block->exit = HL_X86_A64_INTERPRETER;
    ...
    if ((opcode == 0xc4u || opcode == 0xc5u) && rex == 0u && operand_16 == 0u &&
      ...
      if (opcode == 0xc5u) {
        ...
        vex_l = (uint8_t)((vex_two >> 2) & 1u);
        vex_pp = (uint8_t)(vex_two & 3u);
      } else {
        uint8_t vex_three;
        ...
        vex_three = request->guest_bytes[cursor++];
        vex = 1u;
        vex_w = (uint8_t)((vex_three >> 7) & 1u);
        vex_vvvv = (uint8_t)((~vex_three >> 3) & 15u);
        vex_l = (uint8_t)((vex_three >> 2) & 1u);
        vex_pp = (uint8_t)(vex_three & 3u);
      }
      if (cursor >= request->guest_size || cursor - start >= 15u) {
        cursor = start;
        block->status = HL_X86_A64_TRUNCATED;
    ...
    if (vex == 0u && semantic_prefix == 0xf0u && opcode != 0xf6u &&
        opcode != 0xf7u && opcode != 0x86u && opcode != 0x87u &&
        !((opcode == 0x80u || opcode == 0x81u || opcode == 0x83u) &&
          (request->flags & HL_X86_A64_LSE) != 0u &&
          cursor < request->guest_size &&
          (request->guest_bytes[cursor] >> 6) != 3u &&
          atomic_alu_kind(
              (uint8_t)((request->guest_bytes[cursor] >> 3) & 7u))) &&
        !(atomic_alu_register_opcode(opcode) &&
          (request->flags & HL_X86_A64_LSE) != 0u &&
          cursor < request->guest_size &&
          (request->guest_bytes[cursor] >> 6) != 3u) &&
        !(opcode == 0x0fu && cursor < request->guest_size &&
          (request->guest_bytes[cursor] == 0xb0u ||
           request->guest_bytes[cursor] == 0xb1u ||
           request->guest_bytes[cursor] == 0xc0u ||
           request->guest_bytes[cursor] == 0xc1u))) {
      cursor = start;
      block->status = HL_X86_A64_UNSUPPORTED;
      block->exit = HL_X86_A64_INTERPRETER;
      break;
    ...
    if (vex == 0u && semantic_prefix != 0 && semantic_prefix != 0xf0u &&
        ...
        !(semantic_prefix == 0xf3u && opcode == 0x0fu &&
          ...
          (request->guest_bytes[cursor] == 0x6fu ||
           ...
           request->guest_bytes[cursor] == 0x2au ||
           request->guest_bytes[cursor] == 0x10u ||
           request->guest_bytes[cursor] == 0x11u ||
           request->guest_bytes[cursor] == 0xbcu ||
           request->guest_bytes[cursor] == 0xbdu ||
           (cursor + 1u < request->guest_size &&
            request->guest_bytes[cursor] == 0x1eu &&
            (request->guest_bytes[cursor + 1u] == 0xfau ||
             request->guest_bytes[cursor + 1u] == 0xfbu ||
             ((request->guest_bytes[cursor + 1u] >> 6) == 3u &&
              ((request->guest_bytes[cursor + 1u] >> 3) & 7u) == 1u))))) &&
        !(semantic_prefix == 0xf2u && opcode == 0x0fu &&
          cursor < request->guest_size &&
    ...
    if (vex != 0u && vex_map == 1u && opcode == 0x5bu && vex_pp <= 2u &&
      ...
      if ((modrm >> 6) == 3u)
        ...
        if (!hl_x86_decode_address(request, block, item, rex, 0u, 0u, start,
                                   &cursor))
          break;
        item->operation = OP_VECTOR;
      ...
      item->vector_kind =
          ...
          : vex_pp == 1u ? VECTOR_SHUFFLE_DWORD
                         : VECTOR_SHUFFLE_WORD;
      ...
      item->vector_lane =
          (uint8_t)(1u << (opcode - (opcode < 0x1cu ? 0x08u : 0x1cu)));
      if ((modrm >> 6) == 3u)
        ++cursor;
      else {
        if (!hl_x86_decode_address(request, block, item, rex, 0u, 0u, start,
      ...
      item->vector_source_one = vex_vvvv;
      item->vector_kind =
          opcode >= 0x4au ? VECTOR_BLEND_VARIABLE : VECTOR_BLEND_IMMEDIATE;
      ...
      if (item->vector_kind == VECTOR_BLEND_VARIABLE)
        item->vector_subopcode = (uint8_t)((item->vector_immediate >> 4) & 15u);
               ...
               ((vex_map == 1u &&
                 (opcode == 0x63u || opcode == 0x67u || opcode == 0x6bu)) ||
      ...
      if (opcode == 0xd4u || opcode >= 0xfcu) {
        ...
        item->vector_lane = opcode == 0xfcu   ? 1u
                            : opcode == 0xfdu ? 2u
                            : opcode == 0xfeu ? 4u
                                              : 8u;
      ...
      if ((modrm >> 6) == 3u)
        ...
        item->source = 16u;
               ...
               ((opcode >= 0x60u && opcode <= 0x62u) ||
                (opcode >= 0x68u && opcode <= 0x6au) || opcode == 0x6cu ||
                opcode == 0x6du)) {
      uint8_t modrm;
      if (cursor >= request->guest_size || cursor - start >= 15u) {
        cursor = start;
      ...
      item->vector_kind = opcode == 0x03u || opcode == 0x07u
                              ? VECTOR_HORIZONTAL_SATURATING
                          : opcode >= 0x05u ? VECTOR_HORIZONTAL_SUBTRACT
                                            : VECTOR_HORIZONTAL_ADD;
      if ((modrm >> 6) == 3u)
        ++cursor;
      else {
        if (!hl_x86_decode_address(request, block, item, rex, 0u, 0u, start,
        ...
        item->source = 16u;
               ...
               ((vex_map == 1u && (opcode == 0xdau || opcode == 0xdeu ||
                                   opcode == 0xeau || opcode == 0xeeu)) ||
      ...
      item->vector_lane = vex_map == 1u ? (opcode >= 0xeau ? 2u : 1u)
                                        : ((opcode & 3u) == 0u   ? 1u
                                           : (opcode & 3u) == 2u ? 2u
                                                                 : 4u);
      {
        uint8_t maximum = vex_map == 1u ? (opcode == 0xdeu || opcode == 0xeeu)
                                        : opcode >= 0x3cu;
        uint8_t unsigned_kind = vex_map == 1u
                                    ? opcode < 0xe0u
                                    : (opcode == 0x3au || opcode == 0x3bu ||
                                       opcode == 0x3eu || opcode == 0x3fu);
        item->vector_kind = maximum != 0u
                                ? (unsigned_kind != 0u ? VECTOR_MAXIMUM_UNSIGNED
                                                       : VECTOR_MAXIMUM_SIGNED)
                                : (unsigned_kind != 0u ? VECTOR_MINIMUM_UNSIGNED
      ...
      if ((modrm >> 6) == 3u)
        ...
        item->source = 16u;
               ...
               ((vex_map == 1u && ((opcode >= 0x64u && opcode <= 0x66u) ||
      ...
      block->exit = HL_X86_A64_INTERPRETER;
      break;
    } else if ((semantic_prefix == 0u || semantic_prefix == 0xf2u ||
      ...
      block->next_pc = next;
      block->exit = HL_X86_A64_DYNAMIC_BRANCH;
      ++block->count;
      break;
    } else if (semantic_prefix == 0xf3u && opcode == 0x0fu &&
               cursor + 1u < request->guest_size &&
               request->guest_bytes[cursor] == 0x1eu &&
               (request->guest_bytes[cursor + 1u] >> 6) == 3u &&
               ((request->guest_bytes[cursor + 1u] >> 3) & 7u) == 1u) {
      ...
      if (immediate_size > request->guest_size - cursor ||
          immediate_size > 15u - (cursor - start)) {
        cursor = start;
        block->status = HL_X86_A64_TRUNCATED;
        block->exit = HL_X86_A64_INTERPRETER;
        break;
      ...
      item->immediate =
          (uint64_t)load_signed(&request->guest_bytes[cursor], immediate_size);
      cursor += immediate_size;
    } else if (operand_16 != 0u && semantic_prefix == 0u && opcode == 0x0fu &&
               cursor + 2u < request->guest_size &&
               request->guest_bytes[cursor] == 0x3au &&
      ...
      if ((item->vector_immediate & 0x05u) != 0x00u) {
        ...
        break;
               ...
               cursor + 1u < request->guest_size &&
               request->guest_bytes[cursor] == 0x38u &&
      ...
      item->source = (uint8_t)((modrm & 7u) | ((rex & 1u) << 3));
      item->vector_kind =
          extension == 0xdcu ? VECTOR_AES_ENCRYPT : VECTOR_AES_ENCRYPT_LAST;
      if ((modrm >> 6) == 3u) {
        ++cursor;
      } else {
        if (!hl_x86_decode_address(request, block, item, rex, 0, address_32,
      ...
      item->vector_lane = extension == 0x71u   ? 2u
                          : extension == 0x72u ? 4u
                                               : 8u;
      item->vector_subopcode = (uint8_t)((modrm >> 3) & 7u);
      ...
      item->vector_immediate = request->guest_bytes[cursor++];
               ...
               cursor < request->guest_size &&
               request->guest_bytes[cursor] == 0xc4u) {
      uint8_t modrm;
      ++cursor;
      if (cursor >= request->guest_size || cursor - start >= 15u) {
        cursor = start;
      ...
      item->vector_kind = semantic_prefix == 0xf3u
                              ? VECTOR_TRUNC_FLOAT_TO_SIGNED_DWORD
      ...
      if ((modrm >> 6) == 3u) {
        ...
        item->source = 16u;
               ...
               (request->guest_bytes[cursor] == 0x58u ||
                ...
                request->guest_bytes[cursor] == 0x5cu ||
                request->guest_bytes[cursor] == 0x5eu) &&
               (semantic_prefix == 0xf2u || semantic_prefix == 0u ||
                semantic_prefix == 0xf3u)) {
      uint8_t extension = request->guest_bytes[cursor++];
      uint8_t modrm;
      if (cursor >= request->guest_size || cursor - start >= 15u) {
      ...
      item->vector_lane = semantic_prefix == 0xf2u ||
                                  (semantic_prefix == 0u && operand_16 != 0u)
                              ? 8u
                              : 4u;
    ...
    } else if (semantic_prefix == 0xf2u && opcode == 0x0fu &&
               cursor < request->guest_size &&
               request->guest_bytes[cursor] == 0x51u) {
      uint8_t modrm;
      ++cursor;
    ...
    } else if (semantic_prefix == 0u && opcode == 0x0fu &&
               cursor < request->guest_size &&
               (request->guest_bytes[cursor] == 0x2eu ||
                request->guest_bytes[cursor] == 0x2fu)) {
      uint8_t extension = request->guest_bytes[cursor++];
      uint8_t modrm;
    ...
    } else if ((semantic_prefix == 0xf2u || semantic_prefix == 0xf3u) &&
               opcode == 0x0fu && cursor < request->guest_size &&
               request->guest_bytes[cursor] == 0x2au) {
      uint8_t modrm;
      /* F3 produces a single, F2 a double; REX.W picks the integer source
       * width. */
      uint8_t lane = semantic_prefix == 0xf3u ? 4u : 8u;
      uint8_t integer = (rex & 8u) != 0u ? 8u : 4u;
      ++cursor;
      if (cursor >= request->guest_size || cursor - start >= 15u) {
        cursor = start;
        block->status = HL_X86_A64_TRUNCATED;
      ...
      item->destination = (uint8_t)(((modrm >> 3) & 7u) | ((rex & 4u) << 1));
      item->source = (uint8_t)((modrm & 7u) | ((rex & 1u) << 3));
      item->vector_kind = VECTOR_MERGE_FROM_INTEGER;
      if ((modrm >> 6) == 3u) {
        ++cursor;
      } else {
        if (!hl_x86_decode_address(request, block, item, rex, 0, address_32,
      ...
      if ((modrm >> 6) == 3u) {
        ...
        item->source = 16u;
               ...
               ((request->guest_bytes[cursor] == 0x70u &&
                 (operand_16 != 0u || semantic_prefix == 0xf2u ||
                  semantic_prefix == 0xf3u)) ||
      ...
      item->vector_kind = extension == 0xc6u
                              ? (operand_16 != 0u ? VECTOR_SHUFFLE_DOUBLE
                                                  : VECTOR_SHUFFLE_FLOAT)
                          : operand_16 != 0u ? VECTOR_SHUFFLE_DWORD
                                             : VECTOR_SHUFFLE_WORD;
      ...
      ++cursor;
      if (!hl_x86_decode_nop(request, block, item, start, &cursor))
        break;
    } else if (opcode == 0x0fu && cursor < request->guest_size &&
               ((semantic_prefix == 0u &&
                 (request->guest_bytes[cursor] == 0x28u ||
                  request->guest_bytes[cursor] == 0x29u)) ||
                (operand_16 != 0u &&
                 ((request->guest_bytes[cursor] >= 0x60u &&
                   request->guest_bytes[cursor] <= 0x62u) ||
                  (request->guest_bytes[cursor] >= 0x68u &&
                   request->guest_bytes[cursor] <= 0x6au) ||
                  request->guest_bytes[cursor] == 0x6cu ||
                  request->guest_bytes[cursor] == 0x6du ||
                  request->guest_bytes[cursor] == 0x6fu ||
                  (request->guest_bytes[cursor] >= 0x74u &&
                   request->guest_bytes[cursor] <= 0x76u) ||
                  request->guest_bytes[cursor] == 0x7fu ||
                  request->guest_bytes[cursor] == 0xd7u ||
                  request->guest_bytes[cursor] == 0xd4u ||
                  request->guest_bytes[cursor] == 0xd5u ||
                  request->guest_bytes[cursor] == 0xdbu ||
                  request->guest_bytes[cursor] == 0xdfu ||
                  request->guest_bytes[cursor] == 0xdau ||
                  request->guest_bytes[cursor] == 0xdeu ||
                  request->guest_bytes[cursor] == 0xeau ||
                  request->guest_bytes[cursor] == 0xeeu ||
                  request->guest_bytes[cursor] == 0xe4u ||
                  request->guest_bytes[cursor] == 0xe5u ||
                  request->guest_bytes[cursor] == 0xebu ||
                  request->guest_bytes[cursor] == 0xefu ||
                  request->guest_bytes[cursor] == 0xf4u ||
                  (request->guest_bytes[cursor] >= 0xf8u &&
                   request->guest_bytes[cursor] <= 0xfeu))) ||
                (semantic_prefix == 0xf3u &&
                 (request->guest_bytes[cursor] == 0x6fu ||
                  request->guest_bytes[cursor] == 0x7fu)))) {
      uint8_t extension = request->guest_bytes[cursor++];
      ...
      item->source = rm;
      item->vector_aligned =
          extension == 0x28u || extension == 0x29u ||
          (operand_16 != 0u && (extension == 0x6fu || extension == 0x7fu));
      if (extension == 0x28u || extension == 0x29u || extension == 0x6fu ||
          extension == 0x7fu) {
        ...
        item->vector_kind = VECTOR_ADD;
        item->vector_lane = extension == 0xfcu   ? 1u
                            : extension == 0xfdu ? 2u
                            : extension == 0xfeu ? 4u
                                                 : 8u;
      ...
      if ((modrm >> 6) == 3u) {
        ...
        if (!store && item->vector_kind != VECTOR_COPY)
          item->source = 16u;
      }
    } else if ((semantic_prefix == 0xf3u || semantic_prefix == 0xf2u) &&
               opcode == 0x0fu && cursor < request->guest_size &&
               (request->guest_bytes[cursor] == 0x10u ||
                request->guest_bytes[cursor] == 0x11u)) {
      uint8_t extension = request->guest_bytes[cursor++];
      ...
      item->operation = OP_VECTOR;
      item->width = lane;
      item->destination = extension == 0x10u ? reg : rm;
      item->source = extension == 0x10u ? rm : reg;
      if ((modrm >> 6) == 3u) {
        /* Register to register merges the low lane and preserves the rest. */
        item->vector_kind = VECTOR_MERGE_LOW;
        ++cursor;
      } else {
        /* A memory source zeroes the upper bits, so the plain sized load is
         * exact. */
        if (!hl_x86_decode_address(request, block, item, rex, 0, address_32,
                                   start, &cursor))
          break;
        ...
        item->source = reg;
               ...
               ((semantic_prefix == 0u &&
                 (request->guest_bytes[cursor] == 0x10u ||
                  request->guest_bytes[cursor] == 0x11u ||
                  (operand_16 != 0u &&
                   (request->guest_bytes[cursor] == 0x6eu ||
                    request->guest_bytes[cursor] == 0x7eu ||
      ...
      item->width = extension == 0x10u || extension == 0x11u       ? 16u
                    : semantic_prefix == 0xf3u || (rex & 8u) != 0u ? 8u
                                                                   : 4u;
      if (extension == 0x10u) {
        item->destination = reg;
        item->source = rm;
      ...
      if ((modrm >> 6) == 3u) {
        ...
        item->memory_operand = 1u;
        item->memory_write = extension == 0x11u ||
                             (extension == 0x7eu && semantic_prefix == 0u) ||
                             extension == 0xd6u;
        item->destination = vector;
        item->source = vector;
        item->vector_kind =
            item->memory_write != 0u && extension == 0x7eu   ? VECTOR_TO_INTEGER
            : item->memory_write != 0u && extension == 0xd6u ? VECTOR_ZERO_LOW
                                                             : VECTOR_COPY;
      }
      ...
      if (opcode == 0x0fu) {
        extension = request->guest_bytes[cursor];
        ++cursor;
      }
      ...
      item->source_high = opcode == 0x99u;
               ...
               cursor - start < 15u && request->guest_bytes[cursor] >= 0x40u &&
               request->guest_bytes[cursor] <= 0x4fu) {
      uint8_t extension = request->guest_bytes[cursor++];
      uint8_t modrm;
      ...
      if ((modrm >> 6) == 3u) {
        ...
        item->width = (rex & 8u) != 0 ? 8u : operand_16 != 0 ? 2u : 4u;
               ...
               request->guest_bytes[cursor] >= 0x90u &&
               request->guest_bytes[cursor] <= 0x9fu) {
      uint8_t extension = request->guest_bytes[cursor++];
      uint8_t modrm;
      uint8_t raw;
      ...
      cursor += immediate_size;
               ...
               cursor - start < 15u && request->guest_bytes[cursor] >= 0xc8u &&
               request->guest_bytes[cursor] <= 0xcfu) {
      uint8_t extension = request->guest_bytes[cursor++];
      ...
      item->width = (rex & 8u) != 0 ? 8u : 4u;
               ...
               cursor < request->guest_size &&
               (request->guest_bytes[cursor] == 0xa3u ||
                request->guest_bytes[cursor] == 0xabu ||
                request->guest_bytes[cursor] == 0xb3u ||
                request->guest_bytes[cursor] == 0xbbu ||
      ...
      if ((modrm >> 6) == 3u) {
        ...
        item->memory_operand = 1u;
               ...
               (opcode == 0x0fu && cursor < request->guest_size &&
                cursor - start < 15u &&
                (request->guest_bytes[cursor] == 0xb6u ||
                 request->guest_bytes[cursor] == 0xb7u ||
                 request->guest_bytes[cursor] == 0xbeu ||
                 request->guest_bytes[cursor] == 0xbfu))) {
      uint8_t extension =
          opcode == 0x0fu ? request->guest_bytes[cursor++] : 0x63u;
      uint8_t modrm;
      ...
      if (!hl_x86_decode_xadd(request, block, item, extension, rex, operand_16,
                              address_32, semantic_prefix == 0xf0u, start,
                              &cursor))
        break;
    } else if ((semantic_prefix == 0u || semantic_prefix == 0xf0u) &&
      ...
      if (!hl_x86_decode_store(request, block, item, 0x89u, rex, operand_16,
                               address_32, start, &cursor))
        break;
    } else if ((opcode == 0x88u || opcode == 0x89u) &&
               cursor < request->guest_size &&
               (request->guest_bytes[cursor] >> 6) != 3u) {
      if (!hl_x86_decode_store(request, block, item, opcode, rex, operand_16,
                               address_32, start, &cursor))
        break;
    } else if ((opcode == 0x8au || opcode == 0x8bu) &&
               cursor < request->guest_size &&
               (request->guest_bytes[cursor] >> 6) != 3u) {
      ...
      item->width = opcode == 0xc6u    ? 1u
                    : (rex & 8u) != 0u ? 8u
                    : operand_16 != 0u ? 2u
                                       : 4u;
      item->has_immediate = 1u;
      item->operand_immediate =
          opcode == 0xc6u ? request->guest_bytes[cursor]
          : item->width == 8u
      ...
      if ((modrm >> 6) != 3u && (byte != 0u || operand_16 == 0u)) {
        ...
        if ((opcode & 2u) == 0u) {
          ...
          /* LOCK ALU r,m reuses the atomic pre-image RMW rather than
           * load-modify-store. */
        ...
        cursor = start;
        block->status = (modrm >> 6) == 3u ? HL_X86_A64_FLAGS_ABI_REQUIRED
        ...
        item->flags_only = item->alu_kind == 7u;
               ...
               cursor < request->guest_size &&
               (request->guest_bytes[cursor] == 0xa4u ||
                request->guest_bytes[cursor] == 0xa5u ||
                request->guest_bytes[cursor] == 0xacu ||
                request->guest_bytes[cursor] == 0xadu)) {
      uint8_t extension = request->guest_bytes[cursor++];
      if (!hl_x86_decode_double_shift(request, block, item, extension, rex,
                                      operand_16, address_32, start, &cursor))
        break;
    } else if (hl_x86_test_opcode(opcode)) {
      if (!hl_x86_decode_test(request, block, item, opcode, rex, operand_16,
                              address_32, semantic_prefix == 0xf0u, start,
                              &cursor))
        break;
      ...
      if (!hl_x86_decode_immediate(request, block, item, opcode, rex,
                                   operand_16, address_32, start, &cursor))
        break;
      /* LOCK ALU imm,m reuses the atomic pre-image RMW rather than
       * load-modify-store. */
      if (semantic_prefix == 0xf0u) {
        item->operation = OP_XADD;
        item->atomic_alu = 1u;
      ...
      if (!hl_x86_decode_address(request, block, item, rex, operand_16,
                                 address_32, start, &cursor))
        break;
      ...
      if (4u > request->guest_size - cursor || 4u > 15u - (cursor - start) ||
          ...
          !branch_target(next, load_signed(&request->guest_bytes[cursor], 4u),
                         &block->target)) {
        cursor = start;
        block->status = HL_X86_A64_TRUNCATED;
        block->exit = HL_X86_A64_INTERPRETER;
      ...
      item->operation = OP_CALL;
      item->has_immediate = 1u;
      item->immediate = block->target;
      item->length = (uint8_t)(cursor - start);
      ++block->count;
      break;
    } else if (opcode == 0xc9u && operand_16 == 0u && address_32 == 0u &&
               semantic_prefix == 0u) {
      ...
      uint64_t next;
      if (opcode == 0xc2u &&
          (2u > request->guest_size - cursor || 2u > 15u - (cursor - start))) {
        cursor = start;
        block->status = HL_X86_A64_TRUNCATED;
        block->exit = HL_X86_A64_INTERPRETER;
        break;
      ...
      if (opcode == 0xc2u) {
        item->immediate = load_unsigned(&request->guest_bytes[cursor], 2u);
        cursor += 2u;
      }
      if (!add_pc(block->next_pc, cursor - start, &next)) {
        cursor = start;
      ...
      if (sub == 0u || sub == 1u) {
      ...
      } else if (opcode != 0xffu || (sub != 2u && sub != 4u) ||
                 operand_16 != 0u) {
        cursor = start;
        block->status = HL_X86_A64_UNSUPPORTED;
        block->exit = HL_X86_A64_INTERPRETER;
        ...
        item->operation = sub == 2u ? OP_CALL : OP_JUMP;
        item->width = 8u;
        if ((modrm >> 6) == 3u) {
          ++cursor;
          item->source = (uint8_t)((modrm & 7u) | ((rex & 1u) << 3));
        ...
        break;
               ...
               (opcode == 0x0fu && cursor < request->guest_size &&
                cursor - start < 15u && request->guest_bytes[cursor] >= 0x80u &&
                request->guest_bytes[cursor] <= 0x8fu)) {
      size_t immediate_size = opcode == 0x0fu ? 4u : 1u;
      uint8_t condition =
          opcode == 0x0fu ? (uint8_t)(request->guest_bytes[cursor++] & 0x0fu)
      ...
      if (immediate_size > request->guest_size - cursor ||
          ...
          !branch_target(
              next, load_signed(&request->guest_bytes[cursor], immediate_size),
              &block->target)) {
        block->status = HL_X86_A64_TRUNCATED;
        block->exit = HL_X86_A64_INTERPRETER;
        break;
      ...
      break;
               ...
               cursor - start < 15u && request->guest_bytes[cursor] == 0x05u) {
      ++cursor;
      if (!add_pc(block->next_pc, cursor - start, &block->next_pc))
        block->status = HL_X86_A64_TRUNCATED;
      block->exit = block->status == HL_X86_A64_OK ? HL_X86_A64_SYSCALL
                                                   : HL_X86_A64_INTERPRETER;
      item->operation = OP_CONTROL;
      item->length = (uint8_t)(cursor - start);
      ++block->count;
      break;
  ...
  if (block->status == HL_X86_A64_UNSUPPORTED && refusal < request->guest_size)
    hl_native_state_untranslatable_x86(&request->guest_bytes[refusal],
                                       request->guest_size - refusal,
                                       block->count == 0);
  if (block->count != 0 && block->status == HL_X86_A64_UNSUPPORTED)
    block->status = HL_X86_A64_OK;
}

/* Host words the block needs, and via `fit` the widest instruction prefix that
 * would still fit. */
static uint32_t block_words(const hl_x86_a64_request *request,
                            const decode *block, int checkpoints,
  ...
  *fit = 0;
  for (index = 0; index < block->count; ++index) {
    ...
    if (block->instructions[index].operation == OP_MOV_IMMEDIATE) {
      ...
      dirty |= UINT32_C(1) << 7;
      if (block->instructions[index].condition == 0u)
        dirty |= UINT32_C(1) << 6;
      if (block->instructions[index].condition == 2u)
        dirty |= UINT32_C(1);
      if (block->instructions[index].conditional != 0u)
        dirty |= UINT32_C(1) << 1;
      ...
      if (!block->instructions[index].flags_only &&
          (block->instructions[index].memory_operand == 0u ||
           block->instructions[index].memory_write == 0u))
        dirty |= UINT32_C(1) << block->instructions[index].destination;
      ...
      dirty |= UINT32_C(1) << 0;
      if (block->instructions[index].width != 1u)
        dirty |= UINT32_C(1) << 2;
      ...
      if (block->instructions[index].vector_kind == VECTOR_STRING_EQUAL_EACH)
        dirty |= UINT32_C(1) << 1;
      else if (block->instructions[index].vector_kind ==
      ...
      dirty |= UINT32_C(1) << 0;
      if (block->instructions[index].source_high != 0u)
        dirty |= UINT32_C(1) << 2;
      ...
      if (block->instructions[index].operation == OP_POP)
        dirty |= UINT32_C(1) << 4 |
                 UINT32_C(1) << block->instructions[index].destination;
    }
    /* Widest prefix whose own fallthrough exit still fits, used to split rather
     * than abandon. */
    if (index + 1u < block->count) {
      ...
      if (words + tail <= request->host_capacity)
        *fit = index + 1u;
    }
  }
  words += (live_chain ? 0u : bit_count(dirty)) +
           (checkpoints ? (live_chain ? 1u : 3u) : 0u);
  if (block->exit == HL_X86_A64_CONDITIONAL_BRANCH &&
      (request->flags & HL_X86_A64_CONDITIONAL_SELF_LOOP) != 0u &&
      block->target == request->guest_pc)
    words += (live_chain ? 64u : 26u) + constant_words(block->next_pc);
  else if (block->exit == HL_X86_A64_CONDITIONAL_BRANCH && block->count != 0u)
    words += (block->instructions[block->count - 1u].condition == 0xau ||
                      block->instructions[block->count - 1u].condition == 0xbu
                  ? 5u
                  : 35u) +
             constant_words(block->target) + constant_words(block->next_pc);
  else
    words += constant_words(block->exit == HL_X86_A64_DIRECT_BRANCH
                                ? block->target
                                : block->next_pc) +
             1u;
...
hl_x86_a64_status hl_x86_a64_emit(const hl_x86_a64_request *request,
  ...
  if (!hl_x86_request_valid(request, result))
    return request != NULL && hl_x86_a64_unknown_flags(request->flags) != 0u
               ? HL_X86_A64_UNKNOWN_FLAG
               : HL_X86_A64_ARGUMENT;
  hl_x86_set_write_diagnostics((request->flags & HL_X86_A64_DIAGNOSTICS) != 0u);
  decode_block(request, &block);
  if (block.count > request->provenance_capacity)
    return HL_X86_A64_CAPACITY;
  for (;;) {
    uint32_t fit = 0;
    mark_dead_flags(&block);
    words = block_words(request, &block, checkpoints, live_chain, &fit);
    if (words <= request->host_capacity)
      break;
    if (fit == 0)
      return HL_X86_A64_CAPACITY;
    /* Split at an instruction boundary rather than abandoning the block; the
     * dropped tail is re-entered at its own guest pc, exactly as a
     * max_instructions truncation is. */
    block.count = fit;
    block.exit = HL_X86_A64_FALLTHROUGH;
  ...
  for (index = 0; index < block.count; ++index) {
    ...
    if (checkpoints && may_fallback(item)) {
      if (!live_chain)
        hl_x86_spill(request->host_words, &words, dirty);
      dirty = 0;
      hl_x86_checkpoint(request->host_words, &words, index - accounted,
                        live_chain);
      accounted = index;
    }
    ...
    if (item->operation == OP_MOV_IMMEDIATE) {
      ...
      if (item->width == 2u)
        request->host_words[words++] =
            UINT32_C(0xb3403c00) | 16u << 5 | item->destination;
      dirty |= UINT32_C(1) << item->destination;
    } else if (item->operation == OP_MOV_REGISTER) {
      if (item->width == 2u)
        request->host_words[words++] = UINT32_C(0xb3403c00) |
                                       (uint32_t)item->source << 5 |
                                       item->destination;
      else
        request->host_words[words++] =
      ...
      request->host_words[words++] =
          (item->width == 8u ? UINT32_C(0xdac00c00) : UINT32_C(0x5ac00800)) |
      ...
      if (item->source_high == 4u && item->signed_extend != 0)
        request->host_words[words++] =
            UINT32_C(0x93407c00) | source << 5 | target;
      else if (item->source_high == 4u)
        request->host_words[words++] =
            UINT32_C(0x2a0003e0) | source << 16 | target;
        ...
        request->host_words[words++] =
            (source_bytes == 1u ? UINT32_C(0x12001c00) : UINT32_C(0x12003c00)) |
            source << 5 | target;
      if (item->width == 2u)
        request->host_words[words++] =
            UINT32_C(0xb3403c00) | 16u << 5 | item->destination;
      ...
      request->host_words[words++] =
          (item->destination_high != 0 ? UINT32_C(0xb3781c00)
                                       : UINT32_C(0xb3401c00)) |
      ...
      if (!item->flags_only &&
          (item->memory_operand == 0u || item->memory_write == 0u))
        dirty |= UINT32_C(1) << item->destination;
  ...
  if (!live_chain)
    hl_x86_spill(request->host_words, &words, dirty);
  if (checkpoints) {
    hl_x86_checkpoint(request->host_words, &words, block.count - accounted,
                      live_chain);
  }
