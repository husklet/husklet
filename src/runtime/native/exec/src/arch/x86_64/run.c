...
static int host_has_lse(void) {
  ...
  return sysctlbyname("hw.optional.armv8_1_atomics", &available, &size, NULL,
                      0) == 0 &&
...
static int host_has_aes(void) {
  ...
  int available = 0;
  size_t size = sizeof available;
  return sysctlbyname("hw.optional.arm.FEAT_AES", &available, &size, NULL, 0) ==
             0 &&
         size == sizeof available && available != 0;
#else
  return 0;
...
static int view_contains(const hl_native_projection_view *view,
...
static int view_resolve(x86_run_views *cache, hl_native_x86_64_cpu *cpu,
  ...
  for (size_t index = 0; index < cache->count; ++index) {
    ...
    if (!view_contains(&cache->entries[index], address, size, required))
      continue;
    view_promote(cache, index);
    projection = (hl_native_projection){
        &cache->entries[0], 1, cache->entries[0].mapping_incarnation, 0};
...
static int view_subsumes(const hl_native_projection_view *wide,
  ...
  return wide->guest_first == narrow->guest_first &&
         wide->guest_last == narrow->guest_last &&
...
static void view_install(x86_run_views *cache,
  ...
  for (size_t index = cache->count; index > 0; --index) {
    if (view_subsumes(&cache->entries[index - 1], view))
      return;
    if (view_subsumes(view, &cache->entries[index - 1]))
      view_discard(cache, index - 1);
  }
  if (cache->count < X86_RUN_VIEW_COUNT)
    cache->count++;
  if (cache->count > 1)
    memmove(&cache->entries[1], &cache->entries[0],
...
static int rep_decode(const uint8_t *bytes, size_t size, x86_rep *rep) {
  uint8_t opcode;
  if (bytes == NULL || rep == NULL || size < 2 || bytes[0] != 0xf3u)
    return 0;
...
static const hl_native_projection_view *
rep_view(const x86_run_views *cache, const hl_native_projection *projection,
         uint64_t address, size_t width, uint32_t permissions) {
  if (cache != NULL) {
    for (size_t index = 0; index < cache->count; ++index) {
...
static size_t rep_span(const hl_native_projection_view *view, uint64_t address,
                       size_t width, int backward) {
  uint64_t bytes = backward ? address - view->guest_first + width
                            : view->guest_last - address;
  if (bytes > X86_REP_CHUNK_BYTES)
    bytes = X86_REP_CHUNK_BYTES;
  return (size_t)(bytes - bytes % width);
}

static void rep_copy(void *destination, const void *source, size_t count,
                     size_t width, int backward) {
  ...
  for (size_t index = 0; index < count; ++index) {
    ...
    const uint8_t *source_element =
        backward ? src - index * width : src + index * width;
    uint8_t *destination_element =
        backward ? dst - index * width : dst + index * width;
    memcpy(&element, source_element, width);
    memcpy(destination_element, &element, width);
  }
}

static void rep_fill(void *destination, uint64_t value, size_t count,
                     size_t width, int backward) {
  uint8_t *dst = destination;
...
static x86_rep_result rep_execute(const x86_rep *rep,
                                  const x86_run_views *cache,
  ...
  if (*budget == 0 || destination > UINT64_MAX - rep->width)
    return X86_REP_SCALAR;
  destination_view =
      rep_view(cache, full_projection, destination, rep->width, 2u);
  if (destination_view == NULL)
    return X86_REP_SCALAR;
  destination_span =
      rep_span(destination_view, destination, rep->width, backward);
  if (rep->move) {
    if (source > UINT64_MAX - rep->width)
      return X86_REP_SCALAR;
  ...
  if (rep->move && elements > source_span / rep->width)
    elements = source_span / rep->width;
  if ((uint64_t)elements > count)
    elements = (size_t)count;
  if ((uint64_t)elements > *budget)
    elements = (size_t)*budget;
  if (elements == 0)
    return X86_REP_SCALAR;
  ...
  if (!hl_x86_projection_switch_writable(cpu, destination_view->guest_first,
                                         first + bytes))
    return X86_REP_EPOCH;
  if (!hl_x86_projection_resolve(&destination_projection, cpu, first, bytes,
                                 2u))
    return X86_REP_SCALAR;
  uint8_t *destination_host =
      (uint8_t *)(uintptr_t)(destination_view->host_first + destination -
                             destination_view->guest_first);
  if (rep->move) {
    const uint8_t *source_host =
        (const uint8_t *)(uintptr_t)(source_view->host_first + source -
                                     source_view->guest_first);
    rep_copy(destination_host, source_host, elements, rep->width, backward);
  } else {
    rep_fill(destination_host, cpu->registers[0], elements, rep->width,
             backward);
  }
  ...
  if (!hl_x86_projection_written(cpu, first, bytes))
    return X86_REP_FATAL;
  step = (uint64_t)bytes;
  cpu->registers[7] = backward ? destination - step : destination + step;
  if (rep->move)
    cpu->registers[6] = backward ? source - step : source + step;
...
static void view_publish(const x86_run_views *cache, hl_native_x86_64_cpu *cpu,
  ...
  cpu->read_count = 0;
  if (count > X86_RUN_VIEW_COUNT)
    count = X86_RUN_VIEW_COUNT;
  for (size_t index = 0; index < count; ++index) {
    const hl_native_projection_view *view = &cache->entries[index];
...
static size_t source_bytes(const hl_native_source *source, uint64_t pc,
                           const uint8_t **bytes) {
  size_t index;
  for (index = 0; index < source->span_count; ++index) {
    const hl_native_source_span *span = &source->spans[index];
...
static hl_native_status leave_exit(hl_native_execution *execution,
                                   hl_native_exit *output, uint32_t kind,
                                   uint64_t pc) {
  if (execution->owner != NULL && execution->owner->diagnostics) {
    execution->owner->x86_public_exits++;
    if (kind == HL_NATIVE_EXIT_EPOCH)
      execution->owner->x86_public_epochs++;
  }
  return hl_native_execution_exit(execution, output, kind,
                                  HL_NATIVE_ACCESS_UNKNOWN, pc, pc, 0, 0);
}

static hl_native_status fatal_exit(hl_native_execution *execution,
...
static hl_native_status target_fallback(hl_native_execution *execution,
  ...
  return hl_native_execution_exit(execution, output, HL_NATIVE_EXIT_FALLBACK,
                                  HL_NATIVE_ACCESS_EXECUTE, cpu->indirect_site,
                                  cpu->program, cpu->program, 0);
}
...
_Static_assert(
    offsetof(hl_native_x86_64_cpu, registers) % (2u * sizeof(uint64_t)) == 0,
    "paired x86 register spills require 16-byte-aligned register storage");
_Static_assert(
    sizeof(((hl_native_x86_64_cpu *)0)->registers) == 16u * sizeof(uint64_t),
    "paired x86 register spills require sixteen contiguous registers");

static uint32_t store_pair_word(unsigned first, unsigned second,
                                size_t offset) {
  return UINT32_C(0xa9000000) |
         ((uint32_t)(offset / 8u) & UINT32_C(0x7f)) << 15 | second << 10 |
         28u << 5 | first;
}
...
static hl_native_status emit_block(hl_native_executor *executor,
                                   const hl_native_source *source, uint64_t pc,
                                   const uint8_t *bytes, size_t size,
                                   uint64_t memory_mode,
                                   uint64_t authority_generation,
                                   const hl_native_direct_token *direct_token,
                                   uint64_t direct_generation, int *supported) {
  ...
  hl_x86_a64_request request = {
      ...
      .flags = HL_X86_A64_CHECKPOINTS | HL_X86_A64_CONDITIONAL_SELF_LOOP |
               HL_X86_A64_LIVE_CHAIN | (host_has_lse() ? HL_X86_A64_LSE : 0u) |
               (host_has_aes() ? HL_X86_A64_AES : 0u) |
               (executor->diagnostics ? HL_X86_A64_DIAGNOSTICS : 0u),
  };
  hl_x86_a64_result result = {.abi = HL_X86_A64_FRONTEND_ABI,
                              .size = sizeof(result)};
  ...
  *supported = 0;
  if (frontend != HL_X86_A64_OK || result.instruction_count == 0 ||
      ...
      result.word_count > X86_MAX_WORDS - 32u || result.source_end <= pc)
    return HL_NATIVE_OK;
  reason = result.exit == HL_X86_A64_SYSCALL       ? HL_NATIVE_EXIT_SYSCALL
           : result.exit == HL_X86_A64_INTERPRETER ? HL_NATIVE_EXIT_FALLBACK
                                                   : HL_NATIVE_EXIT_BRANCH;
  int self_loop = result.exit == HL_X86_A64_CONDITIONAL_BRANCH &&
                  result.branch_target == pc;
  ...
  if (self_loop) {
    ...
    words[cursor++] =
        load_word(16u, offsetof(hl_native_x86_64_cpu, executable_written));
    executable_return = cursor++;
    emit_constant(words, &cursor, 16u,
                  frontend_map[result.instruction_count - 1u].guest_pc);
    words[cursor++] = store_word(16u, offsetof(hl_native_x86_64_cpu, scratch) +
                                          sizeof(uint64_t));
    if (result.exit == HL_X86_A64_DYNAMIC_BRANCH) {
      ...
      words[cursor++] =
          store_word(31u, offsetof(hl_native_x86_64_cpu, indirect_site));
    ...
    words[cursor++] =
        UINT32_C(0xd2800000) | ((reason & UINT32_C(0xffff)) << 5) | 16u;
    ...
    if (result.exit == HL_X86_A64_CONDITIONAL_BRANCH) {
      ...
      words[taken_branch] = UINT32_C(0x54000000) |
                            ((taken_tail - taken_branch) & UINT32_C(0x7ffff))
                                << 5;
  ...
  for (uint32_t index = 0; index < return_count; ++index)
    words[return_sites[index]] =
        direct_branch(return_sites[index], guard_return);
  if (executable_return != 0)
    words[executable_return] =
        UINT32_C(0x37000000) | /* tbnz x16,#2,guard_return */
        ((guard_return - executable_return) & UINT32_C(0x3fff)) << 5 |
        2u << 19 | 16u;
  for (uint32_t index = 0; index < relocation_count; ++index) {
    uint32_t site =
        (uint32_t)(relocations[index].code_offset / sizeof(uint32_t));
    relocations[index].expected = words[site];
  }
  words[entry_irq] = UINT32_C(0xb5000000) |
                     ((guard_return - entry_irq) & UINT32_C(0x7ffff)) << 5 |
                     16u;
  words[entry_budget] =
      UINT32_C(0x54000000) |
  ...
  for (uint32_t index = 0; index < result.provenance_count; ++index) {
    if (frontend_map[index].word_end == frontend_map[index].word_start)
      continue;
    provenance[provenance_count++] = (hl_native_provenance){
        .code_offset = (frontend_map[index].word_start + X86_ENTRY_WORDS) *
                       sizeof(uint32_t),
        .code_size =
            (frontend_map[index].word_end - frontend_map[index].word_start) *
            sizeof(uint32_t),
...
hl_native_status hl_native_x86_64_run(hl_native_executor *executor,
                                      hl_native_x86_64_cpu *cpu,
  ...
  if (request->projection != NULL) {
    const hl_native_projection_view *view =
        &request->projection->views[request->projection->active];
  ...
  hl_native_status epoch_status =
      memory_mode == 0
          ? hl_native_synchronize_epoch(executor, source->mapping_incarnation,
                                        0, 0, 0)
          : hl_native_synchronize_direct(
                executor, source->mapping_incarnation, 0, direct_token,
                authority_generation, authority_identity, request->projection);
  ...
  for (;;) {
    ...
    if (execution.owner == NULL) {
      ...
      if (memory_mode != 0 && !hl_native_direct_request_valid(
                                  executor, direct_token, authority_generation,
                                  authority_identity, request->projection)) {
        (void)hl_native_execution_leave(&execution);
        return HL_NATIVE_STATE;
      }
    }
    if (cpu->interrupt != 0)
      return leave_exit(&execution, output, HL_NATIVE_EXIT_INTERRUPT, pc);
    if (budget == 0) {
    ...
    x86_rep rep;
    x86_rep_result rep_result =
        size != 0 && rep_decode(bytes, size, &rep)
            ? rep_execute(&rep, &operand_views, request->projection, cpu,
                          &budget)
            : X86_REP_SCALAR;
    if (rep_result == X86_REP_FATAL)
      return fatal_exit(&execution, output, X86_FATAL_REASON);
    if (rep_result == X86_REP_EPOCH)
    ...
    if (rep_result == X86_REP_COMPLETE) {
      ...
      if ((cpu->executable_written & 4u) != 0)
        return leave_exit(&execution, output, HL_NATIVE_EXIT_EPOCH,
                          cpu->program);
      continue;
    }
    lookup = (hl_native_translation_key){
        ...
        .authority_generation =
            memory_mode != 0 ? authority_identity : authority_generation,
    };
    if (hl_native_translation_lookup(executor, &lookup, &code) !=
        HL_NATIVE_HIT) {
      ...
      if (size == 0 && resolver != NULL) {
        ...
        if (resolver(resolver_context, pc, source->mapping_incarnation,
          ...
          resolved_source =
              (hl_native_source){&resolved_span, 1, source->mapping_incarnation,
                                 resolved_span.instruction_epoch};
          if (!source_valid(&resolved_source))
            return HL_NATIVE_ARGUMENT;
      ...
      if (size != 0 && hl_native_translation_lookup(executor, &lookup, &code) ==
                           HL_NATIVE_HIT) {
        supported = 1;
      } else if (size != 0) {
        if (cold_builds == X86_COLD_BUILD_QUOTA) {
          if (executor->diagnostics)
            atomic_fetch_add_explicit(&executor->x86_cold_quota_exits, 1,
                                      memory_order_relaxed);
          if ((status = execution_enter(executor, &execution, cpu)) !=
              HL_NATIVE_OK)
            return status;
          return leave_exit(&execution, output, HL_NATIVE_EXIT_YIELD, pc);
        }
        cold_builds++;
        status = emit_block(
            executor, active_source, pc, bytes, size, memory_mode,
            memory_mode != 0 ? authority_identity : authority_generation,
            direct_token, authority_generation, &supported);
        if (executor->diagnostics) {
          atomic_fetch_add_explicit(&executor->x86_cold_builds, 1,
                                    memory_order_relaxed);
        }
      }
      if (size != 0 && status != HL_NATIVE_OK)
        return status;
      if (size == 0 || !supported) {
        if ((status = execution_enter(executor, &execution, cpu)) !=
            HL_NATIVE_OK)
          return status;
        if (size == 0)
          return target_fallback(&execution, output, cpu);
        return leave_exit(&execution, output, HL_NATIVE_EXIT_FALLBACK, pc);
      }
      if ((status = execution_enter(executor, &execution, cpu)) != HL_NATIVE_OK)
        return status;
      if (hl_native_translation_lookup(executor, &lookup, &code) !=
          HL_NATIVE_HIT)
        return leave_exit(&execution, output, HL_NATIVE_EXIT_EPOCH, pc);
    ...
    if (budget < code.instruction_count)
      return leave_exit(&execution, output, HL_NATIVE_EXIT_YIELD, pc);
    if (cpu->indirect_site != 0) {
      if (executor->diagnostics) {
    ...
    int loop_active =
        code.conditional_self_loop != 0 && code.identity_token != 0 &&
        code.mapping_epoch == source->mapping_incarnation &&
        code.instruction_epoch == lookup.instruction_epoch &&
        code.loop_pc == pc && code.source_first == pc && code.source_last > pc;
    if (loop_active) {
...
#define X86_DRAIN(counter, field)                                              \
  atomic_fetch_add_explicit(&executor->counter, cpu->field,                    \
                            memory_order_relaxed);                             \
  cpu->field = 0
...
#undef X86_DRAIN
    ...
    if (loop_active) {
      if (cpu->loop_block_count != code.instruction_count ||
          cpu->loop_pc != pc ||
    ...
    if (completed > budget)
      return fatal_exit(&execution, output, X86_FATAL_COMPLETED);
    budget -= completed;
    cpu->budget = budget;
    ...
    if (cpu->interrupt != 0)
      return leave_exit(&execution, output, HL_NATIVE_EXIT_INTERRUPT,
                        cpu->program);
    ...
    if (cpu->reason == HL_NATIVE_EXIT_FALLBACK && cpu->fault_access != 0 &&
      ...
      if (result == HL_NATIVE_OPERAND_RESOLVED) {
        resolved_projection =
            (hl_native_projection){&view, 1, source->mapping_incarnation, 0};
        if (!hl_x86_projection_validate(&resolved_projection) ||
            !hl_x86_projection_resolve(&resolved_projection, cpu,
      ...
      return leave_exit(&execution, output, HL_NATIVE_EXIT_FALLBACK,
                        cpu->program);
    }
    ...
    if (cpu->reason == HL_NATIVE_EXIT_SYSCALL) {
      if (executor->diagnostics) {
        ...
        if (vector_dirty != 0)
          executor->x86_syscall_vector_dirty++;
      }
      return hl_native_execution_exit(
          &execution, output, HL_NATIVE_EXIT_SYSCALL, HL_NATIVE_ACCESS_UNKNOWN,
          cpu->scratch[1], cpu->program, 0, 0);
    }
    if (cpu->reason == HL_NATIVE_EXIT_FALLBACK)
      return leave_exit(&execution, output, HL_NATIVE_EXIT_FALLBACK,
                        cpu->program);
