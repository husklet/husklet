...
enum vector_operation {
  ...
  VECTOR_MULTIPLY_EVEN_SIGNED_DWORD,
  VECTOR_MULTIPLY_LOW_DWORD,
  VECTOR_AVERAGE_UNSIGNED,
  VECTOR_SUM_ABSOLUTE_DIFFERENCES_BYTE,
  VECTOR_HORIZONTAL_ADD,
  VECTOR_HORIZONTAL_SUBTRACT,
...
typedef struct instruction {
  ...
  /* Set on a LOCKed ALU RMW retagged onto the atomic pre-image path, which
   * keeps its own alu_kind and has no XADD pre-image writeback. */
  uint8_t atomic_alu;
...
static inline int add_pc(uint64_t pc, uint64_t amount, uint64_t *result) {
  if (amount > UINT64_MAX - pc)
    return 0;
  *result = pc + amount;
  return 1;
}

static inline int branch_target(uint64_t next, int64_t displacement,
                                uint64_t *target) {
  uint64_t magnitude;

  if (displacement >= 0)
    return add_pc(next, (uint64_t)displacement, target);
