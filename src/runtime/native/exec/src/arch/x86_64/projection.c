...
static void flush_dirty(hl_native_x86_64_cpu *cpu) {
  ...
  for (uint64_t index = 0; index < cpu->dirty_count; ++index) {
    ...
    if (record[0] != cpu->dirty_view_first ||
        record[1] != cpu->dirty_view_last || record[2] > cpu->dirty_last ||
        record[3] < cpu->dirty_first)
      continue;
    if (cpu->dirty_first < record[2])
      record[2] = cpu->dirty_first;
    if (cpu->dirty_last > record[3])
...
int hl_x86_projection_switch_writable(const hl_native_x86_64_cpu *cpu,
  ...
  if (cpu == NULL || memory_first >= memory_last)
    return 0;
  if (cpu->dirty_first == UINT64_MAX ||
...
containing(const hl_native_projection *projection, uint64_t address) {
  size_t index;
  for (index = 0; index < projection->count; ++index) {
    const hl_native_projection_view *view = &projection->views[index];
    if (address >= view->guest_first && address < view->guest_last)
      return view;
    if (view->guest_first > address)
      break;
  }
  return NULL;
}
...
int hl_x86_projection_validate(const hl_native_projection *projection) {
  uint64_t previous = 0;
  size_t index;
  if (projection == NULL || projection->views == NULL ||
      projection->count == 0 || projection->active >= projection->count ||
  ...
  for (index = 0; index < projection->count; ++index) {
    ...
    if (view->guest_last <= view->guest_first ||
        ...
        view->host_first >
            UINT64_MAX - (view->guest_last - view->guest_first) ||
...
int hl_x86_projection_resolve(const hl_native_projection *projection,
                              hl_native_x86_64_cpu *cpu, uint64_t address,
  ...
  if (cpu->memory_first != first || cpu->memory_last != cursor)
    flush_dirty(cpu);
...
int hl_x86_projection_written(hl_native_x86_64_cpu *cpu, uint64_t address,
                              uint64_t size) {
  if (cpu == NULL || size == 0 || address < cpu->memory_first ||
      address > UINT64_MAX - size || address + size > cpu->memory_last ||
      (cpu->memory_permissions & 2u) == 0) {
    if (cpu != NULL) {
      cpu->fault_address = address;
      cpu->fault_access = 2;
      cpu->fault_size = size;
  ...
  if (cpu->dirty_first == UINT64_MAX || address < cpu->dirty_first)
    cpu->dirty_first = address;
  if (address + size > cpu->dirty_last)
    cpu->dirty_last = address + size;
