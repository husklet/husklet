...
uint32_t hl_x86_bit(unsigned destination, unsigned source, unsigned lsb) {
  return UINT32_C(0xd3400000) | lsb << 16 | lsb << 10 | source << 5 |
         destination;
}
...
void hl_x86_emit_nzcv_keep_carry(uint32_t *words, uint32_t *cursor) {
  ...
  words[(*cursor)++] = load_word(20, offsetof(hl_native_x86_64_cpu, flags));
  words[(*cursor)++] =
      UINT32_C(0x927ff800) | 20u << 5 | 20u; /* and x20,x20,#~1 */
...
static uint32_t extract_byte(unsigned destination, unsigned source,
                             unsigned high) {
  unsigned lsb = high ? 8u : 0u;

  return UINT32_C(0xd3400000) | lsb << 16 | (lsb + 7u) << 10 | source << 5 |
         destination;
}
...
static void emit_pfaf(uint32_t *words, uint32_t *cursor, int logical) {
  words[(*cursor)++] = load_word(20, offsetof(hl_native_x86_64_cpu, flags));
  emit_constant(words, cursor, 23,
                HL_X86_RFLAGS_PF | HL_X86_RFLAGS_AF |
                    (logical ? HL_X86_RFLAGS_CF | HL_X86_RFLAGS_OF : 0));
  words[(*cursor)++] = UINT32_C(0x8a200000) | 23u << 16 | 20u << 5 | 20u;
  words[(*cursor)++] = move_register(22, 18, 1);
  words[(*cursor)++] =
      UINT32_C(0xca400000) | 22u << 16 | 4u << 10 | 22u << 5 | 22u;
  words[(*cursor)++] =
      UINT32_C(0xca400000) | 22u << 16 | 2u << 10 | 22u << 5 | 22u;
  words[(*cursor)++] =
      UINT32_C(0xca400000) | 22u << 16 | 1u << 10 | 22u << 5 | 22u;
  emit_constant(words, cursor, 23, 1);
  words[(*cursor)++] = UINT32_C(0xca000000) | 23u << 16 | 22u << 5 | 22u;
...
uint32_t hl_x86_alu_words(const instruction *item) {
  ...
  int logical =
      item->alu_kind == 1u || item->alu_kind == 4u || item->alu_kind == 6u;
  uint64_t immediate =
      item->memory_operand != 0u ? item->operand_immediate : item->immediate;
  uint32_t words =
      1u + (item->has_immediate ? constant_words(immediate) : 1u) +
      (logical ? 2u : 1u) +
      (item->nzcv_dead ? 0u : 18u + (item->preserve_carry ? 6u : 0u)) +
      (item->pfaf_dead ? 0u : (logical ? 13u : 18u));
  /* A logical op's CF and OF are cleared by the PF/AF tail, so eliding only
     that half still owes them; the NZCV transfer leaves CF set for a logical
     result. */
  ...
  if (item->alu_kind == 3u)
    words += 6u;
  if ((item->alu_kind == 0u || item->alu_kind == 2u) && !item->nzcv_dead)
    words += 5u;
  if (item->width == 1u || item->width == 2u)
    words += 5u;
  if ((item->alu_kind == 2u || item->alu_kind == 3u) && item->width < 4u)
    words += 5u + constant_words(item->width == 1u ? UINT64_C(0xffffff)
                                                   : UINT64_C(0xffff));
  if (!item->flags_only)
    ++words;
  return words;
...
void hl_x86_emit_alu(uint32_t *words, uint32_t *cursor,
  ...
  if (item->memory_operand != 0u) {
    ...
    if (item->memory_write != 0u) {
      ...
      words[(*cursor)++] = (item->width == 1u   ? UINT32_C(0x39000300)
                            ...
                            : item->width == 4u ? UINT32_C(0xb9000300)
                                                : UINT32_C(0xf9000300)) |
                           24u << 5 | 25u;
      ...
      words[(*cursor)++] =
          UINT32_C(0xcb110310); /* sub x16,x24,x17: recover guest address */
      words[(*cursor)++] =
  ...
  words[(*cursor)++] =
      item->unary_neg != 0u ? move_register(16u, 31u, wide)
      : item->width == 1u
          ? extract_byte(16, item->destination, item->destination_high)
          : move_register(16, item->destination, wide);
  if (item->has_immediate)
    emit_constant(words, cursor, 17,
                  item->memory_operand != 0u ? item->operand_immediate
                                             : item->immediate);
  else if (item->unary_neg != 0u)
    words[(*cursor)++] =
        item->width == 1u
            ? extract_byte(17, item->destination, item->destination_high)
  ...
  if (item->width == 1u || item->width == 2u) {
    ...
    if (item->alu_kind == 2u || item->alu_kind == 3u) {
      ...
      emit_constant(words, cursor, 22,
                    item->width == 1u ? UINT64_C(0xffffff) : UINT64_C(0xffff));
      words[(*cursor)++] = UINT32_C(0x0a1602f7); /* and w23, w23, w22 */
      words[(*cursor)++] = UINT32_C(0x0b170231); /* add w17, w17, w23 */
    }
  }
  if (item->alu_kind == 2u || item->alu_kind == 3u)
    emit_carry(words, cursor, item->alu_kind == 3u);
  if (item->alu_kind == 0u)
    base = UINT32_C(0x2b000000);
  else if (item->alu_kind == 2u)
    base = UINT32_C(0x3a000000);
  else if (item->alu_kind == 3u)
    base = UINT32_C(0x7a000000);
  else if (item->alu_kind == 5u || item->alu_kind == 7u)
    base = UINT32_C(0x6b000000);
  else if (item->alu_kind == 1u)
    base = UINT32_C(0x2a000000);
  else if (item->alu_kind == 4u)
    base = UINT32_C(0x0a000000);
  else
    base = UINT32_C(0x4a000000);
  words[(*cursor)++] =
      base | (wide ? UINT32_C(0x80000000) : 0) | 17u << 16 | 16u << 5 | 18u;
  if (logical)
    words[(*cursor)++] = UINT32_C(0x6a00001f) |
                         (wide ? UINT32_C(0x80000000) : 0) | 18u << 16 |
                         18u << 5;
