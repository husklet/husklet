...
static uint32_t flags_words(void) {
  return 21u + constant_words(1u) * 2u +
         constant_words(HL_X86_RFLAGS_NZCV_MASK);
}
...
static uint32_t bitfield(unsigned destination, unsigned source, unsigned lsb,
                         unsigned width) {
  unsigned immr = (64u - lsb) & 63u;
  return UINT32_C(0xb3400000) | immr << 16 | (width - 1u) << 10 | source << 5 |
         destination;
}

static void div_fallback(uint32_t *words, uint32_t *cursor,
...
static void patch_cbnz(uint32_t *site, uint32_t *target, unsigned reg) {
  *site = UINT32_C(0xb5000000) |
          ((uint32_t)(target - site) & UINT32_C(0x7ffff)) << 5 | reg;
}

static void patch_cbz(uint32_t *site, uint32_t *target, unsigned reg) {
  *site = UINT32_C(0xb4000000) |
          ((uint32_t)(target - site) & UINT32_C(0x7ffff)) << 5 | reg;
}

static void patch_condition(uint32_t *site, uint32_t *target,
                            unsigned condition) {
  *site = UINT32_C(0x54000000) |
          ((uint32_t)(target - site) & UINT32_C(0x7ffff)) << 5 | condition;
}
...
void hl_x86_emit_div(uint32_t *words, uint32_t *cursor,
  ...
  if (item->width < 8u) {
    uint32_t extension = signed_divide
                             ? (item->width == 1u   ? UINT32_C(0x93401c00)
                                : item->width == 2u ? UINT32_C(0x93403c00)
                                                    : UINT32_C(0x93407c00))
                             : (item->width == 1u   ? UINT32_C(0x12001c00)
                                : item->width == 2u ? UINT32_C(0x12003c00)
                                                    : UINT32_C(0xd3407c00));
  ...
  if (item->width == 1u) {
    words[(*cursor)++] =
        (signed_divide ? UINT32_C(0x93403c14) : UINT32_C(0xd3403c14));
    ...
    if (signed_divide)
      words[(*cursor)++] = UINT32_C(0x93407e94); /* sxtw x20,w20 */
  ...
  words[(*cursor)++] =
      (signed_divide ? UINT32_C(0x9ac00c00) : UINT32_C(0x9ac00800)) |
      divisor << 16 | 20u << 5 | 18u;
  words[(*cursor)++] =
      UINT32_C(0x9b008000) | divisor << 16 | 20u << 10 | 18u << 5 | 21u;
  if (item->width < 8u) {
    if (signed_divide) {
      uint32_t extension = item->width == 1u   ? UINT32_C(0x93401c11)
                           : item->width == 2u ? UINT32_C(0x93403c11)
                                               : UINT32_C(0x93407c11);
...
uint32_t hl_x86_mul_words(const instruction *item) {
  ...
  if (item->width == 1u || item->width == 2u) {
    ...
    if (item->memory_operand == 0u)
      ++words; /* truncate/extract the register source */
    words += item->width == 1u
                 ? 3u
                 : 5u; /* product, high half, and architectural writes */
  } else {
...
void hl_x86_emit_mul(uint32_t *words, uint32_t *cursor,
  ...
  if (item->width == 1u || item->width == 2u) {
    uint32_t mask =
        item->width == 1u ? UINT32_C(0x53001c00) : UINT32_C(0x53003c00);
    ...
    words[(*cursor)++] =
        (item->width == 1u ? UINT32_C(0x53087c00) : UINT32_C(0x53107c00)) |
        18u << 5 | 21u;
    if (item->width == 1u) {
  ...
  /* The widening forms define only CF and OF; routing them through the NZCV
     transfer would also clear SF and ZF, which the interpreter leaves alone. */
...
uint32_t hl_x86_imul_words(const instruction *item) {
  ...
  if (item->has_immediate != 0u)
    words += constant_words(item->operand_immediate);
  /* With NZCV dead the overflow test and its materialization both go, leaving
   * the product alone. */
  if (item->nzcv_dead != 0u)
    return words + (item->width == 8u ? 2u : 4u);
  /* The SF/ZF test costs an extra shift at halfword width, plus the read-back
   * and the merge. */
...
static uint32_t move(unsigned destination, unsigned source, int wide) {
  return (wide ? UINT32_C(0xaa0003e0) : UINT32_C(0x2a0003e0)) | source << 16 |
         destination;
}
...
void hl_x86_emit_imul(uint32_t *words, uint32_t *cursor,
  ...
  if (item->nzcv_dead != 0u)
    return;
  /* SF and ZF are undefined but not free: the interpreter publishes them from
     the truncated product, so build them here rather than letting a zeroed NZCV
     word claim both are clear. */
  if (item->width == 2u) {
    words[(*cursor)++] = UINT32_C(0x53103c00) | 18u << 5 | 19u;
    words[(*cursor)++] = UINT32_C(0x6a00001f) | 19u << 16 | 19u << 5;
  } else {
    words[(*cursor)++] =
        (item->width == 8u ? UINT32_C(0xea00001f) : UINT32_C(0x6a00001f)) |
        18u << 16 | 18u << 5;
  }
  words[(*cursor)++] = UINT32_C(0xd53b4200) |
                       19u; /* mrs x19,nzcv: N and Z, with C and V clear */
