...
int hl_x86_decode_double_shift(const hl_x86_a64_request *request, decode *block,
  ...
  if ((modrm >> 6) == 3u) {
    ...
    if (!hl_x86_decode_address(request, block, item, rex, 0u, address_32, start,
                               cursor))
      return 0;
    item->operation = OP_DOUBLE_SHIFT;
    item->width = (rex & 8u) != 0u ? 8u : operand_16 != 0u ? 2u : 4u;
...
static void compare_zero(uint32_t *instruction, uint32_t *target,
                         unsigned reg) {
...
void hl_x86_emit_double_shift(uint32_t *words, uint32_t *cursor,
  ...
  uint64_t width_mask =
      item->width == 8u ? UINT64_MAX : (UINT64_C(1) << bits) - 1u;
  uint32_t *preserved;
  ...
  if (item->width == 2u)
    words[(*cursor)++] =
        UINT32_C(0xd3403c00) | (uint32_t)item->source << 5 | 19u;
  else
    words[(*cursor)++] = move(19u, item->source, item->width == 8u);
  if (item->variable_count == 0u &&
      (item->shift_count == 0u ||
       (item->width == 2u && item->shift_count > 16u))) {
    if (item->memory_operand != 0u) {
      ...
      /* The destination is still written, and a 32-bit write clears the upper
       * half. */
  ...
  *again |= UINT32_C(0x01000000);
  ...
  emit_constant(words, cursor, 23u,
  ...
  if (preserved != NULL || out_of_range != NULL) {
    /* A zero count leaves flags alone but still writes a 32-bit destination,
       which clears its upper half. */
    uint32_t *done =
        writeback != NULL && item->width == 4u ? writeback : &words[*cursor];
    if (item->memory_operand != 0u) {
      instruction store = *item;
    ...
    if (preserved != NULL)
      compare_zero(preserved, done, 17u);
    if (out_of_range != NULL)
      branch(out_of_range, done, 8u);
  }
}
