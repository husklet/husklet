...
int hl_x86_decode_test(const hl_x86_a64_request *request, decode *block,
  ...
  if (group && extension >= 2u) {
    ...
    if (item->width == 1u && item->memory_operand == 0u && rex == 0u &&
        (modrm & 7u) >= 4u) {
  ...
  item->alu_kind = 4u;
  item->flags_only = 1u;
  if (opcode == 0xf6u && item->memory_operand == 0u && rex == 0u &&
      (modrm & 7u) >= 4u) {
    item->destination = (uint8_t)((modrm & 7u) - 4u);
    item->destination_high = 1u;
  }
  ...
  size_t immediate_size = opcode == 0xa8u || opcode == 0xf6u ? 1u
                          : operand_16 != 0                  ? 2u
                                                             : 4u;
  ...
  if (item->memory_operand == 0u)
    item->immediate = item->operand_immediate;
