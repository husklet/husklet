...
_Static_assert(sizeof(hl_native_ibtc_entry) == 16,
               "IBTC entry footprint drifted");
...
_Static_assert(offsetof(hl_native_ibtc_authenticated_entry,
                        authenticated_ingress) == 8,
               "authenticated IBTC ingress offset drifted");
_Static_assert(offsetof(hl_native_ibtc_authenticated_entry, target_identity) ==
                   16,
...
_Static_assert(sizeof(((hl_native_ibtc_authenticated_entry *)0)->sequence) ==
                   sizeof(uint64_t),
...
_Static_assert(HL_NATIVE_IBTC_AUTHENTICATED_BYTES % HL_NATIVE_IBTC_ALIGNMENT ==
                   0,
               "authenticated IBTC allocation multiple drifted");
_Static_assert((HL_NATIVE_IBTC_COUNT * sizeof(hl_native_ibtc_entry)) %
                       HL_NATIVE_IBTC_ALIGNMENT ==
                   0,
...
int hl_native_ibtc_authenticated_storage_bytes(size_t count, size_t *output) {
  if (output == NULL ||
      count > SIZE_MAX / sizeof(hl_native_ibtc_authenticated_entry))
    return 0;
...
hl_native_ibtc_authenticated_storage_create(void) {
  hl_native_ibtc_authenticated_entry *storage;
  size_t bytes;
  if (!hl_native_ibtc_authenticated_storage_bytes(HL_NATIVE_IBTC_COUNT, &bytes))
    return NULL;
#if defined(_MSC_VER)
...
void hl_native_ibtc_authenticated_storage_clear(
    hl_native_ibtc_authenticated_entry *storage) {
  if (storage == NULL)
    return;
  for (size_t index = 0; index < HL_NATIVE_IBTC_COUNT; ++index) {
...
void hl_native_ibtc_authenticated_storage_destroy(
    hl_native_ibtc_authenticated_entry *storage) {
#if defined(_MSC_VER)
  _aligned_free(storage);
#else
