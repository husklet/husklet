...
static _Thread_local __attribute__((tls_model(
    "initial-exec"))) hl_native_fault_thread_state hl_native_fault_thread;
#else
...
hl_native_status hl_native_fault_thread_attach(void) {
  ...
  stack_t installed = {
      .ss_sp = storage, .ss_size = HL_NATIVE_ALTSTACK_SIZE, .ss_flags = 0};
...
static int same_scope(const hl_native_fault_scope *left,
  ...
  return left->abi == right->abi && left->size == right->size &&
         ...
         left->executor == right->executor && left->cpu == right->cpu;
}

hl_native_status
hl_native_fault_thread_publish(const hl_native_fault_scope *scope,
                               uint64_t *generation) {
  hl_native_fault_thread_state *state = &hl_native_fault_thread;
...
hl_native_status
hl_native_fault_thread_unpublish(const hl_native_fault_scope *scope,
                                 uint64_t generation) {
  hl_native_fault_thread_state *state = &hl_native_fault_thread;
  if (scope == NULL || generation == 0)
    return HL_NATIVE_ARGUMENT;
  if (!state->attached || !state->active || state->recursive ||
...
HL_NATIVE_NO_STACK_PROTECTOR int
hl_native_fault_thread_prepare(uint64_t host_pc, uint64_t host_address,
                               void *host_context) {
  hl_native_fault_thread_state *state = &hl_native_fault_thread;
  if (!state->attached || !state->active || state->recursive ||
      host_context == NULL)
  ...
  int prepared = hl_native_fault_scope_contains(&scope, host_pc) &&
                 hl_native_fault_scope_prepare_return(
                     &scope, host_pc, host_address, host_context);
  atomic_signal_fence(memory_order_release);
  state->recursive = 0;
  return prepared;
}
...
hl_native_status hl_native_fault_thread_detach(void) {
  ...
  if (!state->attached)
    return HL_NATIVE_STATE;
  if (state->active || state->recursive)
    return HL_NATIVE_STATE;
#if defined(__linux__)
  stack_t current;
  if (sigaltstack(NULL, &current) != 0 || current.ss_sp != state->storage ||
    ...
    return HL_NATIVE_STATE;
  if (sigaltstack(&state->prior, NULL) != 0)
    return HL_NATIVE_PLATFORM;
  free(state->storage);
  state->storage = NULL;
