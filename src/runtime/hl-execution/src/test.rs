...
fn relocation_is_exact() {
    let words = [0xd282_4683, 0xf2aa_c783, 0xf2d3_5783, 0xf2e0_0003];
    let slid = Materialization::slide(words, 1).unwrap();
    assert_eq!(slid[0] & 0x1f, 3);
    let mut corrupt = words;
    corrupt[2] = 0;
    assert_eq!(
        Materialization::slide(corrupt, 1),
        Err(RelocationError::InvalidInstruction)
    );
    assert_eq!(
        RelocationRecord { offset: 4, info: 9 }.encode(),
        [4, 0, 0, 0, 9, 0, 0, 0]
    );
}
...
impl CodePublisher for FakePublisher {
    type Prepared = ();
    type Error = &'static str;
    fn prepare(&mut self, _: &ValidatedCodeArtifact) -> Result<(), Self::Error> {
        self.prepared += 1;
        Ok(())
    }
    fn commit(&mut self, _: &mut ()) -> Result<Publication, Self::Error> {
        if self.fail_commit {
            return Err("commit");
        }
        self.committed += 1;
...
fn validation_precedes_mutation() {
    let invalid = CodeArtifact {
        architecture: GuestArchitecture::Aarch64,
        identity: 1,
        bytes: vec![0; 8],
        relocations: vec![RelocationRecord { offset: 4, info: 0 }],
    };
    let mut publisher = FakePublisher::default();
    assert!(invalid.publish(&mut publisher).is_err());
    assert_eq!(publisher.prepared, 0);

    publisher.fail_commit = true;
    let valid = CodeArtifact {
        architecture: GuestArchitecture::X86_64,
        identity: 1,
        bytes: vec![0; 16],
        relocations: vec![],
    };
    assert!(valid.publish(&mut publisher).is_err());
    assert_eq!(publisher.rolled_back, 1);
}

fn retained_header(architecture: GuestArchitecture, arena: &[u8]) -> (Vec<u8>, CacheExpectations) {
    let expected = CacheExpectations {
        cpu_size: 512,
        map_entries: 1 << 19,
        ibtc_entries: 65536,
        binary_identity: 7,
    ...
    let (count, magic, version, abi) = match architecture {
        GuestArchitecture::Aarch64 => (22, 0x3441_4350_544a_4c48, 11, AARCH64_CACHE_ABI),
        GuestArchitecture::X86_64 => (18, 0x3130_4350_544a_4c48, 9, X86_64_CACHE_ABI),
    };
    let mut words = vec![0_u64; count];
    words[..11].copy_from_slice(&[
        magic,
        version,
        abi,
        expected.cpu_size,
        expected.map_entries,
        expected.ibtc_entries,
        0x0000_0400_0000_0000,
        0x0000_0480_0000_0000,
        expected.binary_identity,
        expected.entry,
        arena.len() as u64,
    ]);
    let checksum_index = if architecture == GuestArchitecture::Aarch64 {
        18
    } else {
        15
    };
    words[checksum_index] = ArtifactDigest::bytes(DIGEST_SEED, arena);
...
fn retained_v11_and() {
    for architecture in [GuestArchitecture::Aarch64, GuestArchitecture::X86_64] {
        let arena: &[u8] = if architecture == GuestArchitecture::Aarch64 {
            &[0, 0, 0, 0]
        } else {
            &[0]
        };
        let (bytes, expected) = retained_header(architecture, arena);
        let cache = RetainedCache::parse(architecture, &bytes, expected).unwrap();
        assert_eq!(cache.arena(), arena);
        for end in 0..bytes.len() {
            assert!(RetainedCache::parse(architecture, &bytes[..end], expected).is_err());
        }
        let checked_header = if architecture == GuestArchitecture::Aarch64 {
            152
        } else {
            128
        };
        for index in 0..checked_header {
            let mut corrupt = bytes.clone();
            corrupt[index] ^= 1;
            assert!(RetainedCache::parse(architecture, &corrupt, expected).is_err());
        }
        let mut corrupt_payload = bytes.clone();
        *corrupt_payload.last_mut().unwrap() ^= 1;
        assert!(RetainedCache::parse(architecture, &corrupt_payload, expected).is_err());
    }
}

#[test]
fn retained_abi_gate() {
    let arena = [0_u8; 4];
    let (bytes, expected) = retained_header(GuestArchitecture::Aarch64, &arena);
    assert!(RetainedCache::parse(GuestArchitecture::Aarch64, &bytes, expected).is_ok());
    let incompatible_cases: [(u64, u64); 4] = [
        (11, 0x4136_3450_4341_3031),
        (10, AARCH64_CACHE_ABI),
        (11, AARCH64_CACHE_ABI - 1),
        (11, X86_64_CACHE_ABI),
    ];
    for (format, abi) in incompatible_cases {
        let mut incompatible = bytes.clone();
        incompatible[8..16].copy_from_slice(&format.to_le_bytes());
        incompatible[16..24].copy_from_slice(&abi.to_le_bytes());
        assert_eq!(
            RetainedCache::parse(GuestArchitecture::Aarch64, &incompatible, expected),
            Err(RetainedCacheError::Header),
        );
    }
}
