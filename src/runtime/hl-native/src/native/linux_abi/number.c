...
// x86-64 -> canonical (aarch64) syscall-number map, auto-generated by
...
static uint16_t x86_guest_by_canonical[HL_LINUX_CANONICAL_MAX + 1u];
static pthread_once_t x86_inverse_once = PTHREAD_ONCE_INIT;

/*
 * Derive the inverse once from x86_number(), the authoritative map. Entries
 * store guest+1 so raw syscall zero remains distinguishable from no mapping.
 * Duplicate canonical aliases retain the highest raw number. Linux adds the
 * semantic syscall after its legacy predecessor (dup3 after dup2, openat after
 * open), so this avoids re-entering legacy normalization and losing flags.
 */
...
uint64_t hl_linux_syscall_number(enum hl_linux_guest_isa guest_isa, uint64_t guest_number) {
    return guest_isa == HL_LINUX_GUEST_X86_64 ? x86_number(guest_number) : guest_number;
}

uint64_t hl_linux_syscall_guest_number(enum hl_linux_guest_isa guest_isa, uint64_t canonical_number) {
    if (canonical_number & HL_LINUX_SYSCALL_X86_ONLY) return UINT64_MAX;
    if (guest_isa != HL_LINUX_GUEST_X86_64) return canonical_number;
    if (canonical_number > HL_LINUX_CANONICAL_MAX) return UINT64_MAX;
    if (pthread_once(&x86_inverse_once, x86_inverse_init) != 0) return UINT64_MAX;
    uint16_t encoded = x86_guest_by_canonical[canonical_number];
    return encoded == 0 ? UINT64_MAX : (uint64_t)encoded - 1u;
}
