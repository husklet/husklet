...
static void fork_diagnostic_emit(struct cpu *c, uint64_t nr, uint64_t flags, const char *stage, int failure,
    ...
    int length = snprintf(
        line, sizeof line,
        "hl-fork-failure: stage=%s result_errno=%d ambient_errno=%d syscall=%llu flags=%#llx guest_pc=%#llx "
        "guest_sp=%#llx "
        ...
        g_fork_diagnostic_route.worker_pid, g_fork_diagnostic_route.sentry_pid, g_fork_diagnostic_route.guest_children,
        g_fork_diagnostic_route.worker_threads, g_fork_diagnostic_route.ring, host_snapshot_status,
        host_snapshot.threads, host_snapshot.caller_children, host_snapshot.children_truncated, local_tasks, pids_total,
        (unsigned long long)g_pids_max, host_snapshot.open_descriptors,
        (unsigned long long)host_snapshot.nofile_current, (unsigned long long)host_snapshot.nofile_maximum,
        host_snapshot.nofile_status, (unsigned long long)host_snapshot.nproc_current,
        (unsigned long long)host_snapshot.nproc_maximum, host_snapshot.nproc_status,
        (unsigned long long)atomic_load_explicit(&g_mem_charged, memory_order_relaxed), (unsigned long long)g_mem_max,
        snapshot_stage, ofd_count, ofd_bytes, ofd_capacity, ofd_capacity_bytes, ofd_watermark, reserved_fds,
        watch_count, watch_bytes, watch_capacity, watch_capacity_bytes, fdvis_count, fdvis_bytes,
        state != NULL && state->watch_prepared, state != NULL && state->private_prepared,
        state != NULL && state->fdvis_prepared, state != NULL && state->seq_prepared);
    if (length > 0) {
