case 222: {
    ...
    if (prot & PROT_EXEC) {
        /*
         * Ordinary file-backed private RX text is byte-stable. Arm when this mapping is writable;
    ...
    if (r == MAP_FAILED && !(a3 & 0x10) && !(a3 & 0x20) && (int)a4 >= 0 && hp > (size_t)guest_pagesz() &&
        ...
        void *fallback_hint = (void *)(uintptr_t)hl_linux_snapshot_reserve(&g_ckpt_snapshot, (uint64_t)mapped_size);
    ...
    if (bus_prepared) {
        ...
        if (r != MAP_FAILED && gbus_add((uint64_t)(uintptr_t)r + bus_accessible, (uint64_t)(uintptr_t)r + a1) != 0) {
