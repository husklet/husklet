...
case 216: {
    ...
    if (a3 & 2) {
        ...
        if (anon_prot_if_contained(a0, (size_t)a1) >= 0) {
            ...
            void *r;
            if ((a4 & (hp - 1)) == 0) {
                r = mmap((void *)a4, (size_t)a2 + guard, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_ANON | MAP_PRIVATE, -1,
                         0);
            } else {
    ...
    if (!(a3 & 2 /*MREMAP_FIXED*/)) {
        ...
        void *ext = mmap((void *)end, (size_t)(a0 + want - end), PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
        if (ext == (void *)end) {
            hl_gmap_remove(a0);
