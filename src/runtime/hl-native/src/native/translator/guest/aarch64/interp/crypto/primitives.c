...
// these paths. The tables are generated from the GF(2^8) inverse and the FIPS-197 affine transform.
const uint8_t interp_aes_sbox[256] = {
...
const uint8_t interp_aes_inv_sbox[256] = {
...
void interp_aes_shift_rows(const uint8_t *input, uint8_t *output, int inverse) {
    for (unsigned column = 0; column < 4u; column++)
        for (unsigned row = 0; row < 4u; row++) {
            ...
            if (inverse)
                output[source] = input[destination];
            else
    ...
    for (unsigned column = 0; column < 4u; column++)
        for (unsigned row = 0; row < 4u; row++) {
            ...
            for (unsigned term = 0; term < 4u; term++)
                value ^= aes_multiply(input[4u * column + ((row + term) & 3u)], coefficient[term]);
            output[4u * column + row] = value;
        }
}
