...
fn the_host_termios_translation_cannot_carry_every_linux_flag() {
    ...
    assert_eq!(
        control
            .matches("terminal_termios_observe_set(tfd, (const uint8_t *)arg, terminal_termios_flush_request(rq))",)
            .count(),
        2,
    ...
    assert_eq!(
        route
            .matches("terminal_termios_observe_set(native_fd, argument, terminal_termios_flush_request(request))",)
            .count(),
        2,
