/* VK RENDER-CORRECTNESS DEMO #5 — vk_scissor: a dynamic SCISSOR rect clips a full-screen draw to an EXACT
 * sub-rectangle; everything outside stays the clear color.
 *
 * A real C Vulkan program (real loader + OUR ICD → IR → lavapipe). The pipeline declares
 * VK_DYNAMIC_STATE_SCISSOR; the program sets the scissor to offset (16,16) extent (32,32) with
 * `vkCmdSetScissor`, then draws a GREEN full-screen quad (6 verts). The scissor clips the raster to pixel
 * cols/rows [16,48); outside remains the BLUE clear. This is the exact-rect guard for `Enc::SetScissor`:
 * without the scissor the whole 64×64 frame would be GREEN. Prints "VK_SCISSOR_OK". */
#define _GNU_SOURCE
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-vk-scissor", .apiVersion = VK_API_VERSION_1_1};
    ...
    /* Scissor is DYNAMIC: the pipeline's static pScissors is ignored; vkCmdSetScissor drives the clip. */
    ...
    VkRect2D scissor = {{16, 16}, {32, 32}};   /* clip to cols/rows [16,48) */
    pCmdSetScissor(cb_h, 0, 1, &scissor);
    pCmdDraw(cb_h, 6, 1, 0, 0);   /* full-screen GREEN quad, clipped by the scissor */
    pCmdEndRP(cb_h);
