/* VK RENDER-CORRECTNESS DEMO #2 — vk_depth: the DEPTH TEST occludes correctly. Two overlapping quads at
 * different depths, depth-test ON — the NEAR color wins in the overlap.
 *
 * A real C Vulkan program (real loader + OUR ICD → IR → lavapipe), driven through VK_KHR_dynamic_rendering
 * (core 1.3) so the pass carries a real DEPTH attachment (D32_SFLOAT). A NEAR GREEN quad over the LEFT half
 * (z = 0.3) is drawn FIRST, then a FAR RED FULL-screen quad (z = 0.7). With depthCompareOp = LESS and the
 * depth cleared to 1.0, the far quad FAILS the depth test everywhere the near quad already wrote 0.3, so the
 * left half stays GREEN and only the right half (never covered by the near quad) becomes RED. If the depth
 * state were dropped (the shim bug this guards) the far RED — drawn second — would overwrite everything and
 * the whole frame would be RED. Prints "VK_DEPTH_OK". The Rust test asserts the depth-resolved raster. */
#define _GNU_SOURCE
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-vk-depth", .apiVersion = VK_API_VERSION_1_3};
    ...
    /* Enable core-1.3 dynamic rendering via its feature struct (the ICD backs it with cmd_begin_rendering). */
    ...
    VkDeviceCreateInfo dci = {.sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO, .pNext = &dynfeat,
    ...
    PFN_vkCmdBeginRendering pCmdBeginRendering = DPROC(dev, vkCmdBeginRendering);
    PFN_vkCmdEndRendering pCmdEndRendering = DPROC(dev, vkCmdEndRendering);
    ...
    if (!pCmdBeginRendering || !pCmdEndRendering) {
        fprintf(stderr, "vkCmdBeginRendering/EndRendering not resolvable (no dynamic rendering)\n");
        return 6;
    }
    ...
    /* ---- graphics pipeline: dynamic rendering (null render pass) + DEPTH-TEST state ------------- */
    ...
    /* Dynamic-rendering formats (no VkRenderPass object): 1 color + 1 depth. */
    VkFormat color_fmt = FMT;
    ...
    VkGraphicsPipelineCreateInfo gpci = {.sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO,
                                         ...
                                         .pColorBlendState = &cb,
                                         .layout = layout, .renderPass = VK_NULL_HANDLE, .subpass = 0};
    VkPipeline pipe;
    VK(pCreateGP(dev, VK_NULL_HANDLE, 1, &gpci, NULL, &pipe), "vkCreateGraphicsPipelines");

    /* ---- record: begin rendering (color+depth) -> near quad -> far quad -> end -> copy ---------- */
    ...
    color_att.clearValue.color.float32[0] = 0.0f; color_att.clearValue.color.float32[1] = 0.0f;
    color_att.clearValue.color.float32[2] = 0.0f; color_att.clearValue.color.float32[3] = 1.0f; /* BLACK */
    ...
    VkRenderingAttachmentInfo depth_att = {.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO,
                                           ...
                                           .loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR,
                                           .storeOp = VK_ATTACHMENT_STORE_OP_STORE};
    depth_att.clearValue.depthStencil.depth = 1.0f;   /* clear depth = far plane */
