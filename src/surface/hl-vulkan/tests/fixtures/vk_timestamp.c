/* VK ADVANCED-COMMAND DEMO — vk_timestamp: a TIMESTAMP query pair straddling a draw is monotonic.
 *
 * A real C Vulkan program (real loader + OUR ICD → IR → lavapipe). A 2-slot VK_QUERY_TYPE_TIMESTAMP pool
 * is reset, then `vkCmdWriteTimestamp(TOP_OF_PIPE, pool, 0)` is recorded BEFORE and
 * `vkCmdWriteTimestamp(BOTTOM_OF_PIPE, pool, 1)` AFTER a full-screen GREEN quad draw. After submit,
 * vkGetQueryPoolResults reads both serials; the only guarantee an app may rely on across two timestamps in
 * submission order is that the second is >= the first (monotonic). The guest checks T1 >= T0 (and, for this
 * synchronous host-serial model, strictly increasing) and prints "TIMESTAMP T0=<n> T1=<n>" + "VK_TIMESTAMP_OK";
 * the Rust test re-parses the serials, asserts monotonicity, and asserts the drawn GREEN frame. */
#define _GNU_SOURCE
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-vk-timestamp", .apiVersion = VK_API_VERSION_1_1};
    ...
    PFN_vkCmdWriteTimestamp pCmdWriteTS = DPROC(dev, vkCmdWriteTimestamp);
    ...
    if (!pCmdWriteTS || !pGetQPR) { fprintf(stderr, "timestamp entry points not resolvable\n"); return 6; }
    ...
    /* Timestamp BEFORE the draw (top of pipe), draw, timestamp AFTER (bottom of pipe). */
    pCmdWriteTS(cb_h, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, qpool, 0);
    pCmdBeginRP(cb_h, &rpbi, VK_SUBPASS_CONTENTS_INLINE);
    pBindPipe(cb_h, VK_PIPELINE_BIND_POINT_GRAPHICS, pipe);
    pCmdDraw(cb_h, 6, 1, 0, 0);   /* full-screen GREEN quad */
    pCmdEndRP(cb_h);
    pCmdWriteTS(cb_h, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, qpool, 1);
    ...
    uint64_t ts[2] = {0, 0};
    VK(pGetQPR(dev, qpool, 0, 2, sizeof(ts), ts, sizeof(uint64_t),
               VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT),
       "vkGetQueryPoolResults");
    printf("TIMESTAMP T0=%" PRIu64 " T1=%" PRIu64 "\n", ts[0], ts[1]);
    if (ts[1] < ts[0]) { fprintf(stderr, "timestamps not monotonic: T0=%" PRIu64 " T1=%" PRIu64 "\n", ts[0], ts[1]); return 7; }
    if (ts[1] == ts[0]) { fprintf(stderr, "timestamps not strictly increasing: T0==T1==%" PRIu64 "\n", ts[0]); return 8; }

    printf("VK_TIMESTAMP_OK\n");
