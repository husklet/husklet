/* VK ADVANCED-COMMAND DEMO — vk_draw_indirect: the draw parameters come from a VkBuffer, not the API call.
 *
 * A real C Vulkan program (real loader + OUR ICD → IR → lavapipe). The program fills a host-visible
 * VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT buffer with TWO `VkDrawIndirectCommand` structs
 * ({vertexCount, instanceCount, firstVertex, firstInstance}) and issues ONE `vkCmdDrawIndirect(buf, 0,
 * drawCount=2, stride=16)`. The vertex shader's 12-entry position table splits into a LEFT quad
 * (verts 0..5, NDC x∈[-0.9,-0.1], RED) and a RIGHT quad (verts 6..11, x∈[0.1,0.9], BLUE); the two
 * indirect commands select each quad purely through their buffer-sourced `firstVertex` (0 and 6). The
 * result is pixel-identical to the equivalent DIRECT `vkCmdDraw(6,1,0,0); vkCmdDraw(6,1,6,0)` (the
 * committed vk_multidraw demo) — proving the shim reads the indirect argument buffer and lowers each
 * struct to the same direct draw. Prints "VK_DRAW_INDIRECT_OK". */
#define _GNU_SOURCE
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-vk-draw-indirect", .apiVersion = VK_API_VERSION_1_1};
    ...
    PFN_vkCmdDrawIndirect pCmdDrawIndirect = DPROC(dev, vkCmdDrawIndirect);
    ...
    if (!pCmdDrawIndirect) { fprintf(stderr, "vkCmdDrawIndirect not resolvable\n"); return 6; }
    ...
    VkBufferCreateInfo bci = {.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO, .size = buf_bytes,
                              ...
                              .sharingMode = VK_SHARING_MODE_EXCLUSIVE};
    VkBuffer buf;
    VK(pCreateBuffer(dev, &bci, NULL, &buf), "vkCreateBuffer(readback)");
    ...
    VkDeviceMemory bmem;
    VK(pAlloc(dev, &bmai, NULL, &bmem), "vkAllocateMemory(readback)");
    VK(pBindBuf(dev, buf, bmem, 0), "vkBindBufferMemory(readback)");

    /* ---- INDIRECT argument buffer: two VkDrawIndirectCommands, filled on the CPU ------------------- */
    VkDeviceSize ind_bytes = 2 * sizeof(VkDrawIndirectCommand);
    ...
    VK(pBindBuf(dev, indbuf, indmem, 0), "vkBindBufferMemory(indirect)");
    /* Populate the draw parameters IN THE BUFFER (not in the API call). */
    void *ind_ptr = NULL;
    VK(pMap(dev, indmem, 0, ind_bytes, 0, &ind_ptr), "vkMapMemory(indirect)");
    VkDrawIndirectCommand cmds[2] = {
        {.vertexCount = 6, .instanceCount = 1, .firstVertex = 0, .firstInstance = 0}, /* LEFT  RED  quad */
        {.vertexCount = 6, .instanceCount = 1, .firstVertex = 6, .firstInstance = 0}, /* RIGHT BLUE quad */
    };
    memcpy(ind_ptr, cmds, sizeof(cmds));
    pUnmap(dev, indmem);
    ...
    /* ONE indirect call draws BOTH quads — every draw parameter is read from `indbuf`. */
    pCmdDrawIndirect(cb_h, indbuf, 0, 2, sizeof(VkDrawIndirectCommand));
    pCmdEndRP(cb_h);
