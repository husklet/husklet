/* REAL GRAPHICS #2 — a real C Vulkan program rasterizes a MULTI-TRIANGLE, per-vertex-COLORED quad.
 *
 * An escalation over vk_triangle.c (one flat solid triangle): this draws a full-screen quad built from
 * TWO triangles (6 vertices via `@builtin(vertex_index)`, no vertex buffer), each quad corner carrying a
 * DISTINCT vertex color that the rasterizer INTERPOLATES across the fragments. It exercises exactly what
 * escalation #1 (a 2-triangle mesh + per-corner interpolated vertex colors + full coverage) targets, but
 * WITHOUT `vkCmdDrawIndexed` / an index buffer — those lower correctly in our Vulkan driver but are NOT
 * replayed by the wgpu (lavapipe) executor's render pass yet (Enc::DrawIndexed / Enc::SetIndexBuffer fall
 * through unhandled in hl_wip-gpu-wgpu/src/submit.rs::run_render_pass), so an indexed draw rasterizes
 * nothing. See the Rust test + the reported gap. A non-indexed Draw(6) IS fully supported, so this is the
 * real, passing variant of the mesh-with-interpolation escalation.
 *
 * Same plumbing as vk_triangle.c: dlopen the REAL loader (RTLD_LOCAL), drive OUR ICD (libvk_hl.so) via the
 * canonical proc-addr chain, 64x64 RGBA8 color image (COLOR_ATTACHMENT|TRANSFER_SRC), render pass
 * (loadOp=CLEAR to BLACK — a color the interpolated quad can never produce, so full coverage is
 * unambiguous), a graphics pipeline with REAL SPIR-V (vertex shader emits position + @location(0) color;
 * fragment forwards the interpolated color), record BeginRenderPass/Draw(6)/EndRenderPass, copy image to a
 * buffer, submit, wait. Prints "VK_QUAD_DRAW_OK". The Rust test asserts the ACTUAL lavapipe raster off the
 * host executor (the four corners each ~their vertex color, the center the blend). */
#define _GNU_SOURCE
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-realsw-vk-quad", .apiVersion = VK_API_VERSION_1_1};
    ...
    /* ---- record: clear(black) -> draw(6, two triangles) -> end, then copy image->buffer -------- */
    ...
    pCmdDraw(cb_h, 6, 1, 0, 0);   /* two triangles = a full-screen quad */
    pCmdEndRP(cb_h);
