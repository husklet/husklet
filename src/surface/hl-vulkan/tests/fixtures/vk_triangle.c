/* REAL SOFTWARE — a real C Vulkan GRAPHICS program: a solid-color triangle rasterized end to end.
 *
 * Compiled against the REAL Khronos Vulkan-Headers; at runtime it `dlopen`s the REAL loader
 * (libvulkan.so.1, RTLD_LOCAL — see vk_compute.c for the self-interposition rationale) and drives every
 * call through the canonical vkGetInstanceProcAddr / vkGetDeviceProcAddr chain. `VK_ICD_FILENAMES` points
 * the loader at OUR ICD (libvk_hl.so), so each call flows loader → our ICD → `$HL_GPU_EXEC` socket → the
 * host WgpuExecutor on lavapipe (software Vulkan), which really rasterizes the triangle.
 *
 * It creates a 64x64 R8G8B8A8_UNORM color image (COLOR_ATTACHMENT | TRANSFER_SRC), an image view, a render
 * pass (loadOp=CLEAR to RED, storeOp=STORE), a framebuffer, TWO SPIR-V shader modules (a vertex shader that
 * emits a full-screen triangle from gl_VertexIndex, and a fragment shader that outputs solid GREEN), a
 * graphics pipeline, then records BeginRenderPass(clear=red) / BindPipeline / Draw(3) / EndRenderPass,
 * copies the image into a host-visible buffer (vkCmdCopyImageToBuffer), submits, and waits. The SPIR-V is
 * loaded from files passed via `HL_VK_VS_SPV` / `HL_VK_FS_SPV` (minted by the Rust test with naga so it is
 * real, valid SPIR-V that lavapipe/naga can compile). Prints "VK_TRIANGLE_DRAW_OK" on success.
 *
 * HONEST SCOPE: the program does NOT read the rendered pixels back — our ICD's vkMapMemory is write-through
 * (hands back the app's own staging bytes, no device→host readback). The Rust test asserts the ACTUAL
 * lavapipe raster output by reading the render target off the host executor. See the test + REALSOFTWARE. */
#define _GNU_SOURCE
...
/* Read a whole SPIR-V file into a malloc'd buffer; *out_words is the u32 count. */
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-realsw-vk-triangle", .apiVersion = VK_API_VERSION_1_1};
    ...
    /* ---- color image (COLOR_ATTACHMENT | TRANSFER_SRC) + memory ------------------------------- */
    ...
    /* ---- readback buffer (TRANSFER_DST) + host-visible memory --------------------------------- */
    ...
    /* ---- render pass: one color attachment, loadOp=CLEAR, storeOp=STORE ----------------------- */
    ...
    /* ---- shader modules (real SPIR-V minted by naga) ------------------------------------------ */
    ...
    /* ---- record: clear→draw→end, then copy image→buffer --------------------------------------- */
