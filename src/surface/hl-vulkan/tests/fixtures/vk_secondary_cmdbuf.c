/* VK ADVANCED-COMMAND DEMO — vk_secondary_cmdbuf: draws recorded into a SECONDARY command buffer and
 * replayed into a PRIMARY render pass via vkCmdExecuteCommands compose EXACTLY as if recorded inline.
 *
 * A real C Vulkan program (real loader + OUR ICD → IR → lavapipe). A level-SECONDARY command buffer is
 * begun with VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT + a VkCommandBufferInheritanceInfo naming
 * the primary's render pass/subpass/framebuffer, then records bind-pipeline + the two quad draws
 * (firstVertex 0 = RED LEFT, firstVertex 6 = BLUE RIGHT — the vk_multidraw geometry). The PRIMARY begins
 * the render pass with VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS and issues
 * `vkCmdExecuteCommands(1, &secondary)`; the executed secondary's recorded encoder is spliced into the
 * primary's pass. The result is pixel-identical to recording the two draws directly in the primary,
 * proving the shim replays a secondary command buffer's ops. Prints "VK_SECONDARY_OK". */
#define _GNU_SOURCE
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-vk-secondary", .apiVersion = VK_API_VERSION_1_1};
    ...
    PFN_vkCmdExecuteCommands pCmdExec = DPROC(dev, vkCmdExecuteCommands);
    ...
    if (!pCmdExec) { fprintf(stderr, "vkCmdExecuteCommands not resolvable\n"); return 6; }
    ...
    /* ---- SECONDARY command buffer: records the two quad draws inside the inherited render pass -------- */
    VkCommandBufferAllocateInfo scbai = {.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
                                         .commandPool = pool, .level = VK_COMMAND_BUFFER_LEVEL_SECONDARY,
                                         .commandBufferCount = 1};
    VkCommandBuffer sec;
    VK(pAllocCB(dev, &scbai, &sec), "vkAllocateCommandBuffers(secondary)");
    VkCommandBufferInheritanceInfo inherit = {.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_INHERITANCE_INFO,
                                              .renderPass = rp, .subpass = 0, .framebuffer = fb};
    VkCommandBufferBeginInfo scbbi = {.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO,
                                      .flags = VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT,
                                      .pInheritanceInfo = &inherit};
    VK(pBegin(sec, &scbbi), "vkBeginCommandBuffer(secondary)");
    pBindPipe(sec, VK_PIPELINE_BIND_POINT_GRAPHICS, pipe);
    pCmdDraw(sec, 6, 1, 0, 0);   /* LEFT  RED  quad (verts 0..5) */
    pCmdDraw(sec, 6, 1, 6, 0);   /* RIGHT BLUE quad (verts 6..11) */
    VK(pEnd(sec), "vkEndCommandBuffer(secondary)");

    /* ---- PRIMARY command buffer: begin pass (SECONDARY contents) → execute the secondary → end -------- */
    VkCommandBufferAllocateInfo pcbai = {.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
                                         .commandPool = pool, .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY,
                                         .commandBufferCount = 1};
    VkCommandBuffer cb_h;
    VK(pAllocCB(dev, &pcbai, &cb_h), "vkAllocateCommandBuffers(primary)");
    VkCommandBufferBeginInfo cbbi = {.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};
    VK(pBegin(cb_h, &cbbi), "vkBeginCommandBuffer(primary)");
    ...
    pCmdBeginRP(cb_h, &rpbi, VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS);
    pCmdExec(cb_h, 1, &sec);   /* replay the secondary's recorded draws into this pass */
    pCmdEndRP(cb_h);
    ...
    VK(pEnd(cb_h), "vkEndCommandBuffer(primary)");
