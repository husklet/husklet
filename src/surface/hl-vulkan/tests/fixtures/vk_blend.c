/* VK RENDER-CORRECTNESS DEMO #6 — vk_blend: fixed-function ALPHA BLENDING composites correctly. An OPAQUE
 * background quad then a 50%-alpha foreground quad, drawn with a src-alpha-over blend pipeline, must
 * COMPOSITE (not overwrite) in the overlap.
 *
 * A real C Vulkan program (real loader + OUR ICD → IR → lavapipe). One graphics pipeline whose
 * VkPipelineColorBlendAttachmentState requests blendEnable + srcColor=SRC_ALPHA / dstColor=1-SRC_ALPHA /
 * ADD (straight "source over") is bound once; the vertex shader emits, from gl_VertexIndex, a FULL-FRAME
 * background quad (verts 0..5, color BG = (200,100,40), alpha 1.0) drawn FIRST, then a CENTERED foreground
 * quad (verts 6..11, NDC [-0.5,0.5] → pixel cols/rows [16,48), color FG = (40,180,220), alpha 0.5). Because
 * the background is opaque (srcAlpha = 1) its blend is a no-op replace; the foreground (srcAlpha = 0.5)
 * composites over it, so the overlap is the EXACT straight-alpha composite bg*(1-a)+fg*a = (120,140,130,191)
 * and the border stays the background. If the color-blend state were dropped (the shim bug this guards) the
 * foreground would OVERWRITE and the overlap would be the raw foreground (40,180,220,128).
 *
 * HL_VK_BLEND=0 flips blendEnable to VK_FALSE (the regression control): the same geometry then OVERWRITES,
 * proving the assert distinguishes composite from overwrite. Prints "VK_BLEND_OK". */
#define _GNU_SOURCE
...
int main(void) {
    ...
    /* HL_VK_BLEND=0 => blendEnable VK_FALSE (regression control, overwrite); default/anything-else => TRUE. */
    const char *blend_env = getenv("HL_VK_BLEND");
    VkBool32 blend_enable = (blend_env && strcmp(blend_env, "0") == 0) ? VK_FALSE : VK_TRUE;
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-vk-blend", .apiVersion = VK_API_VERSION_1_1};
    ...
    /* ---- pipeline layout (no descriptors: color+alpha come from gl_VertexIndex) ---------------- */
    VkPipelineLayoutCreateInfo plci = {.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO};
    VkPipelineLayout layout;
    VK(pCreatePL(dev, &plci, NULL, &layout), "vkCreatePipelineLayout");

    /* ---- graphics pipeline: the CRUX is the color-blend attachment (src-alpha-over) ------------ */
    ...
    /* Straight "source over": out = src*srcAlpha + dst*(1 - srcAlpha), on BOTH color and alpha. */
    VkPipelineColorBlendAttachmentState cba = {.blendEnable = blend_enable,
    ...
    /* ---- record: clear(black) -> bind -> draw bg(0..5) -> draw fg(6..11) -> end -> copy -------- */
    ...
    pCmdDraw(cb_h, 6, 1, 0, 0);   /* OPAQUE background quad (verts 0..5) — drawn FIRST, replaces the clear */
    pCmdDraw(cb_h, 6, 1, 6, 0);   /* 50%-alpha foreground quad (verts 6..11) — composites over the bg */
    pCmdEndRP(cb_h);
