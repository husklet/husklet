/* VK ADVANCED-COMMAND DEMO — vk_occlusion_query: an OCCLUSION query's sample count reflects the drawn
 * coverage — >0 for a visible draw, 0 for a fully-scissored (occluded) one.
 *
 * A real C Vulkan program (real loader + OUR ICD → IR → lavapipe). A 2-slot VK_QUERY_TYPE_OCCLUSION pool
 * wraps two draws of the SAME full-screen GREEN quad, each under vkCmdBeginQuery/vkCmdEndQuery, with a
 * DYNAMIC scissor:
 *   Q0 — scissor = LEFT half (0,0)+(32,64): the quad rasterizes 32*64 = 2048 samples → the left half goes
 *        GREEN and the query reports 2048.
 *   Q1 — scissor = EMPTY (0,0)+(0,0): every fragment is scissored away → 0 samples drawn and the query
 *        reports 0 (the "fully occluded" case); the right half stays the BLACK clear.
 * vkGetQueryPoolResults reads both counts back (64-bit, WAIT). The guest checks Q0==2048 && Q1==0 and
 * prints "OCCLUSION Q0=<n> Q1=<n>" + "VK_OCCLUSION_OK"; the Rust test re-parses the counts and asserts the
 * exact left-green/right-black raster. This guards the shim's occlusion model (before the fix vkCmdEndQuery
 * recorded a constant 0 regardless of coverage — a false result). */
#define _GNU_SOURCE
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-vk-occlusion", .apiVersion = VK_API_VERSION_1_1};
    ...
    PFN_vkCreateQueryPool pCreateQP = DPROC(dev, vkCreateQueryPool);
    PFN_vkGetQueryPoolResults pGetQPR = DPROC(dev, vkGetQueryPoolResults);
    ...
    PFN_vkCmdResetQueryPool pCmdResetQP = DPROC(dev, vkCmdResetQueryPool);
    ...
    PFN_vkCmdSetScissor pCmdSetScissor = DPROC(dev, vkCmdSetScissor);
    PFN_vkCmdBeginQuery pCmdBeginQuery = DPROC(dev, vkCmdBeginQuery);
    PFN_vkCmdEndQuery pCmdEndQuery = DPROC(dev, vkCmdEndQuery);
    ...
    if (!pCmdBeginQuery || !pCmdEndQuery || !pGetQPR) {
        fprintf(stderr, "occlusion-query entry points not resolvable\n");
        return 6;
    }
    ...
    /* Scissor is DYNAMIC — vkCmdSetScissor drives the per-query clip that bounds the occlusion coverage. */
    VkDynamicState dyn_states[1] = {VK_DYNAMIC_STATE_SCISSOR};
    ...
    VkQueryPool qpool;
    VK(pCreateQP(dev, &qpci, NULL, &qpool), "vkCreateQueryPool");
    ...
    /* Q0 — visible: scissor to the LEFT half, draw the full-screen quad → 32*64 = 2048 samples pass. */
    VkRect2D sc_left = {{0, 0}, {32, 64}};
    pCmdSetScissor(cb_h, 0, 1, &sc_left);
    pCmdBeginQuery(cb_h, qpool, 0, 0);
    pCmdDraw(cb_h, 6, 1, 0, 0);
    pCmdEndQuery(cb_h, qpool, 0);

    /* Q1 — fully occluded: scissor to an EMPTY rect, draw the same quad → 0 samples pass. */
    VkRect2D sc_none = {{0, 0}, {0, 0}};
    pCmdSetScissor(cb_h, 0, 1, &sc_none);
    pCmdBeginQuery(cb_h, qpool, 1, 0);
    pCmdDraw(cb_h, 6, 1, 0, 0);
    pCmdEndQuery(cb_h, qpool, 1);
    ...
    VK(pGetQPR(dev, qpool, 0, 2, sizeof(results), results, sizeof(uint64_t),
               VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT),
       "vkGetQueryPoolResults");
    printf("OCCLUSION Q0=%" PRIu64 " Q1=%" PRIu64 "\n", results[0], results[1]);
    if (results[0] != 2048) { fprintf(stderr, "Q0 expected 2048, got %" PRIu64 "\n", results[0]); return 7; }
    if (results[1] != 0)    { fprintf(stderr, "Q1 expected 0, got %" PRIu64 "\n", results[1]); return 8; }

    printf("VK_OCCLUSION_OK\n");
