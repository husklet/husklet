/* VK COMPUTE DEMO — vk_compute_saxpy: a REAL VkComputePipeline + vkCmdDispatch runs a buffer compute on
 * lavapipe and the GPU-computed result buffer is read back and asserted BIT-EXACT vs a CPU reference.
 *
 * A real C Vulkan program (REAL Khronos loader + OUR ICD → IR → WgpuExecutor/lavapipe). It creates THREE
 * STORAGE buffers A, B, C (256 x u32), fills A[i]=i+1 and B[i]=i*7+2 through host-visible mapped memory,
 * builds a compute pipeline from a real SPIR-V compute shader (saxpy: C[i] = A[i]*3 + B[i], loaded from
 * $HL_VK_CS_SPV), binds A/B/C as a 3-binding descriptor set, and issues ONE vkCmdDispatch(4,1,1) over
 * @workgroup_size(64) = 256 invocations. Distinct from the executor's own SPIR-V compute path: here the
 * dispatch flows through the REAL vkCreateComputePipelines + vkCmdBindDescriptorSets + vkCmdDispatch shim
 * lowering. The Rust test reads C back off the host executor and asserts C[i] == 3*(i+1) + (i*7+2) exactly.
 * Prints "VK_COMPUTE_SAXPY_OK". */
#define _GNU_SOURCE
...
int main(void) {
    ...
    VkApplicationInfo ai = {.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
                            .pApplicationName = "hl-vk-compute-saxpy", .apiVersion = VK_API_VERSION_1_1};
    ...
    /* ---- three STORAGE buffers: A, B inputs (host-visible), C output (readable via TRANSFER_SRC) ------ */
    VkBuffer bufs[3];
    VkDeviceMemory mems[3];
    const VkBufferUsageFlags usages[3] = {
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,                                       /* A */
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,                                       /* B */
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT,    /* C (host reads back) */
    };
    for (int k = 0; k < 3; k++) {
    ...
    /* Fill A[i]=i+1 and B[i]=i*7+2 via host-visible map/write/unmap (flushed as Cmd::WriteBuffer). */
    ...
    /* ---- shader module + descriptor set layout (3 STORAGE bindings, COMPUTE stage) + pipeline layout -- */
    ...
    /* ---- descriptor pool + set, point bindings 0/1/2 at A/B/C ---------------------------------------- */
    ...
    /* ---- record: bind pipeline + descriptor set, dispatch 4 workgroups of 64 = 256 invocations ------- */
