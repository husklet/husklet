...
fn depth_scaled_3d_blit_records() {
    ...
    let src = create::create_image_geometry(
        &mut d, &mut sink, 4, 4, 4, 1, 1, TextureDim::D3,
        vk_format::R8G8B8A8_UNORM, vk_image_usage::TRANSFER_SRC, 1,
    ).unwrap();
    let dst = create::create_image_geometry(
        &mut d, &mut sink, 4, 4, 2, 1, 1, TextureDim::D3,
        vk_format::R8G8B8A8_UNORM, vk_image_usage::TRANSFER_DST, 1,
    ).unwrap();
    let cb = recording_cb(&mut d);
    ...
    assert!(
        record::cmd_blit_image(
            ...
            SubresourceLayers::base(),
            Origin3d::default(),
            Extent3d { width: 4, height: 4, depth: 4 },
            Origin3d::default(),
            Extent3d { width: 4, height: 4, depth: 2 },
            true,
            Mirror::NONE,
        ).is_ok(),
