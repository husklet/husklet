...
//! the golden 145-entry `cu*` set. Bring-up + the compute path have real hand-written bodies in
...
mod tests {
    ...
    fn surface_is_complete_and_matches_the_census() {
        assert_eq!(
            CUDA_DRIVER_ENTRYPOINTS, 145,
            "CUDA driver surface drifted from the golden 145"
        );
