...
fn htod_over_length_and_interior_overrun_are_out_of_bounds() {
    ...
    assert!(
        got[..48].iter().all(|&b| b == 0xAB),
        "the head is untouched by the rejected OOB writes"
    );
    assert!(
        got[48..].iter().all(|&b| b == 0),
        "only the in-bounds tail write landed"
    );
}
...
fn dtoh_huge_len_is_bounded_before_any_readback_allocation() {
    ...
    assert_eq!(
        transfer::read_dtoh(&c, &mut sink, DevicePtr(p.0 + 40), 24).unwrap(),
        payload[40..].to_vec()
    );
}
...
fn dtod_size_and_offsets_near_u64_max_are_checked_not_overflowed() {
    ...
    assert_oob(
        &transfer::memcpy_dtod(&mut c, &mut sink, b, DevicePtr(a.0 + 200), 100).unwrap_err(),
    );
...
fn memset_huge_count_is_checked_and_bounded_no_giant_vec() {
    ...
    assert_oob(
        &transfer::memset_elements(&mut c, &mut sink, p, 0xDEAD_BEEF, 4, usize::MAX).unwrap_err(),
    );
...
fn freed_and_bogus_pointers_are_rejected_across_every_path() {
    ...
    assert!(
        allocate::mem_free(&mut c, &mut sink, p).is_err(),
...
fn launch_with_stale_or_dangling_handles_never_panics_but_a_valid_launch_computes() {
    ...
    assert!(
        r.is_err(),
        "stale function handle is a typed error, got {r:?}"
    );
    assert_eq!(
        sink.executor().dispatches,
        0,
        "the stale launch dispatched nothing"
    );
    ...
    assert_eq!(
        sink.executor().dispatches,
        0,
        "the dangling-arg launch dispatched nothing"
    );
    ...
    assert_eq!(
        got, want,
        "the valid launch is unaffected by the rejected hostile launches"
    );
}
