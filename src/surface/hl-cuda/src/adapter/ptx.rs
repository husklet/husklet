        ...
        if name == entry {
            ...
            let params_src = if let Some(lp) = lp {
                if lp < lb {
                    let rp = rest[lp..]
                        .find(')')
                        .ok_or_else(|| err("unterminated param list"))?
                        + lp;
...
fn parse_shared_decl(
    ...
    let name_tok = *toks
        .last()
        .ok_or_else(|| err(format!("bad .shared decl: `{s}`")))?;
    ...
    let inst = match base {
        ...
        "mov" => {
            ...
            if is_reg(src) {
                ...
                Inst::MovImmF {
                    d,
                    bits: parse_imm_f(src)?,
                }
            } else {
                Inst::MovImmI {
                    d,
                    imm: parse_imm_i(src)? as u64,
                }
            }
        }
        "ld" => {
...
fn parse_imm_i(t: &str) -> Result<i64> {
    ...
    if let Some(h) = t.strip_prefix("0x").or_else(|| t.strip_prefix("0X")) {
        ...
        t.parse::<i64>()
            .map_err(|_| err(format!("bad integer immediate `{t}`")))
    }
}
