...
fn tearing_control_hint_read_at_commit() {
    ...
    let tc_mgr: WpTearingControlManagerV1 = globals
        .bind(&qh, 1..=1, ())
        .expect("wp_tearing_control_manager_v1 advertised");
    ...
    assert!(
        saw_async,
        "compositor read the `async` tearing hint at commit (got {:?})",
    ...
    assert!(
        saw_vsync,
        "compositor re-read the `vsync` tearing hint after re-commit (got {:?})",
        h.observations.lock().unwrap().tearing_hint.get(&pid)
    );
