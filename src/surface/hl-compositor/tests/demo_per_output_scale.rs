...
fn per_output_scale() {
    ...
    while !(app.drawn && app.frame_done && !app.preferred.is_empty()) {
        assert!(
            ...
            "no map / initial preferred_scale (preferred={:?})",
            app.preferred
        );
        queue
            .blocking_dispatch(&mut app)
            .expect("dispatch map + preferred_scale");
    }
    assert_eq!(
        app.preferred,
        vec![SCALE1_PREFERRED],
        "on scale-1 output A, preferred_scale is 120"
    );
    ...
    while app.preferred.len() < 2 {
        assert!(
            Instant::now() < deadline,
            "no re-sent preferred_scale after routing to B (preferred={:?})",
            app.preferred
        );
    ...
    assert_ne!(
        SCALE1_PREFERRED, SCALE2_PREFERRED,
        "the two outputs' preferred scales differ"
    );
