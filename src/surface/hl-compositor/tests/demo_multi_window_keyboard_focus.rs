...
fn multi_window_keyboard_focus() {
    ...
    h.input_tx
        ...
        .expect("focus A");
    pump2(&mut qa, &mut a, &mut qb, &mut b, 5, |a, _b| {
        a.kbd == [KbdEv::Enter]
    });
    assert_eq!(
        a.kbd,
    ...
    h.input_tx
        ...
        .expect("inject key");
    pump2(&mut qa, &mut a, &mut qb, &mut b, 5, |_a, b| {
        b.kbd.contains(&KbdEv::Key(KEY_A, true))
    });
    assert_eq!(
        b.keys(),
        vec![(KEY_A, true)],
        "B (focused) received the key"
    );
    assert!(
        a.keys().is_empty(),
        "A (unfocused) received NO key, got {:?}",
        a.keys()
    );
    assert_eq!(
        a.kbd,
        [KbdEv::Enter, KbdEv::Leave],
        "A's log is unchanged by the key (still enter,leave)"
    );
    assert_eq!(
        b.kbd,
...
fn pump2(
    qa: &mut wayland_client::EventQueue<Client>,
    a: &mut Client,
    qb: &mut wayland_client::EventQueue<Client>,
    b: &mut Client,
    secs: u64,
    done: impl Fn(&Client, &Client) -> bool,
) {
    ...
    fn event(
        ...
        match e {
            wl_keyboard::Event::Enter { surface, .. } if surface.id() == app.surface.id() => {
                app.kbd.push(KbdEv::Enter)
            }
            wl_keyboard::Event::Leave { surface, .. } if surface.id() == app.surface.id() => {
                app.kbd.push(KbdEv::Leave)
            }
            wl_keyboard::Event::Key { key, state, .. } => {
                app.kbd.push(KbdEv::Key(
                    key,
