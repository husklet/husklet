...
fn single_pixel_buffer_scales_to_solid_fill() {
    ...
    let spb: WpSinglePixelBufferManagerV1 = globals
        .bind(&qh, 1..=1, ())
        .expect("wp_single_pixel_buffer_manager_v1 advertised");
    let viewporter: WpViewporter = globals
        .bind(&qh, 1..=1, ())
        .expect("wp_viewporter advertised");

    // A 1×1 opaque solid-color buffer — no shm pool, no fd.
    let buffer = spb.create_u32_rgba_buffer(
        ch(COLOR[0]),
        ch(COLOR[1]),
        ch(COLOR[2]),
        ch(COLOR[3]),
