...
impl Server {
    fn start(tag: &str) -> Server {
        let socket =
            std::env::temp_dir().join(format!("hl-wip-adv-{}-{}.sock", tag, std::process::id()));
...
impl Client {
    ...
    fn buffer(&mut self, r: u8, g: u8, b: u8) -> WlBuffer {
        ...
        let path =
            std::env::temp_dir().join(format!("hl-wip-adv-{}-{}.shm", std::process::id(), uniq));
...
fn wait_captures(
    server: &Server,
    timeout: Duration,
    mut pred: impl FnMut(&[CapturedFrame]) -> bool,
    msg: &str,
) -> Vec<CapturedFrame> {
    let deadline = Instant::now() + timeout;
    loop {
...
fn throttled_commit_then_idle_ships_and_releases_the_frame_callback() {
    ...
    assert_eq!(
        color_of(&caps[0]),
        [0xF0, 0x10, 0x10, 0xFF],
        "first shipped frame is red"
    );
    assert_eq!(
        color_of(caps.last().unwrap()),
        [0x10, 0x10, 0xF0, 0xFF],
        "the retained blue frame shipped"
    );
}
...
fn a_burst_of_commits_coalesces_to_far_fewer_presents() {
    ...
    wait_captures(
        ...
        Duration::from_secs(5),
        |c| c.len() >= 1,
        "first frame never shipped",
    );

    // Ten more frames committed back-to-back (all well within one 16.6 ms interval), then idle.
    let bursts: Vec<WlBuffer> = (0..10)
        .map(|i| client.buffer(0x30 + i * 4, 0x00, 0x00))
        .collect();
    for b in &bursts {
    ...
    let caps = wait_captures(
        ...
        Duration::from_secs(5),
        |c| c.len() >= 2,
        "the coalesced frame never shipped",
    );
...
fn no_buffer_commit_captures_nothing_but_releases_its_callback() {
    ...
    assert!(
        server.captures().is_empty(),
        "a bufferless commit must not present a frame"
    );
    ...
    wait_captures(
        ...
        Duration::from_secs(5),
        |c| c.len() >= 1,
        "a real frame after a bufferless commit never shipped",
    );
}
...
fn client_disconnect_mid_throttle_does_not_wedge_the_compositor() {
    ...
    client2.pump_until(
        Duration::from_secs(5),
        |a| a.callbacks_done >= 1,
        "second client never got its callback",
    );
    wait_captures(
        &server,
        Duration::from_secs(5),
...
fn two_interleaved_toplevels_both_present() {
    ...
    client.pump_until(
        Duration::from_secs(5),
        |a| a.callbacks_done >= 2,
        "both toplevels never released callbacks",
    );
    wait_captures(
        &server,
        Duration::from_secs(5),
