...
fn popup_composites_at_positioner_rect() {
    ...
    let tl_frame = wait_for(&h.captures, 5, |f| {
        f.width == TL_W && f.pixel_is(TL_W / 2, TL_H / 2, TL)
    })
    ...
    assert_eq!(
        (pop1_frame.x, pop1_frame.y),
        EXPECT_POP,
        "popup1 exact placement"
    );
    assert_eq!(
        pop1_frame.pixel(POP_W / 2, POP_H / 2).unwrap(),
        POP,
        "popup1 color"
    );
    ...
    for &(sx, sy) in &[
        ...
        assert_eq!(
            pop1_frame.pixel(sx, sy).unwrap(),
            POP,
            "popup1 fully B at ({sx},{sy})"
        );
    }
    ...
    let pop2_frame = pump_until(&mut queue, &mut app, &h.captures, 5, |f| {
        f.width == SUB_W
            && f.height == SUB_H
            && (f.x, f.y) == EXPECT_SUB
            && f.pixel_is(SUB_W / 2, SUB_H / 2, SUBM)
    })
    .expect("nested popup never composited at its resolved rect");
    assert_eq!(
        (pop2_frame.x, pop2_frame.y),
        EXPECT_SUB,
        "submenu exact placement"
    );
    assert_eq!(
        pop2_frame.pixel(SUB_W / 2, SUB_H / 2).unwrap(),
        SUBM,
        "submenu color"
    );
    ...
    assert!(
        tl_frame.serial < pop1_frame.serial,
        "toplevel presents below popup1"
    );
    assert!(
        pop1_frame.serial < pop2_frame.serial,
        "popup1 presents below the submenu"
    );
    ...
    save_composited(
        ...
        TL,
        &[
            (&pop1_frame, EXPECT_POP.0, EXPECT_POP.1),
            (&pop2_frame, EXPECT_SUB.0, EXPECT_SUB.1),
        ],
    );
