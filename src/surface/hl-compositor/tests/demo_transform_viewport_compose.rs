...
fn transform_viewport_compose() {
    ...
    assert_eq!(
        ...
        BLUE,
        "top-left = surface bottom-left (blue)"
    );
    assert_eq!(
        frame.pixel(0, DST.1 - 1).unwrap(),
        BLUE,
        "bottom-left = surface bottom-left (blue)"
    );
    assert_eq!(
        frame.pixel(20, DST.1 / 2).unwrap(),
        BLUE,
        "left interior = blue"
    );
    assert_eq!(
        ...
        YELLOW,
        "top-right = surface bottom-right (yellow)"
    );
    assert_eq!(
        frame.pixel(DST.0 - 1, DST.1 - 1).unwrap(),
        YELLOW,
        "bottom-right = surface bottom-right (yellow)"
    );
    assert_eq!(
        frame.pixel(60, DST.1 / 2).unwrap(),
        YELLOW,
        "right interior = yellow"
    );
