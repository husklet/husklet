...
fn input_burst_no_backlog() {
    ...
    loop {
        ...
        if marker_frame.is_none() {
            if let Some(f) = h
                ...
                .iter()
                .rev()
                .find(|f| f.pixel_is(PX, PY, MARKER))
                .cloned()
            {
                t1 = Some(Instant::now());
                marker_frame = Some(f);
        ...
        assert!(
            ...
            "burst not drained: keys={} enters={} marker={}",
            app.keys,
            app.enters,
    ...
    assert_eq!(
        delivered, 60,
        "exactly 60 inputs delivered (delivered count == N)"
    );
    ...
    assert!(
        cycles >= 1 && cycles <= 3,
        "final render bounded to a few cycles (serial delta {cycles})"
    );
    assert!(
        elapsed < LATENCY_CEILING,
        "burst→final-present {elapsed:?} exceeded ceiling {LATENCY_CEILING:?}"
    );
    assert_eq!(
        mf.pixel(PX, PY).unwrap(),
        MARKER,
        "final marker at the injected pointer position"
    );
...
impl Dispatch<WlKeyboard, ()> for App {
    fn event(
        ...
        if let wl_keyboard::Event::Key { .. } = e {
            app.keys += 1;
        }
    }
}
