...
fn pointer_axis_and_buttons() {
    ...
    h.input_tx
        .send(InputCommand::PointerMotion { x: 80.0, y: 60.0 })
        .expect("enter motion");
    ...
    pump_while(&mut queue, &mut app, 5, |a| {
        a.has(PtrEv::AxisV(VSCROLL)) && a.has(PtrEv::AxisH(HSCROLL))
    });
    assert!(
        app.has(PtrEv::AxisV(VSCROLL)),
        "vertical scroll value delivered exactly (with sign)"
    );
    assert!(
        app.has(PtrEv::AxisH(HSCROLL)),
        "horizontal scroll value delivered exactly (with sign)"
    );
    ...
    h.input_tx
        ...
        .expect("right down");
    pump_while(&mut queue, &mut app, 5, |a| {
        a.has(PtrEv::Button(BTN_LEFT, true)) && a.has(PtrEv::Button(BTN_RIGHT, true))
    });
    assert!(
        app.has(PtrEv::Button(BTN_LEFT, true)),
        "BTN_LEFT pressed delivered"
    );
    assert!(
        app.has(PtrEv::Button(BTN_RIGHT, true)),
        "BTN_RIGHT pressed delivered"
    );
    ...
    let scroll_group = groups
        .iter()
        .find(|g| g.contains(&PtrEv::AxisV(VSCROLL)))
        .expect("a frame group contains the vertical axis");
    assert!(
        scroll_group.contains(&PtrEv::AxisH(HSCROLL)),
        "both axes are grouped in ONE wl_pointer.frame, got group {scroll_group:?}"
    );
    assert!(
        !scroll_group.iter().any(|e| matches!(e, PtrEv::Button(..))),
    ...
    for g in &groups {
        ...
        assert!(
            btns <= 1,
            "each wl_pointer.frame carries at most one button, got {g:?}"
        );
    }
    // Frames actually happened (v5+ grouping is live, not a degenerate single stream).
    assert!(
        app.ev.iter().filter(|e| **e == PtrEv::Frame).count() >= 3,
        "at least three frames (scroll + two buttons), got {:?}",
        app.ev
    );
    assert!(
        app.ev.len() > events_before,
        "events accumulated after enter"
    );
