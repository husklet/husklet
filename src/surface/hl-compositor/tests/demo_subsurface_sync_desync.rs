...
fn subsurface_sync_desync() {
    ...
    let sync_buffer = make_buffer(
        ...
        C_W,
        C_H,
        &solid(C_W, C_H, SYNC_COL),
    );
    let desync_buffer = make_buffer(
        &shm,
        &qh,
        &h.runtime_dir,
        "sd-desync",
    ...
    let sync_frame = pump_until(&mut queue, &mut app, &h.captures, 5, |f| {
        is_child(f, SYNC_COL) && (f.x, f.y) == C_POS
    })
    .expect("SYNC subsurface never became visible after the parent committed");
    assert_eq!(
        (sync_frame.x, sync_frame.y),
        C_POS,
        "sync child exact placement (parent + set_position)"
    );
    assert_eq!(
        sync_frame.pixel(C_W / 2, C_H / 2).unwrap(),
        SYNC_COL,
        "sync child color"
    );
    // It first appeared strictly after the pre-parent-commit baseline.
    assert!(
        sync_frame.serial > baseline,
        "sync child present serial ({}) is after the parent commit",
        sync_frame.serial
    );
    ...
    // Confront: composite the green child over the blue toplevel and write a PNG.
    {
        let tl0 = h
            ...
            .iter()
            .rev()
            .find(|f| f.width == TL_W && f.pixel_is(1, 1, TL))
            .cloned()
            .unwrap();
        save_composited(
            "subsurface_sync_desync-sync-visible",
            TL_W,
            TL_H,
            TL,
            &[(&tl0, 0, 0), (&sync_frame, C_POS.0, C_POS.1)],
        );
    }
    ...
    let desync_frame = pump_until(&mut queue, &mut app, &h.captures, 5, |f| {
        is_child(f, DESYNC_COL) && (f.x, f.y) == C_POS
    })
    .expect(
        "DESYNC subsurface never became visible on its own commit (no parent commit was issued)",
    );
    assert_eq!(
        (desync_frame.x, desync_frame.y),
        C_POS,
        "desync child exact placement"
    );
    assert_eq!(
        desync_frame.pixel(C_W / 2, C_H / 2).unwrap(),
        DESYNC_COL,
        "desync child color"
    );
    assert!(
    ...
    // Confront: composite the red child over the blue toplevel and write a PNG.
    {
        let tl1 = h
            ...
            .unwrap();
        save_composited(
            "subsurface_sync_desync-desync-visible",
            TL_W,
            TL_H,
            TL,
            &[(&tl1, 0, 0), (&desync_frame, C_POS.0, C_POS.1)],
        );
    }
