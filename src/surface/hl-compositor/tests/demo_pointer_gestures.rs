...
fn pointer_gestures() {
    ...
    h.input_tx
        .send(InputCommand::PointerMotion { x: 100.0, y: 75.0 })
        .unwrap();
    let deadline = Instant::now() + Duration::from_secs(5);
    while !app.entered {
        let _ = queue.roundtrip(&mut app);
    ...
    // ---- three-finger swipe ----
    h.input_tx
        .send(InputCommand::GestureSwipeBegin { fingers: 3 })
        .unwrap();
    h.input_tx
        .send(InputCommand::GestureSwipeUpdate { dx: 10.0, dy: -5.0 })
        .unwrap();
    h.input_tx
        .send(InputCommand::GestureSwipeEnd { cancelled: false })
        .unwrap();
    // ---- two-finger pinch (zoom 1.5x, rotate 15°) ----
    h.input_tx
        .send(InputCommand::GesturePinchBegin { fingers: 2 })
        .unwrap();
    h.input_tx
        .send(InputCommand::GesturePinchUpdate {
            dx: 4.0,
            dy: 2.0,
            scale: 1.5,
            rotation: 15.0,
        ...
        .unwrap();
    h.input_tx
        .send(InputCommand::GesturePinchEnd { cancelled: false })
        .unwrap();
    ...
    assert_eq!(
        app.events, expected,
        "exact swipe + pinch gestures with fingers/delta/scale/rotation"
    );
...
impl Dispatch<WlPointer, ()> for App {
    fn event(
        ...
        if let wayland_client::protocol::wl_pointer::Event::Enter { .. } = e {
            app.entered = true;
        }
    }
}
impl Dispatch<ZwpPointerGestureSwipeV1, ()> for App {
    fn event(
        ...
        e: <ZwpPointerGestureSwipeV1 as Proxy>::Event,
        _: &(),
        _: &Connection,
        _: &QueueHandle<Self>,
    ) {
        ...
        match e {
            Event::Begin {
                ...
                app.events.push(Ev::SwipeBegin {
                    fingers,
                    on_surface: surface == app.surface,
                });
            }
            Event::Update { dx, dy, .. } => app.events.push(Ev::SwipeUpdate {
                dx: m(dx),
                dy: m(dy),
            }),
            Event::End { cancelled, .. } => app.events.push(Ev::SwipeEnd {
...
impl Dispatch<ZwpPointerGesturePinchV1, ()> for App {
    fn event(
        ...
        e: <ZwpPointerGesturePinchV1 as Proxy>::Event,
        _: &(),
        _: &Connection,
        _: &QueueHandle<Self>,
    ) {
        ...
        match e {
            ...
            Event::Update {
                ...
                app.events.push(Ev::PinchUpdate {
                    dx: m(dx),
                    dy: m(dy),
                    scale_x1000: m(scale),
                    rot_x1000: m(rotation),
                });
            }
            Event::End { cancelled, .. } => app.events.push(Ev::PinchEnd {
                cancelled: cancelled != 0,
            }),
            _ => {}
        }
