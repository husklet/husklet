...
fn set_input_region_gates_pointer_hit_testing() {
    ...
    let runtime_dir =
        std::env::temp_dir().join(format!("hl-wip-inputregion-xdg-{}", std::process::id()));
    ...
    queue
        .roundtrip(&mut app)
        .expect("roundtrip after creating pointer");
    ...
    input_tx
        ...
        .expect("send focus");
    input_tx
        .send(InputCommand::PointerMotion { x: OUT_X, y: OUT_Y })
    ...
    while Instant::now() < settle {
        queue
            .roundtrip(&mut app)
            .expect("client dispatch (outside)");
    ...
    input_tx
        .send(InputCommand::PointerMotion { x: IN_X, y: IN_Y })
        .expect("send inside motion");
    let deadline = Instant::now() + Duration::from_secs(5);
    while app.pointer_enter.is_none() {
        assert!(
            Instant::now() < deadline,
            "pointer never entered at a point INSIDE the input region"
        );
    ...
    assert_eq!(
        (ex, ey),
        (IN_X, IN_Y),
        "wl_pointer.enter surface-local coordinate (inside the region)"
    );
