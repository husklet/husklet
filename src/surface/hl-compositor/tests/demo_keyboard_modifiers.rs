...
fn keyboard_modifiers() {
    ...
    pump_while(&mut queue, &mut app, 5, |a| {
        a.last_mods() == Some(MOD_SHIFT)
    });
    assert_eq!(
        app.last_mods(),
        Some(MOD_SHIFT),
        "mods_depressed == Shift while LEFTSHIFT held"
    );
    ...
    assert!(
        app.keys.contains(&(KEY_A, true)),
        "key event for KEY_A (pressed) delivered while Shift held"
    );
    assert_eq!(
        app.last_mods(),
        Some(MOD_SHIFT),
    ...
    assert_eq!(
        app.last_mods(),
        Some(0),
        "mods_depressed back to 0 after Shift release"
    );
    ...
    assert_eq!(
        app.last_mods(),
        Some(MOD_CTRL),
        "mods_depressed == Control while LEFTCTRL held"
    );
    h.input_tx
        .send(InputCommand::Key {
            keycode: KEY_LEFTCTRL,
    ...
    assert_eq!(
        app.last_mods(),
        Some(0),
        "mods_depressed back to 0 after Control release"
    );
