...
fn real_client_discovers_compositor_via_wayland_display_and_composites() {
    ...
    assert!(
        lock_path.exists(),
        "the standard `.lock` file was not created next to the socket"
    );
    ...
    assert_eq!(
        count("wl_output"),
        1,
        "expected exactly one wl_output global, got {:?}",
        global_list
    );
    assert_eq!(
        count("wl_seat"),
        1,
        "expected exactly one wl_seat global, got {:?}",
        global_list
    );
    ...
    assert_eq!(
        count("wl_data_device_manager"),
        1,
        "expected exactly one wl_data_device_manager global (clipboard/DnD), got {:?}",
        global_list,
    );
    ...
    let data_device_manager: WlDataDeviceManager = globals
        .bind(&qh, 1..=3, ())
        .expect("wl_data_device_manager global");
    ...
    while !(app.output_done && app.seat_caps.is_some()) {
        ...
        queue
            ...
            .expect("client dispatch (output/seat)");
    }
    ...
    assert!(
        app.output_geometry,
        "wl_output.geometry (physical size/subpixel/make/model) never arrived"
    );
    assert_eq!(
        app.output_mode_px,
        Some((1920, 1080)),
        "wl_output.mode current pixel size"
    );
    assert_eq!(app.output_scale, Some(1), "wl_output.scale integer factor");
    assert_eq!(
        app.output_name.as_deref(),
        Some("HL-0"),
        "wl_output.name (v4) matches the scene output"
    );
    ...
    assert!(
        ...
        "wl_seat advertises pointer capability, got {caps:?}"
    );
    assert!(
        caps.contains(Capability::Keyboard),
        "wl_seat advertises keyboard capability, got {caps:?}"
    );
    ...
    queue
        .roundtrip(&mut app)
        .expect("roundtrip after get_data_device (no protocol error)");
    assert!(
        data_device_manager.is_alive(),
        "wl_data_device_manager object created and alive"
    );
    assert!(
        data_device.is_alive(),
        "wl_data_device object created from the seat and alive"
    );
    ...
    assert_eq!(
        frame.pixel(0, 0).expect("corner pixel"),
        [R, G, B, A],
        "corner pixel matches"
    );
    assert!(
