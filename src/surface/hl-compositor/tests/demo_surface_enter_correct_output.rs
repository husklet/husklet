...
fn surface_enter_correct_output() {
    ...
    globals.contents().with_list(|list| {
        for global in list {
            if global.interface == WlOutput::interface().name {
                ...
                let output: WlOutput =
                    registry.bind(global.name, global.version.min(4).max(2), &qh, idx);
                app.outputs.push(BoundOutput {
                    id: output.id(),
                    geometry_x: None,
                });
    ...
    while app.enters.len() < 2 {
        assert!(
            ...
            "no enter for B after routing (enters={:?})",
            app.enters
        );
        let _ = queue.roundtrip(&mut app);
        std::thread::sleep(Duration::from_millis(5));
    ...
    assert_eq!(
        app.leaves,
        vec![a_id.clone()],
        "routing to B leaves A exactly once"
    );
    assert_eq!(
        app.enters,
        vec![a_id.clone(), b_id.clone()],
        "second enter names EXACTLY output B"
    );
    ...
    while app.enters.len() < 3 {
        assert!(
            Instant::now() < deadline,
            "no re-enter for A after routing back (enters={:?})",
            app.enters
        );
        let _ = queue.roundtrip(&mut app);
        std::thread::sleep(Duration::from_millis(5));
    ...
    assert_eq!(
        app.leaves,
        vec![a_id.clone(), b_id.clone()],
        "routing back leaves B exactly once"
    );
    assert_eq!(
        app.enters,
