...
fn input_to_present_latency() {
    ...
    while marker_frame.is_none() {
        ...
        assert!(
            ...
            "input-driven render never presented (STALL)"
        );
    ...
    assert!(
        cycles >= 1 && cycles <= 2,
        "input→present bounded to <=2 cycles (serial delta {cycles})"
    );
    // ---- (b) generous wall-clock ceiling ----
    assert!(
        elapsed < LATENCY_CEILING,
        "input→present latency {elapsed:?} exceeded ceiling {LATENCY_CEILING:?}"
    );
    ...
    assert!(
        after_baseline <= 2,
        "at most 2 presents after baseline (got {after_baseline})"
    );
