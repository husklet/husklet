...
fn present_throughput() {
    ...
    for (i, buf) in frame_buffers.iter().enumerate() {
        ...
        assert_eq!(
            frame.rgba,
            solid_rgba(color),
            "frame {i} pixels match its committed buffer exactly"
        );
    }
    ...
    assert_eq!(
        caps.len(),
        M + 1,
        "exactly M+1 presents (1 map + {M} frames): no dropped/duplicated frame"
    );
    ...
    assert_eq!(
        caps[0].rgba,
        solid_rgba(BASE),
        "present #0 is the BASE map frame"
    );
    for i in 0..M {
        assert_eq!(
            caps[i + 1].rgba,
            solid_rgba(frame_color(i)),
            "present #{} is committed frame {i}",
            i + 1
        );
    }
