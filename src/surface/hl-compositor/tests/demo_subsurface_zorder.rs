...
fn subsurface_positions_and_zorder() {
    ...
    let a_frame = pump_until(&mut queue, &mut app, &h.captures, 5, |f| {
        is_a(f) && (f.x, f.y) == A_POS
    })
    .expect("subsurface A never composited at parent + set_position");
    assert_eq!(
        (a_frame.x, a_frame.y),
        A_POS,
        "subsurface A exact placement"
    );
    assert_eq!(
        a_frame.pixel(A_W / 2, A_H / 2).unwrap(),
        A_COL,
        "subsurface A color"
    );
    ...
    let b_frame = pump_until(&mut queue, &mut app, &h.captures, 5, |f| {
        is_b(f) && (f.x, f.y) == B_POS
    })
    .expect("subsurface B never composited at parent + set_position");
    assert_eq!(
        (b_frame.x, b_frame.y),
        B_POS,
        "subsurface B exact placement"
    );
    assert_eq!(
        b_frame.pixel(B_W / 2, B_H / 2).unwrap(),
        B_COL,
        "subsurface B color"
    );
    ...
    assert!(
        b_serial_before > a_serial_before,
        "B (created later) composites above A: b={b_serial_before} a={a_serial_before}"
    );
    ...
    let b0 = h
        ...
        .unwrap();
    save_composited(
        "subsurface_zorder-frame0-Btop",
        TL_W,
        TL_H,
        TL,
        &[(&a0, A_POS.0, A_POS.1), (&b0, B_POS.0, B_POS.1)],
    );
    ...
    let reordered = pump_while(&mut queue, &mut app, 5, |_| {
        match (
            newest_serial(&h.captures, is_a),
            newest_serial(&h.captures, is_b),
        ) {
    ...
    assert!(
        reordered,
        "after place_above, A never composited above B: a={a_serial_after} b={b_serial_after}"
    );
    assert!(
        a_serial_after > b_serial_after,
        "z-order flipped: A now above B"
    );
    ...
    let b1 = h
        ...
        .unwrap();
    save_composited(
        "subsurface_zorder-frame1-Atop",
        TL_W,
        TL_H,
        TL,
        &[(&b1, B_POS.0, B_POS.1), (&a1, A_POS.0, A_POS.1)],
    );
