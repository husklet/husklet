...
pub fn surface_at(scene: &Scene, root: SurfaceId, x: i32, y: i32) -> Option<(SurfaceId, i32, i32)> {
    // XDG popups are siblings in the window tree rather than wl_subsurface children. They present above
    // the owning toplevel and must therefore hit-test first, deepest/latest popup first. Omitting this
    // walk made menu clicks land on the toplevel underneath: GTK dismissed the select without choosing.
    let mut popups = scene.collect_popups_for_root(root);
    popups.reverse();
    for (popup, ox, oy) in popups {
        if let Some(hit) = surface_at_offset(scene, popup, x, y, ox, oy) {
            return Some(hit);
        }
    }
