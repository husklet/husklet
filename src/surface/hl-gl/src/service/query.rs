...
pub const EXTENSIONS: &[&[u8]] = &[
    ...
    // Full NPOT textures are served: arbitrary positive extents upload and sample for 2D/cube targets,
    // repeat and mirrored-repeat address modes lower directly, and complete NPOT mip pyramids are retained.
    // dEQP's positive control renders a 127x127 repeated cube pixel-exactly blue; omitting the extension
    // made the test apply core ES2's restricted-NPOT rule and expect black from behavior we do not have.
