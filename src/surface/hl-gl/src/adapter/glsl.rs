...
pub(crate) fn flip_render_target_samplers(source: &str, samplers: &[String]) -> String {
            ...
            if brace == 0 && paren == 0 && (word == "in" || word == "out") {
                if let Some(name) = parse_io_decl_forward(b, i) {
                    let (stmt_start, merge_rparen, explicit_loc) =
                        preceding_io_qualifiers(b, start);
                    out.push(IoDecl {
                        is_in: word == "in",
                        name,
                        stmt_start,
        ...
        if ch == b')' {
            ...
            if pp >= 6 && &b[pp - 6..pp] == b"layout" && !(pp > 6 && is_word(b[pp - 7])) {
                ...
                if grp.windows(8).any(|w| w == b"location") {
                    explicit =
                        explicit.or_else(|| Some(parse_location_value(grp).unwrap_or(u32::MAX)));
                }
...
pub fn inject_io_locations(vs: &str, fs: &str) -> (String, String) {
    ...
    (
        apply_edits(vs, &mut vs_edits),
        apply_edits(fs, &mut fs_edits),
    )
}
