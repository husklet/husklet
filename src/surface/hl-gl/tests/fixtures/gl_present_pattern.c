 ...
 * pattern to a real WINDOW surface and PRESENTS it through the ENTIRE hl stack, so a test can assert the
 ...
 *
 * WHY A WINDOW SURFACE (not a pbuffer): `eglCreateWindowSurface` is passed a real `wl_egl_window` ABI value
 * with a null wrapped surface, so the shim reports `wl_surface == 0`. With
...
int main(void) {
    ...
    /* Stable wayland-egl ABI: intptr_t version, width/height/dx/dy, then wl_surface*. A null surface
     * deliberately selects the shim's self-owned xdg_toplevel present path. */
    struct {
        intptr_t version;
