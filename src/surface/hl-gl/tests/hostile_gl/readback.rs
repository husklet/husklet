...
fn read_pixels_huge_extent_does_not_unbounded_alloc() {
    ...
    let px = readpixels::read_pixels(&mut c, &mut sink, 0, 0, i32::MAX, i32::MAX, readpixels::PixelFormat::new(GL_RGBA, GL_UNSIGNED_BYTE)).unwrap();
    assert!(px.is_empty());
    ...
    let px = readpixels::read_pixels(&mut c, &mut sink, 0, 0, 4, 4, readpixels::PixelFormat::new(GL_RGBA, GL_UNSIGNED_BYTE)).unwrap();
...
fn read_pixels_out_of_bounds_region_is_zero_filled_no_oob() {
    ...
    let px = readpixels::read_pixels(&mut c, &mut sink, -100, -100, 8, 8, readpixels::PixelFormat::new(GL_RGBA, GL_UNSIGNED_BYTE)).unwrap();
    assert_eq!(px.len(), 8 * 8 * 4);
    assert!(px.iter().all(|&b| b == 0));
    // A non-positive extent is an empty no-op.
    let px = readpixels::read_pixels(&mut c, &mut sink, 0, 0, -1, 4, readpixels::PixelFormat::new(GL_RGBA, GL_UNSIGNED_BYTE)).unwrap();
    assert!(px.is_empty());
    assert_eq!(c.take_gl_error(), GL_NO_ERROR);
}
