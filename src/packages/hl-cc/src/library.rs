...
impl<'a> SharedLibrarySpec<'a> {
    ...
    pub fn link(
        ...
        if status.success() {
            ...
            Err(format!("shared-library link failed with {status}"))
        }
    }

    fn darwin(&self, environment: &BuildEnvironment, command: &mut Command) -> Result<(), String> {
        let exports = environment.output.join(format!("{}.exports", self.name));
        fs::write(&exports, darwin_exports(self.exports)).map_err(|error| error.to_string())?;
    ...
    fn gnu_windows(&self, environment: &BuildEnvironment, command: &mut Command) -> Result<(), String> {
    ...
    fn msvc_windows(&self, environment: &BuildEnvironment, command: &mut Command) -> Result<(), String> {
        let definition = self.windows_definition(environment)?;
        command
            .arg("-shared")
            .arg(format!(
    ...
    fn windows_definition(&self, environment: &BuildEnvironment) -> Result<std::path::PathBuf, String> {
        let definition = environment.output.join(format!("{}.def", self.name));
        fs::write(&definition, windows_exports(self.exports)).map_err(|error| error.to_string())?;
        Ok(definition)
    }

    fn elf(&self, environment: &BuildEnvironment, command: &mut Command) -> Result<(), String> {
        let version = self
            .elf_version
            .ok_or_else(|| "ELF shared libraries require an explicit version namespace".to_owned())?;
        let map = environment.output.join(format!("{}.map", self.name));
        fs::write(&map, elf_exports(version, self.exports)).map_err(|error| error.to_string())?;
        command.arg("-shared");
