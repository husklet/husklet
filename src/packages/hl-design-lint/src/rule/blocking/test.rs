...
use crate::{AsyncBlocking, Workspace, rule::Rule};
...
fn findings(source: &str) -> Vec<crate::Finding> {
    ...
    let values = AsyncBlocking.check(&Workspace::load([path]).unwrap()).unwrap();
...
"#,
    ...
    assert!(values.iter().any(|value| value.message.contains("filesystem")));
