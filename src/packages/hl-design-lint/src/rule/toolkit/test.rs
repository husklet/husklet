...
"#,
    ...
    assert!(
        values
            .iter()
            .any(|finding| { finding.subject == "Public" && finding.message.contains("gtk (native :: Window)") })
    );
    assert!(
        values
            .iter()
            .any(|finding| { finding.subject == "Render::Native" && finding.message.contains("glib") })
    );
    assert!(values.iter().any(|finding| {
        finding.subject == "Render::render" && finding.message.contains("gdk") && finding.message.contains("gtk")
    }));
    fs::remove_dir_all(root).unwrap();
}

#[test]
fn detects_private_surfaces() {
    let root = fixture("hl-gui");
    write(
        &root,
        "lib.rs",
...
"#,
    ...
    assert!(
        values
            .iter()
            .any(|finding| finding.message.contains("re-export") && finding.message.contains("gtk (gtk::Button)"))
    );
    assert!(
        values
            .iter()
            .any(|finding| { finding.subject == "Generic" && finding.message.contains("gtk :: glib :: ObjectType") })
    );
    assert!(values.iter().any(|finding| {
        finding.subject == "NativeRender" && finding.message.contains("gdk :: prelude :: GdkCairoContextExt")
    }));
    fs::remove_dir_all(root).unwrap();
}

#[test]
fn follows_public_reexports() {
    let root = fixture("hl-gui");
