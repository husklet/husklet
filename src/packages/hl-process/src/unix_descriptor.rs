...
/// Closes a standard descriptor until the returned guard is restored or dropped.
///
...
pub fn close_standard(descriptor: StandardDescriptor) -> io::Result<ClosedStandardDescriptor> {
    let target = descriptor.raw();
...
/// Reads the stable filesystem identity associated with an open descriptor.
///
...
pub fn identity(descriptor: RawFd) -> io::Result<Identity> {
