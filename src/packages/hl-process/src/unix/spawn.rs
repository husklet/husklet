...
pub(super) fn spawn(command: &OwnedCommand) -> std::io::Result<SpawnResult> {
    if command.environment().is_some() {
        SpawnResult::exact(command)
    } else {
        SpawnResult::standard(command)
    }
}

impl SpawnResult {
    fn standard(command: &OwnedCommand) -> std::io::Result<Self> {
    ...
    fn exact(command: &OwnedCommand) -> std::io::Result<Self> {
        let _spawn = spawn_lock()?;
        let stdout = pipe()?;
        let stderr = pipe()?;
