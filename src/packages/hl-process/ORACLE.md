...
The remaining deliberate POSIX limitation is narrower than the engine
activation domain: a hostile worker can escape with `setsid`. Runtime fixtures
and the testing binary are repository-owned, so process-group containment is the
selected mechanism; untrusted application containment remains an engine or
