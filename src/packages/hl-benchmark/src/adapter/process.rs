use crate::{Run, Sample, parse_phase};
...
#[hl_design::classify(pkg)]
fn capture(mut reader: impl Read) -> Result<Vec<u8>, String> {
...
fn descendants(root: u32) -> Vec<u32> {
    ...
    for entry in entries.flatten() {
        ...
        if let Some(parent) = fields.split_whitespace().nth(1).and_then(|value| value.parse::<u32>().ok()) {
...
fn terminate(process: u32, descendants: &[u32]) {
    ...
    for process in descendants {
        signal("-TERM", &process.to_string());
    }
    std::thread::sleep(Duration::from_millis(100));
    signal("-KILL", &group);
    for process in descendants {
        signal("-KILL", &process.to_string());
    }
}
...
#[hl_design::classify(pkg)]
pub(crate) fn sample(run: &Run) -> Result<Sample, String> {
    let mut command = super::command(run)?;
    let start = Instant::now();
    let mut child = command.spawn().map_err(|error| format!("spawn failed: {error}"))?;
    let stdout = child.stdout.take().ok_or_else(|| "missing stdout capture".to_string())?;
    let stderr = child.stderr.take().ok_or_else(|| "missing stderr capture".to_string())?;
    let stdout = std::thread::spawn(move || capture(stdout));
    let stderr = std::thread::spawn(move || capture(stderr));
    let mut timed_out = false;
    let mut tree = Vec::new();
    loop {
        for process in descendants(child.id()) {
    ...
    let stdout = stdout
        .join()
        .map_err(|_| "stdout capture panicked".to_string())??;
    let stderr = stderr
        .join()
        .map_err(|_| "stderr capture panicked".to_string())??;
    if timed_out {
    ...
    if run.native_requested() {
        match native_runs(&diagnostics_text) {
            Some(0) => return Err("native execution was requested but diagnostics report zero native runs".into()),
            Some(_) => {}
    ...
    for line in text.lines() {
        if let Some((name, phase)) = parse_phase(line)? {
...
mod test {
    ...
    #[test]
    fn native_diagnostics_count_boundaries() {
        assert_eq!(
            native_runs(
        ...
        assert_eq!(native_runs("unrelated diagnostic"), None);
    }
}
