...
fn unpack_build_context(body: &[u8], ctx: &std::path::Path, _images_dir: &str) -> Result<(), String> {
            ...
            if inst == "ARG" {
                ...
                if let Some(fsid) = pending_fs.take() {
                    if !bc.materialize(&fsid, &rootfs) {
                        return fail(log, "build cache: failed to restore a cached layer".into(), &ctx);
                    }
                }
        ...
        match inst {
            "FROM" => {
                ...
                if use_cache {
                    if let Some(fsid) = pending_fs.take() {
                        if !bc.materialize(&fsid, &rootfs) {
                            return fail(log, "build cache: failed to restore a stage layer".into(), &ctx);
                        }
                    }
                }
                ...
                if found.is_none() {
                    ...
                    .unwrap_or_else(|e| Err(format!("pull task crashed: {e}")))
                    {
                        ...
                        Err(e) => {
                            return fail(log, format!("pull of base image '{base}' failed: {e}"), &ctx);
                        }
                    }
                }
            ...
            "RUN" => {
                ...
                if let Err(e) = run_step(arch, &workdir, &rootfs, &env, &args, &shell, &mut log).await
                {
            ...
            "COPY" | "ADD" => {
                ...
                if let Some(fromv) =
                    args.split_whitespace().find_map(|p| p.strip_prefix("--from="))
                {
                ...
                if let Err(e) =
                    copy_step(inst, &args, &rootfs, &workdir, &eff_names, &eff_stages, &ctx)
                {
                    return fail(log, e, &ctx);
                }
            }
