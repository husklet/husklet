...
pub(crate) fn strip_api_version<B>(req: &mut hyper::Request<B>) {
    let mut pq = req
        .uri()
        .path_and_query()
...
pub(crate) fn normalize_image_path(pq: &str) -> String {
    let (path, query) = pq
        .split_once('?')
        .map(|(p, q)| (p, Some(q)))
        .unwrap_or((pq, None));
...
pub(crate) async fn docker_headers(mut resp: Response) -> Response {
