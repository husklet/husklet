...
//! driver's `cuModuleLoadData` (hl's PTX front-end). In the C world the *driver's* `cuModuleLoadData`
//! does this walk; hl's Rust driver (`hl-shim-cuda`) treats its image as raw PTX text, so — exactly like
