...
fn executor_transport_rejects_a_failed_frame_acknowledgement() {
    ...
    let mut connection = hl_shim::transport::ExecConn::new(path.to_string_lossy().into_owned());
    let surface = hl_shim::transport::Surface { id: 7, generation: 0, width: 16, height: 9, stride: 64, fd: -1 };
    ...
    let ir = hl_gpu::ir::encode_stream(&[hl_gpu::ir::Cmd::CreateFence(1)]);
