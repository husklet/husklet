...
// A guest shim `use hl_shim::ir::Cmd;` gets the exact type the host replays. There is no second
// definition to keep in sync — the "one source of truth" the architecture demands.
pub use hl_gpu::backend;
pub use hl_gpu::id;
pub use hl_gpu::ir;
pub use hl_gpu::wire;
pub use hl_gpu::{GpuError, Result};
...
pub const IR_WIRE_VERSION: u32 = hl_gpu::ir::WIRE_VERSION;
