...
pub fn launch_ex(ws: &WorkspaceConfig, cols: u16, rows: u16, restore: bool, cwd: Option<&str>, slot: Option<&str>) -> io::Result<Box<dyn PtyBackend>> {
    ...
    let rt = hl_jit::Runtime::new().map_err(to_io)?.cache_dir(pcache_dir.to_string_lossy().into_owned());
    ...
    let image = hl_jit::Image::overlay(upper, [rootfs.clone()]).guest(guest);
    ...
    let mut gpu = hl_gpu::integration::GpuIntegration::new(match ws.arch {
        Arch::Amd64 => hl_gpu::integration::GuestArch::X86_64,
    ...
    if ws.gui {
        ...
        gpu = gpu.with_display(hl_gpu::integration::DisplayIntegration {
    ...
    if let Some(cuda) = &ws.cuda {
        ...
        gpu = gpu.with_cuda(hl_gpu::integration::CudaIntegration {
    ...
    let launched = trt
        .block_on(async { rt.start_into(&container, hl_jit::Stdio3 { tty: true }, out, log_chunks, stdin_rx) })
        .map_err(to_io)?;
