...
impl WorkspaceConfig {
    ...
    pub fn from_ws(ws: Workspace) -> WorkspaceConfig {
        WorkspaceConfig { ws, docker_sock: true, gui: false, scrollback: None, vpn: None, cuda: None }
    }
...
mod tests {
    ...
    fn store_persists_across_reload() {
        ...
        store.upsert(WorkspaceConfig::new("ubuntu-dev", "ubuntu:24.04", Arch::Arm64)).unwrap();
        store.upsert(WorkspaceConfig::new("legacy", "centos:7", Arch::Amd64)).unwrap();
        store.upsert(WorkspaceConfig::new("ubuntu-dev", "ubuntu:22.04", Arch::Arm64)).unwrap();
    ...
    fn rich_config_roundtrips() {
        ...
        assert_eq!(got, cfg, "rich workspace config should round-trip through the block format");
        let _ = std::fs::remove_file(&path);
    }
