...
/// Map a WebGPU logical vertex-buffer slot into Metal's shared physical buffer namespace.
///
/// Ordinary shader resources grow upward from zero. Vertex buffers grow downward from the physical
/// per-stage buffer limit, leaving the widest possible collision-free interval between them. The logical
/// WebGPU vertex-buffer count is a separate API limit and must not be used as a physical resource index.
const fn vertex_buffer_resource_index(
    max_buffers_per_stage: ResourceIndex,
    logical_index: ResourceIndex,
) -> ResourceIndex {
    max_buffers_per_stage - 1 - logical_index
}

const fn vertex_and_layout_buffers_fit(
    max_buffers_per_stage: ResourceIndex,
    layout_buffers: ResourceIndex,
    vertex_buffers: ResourceIndex,
) -> bool {
    layout_buffers.saturating_add(vertex_buffers) <= max_buffers_per_stage
}

#[cfg(test)]
mod resource_layout_tests {
    use super::{vertex_and_layout_buffers_fit, vertex_buffer_resource_index};

    #[test]
    fn wide_vertex_layout_uses_high_physical_slots_above_shader_resources() {
        // The separate-array matrix transform-feedback shape: five shader-side buffers and thirteen
        // logical vertex buffers. It exceeds the logical count if that count is incorrectly treated as
        // the entire Metal namespace, but comfortably fits Metal's 31 physical buffer arguments.
        assert!(vertex_and_layout_buffers_fit(31, 5, 13));
        assert!(!vertex_and_layout_buffers_fit(16, 5, 13));
        assert_eq!(vertex_buffer_resource_index(31, 0), 30);
        assert_eq!(vertex_buffer_resource_index(31, 12), 18);
        assert!(vertex_buffer_resource_index(31, 12) >= 5);
    }

    #[test]
    fn combined_resources_at_or_beyond_physical_limit_are_classified_exactly() {
        assert!(vertex_and_layout_buffers_fit(31, 15, 16));
        assert!(!vertex_and_layout_buffers_fit(31, 16, 16));
    }
}
