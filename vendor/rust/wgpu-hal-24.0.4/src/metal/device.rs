...
impl crate::Device for super::Device {
    ...
    unsafe fn create_render_pipeline(
        ...
        objc::rc::autoreleasepool(|| {
            ...
            let (vs_lib, vs_info) = {
                ...
                for (i, vbl) in desc.vertex_buffers.iter().enumerate() {
                    ...
                    vertex_buffer_mappings.push(naga::back::msl::VertexBufferMapping {
                        id: super::vertex_buffer_resource_index(
            ...
            if !super::vertex_and_layout_buffers_fit(
                self.shared.private_caps.max_buffers_per_stage,
                desc.layout.total_counters.vs.buffers,
                desc.vertex_buffers.len() as u32,
            ) {
                let msg = format!(
            ...
            if !desc.vertex_buffers.is_empty() {
                ...
                for (i, vb) in desc.vertex_buffers.iter().enumerate() {
                    let buffer_index = super::vertex_buffer_resource_index(
                        self.shared.private_caps.max_buffers_per_stage,
                        i as u32,
                    ) as u64;
