// A real GLES2 app (no dd-specific calls): compiles a vertex+fragment shader, uploads an interleaved VBO,
// and draws an animated colored triangle, swapping buffers each frame. Linked against -lEGL -lGLESv2 — it
// runs unmodified on the dd GL shim (which forwards its GL calls as dd-gpu IR to the host Metal executor).
