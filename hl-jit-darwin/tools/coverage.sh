...
dynamic() {
    ...
    A="${DDCOV_ENGINE_A:-$(ls "$ROOT"/target/debug/build/hl-jit-darwin-*/out/ddjit-linux_aarch64 2>/dev/null | head -1)}"
    X="${DDCOV_ENGINE_X:-$(ls "$ROOT"/target/debug/build/hl-jit-darwin-*/out/ddjit-linux_x86_64 2>/dev/null | head -1)}"
    ...
    [ -z "$A" ] && { echo "FATAL: engines not built (no target/debug/build/hl-jit-darwin-*/out/ddjit-linux_aarch64); run \`make jit\` first" >&2; exit 4; }
