...
static uint32_t guest_insn(uint64_t va) {
    return *(uint32_t *)va;
} // pc is a LOADED address
...
static void *map_get(uint64_t g) {
    for (uint32_t h = (g >> 2) & 0x1FFFF;; h = (h + 1) & 0x1FFFF) {
        if (MAP[h].host && MAP[h].gpc == g) return MAP[h].host;
        if (!MAP[h].host) return 0;
    }
}

static void map_put(uint64_t g, void *host) {
    for (uint32_t h = (g >> 2) & 0x1FFFF;; h = (h + 1) & 0x1FFFF) {
        if (!MAP[h].host) {
            MAP[h].gpc = g;
            MAP[h].host = host;
...
static void e_movc(int rd, uint64_t v) {
    emit(0xD2800000u | ((v & 0xFFFF) << 5) | rd);
    for (int s = 1; s < 4; s++) {
        uint32_t h = (v >> (16 * s)) & 0xFFFF;
        if (h) emit(0xF2800000u | ((unsigned)s << 21) | (h << 5) | rd);
    }
}

static void e_ldr(int rt, int rn, int off) {
    emit(0xF9400000u | (((unsigned)(off / 8) & 0xFFF) << 10) | (rn << 5) | rt);
}

static void e_str(int rt, int rn, int off) {
    emit(0xF9000000u | (((unsigned)(off / 8) & 0xFFF) << 10) | (rn << 5) | rt);
}

static void e_stur(int rt, int rn, int off) {
    emit(0xF8000000u | (((unsigned)off & 0x1FF) << 12) | (rn << 5) | rt);
}

static void e_ldur(int rt, int rn, int off) {
    emit(0xF8400000u | (((unsigned)off & 0x1FF) << 12) | (rn << 5) | rt);
}

static void e_movsp_from(int r) {
    emit(0x91000000u | (r << 5) | 31);
} // mov sp, xr

static void e_movsp_to(int r) {
    emit(0x91000000u | (31 << 5) | r);
} // mov xr, sp

static void e_cpu(int r) {
    e_movc(r, (uint64_t)&CPU);
} // r = &CPU (single-threaded)

// Prologue: entered with x0=&CPU. Save host callee-saved x19..x30, load guest sp + x1..x30, x0 last.
static void emit_prologue(void) {
    for (int r = 19; r <= 30; r++)
        e_str(r, 0, OFF_HS + (r - 19) * 8); // preserve host callee-saved
    e_movsp_to(9);
    e_str(9, 0, OFF_HSP); // save host sp
    e_ldr(9, 0, OFF_SP);
    e_movsp_from(9); // load guest sp
    for (int r = 1; r <= 30; r++)
        if (r != 18) e_ldr(r, 0, r * 8); // x18 stays in CPU (macOS-volatile)
...
static void emit_spill(void) {
    e_stur(0, 31, -16);
    e_cpu(0); // park guest x0; x0=&CPU
    for (int r = 1; r <= 30; r++)
        if (r != 18) e_str(r, 0, r * 8);
    e_ldur(1, 31, -16);
    e_str(1, 0, 0); // CPU.x[0]=guest x0
    e_movsp_to(1);
    e_str(1, 0, OFF_SP); // save guest sp
}
...
static void emit_ret(void) { // restore host sp + callee-saved, ret to dispatcher
    ...
    e_movsp_from(1);
    for (int r = 19; r <= 30; r++)
        e_ldr(r, 0, OFF_HS + (r - 19) * 8);
    emit(0xD65F03C0u);
}

// exits (x0=&CPU after emit_spill). pc is a LOADED address.
static void exit_const(uint64_t pc, int rsn) {
    emit_spill();
    e_movc(1, pc);
    e_str(1, 0, OFF_PC);
    e_movc(1, rsn);
...
static void exit_call(uint64_t ret, uint64_t pc, int rn) { // bl(rn<0): pc const; blr(rn>=0): pc=xrn. set guest x30=ret.
    ...
    e_movc(1, ret);
    e_str(1, 0, 30 * 8); // CPU.x[30] = ret addr
    if (rn >= 0)
        e_str(2, 0, OFF_PC);
    else {
        e_movc(1, pc);
...
static void translate_block(uint64_t gpc) { // gpc is a LOADED address
    ...
    for (;;) {
        ...
        if (in == 0xD4001001u) {
            exit_const(gpc + 4, R_SYSCALL);
            return;
        } // svc
        if ((in & 0xFC000000u) == 0x14000000u) {
            int64_t o = in & 0x3FFFFFF;
            if (o & (1 << 25)) o |= ~0x3FFFFFFLL;
            exit_const(gpc + (o << 2), R_NEXT);
            return;
        } // b
        if ((in & 0xFC000000u) == 0x94000000u) {
            int64_t o = in & 0x3FFFFFF;
            if (o & (1 << 25)) o |= ~0x3FFFFFFLL;
            exit_call(gpc + 4, gpc + (o << 2), -1);
            return;
        } // bl
        ...
        if ((in & 0xFFFFFC1Fu) == 0xD63F0000u) {
            exit_call(gpc + 4, 0, (in >> 5) & 31);
            return;
        } // blr
        if ((in & 0xFF000010u) == 0x54000000u) {
            int64_t o = (in >> 5) & 0x7FFFF;
            if (o & (1 << 18)) o |= ~0x7FFFFLL; // b.cond
            uint32_t *p = CP;
            emit(0);
            exit_const(gpc + 4, R_NEXT);
            *p = (in & 0xFF00001Fu) | (((uint32_t)(CP - p) & 0x7FFFF) << 5);
            exit_const(gpc + (o << 2), R_NEXT);
            return;
        }
        if ((in & 0x7E000000u) == 0x34000000u) {
            int64_t o = (in >> 5) & 0x7FFFF;
            if (o & (1 << 18)) o |= ~0x7FFFFLL; // cbz/cbnz
            uint32_t *p = CP;
            emit(0);
            exit_const(gpc + 4, R_NEXT);
            *p = (in & 0xFF00001Fu) | (((uint32_t)(CP - p) & 0x7FFFF) << 5);
        ...
        if ((in & 0x7E000000u) == 0x36000000u) {
            int64_t o = (in >> 5) & 0x3FFF;
            if (o & (1 << 13)) o |= ~0x3FFFLL; // tbz/tbnz (imm14)
            uint32_t *p = CP;
            emit(0);
            exit_const(gpc + 4, R_NEXT);
            *p = (in & 0xFFF8001Fu) | (((uint32_t)(CP - p) & 0x3FFF) << 5);
            exit_const(gpc + (o << 2), R_NEXT);
            return;
        }
        if ((in & 0x9F000000u) == 0x10000000u) {
            int rd = in & 0x1F;
            int64_t imm = (((in >> 5) & 0x7FFFF) << 2) | ((in >> 29) & 3);
        ...
        if ((in & 0x9F000000u) == 0x90000000u) {
            ...
            int64_t imm = (((in >> 5) & 0x7FFFF) << 2) | ((in >> 29) & 3);
            if (imm & (1LL << 20)) imm |= ~((1LL << 21) - 1);
            e_movc(rd, (int64_t)(gpc & ~0xFFFLL) + (imm << 12));
            gpc += 4;
            continue;
        } // adrp -> loaded PAGE (slide-relocated; copying it verbatim would page off the host cache PC)
        emit(in);
        gpc += 4; // same-ISA copy
    }
}
...
struct vol {
    const char *cont, *host;
} g_vols[16];

int g_nvol;     // bind volumes (--volume HOST:CONT)
...
static const char *jail(const char *p, char *out) {
    ...
    for (int i = 0; i < g_nvol; i++) {
        size_t L = strlen(g_vols[i].cont);
        if (!strncmp(p, g_vols[i].cont, L) && (p[L] == '/' || p[L] == 0)) {
            snprintf(out, 1024, "%s%s", g_vols[i].host, p + L);
            return out;
        }
    }
    if (g_rootfs) {
...
static void darwin_service(void) {
    ...
    if (g_pid1 && nr == SYS_getpid) {
        CPU.x[0] = 1;
        return;
    }
    if (g_pid1 && nr == SYS_getppid) {
        CPU.x[0] = 0;
        return;
    }
    ...
    case SYS_getattrlist:
        a[0] = (long)jail((char *)a[0], b0);
        tag = "  (jailed)";
        break;
    case SYS_openat: // openat: path is arg1
        a[1] = (long)jail((char *)a[1], b1);
        tag = "  (jailed)";
        break;
    case SYS_rename:
    case SYS_link:
    case SYS_symlink: // two paths
        a[0] = (long)jail((char *)a[0], b0);
        a[1] = (long)jail((char *)a[1], b1);
        tag = "  (jailed)";
        break;
    ...
    if (g_log)
        fprintf(stderr, "[jitdarwin] %s #%ld a0=%#lx a1=%#lx%s\n", nr < 0 ? "MACH-TRAP" : "BSD", nr < 0 ? -nr : nr,
                a[0], a[1], tag);
...
static void run_guest(void) {
    for (;;) {
        ...
        if (!host) {
            ...
            translate_block(CPU.pc);
            pthread_jit_write_protect_np(1);
            __builtin___clear_cache((char *)CACHE, (char *)CP);
            host = map_get(CPU.pc);
        }
        ((blockfn)host)(&CPU);
...
int dd_run(const char *rootfs, int argc, char *const argv[]) {
    ...
    lseek(fd, 0, SEEK_SET);
    uint8_t *f = malloc(sz);
    if (read(fd, f, sz) != sz) return 2;
    close(fd);
    struct mach_header_64 *mh = (void *)f;
    if (mh->magic != MH_MAGIC_64) {
        fprintf(stderr, "not Mach-O 64\n");
        return 2;
    }
    uint8_t *base = mmap(0, 256 << 20, PROT_NONE, MAP_PRIVATE | MAP_ANON, -1, 0); // reserve a non-colliding window
    uint8_t *lc = f + sizeof *mh;
    uint64_t first_vm = 0, entryoff = 0;
    int got = 0;
    for (uint32_t i = 0; i < mh->ncmds; i++) {
        ...
        if (c->cmd == LC_SEGMENT_64) {
            ...
            if (s->vmsize && strcmp(s->segname, "__PAGEZERO")) {
                if (!got) {
                    first_vm = s->vmaddr;
                    g_slide = (int64_t)base - (int64_t)first_vm;
                    got = 1;
                }
                uint64_t dst = s->vmaddr + g_slide;
                mmap((void *)dst, (s->vmsize + 0xFFF) & ~0xFFFULL, PROT_READ | PROT_WRITE,
            ...
            entryoff = ((struct entry_point_command *)lc)->entryoff;
        lc += c->cmdsize;
    }
    uint8_t *stk = mmap(0, 1 << 20, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, -1, 0);
    CPU.sp = ((uint64_t)(stk + (1 << 20) - 256)) & ~15ULL;
    CPU.pc = g_text_lo + entryoff + g_slide;
    CPU.x[18] = 0; // LOADED entry address
    CACHE = mmap(0, CACHE_SZ, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANON | MAP_JIT, -1, 0);
    CP = (uint32_t *)CACHE;
