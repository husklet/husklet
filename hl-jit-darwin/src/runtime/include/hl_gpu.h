...
// returns the IOSurface's global id, which the guest carries to `hl-display` in the linux-dmabuf modifier
...
// ioctl request number (hl-private magic 0xDD; _IOWR-shaped, 32-byte arg). Guest sends this on the
...
struct hl_gpu_alloc {
    ...
    uint32_t id;      // out (IOSurface global id — carried to hl-display in the dmabuf modifier)
...
// hl-private dmabuf modifier: modifier_lo = the IOSurface global id; modifier_hi low-16 = this magic
