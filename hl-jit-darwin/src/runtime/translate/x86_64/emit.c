...
// on slimsys_on(): with HL_JIT_NOSLIMSYS=1 every exit full-spills and never reads cpu->vdirty, so the
...
static int g_fastsys = 1;         // master switch (HL_JIT_NOFASTSYS=1 -> 0 = byte-identical old path)
...
static int g_siginline = 1;           // W4F switch (HL_JIT_NOSIGINLINE=1 disables ONLY the W4F arms)
...
static void s1_calibrate(void) {
    const char *off = getenv("HL_JIT_NOFASTSYS");
    if (off && off[0] && off[0] != '0') {
        g_fastsys = 0;
        return;
    }
    const char *nosi = getenv("HL_JIT_NOSIGINLINE");
    ...
    if (getenv("HL_JIT_FASTSYS_FORCE")) goto anchor;
#ifdef __APPLE__
    ...
    *m1 = (*m1 & 0xFF00001Fu) | ((uint32_t)(((uint32_t *)g_cp - m1) & 0x7FFFF) << 5);
    ...
    if (g_siginline) {
        ...
        *spm_miss = (*spm_miss & 0xFF00001Fu) | ((uint32_t)(((uint32_t *)g_cp - spm_miss) & 0x7FFFF) << 5);
        ...
        to_slow[nsl++] = gtod_miss; // W4F off (HL_JIT_NOSIGINLINE): gtod miss -> slow, exactly as S1
    }
