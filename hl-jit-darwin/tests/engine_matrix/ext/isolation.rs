...
//!
//! Coverage spans both matrix engines wherever the control can apply:
//!  - CPU-count cap + ulimit: PORTABLE guests, run JIT-emulated on both Linux engines -- one source,
//!    byte-identical golden output on every engine.
//!  - read-only rootfs: Linux via a busybox shell in a real image rootfs (both arches). Asserts rootfs
//!    write -> EROFS, /tmp still writable.
//!  - masked / read-only /proc paths: a Linux-kernel concept, covered on both Linux engines.
