...
fn robust_buffer_access_is_not_advertised_by_either_core_query() {
    ...
    vk_dd::vkGetPhysicalDeviceFeatures(core::ptr::null_mut(), &mut legacy);
    assert_eq!(legacy.robust_buffer_access, vk::FALSE);

    let mut features2 = vk::PhysicalDeviceFeatures2::default();
    features2.features.robust_buffer_access = vk::TRUE;
    vk_dd::vkGetPhysicalDeviceFeatures2(core::ptr::null_mut(), &mut features2);
...
fn logical_device_rejects_request_for_unadvertised_robust_access() {
    ...
    let physical_device = 1usize as vk_dd::types::VkPhysicalDevice;
    let mut device = 1usize as vk_dd::types::VkDevice;
    let result = vk_dd::vkCreateDevice(
        physical_device,
        &create_info,
        core::ptr::null(),
        &mut device,
    );
    assert_eq!(result, vk_dd::types::VK_ERROR_FEATURE_NOT_PRESENT);
