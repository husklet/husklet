...
mod macos {
    ...
    use hl_gpu::id::{BindGroupId, BufferId, PipelineId, SamplerId, TextureId};
    use hl_gpu::ir::{
    ...
    fn compute_pipeline_with_non_msl_shader_routes_to_wgpu() {
        use hl_gpu::id::ShaderId;
        use hl_gpu::ir::{ComputePipelineDesc, ShaderRef};
        let Some(ctx) = ctx() else { return };
        let mut be = MetalBackend::new(&ctx);
    ...
    fn msl_compute_vecadd_reads_back_sum_on_metal() {
        ...
        be.create_shader(ShaderId(20), hl_gpu::ir::ShaderPayloadKind::LegacyMsl, &words).unwrap();
    ...
    fn destroy_pipeline_and_bind_group_are_supported() {
        ...
        use hl_gpu::ir::{AddressMode, Filter, SamplerDesc};
    ...
    fn guest_create_texture_for_reserved_present_id_is_not_swallowed() {
        use hl_gpu::ir::{texture_usage, TextureDesc, TextureDim, TextureFormat};
