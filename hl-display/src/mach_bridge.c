...
// Rust structs is the classic footgun). macOS-only; compiled by hl-display/build.rs and linked with the
...
int hl_mach_server_start(const char *name) {
...
int hl_mach_recv(uint32_t *out_id, uint32_t *out_generation, void **out_surface) {
    struct {
        hl_gpu_msg_t msg;
