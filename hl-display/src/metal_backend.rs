...
pub fn selftest_shader(out: &str) -> ! {
    ...
    unsafe {
        ...
        let cmds = vec![
            ...
            Cmd::CreateShader {
                id: 20,
                kind: hl_gpu::ir::ShaderPayloadKind::LegacyMsl,
                spirv: words,
            },
        ...
        hl_gpu::replay::replay_stream(&mut be, &bytes).expect("replay");
...
pub fn selftest_shim_ir(irfile: &str, out: &str, w: u32, h: u32, target_id: u32) -> ! {
    ...
    unsafe {
        ...
        match hl_gpu::replay::replay_stream(&mut be, &bytes) {
...
pub fn run_executor(sock_path: String) {
    ...
    fn handle(
        ...
        let mut be = MetalBackend::new(ctx);
        let limits = hl_gpu::limits::ReplayLimits::from_capabilities(be.capabilities());
        let mut budget = hl_gpu::limits::ExecutorBudget::new(limits, global_budget);
        ...
        loop {
            ...
            unsafe {
                ...
                be.wrap_iosurface_texture(
                    ...
                    w,
                    h,
                    hl_gpu::ir::TextureFormat::Bgra8Unorm,
                );
            }
...
pub fn selftest_replay(out: &str) -> ! {
    ...
    unsafe {
        ...
        let cmds = vec![
            ...
            Cmd::CreateShader {
                id: 20,
                kind: hl_gpu::ir::ShaderPayloadKind::DemoBuiltin,
                spirv: vec![],
            },
            Cmd::CreateRenderPipeline(
                30,
        ...
        hl_gpu::replay::replay_stream(&mut be, &bytes).expect("replay_stream");
...
impl GpuBackend for MetalBackend {
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            present_kinds: vec![],
            wire_version: hl_gpu::ir::WIRE_VERSION,
            command_bits: hl_gpu::backend::command_bits(hl_gpu::backend::HARDWARE_COMMANDS),
            ...
            shader_payloads: hl_gpu::backend::shader_payload::MSL | hl_gpu::backend::shader_payload::SPIRV,
            ...
            texture_formats: hl_gpu::backend::format_bits(hl_gpu::backend::COLOR_FORMATS)
                | hl_gpu::backend::format_bits(&[
                    hl_gpu::ir::TextureFormat::Depth32Float,
                    hl_gpu::ir::TextureFormat::Depth24PlusStencil8,
                ]),
    ...
    fn write_buffer(&mut self, id: BufferId, offset: u64, data: &[u8]) -> Result<()> {
        ...
        if end.map_or(true, |e| e > cap) {
            ...
            eprintln!("[hl-display/metal_backend] write_buffer OOB: id={} offset={} len={} cap={}",
    ...
    fn create_sampler(&mut self, id: SamplerId, desc: &SamplerDesc) -> Result<()> {
        use hl_gpu::ir::{AddressMode, Filter};
        let sd = MTLSamplerDescriptor::new();
    ...
    fn create_shader(&mut self, id: ShaderId, kind: hl_gpu::ir::ShaderPayloadKind, spirv: &[u32]) -> Result<()> {
        ...
        let src: Option<String> = if kind == hl_gpu::ir::ShaderPayloadKind::SpirV {
                ...
                Enc::CopyBufferToTexture {
                    ...
                    };
                    {
                        ...
                        if need.map_or(true, |n| n > buf.length()) {
                            eprintln!("[hl-display/metal_backend] CopyBufferToTexture OOB: src_off={} bpr={} h={} buf_len={}",
