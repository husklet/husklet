...
fn main() {
        ...
        match a.as_str() {
            ...
            "selftest-shim-ir" => {
                #[cfg(target_os = "macos")]
                {
                    ...
                    hl_display::metal_backend::selftest_shim_ir(&irf, &out, w, h, target);
                }
                #[cfg(not(target_os = "macos"))]
                {
            ...
            "selftest-msl" => {
                #[cfg(target_os = "macos")]
                {
                    ...
                    hl_display::metal_backend::selftest_msl(&f);
                }
                #[cfg(not(target_os = "macos"))]
                {
    ...
    #[cfg(target_os = "macos")]
    {
        if window {
            ...
            return hl_display::present_cocoa::run_window(lfd, socket, win_metal);
        }
        if png_dir.is_none() {
            return hl_display::present_cocoa::run(lfd, socket, metal);
        }
    }
...
fn serve_loop_metal(lfd: RawFd, dir: &str) {
    ...
    serve_multiplex(lfd, "metal-png", move || {
        hl_display::metal::MetalPngPresenter::new(&dir)
    });
}
