...
impl Vulkan {
    ...
    fn staged(&self, name: &str) -> PathBuf {
        self.spec.stage_root.join("vulkan").join(self.spec.arch.dir()).join(name)
    }
}
...
mod tests {
    ...
    fn device_request_binds_the_icd_manifest_and_socket() {
        ...
        let want = [
            ("/opt/hlroot/vulkan/aarch64/libvk_hl.so.1", format!("{libdir}/libvk_hl.so.1")),
            ("/opt/hlroot/vulkan/aarch64/libvk_hl.so.1", format!("{libdir}/libvk_hl.so")),
            ("/opt/hlroot/vulkan/aarch64/icd.json", format!("{libdir}/hl_vulkan_icd.json")),
        ];
        for (host, container) in want {
            assert!(
                req.mounts.iter().any(|m| m.host == host && m.container == container && m.read_only),
    ...
    fn env_points_loader_at_the_icd_and_prepends_ld_library_path() {
        let req = Vulkan::new(spec()).device_request(&["LD_LIBRARY_PATH=/usr/lib:/lib".to_string()]);
        assert!(req
            .env
            .contains(&"LD_LIBRARY_PATH=/usr/lib/aarch64-linux-gnu:/usr/lib:/lib".to_string()));
        assert!(req
            .env
            .contains(&"VK_ICD_FILENAMES=/usr/lib/aarch64-linux-gnu/hl_vulkan_icd.json".to_string()));
