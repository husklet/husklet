...
impl QueryPoolRec {
    ...
    pub fn new(query_type: i32, count: u32) -> Self {
        Self { query_type, count, results: vec![QueryResult::default(); count as usize] }
    }
}
...
pub enum DeferredOp {
    ...
    QueryReset { pool: VkQueryPool, first: u32, count: u32 },
    /// `vkCmdEndQuery` (occlusion / pipeline-statistics) — mark the slot available with a bounded value.
    QueryEnd { pool: VkQueryPool, query: u32, value: u64 },
