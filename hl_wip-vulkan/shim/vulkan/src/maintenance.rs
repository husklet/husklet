...
/// Fill a `VkMemoryRequirements2`'s base requirements with `size` (the single unified memory type).
fn write_mem_reqs2(p_memory_requirements: *mut c_void, size: u64) {
    if let Some(out) = unsafe { (p_memory_requirements as *mut VkMemoryRequirements2).as_mut() } {
        out.memory_requirements = VkMemoryRequirements { size, alignment: 256, memory_type_bits: 1 };
    }
}
...
/// `vkGetDeviceImageMemoryRequirements(KHR)` — derive an image's requirements from its create info: the
/// modeled images are RGBA8 (4 bytes/texel) single-mip 2D targets, so `size = width*height*4`.
#[no_mangle]
pub extern "C" fn vkGetDeviceImageMemoryRequirements(
    ...
    let size = unsafe {
        (p_info as *const VkDeviceImageMemoryRequirements)
            .as_ref()
            .and_then(|i| i.p_create_info.as_ref())
            .map(|ci| ci.extent.width as u64 * ci.extent.height.max(1) as u64 * 4)
            .unwrap_or(0)
    };
    write_mem_reqs2(p_memory_requirements, size);
}
