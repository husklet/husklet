...
fn tex_of(d: &Device, image: u64) -> u32 {
    d.images.get(&image).expect("presentable image tracked in dev.images").ir_id
}
...
fn swapchain_create_and_images_tracks_real_render_targets() {
    ...
    assert_eq!(images.len(), N as usize, "the swapchain reports its N presentable images");
    let uniq: std::collections::BTreeSet<u64> = images.iter().copied().collect();
    assert_eq!(uniq.len(), N as usize, "each presentable image is a distinct VkImage handle");
    assert!(images.iter().all(|&h| h != 0), "no VkImage handle is VK_NULL_HANDLE");
    // Real Vulkan returns identical handles on every call — the shim must too.
    assert_eq!(present::get_swapchain_images(&d, sc).unwrap(), images, "image handles are stable across calls");
    ...
    for &h in &images {
        let rec = d.images.get(&h).expect("presentable image tracked in dev.images");
        assert!(rec.is_render_target, "a presentable image is a render target");
        assert_ne!(rec.usage & texture_usage::RENDER_TARGET, 0, "renderable — the app draws into it");
        assert_ne!(rec.usage & texture_usage::COPY_SRC, 0, "copy-source-able — present reads it back");
        assert_eq!((rec.width, rec.height), (W, H), "sized from the surface");
        assert_eq!(rec.format, TextureFormat::Bgra8Unorm, "formatted from the surface");
        texids.insert(rec.ir_id);
    }
    assert_eq!(texids.len(), N as usize, "each image has its own distinct backing texture id");

    // create_swapchain emitted exactly one CreateTexture per presentable image.
    let create_texs = sink.batches.iter().flatten().filter(|c| matches!(c, Cmd::CreateTexture(..))).count();
    assert_eq!(create_texs, N as usize, "one CreateTexture emitted per presentable image");
        ...
        match sink.batches.last().unwrap().as_slice() {
            [Cmd::Submit(cbuf)] => match cbuf.encoder.as_slice() {
                [Enc::BeginRenderPass { color, .. }, Enc::EndRenderPass] => {
                    assert_eq!(color[0].texture, tex, "a render pass targets the swapchain image's own texture");
                }
...
fn acquire_round_robin_present_sources_the_acquired_image() {
    ...
    for _ in 0..ITERS {
        ...
        let cb = record::allocate_command_buffer(&mut d);
        record::begin(&mut d, cb, false).unwrap();
        record::cmd_begin_render_pass(&mut d, cb, images[idx as usize], [0.2, 0.4, 0.6, 1.0], true, None).unwrap();
        record::cmd_end_render_pass(&mut d, cb).unwrap();
        record::end(&mut d, cb).unwrap();
        submit::queue_submit(&mut d, &mut sink, &[cb], None).unwrap();
        ...
        assert_eq!(rendered_tex, texids[idx as usize], "the frame renders into the acquired image");

        // Present it — the present must source the acquired image's texture.
        present::queue_present(&mut d, &mut sink, sc, idx).unwrap();
        match sink.batches.last().unwrap().as_slice() {
            [Cmd::Present { texture, .. }] => {
                assert_eq!(*texture, texids[idx as usize], "the present sources the acquired image's texture")
            }
    ...
    assert_eq!(acquired, vec![0, 1, 2, 0, 1, 2, 0, 1], "acquire cycles through all {N} images in FIFO order");
    assert!(acquired.iter().any(|&i| i == N - 1), "the whole ring is used, not just image 0");
}
...
fn multi_frame_resource_lifetime_reuses_transients_without_id_leak() {
    ...
    let before = create::create_buffer(&mut d, &mut sink, vk_buffer_usage::VERTEX_BUFFER, 16).unwrap();
    ...
    for frame in 0..FRAMES {
        ...
        record::cmd_begin_render_pass(&mut d, cb, images[idx as usize], [0.1, 0.2, 0.3, 1.0], true, None).unwrap();
        record::cmd_end_render_pass(&mut d, cb).unwrap();
        record::end(&mut d, cb).unwrap();
        ...
        match sink.batches.last().unwrap().as_slice() {
            [Cmd::Submit(cbuf)] => match cbuf.encoder.as_slice() {
                [Enc::BeginRenderPass { color, .. }, Enc::EndRenderPass] => {
                    assert_eq!(color.len(), 1, "frame {frame}: one color target (recording did not accumulate)");
                    assert_eq!(color[0].texture, texids[idx as usize], "frame {frame}: renders into the acquired image");
                }
                other => panic!("frame {frame}: expected [BeginRenderPass, EndRenderPass], got {other:?}"),
            },
        ...
        match sink.batches.last().unwrap().as_slice() {
            [Cmd::Present { texture, .. }] => {
                assert_eq!(*texture, texids[idx as usize], "frame {frame}: presents the acquired image")
            }
        ...
        assert_eq!(sync::semaphore_counter(&d, sem).unwrap(), (frame + 1) as u64, "frame {frame}: semaphore reused");

        // The transients are REUSED, not reallocated: the device tables never grow a per-frame object.
        assert_eq!(d.command_buffers.len(), 1, "frame {frame}: the one command buffer is reused");
        assert_eq!(d.fences.len(), 1, "frame {frame}: the one fence is reused");
        assert_eq!(d.semaphores.len(), 1, "frame {frame}: the one semaphore is reused");
        assert_eq!(d.fences.get(&fence).unwrap().ir_id, fence_ir, "frame {frame}: the fence keeps its backing ir id");
        assert_eq!(d.images.len(), N as usize, "frame {frame}: the swapchain images persist (none minted/dropped)");
    }

    // No id leak: the whole loop consumed ZERO fresh ir ids — the next sentinel is exactly before + 1.
    let after = create::create_buffer(&mut d, &mut sink, vk_buffer_usage::VERTEX_BUFFER, 16).unwrap();
    let after_ir = d.buffers.get(&after).unwrap().ir_id;
    assert_eq!(after_ir, before_ir + 1, "the {FRAMES}-frame present loop leaked no ir ids (transients reused)");
    assert_eq!(d.images.len(), N as usize, "exactly the swapchain's own images remain");
}
...
fn swapchain_recreation_retires_old_images_without_leak() {
    ...
    let surf_a = present::create_surface(&mut d, &mut sink, 640, 480, vk_format::B8G8R8A8_UNORM, 0).unwrap();
    ...
    let surf_b = present::create_surface(&mut d, &mut sink, 800, 600, vk_format::B8G8R8A8_UNORM, 0).unwrap();
    let sc_b = present::create_swapchain(&mut d, &mut sink, surf_b, 3).unwrap();
    let imgs_b = present::get_swapchain_images(&d, sc_b).unwrap();
    assert_eq!(d.images.len(), 5, "both swapchains' images coexist momentarily (2 + 3)");
    for &h in &imgs_b {
        let rec = d.images.get(&h).unwrap();
        assert_eq!((rec.width, rec.height), (800, 600), "B's images carry the NEW extent");
    }
    ...
    assert!(d.swapchains.get(&sc_a).is_none(), "A's swapchain record is retired");
    for &h in &imgs_a {
        assert!(d.images.get(&h).is_none(), "A's presentable image {h:#x} is retired from dev.images (no leak)");
    }
    assert!(d.surfaces.get(&surf_a).is_none(), "A's presentation surface is retired");
    ...
    let destroyed_texs: Vec<u32> = sink
        ...
        .collect();
    assert_eq!(destroyed_texs, texs_a, "each of A's textures is freed on the host");
    assert!(
        sink.batches.iter().flatten().any(|c| matches!(c, Cmd::DestroySurface(id) if *id == surf_a_ir)),
    ...
    assert_eq!(d.images.len(), 3, "exactly B's three images remain — A leaked nothing");
    for &h in &imgs_b {
        assert!(d.images.get(&h).is_some(), "B's image {h:#x} survives A's destruction");
    }
    let idx = present::acquire_next_image(&mut d, sc_b).unwrap();
    assert!(present::queue_present(&mut d, &mut sink, sc_b, idx).is_ok(), "B still presents after A is gone");

    // The destroyed swapchain is truly dead: a present against it is a truthful error, not a false success.
    assert!(present::queue_present(&mut d, &mut sink, sc_a, 0).is_err(), "presenting a destroyed swapchain errors");
