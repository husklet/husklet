...
//!     `hl_wip-vulkan`. See `render_to_layer_and_mip_is_a_documented_whole_texture_limit`.
...
fn pipeline_samples(
    ...
    create::create_graphics_pipeline(d, sink, (vs, "vsmain"), Some((fs, "fsmain")), vec![], color_formats, depth, None, sample_count, Topology::TriangleList, 0, 0, 0xf)
        .unwrap()
}
...
fn combined_sampler_set(
    ...
    let bindings = (0..images.len() as u32)
        .map(|b| LayoutBinding { binding: b, descriptor_type: vk_descriptor_type::COMBINED_IMAGE_SAMPLER, descriptor_count: 1, stage_flags: 0 })
        .collect();
...
fn color_clear(image: u64, clear: [f32; 4]) -> RenderingColorAttachment {
    RenderingColorAttachment { image, clear, load_clear: true, store: true }
}
...
fn deferred_shading_mrt_gbuffer_then_lighting_samples_three_targets() {
    ...
    record::cmd_begin_rendering(
        &mut d,
        cb1,
        &[color_clear(albedo, [0.0; 4]), color_clear(normal, [0.0; 4]), color_clear(position, [0.0; 4])],
        None,
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        vec![
            Enc::BeginRenderPass {
                color: vec![
                    ColorAttachment { texture: a_ir, load: LoadOp::Clear, clear: [0.0; 4], store: true },
                    ColorAttachment { texture: n_ir, load: LoadOp::Clear, clear: [0.0; 4], store: true },
                    ColorAttachment { texture: p_ir, load: LoadOp::Clear, clear: [0.0; 4], store: true },
                ],
                depth: None,
            },
            Enc::SetPipeline(d.pipelines.get(&geo).unwrap().ir_id),
            Enc::Draw { vertex_count: 36, instance_count: 1, first_vertex: 0, first_instance: 0 },
            Enc::EndRenderPass,
        ],
    ...
    let entries = combined_sampler_set(&mut d, &mut sink, cb2, &[albedo, normal, position], sampler);
    ...
    assert_eq!(
        entries,
        vec![
            hl_gpu::protocol::model::descriptor::BindEntry { binding: 0, resource: BindResource::Texture { id: a_ir } },
            hl_gpu::protocol::model::descriptor::BindEntry { binding: 16, resource: BindResource::Sampler { id: s } },
            hl_gpu::protocol::model::descriptor::BindEntry { binding: 1, resource: BindResource::Texture { id: n_ir } },
            hl_gpu::protocol::model::descriptor::BindEntry { binding: 17, resource: BindResource::Sampler { id: s } },
            hl_gpu::protocol::model::descriptor::BindEntry { binding: 2, resource: BindResource::Texture { id: p_ir } },
            hl_gpu::protocol::model::descriptor::BindEntry { binding: 18, resource: BindResource::Sampler { id: s } },
        ],
    ...
    assert_eq!(
        ...
        vec![
            Enc::BeginRenderPass {
                color: vec![ColorAttachment { texture: out_ir, load: LoadOp::Clear, clear: [0.0; 4], store: true }],
                depth: None,
            },
...
fn shadow_mapping_depth_only_pass_then_samples_depth_map() {
    ...
    let occluder = pipeline(&mut d, &mut sink, vec![], Some(DepthState::depth_only(TextureFormat::Depth32Float, true, compare::LESS)));
    ...
    record::cmd_begin_rendering(
        ...
        Some(RenderingDepthAttachment { image: shadow, clear_depth: 1.0, load_clear: true }),
    )
    .unwrap();
    ...
    assert_eq!(
        entries,
        vec![
            hl_gpu::protocol::model::descriptor::BindEntry { binding: 0, resource: BindResource::Texture { id: shadow_ir } },
            hl_gpu::protocol::model::descriptor::BindEntry { binding: 16, resource: BindResource::Sampler { id: s } },
        ],
        "the main pass must bind the DEPTH texture as a sampled shadow map"
    );

    record::cmd_begin_rendering(&mut d, cb2, &[color_clear(scene, [0.1, 0.1, 0.1, 1.0])], None).unwrap();
    ...
    assert!(
        enc2.contains(&Enc::BeginRenderPass {
            color: vec![ColorAttachment { texture: scene_ir, load: LoadOp::Clear, clear: [0.1, 0.1, 0.1, 1.0], store: true }],
            depth: None,
        }),
...
fn post_process_chain_ping_pongs_sampler_and_target() {
    ...
    assert_eq!(enc0[0], Enc::BeginRenderPass {
        color: vec![ColorAttachment { texture: a_ir, load: LoadOp::Clear, clear: [0.0; 4], store: true }],
        depth: None,
    });
    ...
    assert_eq!((s2, t2), (b_ir, c_ir), "stage 2 samples B and targets C (ping-pong advanced, no aliasing)");
    assert_ne!(s2, t2, "a ping-pong stage never samples its own render target");
}
...
fn msaa_resolve_pass_lowers_to_a_real_resolve() {
    ...
    let sample_counts: Vec<(u32, u32)> = sink
        ...
        .filter_map(|c| match c {
            Cmd::CreateTexture(id, desc) if *id == msaa_ir || *id == resolve_ir => Some((*id, desc.sample_count)),
            _ => None,
        })
        .collect();
    ...
    record::cmd_begin_rendering(&mut d, cb, &[color_clear(msaa, [0.2, 0.4, 0.6, 1.0])], None).unwrap();
...
fn render_to_layer_and_mip_is_a_documented_whole_texture_limit() {
    ...
    match sink.batches.iter().flatten().find(|c| matches!(c, Cmd::CreateTexture(id, _) if *id == tex_ir)) {
        Some(Cmd::CreateTexture(_, desc)) => {
            assert_eq!(desc.mip_levels, 1, "vkCreateImage models a single-mip image (no mip to render into)");
            assert_eq!(desc.depth, 1, "vkCreateImage models a single-layer 2D image (no array layer to render into)");
        }
    ...
    assert_eq!(
        ...
        vec![
            Enc::BeginRenderPass {
                color: vec![ColorAttachment { texture: tex_ir, load: LoadOp::Clear, clear: [0.0; 4], store: true }],
                depth: None,
            },
            Enc::EndRenderPass,
        ],
