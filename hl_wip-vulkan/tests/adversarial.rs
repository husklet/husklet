...
fn graphics_pipeline_rejects_missing_fragment_entry() {
    ...
    let r = create::create_graphics_pipeline(&mut d, &mut s, (vs, "vs"), Some((fs, "bad")), vec![], vec![TextureFormat::Rgba8Unorm]);
...
fn graphics_pipeline_with_no_color_targets_is_valid() {
    ...
    let pipe = create::create_graphics_pipeline(&mut d, &mut s, (vs, "vs"), None, Vec::<VertexLayout>::new(), vec![]).unwrap();
