...
fn acquire_round_robins_across_swapchain_images() {
    ...
    let mut d = dev();
    let mut sink = RecordingSink::with_full_caps();
    let surface = present::create_surface(&mut d, &mut sink, 64, 64, vk_format::B8G8R8A8_UNORM, 0).unwrap();
    ...
    let img_texs: Vec<u32> =
        d.swapchains.get(&sc).unwrap().images.iter().map(|i| i.ir_texture_id).collect();
    ...
    for _ in 0..ITERS {
        ...
        match sink.batches.last().unwrap().as_slice() {
            [Cmd::Present { texture: t, .. }] => {
                assert_eq!(*t, img_texs[idx as usize], "present targets the acquired image's texture")
            }
            other => panic!("expected Present, got {other:?}"),
        }
    }
    ...
    assert_eq!(acquired, vec![0, 1, 2, 0, 1, 2, 0], "acquire cycles through all {N} images in FIFO order");
    // Back in the pool after the loop: a fresh acquire continues the cycle rather than failing.
    assert_eq!(present::acquire_next_image(&mut d, sc).unwrap(), 1, "the cursor persists across the loop");
}
...
fn extended_dynamic_state_records_every_field_and_emits_no_ir() {
    ...
    record::set_dynamic(&mut d, cb, |ds| ds.extra_primitive_overestimation_size = 0.5).unwrap();
    ...
    assert!(rec.enc.is_empty(), "extended dynamic state emits no encoder op, got {:?}", rec.enc);
}
...
fn set_stencil_write_mask_selects_faces_by_mask() {
    ...
    record::end(&mut d, cb).unwrap();
    assert_eq!(d.command_buffers.get(&cb).unwrap().dynamic.stencil_write_mask, (0xAA, 0xBB));

    let cb2 = record::allocate_command_buffer(&mut d);
    record::begin(&mut d, cb2, false).unwrap();
    record::cmd_set_stencil_write_mask(&mut d, cb2, 0x3, 0xCC).unwrap(); // FRONT_AND_BACK
    record::end(&mut d, cb2).unwrap();
    assert_eq!(d.command_buffers.get(&cb2).unwrap().dynamic.stencil_write_mask, (0xCC, 0xCC));
}
...
fn dynamic_attachment_arrays_record_at_span_and_bound_check() {
    ...
    record::set_dynamic_attachment_array(&mut d, cb, 1, &[0x7, 0xF], |ds| &mut ds.color_write_masks).unwrap();
    // Write enables at [0,2).
    record::set_dynamic_attachment_array(&mut d, cb, 0, &[1, 0], |ds| &mut ds.color_write_enables).unwrap();
    {
        let ds = &d.command_buffers.get(&cb).unwrap().dynamic;
...
fn pipeline_layout_records_composed_set_layouts() {
    ...
    let sl0 = create::create_descriptor_set_layout(
        &mut d,
        vec![LayoutBinding { binding: 0, descriptor_type: vk_descriptor_type::STORAGE_BUFFER, descriptor_count: 1, stage_flags: 0x20 }],
    );
    let sl1 = create::create_descriptor_set_layout(
        &mut d,
        vec![LayoutBinding { binding: 0, descriptor_type: vk_descriptor_type::UNIFORM_BUFFER, descriptor_count: 1, stage_flags: 0x20 }],
    );
    let layout = create::create_pipeline_layout(&mut d, vec![sl0, sl1]);
    let rec = d.pipeline_layouts.get(&layout).expect("pipeline layout recorded");
    ...
    assert!(d.pipeline_layouts.get(&empty).unwrap().set_layouts.is_empty());
...
fn submit_side_timeline_signal_advances_counter_monotonically() {
    ...
    assert!(sync::semaphore_counter(&d, bin).is_err(), "binary semaphore has no counter");
