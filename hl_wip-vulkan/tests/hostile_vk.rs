...
fn blit_image_extent_overflow_is_out_of_bounds_then_valid() {
    let mut d = dev();
    let mut s = sink();
    let a = create::create_image(&mut d, &mut s, 8, 8, vk_format::R8G8B8A8_UNORM, vk_image_usage::TRANSFER_SRC).unwrap();
    let b = create::create_image(&mut d, &mut s, 8, 8, vk_format::R8G8B8A8_UNORM, vk_image_usage::TRANSFER_DST).unwrap();
    let cb = recording_cb(&mut d);
...
fn create_image_zero_oversized_extent_bad_format_and_usage() {
    ...
    assert!(matches!(create::create_image(&mut d, &mut s, 0, 4, vk_format::R8G8B8A8_UNORM, vk_image_usage::SAMPLED), Err(GpuError::Invalid(_))));
    assert!(matches!(create::create_image(&mut d, &mut s, 4, 0, vk_format::R8G8B8A8_UNORM, vk_image_usage::SAMPLED), Err(GpuError::Invalid(_))));
    ...
    assert!(matches!(create::create_image(&mut d, &mut s, big, 4, vk_format::R8G8B8A8_UNORM, vk_image_usage::SAMPLED), Err(GpuError::Invalid(_))));
    // A bad/unsupported VkFormat folds to Rgba8Unorm (documented bounded translation), no panic.
    let img = create::create_image(&mut d, &mut s, 4, 4, 0xDEAD_BEEF, vk_image_usage::SAMPLED).unwrap();
    ...
    let img2 = create::create_image(&mut d, &mut s, 4, 4, vk_format::R8G8B8A8_UNORM, 0xFFFF_FFFF).unwrap();
...
fn begin_render_pass_bad_attachments_reject_and_record_nothing() {
    let mut d = dev();
    let mut s = sink();
    let good = create::create_image(&mut d, &mut s, 8, 8, vk_format::R8G8B8A8_UNORM, vk_image_usage::COLOR_ATTACHMENT).unwrap();
