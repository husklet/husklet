...
fn allocate_memory_zero_over_budget_and_u64max_then_valid() {
    ...
    assert!(matches!(create::allocate_memory(&mut d, 0), Err(GpuError::Invalid(_))));
...
fn write_mapped_offset_overflow_is_out_of_bounds_then_valid() {
    ...
    assert!(matches!(create::write_mapped(&mut d, mem, u64::MAX, &[0u8; 4]), Err(GpuError::OutOfBounds)));
    assert!(matches!(create::write_mapped(&mut d, mem, u64::MAX - 2, &[0u8; 4]), Err(GpuError::OutOfBounds)));
    // A plainly out-of-range (non-overflowing) write is also OutOfBounds.
    assert!(matches!(create::write_mapped(&mut d, mem, 14, &[0u8; 4]), Err(GpuError::OutOfBounds)));
    // Unknown memory is a typed Invalid, never a panic.
    assert!(matches!(create::write_mapped(&mut d, 0xdead, 0, &[0u8; 4]), Err(GpuError::Invalid(_))));
...
fn copy_image_origin_overflow_is_out_of_bounds_then_valid() {
    ...
    assert!(matches!(record::cmd_copy_image(&mut d, cb, a, b, (u32::MAX, 0), (0, 0), (4, 4)), Err(GpuError::OutOfBounds)));
    assert!(matches!(record::cmd_copy_image(&mut d, cb, a, b, (0, 0), (u32::MAX, 0), (4, 4)), Err(GpuError::OutOfBounds)));
    assert!(matches!(record::cmd_copy_image(&mut d, cb, a, b, (0, u32::MAX), (0, 0), (4, 4)), Err(GpuError::OutOfBounds)));
...
fn blit_image_extent_overflow_is_out_of_bounds_then_valid() {
    let mut d = dev();
    let mut s = sink();
    let a = create::create_image(&mut d, &mut s, 8, 8, vk_format::R8G8B8A8_UNORM, vk_image_usage::TRANSFER_SRC, 1).unwrap();
    let b = create::create_image(&mut d, &mut s, 8, 8, vk_format::R8G8B8A8_UNORM, vk_image_usage::TRANSFER_DST, 1).unwrap();
    let cb = recording_cb(&mut d);
    // Origin/extent near `u32::MAX` previously overflow-panicked the src/dst bounds checks.
    assert!(matches!(record::cmd_blit_image(&mut d, cb, a, b, (u32::MAX, 0), (4, 4), (0, 0), (4, 4), false), Err(GpuError::OutOfBounds)));
    assert!(matches!(record::cmd_blit_image(&mut d, cb, a, b, (0, 0), (4, 4), (0, u32::MAX), (4, 4), true), Err(GpuError::OutOfBounds)));
    assert!(matches!(record::cmd_blit_image(&mut d, cb, a, b, (4, 4), (u32::MAX, 1), (0, 0), (4, 4), false), Err(GpuError::OutOfBounds)));
...
fn push_constants_out_of_range_rejected_then_valid() {
    ...
    assert!(matches!(record::cmd_push_constants(&mut d, cb, 0xFFFF_FFF0, &[0u8; 16]), Err(GpuError::Invalid(_))));
    assert!(matches!(record::cmd_push_constants(&mut d, cb, 0, &[0u8; 8192]), Err(GpuError::Invalid(_))));
    assert!(matches!(record::cmd_push_constants(&mut d, cb, 4096, &[0u8; 4]), Err(GpuError::Invalid(_))));
    // A valid push within the range still records at its offset.
    record::cmd_push_constants(&mut d, cb, 0, &[7u8; 128]).unwrap();
    assert_eq!(d.command_buffers.get(&cb).unwrap().push_constants.len(), 128);
}
...
fn dynamic_attachment_array_out_of_range_rejected_then_valid() {
    ...
    let r = record::set_dynamic_attachment_array(&mut d, cb, u32::MAX, &[1u32], |ds| &mut ds.color_blend_enables);
    assert!(matches!(r, Err(GpuError::Invalid(_))));
    let r2 = record::set_dynamic_attachment_array(&mut d, cb, 4, &[1u32; 8], |ds| &mut ds.color_write_masks);
    assert!(matches!(r2, Err(GpuError::Invalid(_))));
    // A valid attachment range (within maxColorAttachments == 8) still records.
    record::set_dynamic_attachment_array(&mut d, cb, 0, &[1, 0], |ds| &mut ds.color_blend_enables).unwrap();
    assert_eq!(d.command_buffers.get(&cb).unwrap().dynamic.color_blend_enables, vec![1, 0]);
}
...
fn copy_query_pool_results_hostile_stride_is_out_of_bounds_then_valid() {
    ...
    assert!(matches!(record::cmd_copy_query_pool_results(&mut d, cb, pool, 0, 4, dst, 0, u64::MAX, false, false), Err(GpuError::OutOfBounds)));
    assert!(matches!(record::cmd_copy_query_pool_results(&mut d, cb, pool, 0, 2, dst, 0, u64::MAX / 2, true, true), Err(GpuError::OutOfBounds)));
...
fn get_query_pool_results_hostile_stride_does_not_panic_then_valid() {
    ...
    let _ = sync::get_query_pool_results(&d, pool, 0, 4, &mut out, u64::MAX, false, true, false, false).unwrap();
    let _ = sync::get_query_pool_results(&d, pool, 0, 3, &mut out, u64::MAX / 2, true, false, true, false).unwrap();
    // A valid readback (stride 8, availability) still succeeds.
    let mut ok = [0u8; 32];
    let _ = sync::get_query_pool_results(&d, pool, 0, 2, &mut ok, 8, false, true, true, false).unwrap();
}
...
fn create_image_zero_oversized_extent_bad_format_and_usage() {
    ...
    assert!(matches!(create::create_image(&mut d, &mut s, 0, 4, vk_format::R8G8B8A8_UNORM, vk_image_usage::SAMPLED, 1), Err(GpuError::Invalid(_))));
    assert!(matches!(create::create_image(&mut d, &mut s, 4, 0, vk_format::R8G8B8A8_UNORM, vk_image_usage::SAMPLED, 1), Err(GpuError::Invalid(_))));
    ...
    assert!(matches!(create::create_image(&mut d, &mut s, big, 4, vk_format::R8G8B8A8_UNORM, vk_image_usage::SAMPLED, 1), Err(GpuError::Invalid(_))));
    // A bad/unsupported VkFormat folds to Rgba8Unorm (documented bounded translation), no panic.
    let img = create::create_image(&mut d, &mut s, 4, 4, 0xDEAD_BEEF, vk_image_usage::SAMPLED, 1).unwrap();
    assert_eq!(d.images.get(&img).unwrap().format, TextureFormat::Rgba8Unorm);
    // Garbage usage bits: unknown bits are ignored (known bits translated), no panic.
    let img2 = create::create_image(&mut d, &mut s, 4, 4, vk_format::R8G8B8A8_UNORM, 0xFFFF_FFFF, 1).unwrap();
...
fn begin_render_pass_bad_attachments_reject_and_record_nothing() {
    let mut d = dev();
    let mut s = sink();
    let good = create::create_image(&mut d, &mut s, 8, 8, vk_format::R8G8B8A8_UNORM, vk_image_usage::COLOR_ATTACHMENT, 1).unwrap();
    // Classic path: an unknown color image is rejected up front.
    let cb = recording_cb(&mut d);
    assert!(matches!(record::cmd_begin_render_pass(&mut d, cb, 0xdead, [0.0; 4], true, None), Err(GpuError::Invalid(_))));
    ...
    let depth = record::RenderingDepthAttachment { image: 0xdead, clear_depth: 1.0, load_clear: true };
    assert!(matches!(record::cmd_begin_render_pass(&mut d, cb, good, [0.0; 4], true, Some(depth)), Err(GpuError::Invalid(_))));
    ...
    let mix = [
        record::RenderingColorAttachment { image: good, clear: [0.0; 4], load_clear: true, store: true },
        record::RenderingColorAttachment { image: 0xdead, clear: [0.0; 4], load_clear: true, store: true },
    ];
    assert!(matches!(record::cmd_begin_rendering(&mut d, cb, &mix, None), Err(GpuError::Invalid(_))));
    assert!(d.command_buffers.get(&cb).unwrap().enc.is_empty());
...
fn dangling_handles_across_entrypoints_are_typed_errors_or_safe_noops() {
    ...
    assert!(matches!(record::cmd_bind_pipeline(&mut d, cb, 0xdead), Err(GpuError::Invalid(_))));
    assert!(matches!(record::cmd_bind_index_buffer(&mut d, cb, 0xdead, 0, 1), Err(GpuError::Invalid(_))));
    assert!(matches!(record::cmd_copy_buffer(&mut d, cb, 0xdead, 0xbeef, 0, 0, 4), Err(GpuError::Invalid(_))));
    ...
    assert!(matches!(present::get_swapchain_images(&d, 0xdead), Err(GpuError::Invalid(_))));
...
fn out_of_range_indices_and_offsets_do_not_panic_then_valid() {
    let mut d = dev();
    let mut s = sink();
    let buf = create::create_buffer(&mut d, &mut s, vk_buffer_usage::VERTEX_BUFFER | vk_buffer_usage::INDEX_BUFFER, 16).unwrap();
    let ir = buf_ir(&d, buf);
    let cb = recording_cb(&mut d);
    ...
    assert_eq!(d.descriptor_sets.get(&set).unwrap().buffers.get(&u32::MAX), Some(&(buf, 0, 16)));
    // A valid, in-range bind still records against the real ir id.
    record::cmd_bind_vertex_buffer(&mut d, cb, 1, buf, 0).unwrap();
    use hl_gpu::protocol::model::command::Enc;
    assert!(d.command_buffers.get(&cb).unwrap().enc.iter().any(|e| matches!(e, Enc::SetVertexBuffer { buffer, offset: 0, .. } if *buffer == ir)));
}
...
fn double_free_and_use_after_free_survive() {
    ...
    assert!(matches!(sync::set_event(&mut d, e, true), Err(GpuError::Invalid(_))));
    ...
    create::destroy_descriptor_update_template(&mut d, t);
    assert!(matches!(create::update_descriptor_set_with_template(&mut d, 0, t, &[]), Err(GpuError::Invalid(_))));
...
fn descriptor_type_mismatch_writes_do_not_panic_then_valid() {
    ...
    let layout = create::create_descriptor_set_layout(
        &mut d,
        vec![LayoutBinding { binding: 0, descriptor_type: vk_descriptor_type::COMBINED_IMAGE_SAMPLER, descriptor_count: 1, stage_flags: 0 }],
    );
    let pool = create::create_descriptor_pool(&mut d, 1);
    let set = create::allocate_descriptor_set(&mut d, pool, layout, 0).unwrap();
