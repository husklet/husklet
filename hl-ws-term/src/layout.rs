            ...
            Node::Split { dir, ratio, a, b } => {
                ...
                match dir {
                    Dir::Vertical => {
                        ...
                        let wa = avail * r;
                        let wb = avail - wa;
                        let ra = Rect { x: rect.x, y: rect.y, w: wa, h: rect.h };
                        let rb = Rect { x: rect.x + wa + gap, y: rect.y, w: wb, h: rect.h };
                        a.rects(ra, gap, out);
                        b.rects(rb, gap, out);
                    }
                    Dir::Horizontal => {
                        ...
                        let ha = avail * r;
                        let hb = avail - ha;
                        let ra = Rect { x: rect.x, y: rect.y, w: rect.w, h: ha };
                        let rb = Rect { x: rect.x, y: rect.y + ha + gap, w: rect.w, h: hb };
                        a.rects(ra, gap, out);
                        b.rects(rb, gap, out);
                    }
                }
...
mod tests {
    ...
    const AREA: Rect = Rect { x: 0.0, y: 0.0, w: 100.0, h: 80.0 };
