...
use dd_term_core::session::{cwd_from_uri, Pane, PaneNode, Session, SessionTab};
use dd_term_core::{Attrs, Vt};
...
fn osc7_percent_encoded_utf8_path() {
    assert_eq!(cwd_from_uri("file://host/tmp/%C3%A9").as_deref(), Some("/tmp/é"));
    assert_eq!(cwd_from_uri("file:///a/%E4%B8%96%E7%95%8C").as_deref(), Some("/a/世界"));
}
