...
pub fn group() -> ScenGroup {
    sgroup("execcmd", vec![
        ...
        s("execcmd/basic").host(r#"
docker run -d --name ${C}c $PLAT $IMG sleep 60 >/dev/null; sleep 0.3
docker exec ${C}c echo EXECOK"#).has("EXECOK"),

        // exec -e ENV
        s("execcmd/env-e").host(r#"
docker run -d --name ${C}c $PLAT $IMG sleep 60 >/dev/null; sleep 0.3
docker exec -e XX=yyval ${C}c printenv XX"#).has("yyval"),

        // exec -w WORKDIR
        s("execcmd/workdir-w").host(r#"
docker run -d --name ${C}c $PLAT $IMG sleep 60 >/dev/null; sleep 0.3
docker exec -w /etc ${C}c pwd"#).has("/etc"),

        // exec -u USER
        s("execcmd/user-u").host(r#"
docker run -d --name ${C}c $PLAT $IMG sleep 60 >/dev/null; sleep 0.3
docker exec -u 1000 ${C}c id -u"#).has("1000"),

        // exec -d detached; side effect is visible on a later exec
        s("execcmd/detached-d").host(r#"
docker run -d --name ${C}c $PLAT $IMG sleep 60 >/dev/null; sleep 0.3
docker exec -d ${C}c sh -c "echo DETACHEDWROTE > /tmp/d"; sleep 0.5
docker exec ${C}c cat /tmp/d"#).has("DETACHEDWROTE"),

        // exec exit-code propagation to the client
        s("execcmd/exit-code").host(r#"
docker run -d --name ${C}c $PLAT $IMG sleep 60 >/dev/null; sleep 0.3
docker exec ${C}c sh -c "exit 9"; echo RC=$?"#).has("RC=9"),

        // exec -i stdin piped in
        s("execcmd/stdin-i").host(r#"
docker run -d --name ${C}c $PLAT $IMG sleep 60 >/dev/null; sleep 0.3
echo INPUTLINE | docker exec -i ${C}c cat"#).has("INPUTLINE"),
    ])
}
