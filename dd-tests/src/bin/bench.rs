//! dd microbenchmark — TRUE DBT execution overhead, with process/VM startup EXCLUDED (task #326).
//!
...
//!
//! This is a SEPARATE target: it does not touch `make test` or the #309 `make perf` table.
...
fn parse_ns(stdout: &str, kernel: &str) -> Option<f64> {
    for line in stdout.lines() {
        ...
        if it.next() == Some("KERNEL") && it.next() == Some(kernel) {
            if let Some(n) = it.next() { return n.parse::<f64>().ok(); }
        }
    }
    None
}
...
fn run_once(prog: &str, args: &[String], kernel: &str) -> Option<f64> {
    ...
    if ns.is_none() {
        eprintln!("[bench]   {kernel}: no output from `{prog}` (code {:?}) — stderr: {}",
...
fn compile(cc: &str, src: &Path, out: &Path) -> Result<(), String> {
    ...
    if !o.status.success() {
        return Err(format!("{cc} {}: {}", src.display(), String::from_utf8_lossy(&o.stderr).trim()));
    }
    Ok(())
}
...
fn ensure_ddjit_dir(repo: &Path) {
    if std::env::var_os("DDJIT_DIR").is_some() { return; }
    ...
    if let Ok(rd) = std::fs::read_dir(&build) {
        for e in rd.flatten() {
            ...
            if let Ok(md) = std::fs::metadata(&eng) {
                ...
                if best.as_ref().map_or(true, |(bt, _)| t > *bt) { best = Some((t, out)); }
            }
        }
    }
...
fn main() {
    let n: usize = std::env::var("BENCH_N").ok().and_then(|s| s.parse().ok()).unwrap_or(3);
    let only: Option<Vec<String>> = std::env::var("BENCH_K").ok()
        .map(|s| s.split(',').map(|x| x.trim().to_string()).filter(|x| !x.is_empty()).collect());
    let kernels: Vec<&str> = KERNELS.iter().cloned()
        .filter(|k| only.as_ref().map_or(true, |o| o.iter().any(|x| x == k))).collect();
    ...
    eprintln!("[bench] compiling guest (aarch64 + x86_64) → {}", bench_dir.display());
    if let Err(e) = compile("gcc", &src, &g_arm) { eprintln!("[bench] FATAL: {e}"); std::process::exit(1); }
    if let Err(e) = compile("x86_64-linux-gnu-gcc", &src, &g_x86) { eprintln!("[bench] FATAL: {e}"); std::process::exit(1); }
    let (g_arm, g_x86) = (g_arm.to_string_lossy().into_owned(), g_x86.to_string_lossy().into_owned());
    ...
    let mut nat = Lane { name: "native-arm64", ns: vec![] };
    let mut dda = Lane { name: "dd-arm64",     ns: vec![] };
    let mut ddx = Lane { name: "dd-x86",       ns: vec![] };
    let mut qem = Lane { name: "qemu-x86",     ns: vec![] };
    ...
    for k in &kernels {
        ...
        qem.ns.push(measure("qemu-x86_64", &[g_x86.clone(), k.to_string()], k, n));
    }

    // ── render: a row per kernel, sorted by dd-x86 slowdown (×native) descending ──
    let ratio = |lane: &Lane, i: usize| -> Option<f64> {
        match (lane.ns[i], nat.ns[i]) { (Some(v), Some(b)) if b > 0.0 => Some(v / b), _ => None }
    };
    ...
    let fmt_x = |o: Option<f64>| o.map(|v| format!("{v:.2}×")).unwrap_or_else(|| "n/a".into());
    ...
    println!("  {:<9} {:>14} {:>12} {:>12} {:>12}", "Kernel", "native-arm64", "dd-arm64", "dd-x86", "qemu-x86");
    println!("  {:<9} {:>14} {:>12} {:>12} {:>12}", "",       "(ms)",         "(×nat)",   "(×nat)", "(×nat)");
    println!("  {}", "─".repeat(63));
    for &i in &order {
        println!("  {:<9} {:>14} {:>12} {:>12} {:>12}",
            kernels[i], fmt_ms(ms(nat.ns[i])), fmt_x(ratio(&dda, i)), fmt_x(ratio(&ddx, i)), fmt_x(ratio(&qem, i)));
    }
    println!();
    ...
    let mut csv = String::from("kernel,native_arm64_ms,dd_arm64_x,dd_x86_x,qemu_x86_x,dd_arm64_ms,dd_x86_ms,qemu_x86_ms\n");
    ...
    for &i in &order {
        csv += &format!("{},{},{},{},{},{},{},{}\n",
            kernels[i], cell_ms(nat.ns[i]), cell_x(ratio(&dda, i)), cell_x(ratio(&ddx, i)), cell_x(ratio(&qem, i)),
            cell_ms(dda.ns[i]), cell_ms(ddx.ns[i]), cell_ms(qem.ns[i]));
    }
    std::fs::write(&csv_path, &csv).ok();

    let jn = |o: Option<f64>| o.map(|v| format!("{v:.6}")).unwrap_or_else(|| "null".into());
