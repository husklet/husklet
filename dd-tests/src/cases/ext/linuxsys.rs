...
fn miscx() -> Group {
    group("lsys-misc", vec![
        ...
        // (#271). A fake setup that succeeds but never completes SQEs would hang every real guest that then
        ...
        // an accepted-gap artifact on aarch64 (x86's qemu oracle also ENOSYS -> matches). xfail'd (#271/#220).
        ...
        // ENOSYS (eperm=0), so the x86 differential is an oracle artifact — xfail'd (cf. process-vm). #220.
        src("fanotify", "ext_linuxsys/fanotify.c").oracle().xfail(&[Engine::LinuxX86_64]),
