...
fn ext_threads() -> Group {
    group(
        ...
        vec![
            ...
            port("attr-stack", "ext_threads/th_attr_stack.c")
                .out("attr_stack size_ok=1 result=501500\n"),
            port("sched", "ext_threads/th_sched.c")
                .out("sched setpol=1 created=1 yield=1 range_ok=1\n"),
            // ---- mutexes ----
            port("mutex-recursive", "ext_threads/th_mutex_recursive.c")
                .out("recursive depth=100 relock=50\n"),
            port("mutex-errorcheck", "ext_threads/th_mutex_errorcheck.c")
                .out("errorcheck edeadlk=1 eperm=1\n"),
            port("mutex-trylock", "ext_threads/th_mutex_trylock.c")
                .out("trylock ok=1 busy=1 again=1\n"),
            port("mutex-stress", "ext_threads/th_mutex_stress.c")
                .out("mutex_stress shared=1600000\n"),
            // ---- condvars ----
            port("condvar-broadcast", "ext_threads/th_condvar_broadcast.c")
                .out("broadcast awake=16\n"),
            ...
            port("condvar-timedwait", "ext_threads/th_condvar_timedwait.c")
                .out("timedwait timeout=1\n"),
            port("condvar-signal-wins", "ext_threads/th_cond_signal_wins.c")
                .out("signal_wins rc=0 ready=1\n"),
            // ---- rwlock ----
            ...
            // ---- atomics ----
            port("atomics-ops", "ext_threads/th_atomics_ops.c")
                .out("atomics v=249 cas_ok=1 cas_fail=1 old=999 final=7\n"),
            port("atomics-orders", "ext_threads/th_atomics_orders.c")
                .out("orders relaxed=320000 acqrel=320000 seqcst=320000\n"),
