//! pcachex — persistent cross-process translated-code cache (#339 aarch64 + opt8 x86_64), matrix lane.
...
pub fn groups() -> Vec<Group> { vec![pcachex()] }

fn pc(c: Case) -> Case {
    c.env("DDJIT_PCACHE", "1").env("DDJIT_PCACHE_DIR", "/tmp/ddjit-pcache-matrix")
}
...
fn pcachex() -> Group {
    group("pcachex", vec![
        ...
        pc(src("pc-forkpipe", "pcachex/forkpipe.c").out("pcache forkpipe r=8 h=5807372f99a952f5 exit=0\n")),
        ...
        pc(src("pc-selfexec", "pcachex/selfexec.c").out("pcache selfexec reaped=6 sum=75 bad=0\n")),
        // fork child that OUTLIVES the parent without execve — the exact #339 crash repro: pre-fix, the
