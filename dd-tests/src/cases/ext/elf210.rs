...
//!
//! Under DDJIT_PCACHE the x86_64 loader (translate/x86_64/elf.c load_elf) maps the guest image at a FIXED
...
//!
//! DDX_FORCE_BASE_COLLIDE=1 (a load_elf test hook, gated + inert otherwise) deterministically forces that
//! collision on every fixed-VA load — the geometry never triggers it naturally (the two fixed bases sit
//! 512GB apart). We run the guest under DDJIT_PCACHE=1 + the collision hook and assert byte-exactness vs the
...
fn elf210() -> Group {
    group(
        ...
        vec![src("elf210-basecollide", "elf210.c")
            ...
            .env("DDJIT_PCACHE", "1")
            .env("DDJIT_PCACHE_DIR", "/tmp/ddjit-pcache-elf210")
            .env("DDX_FORCE_BASE_COLLIDE", "1")
            .oracle()],
    )
}
