// guests/bench/ctxbench.c -- SELF-TIMED indirect-dispatch microbench (perf lever #4 follow-up).
//
// Same two dispatch shapes as guests/ibtc_dispatch.c (which is the GOLDEN-checked matrix case);
// this variant self-times each phase with CLOCK_MONOTONIC (startup excluded, same recipe as
// guests/bench/microbench.c) so the harness can compute ns/dispatch for:
//   vm   the 128-target megamorphic computed-goto bytecode VM (CPython/VDBE shape), 200M dispatches
//   rec  monomorphic deep recursion fib(37) (~78M real call/ret pairs)
// Output: "KERNEL vm <ns>" / "KERNEL rec <ns>" + a deterministic CHK line (corruption guard).
#include <stdio.h>
#include <stdint.h>
#include <time.h>
...
#define DISPATCHES 200000000ull
...
vmdone:;
    ...
    uint64_t t2 = now_ns();
    uint64_t rec = fib(37); // 78,176,337 calls
    uint64_t t3 = now_ns();

    uint64_t chk = acc ^ (rec * 1099511628211ull);
    printf("KERNEL vm %llu\n", (unsigned long long)(t1 - t0));
    printf("KERNEL rec %llu\n", (unsigned long long)(t3 - t2));
    printf("NSPD vm %.3f rec %.3f\n", (double)(t1 - t0) / (double)DISPATCHES,
           (double)(t3 - t2) / 78176337.0);
    printf("CHK vm=%llu rec=%llu chk=%llu\n", (unsigned long long)acc, (unsigned long long)rec,
