// #425: models V8's embedded-builtins code-range invariant in a non-PIE ET_EXEC. dd maps the image HIGH
...
// SAME address-space half. qemu -> both low; dd fixed -> both high; dd unfixed -> base low, ret high (0).
...
int main(void) {
    capture(); // g_ret = a return address into main -- where code actually executes (HIGH under dd)
